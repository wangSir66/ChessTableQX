...
/****************************************    刷新玩家手牌   *******************************************************/
majiang_panel_huangShiHHMJ.prototype.updatePlayerCards = function (node) {
    if(!node) {
        return;
    }
...
/****************************************     3D相关   *******************************************************/
majiang_panel_huangShiHHMJ.prototype.getCardSortArray3D = function (node, newCardTag) {
    var tData = MjClient.data.sData.tData;
...
majiang_panel_huangShiHHMJ.prototype.updateDownEatAndHandCards3D = function (cardArray) {
    var tData = MjClient.data.sData.tData;
...
majiang_panel_huangShiHHMJ.prototype.updateRightEatAndHandCards3D = function (cardArray) {
    var playerNode = cardArray[0].getParent();
    var templetEatCard = playerNode.getChildByName(this.CsdDefaultCardType.EatCardFront);
    var templetHandCard = playerNode.getChildByName(this.CsdDefaultCardType.HandCard);
    var player = this.getPlayerInfoByName(playerNode.getName());
...
majiang_panel_huangShiHHMJ.prototype.updateTopEatAndHandCards3D = function (cardArray) {
    var playerNode = cardArray[0].getParent();
    var templetEatCard = playerNode.getChildByName(this.CsdDefaultCardType.EatCardFront);
    var player = this.getPlayerInfoByName(playerNode.getName());
...
majiang_panel_huangShiHHMJ.prototype.updateLeftEatAndHandCards3D = function (cardArray) {
    var playerNode = cardArray[0].getParent();
    var templetEatCard = playerNode.getChildByName(this.CsdDefaultCardType.EatCardFront);
    var templetHandCard = playerNode.getChildByName(this.CsdDefaultCardType.HandCard);
    var player = this.getPlayerInfoByName(playerNode.getName());
...
/****************************************     2D相关   *******************************************************/
majiang_panel_huangShiHHMJ.prototype.getCardSortArray2D = function(node, newCardTag){
    var tData = MjClient.data.sData.tData;
...
majiang_panel_huangShiHHMJ.prototype.updateCardPosition2D = function(node, sortArr) {
 ...
 *  根据牌的类型获得牌与牌之间的距离
 **/
majiang_panel_huangShiHHMJ.prototype.getMoveDistance2D = function(node, cardArr, index, chiFirstIndex){
    if(index == 0){
        return 0;
    }
    var handCard = node.getChildByName("img_handCard");
...
majiang_panel_huangShiHHMJ.prototype.switchUpdate2D = function(node, sortArr, pos, index, chiFirstIndex){
...
majiang_panel_huangShiHHMJ.prototype.setHandPosition2D = function(node, cardArr, pos, index) {
    var cardNode = cardArr[index];
...
majiang_panel_huangShiHHMJ.prototype.setLiangPosition2D = function(node, cardArr, pos, index, chiFirstIndex){
    var cardNode = cardArr[index];
    cardNode.setPosition(pos);
};

majiang_panel_huangShiHHMJ.prototype.setGDGangCardPosition2D = function(node, sortArr, pos, index) {
 ...
 *  处理麻将吃、碰、杠
 **/
majiang_panel_huangShiHHMJ.prototype.handleCommand = function(playerNode, msg, type){
    var tData = MjClient.data.sData.tData;
    var player = this.getPlayerInfoByName(playerNode.getName());
 ...
 *  处理麻将亮牌
 **/
majiang_panel_huangShiHHMJ.prototype.dealLiang = function(playerNode, msg) {
 ...
 *  处理麻将红中发财杠牌
 **/
majiang_panel_huangShiHHMJ.prototype.dealGDGang = function(playerNode, msg) {
    var tData = MjClient.data.sData.tData;
    var player = this.getPlayerInfoByName(playerNode.getName());
    if(msg && msg.uid && msg.uid != player.info.uid){
        return;
 ...
 *  刷新癞子牌是否可打出状态
 **/
majiang_panel_huangShiHHMJ.prototype.updateCardColorIsHunCard = function () {
    var tData = MjClient.data.sData.tData;
    // 计算生牌和熟牌
    ...
    // 是否可打出
    var isHandle = false;
    if (hunArr.length > 0 && noHunArr.length == 0) {
        isHandle = true;
    }
    // 添加癞子牌的不可打出状态
    ...
    for (let i = 0; i < children.length; i++) {
        ...
        if (cardNode.name === this.HandleCardType.Hand && cardNode.tag == tData.hunCard) {
            cardNode.isGray = isHandle;
            // cardNode.setColor(color);
            // cardNode.addTouchEventListener(function () {});
            cardNode.setTouchEnabled(isHandle);
