function addWxHeadToEndUI_ningXiangPaoHuZi_GO(node, off) {
...
function SetGameOverLayer_ningXiangPaoHuZi(node,off,isNewUi){
    ...
    addWxHeadToEndUI_ningXiangPaoHuZi_GO(wxNode,off);
    ...
    var showRoomStatistics = function(node,showList,data){
        ...
        var showStatistics = {"huNum":0,"ziNum":1,"jiePaoNum":2,"tiNum":3,"paoNum":4,"winall":5};
    ...
    textNode.setScrollBarEnabled(false);
    var name = textNode.getChildByName("name");
    var allCountNode = [name];
    name.getChildByName("num").setString(pl.roomStatistics[0]);
    var nameClone = name.clone();
    nameClone.getChildByName("num").setString(pl.roomStatistics[1]);
    nameClone.setString("自摸次数");
    textNode.addChild(nameClone);
    allCountNode.push(nameClone);
    ...
    for (var index in pl.mingTang) {
       if (table[index]){
           nameClone = name.clone();
           nameClone.getChildByName("num").setString(pl.mingTang[index]);
           nameClone.setString(table[index]);
           textNode.addChild(nameClone);
           allCountNode.push(nameClone);
       }
    }

    var h = allCountNode.length * 40;
    var size = textNode.getContentSize();
    for (var i = allCountNode.length - 1; i >= 0; i--){
        allCountNode[i].y = (allCountNode.length - 1 - i) * 40 + 4;
    }

    if (h < size.height){
        textNode.setContentSize(cc.size(size.width, h));
    }
    textNode.setInnerContainerSize(cc.size(size.width, h));
}

var GameOverLayer_ningXiangPaoHuZi = cc.Layer.extend({
    sprite:null,
    jsBind:{
        back:{
            close :{
    ...
    ctor:function () {
        this._super();
        var endallui = ccs.load("endAll_NingXiangPaoHuZi.json");
        ...
        for(var i = 0 ; i < playerCount ; i++){
            ...
            SetGameOverLayer_ningXiangPaoHuZi(clone,i, false);
        ...
        information.setSize(cc.size(370, 120));
