MjClient.MaxPlayerNum_ningXiangPaoHuZi = 3;
MjClient.cardPath_ningXiangPaoHuZi = "playing/paohuzi/";
var ActionType_ningXiangPaoHuZi =  //图片提示
    {
        CHI:1,
        PENG:2,
        WEI:3,
...
//初始化玩家金币、名字、胡息
function InitUserCoinAndName_ningXiangPaoHuZi(node, off){
...
//向服务器发送王闯王钓操作
function HZWangChuangToServer_ningXiangPaoHuZi(type){
    cc.log("====================HZWangChuangToServer_ningXiangPaoHuZi=================");
...
//向服务器发送打牌操作
function HZPutCardToServer_ningXiangPaoHuZi(card){
    cc.log("====================HZPutCardToServer_ningXiangPaoHuZi=================");
    MjClient.gamenet.request("pkroom.handler.tableMsg", {
        cmd: "MJPut",
        card: card
    });
...
// 向服务器发送吃牌
function HZChiToServer_ningXiangPaoHuZi(eatCards, biCards){
...
//像服务器发送碰牌
function HZPengToServer_ningXiangPaoHuZi()
{
    if (MjClient.rePlayVideo != -1) return; // 回放时候不能请求
    cc.log("====================HZPengToServer_ningXiangPaoHuZi=================");
    MjClient.gamenet.request("pkroom.handler.tableMsg", {
        cmd: "MJPeng"
    });
}

function MJPass2Net_ningXiangPaoHuZi(){
    cc.log("====================send======pass=====");
    ...
    if(IsTurnToMe() && tData.tState == TableState.waitPut){
        ...
        HZPassConfirmToServer_ningXiangPaoHuZi();
    }
};

// 向服务器发送过牌
function HZPassConfirmToServer_ningXiangPaoHuZi() {
    if (MjClient.rePlayVideo != -1)  {
        return; // 回放时候不能请求
    }
    ...
    MjClient.gamenet.request("pkroom.handler.tableMsg", {
        cmd: "MJPass",
        eatFlag: EatFlag_ningXiangPaoHuZi(),
        cardNext: tData.cardNext,
        card: tData.lastPutCard
    });
}
...
//发送跑牌命令
function HZGangToServer_ningXiangPaoHuZi(type){
    if (MjClient.rePlayVideo != -1) return; // 回放时候不能请求
    cc.log("====================HZGangToServer_ningXiangPaoHuZi=================");
    MjClient.gamenet.request("pkroom.handler.tableMsg", {
        cmd: "MJGang",
        type: type,
        eatFlag: EatFlag_ningXiangPaoHuZi()
...
//发送偎牌命令
function HZWeiToServer_ningXiangPaoHuZi(type){
    if (MjClient.rePlayVideo != -1) return; // 回放时候不能请求
    cc.log("====================HZWeiToServer_ningXiangPaoHuZi=================");
    MjClient.gamenet.request("pkroom.handler.tableMsg", {
        cmd: "HZWeiCard"
    });
}
...
//向服务器发送发牌命令
function HZNewCardToServer_ningXiangPaoHuZi(){
    if (MjClient.rePlayVideo != -1) return; // 回放时候不能请求
    cc.log("====================HZNewCardToServer_ningXiangPaoHuZi=================");
    MjClient.gamenet.request("pkroom.handler.tableMsg", {
        cmd: "HZNewCard"
    });
}
...
//向服务器发送胡牌命令
function MJHuToServer_ningXiangPaoHuZi(){
    if (MjClient.rePlayVideo != -1) return; // 回放时候不能请求
    cc.log("====================MJHuToServer=================");
    MjClient.gamenet.request("pkroom.handler.tableMsg", {
        cmd: "MJHu",
...
//添加吃的牌
function addEatCard_ningXiangPaoHuZi(node,name,mjNum,off){
    //根据牌的类型获得需要添加的节点
    ...
    var type = 2;
    //设置牌
    var newCard = getNewCard_ningXiangPaoHuZi(mjNum,type,off);
    ...
    //
    // if(MjClient.MaxPlayerNum_ningXiangPaoHuZi == 2 &&
    //     (node.getName() == "left")) {
    //     newCard.y -= 10;
    // }
...
/**
 添加手牌(回放)
 */
function addHandCardReplay_ningXiangPaoHuZi(tagName,index,mjNum,off){
    var _node = getNode_ningXiangPaoHuZi(off);
    //根据牌的类型获得需要添加的节点
...
function cleanCardNode_ningXiangPaoHuZi(off) {
    //获取结点
    if(off != 0){
        return;
    }
...
function resetCardNode_ningXiangPaoHuZi(off) {
    if(off != 0){
        return;
    }
    var _node = getNode_ningXiangPaoHuZi(off);
...
function addOrAdjustHandCard_ningXiangPaoHuZi(tagName,index,mjNum,off){
    if(off != 0){
        return;
    }
    var _node = getNode_ningXiangPaoHuZi(off);
    ...
    if(!newCard){
        cc.log("chow", "newCard");
        newCard = getNewCard_ningXiangPaoHuZi(mjNum,1,off);
    }else{
        cc.log("chow", "getCard");
    }
    var scale_y = newCard.scaleY;
    ...
    if (MjClient.movingCard_paohuzi == newCard) {
        setCardSprite_ningXiangPaoHuZi(newCard, newCard.tag, 1, false);
    ...
    if(newCard.lastPosition){
        ...
        doMovetoAction_ningXiangPaoHuZi(newCard, cc.p(beginPoint.x, beginPoint.y + cardCount * off_y));
    }else{
        newCard.x = beginPoint.x;
        newCard.y = beginPoint.y + cardCount * off_y;
    }
    //newCard.opacity = 255;
    SetTouchCardHandler_ningXiangPaoHuZi(newCard,off);
    ...
    //cc.log("chow", "addOrAdjustHandCard_ningXiangPaoHuZi: name=" + newCard.getName() + " num=" + newCard.num + " tag =" + newCard.tag);
}

function doMovetoAction_ningXiangPaoHuZi(node,endP){
    node.stopAllActions();
    var action = cc.moveTo(0.15,endP);
    node.runAction(action);
};

function resetHandCard_ningXiangPaoHuZi(posNode,off, needAction, isDelay){
    if(MjClient.rePlayVideo == -1){
        if(off == 0) {
            var handNode = posNode.getChildByName("handNode");
            handNode.visible = true;
            cleanCardNode_ningXiangPaoHuZi(off);
            ...
            for (var k = 0; k < cardArr.length; k++) {
                ...
                for (var j = 0; j < groupList.length; j++) {
                    addOrAdjustHandCard_ningXiangPaoHuZi(k, j, groupList[j], off);
                }
            }
            resetCardNode_ningXiangPaoHuZi(off);

            addTingSign_ningXiangPaoHuZi(posNode); // 添加听牌角标
            ...
            for (var i = 0; i < cardArr.length; i++) {
                ...
                if(addNode.lastPosition){
                    ...
                    doMovetoAction_ningXiangPaoHuZi(addNode, cc.p((winSize.width - totalWidth) / 2 + i * width * scale_x, 0));
                }else{
        ...
        var handCard = null;
        if(off == 0){
            if(MjClient.MaxPlayerNum_ningXiangPaoHuZi == 3 || MjClient.MaxPlayerNum_ningXiangPaoHuZi == 2){
                handNode = posNode.getChildByName("handNode");
                handCard = posNode.getChildByName("handCard");
            }else if(MjClient.MaxPlayerNum_ningXiangPaoHuZi == 4){
        ...
        for(var k = 0;k < cardArr.length;k++){
            ...
            for(var j = 0;j < groupList.length;j++){
                if(off == 0){
                    if(MjClient.MaxPlayerNum_ningXiangPaoHuZi == 3 || MjClient.MaxPlayerNum_ningXiangPaoHuZi == 2){
                        addHandCard_ningXiangPaoHuZi(k,j,groupList[j],off);
                    }else if(MjClient.MaxPlayerNum_ningXiangPaoHuZi == 4){
                        addHandCardReplay_ningXiangPaoHuZi(k,j,groupList[j],off);
                    }
                }else {
                    addHandCardReplay_ningXiangPaoHuZi(k,j,groupList[j],off);
                }
        ...
        if(off == 0 && (MjClient.MaxPlayerNum_ningXiangPaoHuZi == 3 || MjClient.MaxPlayerNum_ningXiangPaoHuZi == 2)) {
            var scale_x = handCard.scaleX;
            var winSize = MjClient.size;
...
function checkCard_ningXiangPaoHuZi(node, off){
    ...
    if(cardHandArr.length > cardHand.length){
        ...
        for(var i = 0;i < cardHandArr.length; i++){
            if(cardHandArr[i] != cardHand[i]){
                RemoveHandCard_ningXiangPaoHuZi(node, cardHandArr[i], off);
...
function resetHandCard_ningXiangPaoHuZiEx(posNode,off, needAction, isDelay){
    if(off == 0) {
        checkCard_ningXiangPaoHuZi(posNode, off);
        var handNode = posNode.getChildByName("handNode");
        handNode.visible = true;
        cleanCardNode_ningXiangPaoHuZi(off);
        handNode.removeAllChildren();
 ...
 */
function addHandCard_ningXiangPaoHuZi(tagName,index,mjNum,off){
    if(off != 0){
        return;
    }
    var _node = getNode_ningXiangPaoHuZi(off);
    ...
    //设置牌
    var newCard = getNewCard_ningXiangPaoHuZi(mjNum,1,off);
    var scale_y = newCard.scaleY;
    //var parentCount = addNode.childrenCount;
    //首先根据name判断cpNode中是否已经添加
    var cardParent = addNode.getChildByTag(tagName);
    if(!cardParent){
        ...
        cardParent.width = newCard.width;
        addNode.addChild(cardParent);
        // SetTouchCardHandler_ningXiangPaoHuZi(cardParent);
    }
    ...
    cc.log("chow", "addHandCard_ningXiangPaoHuZi: name=" + newCard.getName() + " num=" + newCard.num + " tag =" + newCard.tag);
}
 ...
 **/
function getNewCard_ningXiangPaoHuZi(mjNum, type, off, isTurn){
    var _node = getNode_ningXiangPaoHuZi(off);
    var copyNode = null;
    if(type == 1){
        //手牌
    ...
    //cc.log("getNewCard_ningXiangPaoHuZi:"+mjNum+","+type+","+off+","+isTurn);
    if(mjNum > 0){
        //创建一个带有麻将信息的麻将 cp为创建后的麻将
        setCardSprite_ningXiangPaoHuZi(mjNode, mjNum, type, isTurn);
        if(type == 1){
            //只有手牌才会有点击事件
            SetTouchCardHandler_ningXiangPaoHuZi(mjNode,off);
        }
    }
...
/**
 ...
 type:类型，1:手牌 2：吃牌 3：打的牌
 */
function setCardSprite_ningXiangPaoHuZi(mjNode, mjNum, type, isTurn){
    MjClient.cardPath_ningXiangPaoHuZi = ziPai.getCardFilePath();

    if(type == 1){
        mjNode.loadTexture(MjClient.cardPath_ningXiangPaoHuZi+"hand" + mjNum + ".png");
    }
    if(type == 2){
        mjNode.loadTexture(MjClient.cardPath_ningXiangPaoHuZi+"out" + mjNum + ".png");
        if(isTurn){
            mjNode.loadTexture(MjClient.cardPath_ningXiangPaoHuZi+"huxiBG.png");
        }
    }
    if(type == 3 || type == 4){
        mjNode.loadTexture(MjClient.cardPath_ningXiangPaoHuZi+"put" + mjNum + ".png");
        if(isTurn){
            mjNode.loadTexture(MjClient.cardPath_ningXiangPaoHuZi+"normalBG.png");
        }
    }
    mjNode.tag = mjNum;
}

function checkTingCards_ningXiangPaoHuZi(putCard){
    //听牌检测
    var sData = MjClient.data.sData;
    cc.log("xw MjClient.data.sData.tData.tState", MjClient.data.sData.tData.tState);

    if(MjClient.data.sData.tData.tState == TableState.waitPut && curPlayerIsMe_ningXiangPaoHuZi(0) && putCard === undefined){
        postEvent("showTingLayer", []);
        return;
    }

    if(putCard && (!curPlayerIsMe_ningXiangPaoHuZi(0) || sData.tData.tState != TableState.waitPut)){
        putCard = undefined;
    }
...
//刷新手牌移动时的坐标
var btn_oPos = null;
function updateBtnMovedPosition_ningXiangPaoHuZi(btn, eventType){
...
//MjClient.movingCard_ningXiangPaoHuZi = null;
//MjClient.cloneCard = null;
//MjClient.putCard = null;
MjClient.hasPut = false;
function SetTouchCardHandler_ningXiangPaoHuZi(card,off){
    ...
    card.addTouchEventListener(function(btn,eventType){
        ...
        if(eventType == ccui.Widget.TOUCH_BEGAN){
            ...
            setCardSprite_ningXiangPaoHuZi(cloneCard, cardTag, 1, false);
            ...
            btn.x = touchPos.x;
            btn.y = touchPos.y;
            // updateBtnMovedPosition_ningXiangPaoHuZi(btn, eventType);
            ShowPutCardIcon_ningXiangPaoHuZi();

            checkTingCards_ningXiangPaoHuZi(btn.tag);
            var alignWidth = btn.scale * btn.width;
            setCardSprite_ningXiangPaoHuZi(btn, btn.tag, 3, false);
        ...
        if(eventType == ccui.Widget.TOUCH_ENDED ||
            ...
            setCardSprite_ningXiangPaoHuZi(btn, btn.tag, 1, false);
            ...
            if(((!pl.canNotPutCard||pl.canNotPutCard.indexOf(btnTag)==-1) &&(getOtherWeiCards_ningXiangPaoHuZi(btnTag)||(pl.limitHuPutCard && pl.limitHuPutCard.indexOf(btnTag) != -1))) &&
                ...
                !MjClient.isCommon && endPos.y >= cutLine.y && !MjClient.hasPut)
            {
                ...
                var putCard = getNewCard_ningXiangPaoHuZi(btnTag, 3 ,off);
                ...
                if(getOtherWeiCards_ningXiangPaoHuZi(btnTag)){
                            ...
                            if(pl.limitHuPutCard && pl.limitHuPutCard.indexOf(btnTag) != -1){
                                MjClient.showMsg("吃边打边，这局将只能胡"+ MjClient.majiang.getLimitHuDesc(btnTag, tData)+"，是否确定？",
                                    function(){
                                        ...
                                        HZPutCardToServer_ningXiangPaoHuZi(btnTag);
                                        MjClient.playui.ResetHandCard(MjClient.playui._downNode,0);

                                        checkTingCards_ningXiangPaoHuZi(btnTag);
                                    },
                                    function(){
                                        MjClient.playui.ResetHandCard(MjClient.playui._downNode,0);
                                        ShowPutCardIcon_ningXiangPaoHuZi();
            ...
            else
            {
                ...
                if(!isPut || isWangBa || isNoPut)
                {
                    ...
                    if(selectParentTag == btnParent.tag){
                        ...
                        if(selectArr){
                            //_changeArrIndex_ningXiangPaoHuZi(selectArr, btnTag, btn);
                            MjClient.HandCardArr[btnParent.tag].splice(parseInt(btnName),1);
                            _fixArrIndex_ningXiangPaoHuZi(selectArr, btnTag, btn);
                        }
                    }
...
//根据坐标Y，添加到数组对应的位置
function _fixArrIndex_ningXiangPaoHuZi(arr, btnTag, btn){
    if(arr){
        if(_isKan_ningXiangPaoHuZi(arr)){
            arr.push(btnTag);
        ...
        cc.log('_fixArrIndex_ningXiangPaoHuZi====:',arr);
    }

}

//改变数组元素的位置
function _changeArrIndex_ningXiangPaoHuZi(arr, btnTag, btn){
    if(arr && !_isKan_ningXiangPaoHuZi(arr)){
        var len = arr.length;
        for(var i = 0; i < len; i++){
            var tag = arr[i];
            if(tag == btnTag){
        ...
        _fixArrIndex_ningXiangPaoHuZi(arr, btnTag, btn);
    }
}

//数组里的是否为三个相同的牌
function _isKan_ningXiangPaoHuZi(arr){
    if(arr){
        var len = arr.length;
        if(len < 3){
            return false;
...
//断线重连之后，显示之前玩家打的牌
function DealOffLineCard_ningXiangPaoHuZi(node, off) {
    cc.log("====================DealOffLineCard_ningXiangPaoHuZi=================", off);
    var sData = MjClient.data.sData;
    var tData = sData.tData;
    ...
    if(typeof(tData.currCard) != "undefined" && tData.currCard != -1){
        var uids = tData.uids;
        var selfIndex = (uids.indexOf(SelfUid()) + off) % MjClient.MaxPlayerNum_ningXiangPaoHuZi;
        if(uids[selfIndex] == uids[tData.curPlayer]){
            var putCard = getNewCard_ningXiangPaoHuZi(tData.currCard, 3, off);
    ...
    cc.log("====================DealOffLineCard_ningXiangPaoHuZi================= end", off);
}

// 通过玩家idx获取UIBind（down left right xing）
function getUIBindByIndex_ningXiangPaoHuZi(idx) {
    var sData = MjClient.data.sData;
    var tData = sData.tData;
...
// 落牌动画
function fall_ningXiangPaoHuZi() {
    var sData = MjClient.data.sData;
    var tData = sData.tData;

    // tData.lastPlayer没同步!
    var lastPlayer = (tData.curPlayer - 1 + tData.maxPlayer) % tData.maxPlayer;
    cc.log("chow", "fall_ningXiangPaoHuZi"+ " lastPlayer = " + lastPlayer);
    ...
    //putNode.runAction会导致putNode.getChildren().length 为 0，如果刚好动画执行完成，又摸起一张新牌时， 下面的判断语句直接return，直接导致outNode中的out不显示出来，所以把put动画前置，规避这个现象
    ...
    if (out) {
        ...
        cc.log("chow", "fall_ningXiangPaoHuZi" + JSON.stringify(out.getPosition()));
...
//发牌
MjClient.isCommon = false;
function DealNewCard_ningXiangPaoHuZi(node, msg, off) {
    //没有发牌过来的时候直接忽略
    if (!msg.newCard) {
        RemovePutCardOut_ningXiangPaoHuZi();
        MjClient.playui.EatVisibleCheck();
        return;
    }

    AddPutCard_ningXiangPaoHuZi(node, msg, off); // 注意各个玩法验证！
    //存在发牌
    var sData = MjClient.data.sData;
    var tData = sData.tData;
    var uids = tData.uids;
    ...
    if (uids[selfIndex] == msg.uid) {
        cc.log("DealNewCard_ningXiangPaoHuZi  newCard@@ ", msg.newCard);
        //清理之前玩家打的牌
        var lastPlayer = tData.lastPlayer;
        if (lastPlayer != -1) {
            //var lastOff = (off + MjClient.MaxPlayerNum_ningXiangPaoHuZi - selfIndex) % MjClient.MaxPlayerNum_ningXiangPaoHuZi;
            // RemovePutCardOut_ningXiangPaoHuZi();

            fall_ningXiangPaoHuZi();
        }
        ...
        var putCard = getNewCard_ningXiangPaoHuZi(msg.newCard, 3, off, showPkBack);
        var putNode = node.getChildByName("put");
        ...
        if (!msg.isCommon) {
            if (MjClient.rePlayVideo == -1) {
                ...
                for (var i = 0; i < cardArr.length; i++) {
                    ...
                    if (tmpArr.length <= 2) {
                        isAdd = true;
                        //addHandCard_ningXiangPaoHuZi(i,tmpArr.length.length + 1,msg.newCard,off);
...
function setPutCardPos_ningXiangPaoHuZi(outNode, outCard, idx, off) {
    var tData = MjClient.data.sData.tData;
    var maxPlayer = tData.maxPlayer;
    var type = ziPai.getUiLayoutType();
    var isZuoXing = false;
    cc.log("setPutCardPos_ningXiangPaoHuZi:" + isZuoXing);
...
function setJiachuiTextTips_ningXiangPaoHuZi(node, off) {
    node.visible = false;
    var tData = MjClient.data.sData.tData;
    var pl = getUIPlayer_ningXiangPaoHuZi(off);
...
function setJiachuiImgTips_ningXiangPaoHuZi(node, off) {
    node.visible = false;
    var tData = MjClient.data.sData.tData;
    var pl = getUIPlayer_ningXiangPaoHuZi(off);
...
function AddPutCard_ningXiangPaoHuZi(node, msg, off) {
    var sData = MjClient.data.sData;
    var tData = sData.tData;
    var off_index = (tData.uids.indexOf(SelfUid()) + off) % MjClient.MaxPlayerNum_ningXiangPaoHuZi;
    if (off_index == tData.curPlayer) { // off对应玩家为当前玩家
        ...
        var out = getNewCard_ningXiangPaoHuZi(card, 2, off);
        var count = pl.mjput.length - 1;
        var width = out.width;
        var height = out.height;
        setPutCardPos_ningXiangPaoHuZi(outNode, out, pl.mjput.length - 1, off);
...
//翻醒发牌
function DealFanXingNewCard_ningXiangPaoHuZi(card){
    var putCard = getNewCard_ningXiangPaoHuZi(card, 4, 0);
...
//王闯/王钓
function DealWangChuang_ningXiangPaoHuZi(node,msg,off){
    var sData = MjClient.data.sData;
    var tData = sData.tData;
    ...
    if(uids[selfIndex] == msg.uid){
        ...
        if(pl.wangType == 1){
            var putCard = getNewCard_ningXiangPaoHuZi(91, 3, off);
            var putNode = node.getChildByName("put");
            putCard.scaleX = putCard.width/putNode.width - 0.16;
            putCard.scaleY = putCard.width/putNode.width - 0.06;
            putCard.x = putCard.width/2;
            ...
            ShowEatActionAnim_ningXiangPaoHuZi(node, ActionType_ningXiangPaoHuZi.WANGDIAO, off);

            var delay = cc.delayTime(1.5);
            var callback = function(){
                RemovePutCardOut_ningXiangPaoHuZi();
                //HZNewCardToServer_ningXiangPaoHuZi();
            }
            //putNode.runAction(cc.sequence(delay,cc.callFunc(callback)));
        }else if(pl.wangType == 2){
            var putCard = getNewCard_ningXiangPaoHuZi(91, 3, off);
            var putCard1 = getNewCard_ningXiangPaoHuZi(91, 3, off);
            var putNode = node.getChildByName("put");
            putCard.scaleX = putCard.width/putNode.width - 0.16;
            putCard.scaleY = putCard.width/putNode.width - 0.06;
            ...
            ShowEatActionAnim_ningXiangPaoHuZi(node, ActionType_ningXiangPaoHuZi.WANGCHUANG, off);
            var delay = cc.delayTime(1.5);
            var callback = function(){
                RemovePutCardOut_ningXiangPaoHuZi();
                //HZNewCardToServer_ningXiangPaoHuZi();
            ...
            var putCard = getNewCard_ningXiangPaoHuZi(91, 3, off);
            var putCard1 = getNewCard_ningXiangPaoHuZi(91, 3, off);
            var putCard2 = getNewCard_ningXiangPaoHuZi(91, 3, off);
            var putNode = node.getChildByName("put");
            putCard.scaleX = putCard.width/putNode.width - 0.16;
            putCard.scaleY = putCard.width/putNode.width - 0.06;
            putCard1.scaleX = putCard.width/putNode.width - 0.16;
            ...
            ShowEatActionAnim_ningXiangPaoHuZi(node, ActionType_ningXiangPaoHuZi.WANGZHA, off);
            var delay = cc.delayTime(1.5);
            var callback = function(){
                RemovePutCardOut_ningXiangPaoHuZi();
                //HZNewCardToServer_ningXiangPaoHuZi();
...
//处理摸最后一张牌
function DealShowLastCard_ningXiangPaoHuZi(node, msg, off){
    cc.log("======DealShowLastCard_ningXiangPaoHuZi======= " + off);
    MjClient.hasPut = true;
    var sData = MjClient.data.sData;
    var tData = sData.tData;
    var uids = tData.uids;
    ...
    if(uids[selfIndex] == msg.uid)
    {
        ...
        setTimeout(function(){
            RemovePutCardOut_ningXiangPaoHuZi(true);
            var pl = getUIPlayer_ningXiangPaoHuZi(0);
...
//处理出牌
function DealPutCard_ningXiangPaoHuZi(node, msg, off) {
    AddPutCard_ningXiangPaoHuZi(node, msg, off); // 注意各个玩法验证！
    ...
    if (uids[selfIndex] == msg.uid) {
        ...
        if (SelfUid() != msg.uid || MjClient.rePlayVideo != -1) {
            ...
            putNode.stopAllActions();
            putNode.removeAllChildren();
            var putCard = getNewCard_ningXiangPaoHuZi(msg.card, 3, off);
            putCard.scaleX = putCard.width / putNode.width - 0.16;
            putCard.scaleY = putCard.width / putNode.width - 0.06;
            putCard.x = putCard.width / 2;
            putCard.y = putCard.height / 2;
        ...
        if (off == 0) {
            removeTingSign_ningXiangPaoHuZi(node);
        }

        if (MjClient.rePlayVideo != -1) {
            RemoveHandCard_ningXiangPaoHuZi(node, msg.card, off);
            MjClient.playui.ResetHandCard(node, off);
        }
    }
}

function calculateHintPutList_ningXiangPaoHuZi() {
...
function addTingSign_ningXiangPaoHuZi(node) {
...
function removeTingSign_ningXiangPaoHuZi(node) {
    var handNode = node.getChildByName("handNode");
    if (!handNode) {
        return;
    }
...
//吃牌
function DealChiCard_ningXiangPaoHuZi(node, msg, off) {
    cc.log("======DealChiCard_ningXiangPaoHuZi======= " + off);
    var sData = MjClient.data.sData;
    var tData = sData.tData;
    var uids = tData.uids;
    var selfIndex = (uids.indexOf(SelfUid()) + off) % MjClient.MaxPlayerNum_ningXiangPaoHuZi;
    if (tData.curPlayer == selfIndex) {
        ...
        for (var i = 0; i < tmpEatCards.length; i++) {
            RemoveHandCard_ningXiangPaoHuZi(node, tmpEatCards[i], off);
        }
            ...
            for (var k = 0; k < biCards.length; k++) {
                ...
                for (var m = 0; m < biArr.length; m++) {
                    if (tmpBiArr.indexOf(biArr[m]) >= 0) {
                        ...
                        RemoveHandCard_ningXiangPaoHuZi(node, biArr[m], off);
                    }
                }
            }
        }
        ...
        for (var i = 0; i < eatCards.length; i++) {
            var card = getNewCard_ningXiangPaoHuZi(eatCards[i], 2, off);
            if (i == 2) {
                card.setColor(cc.color(170, 170, 170));
            }
            ...
            card.zIndex = 4 - i;
            card.x = 0;
            card.y = off_count * getOffY_ningXiangPaoHuZi(node, card);
            setChiCardAnchorPoint_ningXiangPaoHuZi(node, card);
        ...
        if (biCards) {
            ...
            for (var k = 0; k < biCards.length; k++) {
                var biArr = biCards[k];
                for (var m = 0; m < biArr.length; m++) {
                    var card = getNewCard_ningXiangPaoHuZi(biArr[m], 2, off);
                    if (m == 2) {
                        card.setColor(cc.color(170, 170, 170));
                    }
                    setChiCardAnchorPoint_ningXiangPaoHuZi(node, card);
        ...
        var parentPos = getChiCardParentPosition_ningXiangPaoHuZi(node, cardWidth, off, selfColCount);
        var moveX0 = 0;
        ...
        parent.x = moveX0;
        setCardParentPosY_ningXiangPaoHuZi(node, parent);
        ...
        if (biCards) {
            ShowEatActionAnim_ningXiangPaoHuZi(node, ActionType_ningXiangPaoHuZi.XIAHUO, off);
        } else {
            ShowEatActionAnim_ningXiangPaoHuZi(node, ActionType_ningXiangPaoHuZi.CHI, off);
        }
...
function setChiCardAnchorPoint_ningXiangPaoHuZi(node,newCard) {
    if(node.getName() == "down"){
        newCard.anchorX = 0;
        newCard.anchorY = 0;
    }else if(node.getName() == "right"){
...
function getOffY_ningXiangPaoHuZi(node,newCard) {
    var off_y = 0;
    if(node.getName() == "down"){
        off_y = newCard.height;
    }else if(node.getName() == "right"){
...
function getChiCardParentPosition_ningXiangPaoHuZi(node, newCardWidth, off, selfColCount, orignColPosX) {
...
function setCardParentPosY_ningXiangPaoHuZi(node, cardParent) {
    var eatNode = node.getChildByName("eatNode");
    if(node.getName() == "down"){
        cardParent.y = 0;
    }else if (node.getName() == "right"){
...
function getShowCardIdx_ningXiangPaoHuZi(node, chiTip) {
    var idx;
    switch(node.getName()) {
        case "down":
        case "xing":
...
// 处理碰
function DealPengCard_ningXiangPaoHuZi(node, msg, off){
    cc.log("======DealChiCard_ningXiangPaoHuZi======= ");
    var sData = MjClient.data.sData;
    var tData = sData.tData;
    var uids = tData.uids;
    var selfIndex = (uids.indexOf(SelfUid()) + off) % MjClient.MaxPlayerNum_ningXiangPaoHuZi;
    if(tData.curPlayer == selfIndex){
        ...
        for(var i = 0;i < 3; i++) {
            var card = getNewCard_ningXiangPaoHuZi(pengCard,2,off);
            cardWidth = card.width//*eatNode.scaleX;
            var childCount = parent.childrenCount;
            card.zIndex = 4 - childCount;
            card.x = 0;
            card.y = childCount * getOffY_ningXiangPaoHuZi(node, card);
            setChiCardAnchorPoint_ningXiangPaoHuZi(node, card);
            parent.addChild(card);
        }
        eatNode.addChild(parent);

        RemoveHandCard_ningXiangPaoHuZi(node,pengCard,off);
        RemoveHandCard_ningXiangPaoHuZi(node,pengCard,off);
        MjClient.playui.ResetHandCard(node, off);
        ShowEatActionAnim_ningXiangPaoHuZi(node, ActionType_ningXiangPaoHuZi.PENG, off);
        ...
        var parentPos = getChiCardParentPosition_ningXiangPaoHuZi(node, cardWidth, off, 1);
        var moveX0 = 0;
        var outNodeCount = eatNode.getChildrenCount();
        if(outNodeCount>0){
            if(node.getName() == "down" || node.getName() == "left"){
...
//偎牌
function DealWeiCard_ningXiangPaoHuZi(node, msg, off) {
    cc.log("======DealWeiCard_ningXiangPaoHuZi======= ");
    var sData = MjClient.data.sData;
    var tData = sData.tData;
    var uids = tData.uids;
    var selfIndex = (uids.indexOf(SelfUid()) + off) % MjClient.MaxPlayerNum_ningXiangPaoHuZi;
    if (tData.curPlayer == selfIndex) {
        ...
        RemovePutCardOut_ningXiangPaoHuZi();
        ShowEatActionAnim_ningXiangPaoHuZi(node, ActionType_ningXiangPaoHuZi.WEI, off);
        ...
        for (var i = 0; i < 3; i++) {
            ...
            {
                if (i == getShowCardIdx_ningXiangPaoHuZi(node, "wei")) {
                    isTurn = false;
                }
                var card = getNewCard_ningXiangPaoHuZi(weiCard, 2, off, isTurn);
            } else if (MjClient.majiang.getCardShowType(tData.uids[tData.curPlayer], weiCard) == 1) {
                var card = getNewCard_ningXiangPaoHuZi(weiCard, 2, off);
                ...
                var card = getNewCard_ningXiangPaoHuZi(weiCard, 2, off, true);
            }
            cardWidth = card.width //*eatNode.scaleX;
            card.zIndex = 4 - i;
            card.x = 0;
            card.y = i * getOffY_ningXiangPaoHuZi(node, card);
            setChiCardAnchorPoint_ningXiangPaoHuZi(node, card);
            parent.addChild(card);
        }
        eatNode.addChild(parent);

        RemoveHandCard_ningXiangPaoHuZi(node, weiCard, off);
        RemoveHandCard_ningXiangPaoHuZi(node, weiCard, off);
        MjClient.playui.ResetHandCard(node, off);
...
// 跑牌或者提牌
function DealGangCard_ningXiangPaoHuZi(node, msg, off) {
    var sData = MjClient.data.sData;
    var tData = sData.tData;
    var uids = tData.uids;
    var selfIndex = (uids.indexOf(SelfUid()) + off) % MjClient.MaxPlayerNum_ningXiangPaoHuZi;
    ...
    if (uids.indexOf(gangUid) == selfIndex) {
        ...
        if (msg.type == 2) {
            RemovePutCardOut_ningXiangPaoHuZi();
            ShowEatActionAnim_ningXiangPaoHuZi(node, ActionType_ningXiangPaoHuZi.PAO, off);
        } else {
            if (!msg.isGangHand && msg.type == 1) {
                RemovePutCardOut_ningXiangPaoHuZi();
            }
            ShowEatActionAnim_ningXiangPaoHuZi(node, ActionType_ningXiangPaoHuZi.TI, off);
        }
        ...
        for (var i = 0; i < 4; i++) {
            ...
            if (msg.type == 2) {
                isTurn = false;
                var card = getNewCard_ningXiangPaoHuZi(gangCard, 2, off, isTurn);
            } else if (
            ...
            {
                if (i == getShowCardIdx_ningXiangPaoHuZi(node, "ti")) {
                    isTurn = false;
                }
                var card = getNewCard_ningXiangPaoHuZi(gangCard, 2, off, isTurn);
            } else if (MjClient.majiang.getCardShowType(tData.uids[tData.curPlayer], gangCard) == 1) {
                var card = getNewCard_ningXiangPaoHuZi(gangCard, 2, off, false);
                var shade = new cc.Sprite("playing/paohuzi/huxiBG1.png");
                shade.opacity = 100;
                shade.x = shade.width / 2;
                shade.y = shade.height / 2;
                ...
                var card = getNewCard_ningXiangPaoHuZi(gangCard, 2, off, true);
            }
            cardWidth = card.width //*eatNode.scaleX;
            card.zIndex = 4 - i;
            card.x = 0;
        ...
        RemoveHandCard_ningXiangPaoHuZi(node, gangCard, off);
        RemoveHandCard_ningXiangPaoHuZi(node, gangCard, off);
        RemoveHandCard_ningXiangPaoHuZi(node, gangCard, off);
        if (msg.isGangHand) {
            RemoveHandCard_ningXiangPaoHuZi(node, gangCard, off);
        ...
        MjClient.playui.ResetHandCard(node, off);
        // addTingSign_ningXiangPaoHuZi(node); // 添加听牌角标 (提 跑可能手牌未变动)
        ...
        var parentPos = getChiCardParentPosition_ningXiangPaoHuZi(node, cardWidth, off, 1, orignColPosX);
        var moveX0 = 0;
        var outNodeCount = eatNode.getChildrenCount();
        if (outNodeCount > 0) {
            if (node.getName() == "down" || node.getName() == "left") {
...
// 处理胡
function DealHu_ningXiangPaoHuZi(node, msg, off){
    var sData = MjClient.data.sData;
    var tData = sData.tData;
    var selfIndex = (tData.uids.indexOf(SelfUid()) + off) % MjClient.MaxPlayerNum_ningXiangPaoHuZi;
    var pl = getUIPlayer_ningXiangPaoHuZi(off);
    ...
    if(pl){
        pl.eatFlag = 0;
        MjClient.playui.EatVisibleCheck();
        ShowEatActionAnim_ningXiangPaoHuZi(node,ActionType_ningXiangPaoHuZi.HU,off);
    }
}

function DealAddCard_ningXiangPaoHuZi(node,msg, off){
    if(MjClient.rePlayVideo == -1 && off != 0){
        return;
    }
    var cardArr = MjClient.HandCardArr;
...
//清除打出的牌
function RemovePutCardOut_ningXiangPaoHuZi(noAction){

    var jsBind = MjClient.playui.jsBind;
    var uiList = [jsBind.down, jsBind.right, jsBind.left];
    if (MjClient.MaxPlayerNum_ningXiangPaoHuZi == 4)
    {
        uiList = [jsBind.down, jsBind.xing, jsBind.right, jsBind.left];
    }else if (MjClient.MaxPlayerNum_ningXiangPaoHuZi == 2)
    {
        uiList = [jsBind.down, jsBind.left];
    }
...
//发手牌时再清理一遍
function clearMJHandCardUI_ningXiangPaoHuZi() {
    // var handNode = MjClient.playui._downNode.getChildByName("handNode");
    // handNode.removeAllChildren();
...
//出牌表示状态
function ShowPutCardIcon_ningXiangPaoHuZi() {
...
//清除手牌
function RemoveHandCard_ningXiangPaoHuZi(node,card,off){
    if(MjClient.rePlayVideo == -1 && off != 0){
        return;
    }
    var cardArr = MjClient.HandCardArr;
...
// 初始化吃碰杠胡动作
function InitShowEatActionNode_ningXiangPaoHuZi(plNode){
...
// 重置吃碰杠胡动作
function resetEatActionAnim_ningXiangPaoHuZi()
{
    var jsBind = MjClient.playui.jsBind;
    ...
    var tData = MjClient.data.sData.tData;
    var count = MjClient.MaxPlayerNum_ningXiangPaoHuZi;
    for(var i = 0; i < count; i++)
    {
        InitShowEatActionNode_ningXiangPaoHuZi(ui[i]._node);
    }
}

//播放头像移动
function tableStartHeadMoveAction_ningXiangPaoHuZi(node){
...
//重置3家头像位置
function reConectHeadLayout_ningXiangPaoHuZi(node){
    var tData = MjClient.data.sData.tData;
    var down = node.getChildByName("down").getChildByName("head");
    var left = node.getChildByName("left").getChildByName("head");
    var right = node.getChildByName("right").getChildByName("head");
    ...
    resetEatActionAnim_ningXiangPaoHuZi();
...
//根据偏移获得玩家node
function getNode_ningXiangPaoHuZi(off){
    var _node = null;
    var tData = MjClient.data.sData.tData;
 ...
 * @return {object} 玩家数据
 */
function getUIPlayer_ningXiangPaoHuZi(off){
    var sData = MjClient.data.sData;
    var tData = sData.tData;
    var uids = tData.uids;
...
// 获取ui头像，通过偏移值
function getUIHeadByOff_ningXiangPaoHuZi(off){
    var pl = getUIPlayer_ningXiangPaoHuZi(off);
    if(!pl)
    {
        return {};
 ...
 * 下载录音, 调用 播放函数
 * */
function downAndPlayVoice_ningXiangPaoHuZi(uid, filePath){
    var index = getUiOffByUid_ningXiangPaoHuZi(uid);
...
function getUiOffByUid_ningXiangPaoHuZi(uid){
    var sData = MjClient.data.sData;
    var tData = sData.tData;
    var uids = tData.uids;
    var selfIndex = uids.indexOf(SelfUid());
...
//设置微信头像
function setWxHead_ningXiangPaoHuZi(node, d, off){
    if(d.uid == getUIHeadByOff_ningXiangPaoHuZi(off).uid){
        ...
        COMMON_UI.addNobleHeadFrame(nobody,getUIPlayer_ningXiangPaoHuZi(off))
    }
}

//显示玩家庄的ui
function showUserZhuangLogo_ningXiangPaoHuZi(node, off){
    var tData = MjClient.data.sData.tData;
    var pl = getUIPlayer_ningXiangPaoHuZi(off);
    node.zIndex = 100;
    if(tData && pl){
...
function showUserZhuangLogoZuoXing_ningXiangPaoHuZi(node, off){
    var tData = MjClient.data.sData.tData;
    var pl = getUIPlayer_ningXiangPaoHuZi(off);
    node.zIndex = 100;
    if(tData && pl){
...
//显示房主  add by sking
function showFangzhuTagIcon_ningXiangPaoHuZi(node,off)
{
    var pl = getUIPlayer_ningXiangPaoHuZi(off);
    if(!pl) //位置上没人则删掉房主标签
    {
...
// 清理ui
function clearCardUI_ningXiangPaoHuZi(node,off){
    mylog("clearCardUI_ningXiangPaoHuZi");
    ...
    for(var i = 0; i < children.length; i++)
    {
        ...
        {
            InitShowEatActionNode_ningXiangPaoHuZi(ni.getParent());
        }
    }
}

function EatFlag_ningXiangPaoHuZi(){
    var eat = MjClient.playui.jsBind.eat;
    var eatFlag = 0;
...
function huXiScore_ningXiangPaoHuZi(type,card)
{
...
//刷新胡息
function UpdateHuXi_ningXiangPaoHuZi(off){
    var sData = MjClient.data.sData;
    var tData = sData.tData;
    var score = 0;
    var pl = getUIPlayer_ningXiangPaoHuZi(off);
    ...
    if(mjpao.length>0){
        for(var p=0;p<mjpao.length;p++){
            score += huXiScore_ningXiangPaoHuZi("paoPai",mjpao[p]);
        }
    }
    ...
    if(mjpeng.length>0){
        for(var i=0;i<mjpeng.length;i++){
            score += huXiScore_ningXiangPaoHuZi("peng",mjpeng[i]);
        }
    }
        ...
        var chiScore = function(cards){
            ...
            for(var k=0;k<chiXiArr.length;k++){
                ...
                if(cards.toString() == chiXiList.toString()){
                    score += huXiScore_ningXiangPaoHuZi("chi",cards[0]);
                }
            }
        };
    ...
    if(mjti.length>0){
        for (var t = 0; t < mjti.length; t++) {
            score += huXiScore_ningXiangPaoHuZi("tiPai", mjti[t]);
        }
    }
    ...
    if(mjwei && mjwei.length>0){
        for (var w = 0; w < mjwei.length; w++) {
            if(MjClient.majiang.getCardShowType(pl.info.uid, mjwei[w]) != 0) {
                score += huXiScore_ningXiangPaoHuZi("weiPai", mjwei[w]);
            }
        }
    }
    ...
    if(tData.xingPlayer == selfIndex){
        pl = getUIPlayer_ningXiangPaoHuZi(2);
        //提牌
        var mjti = pl.mjgang1;
        if(mjti.length>0){
            for(var t=0;t<mjti.length;t++){
...
//设置转盘显示状态
function IsArrowVisible_ningXiangPaoHuZi(){
    var pl = getUIPlayer_ningXiangPaoHuZi(0);
    if (!pl)
    {
        return;
...
// 显示吃碰杠胡动作
function ShowEatActionAnim_ningXiangPaoHuZi(node, actType, off){
    var delayTime = 1;
    switch(actType)
    {
        case ActionType_ningXiangPaoHuZi.FANXING:
            delayTime = 1.6;
            break;
        default:
            break;
    ...
    switch(actType)
    {
        case ActionType_ningXiangPaoHuZi.CHI:
        ...
        case ActionType_ningXiangPaoHuZi.PENG:
        ...
        case ActionType_ningXiangPaoHuZi.WEI:
        ...
        case ActionType_ningXiangPaoHuZi.HU:

            eatActionChild = eatActionNode.getChildByName("hu");
            eatActionChild.visible = true;
            break;
        case ActionType_ningXiangPaoHuZi.PAO:
            eatActionChild = eatActionNode.getChildByName("pao");
            eatActionChild.visible = true;
            eatActionChild.runAction(cc.sequence(cc.delayTime(delayTime), cc.callFunc(callback)));
            break;
        case ActionType_ningXiangPaoHuZi.TI:
            ...
            eatActionChild.runAction(cc.sequence(cc.delayTime(delayTime), cc.callFunc(callback)));
            break;
        case ActionType_ningXiangPaoHuZi.WANGDIAO:
            eatActionChild = eatActionNode.getChildByName("wangDiao");
            eatActionChild.visible = true;
            eatActionChild.runAction(cc.sequence(cc.delayTime(delayTime), cc.callFunc(callback)));
            break;
        case ActionType_ningXiangPaoHuZi.WANGCHUANG:
            eatActionChild = eatActionNode.getChildByName("wangChuang");
            eatActionChild.visible = true;
            eatActionChild.runAction(cc.sequence(cc.delayTime(delayTime), cc.callFunc(callback)));
            break;
        case ActionType_ningXiangPaoHuZi.XIAHUO:
...
//显示玩家信息
function showPlayerInfo_ningXiangPaoHuZi(off, node){
    //var tData = MjClient.data.sData.tData;
    var pl = getUIPlayer_ningXiangPaoHuZi(off);
    if(pl){
...
function resetChiParam_ningXiangPaoHuZi(){

}

function setChiVisible_ningXiangPaoHuZi(){
    var eat = MjClient.playui.jsBind.eat;
...
function showSelectEatCards_ningXiangPaoHuZi(node,cardNum){
    var eat = MjClient.playui.jsBind.eat;
    eat.chi._node.visible = false;
    eat.guo._node.visible = false;
    eat.peng._node.visible = false;
    ...
    addEatCards_ningXiangPaoHuZi(parent,cardArr,putCard);
}

//居中动画,数组里的节点居中排列 只关心x轴
doMoveCenterAction_ningXiangPaoHuZi = function(arr,isScale){
    arr.reverse();
    var gap = 5;
...
function commitEatCards_ningXiangPaoHuZi(eatCards, biArr){
    if ((MjClient.data.sData.players[SelfUid()].eatFlag & 32) > 0) {
        MjClient.showMsg("吃牌后视为过胡，确定吃吗？", function() {
            setChiVisible_ningXiangPaoHuZi();
            HZChiToServer_ningXiangPaoHuZi(eatCards, biArr);
        }, function() {}, "1");
    } else {
        setChiVisible_ningXiangPaoHuZi();
        HZChiToServer_ningXiangPaoHuZi(eatCards, biArr);
...
//排序 吃牌放最上面
function sortSelectEatCards_ningXiangPaoHuZi(cards) {
    var sData = MjClient.data.sData;
    var pl = sData.players[SelfUid()];
...
//添加可以吃的牌
function addEatCards_ningXiangPaoHuZi(node,handCardArr,putCard){
    var cardPath = MjClient.cardPath_ningXiangPaoHuZi;
    ...
    for(var i = 0;i < chiSet.length;i++){
        ...
        var cardList = sortSelectEatCards_ningXiangPaoHuZi(chiSet[i]);
        ...
        chiCloneBtn.addClickEventListener(function(chiBtn){
            ...
            if(tmpHandCardArr.indexOf(putCard) < 0){
                // setChiVisible_ningXiangPaoHuZi();
                // HZChiToServer_ningXiangPaoHuZi(chiArr,null);

                commitEatCards_ningXiangPaoHuZi(chiArr, null);
            }else{
                var biSet = MjClient.majiang.getBiCards(tmpHandCardArr,putCard);
                if(biSet && biSet.length > 0){
                    addFirstBiCards_ningXiangPaoHuZi(node,biSet,putCard,tmpHandCardArr,chiArr);
                }
            }
        });
    }
    if(ziPai.getUiLayoutType() == 0){//新布局
        doMoveCenterAction_ningXiangPaoHuZi([chiBg], false);
    }
}

//添加第一个比牌
function addFirstBiCards_ningXiangPaoHuZi(node,biSet,putCard,handCardArr,chiArr){
    var cardPath = MjClient.cardPath_ningXiangPaoHuZi;
    cardPath = ziPai.getCardFilePath();
    ...
    for(var i = 0;i < biSet.length;i++){
        var cardList = sortSelectEatCards_ningXiangPaoHuZi(biSet[i]);
        ...
        cloneBtn.addClickEventListener(function(btn){
            ...
            if(tmpHandCardArr.indexOf(putCard) >= 0){
                var biSet = MjClient.majiang.getBiCards(tmpHandCardArr,putCard);
                if(biSet && biSet.length > 0){
                    addSecBiCards_ningXiangPaoHuZi(node,biSet,putCard,tmpHandCardArr,chiArr,biArr);
                }
            }else{
                // setChiVisible_ningXiangPaoHuZi();
                // HZChiToServer_ningXiangPaoHuZi(chiArr,[biArr]);
                commitEatCards_ningXiangPaoHuZi(chiArr,[biArr]);
            }
        });
    }
    ...
    if(ziPai.getUiLayoutType() == 0){//新布局
        ...
        doMoveCenterAction_ningXiangPaoHuZi([chiBg,biBg],isScale);
    }else{
        biBg.scale = chiBg.scale;
        doMoveCenterAction_ningXiangPaoHuZi2([chiBg,biBg],isScale);
    }
}

//居中动画,数组里的节点居中排列 只关心x轴
doMoveCenterAction_ningXiangPaoHuZi2 = function(arr,isScale){
    isScale = isScale === undefined ? true : isScale;
    var gap = 5;
    if(arr && arr.length > 0){
        var len = arr.length;
...
//添加第二个比牌
function addSecBiCards_ningXiangPaoHuZi(node,biSet,putCard,handCardArr,chiArr,firstBiArr){
    var cardPath = MjClient.cardPath_ningXiangPaoHuZi;
    cardPath = ziPai.getCardFilePath();
    ...
    for(var i = 0;i < biSet.length;i++){
        ...
        cloneBtn.addClickEventListener(function(btn){
            ...
            btn.setBright(false);
            // setChiVisible_ningXiangPaoHuZi();
            // HZChiToServer_ningXiangPaoHuZi(chiArr,[firstBiArr,biArr]);

            commitEatCards_ningXiangPaoHuZi(chiArr,[firstBiArr,biArr]);
        });
    }
    ...
    if(ziPai.getUiLayoutType() == 0){//新布局
        ...
        doMoveCenterAction_ningXiangPaoHuZi([chiBg,biBg0,biBg],isScale);
    }else{
        biBg.scale = chiBg.scale;
        biBg0.scale = chiBg.scale;
        doMoveCenterAction_ningXiangPaoHuZi2([chiBg,biBg0,biBg],isScale);
    }
}

function GetReadyVisible_ningXiangPaoHuZi(node, off) {
    if (off < 0) {
        node.visible = false;
        return false;
    }
...
//设置玩家掉线头像
function setUserOffline_ningXiangPaoHuZi(node, off){
...
//设置玩家掉线头像（邵阳新版大小结算专用）
function setRoundEndUserOffline_ningXiangPaoHuZi(frame, pl){
    if(!pl){
        return;
    }
    ...
    if (pl.onLine == false){
        ...
        _timeNode.setPosition(cc.p(frame.getContentSize().width/2,frame.getContentSize().height * 0.65));
        if (pl.offLineTime){
            var _showTime = _currentTime - pl.offLineTime;
            _timeNode.setString(MjClient.dateFormat(new Date(_showTime),"mm:ss"));
        }
...
//显示玩家文字
function showUserChat_ningXiangPaoHuZi(node, off, msg){
    var tData = MjClient.data.sData.tData;
 ...
 */
function curPlayerIsMe_ningXiangPaoHuZi(off){
    var sData = MjClient.data.sData;
    var tData = sData.tData;
    ...
    var selfIndex = tData.uids.indexOf(SelfUid());
    selfIndex = (selfIndex + off)%MjClient.MaxPlayerNum_ningXiangPaoHuZi;
    return selfIndex == tData.curPlayer;
}

function getVisablePlayerCount_ningXiangPaoHuZi(){
    var sData = MjClient.data.sData;
    var tData = sData.tData;
    var uids = tData.uids;
    var count = 0;
 ...
 */
function getXingPlayerIndex_ningXiangPaoHuZi(){
    if(MjClient.MaxPlayerNum_ningXiangPaoHuZi != 4){
        return -1;
    }
    if(getVisablePlayerCount_ningXiangPaoHuZi() < 3){
        return -1;
    }
    var sData = MjClient.data.sData;
    var tData = sData.tData;
    ...
    var xingIndex = (zhuang + 2 + MjClient.MaxPlayerNum_ningXiangPaoHuZi) % MjClient.MaxPlayerNum_ningXiangPaoHuZi;
    var selfIndex = tData.uids.indexOf(SelfUid());
    xingIndex = (xingIndex - selfIndex + MjClient.MaxPlayerNum_ningXiangPaoHuZi) % MjClient.MaxPlayerNum_ningXiangPaoHuZi;
    return xingIndex;
}


//重新开始后，重置MjClient.HandCardArr
function resetHandAfterBegin_ningXiangPaoHuZi(){
    var sData = MjClient.data.sData;
    var pl = sData.players[SelfUid() + ""];
...
/*
 设置弃胡状态
 */
function setQiHuState_ningXiangPaoHuZi()
{
    var pl = getUIPlayer_ningXiangPaoHuZi(0);
    if (pl.isQiHu) {
...
function changeMJBg_ningXiangPaoHuZi(node, type)
{
    if (node.toString() == "[object ImageView]") {
        var oldFile = node.getRenderFile().file;
        var newFile = getNewMJBgFile_ningXiangPaoHuZi(oldFile, type);
    ...
    for(var index in childArray)
    {
        ...
        changeMJBg_ningXiangPaoHuZi(child, type);
    }
}

function getNewMJBgFile_ningXiangPaoHuZi(oldFile, type)
{
    ...
    if (type == 0) {
        ...
        if (oldFile.indexOf("/MJBg1") != -1)
            ...
            MjClient.cardPath_ningXiangPaoHuZi = "playing/paohuzi/MJBg1/";
        }
        ...
        if (oldFile.indexOf("/MJBg2") != -1)
            ...
            MjClient.cardPath_ningXiangPaoHuZi = "playing/paohuzi/MJBg2/";
        }
        else if (oldFile.indexOf("playing/paohuzi/") != -1) {
...
// 宁乡跑胡子放偎之后也能吃碰
function getOtherWeiCards_ningXiangPaoHuZi(card) {
    // var sData = MjClient.data.sData;
 ...
 */
function createRadioBoxForCheckBoxs_ningXiangPaoHuZi(nodes, callback, defaultIndex){
    var newobj = {};
 ...
 *  off:玩家的实际off
 **/
function changeUIOff_ningXiangPaoHuZi(off){
    var sData = MjClient.data.sData;
    var tData = sData.tData;
...
//是否显示离开按钮（包括准备阶段）
function isShowBackBtn_ningXiangPaoHuZi(){
    if(IsInviteVisible()){
        return true;
    }else{
        var sData = MjClient.data.sData;
 ...
 */
function getAnimate_ningXiangPaoHuZi(preName, len, delaytime, playCount, startIndex){
...
function paioFenToServer_ningXiangPaoHuZi(index) {
    var tData = MjClient.data.sData.tData;
