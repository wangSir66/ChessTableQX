...
function SetUserVisible_ningXiangPaoHuZi(node, off){
    ...
    if(pl){
        ...
        setUserOffline_ningXiangPaoHuZi(node, off);
...
function InitUserHandUI_ningXiangPaoHuZi(node, off){
    ...
    if (pl.jiazhuNum == 2 && MjClient.rePlayVideo == -1)
    {
        ...
        MjClient.majiang.setJiaZhuNum(node, getUIPlayer_ningXiangPaoHuZi(off));
    }
...
var PlayLayer_ningXiangPaoHuZi = cc.Layer.extend({
    jsBind: {
        _event: {
            ...
            endRoom: function(msg) {
                ...
                if (msg.showEnd){
                    this.addChild(new GameOverLayer_ningXiangPaoHuZi(),500);
                }else{
            ...
            roundEnd: function() {
                ...
                if(!MjClient.isDismiss && tData.hunCard && tData.hunCard != -1 && tData.areaSelectMode.xingType==2){
                    ...
                    ShowEatActionAnim_ningXiangPaoHuZi(MjClient.playui._downNode, ActionType_paohuzi.FANXING, 0);
                    DealFanXingNewCard_ningXiangPaoHuZi(hunCard);
                }
                ...
                function delayExe(){
                    ...
                    if (sData.tData.roundNum <= 0){
                        var layer = new GameOverLayer_ningXiangPaoHuZi();
                        layer.setVisible(false);
                        self.addChild(layer,500);
                    }
                    if(!MjClient.endoneui){
                        self.addChild(new EndOneView_ningXiangPaoHuZi(),500);
                    }
            ...
            moveHead: function() {
                ...
                //tableStartHeadMoveAction_ningXiangPaoHuZi(this);   //不涉及到头像移动动作
            },
            initSceneData: function() {
                ...
                reConectHeadLayout_ningXiangPaoHuZi(this);        //不涉及到头像移动动作
                CheckRoomUiDelete();
                tableStartHeadMoveAction_ningXiangPaoHuZi(this);   //不涉及到头像移动动作
            ...
            changeMJBgEvent: function() {
                changeMJBg_ningXiangPaoHuZi(this, ziPai.getZiPaiType());
            },
            MJPeng: function() {
                MjClient.playui.showAndHideHeadEffect();
            },
            ...
            EZP_layout : function(){
                if(MjClient.data.sData.tData.maxPlayer < 4){
                    ...
                    if(IsArrowVisible_ningXiangPaoHuZi()){
                        MjClient.playui.ResetPutCard(MjClient.playui._downNode, changeUIOff_ningXiangPaoHuZi(0));
                        MjClient.playui.ResetPutCard(MjClient.playui._rightNode, changeUIOff_ningXiangPaoHuZi(2));
                        MjClient.playui.ResetPutCard(MjClient.playui._topNode, changeUIOff_ningXiangPaoHuZi(3));
                    }
                }
            }
        },
        cardNumImg: {
            ...
            _event: {
                initSceneData: function(eD) {
                    var sData = MjClient.data.sData;
                    var tData = sData.tData;
                    this.visible = IsArrowVisible_ningXiangPaoHuZi() && TableState.waitJiazhu != tData.tState;
                },
                mjhand: function(eD) {
                    var sData = MjClient.data.sData;
                    var tData = sData.tData;
        ...
        roundInfo:{
            ...
            _run:function(){
                ...
                if (item = [undefined,"扎2分","扎5分"][tData.areaSelectMode.zhaNiaoNum]){
                    infos.push(item);
                }

                if (item = [undefined,"加1分","加2分","加2番","翻倍"][tData.areaSelectMode.ziMoNum]){
                    infos.push(item);
                }

                if (item = ["十六小","十八小"][tData.areaSelectMode.mingTangNum]){
                    infos.push(item);
                }

                if (tData.areaSelectMode.isHaiDi){
                    infos.push("海底胡");
                }

                if (tData.areaSelectMode.isHaiDi && tData.areaSelectMode.isHaiDiSiFan){
                    infos.push("海底胡4番");
                }

                if (tData.areaSelectMode.isJiaHongJiaXiao){
                    infos.push("加红加小加大");
                }

                if (tData.areaSelectMode.isJiaYiFen){
                    infos.push("27胡加1分");
                }

                if (tData.areaSelectMode.isShuaHouSiFan){
                    infos.push("耍候4番");
                }

                if (tData.areaSelectMode.isMingWei){
                    infos.push("明偎");
                }

                infos.push(tData.areaSelectMode.isManualCutCard ? "手动切牌" : "系统切牌");

                if (item = ["首局房主庄", "首局随机庄"][tData.areaSelectMode.shouJu]){
                    infos.push(item);
                }

                if (item = ["坐压0分","坐压1分","坐压2分","坐压4分"][tData.areaSelectMode.zuoYa]){
                    infos.push(item);
                }

                if (tData.areaSelectMode.fanBei == 1){
                    infos.push("小于" + tData.areaSelectMode.fanBeiScore + "分翻倍");
                }else{
                    infos.push("不翻倍");
                }
                infos.push(",底分" + tData.areaSelectMode.jieSuanDiFen);
        ...
        banner: {
            ...
            getRoomNum: {
                ...
                _click: function() {
                    ...
                    var sData = MjClient.data.sData;
                    var str7 = " ";
                    if (Object.keys(sData.players).length > 0 && MjClient.MaxPlayerNum_ningXiangPaoHuZi > Object.keys(sData.players).length){
                        str7 += ["一","二","三"][Object.keys(sData.players).length - 1] + "缺" + ["一","二","三"][MjClient.MaxPlayerNum_ningXiangPaoHuZi - Object.keys(sData.players).length - 1];
                    }
            ...
            gps_btn: {
                ...
                _click: function() {
                    if(MjClient.MaxPlayerNum_ningXiangPaoHuZi == 3){
            ...
            roundnumImg: {
                ...
                _event: {
                    initSceneData: function(eD) {
                        this.visible = IsArrowVisible_ningXiangPaoHuZi();
                    },
                    mjhand: function(eD) {
                        this.visible = IsArrowVisible_ningXiangPaoHuZi();
                    },
        ...
        wait: {
            ...
            backHomebtn: {
                ...
                _event: {
                    ...
                    initSceneData: function(eD) {
                        this.visible = isShowBackBtn_ningXiangPaoHuZi();
                    },
                    removePlayer: function(eD) {
                        this.visible = isShowBackBtn_ningXiangPaoHuZi();
                    },
        ...
        btns_piao:{
            ...
            //         var pl = getUIPlayer_ningXiangPaoHuZi(0);
            //         pl.piaoFen = -1;
            //         paioFenToServer_ningXiangPaoHuZi(0);
            //     },
            // },
            // btn_piao1: {
            //     _click:function() {
            //         var pl = getUIPlayer_ningXiangPaoHuZi(0);
            //         pl.piaoFen = -1;
            //         paioFenToServer_ningXiangPaoHuZi(1);
            //     },
            // },
            // btn_piao2: {
            //     _click:function() {
            //         var pl = getUIPlayer_ningXiangPaoHuZi(0);
            //         pl.piaoFen = -1;
            //         paioFenToServer_ningXiangPaoHuZi(2);
            //     },
            // },
            // btn_piao3: {
            //     _click:function() {
            //         var pl = getUIPlayer_ningXiangPaoHuZi(0);
            //         pl.piaoFen = -1;
            //         paioFenToServer_ningXiangPaoHuZi(3);
            //     },
            // },
    },
        BtnReady:{
        ...
        down: {
            head: {
                chuizi_tips : {
                    _visible:false,
                    // _run: function(){
                    //     setJiachuiImgTips_ningXiangPaoHuZi(this, 0);
                    // },
                    // _event: {
                    //     initSceneData: function() {
                    //         setJiachuiImgTips_ningXiangPaoHuZi(this, 0);
                ...
                jiachuiText_tips : {
                    _visible:false,
                    // _run: function() {
                    //     setJiachuiTextTips_ningXiangPaoHuZi(this, 0);
                    // },
                    // _event:{
                    //     initSceneData: function() {
                    //         setJiachuiTextTips_ningXiangPaoHuZi(this, 0);
                ...
                zhuang: {
                    ...
                    _event: {
                        waitPut: function(){
                            showUserZhuangLogo_ningXiangPaoHuZi(this, 0);
                        },
                        initSceneData: function(){
                            if (IsArrowVisible_ningXiangPaoHuZi()) showUserZhuangLogo_ningXiangPaoHuZi(this, 0);
                        },
                        mjhand: function(){
                            if (IsArrowVisible_ningXiangPaoHuZi()) showUserZhuangLogo_ningXiangPaoHuZi(this, 0);
                        }
                    ...
                    chattext: {
                        _event: {
                            ...
                            MJChat: function(msg) {
                                ...
                                showUserChat_ningXiangPaoHuZi(this, 0, msg);
                            },
                            playVoice: function(voicePath) {
                                MjClient.data._tempMessage.msg = voicePath;
                                showUserChat_ningXiangPaoHuZi(this, 0, MjClient.data._tempMessage);
                            }
                        }
                    }
                },
                _click: function(btn) {
                    showPlayerInfo_ningXiangPaoHuZi(0, btn);
                },
                _event: {
                    loadWxHead: function(d) {
                        setWxHead_ningXiangPaoHuZi(this, d, 0);
                    },
                    addPlayer: function(eD) {
                        showFangzhuTagIcon_ningXiangPaoHuZi(this,0);
                    },
                    removePlayer: function(eD) {
                        showFangzhuTagIcon_ningXiangPaoHuZi(this,0);
                    }
                ...
                huxi:{
                    ...
                    _event:{
                        ...
                        initSceneData:function(){
                            this.visible = true;
                            var huXi = UpdateHuXi_ningXiangPaoHuZi(0);
                            if(!IsArrowVisible_ningXiangPaoHuZi()){
                                huXi = 0;
                                this.visible = false;
                            }
                ...
                timeImg:{
                    ...
                    // _event:{
                    //     initSceneData: function(eD) {
                    //         this.visible = curPlayerIsMe_ningXiangPaoHuZi(0);
                    //     },
                    //     mjhand: function(eD) {
                    //         this.visible = curPlayerIsMe_ningXiangPaoHuZi(0);
                    //     },
                    //     onlinePlayer: function(eD) {
                    //         this.visible = curPlayerIsMe_ningXiangPaoHuZi(0) && IsArrowVisible_ningXiangPaoHuZi();
                    //     },
                    //     waitPut: function(eD) {
                    //         this.visible = curPlayerIsMe_ningXiangPaoHuZi(0);
                    //     },
                    ...
                    //             playTimeUpEff = null;
                    //             var pl = getUIPlayer_ningXiangPaoHuZi(0);
                    //             if(pl && pl.onLine && curPlayerIsMe_ningXiangPaoHuZi(0)){
                    //                 arrowbkNumberUpdate(this);
                    //             }
                    //         },
                    //         roundEnd: function() {
            ...
            ready: {
                ...
                _run: function() {
                    GetReadyVisible_ningXiangPaoHuZi(this, 0);
                },
                _event: {
                    startShuffleCards:function (d) {
                        GetReadyVisible_ningXiangPaoHuZi(this, -1);
                    },
                    moveHead: function() {
                        GetReadyVisible_ningXiangPaoHuZi(this, -1);
                    },
                    ...
                    addPlayer: function() {
                        GetReadyVisible_ningXiangPaoHuZi(this, 0);//根据状态设置ready 是否可见 add by sking
                    },
                    removePlayer: function() {
                        GetReadyVisible_ningXiangPaoHuZi(this, 0);
                    },
            ...
            handNode: {
                ...
                _event:  {
                    initSceneData: function() {
                        calculateHintPutList_ningXiangPaoHuZi();
                    },
                    HZPickCard: function() {
                        calculateHintPutList_ningXiangPaoHuZi();
                    },
                    ...
                    MJPass: function() { // 天胡 提 偎 跑后过胡
                        calculateHintPutList_ningXiangPaoHuZi();
                        addTingSign_ningXiangPaoHuZi(MjClient.playui._downNode);
                    },
            ...
            _event: {
                clearCardUI: function() {
                    clearCardUI_ningXiangPaoHuZi(this, 0);
                },
                ...
                initSceneData: function(eD) {
                    SetUserVisible_ningXiangPaoHuZi(this, 0);
                    InitUserHandUI_ningXiangPaoHuZi(this, 0);
                    InitUserCoinAndName_ningXiangPaoHuZi(this, 0);
                    MjClient.hasPut = false; // 重连回来重置打掉牌标志（打牌时候断线)
                    ShowPutCardIcon_ningXiangPaoHuZi();
                    DealOffLineCard_ningXiangPaoHuZi(this,0);
                    resetHandAfterBegin_ningXiangPaoHuZi();
                },
                addPlayer: function(eD) {
                    SetUserVisible_ningXiangPaoHuZi(this, 0);
                    InitUserCoinAndName_ningXiangPaoHuZi(this, 0);
                ...
                mjhand: function(eD) {
                    ...
                    var uid = getUIPlayer_ningXiangPaoHuZi(0).info.uid;
                    // if (eD.jiazhuNums)
                    // {
                ...
                HZNewCard: function(eD) {
                    ...
                    DealNewCard_ningXiangPaoHuZi(this,eD,0);
                },
                MJPut: function(eD) {
                    DealPutCard_ningXiangPaoHuZi(this,eD,0);
                },
                HZChiCard: function(eD) {
                    DealChiCard_ningXiangPaoHuZi(this, eD, 0);
                },
                HZWeiCard: function(eD){
                    DealWeiCard_ningXiangPaoHuZi(this,eD,0);
                },
                HZGangCard: function(eD) {
                    DealGangCard_ningXiangPaoHuZi(this, eD, 0);
                },
                MJPeng: function(eD) {
                    DealPengCard_ningXiangPaoHuZi(this, eD, 0);
                },
                MJHu: function(eD) {
                    DealHu_ningXiangPaoHuZi(this, eD, 0);
                },
                onlinePlayer: function(eD) {
                    resetHandAfterBegin_ningXiangPaoHuZi();
                    setUserOffline_ningXiangPaoHuZi(this, 0);
                },
                ...
                //     MjClient.majiang.setJiaZhuNum(MjClient.playui._downNode, getUIPlayer_ningXiangPaoHuZi(0));
                //     MjClient.majiang.setJiaZhuNum(MjClient.playui._rightNode, getUIPlayer_ningXiangPaoHuZi(1));
                //     MjClient.majiang.setJiaZhuNum(MjClient.playui._topNode, getUIPlayer_ningXiangPaoHuZi(2));
                // },
                HZPickCard:function (eD) {
                    ...
                    DealShowLastCard_ningXiangPaoHuZi(this, eD, 0);
                },
                HZAddCards:function (eD) {
                    DealAddCard_ningXiangPaoHuZi(this,eD, 0);
                },
        ...
        right: {
            head: {
                chuizi_tips : {
                    _visible:false,
                    // _run: function(){
                    //     setJiachuiImgTips_ningXiangPaoHuZi(this, changeUIOff_ningXiangPaoHuZi(2));
                    // },
                    // _event: {
                    //     initSceneData: function() {
                    //         setJiachuiImgTips_ningXiangPaoHuZi(this, changeUIOff_ningXiangPaoHuZi(2));
                ...
                jiachuiText_tips : {
                    _visible:false,
                    // _run:function() {
                    //     setJiachuiTextTips_ningXiangPaoHuZi(this, changeUIOff_ningXiangPaoHuZi(2));
                    // },
                    // _event:{
                    //     initSceneData: function() {
                    //         setJiachuiTextTips_ningXiangPaoHuZi(this, changeUIOff_ningXiangPaoHuZi(2));
                    //     },
                    //     MJJiazhu: function(msg) {
                    //         var pl = getUIPlayer_ningXiangPaoHuZi(changeUIOff_ningXiangPaoHuZi(2))
                    //         if (msg.uid == pl.info.uid) {
                    //             setJiachuiTextTips_ningXiangPaoHuZi(this, changeUIOff_ningXiangPaoHuZi(2));
                    //         }
                    //     },
                ...
                zhuang: {
                    ...
                    _event: {
                        waitPut: function() {
                            showUserZhuangLogo_ningXiangPaoHuZi(this, changeUIOff_ningXiangPaoHuZi(2));
                        },
                        initSceneData: function() {
                            if (IsArrowVisible_ningXiangPaoHuZi()) showUserZhuangLogo_ningXiangPaoHuZi(this, changeUIOff_ningXiangPaoHuZi(2));
                        },
                        mjhand: function(){
                            if (IsArrowVisible_ningXiangPaoHuZi()) showUserZhuangLogo_ningXiangPaoHuZi(this, changeUIOff_ningXiangPaoHuZi(2));
                        }
                ...
                chatbg: {
                    ...
                    chattext: {
                        _event: {
                            MJChat: function(msg) {
                                showUserChat_ningXiangPaoHuZi(this, changeUIOff_ningXiangPaoHuZi(2), msg);
                            },
                            playVoice: function(voicePath) {
                                MjClient.data._tempMessage.msg = voicePath;
                                showUserChat_ningXiangPaoHuZi(this, changeUIOff_ningXiangPaoHuZi(2), MjClient.data._tempMessage);
                            }
                        }
                    }
                },
                _click: function(btn) {
                    showPlayerInfo_ningXiangPaoHuZi(changeUIOff_ningXiangPaoHuZi(2), btn);
                },
                _event: {
                    loadWxHead: function(d) {
                        setWxHead_ningXiangPaoHuZi(this, d, changeUIOff_ningXiangPaoHuZi(2));
                    },
                    addPlayer: function(eD) {
                        showFangzhuTagIcon_ningXiangPaoHuZi(this,changeUIOff_ningXiangPaoHuZi(2));
                    },
                    removePlayer: function(eD) {
                        showFangzhuTagIcon_ningXiangPaoHuZi(this,changeUIOff_ningXiangPaoHuZi(2));
                    }
                ...
                huxi:{
                    ...
                    _event:{
                        ...
                        initSceneData:function(){
                            this.visible = true;
                            var huXi = UpdateHuXi_ningXiangPaoHuZi(changeUIOff_ningXiangPaoHuZi(2));
                            if(!IsArrowVisible_ningXiangPaoHuZi()){
                                huXi = 0;
                                this.visible = false;
                            }
                ...
                timeImg:{
                    ...
                    // _event:{
                    //     initSceneData: function(eD) {
                    //         this.visible = curPlayerIsMe_ningXiangPaoHuZi(1);
                    //     },
                    //     mjhand: function(eD) {
                    //         this.visible = curPlayerIsMe_ningXiangPaoHuZi(1);
                    //     },
                    //     onlinePlayer: function(eD) {
                    //         this.visible = curPlayerIsMe_ningXiangPaoHuZi(1) && IsArrowVisible_ningXiangPaoHuZi();
                    //     },
                    //     waitPut: function(eD) {
                    //         this.visible = curPlayerIsMe_ningXiangPaoHuZi(1);
                    //     },
                    ...
                    //             stopEffect(playTimeUpEff);
                    //             playTimeUpEff = null;
                    //             if(curPlayerIsMe_ningXiangPaoHuZi(1)){
                    //                 arrowbkNumberUpdate(this);
                    //                 this.setString("00");
                    //             }
                    //         },
                    ...
                    //             stopEffect(playTimeUpEff);
                    //             playTimeUpEff = null;
                    //             var pl = getUIPlayer_ningXiangPaoHuZi(1);
                    //             if(pl && pl.onLine && curPlayerIsMe_ningXiangPaoHuZi(1)){
                    //                 arrowbkNumberUpdate(this);
                    //             }
                    //         },
                    //         roundEnd: function() {
            ...
            ready: {
                _layout: [[0.07, 0.07],[0.5, 0.5],[2.5, 2.5]],
                _run: function() {
                    GetReadyVisible_ningXiangPaoHuZi(this, changeUIOff_ningXiangPaoHuZi(2));
                },
                _event: {
                    startShuffleCards:function (d) {
                        GetReadyVisible_ningXiangPaoHuZi(this, -1);
            ...
            _event: {
                clearCardUI: function() {
                    clearCardUI_ningXiangPaoHuZi(this, changeUIOff_ningXiangPaoHuZi(2));
                },
                clearCardArr: function(){
                    var eatNode = this.getChildByName("eatNode");
                    eatNode.removeAllChildren();
                ...
                initSceneData: function(eD) {
                    SetUserVisible_ningXiangPaoHuZi(this, changeUIOff_ningXiangPaoHuZi(2));
                    InitUserHandUI_ningXiangPaoHuZi(this, changeUIOff_ningXiangPaoHuZi(2));
                    InitUserCoinAndName_ningXiangPaoHuZi(this, changeUIOff_ningXiangPaoHuZi(2));
                    DealOffLineCard_ningXiangPaoHuZi(this,changeUIOff_ningXiangPaoHuZi(2));
                },
                addPlayer: function(eD) {
                    SetUserVisible_ningXiangPaoHuZi(this, changeUIOff_ningXiangPaoHuZi(2));
                    InitUserCoinAndName_ningXiangPaoHuZi(this, changeUIOff_ningXiangPaoHuZi(2));
                ...
                HZNewCard: function(eD) {
                    ...
                    DealNewCard_ningXiangPaoHuZi(this,eD,changeUIOff_ningXiangPaoHuZi(2));
                },
                roundEnd: function() {
                    InitUserCoinAndName_ningXiangPaoHuZi(this, changeUIOff_ningXiangPaoHuZi(2));
                    MjClient.playui.ResetOtherCard(this, changeUIOff_ningXiangPaoHuZi(2));
                },
                waitPut: function(eD) {
                    DealWaitPut(this, eD, 1);
                },
                MJPut: function(eD) {
                    DealPutCard_ningXiangPaoHuZi(this, eD, changeUIOff_ningXiangPaoHuZi(2));
                },
                HZChiCard: function(eD) {
                    DealChiCard_ningXiangPaoHuZi(this, eD, changeUIOff_ningXiangPaoHuZi(2));
                },
                HZGangCard: function(eD) {
                    DealGangCard_ningXiangPaoHuZi(this, eD, changeUIOff_ningXiangPaoHuZi(2));
                },
                MJPeng: function(eD) {
                    DealPengCard_ningXiangPaoHuZi(this, eD, changeUIOff_ningXiangPaoHuZi(2));
                },
                HZWeiCard: function(eD){
                    DealWeiCard_ningXiangPaoHuZi(this, eD, changeUIOff_ningXiangPaoHuZi(2));
                },
                MJHu: function(eD) {
                    DealHu_ningXiangPaoHuZi(this, eD, changeUIOff_ningXiangPaoHuZi(2));
                },
                onlinePlayer: function(eD) {
                    setUserOffline_ningXiangPaoHuZi(this, changeUIOff_ningXiangPaoHuZi(2));
                },
                playerStatusChange: function(eD) {
                    setUserOffline_ningXiangPaoHuZi(this, changeUIOff_ningXiangPaoHuZi(2));
                },
                MJFlower: function(eD) {
                    HandleMJFlower(this, eD, changeUIOff_ningXiangPaoHuZi(2));
                },
                MJTing: function (eD) {
                    HandleMJTing(this, eD, changeUIOff_ningXiangPaoHuZi(2));
                },
                HZPickCard:function (eD) {
                    if(MjClient.rePlayVideo != -1){
                        MjClient.playui._doPickResetHandCardForPlayBack(eD, changeUIOff_ningXiangPaoHuZi(2));
                    }

                    // RemovePutCardOut_ningXiangPaoHuZi(true);
                    // MjClient.playui.ResetHandCard(this,1);

                    DealShowLastCard_ningXiangPaoHuZi(this, eD, changeUIOff_ningXiangPaoHuZi(2));
                },
                HZAddCards:function (eD) {
                    DealAddCard_ningXiangPaoHuZi(this,eD, changeUIOff_ningXiangPaoHuZi(2));
                },
                HZCheckRaise:function (msg) {
                    ShowPutCardIcon_ningXiangPaoHuZi();
                    //MjClient.playui.EatVisibleCheck();
        ...
        left: {
            head: {
                chuizi_tips  : {
                    _visible:false,
                    // _run: function(){
                    //     setJiachuiImgTips_ningXiangPaoHuZi(this, changeUIOff_ningXiangPaoHuZi(3));
                    // },
                    // _event: {
                    //     initSceneData: function() {
                    //         setJiachuiImgTips_ningXiangPaoHuZi(this, changeUIOff_ningXiangPaoHuZi(3));
                ...
                jiachuiText_tips : {
                    _visible:false,
                    // _run:function() {
                    //     setJiachuiTextTips_ningXiangPaoHuZi(this, changeUIOff_ningXiangPaoHuZi(3));
                    // },
                    // _event:{
                    //     initSceneData: function() {
                    //         setJiachuiTextTips_ningXiangPaoHuZi(this, changeUIOff_ningXiangPaoHuZi(3));
                    //     },
                    //     MJJiazhu: function(msg) {
                    //         var pl = getUIPlayer_ningXiangPaoHuZi(changeUIOff_ningXiangPaoHuZi(3))
                    //         if (msg.uid == pl.info.uid) {
                    //             setJiachuiTextTips_ningXiangPaoHuZi(this, changeUIOff_ningXiangPaoHuZi(3));
                    //         }
                    //     },
                ...
                zhuang: {
                    ...
                    _event: {
                        waitPut: function() {
                            showUserZhuangLogo_ningXiangPaoHuZi(this, changeUIOff_ningXiangPaoHuZi(3));
                        },
                        initSceneData: function() {
                            if (IsArrowVisible_ningXiangPaoHuZi()) showUserZhuangLogo_ningXiangPaoHuZi(this, changeUIOff_ningXiangPaoHuZi(3));
                        },
                        mjhand: function(){
                            if (IsArrowVisible_ningXiangPaoHuZi()) showUserZhuangLogo_ningXiangPaoHuZi(this, changeUIOff_ningXiangPaoHuZi(3));
                        }
                ...
                chatbg: {
                    ...
                    chattext: {
                        _event: {
                            MJChat: function(msg) {
                                showUserChat_ningXiangPaoHuZi(this, changeUIOff_ningXiangPaoHuZi(3), msg);
                            },
                            playVoice: function(voicePath) {
                                MjClient.data._tempMessage.msg = voicePath;
                                showUserChat_ningXiangPaoHuZi(this, changeUIOff_ningXiangPaoHuZi(3), MjClient.data._tempMessage);
                            }
                        }
                    }
                },
                _click: function(btn) {
                    showPlayerInfo_ningXiangPaoHuZi(changeUIOff_ningXiangPaoHuZi(3), btn);
                },
                _event: {
                    loadWxHead: function(d) {
                        setWxHead_ningXiangPaoHuZi(this, d, changeUIOff_ningXiangPaoHuZi(3));
                    },
                    addPlayer: function(eD) {
                        showFangzhuTagIcon_ningXiangPaoHuZi(this,changeUIOff_ningXiangPaoHuZi(3));
                    },
                    removePlayer: function(eD) {
                        showFangzhuTagIcon_ningXiangPaoHuZi(this,changeUIOff_ningXiangPaoHuZi(3));
                    }
                ...
                huxi:{
                    ...
                    _event:{
                        ...
                        initSceneData:function(){
                            this.visible = true;
                            var huXi = UpdateHuXi_ningXiangPaoHuZi(changeUIOff_ningXiangPaoHuZi(3));
                            if(!IsArrowVisible_ningXiangPaoHuZi()){
                                huXi = 0;
                                this.visible = false;
                            }
                ...
                timeImg:{
                    ...
                    // _event:{
                    //     initSceneData: function(eD) {
                    //         this.visible = curPlayerIsMe_ningXiangPaoHuZi(2);
                    //     },
                    //     mjhand: function(eD) {
                    //         this.visible = curPlayerIsMe_ningXiangPaoHuZi(2);
                    //     },
                    //     onlinePlayer: function(eD) {
                    //         this.visible = curPlayerIsMe_ningXiangPaoHuZi(2) && IsArrowVisible_ningXiangPaoHuZi();
                    //     },
                    //     waitPut: function(eD) {
                    //         this.visible = curPlayerIsMe_ningXiangPaoHuZi(2);
                    //     },
                    ...
                    //             stopEffect(playTimeUpEff);
                    //             playTimeUpEff = null;
                    //             if(curPlayerIsMe_ningXiangPaoHuZi(2)){
                    //                 arrowbkNumberUpdate(this);
                    //                 this.setString("00");
                    //             }
                    //         },
                    ...
                    //             stopEffect(playTimeUpEff);
                    //             playTimeUpEff = null;
                    //             var pl = getUIPlayer_ningXiangPaoHuZi(2);
                    //             if(pl && pl.onLine && curPlayerIsMe_ningXiangPaoHuZi(2)){
                    //                 arrowbkNumberUpdate(this);
                    //             }
                    //         },
                    //         roundEnd: function() {
            ...
            ready: {
                _layout: [[0.07, 0.07],[0.5, 0.5],[-2.5, 2.5]],
                _run: function() {
                    GetReadyVisible_ningXiangPaoHuZi(this, changeUIOff_ningXiangPaoHuZi(3));
                },
                _event: {
                    startShuffleCards:function (d) {
                        GetReadyVisible_ningXiangPaoHuZi(this, -1);
            ...
            _event: {
                clearCardUI: function() {
                    clearCardUI_ningXiangPaoHuZi(this, changeUIOff_ningXiangPaoHuZi(3));
                },
                clearCardArr: function(){
                    var eatNode = this.getChildByName("eatNode");
                    eatNode.removeAllChildren();
                ...
                initSceneData: function(eD) {
                    SetUserVisible_ningXiangPaoHuZi(this, changeUIOff_ningXiangPaoHuZi(3));
                    InitUserHandUI_ningXiangPaoHuZi(this, changeUIOff_ningXiangPaoHuZi(3));
                    InitUserCoinAndName_ningXiangPaoHuZi(this, changeUIOff_ningXiangPaoHuZi(3));
                    DealOffLineCard_ningXiangPaoHuZi(this,changeUIOff_ningXiangPaoHuZi(3));
                },
                addPlayer: function(eD) {
                    SetUserVisible_ningXiangPaoHuZi(this, changeUIOff_ningXiangPaoHuZi(3));
                    InitUserCoinAndName_ningXiangPaoHuZi(this, changeUIOff_ningXiangPaoHuZi(3));
                ...
                HZNewCard: function(eD) {
                    ...
                    DealNewCard_ningXiangPaoHuZi(this,eD,changeUIOff_ningXiangPaoHuZi(3));
                },
                roundEnd: function() {
                    InitUserCoinAndName_ningXiangPaoHuZi(this,changeUIOff_ningXiangPaoHuZi(3));
                    MjClient.playui.ResetOtherCard(this, changeUIOff_ningXiangPaoHuZi(3));
                },
                waitPut: function(eD) {
                    DealWaitPut(this, eD, changeUIOff_ningXiangPaoHuZi(3));
                },
                MJPut: function(eD) {
                    DealPutCard_ningXiangPaoHuZi(this, eD, changeUIOff_ningXiangPaoHuZi(3));
                },
                HZChiCard: function(eD) {
                    DealChiCard_ningXiangPaoHuZi(this, eD, changeUIOff_ningXiangPaoHuZi(3));
                },
                HZGangCard: function(eD) {
                    DealGangCard_ningXiangPaoHuZi(this, eD, changeUIOff_ningXiangPaoHuZi(3));
                },
                MJPeng: function(eD) {
                    DealPengCard_ningXiangPaoHuZi(this, eD, changeUIOff_ningXiangPaoHuZi(3));
                },
                HZWeiCard: function(eD){
                    DealWeiCard_ningXiangPaoHuZi(this, eD, changeUIOff_ningXiangPaoHuZi(3));
                },
                MJHu: function(eD) {
                    DealHu_ningXiangPaoHuZi(this, eD, changeUIOff_ningXiangPaoHuZi(3));
                },
                onlinePlayer: function(eD) {
                    setUserOffline_ningXiangPaoHuZi(this, changeUIOff_ningXiangPaoHuZi(3));
                },
                playerStatusChange: function(eD) {
                    setUserOffline_ningXiangPaoHuZi(this, changeUIOff_ningXiangPaoHuZi(3));
                },
                MJFlower: function(eD) {
                    HandleMJFlower(this, eD, changeUIOff_ningXiangPaoHuZi(3));
                },
                MJTing: function (eD) {
                    HandleMJTing(this, eD, changeUIOff_ningXiangPaoHuZi(3));
                },
                HZPickCard:function (eD) {
                    if(MjClient.rePlayVideo != -1) {
                        MjClient.playui._doPickResetHandCardForPlayBack(eD, changeUIOff_ningXiangPaoHuZi(3));
                    }
                    // RemovePutCardOut_ningXiangPaoHuZi(true);
                    // MjClient.playui.ResetHandCard(this,2);

                    DealShowLastCard_ningXiangPaoHuZi(this, eD, changeUIOff_ningXiangPaoHuZi(3));
                },
                HZAddCards:function (eD) {
                    DealAddCard_ningXiangPaoHuZi(this,eD, changeUIOff_ningXiangPaoHuZi(3));
                },
                HZCheckRaise:function (msg) {
                    ShowPutCardIcon_ningXiangPaoHuZi();
                    //MjClient.playui.EatVisibleCheck();
        ...
        xing: {
            ...
            head: {
                chuizi_tips : {
                    _visible:false,
                    // _run: function(){
                    //     setJiachuiImgTips_ningXiangPaoHuZi(this, changeUIOff_ningXiangPaoHuZi(1));
                    // },
                    // _event: {
                    //     initSceneData: function() {
                    //         setJiachuiImgTips_ningXiangPaoHuZi(this, changeUIOff_ningXiangPaoHuZi(1));
                ...
                jiachuiText_tips : {
                    _visible:false,
                    // _run:function() {
                    //     setJiachuiTextTips_ningXiangPaoHuZi(this, changeUIOff_ningXiangPaoHuZi(1));
                    // },
                    // _event:{
                    //     initSceneData: function() {
                    //         setJiachuiTextTips_ningXiangPaoHuZi(this, changeUIOff_ningXiangPaoHuZi(1));
                    //     },
                    //     MJJiazhu: function(msg) {
                    //         var pl = getUIPlayer_ningXiangPaoHuZi(changeUIOff_ningXiangPaoHuZi(1))
                    //         if (msg.uid == pl.info.uid) {
                    //             setJiachuiTextTips_ningXiangPaoHuZi(this, changeUIOff_ningXiangPaoHuZi(1));
                    //         }
                    //     },
                ...
                zhuang: {
                    ...
                    _event: {
                        waitPut: function() {
                            if(MjClient.MaxPlayerNum_ningXiangPaoHuZi == 4 ) {
                                showUserZhuangLogo_ningXiangPaoHuZi(this, changeUIOff_ningXiangPaoHuZi(1));
                            }
                        },
                        initSceneData: function() {
                            if(MjClient.MaxPlayerNum_ningXiangPaoHuZi == 4 ) {
                                if (IsArrowVisible_ningXiangPaoHuZi()) showUserZhuangLogo_ningXiangPaoHuZi(this, changeUIOff_ningXiangPaoHuZi(1));
                            }
                        },
                        mjhand: function(){
                            if(MjClient.MaxPlayerNum_ningXiangPaoHuZi == 4 ) {
                ...
                chatbg: {
                    ...
                    chattext: {
                        _event: {
                            MJChat: function(msg) {
                                if(MjClient.MaxPlayerNum_ningXiangPaoHuZi == 4) {
                                    showUserChat_ningXiangPaoHuZi(this, changeUIOff_ningXiangPaoHuZi(1), msg);
                                }
                            },
                            playVoice: function(voicePath) {
                                if(MjClient.MaxPlayerNum_ningXiangPaoHuZi == 4) {
                                    MjClient.data._tempMessage.msg = voicePath;
                                    showUserChat_ningXiangPaoHuZi(this, changeUIOff_ningXiangPaoHuZi(1), MjClient.data._tempMessage);
                                }
                            }
                        }
                    }
                ...
                _click: function(btn) {
                    if(MjClient.MaxPlayerNum_ningXiangPaoHuZi == 4) {
                        showPlayerInfo_ningXiangPaoHuZi(changeUIOff_ningXiangPaoHuZi(1), btn);
                    }
                },
                _event: {
                    loadWxHead: function(d) {
                        if(MjClient.MaxPlayerNum_ningXiangPaoHuZi == 4) {
                            setWxHead_ningXiangPaoHuZi(this, d, changeUIOff_ningXiangPaoHuZi(1));
                        }
                    },
                    addPlayer: function(eD) {
                        showFangzhuTagIcon_ningXiangPaoHuZi(this,changeUIOff_ningXiangPaoHuZi(1));
                    },
                    removePlayer: function(eD) {
                        showFangzhuTagIcon_ningXiangPaoHuZi(this,changeUIOff_ningXiangPaoHuZi(1));
                    }
                ...
                huxi:{
                    ...
                    _event:{
                        ...
                        initSceneData:function(){
                            if(MjClient.MaxPlayerNum_ningXiangPaoHuZi == 4) {
                                this.visible = true;
                                var huXi = UpdateHuXi_ningXiangPaoHuZi(changeUIOff_ningXiangPaoHuZi(1));
                                if (!IsArrowVisible_ningXiangPaoHuZi()) {
                                    huXi = 0;
                                    this.visible = false;
                                }
            ...
            ready: {
                ...
                _run: function() {
                    if(MjClient.MaxPlayerNum_ningXiangPaoHuZi == 4) {
                        GetReadyVisible_ningXiangPaoHuZi(this, changeUIOff_ningXiangPaoHuZi(1));
                    }
                },
                _event: {
                    startShuffleCards:function (d) {
            ...
            _event: {
                clearCardUI: function() {
                    if(MjClient.MaxPlayerNum_ningXiangPaoHuZi == 4) {
                        clearCardUI_ningXiangPaoHuZi(this, changeUIOff_ningXiangPaoHuZi(1));
                    }
                },
                clearCardArr: function(){
                    if(MjClient.MaxPlayerNum_ningXiangPaoHuZi == 4) {
                ...
                initSceneData: function(eD) {
                    if(MjClient.MaxPlayerNum_ningXiangPaoHuZi == 4) {
                        SetUserVisible_ningXiangPaoHuZi(this, changeUIOff_ningXiangPaoHuZi(1));
                        InitUserHandUI_ningXiangPaoHuZi(this, changeUIOff_ningXiangPaoHuZi(1));
                        InitUserCoinAndName_ningXiangPaoHuZi(this, changeUIOff_ningXiangPaoHuZi(1));
                        DealOffLineCard_ningXiangPaoHuZi(this, changeUIOff_ningXiangPaoHuZi(1));
                    }
                },
                addPlayer: function(eD) {
                    if(MjClient.MaxPlayerNum_ningXiangPaoHuZi == 4) {
                ...
                HZNewCard: function(eD) {
                    if(MjClient.MaxPlayerNum_ningXiangPaoHuZi == 4) {
                        ...
                        DealNewCard_ningXiangPaoHuZi(this, eD, changeUIOff_ningXiangPaoHuZi(1));
                    }
                },
                roundEnd: function() {
                    if(MjClient.MaxPlayerNum_ningXiangPaoHuZi == 4) {
                        InitUserCoinAndName_ningXiangPaoHuZi(this, changeUIOff_ningXiangPaoHuZi(1));
                        MjClient.playui.ResetOtherCard(this, changeUIOff_ningXiangPaoHuZi(1));
                    }
                },
                waitPut: function(eD) {
                    if(MjClient.MaxPlayerNum_ningXiangPaoHuZi == 4) {
                        DealWaitPut(this, eD, changeUIOff_ningXiangPaoHuZi(1));
                    }
                },
                MJPut: function(eD) {
                    if(MjClient.MaxPlayerNum_ningXiangPaoHuZi == 4) {
                        DealPutCard_ningXiangPaoHuZi(this, eD, changeUIOff_ningXiangPaoHuZi(1));
                    }
                },
                HZChiCard: function(eD) {
                    if(MjClient.MaxPlayerNum_ningXiangPaoHuZi == 4) {
                        DealChiCard_ningXiangPaoHuZi(this, eD, changeUIOff_ningXiangPaoHuZi(1));
                    }
                },
                HZGangCard: function(eD) {
                    if(MjClient.MaxPlayerNum_ningXiangPaoHuZi == 4) {
                        DealGangCard_ningXiangPaoHuZi(this, eD, changeUIOff_ningXiangPaoHuZi(1));
                    }
                },
                MJPeng: function(eD) {
                    if(MjClient.MaxPlayerNum_ningXiangPaoHuZi == 4) {
                        DealPengCard_ningXiangPaoHuZi(this, eD, changeUIOff_ningXiangPaoHuZi(1));
                    }
                },
                HZWeiCard: function(eD){
                    if(MjClient.MaxPlayerNum_ningXiangPaoHuZi == 4) {
                        DealWeiCard_ningXiangPaoHuZi(this, eD, changeUIOff_ningXiangPaoHuZi(1));
                    }
                },
                MJHu: function(eD) {
                    if(MjClient.MaxPlayerNum_ningXiangPaoHuZi == 4) {
                        DealHu_ningXiangPaoHuZi(this, eD, changeUIOff_ningXiangPaoHuZi(1));
                    }
                },
                onlinePlayer: function(eD) {
                    if(MjClient.MaxPlayerNum_ningXiangPaoHuZi == 4) {
                        setUserOffline_ningXiangPaoHuZi(this, changeUIOff_ningXiangPaoHuZi(1));
                    }
                },
                playerStatusChange: function(eD) {
                    if(MjClient.MaxPlayerNum_ningXiangPaoHuZi == 4) {
                ...
                MJFlower: function(eD) {
                    if(MjClient.MaxPlayerNum_ningXiangPaoHuZi == 4) {
                        HandleMJFlower(this, eD, changeUIOff_ningXiangPaoHuZi(1));
                    }
                },
                MJTing: function (eD) {
                    if(MjClient.MaxPlayerNum_ningXiangPaoHuZi == 4) {
                        HandleMJTing(this, eD, changeUIOff_ningXiangPaoHuZi(1));
                    }
                },
                HZPickCard:function (eD) {
                    if(MjClient.MaxPlayerNum_ningXiangPaoHuZi == 4) {
                        if (MjClient.rePlayVideo != -1 ) {
                            var pl = getUIPlayer_ningXiangPaoHuZi(changeUIOff_ningXiangPaoHuZi(1));
                            if (eD.uid == pl.info.uid) {
                                MjClient.OtherHandArr[changeUIOff_ningXiangPaoHuZi(1)] = MjClient.majiang.sortHandCardSpecial(pl.mjhand)
                            }
                        }
                        DealShowLastCard_ningXiangPaoHuZi(this, eD, changeUIOff_ningXiangPaoHuZi(1));
                    }
                },
                HZAddCards:function (eD) {
                    if(MjClient.MaxPlayerNum_ningXiangPaoHuZi == 4) {
                        DealAddCard_ningXiangPaoHuZi(this, eD, changeUIOff_ningXiangPaoHuZi(1));
                    }
                },
                HZCheckRaise:function (msg) {
                    if(MjClient.MaxPlayerNum_ningXiangPaoHuZi == 4) {
        ...
        eat: {
            chi: {
                ...
                _touch: function(btn, eT) {
                    if (eT == 2){
                        if(ziPai.getHasFastEat()){
                            ...
                            if (chiSet.length == 1) {
                                ...
                                if (pl.mjhand.indexOf(putCard) < 0 || copy.indexOf(putCard) >= 0) {
                                    ...
                                    commitEatCards_ningXiangPaoHuZi(eatCards, null);
                        ...
                        showSelectEatCards_ningXiangPaoHuZi(this,btn.tag);
                        ...
                        // MJChiCardchange_ningXiangPaoHuZi();
                    }
                }
            },
            noTing : {
                        ...
                        if (eT == 2){
                            if ((MjClient.data.sData.players[SelfUid()].eatFlag & 32) > 0) {
                                MjClient.showMsg("选择碰后视为过胡，确定碰吗？", function() {
                                    HZPengToServer_ningXiangPaoHuZi();
            ...
            hu: {
                ...
                _touch: function(btn, eT) {
                    if (eT == 2) {
                        MJHuToServer_ningXiangPaoHuZi();
                        setChiVisible_ningXiangPaoHuZi();//add by maoyu
                    }
                }
            },
            cancel: {
                ...
                _touch: function(btn, eT) {
                    if (eT == 2) {
                        ...
                        resetChiParam_ningXiangPaoHuZi();
                        MjClient.playui.EatVisibleCheck();
            ...
            wangDiao:{
                ...
                _touch: function(btn, eT) {
                    console.log(">>>> lf，点击碰按钮");
                    if (eT == 2) {
                        HZWangChuangToServer_ningXiangPaoHuZi(1);
                        setChiVisible_ningXiangPaoHuZi();//add by maoyu
                    }
                }
            },
            wangChuang:{
                ...
                _touch: function(btn, eT) {
                    console.log(">>>> lf，点击碰按钮");
                    if (eT == 2) {
                        HZWangChuangToServer_ningXiangPaoHuZi(2);
                        setChiVisible_ningXiangPaoHuZi();//add by maoyu
                    }
                }
            },
            wangZha:{
                ...
                _touch: function(btn, eT) {
                    console.log(">>>> lf，点击碰按钮");
                    if (eT == 2) {
                        HZWangChuangToServer_ningXiangPaoHuZi(4);
                        setChiVisible_ningXiangPaoHuZi();//add by maoyu
                    }
                }
            },
                    ...
                    this.adaptChiLayout = function() {
                        ...
                        if (cards_option.length == 0) { // 吃牌选好了
                            if ((MjClient.data.sData.players[SelfUid()].eatFlag & 32) > 0) {
                                MjClient.showMsg("选择吃后视为过胡，确定吃吗？", function() {
                                    ...
                                    HZChiToServer_ningXiangPaoHuZi(cards_select[0], biCards);
                                    this.visible = false;
                                }.bind(this), function() {
                                    cards_select.pop();
                                    this.adaptChiLayout();
                        ...
                        for (var i = 0; i < cards_select.length; i++) {
                            ...
                            for (var j = 0; j < row.length; j++) {
                                ...
                                chiCardImg.loadTexture(MjClient.cardPath_ningXiangPaoHuZi + "out" + row[j] + ".png");
        ...
        voice_btn: {
            ...
            _event: {
                ...
                downAndPlayVoice: function(msg) {
                    ...
                    downAndPlayVoice_ningXiangPaoHuZi(msg.uid, msg.msg);
                }
            }
        },
        cutLine:{
    ...
    ctor: function() {
        this._super();
        ziPai.initZiPaiData();
        var playui = ccs.load("Play_NingXiangPaoHuZi.json");
        playMusic("bgFight");
        var tData = MjClient.data.sData.tData;
        MjClient.MaxPlayerNum_ningXiangPaoHuZi = tData.maxPlayer;
        ...
        if (MjClient.MaxPlayerNum_ningXiangPaoHuZi == 2){
            ...
            this.jsBind.right = PlayLayer_ningXiangPaoHuZi.jsBindRightCopy;
        }
        ...
        this.tingLayer = new NingXiangPaoHuZiTingLayer();
    ...
    resetJiaZhuTip: function()
    {
        ...
        for (var i = 0; i < MjClient.MaxPlayerNum_ningXiangPaoHuZi; i ++)
        {
            var pl = getUIPlayer_ningXiangPaoHuZi(i);
            if (!pl) 
                continue;
...
PlayLayer_ningXiangPaoHuZi.jsBindRightCopy = PlayLayer_ningXiangPaoHuZi.prototype.jsBind.right;

//重置手牌的顺序
PlayLayer_ningXiangPaoHuZi.prototype.ResetHandCard = function(posNode,off, needAction, isDelay){

    needAction = needAction === undefined ? false : needAction;
    // needAction = false;

    if(off == 0){
        resetHandCard_ningXiangPaoHuZiEx(posNode,off, needAction, isDelay);
...
//回放时其他家补牌重置手牌的处理
PlayLayer_ningXiangPaoHuZi.prototype._doPickResetHandCardForPlayBack = function(msg, off){
    var pl = getUIPlayer_ningXiangPaoHuZi(off);
...
PlayLayer_ningXiangPaoHuZi.prototype._doMovetoAction = function(node,endP){
    node.stopAllActions();
    var ac = cc.moveTo(0.3,endP);
    node.runAction(ac);
};

PlayLayer_ningXiangPaoHuZi.prototype.ResetOtherCard = function(posNode, off) {
    //添加吃、碰、跑、偎、提
    var sData = MjClient.data.sData;
    var tData = null;
    if (sData) {
    ...
    if (mjSortArr) {
            ...
            for (var i = 0; i < childList.length; i++) {
                ...
                if(tData.maxPlayer == 4){
                    if((off <= 1 && i != childList.length - 1) ||
                        (off >= 2 && i != 0)){
                        child.loadTexture(MjClient.cardPath_ningXiangPaoHuZi + "huxiBG.png");
                    }
                }else{
        ...
        for (var k = 0; k < mjSortArr.length; k++) {
            ...
            if (name == "mjgang1") {
                ...
                for (var i = 0; i < 4; i++) {
                    addEatCard_ningXiangPaoHuZi(posNode, "mjgang1" + k, cardNum, off);
                }
            ...
            if (name == "mjwei") {
                var cardNum = pl.mjwei[pos];
                for (var i = 0; i < 3; i++) {
                    addEatCard_ningXiangPaoHuZi(posNode, "mjwei" + k, cardNum, off);
                }
            ...
            if (name == "mjgang0") {
                ...
                for (var i = 0; i < 4; i++) {
                    addEatCard_ningXiangPaoHuZi(posNode, "mjgang0" + k, cardNum, off);
                }
            }
            //碰
            if (name == "mjpeng") {
                ...
                for (var i = 0; i < 3; i++) {
                    addEatCard_ningXiangPaoHuZi(posNode, "mjpeng" + k, cardNum, off);
                }
            }
            //吃
            if (name == "mjchi") {
                ...
                for (var i = 0; i < eatCards.length; i++) {
                    addEatCard_ningXiangPaoHuZi(posNode, "mjchi" + k, eatCards[i], off);
                }
                ...
                if (biCards && biCards.length > 0) {
                    for (var i = 0; i < biCards.length; i++) {
                        ...
                        for (var m = 0; m < biArr.length; m++) {
                            addEatCard_ningXiangPaoHuZi(posNode, "mjbi" + k + i, biArr[m], off);
                        }
                    }
                }
    ...
    //    var outCard = getNewCard_ningXiangPaoHuZi(pl.mjput[i], 2, off);
    //    if(off == 0){
    //        outCard.anchorX = 1;
    //        outCard.anchorY = 0;
...
PlayLayer_ningXiangPaoHuZi.prototype.ResetPutCard = function(posNode,off){
    //添加打出的牌
    var pl = getUIPlayer_ningXiangPaoHuZi(off);
    if (!pl) return;
    var outNode = posNode.getChildByName("outNode");
    ...
    for (var i = 0; i < pl.mjput.length; i++) {
        ...
        setPutCardPos_ningXiangPaoHuZi(outNode, outCard, i, off);
...
//重置牌的顺序
PlayLayer_ningXiangPaoHuZi.prototype.CardLayoutRestore = function(posNode,off){
    if(MjClient.rePlayVideo == -1){
...
// 判断吃、碰、跑、偎、提、胡的状态
PlayLayer_ningXiangPaoHuZi.prototype.EatVisibleCheck = function(off){
    var eat = MjClient.playui.jsBind.eat;
    ...
    //吃碰杠胡node
    var vnode = [];
    //自摸getUIPlayer_ningXiangPaoHuZi
    if(tData.tState != TableState.roundFinish && pl.mjState != TableState.roundFinish){
        if(IsTurnToMe()){
            ...
            if(pl.eatFlag & 4){
                ...
                if(!canHu){
                    // HZGangToServer_ningXiangPaoHuZi(2);
                    return;
                }
            }
            //吃
...
PlayLayer_ningXiangPaoHuZi.prototype.showAndHideHeadEffect = function(){
    ...
    else if (curPlayerIsMe_ningXiangPaoHuZi(1)){
        curPlayerNode = arr[1].getChildByName("head");
    }else if (curPlayerIsMe_ningXiangPaoHuZi(2)){
        curPlayerNode = arr[2].getChildByName("head");
    }else if (curPlayerIsMe_ningXiangPaoHuZi(3)){
