...
var _isShowShaiziAni_MLHZ_AI = false; //为了控制mjhand之后播骰子动画时newCard的状态显示
//向服务器发送 过消息
MjClient.MJPass2NetForMLHZ_AI = function()
{
   // console.log(">>>>>>>>>普通  过 <<<<<<<<");
    cc.log("====================send======pass=====");
...
var showShaiziAni_MLHZ_AI = false;
function showShaiziAni_MLHZ_AI_old(poff)
{
    if(poff == 0) {
        ...
        if(MjClient.rePlayVideo != -1)//回放
        {
            for (var off = 0; off < 4; off++) {
                ...
                InitUserHandUI_MLHZ_AI(_node, off);
            }
            playEffect("shuffle");
        ...
        firstFrame.runAction(cc.sequence(playSoundFunc, animate,showPonitFunc,cc.delayTime(0.5),cc.callFunc(function(){
            //发牌
            _isShowShaiziAni_MLHZ_AI = false;
...
function updateHeadUI_MLHZ_AI(node, off)
{      
    cc.log('updateHeadUI_MLHZ_AI', off)
    var pl = getUIPlayer(off);
    if(!pl) return;
    showHandCard_MLHZ_AI(node, off);
    MjClient.playui.CardLayoutRestore(node, off);
    cc.log('updateHeadUI_MLHZ_AI update success', off)
...
function SetUserVisible_MLHZ_AI(node, off)
{
    if (_isShowShaiziAni_MLHZ_AI)  // 解决摇骰子时断线重连调用本函数，会手牌重复的bug by cyc
        return;
...
function showHandCard_MLHZ_AI(node, off)
{
    var sData = MjClient.data.sData;
    var tData = sData.tData;
    var pl = getUIPlayer(off);
...
function InitUserHandUI_MLHZ_AI(node, off)
{
    var sData = MjClient.data.sData;
    var tData = sData.tData;
    var pl = getUIPlayer(off);
    ...
    if (SelfUid() == pl.info.uid) {
        if (tData.tState == TableState.waitJiazhu) {
            if (pl.mjState == TableState.waitJiazhu && !MjClient.playui.getChildByName("jiaZhuCS")) {
                var layer = new jiaZhuCS(function(){
                    //弹窗等待
                    MjClient.playui._jiazhuWait.visible = true;
                    MjClient.data.sData.players[SelfUid()].mjState = TableState.isReady;
                });
...
function initFlower_MLHZ_AI() {
    initFlower(false, false);
}


var PlayLayer_MLHZ_AI = cc.Layer.extend({
    jsBind: {
        _event: {
            mjhand: function() {
            ...
            roundEnd: function() {
                ...
                function showEndCards()
                {
                    ...
                    if (tData.fieldId) {
                        ...
                        self.addChild(new EndOneView_MLHZ_AI(),500);
                    }
                }
                function delayExe()
                {
      ...
      //           }
        ...
        BtnPutCard:{ //add by  sking for put card button
            ...
            _event:{
                ...
                waitPut: function() {
                    if (_isShowShaiziAni_MLHZ_AI) return;
            ...
            changeui: {
                ...
                changeuibg: {
                    ...
                    guobg: {
                        guo: {
                            _touch: function(btn, eT) {
                                if (eT == 2) MjClient.MJPass2NetForMLHZ_AI();
                            }
                        },
                        fanhui: {
                            _touch: function(btn, et) {
    ...
    ctor: function() {
        this._super();
        var playui = ccs.load(res.Play_MLHZ_json);
    ...
    {
        var tData = MjClient.data.sData.tData;
        if (!tData.areaSelectMode.jiapiao && !tData.areaSelectMode.piaofen)
            return;
        ...
        for (var i = 0; i < MjClient.MaxPlayerNum; i ++)
        {
            ...
            tipNode.setString(pl.jiazhuNum > 0 ? "飘" + pl.jiazhuNum + "分" : "不飘分");
...
PlayLayer_MLHZ_AI.prototype.CardLayoutRestore = function(node, off, isNewCard)
{
...
// 判断吃碰杠胡的状态
PlayLayer_MLHZ_AI.prototype.EatVisibleCheck = function()
{
    var eat = MjClient.playui.jsBind.eat;
    ...
    showTingCardsSignMLHZ_AI();
}
PlayLayer_MLHZ_AI.prototype.getIsZhongBird = function(cd,birdArr){
...
PlayLayer_MLHZ_AI.prototype.jiaozhuToPiaoniao = function(jiazhuLayer, msg) {
    var tData = MjClient.data.sData.tData;
...
function addTingCardsSignMLHZ_AI(children, tingCard, pl) {
    var tingMostCard = calculateTotalCardsOfTingHu(tingCard, pl.mjhand);
    for (var i = 0; i < children.length; i++) {
        if (children[i].name == "mjhand")
        {
            if (Object.keys(tingCard).length > 0)
            {
                if (children[i].tag in tingCard)
                {
                    if(!children[i].getChildByName("tingSign"))
                    {
                        ...
                        if (tingMostCard[children[i].tag] > 0) {
                            tingSign.loadTexture("playing/other/tingcardmost.png");
                        } else {
...
function removeTingCardsSignMLHZ_AI(children) {
    cc.log("get in ---------");
    for (var i = 0; i < children.length; i++) {
        if (children[i].name == "mjhand") {
...
function getTingCardsMLHZ_AI(children, card) {
    var canTingCards = {};
    ...
    // for (var i = 0; i < children.length; i++) {
    //     if (children[i].name == "mjhand") {
    //         cc.log("var tingCards = getCheckTingHuCards(children[i].tag, card);", children[i].tag)
    //         if (MjClient.majiang.canHu(card, 200, MjClient.data.sData.tData.hunCard)) {
    //             canTingCards[children[i].tag] = 1;
    //         }
    //         // var tingCards = getCheckTingHuCards(children[i].tag, card);
...
function showTingCardsSignMLHZ_AI() {
    if (!MjClient.data.sData.tData.areaSelectMode.isOpenTingTip)
        return;
    ...
    // 添加听牌的箭头
    cc.log("听牌判断------------")
    removeTingCardsSignMLHZ_AI(children);
    MjClient.allCanTingCards = {};
    if (MjClient.canTingCards && Object.keys(MjClient.canTingCards).length > 0) {
        cc.log("pl ========="+ JSON.stringify(pl));
        addTingCardsSignMLHZ_AI(children, MjClient.canTingCards, pl);
    }
}

//设置听牌
PlayLayer_MLHZ_AI.prototype.setTingCards = function(node, tingSet) {
    node.zIndex = 500;
    node.visible = true;
...
//设置可听的牌及其数量
PlayLayer_MLHZ_AI.prototype.setCurrentTingNum = function(tingSet) {
