...
//向服务器发送 过消息
MjClient.MJPass2NetForjinzhonglisi = function()
{
   // console.log(">>>>>>>>>普通  过 <<<<<<<<");
    cc.log("====================MJPass2NetForjinzhonglisi======pass=====");
...
function SetUserVisible_jinzhonglisi(node, off)
{
    //var sData = MjClient.data.sData;
    //return;
...
//处理出牌,放一张牌，打牌动作
function DealMJPut_jinzhonglisi(node, msg, off, outNum)
{
    return DealMJPut_shanXiApp(node, msg, off, outNum);
}



function InitUserHandUI_jinzhonglisi(node, off)
{
    var sData = MjClient.data.sData;
    var tData = sData.tData;
    var pl = getUIPlayer(off);
    ...
    for(var i = 0; i < pl.mjpeng.length; i++)
    {
        ...
        for(var j = 0; j < 3; j++)
        {
            ...
            {
                _pengCardNode = getNewCard(node, "up", "peng", pl.mjpeng[i], off, "heng", "heng");
                setCardArrow(_pengCardNode, offIdx, off);
            }
            else
            {
                _pengCardNode = getNewCard(node, "up", "peng", pl.mjpeng[i], off);
            }

            cc.log("=================pl.pengFourCounts = " + JSON.stringify(pl.pengFourCounts));
            if(_pengCardNode && pl.pengFourCounts && j < pl.pengFourCounts[pl.mjpeng[i]])
            {
    ...
    for(var i = 0; i < pl.mjgang0.length; i++)
    {
        ...
        for(var j = 0; j < 4; j++)
        {
            if(j < 3)
            {
                if( (j % 3 == 2 - offIdx && off % 3 == 0) || (j % 3 == offIdx && off % 3 != 0) )
                {
                    _cardNode = getNewCard(node, "up", "gang0", pl.mjgang0[i], off, "heng", "heng");
                ...
                {
                    _cardNode = getNewCard(node, "up", "gang0", pl.mjgang0[i], off);
                }
            }
            else
            {
                _cardNode = getNewCard(node, "up", "gang0", pl.mjgang0[i], off, "isgang4");//最后一张牌放上面
                _cardNode.tag = pl.mjgang0[i];
    ...
    for(var i = 0; i < pl.mjgang1.length; i++)
    {
        for(var j = 0; j < 4; j++)
        {
            if(j == 3)
            {
                _cardNode = getNewCard(node, "down", "gang1", 0, off, "isgang4").tag = pl.mjgang1[i];
            }
            else
            {
                _cardNode = getNewCard(node, "up", "gang1", pl.mjgang1[i], off);
            }

            // cc.log("msg.gangFourCounts[msg.card] = " + pl.gangFourCounts[pl.mjgang1[i]]);
            cc.log("msg.gangFourCounts00000000000= " + JSON.stringify(pl.gangFourCounts));
    ...
    for(var i = 0; i < pl.mjput.length; i++)
    {
        ...
        DealMJPut_jinzhonglisi(node, msg, off, i);
    }
    ...
    if(MjClient.rePlayVideo == -1)//表示正常游戏
    {
        ...
        cc.log("=============111===pl.mjhandFour================= " + pl.mjhandFour);
        if(pl.mjhand && off === 0)
        {
    ...
    MjClient.playui.CardFourLayoutRestore(node,off);
    MjClient.playui.CardLayoutRestore(node, off);
}



function initFlower_jinzhonglisi() {
...
var PlayLayer_jinzhonglisi = cc.Layer.extend({
    jsBind: {
        _event: {
            mjhand: function() {
            ...
            roundEnd: function() {
                ...
                function delayExe()
                {
                    ...
                    self.addChild(new EndOneView_jinzhonglisi(),500);
                }
        ...
        roundnumImg: {
            _run:function () {
                ...
                if(MjClient.MaxPlayerNum === 3){
                    setWgtLayout(this,[0.1, 0.1], [0.5, 0.76], [0, 0]);
                }
            },
            _event: {
        ...
        cardNumImg: {
            _run:function () {
                ...
                setWgtLayout(this,[0.1, 0.1], [0.5, 0.5], [1.2, 1.0]);
                if(MjClient.MaxPlayerNum === 3){
                    setWgtLayout(this,[0.1, 0.1], [0.5, 0.84], [0, 0]);
                }
            },
            _event: {
                initSceneData: function(eD) {
            ...
            changeui: {
                ...
                changeuibg: {
                    ...
                    guobg: {
                        guo: {
                            _touch: function(btn, eT) {
                                if (eT == 2) MjClient.MJPass2NetForjinzhonglisi();
                            }
                        },
                        fanhui: {
                            _touch: function(btn, et) {
        ...
        li_bg: {
            ...
            _run: function() {
                ...
                var _liPos = cc.p(57.16,19.74);
                var _hunPos = cc.p(20.16,19.74);
                this.visible = false;

                MjClient.playui._isHun = util.localStorageEncrypt.getBoolItem("_LI_SI_IS_HUN_", false);
                if(MjClient.playui._isHun)
                {
                    this.getChildByName("Image_btn").setPosition(_liPos);
                }
                else
                {
                    this.getChildByName("Image_btn").setPosition(_hunPos);
                }
                MjClient.playui.CardLayoutRestore(MjClient.playui._downNode, 0);
            },
            _click: function(btn) {
                ...
                if(MjClient.playui._isHun)
                {
                    MjClient.native.umengEvent4CountWithProperty("Fangjiannei_Hun", {uid:SelfUid()});
                    btn.getChildByName("Image_btn").setPosition(_hunPos);
                    MjClient.playui._isHun = false;
                }
                else
                {
                    MjClient.native.umengEvent4CountWithProperty("Fangjiannei_Li", {uid:SelfUid()});
                    btn.getChildByName("Image_btn").setPosition(_liPos);
                    MjClient.playui._isHun = true;
                }
                COMMON_UI.mjhandSizeSet();
                util.localStorageEncrypt.setBoolItem("_LI_SI_IS_HUN_", MjClient.playui._isHun);
                MjClient.playui.CardLayoutRestore(MjClient.playui._downNode, 0);
            },
            _event: {
                clearCardUI:function()
    ...
    ctor: function() {
        this._super();
        var playui = ccs.load("Play_jinzhonglisi.json");
...
PlayLayer_jinzhonglisi.prototype.CardLayoutRestore = function(node, off)
{
    // node 是克隆新建的一个麻将节点 by sking
    var pl = getUIPlayer(off);//获取玩家信息.off 为0 ，就是自己得信息，能看到自己摸牌 by sking
    if(!pl) return;
    if(off == 0 && MjClient.playui._isHun)
    {
        return MjClient.playui.CardFourLayoutRestore(node, off);
    }

    cc.log("==============CardLayoutRestore sking ================ off = " + off);
    ...
    for(var i = 0; i < children.length; i++) //children 为 "down" 节点下的字节点
    {
        ...
        if(ci.name == "mjhand")
        {
            if(newC == null && newVal == ci.tag && !ci.isFour)
            {
    ...
    if((MjClient.rePlayVideo == -1 && off == 0) ||  MjClient.rePlayVideo != -1 )
    {
    ...
    for(var i = 0; i < orders.length; i++)
    {
        ...
        if(off % 2 == 0)//自己或者对家
        {
            ...
            if(off == 0)
            {
                /*
                 ting的情况下，将麻将置灰
                 */
                cc.log("--------ci.isFour = "+ ci.isFour);
                var isGray =  (pl.isTing /*|| ci.isFour*/) && ci.name == "mjhand";
                //if(ci.name == "mjhand")
                ...
                if(MjClient.clickTing)
                {
                    if (ci.name == "mjhand")
                    {
                        if(MjClient.canTingCards[ci.tag] && ci.isFour)
                        {
                            ci.setColor(cc.color(255, 255, 255));
                            if (!hasUp) {
                                ci.y += 20;
            ...
            //if(ci.isFour) ci.setColor(cc.color(230, 0, 230));
        }
        else
        {
            if(i != 0)
        ...
        if(off != 0 && ci.name == "standPri" )
        {
            if(off == 3)
            {
                var length = orders.length - i -1;

                if(length >= 0 && length < pl.cardFourCount)
                {
                    ci.setColor(cc.color(150, 150, 150));
                }
                else {
                    ci.setColor(cc.color(255, 255, 255));
                }
    ...
    // if(off == 0)
    {
        ...
        {
            var ci = children[i];
            //if(ci.name == "mjhand" || ci.name == "mjhand")
            {

                //添加立子图
                if(ci.isFour)
                {
                    var _lizi =  ci.getChildByName("lizi");
                    if(!_lizi)
                    {
                        cc.log("===============添加图片例子 ==== "); //"playing/MJ/lizi.png"
                        _lizi = new ccui.ImageView();
                        _lizi.loadTexture("playing/MJ/lizi.png");
                        _lizi.setName("lizi");
                        _lizi.setPosition(ci.getContentSize().width/2,ci.getContentSize().height/2);
                        ci.addChild(_lizi,100);

                        if(ci.name == "gang0" || ci.name == "gang1" || ci.name == "peng" || ci.name == "mjhand_replay")
                        {
                            if(off == 0)
                            {
                                if(ci.name == "gang0" || ci.name == "gang1" || ci.name == "peng" )
                                {
                                    ...
                                    _lizi.setPosition(ci.getContentSize().width/2,ci.getContentSize().height/2*1);
                                }
                            }
                            else if(off == 1)
                            {
                                _lizi.setPosition(ci.getContentSize().width/2*0.82,ci.getContentSize().height/2*1.3);
                                _lizi.setRotation(-90);
                            }
                            else if(off == 3)
                            {
                                _lizi.setPosition(ci.getContentSize().width/2*1.2,ci.getContentSize().height/2*1.3);
                                _lizi.setRotation(90);
                            }
                            else if(off == 2)
                            {
                                _lizi.setPosition(ci.getContentSize().width/2,ci.getContentSize().height/2*1.35);
                            }
                        }
                    }
                    if (cc.colorEqual(ci.getColor(), cc.color(255, 255, 255))
                        && (!MjClient.clickTing || (!MjClient.canTingCards[ci.tag] && MjClient.clickTing)))
                    {
                        ci.setColor(cc.color(230, 230, 230));
                        ci.addTouchEventListener(function () {});
                    }
                }
                else
                {
                    if(cc.sys.isObjectValid(ci.getChildByName("lizi"))) ci.removeChildByName("lizi");
                }
...
PlayLayer_jinzhonglisi.prototype.CardFourLayoutRestore = function(node,off)
{
    if(off != 0 && MjClient.rePlayVideo == -1) return;
    // node 是克隆新建的一个麻将节点 by sking
    var newC = null; //先创建麻将的UI节点
    var newVal = 0; //新牌的值，是几万，几筒，几条....为数字
    var pl = getUIPlayer(off);//获取玩家信息.off 为0 ，就是自己得信息，能看到自己摸牌 by sking
    ...
    var uistandFour = [];
    var _copyCardFour = pl.mjhandFour.slice();
    cc.log("_copyCardFour 00000    = " + _copyCardFour);
    ...
    for(var k = 0;k < uistand.length;k++)
    {
        if(_copyCardFour.indexOf(uistand[k].tag) >= 0)
        {
            ...
            uistand[k].isFour = true;
            cc.log("_copyCardFour111111111111 isFour = true " + uistand[k].tag);
        }
        else
        {
            uistandFour.isFour = false; //四张当中的牌
            uistand[k].isFour = false;
            newUIStand.push(uistand[k]);
        }
    }

    newUIStand.sort(TagOrder);//排除四张牌后剩余的UI节点

    if(newC)
    {
        newC.isFour = false;
        newUIStand.push(newC); //把这张牌放入手牌的数组里  by sking
    }
    ...
    if(uihun.length > 0) //是否有柰子，有则放在最前面 by sking
    {
        for(var i = 0; i < uihun.length; i++)
        {
            newUIStand.unshift(uihun[i]); //向数组开头添加一个元素 unshift
        }
    }
    ...
    {
        //排列起始的四张牌
        for(var n = 0; n < uistandFour.length;n++)
        {
            var ci = uistandFour[n];
            ci.setColor(cc.color(220, 220, 220));
            ci.addTouchEventListener(function () {});
            // if(n == 0)
            // {
            //     ci.x = start.x + upSize.width * upS * 0.1;
            ...
            // }
            // else{
            //     ci.x = uistandFour[n - 1].x + upSize.width * upS *1.2//1.02
            // }
        }
    }

    var uiOrder = [uigang1, uigang0, uipeng, uichi, uistandFour,newUIStand];
    if(off == 1 || off == 2)
    {
        uiOrder.reverse();//颠倒顺序
    }
            ...
            if(i != 0)
            {
                if(ci.name == orders[i - 1].name)
                {
                    ...
                    else
                    {
                        if(ci.name == "mjhand")
                        {
                            if(orders[i - 1] && !ci.isFour && orders[i - 1].isFour)
                                ci.x = orders[i - 1].x + upSize.width * upS *(COMMON_UI.cardBetween + 0.4);
                            else
                                ci.x = orders[i - 1].x + upSize.width * upS *COMMON_UI.cardBetween
                        }
                        else
        ...
        //if(ci.name == "mjhand" || ci.name == "mjhand")
        {
            ...
            if(ci.isFour)
            {
                ...
                if(!_lizi)
                {
                    ...
                    if(ci.name == "gang0" || ci.name == "gang1" || ci.name == "peng" || ci.name == "mjhand"|| ci.name == "mjhand_replay")
                    {
                        if(off == 0)
                        {
                            if(ci.name == "gang0" || ci.name == "gang1" || ci.name == "peng" )
                ...
                //if(MjClient.clickTing && MjClient.canTingCards[ci.tag])
                //{
                //    ci.setColor(cc.color(255, 255, 255));
                //    SetTouchCardHandler(stand, ci);
                //}
            ...
            {
                if(ci.getChildByName("lizi")) ci.removeChildByName("lizi");
            }
        }
    }
    ...
    //刷新手牌大小
    if(COMMON_UI3D.is3DUI()){
        COMMON_UI3D.set3DCardSprite(off,true);
    }else {
        resetCardSize();
    }
};
...
// 判断吃碰杠胡的状态
PlayLayer_jinzhonglisi.prototype.EatVisibleCheck = function()
{
    var eat = MjClient.playui.jsBind.eat;
    ...
    MjClient.playui.anGangCards = [];    // 记录暗杠数据
    MjClient.gangCards = [];
    MjClient.eatpos = [];
    ...
    if(tData.tState == TableState.waitPut && pl.mjState == TableState.waitPut)
    {
        if(IsTurnToMe())
        {
            ...
            if (!pl.isTing) {
                ...
                for (var i = 0; i < pl.mjhand.length; i++) {
                    if(pl.mjhandFour.indexOf(pl.mjhand[i]) >= 0) {//去掉这几张牌是否能听牌
                        ...
                        if (MjClient.majiang.canTing(cardsAfterPut,
                                cardsAfterPut.concat(pl.mjpeng).concat(pl.mjgang1).concat(pl.mjgang0),
                                pl,
                                tData.areaSelectMode.guaisanjiao)) {
                            MjClient.canTingCards[pl.mjhand[i]] = 1;
                            if (vnode.indexOf(eat.ting._node) < 0) {
                                vnode.push(eat.ting._node);
                                cc.log("￥￥￥￥听牌监测 = " + pl.mjhandFour);
                            }
                        }
                    }
                }
            ...
            var rtn = MjClient.majiang.canGang1(pl.mjpeng, pl.mjhand, pl.isTing,pl.mjhandFour);
            cc.log("$$$$杠牌监测"+JSON.stringify(rtn));
            if(rtn.length > 0 && pl.isNew)
            {
                MjClient.gangCards = rtn;
                MjClient.playui.anGangCards = rtn;
            ...
            if(vnode.length > 0)
            {
                ...
                //当只剩2张牌，并且都是立牌的时候，必须听牌，不能过
                if(pl.mjhand.length == 2 && pl.mjhandFour.length == pl.mjhand.length)
                {
                    if (vnode.indexOf(eat.guo._node) >= 0) {
                        vnode.splice(vnode.indexOf(eat.guo._node),1);
                    }
                }
    ...
    else if(tData.tState == TableState.waitEat)
    {
        ...
        if(!IsTurnToMe())
        {
            ...
            cc.log("===========================tData.putType = " + tData.putType);
...
/*
    玩家ui，信息
 */
PlayLayer_jinzhonglisi.prototype.Node_player = function(off)
{
    ...
    UIEventBind(null, _node, "initSceneData", function (eD) {
        SetUserVisible_jinzhonglisi(_node, off);
    ...
    UIEventBind(null, _node, "mjhand", function (eD) {
        InitUserHandUI_jinzhonglisi(_node, off);
        if(off == 0)
        {
            MjClient.playui.CardFourLayoutRestore(_node,off);

            var _unTouchLayer = new UnTouchLayer();
            MjClient.Scene.addChild(_unTouchLayer,9999);
            MjClient.playui.scheduleOnce(function()
            {
                if (cc.sys.isObjectValid(_unTouchLayer))
                    _unTouchLayer.removeFromParent();
    ...
    UIEventBind(null, _node, "newCard", function (eD) {
        if(off == 0)
        {
            ...
            if (eD.isFistCard)
            {
                ...
                {
                    var ci = children[i];
                    if(ci.name == "mjhand" && ci.y > MjClient.init_y)
                    {
                        ci.setVisible(false);
                        break;
                    }
    ...
    UIEventBind(null, _node, "MJPut", function (eD) {
        ...
        // var pl = getUIPlayer(off);
        // if (pl && eD.uid == pl.info.uid && pl.isTing)
        // {
        ...
        // }
        DealMJPut_jinzhonglisi(_node,eD,off);
        hideTingBtn();
        if(_tingCardNumNode) _tingCardNumNode.visible = false;
        ...
        {
            cc.log("---------------EVENT-MJPut--------------------");
            MjClient.playui.CardLayoutRestore(getNode(off), off);
        }
        if( off != 0 && MjClient.rePlayVideo != -1 )
        {
            MjClient.playui.CardFourLayoutRestore(getNode(off), off);
            MjClient.playui.CardLayoutRestore(getNode(off), off);
        }
    });
