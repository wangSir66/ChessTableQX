...
var PlayPanel_cheHuZi = playLayer_ziPai.extend({
	jsonFile:"Play_ZiPaiCheHuZi.json",
				...
				img_putCard:{
					_event:{
						roundEnd:function(){
							...
							var winner = sData.players[sData.tData.uids[sData.tData.winner]];
			                if(sData && sData.tData.currCard == -1 || (winner && winner.mjgang0 && winner.mjgang0.indexOf(sData.tData.currCard) >= 0)) {
			                    //跑胡 偎胡
			                    this.visible = false;
			                }
						}	
				...
				layout_eatCards:{
					_event:{
						roundEnd : function() {
							...
							var sData = MjClient.data.sData;
							if(sData && sData.tData.currCard == -1 || (pl.mjgang0 && pl.mjgang0.indexOf(sData.tData.currCard) >= 0)) {
								//跑胡 偎胡
...
// 是否需要听牌提示
PlayPanel_cheHuZi.prototype.isNeedShowTing = function(){
    return false;
};

//发送王状态给server
PlayPanel_cheHuZi.prototype.sendWangToServer = function(type){
	var tData = MjClient.data.sData.tData;
...
//处理王状态
PlayPanel_cheHuZi.prototype.handleWang = function(data, nodeName){
	var tData = MjClient.data.sData.tData;
    MjClient.gamenet.request("pkroom.handler.tableMsg", {
		cmd: "HZWangChuang",
		type: type,
...
//Override
//吃牌
PlayPanel_cheHuZi.prototype.commitEatCard = function(eatCards, biArr) {
    var tData = MjClient.data.sData.tData;
    var msg = {
        cmd: "MJChi",
        eatCards: eatCards,
...
//Override
//处理碰牌
PlayPanel_cheHuZi.prototype.handlePeng = function() {
    MjClient.playui.hideEatBtns();
	MjClient.gamenet.request("pkroom.handler.tableMsg", {
		cmd: "MJPeng"
	});
...
// 初始化手牌
PlayPanel_cheHuZi.prototype.initHandCards = function(node, msg) {
    var tData = MjClient.data.sData.tData;
...
//Override
// 获取操作按钮数组
PlayPanel_cheHuZi.prototype.getShowEatNodes = function() {
    var sData = MjClient.data.sData;
    var tData = sData.tData;
...
//处理玩家王吊、王闯、王炸通知
PlayPanel_cheHuZi.prototype.handleWangChuang = function(node, data) {
	var off = this.getUIOffByNode(node);
	var pl = this.getUIPlayer(off);
 ...
 * @param off 位置偏移
 */
PlayPanel_cheHuZi.prototype.displayEatLabel = function(node, eatType, msg) {
    var off = this.getUIOffByNode(node);
...
//获取王类型
PlayPanel_cheHuZi.prototype.getWangType = function(data){
	var wangType = data.type;
	var eatAction = null;
	if(wangType == WANG_TYPE.WANG_DIAO)
		eatAction = "mjwangdiao";
	else if(wangType == WANG_TYPE.WANG_CHUANG)
		eatAction = "mjwangchuang";
	else if(wangType == WANG_TYPE.WANG_ZHA)
		eatAction = "mjwangzha";
	return eatAction;
};

//播放王动画
PlayPanel_cheHuZi.prototype.playWangAnin = function(parentNode, spineJson, spineAtlas){
...
//总结算面板
PlayPanel_cheHuZi.prototype.createGameOverLayer = function(type) {
    return new GameOverLayer_syZiPai();
};

//小结算面板
PlayPanel_cheHuZi.prototype.createEndOneLayer = function(type) {
    return new EndOneView_shaoYangPaoHuZi();
};

//处理翻醒
PlayPanel_cheHuZi.prototype.handleFanXing = function(card) {
 ...
 * Override
 */
PlayPanel_cheHuZi.prototype.checkCardCanPut = function(pl, card) {
    return card != this.HUN_CARD;//王霸不能打出
};

// 初始化弃牌
PlayPanel_cheHuZi.prototype.initOutCard = function(node) {
...
// 更新展示牌(背光跟牌)
PlayPanel_cheHuZi.prototype.updatePutCard = function(node, msg, isReconnect) {
	var sData = MjClient.data.sData;
    var tData = sData.tData;
	...
	if(card == MjClient.playui.HUN_CARD && putType == 1){
		node.isPick = true;
...
//Override
// 展示牌动画
PlayPanel_cheHuZi.prototype.showPutCardAnimation = function(node) {
    var off = this.getUIOffByNode(node);
...
//将展示牌移动到手牌中
PlayPanel_cheHuZi.prototype.movePutCardToHandCard = function(node, card){
	var off = this.getUIOffByNode(node);
	var player = this.getUIPlayer(off);
	if(!player) return;
...
//Override
PlayPanel_cheHuZi.prototype.isHandMoveToTopDiffColumn = function () {
    return true;
};

//Overide
PlayPanel_cheHuZi.prototype.getDefaultSetting = function() {
    return {
        layout: 1,
        bg: 0,
        pai: 1,
...
// 发牌间隔
PlayPanel_cheHuZi.prototype.getSendCardInterval = function() {
...
//Override
PlayPanel_cheHuZi.prototype.isAniParallel = function() {
    return false;
};
...
//@author evan
PlayPanel_cheHuZi.prototype.getAniTimeByType = function(type) {
...
//Override
// 落牌 发牌节奏控制
PlayPanel_cheHuZi.prototype.checkDelayNewCard = function(node, msg) {
...
// 判断吃、碰、胡、过操作按钮的状态
PlayPanel_cheHuZi.prototype.updateEatBtns = function(msg) {
    var sData = MjClient.data.sData;
    var pl = sData.players[SelfUid() + ""];
    if (!pl) {
        MjClient.gamenet.request("pkroom.handler.tableMsg", {
...
//Override
//字牌资源路径
PlayPanel_cheHuZi.prototype.getCardFilePath = function() {
    return "playing/paohuzi/";
};

//Override
// 是否放偎
PlayPanel_cheHuZi.prototype.isOtherWei = function() {
    return false;
};

// 庄
PlayPanel_cheHuZi.prototype.checkZhuangVisible = function(node) {
    var tData = MjClient.data.sData.tData;
    node.visible = false;
    if (!this.isInPlay()) {
        return;
...
//获取牌堆总数
PlayPanel_cheHuZi.prototype.getInitDiPaiCount = function() {
	var tData = MjClient.data.sData.tData;
...
//Override
// 获取提、偎牌的显示类型
PlayPanel_cheHuZi.prototype.getCardShowType = function(card, off) {
	if(MjClient.rePlayVideo != -1){
		return 1;
	}
...
PlayPanel_cheHuZi.prototype.getEatCardShowType = function(eatType, card, cardIndex, off) {
	var showType = 2;
	var indexInUids = this.getIndexInUids(off);
    var pl = MjClient.data.sData.players[MjClient.data.sData.tData.uids[indexInUids]];
    if (eatType == "mjchi") {
...
PlayPanel_cheHuZi.prototype.isUpdateXingPlayer = function (off) {
...
// 更新手牌 (出牌)
PlayPanel_cheHuZi.prototype.updateHandCardByPut = function(node) {
	var off = this.getUIOffByNode(node);
...
// Override
PlayPanel_cheHuZi.prototype.updateHandCardByEat = function(node, eatType, msg) {
    var off = this.getUIOffByNode(node);
...
PlayPanel_cheHuZi.prototype.removeXingHandCardByEat = function(eatType, cardArr, msg, off, zhuangOff) {
...
//Override
// 落牌动画
PlayPanel_cheHuZi.prototype.showOutCardAnimation = function(node) {
    if(node.isPick){
        delete node.isPick;
        return;
    }
...
//Override
PlayPanel_cheHuZi.prototype.is34ColorGrey = function() {
    return false;
};

// 改变手牌大小
PlayPanel_cheHuZi.prototype.changeHandCardSize = function(handCard) {
...
/** Override
 */
PlayPanel_cheHuZi.prototype.setCardTouchHandler = function(card, off) {
    var self = this;
    var pl = this.getUIPlayer(off);
    var dict = {};
...
//Override
//改变出牌虚线布局
PlayPanel_cheHuZi.prototype.changeCutLineLayout = function(cutLine) {
...
// 2人布局 对家在左上还是右上
PlayPanel_cheHuZi.prototype.isLeftTop = function() {
    return false;
};

//出牌摸牌的位置是否适配偏右、传统布局
PlayPanel_cheHuZi.prototype.isPutCardLayout = function() {
    return true;
};

PlayPanel_cheHuZi.prototype.removeShuffleNode = function() {
    this.shuffleNode.removeFromParent(true);
    this.shuffleNode = null;
};

PlayPanel_cheHuZi.prototype.playShuffleEffect = function() {
...
PlayPanel_cheHuZi.prototype.getMjhandDelay = function() {
    return 2;
};