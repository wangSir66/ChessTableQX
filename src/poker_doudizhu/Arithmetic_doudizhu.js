...
DoudizhuNT.prototype.CARDTPYE = CARDTPYE;
var CARDCOUNT = {};
...
DoudizhuNT.prototype.randomCards = function() {
    var cards = [];
...
DoudizhuNT.prototype.calPoint = calPoint;

DoudizhuNT.prototype.isShun = function(oCards) { //oCards有序
    if (!oCards) {
        return false;
    }
...
DoudizhuNT.prototype.isLiandui = function(oCards) { //oCards有序
    if (!oCards) {
        return false;
    }
    var cardCount = oCards.length;
...
DoudizhuNT.prototype.isSanshun = function(cards) { //cards有序
    if (!cards) {
        return false;
    }
...
DoudizhuNT.prototype.isFeiji = function(oCards) { //cards有序
    if (!oCards) {
        return false;
    }
...
// 计算牌型
DoudizhuNT.prototype.calType = function(cards) {
    var cardCount = cards.length;
...
DoudizhuNT.prototype.calCardsValue = function(cards, type) {
...
// 牌点比较函数
DoudizhuNT.prototype.cardValueCmp = function(a, b) {
    var pa = calPoint(a);
    var pb = calPoint(b);
    if (pa == pb) {
        return a - b;
...
// 牌是否能压上
DoudizhuNT.prototype.canPut = function(oCards, oLastCards, tData) {
    var cards = oCards.slice();
...
// 检查是否能出牌
DoudizhuNT.prototype.checkPut = function(oHands, oCards, lastCards, tData) {
...
DoudizhuNT.prototype.checkPutByOneTimes = function (oHands) {
...
// 对手牌排序，普通牌型和同花顺优先排序
DoudizhuNT.prototype.sortHandCards = function(oCards, sortType) {
    var cards = oCards.slice();
...
// 找N张点数一样的牌
DoudizhuNT.prototype.findNSameCard = function(hands, point, n) { 
...
DoudizhuNT.prototype.filterSort = function(hands, cardsArray, type) { 
...
// 找出type牌型的牌
DoudizhuNT.prototype.findCardByType = function(hands, type, cardCount) { 
    var rets = [];
...
// 是否可以包含炸弹
DoudizhuNT.prototype.isCanContainZhadan = function (type) {
    return type == CARDTPYE.zhadan;
};

// 不拆炸弹
DoudizhuNT.prototype.dismantleZhadan = function (hands, cardsArray, type) {
...
// 求解提示出牌
DoudizhuNT.prototype.tipCards = function(oHands, oLastCards) {
    var rets = [];
...
// 求出牌型
DoudizhuNT.prototype.cardsType = function(oCards) {
    if (oCards) {
        var cards = oCards.slice();
        cards.sort(this.cardValueCmp);
...
// 计算真实的点
DoudizhuNT.prototype.calRealPoint = function(num) {
    if (!num) {
        return -1;
    }
    var ceilNum = Math.ceil(num / 4);
...
DoudizhuNT.prototype.findMinPointAndMinZhaDanByHands = function (hands) {
...
// 找最小牌的顺子/连对
DoudizhuNT.prototype.findCardsByMinPointAndType = function (cardsMap, len, minPoint, cType) {
...
DoudizhuNT.prototype.findSanZhangByMinPoint = function (cardsMap, len, minPoint) {
...
// 找最小牌的对子
DoudizhuNT.prototype.findDuiZiByMinPoint = function (cardsMap, len, minPoint) {
...
DoudizhuNT.prototype.changeCardsToMap = function (hands) {
    var cardsMap = {}
    for (var i = 0; i < hands.length; i++) {
...
// 首出逻辑
DoudizhuNT.prototype.getAutoPutCardWithFirstOut = function(hands) {
    // 1. 从小到大出牌
...
// 是否是三张系列
DoudizhuNT.prototype.is3ZhangType = function(cType) {
...
DoudizhuNT.prototype.findSelectCardForAutoUp = function(tipCards, selectValue) {
...
// 更换牌值
DoudizhuNT.prototype.changeSelectCardForAuto = function(tipCards, idx, selectValue) {
    tipCards[idx] = selectValue;
    return tipCards;
}

// 有足够的三张牌组
DoudizhuNT.prototype.isEnough3SameCards = function (upSelectCards, groupNum) { 
    var start = false;
    var tmpCards = [];
...
DoudizhuNT.prototype.findEnough3SameCard = function(oHands, upSelectCards, groupNum, zhadanMap) {
    var tmpSelect = [];
    var lastPoint = -1;
    for (var i = 0; i < upSelectCards.length; i++) {
...
// 找三张配合的牌
DoudizhuNT.prototype.findDaiCardsWithSanZhang = function(oHands, upSelectCards, cType, groupNum, zhadanMap) {
    var findNum = 1;
    var pointCounts = {};
    for (var i = 0; i < upSelectCards.length; i++) {
...
// 自动提牌-划动-三张系列
DoudizhuNT.prototype.handleAutoByTouchMoveSanZhang = function(upSelectCards, cType, oLastCards, oHands) {
    var groupNum = 1; 
...
DoudizhuNT.prototype.findTheRightTouchMoveCards = function(upSelectCards, cType, oLastCards) {
...
DoudizhuNT.prototype.findZhadanTouchMoveCards = function(upSelectCards) {
    var tCards = [];
    var lastPoint = -1;
...
// 自动提牌-划牌
DoudizhuNT.prototype.handleAutoByTouchMove = function (bTouchMove, upSelectCards, cType, tipCardsArray, oLastCards, oHands) {
...
// 自动提牌-点牌
DoudizhuNT.prototype.handleAutoByFirstCard = function(tipCardsArray, upSelectCard, cType) {
...
// 计算斗地主划牌结果
DoudizhuNT.prototype.calAutoPutCards = function (bTouchMove, tipCardsArray, upSelectCards, oLastCards, oHands) {
    var data = {};
...
DoudizhuNT.prototype.getFindCardForFirstPut = function(cards, cType, count) {
    var realCards = null;
    var rateType = {};
...
DoudizhuNT.prototype.calAutoPutCardWithFirstOut = function(upSelectCards, hands) {
...
// 计算选择的牌,排除炸弹
DoudizhuNT.prototype.calSelectWithoutZhadan = function(upSelectCards, zhadanMap) {
...
// 计算牌组的炸弹
DoudizhuNT.prototype.getZhaDanMapByCards = function(oHands) {
    var hands = oHands.slice();
    hands.sort(this.cardValueCmp);
...
if(typeof(module)!="undefined" && module.exports)    
    module.exports = DoudizhuNT;
if(typeof(MjClient)!="undefined")
    MjClient.doudizhu_nantong = new DoudizhuNT();
})();