...
function SetEndOneUserUI_chenzhou(node,off)
{
...
var EndOneView_chenzhou = cc.Layer.extend({
	jsBind:{
		block: {
		...
		head0:{
		   ...
		   _run:function(){ SetEndOneUserUI_chenzhou(this,0); },
		...
		,head1:{ 
		  ...
		  _run:function(){ SetEndOneUserUI_chenzhou(this,1); }
		}
		...
		,head2:{ 
		   ...
		   _run:function(){ SetEndOneUserUI_chenzhou(this,2); }
		}
		,head3:{
			 head:
			 {
			...
			_run:function(){ SetEndOneUserUI_chenzhou(this,3); }
			},
            count_down:{
                _visible :function()
                {
    ...
    ctor:function () {
        this._super();
        var endoneui = ccs.load("endOne_chenzhou.json");
        ...
        cc.log("tData.moZhong 金鸟  ???? ",tData.moZhong,tData.areaSelectMode["zhuaniao"],tData.areaSelectMode["buzhongsuanquanzhong"]);
        if (!tData.moZhong && 
            tData.areaSelectMode["zhuaniao"] != 10 && 
            tData.areaSelectMode["buzhongsuanquanzhong"] && !MjClient.isDismiss 
            && tData.areaSelectMode["zhuaniao"] != 0) 
	    {
	        countNode.setString("金鸟");
	        countNode.setVisible(true);
	        countNode.x += countNode.getContentSize().width * (tData.areaSelectMode["zhuaniao"] / 2);
	        if (tData.areaSelectMode["zhuaniao"] == 8) 
	        {
	        	countNode.x += countNode.getContentSize().width / 2;
	        }
            if(cards.length >= 6)
            {
	            var shareBtn = _back.getChildByName("share");
       	...
       	if (tData.areaSelectMode["zhuaniao"] == 10 && !MjClient.isDismiss) 
        {
        	countNode.setString("+"+(cards[0] % 10));
        	if (cards[0] == 71 || (cards[0] % 10) == 1 || (cards[0] % 10) == 9) 
        	{
        		countNode.setString("+10");
        	}
	        countNode.setVisible(true);
        }
 		return true;
