...
function SetUserVisible_guaisanjiao(node, off)
{
    //var sData = MjClient.data.sData;
    //return;
...
function DealMJPut_guaisanjiao(node, msg, off, outNum)
{
    DealMJPut_shanXiApp(node, msg, off, outNum);
}

function InitUserHandUI_guaisanjiao(node, off)
{
    var sData = MjClient.data.sData;
    var tData = sData.tData;
    var pl = getUIPlayer(off);
    ...
    for(var i = 0; i < pl.mjput.length; i++)
    {
        ...
        DealMJPut_guaisanjiao(node, msg, off, i);
    }
...
function initFlower_guaisanjiao() {
    initFlower(false, false);
}


var PlayLayer_guaisanjiao = cc.Layer.extend({
    jsBind: {
        _event: {
            mjhand: function() {
            ...
            roundEnd: function() {
                ...
                function delayExe()
                {
                    ...
                    self.addChild(new EndOneView_guaisanjiao(),500);
                }
                if(MjClient.rePlayVideo === -1)    // 正常游戏
                {
                    this.runAction(cc.sequence(cc.delayTime(0.1),cc.callFunc(COMMON_UI.showMjhandBeforeEndOne),cc.delayTime(2.0),cc.callFunc(delayExe)));
                }
                else
                {
        ...
        banner: {
            ...
            Button_1: {
                _visible : true,
                _click: function() {
                    MjClient.openWeb({url:MjClient.GAME_TYPE.JIN_ZHONG_GUAI_SAN_JIAO,help:true});
                }
            },
            hunPai:{
                baidaBg:{
    ...
    ctor: function() {
        this._super();
        var playui = ccs.load("Play_guaisanjiao.json");
...
PlayLayer_guaisanjiao.prototype.CardLayoutRestore = function(node, off)
{
...
// 判断吃碰杠胡的状态
PlayLayer_guaisanjiao.prototype.EatVisibleCheck = function()
{
    var eat = MjClient.playui.jsBind.eat;
...
/*
    玩家ui，信息
 */
PlayLayer_guaisanjiao.prototype.Node_player = function(off)
{
    ...
    UIEventBind(null, _node, "initSceneData", function (eD) {
        SetUserVisible_guaisanjiao(_node, off);
    ...
    UIEventBind(null, _node, "mjhand", function (eD) {
        InitUserHandUI_guaisanjiao(_node, off);
    });
    ...
    UIEventBind(null, _node, "MJPut", function (eD) {
        ...
        DealMJPut_guaisanjiao(_node,eD,off);
        hideTingBtn();
        if(_tingCardNumNode) _tingCardNumNode.visible = false;
