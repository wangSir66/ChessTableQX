...
//向服务器发送 过消息
MjClient.MJPass2NetForTYHZ = function()
{
   // console.log(">>>>>>>>>普通  过 <<<<<<<<");
    cc.log("====================send======pass=====");
...
function SetUserVisible_TYHZ(node, off)
{
    //var sData = MjClient.data.sData;
    //return;
    cc.log("====================off======================" + off);
...
function InitUserHandUI_TYHZ(node, off)
{
    var sData = MjClient.data.sData;
    var tData = sData.tData;
    var pl = getUIPlayer(off);
    ...
    if (tData.tState == TableState.waitJiazhu && SelfUid() == pl.info.uid) {
        if (pl.mjState == TableState.waitJiazhu) {
            var jiaZhuLayer = MjClient.playui.getChildByName("jiaZhuCS");
            if(jiaZhuLayer && cc.sys.isObjectValid(jiaZhuLayer)){
        ...
        if(MjClient.playui._jiazhuWait && cc.sys.isObjectValid(MjClient.playui._jiazhuWait)){
...
function initFlower_TYHZ() {
    initFlower(false, false);
}


var PlayLayer_TYHZ = cc.Layer.extend({
    jsBind: {
        _event: {
            mjhand: function() {
                ...
                if(MjClient.ShowCardsLayer && cc.sys.isObjectValid(MjClient.ShowCardsLayer)){
            ...
            roundEnd: function() {
                ...
                function showEndCards()
                {
                    ...
                    self.addChild(new EndOneView_TYHZ(),500);
                }
               ...
               function addshowcardlayer()
                { 
                    var callFunc = function(){
                        if(showCards.length >= 1 && MjClient.getAppType() != MjClient.APP_TYPE.HUNANWANGWANG && MjClient.APP_TYPE.QXSYDTZ != MjClient.getAppType()){
                            delayExe();
                        }else{
                            showEndCards();
                        }
                    ...
                    if(MjClient.getAppType() == MjClient.APP_TYPE.HUNANWANGWANG || MjClient.APP_TYPE.QXSYDTZ == MjClient.getAppType()){
                        MjClient.Scene.addChild(new majiang_showBird(showCards,callFunc));
                    }else{
                        MjClient.Scene.addChild(new HAHZ_showCardLayer(showCards,callFunc));
                    }
                ...
                {  
                    ...
                    if(isQxsydtz){
                        self.runAction(cc.sequence(cc.delayTime(0.5),
                    ...
                    self.runAction(cc.sequence(cc.delayTime(0.25),
                        MjClient.rePlayVideo == -1?cc.callFunc(COMMON_UI.showMjhandBeforeEndOne):cc.delayTime(0),
                        cc.delayTime(0.5),
        ...
        roundInfo:{
            ...
            _run:function()
            {
              ...
              if(MjClient.getAppType() == MjClient.APP_TYPE.HUNANWANGWANG || MjClient.APP_TYPE.QXSYDTZ == MjClient.getAppType()){
                    ...
                    if(tData.areaSelectMode.dianpao){
                ...
                str += tData.areaSelectMode.buzhongsuanquanzhong ? ",不中算全中":"";
                str += tData.areaSelectMode.quanzhongfanbei ? ",全中翻倍":"";

                str += tData.areaSelectMode.huQingYiSeQiDuiJia2Niao ? ",胡清一色和七对加2鸟":"";
                str += tData.areaSelectMode.huangZhuangGang ? ",荒庄荒杠":"";
                str += tData.areaSelectMode.buKePeng ? ",不可碰":"";
                str += tData.areaSelectMode.zhongHongZhongSuan12 ? ",中红中算12":"";
                str += tData.areaSelectMode.zhua1Zhong10 ? ",抓1算10":"";
                ...
                if (tData.areaSelectMode.zhuaniao != 0 && ((tData.areaSelectMode.zhuaniao != 1 &&
                    tData.areaSelectMode.zhuaniao != 8) || (MjClient.getAppType() == MjClient.APP_TYPE.HUNANWANGWANG || MjClient.APP_TYPE.QXSYDTZ == MjClient.getAppType())))
                {
        ...
        banner: {
            ...
            setting: {
                _click: function() {
                    var settingLayer = null;
                    if(MjClient.getAppType() == MjClient.APP_TYPE.HUNANWANGWANG || MjClient.APP_TYPE.QXSYDTZ == MjClient.getAppType() ||
                        MjClient.getAppType() === MjClient.APP_TYPE.QXYZQP)
                    {
                        settingLayer = new RoomMaJiangSetting();
                    }
                    else
                    {
                        settingLayer = new SettingView();
                    }
        ...
        wait: {
            ...
            backHomebtn: {
                ...
                _event: {
                    returnPlayerLayer: function() {
                        if(MjClient.getAppType() != MjClient.APP_TYPE.HUNANWANGWANG && MjClient.APP_TYPE.QXSYDTZ != MjClient.getAppType()) MjClient.playui.visible = true;
                    },
                    initSceneData: function(eD) {
                        if(MjClient.getAppType() != MjClient.APP_TYPE.HUNANWANGWANG && MjClient.APP_TYPE.QXSYDTZ != MjClient.getAppType()) this.visible = IsInviteVisible();
                    },
            ...
            _event: {
                ...
                onlinePlayer: function() {
                    if((MjClient.getAppType() == MjClient.APP_TYPE.HUNANWANGWANG || MjClient.APP_TYPE.QXSYDTZ == MjClient.getAppType()) && IsAllPlayerReadyState()) {
                ...
                waitReady: function() {
                    if(MjClient.getAppType() == MjClient.APP_TYPE.HUNANWANGWANG || MjClient.APP_TYPE.QXSYDTZ == MjClient.getAppType()){
                        this.getChildByName('delroom').visible = true;
                        this.getChildByName('backHomebtn').visible = true;
                    }
                }
            }
        },
        ...
        down: {
            ...
            _event: {
                ...
                initSceneData: function(eD) {
                    ...
                    var notInGame = (tData.tState != TableState.waitPut && tData.tState != TableState.waitEat && tData.tState != TableState.waitCard)
                    if (!notInGame && (pl.tPutCard || Object.keys(tingSet).length > 0))
                    {
                        var AutoPut_btn = AddAutoPutCheckBox();
        ...
        left: {
            ...
            out0: {
                _run: function () {
                    if (MjClient.size.width / MjClient.size.height >= 1.5)
                    {
                        setWgtLayout(this, [0, 0.0545],[0, 0.5],[4.6, 4.8]);
                    }
                    else
                    {
                        setWgtLayout(this, [0, 0.05],[0, 0.5],[4.6, 4.8]);
                    }
            ...
            changeui: {
                ...
                changeuibg: {
                    ...
                    guobg: {
                        guo: {
                            _touch: function(btn, eT) {
                                if (eT == 2) MjClient.MJPass2NetForTYHZ();
                            }
                        },
                        fanhui: {
                            _touch: function(btn, et) {
    ...
    ctor: function() {
        ...
        var playui = ccs.load("Play_TYHZ.json");
        playMusic("bgFight");
        ...
        //东南西北转盘初始化
        if(!COMMON_UI3D.isCanChangTo3D()) COMMON_UI3D.InitSetArrowbk();
...
PlayLayer_TYHZ.prototype.hideEat= function() {
    if (MjClient.getAppType() != MjClient.APP_TYPE.HUNANWANGWANG && MjClient.APP_TYPE.QXSYDTZ != MjClient.getAppType()) {
        return ;
    }
...
// 显示当前局数增加的分数
PlayLayer_TYHZ.prototype.NowRoundScore = function(node,off,isShow) { 
    var pl = getUIPlayer(off);
    if(!pl)return; 
    ...
    if (MjClient.data.sData.tData.areaSelectMode.jiapiao)
        return;
    if(MjClient.getAppType() == MjClient.APP_TYPE.HUNANWANGWANG || MjClient.APP_TYPE.QXSYDTZ == MjClient.getAppType()){
        if(MjClient.data.sData.tData.areaSelectMode.piaoType < 3){
            return;
        }
    }
...
PlayLayer_TYHZ.prototype.CardLayoutRestore = function(node, off,isNewCard)
{
...
// 判断吃碰杠胡的状态
PlayLayer_TYHZ.prototype.EatVisibleCheck = function()
{
    var eat = MjClient.playui.jsBind.eat;
    ...
    if(tData.tState == TableState.waitPut && pl.mjState == TableState.waitPut)
    {
        if(IsTurnToMe())
        {
            ...
            {
                if(pl.isNew || tData.areaSelectMode["suiShiKeGang"]){
            ...
            if(vnode.length > 0)
            {
                if(!_bIsZiMo) vnode.push(eat.guo._node);
                eat.ting._node.visible = false;
...
// 中鸟的内容
PlayLayer_TYHZ.prototype.getIsZhongBird = function(cd){
...
PlayLayer_TYHZ.prototype.ShowAllTingTips = function ()
{
