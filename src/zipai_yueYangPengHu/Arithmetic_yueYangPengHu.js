var majiang_yueYangPengHuLink = function(majiang) {
    ...
    majiang.cardsCount = function(tb) {
        return 80 - (4 - tb.tData.maxPlayer) * 14;
    };
...
(function() {
    ...
    huzi.prototype.canHu = function(tb, pl, card) {
            ...
            if (canPao && plCpy.mjhand.length > 0) {
                ...
                if (allHuMatrix_pao.length > 0) {
                    ...
                    if (this.getHuxi(tb, plCpy, null, false) + max_pao.maxHuxi >= minHuxi
                        || ((tb.tData.areaSelectMode.isMaoHu && this.getHuxi(tb, plCpy, card, false) + max_pao.maxHuxi == 0))
                    ) {
                        canPaoHu = true;
                    }
                }
        ...
        if(!canPaoHu){
            ...
            if (allHuMatrix.length > 0) {
                ...
                var minHuxi = tb.tData.minHuxi;
                if ((pl.alarmFlag == 1 && (max.isPengHu || !card)) || (pl.alarmFlag != 1 && (this.getHuxi(tb, pl, card, false) + max.maxHuxi >= minHuxi
                    || (tb.tData.areaSelectMode.isMaoHu && this.getHuxi(tb, pl, card, false) + max.maxHuxi == 0)))
                ) {
    ...
    huzi.prototype.maxHu = function(tb, pl, card, allHuMatrix) {
        ...
        return {"maxIdx": maxHandIdx, "maxHuxi": maxHandScore, "maxWinOne": maxWinOne, isPengHu:isPengHu};
    }
    ...
    huzi.prototype.getTingCards = function(tb, pl, putCard) {
        ...
        for (var uid in tb.players) {
            ...
            for (var i = 0; i < p.mjwei.length; i++) {
                if (p.info.uid == pl.info.uid || p.mjHide.indexOf(p.mjwei[i]) < 0){
                    add(p.mjwei[i], 3);
                }
            }
            ...
            for (var i = 0; i < p.mjput.length; i++) {
                ...
                if(!this.isPengOrPaoCardByPutForOther(tb, p, p.mjput[i], i)){
                    add(p.mjput[i], 1);
                }
            }
    ...
    huzi.prototype.getTingStats = function(tb, pl, putCard) {
    ...
    huzi.prototype.isPengOrPaoCardByPutForOther = function (tb, pl, card, pos) {
        for(var uid in tb.players){
            var p = tb.players[uid];
            if(p.info.uid != pl.info.uid && p.pengOrPaoCardByPut){
                for(var i = 0; i < p.pengOrPaoCardByPut.length; i++){
                    if(p.pengOrPaoCardByPut[i].card == card && p.pengOrPaoCardByPut[i].fromUid == pl.info.uid && p.pengOrPaoCardByPut[i].pos == pos){
                        return true;
                    }
                }
            }
    ...
    huzi.prototype.isPengOrPaoCardByPutFromOther = function (pl, card) {
        if(pl.pengOrPaoCardByPut){
            for(var i = 0; i < pl.pengOrPaoCardByPut.length; i++){
                if(pl.pengOrPaoCardByPut[i].card == card){
                    return true;
                }
            }
        }
    ...
    if (typeof(MjClient) != "undefined") {
        MjClient.majiang_yueYangPengHu = new huzi();
        majiang_yueYangPengHuLink(MjClient.majiang_yueYangPengHu);
    }
}());
