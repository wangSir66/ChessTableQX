...
var _isShowShaiziAni_ningxiang = false; //为了控制mjhand之后播骰子动画时newCard的状态显示
//向服务器发送 过消息
MjClient.MJPass2NetForningxiang = function()
{
   // console.log(">>>>>>>>>普通  过 <<<<<<<<");
    cc.log("====================send======pass=====");
...
var showShaiziAni_ningxiang = false
function showShaiziAni_ningxiang_old(poff)
{
    if(poff == 0) {
        ...
        if(MjClient.rePlayVideo != -1)//回放
        {
            for (var off = 0; off < 4; off++) {
                ...
                InitUserHandUI_ningxiang(_node, off);
            }
            playEffect("shuffle");
        ...
        _isShowShaiziAni_ningxiang = true;
        ...
        firstFrame.runAction(cc.sequence(playSoundFunc, animate,showPonitFunc,cc.delayTime(0.8),cc.callFunc(function(){
            //发牌
            _isShowShaiziAni_ningxiang = false;
...
function showShaiziAni_pingjiangzhaniao_kaigang(poff)
{
    if(poff == 0) {
...
function updateHeadUI_ningxiang(node, off , actionType)
{      
    cc.log('updateHeadUI_ningxiang', off)
    var pl = getUIPlayer(off);
    if(!pl) return;
    // showHandCard_MLHZ(node, off);
    showHandCard_ningxiang(node,off);
    MjClient.playui.CardLayoutRestore(node, off);
    cc.log('updateHeadUI_ningxiang update success', off)
...
function SetUserVisible_ningxiang(node, off)
{
    if (_isShowShaiziAni_ningxiang)  // 解决摇骰子时断线重连调用本函数，会手牌重复的bug by cyc
        return;
...
function showHandCard_ningxiang(node, off)
{
    var sData = MjClient.data.sData;
    var tData = sData.tData;
    var pl = getUIPlayer(off);
...
function InitUserHandUI_ningxiang(node, off)
{
    var sData = MjClient.data.sData;
    var tData = sData.tData;
    var pl = getUIPlayer(off);
    ...
    // }
     ...
     if (SelfUid() == pl.info.uid) {
        if (tData.tState == TableState.waitJiazhu && pl.jiazhuNum < 0) {
...
var PlayLayer_ningxiang = cc.Layer.extend({
    jsBind: {
        _event: {
            mjhand: function() {
            ...
            roundEnd: function() {
                ...
                function showEndCards()
                {
                    ...
                    self.addChild(new EndOneView_ningxiang(),500);
                }
            ...
            initSceneData: function() {
                ...
                cc.log("initSceneData -----------------ningxiang = " + JSON.stringify(this));
                CheckRoomUiDelete();
        ...
        BtnPutCard:{ //add by  sking for put card button
            ...
            _event:{
                ...
                waitPut: function() {
                    if (_isShowShaiziAni_ningxiang) return;
                    var pl = getUIPlayer(0);
                    var eat = MjClient.playui.jsBind.eat;
        ...
        down: {
            ...
            _event: {
                ...
                initSceneData: function(eD) {
                    SetUserVisible_ningxiang(this, 0);
                    var pl = getUIPlayer(0);
                    if (pl) 
                    {
                        ...
                        if (pl.addScoreCards) 
                        {
                          MjClient.playui.GetAllScoreBtn(MjClient.playui.jsBind.eat.qshu_layout._node);  
                        }
                    }
                    var tingSet = {};
                    if (pl.tingListAfterPut) 
                ...
                mjhand: function(eD) {
					if (showShaiziAni_ningxiang)
                    	showShaiziAni_ningxiang(0);
					else
						InitUserHandUI_ningxiang(this, 0);
                },
                roundEnd: function() {
                ...
                sendKaiGangCard: function(eD) {
                    ...
                    showShaiziAni_pingjiangzhaniao_kaigang(0);
                ...
                MJChi: function(eD) {
                    // DealMJChi(this, eD, 0);
                    updateHeadUI_ningxiang(this, 0 , ActionType.CHI);
                    setUserOffline(this, 0);
                    var AutoPut_btn = this.getChildByName("AutoPut_btn");
                ...
                MJPeng: function(eD) {
                    ...
                    if (MjClient.rePlayVideo != -1) {
                        ...
                        updateHeadUI_ningxiang(this, 0, ActionType.PENG);
                    }
                    setUserOffline(this, 0);
                    MjClient.playui.EatVisibleCheck();
                    if (MjClient.data.sData.tData.curPlayer == getPlayerIndex(0))
                ...
                MJTing: function (eD) {
                    ...
                    MjClient.canTingCards = {};
                    if (pl.tingCard && eD.uid == SelfUid()) 
                    {
                        cc.log("pl.tingCard       ",JSON.stringify(pl.tingCard));
                        MjClient.canTingCards = pl.tingCard;
                        setTingCards(MjClient.playui._tingCardsNode,pl.tingCard)
        ...
        right: {
            ...
            _event: {
                ...
                initSceneData: function(eD) {
                    SetUserVisible_ningxiang(this, 1);
                },
                addPlayer: function(eD) {
                    SetUserVisible_ningxiang(this, 1);
                },
                ...
                mjhand: function(eD) {
					if (!showShaiziAni_ningxiang)
                    	InitUserHandUI_ningxiang(this, 1);
                },
                roundEnd: function() {
                ...
                MJChi: function(eD) {
                    if (MjClient.rePlayVideo != -1)
                        ...
                        updateHeadUI_ningxiang(this, 1, ActionType.CHI);
                    setUserOffline(this, 1);
                },
                MJGang: function(eD) {
                ...
                MJPeng: function(eD) {
                    if (MjClient.rePlayVideo != -1)
                        ...
                        updateHeadUI_ningxiang(this, 1, ActionType.PENG);
                    setUserOffline(this, 1);
                },
                MJHu: function(eD) {
        ...
        top: {
            ...
            _event: {
                ...
                initSceneData: function(eD) {
                    SetUserVisible_ningxiang(this, 2);
                },
                addPlayer: function(eD) {
                    SetUserVisible_ningxiang(this, 2);
                },
                ...
                mjhand: function(eD) {
					if (!showShaiziAni_ningxiang)
                    	InitUserHandUI_ningxiang(this, 2);
                },
                roundEnd: function() {
                ...
                MJChi: function(eD) {
                    if (MjClient.rePlayVideo != -1)
                        ...
                        updateHeadUI_ningxiang(this, 2, ActionType.CHI);
                    setUserOffline(this, 2);
                },
                MJGang: function(eD) {
                ...
                MJPeng: function(eD) {
                    if (MjClient.rePlayVideo != -1)
                        ...
                        updateHeadUI_ningxiang(this, 2, ActionType.PENG);
                    setUserOffline(this, 2);
                },
                MJHu: function(eD) {
        ...
        left: {
            ...
            _event: {
                ...
                initSceneData: function(eD) {
                    SetUserVisible_ningxiang(this, 3);
                },
                addPlayer: function(eD) {
                    SetUserVisible_ningxiang(this, 3);
                },
                ...
                mjhand: function(eD) {
					if (!showShaiziAni_ningxiang)
                   		InitUserHandUI_ningxiang(this, 3);
                },
                roundEnd: function() {
                ...
                MJChi: function(eD) {
                    if (MjClient.rePlayVideo != -1)
                        ...
                        updateHeadUI_ningxiang(this, 3, ActionType.CHI);
                    setUserOffline(this, 3);
                },
                MJGang: function(eD) {
                ...
                MJPeng: function(eD) {
                    if (MjClient.rePlayVideo != -1)
                        ...
                        updateHeadUI_ningxiang(this, 3, ActionType.PENG);
                    setUserOffline(this, 3);
                },
                MJHu: function(eD) {
            ...
            changeui: {
                ...
                changeuibg: {
                    ...
                    guobg: {
                        guo: {
                            _touch: function(btn, eT) {
                                if (eT == 2) MjClient.MJPass2NetForningxiang();
                            }
                        },
                        fanhui: {
                            _touch: function(btn, et) {
                ...
                _event:
                    {
                        ...
                        {
                            cc.log("选牌型啊~~~~");
                            MjClient.playui.GetAllScoreBtn(this);
                        },
                        initSceneData:function()
                        {
                            var pl = getUIPlayer(0);
                        ...
                        MJSelect:function(eD)
                        {
                            if (eD.uid == SelfUid() && MjClient.playui.cleanScorebtn(this)) {
                                this.visible = false;
                                MjClient.gamenet.request("pkroom.handler.tableMsg", {
                                cmd: "MJPassQsHu",
                                });
...
/*                        if( !MjClient.playui.haveQiShouHu() ){
    ...
    ctor: function() {
        ...
        var playui = ccs.load("Play_ningxiang.json");
        playMusic("bgFight");
        ...
        if(pl && (pl.mjState == TableState.isReady || pl.mjState == TableState.roundFinish))
        {
            ...
            if(pl != null)
            {
                if (pl.isTing) {
                    ...
                    if (off == 0)
                    {
                        if (pl && MjClient.data.sData.tData.areaSelectMode.isOpenTingTip) {
                            var tingSet = {};
                            if (pl.tingListAfterPut) 
                            {
                                tingSet = pl.tingListAfterPut;
...
PlayLayer_ningxiang.prototype.CardLayoutRestore = function(node, off, isNewCard)
{
    ...
    for(var i = 0; i < orders.length; i++)
    {
        ...
        if(off % 2 == 0)//自己或者对家
        {
            ...
            if(off == 0)
            {
                ...
                if(MjClient.clickTing && MjClient.notup)
                {
                    if (ci.name == "mjhand")
                    {
                        if(MjClient.canTingCards[ci.tag] && !pl.needTing)
                        {
                            ci.setColor(cc.color(255, 255, 255));
                            if (!hasUp) {
                                ci.y += 20;
                ...
                {
                    console.log(ci.tag+"--------newC---3-----"+newC ,pl.haiDiLaoState);
                    if(newC)
                    {
                        ci.setColor(cc.color(255, 255, 255));
...
PlayLayer_ningxiang.prototype.willHuShowArrow = function()
{
    var tData = MjClient.data.sData.tData;
...
PlayLayer_ningxiang.prototype.GetAllScoreBtn = function(node)
{
    var pl = getUIPlayer(0);
    var that = node;
    var ScoreLists = pl.addScoreCards;
    cc.log("有多少牌型    ",Object.keys(pl.addScoreCards).length,JSON.stringify(pl.addScoreCards));
    if (Object.keys(ScoreLists).length <= 0) return;
    var eat = MjClient.playui.jsBind.eat;
    MjClient.playui.hideAllEat();
    MjClient.playui.removeAllScoreBtn(that);
    
    that.visible = true;
    MjClient.ScoreBtn = ScoreLists;
    var allbtnarry = [];
    for (var i in ScoreLists) {
        var ScoreType = "playing/gameTable/qs_" + ScoreLists[i].name + ".png";
        if (jsb.fileUtils.isFileExist(ScoreType)) 
        {
            cc.log("图片  路径 ",ScoreType);
            if (that.getChildByName(ScoreLists[i].name))continue;
            var newbtn = new ccui.Button(ScoreType);
            newbtn.setName(ScoreLists[i].name);
            ...
            that.addChild(newbtn);
            allbtnarry.push(newbtn);
        }
    }

    var space = isIPad() ? 1.1 : 1.4;
    for (var j = 0; j < allbtnarry.length; j++) {
        setWgtLayout(allbtnarry[j], [0, 0.16], [0.60, 0], [(j - allbtnarry.length + 1) * space * 0.5, 1.8], false, false);
    } 
}
PlayLayer_ningxiang.prototype.removeAllScoreBtn = function(node)
{
    var that = node;
    var allbtnarry = ["banbanhu","bubugao","jintyn","liuliushun","queyise","santong","sixi","yidianhong","yizhihua","zhongtull","zhongtusx"]
    for (var i = 0; i < allbtnarry.length; i++) {
        var now_btn = that.getChildByName(allbtnarry[i]);
        if (now_btn) 
        {
            now_btn.removeFromParent();
        }
    }
}
PlayLayer_ningxiang.prototype.cleanScorebtn = function(node)
{

    var that = node;
    for (var i in MjClient.ScoreBtn) {
        if (that.getChildByName(MjClient.ScoreBtn[i].name)) 
        {
            return false;
        }
    }
...
// 提示等待起手胡
PlayLayer_ningxiang.prototype.ShowQiShouHuTip = function()
{
    var eat = MjClient.playui.jsBind.eat;
    var sData = MjClient.data.sData;
...
PlayLayer_ningxiang.prototype.haveQiShouHu = function()
{
    var sData = MjClient.data.sData;
    for (var uid in sData.players) {
        var p = sData.players[uid];
...
PlayLayer_ningxiang.prototype.EatKaiGangVisibleCheck = function(tData, vnode, eat, pl) 
{
...
PlayLayer_ningxiang.prototype.EatBaoTingVisibleCheck = function(tData, vnode, eat, pl) 
{
    var eat = MjClient.playui.jsBind.eat;
    var sData = MjClient.data.sData;
    var tData = sData.tData;
    ...
    this.hideAllEat();
    var pl = sData.players[SelfUid() + ""];
    cc.log("不报听a       ",pl.needTing);
    if (!pl.needTing || typeof(pl.needTing) == "undefined") 
    {
        if (IsTurnToMe() || pl.mjState == TableState.waitEat) 
        {
            cc.log("刷新按钮");
            return MjClient.playui.EatVisibleCheck();
        }
        else
        {
            return;
...
PlayLayer_ningxiang.prototype.showKaiGangBigCards = function() {
    var tData = MjClient.data.sData.tData;
...
PlayLayer_ningxiang.prototype.hideAllEat = function(){
    var eat = MjClient.playui.jsBind.eat;
...
// 判断吃碰杠胡的状态
PlayLayer_ningxiang.prototype.EatVisibleCheck = function() {
    var eat = MjClient.playui.jsBind.eat;
    var sData = MjClient.data.sData;
    var tData = sData.tData;
    var leftCard = MjClient.majiang.getAllCardsTotal() - tData.cardNext;
    ...
    //自摸
    if((tData.tState == TableState.waitPut ||tData.tState == TableState.waitSelect ) && pl.mjState == TableState.waitPut)
    {
        if(IsTurnToMe())
        {
            //胡
...
/*                    if (vnode.indexOf(eat.ting._node) < 0 && pl.needTing) {
    ...
    else if(tData.tState == TableState.waitEat)
    {
        ...
        if(!IsTurnToMe())
        {
            ...
            if(pl.eatFlag & 1 && tData.areaSelectMode["kechi"])
            {
                var eatpos = pl.canEat || [];
    ...
    if (pl.haiDiLaoState == 1 || MjClient.majiang.getAllCardsTotal() - tData.cardNext <= 0)
    {
        ...
        {
            if (pl.eatFlag & 8)
                MjClient.native.umengEvent4CountWithProperty("ningxiang_debug_" + tData.tableid + "_" + tData.tState + "_" + pl.mjState, {uid:SelfUid()});
            vnode = [];
            eat.hu._node.stopActionByTag(1);
            eat._node.stopActionByTag(1);
    ...
    if(vnode.length > 0)
    {
        ...
        for(var i = 0; i < vnode.length; i++)
        {
            ...
            var space = isIPad() ? 1.4 : 1.7;
            setWgtLayout(vnode[i], [0, 0.16], [0.80, 0], [(i - vnode.length + 1) * space, 1.8], false, false);
        }
    }
...
PlayLayer_ningxiang.prototype.RemoveNewCardOut = function(node, card)
{
...
PlayLayer_ningxiang.prototype.DealKaiGang = function(node, msg, off)
{
    ...
    var cards = [];
    if (msg.card1 && msg.card1 != -1) 
    {
        cards.push(msg.card1)
    }

    if (msg.card2 && msg.card2 != -1) 
    {
        cards.push(msg.card2)
    }
...
PlayLayer_ningxiang.prototype.playQiShouHuEffect = function(qshuName) {
    var soundName = null;
