...
//向服务器发送 过消息
MjClient.MJPass2NetForxuzhou = function()
{
   // console.log(">>>>>>>>>普通  过 <<<<<<<<");
    cc.log("====================send======pass=====");
...
//处理出牌,放一张牌，打牌动作
function DealMJPut_xuzhou(node, msg, off, outNum)
{
    ...
    if(uids[selfIndex] == msg.uid)
    {
        ...
        else if (off == 1)
        {
            ...
            cc.log("DealMJPut_xuzhou remove card  = " + msg.card);
                    ...
                    cc.callFunc(function()
                    {
                        if (cc.sys.isObjectValid(out))
                        {
                            ...
                            out.runAction(cc.sequence(
                                cc.spawn(cc.moveTo(0.3, endPoint), cc.scaleTo(0.1, oSc)),
                                cc.callFunc(function(){
                                    addCurrentPutTag(out, off);
                                    out.zIndex = zoder;
                                })
        ...
        else
        {
            if (!(outNum >= 0))
            {
                cc.log("-----------deal mjput===== xuzhou off = " + off);
...
// 判断停牌操作
//function TingVisibleCheckForxuzhou(eat) {
//
//    var sData = MjClient.data.sData;
//    var tData = sData.tData;
...
function showLeftCards()
{
    ...
    var _node = getNode(0).getParent();

    var _bgText = _node.getChildByName("leftCardNum")
    var cardNum = MjClient.majiang.getAllCardsTotal() - tData.cardNext - tData.huangNum;
    if(cardNum != 4)
    {
        if(_bgText) _bgText.visible = false;
        return;
    }
    ...
    if(!_bgText)
    {
        _bgText = new ccui.ImageView("png/heidi.png");
        setWgtLayout(_bgText,[0.5, 0.5], [0.5, 0.36], [0, 0]);
        _bgText.setName("leftCardNum");
        _node.addChild(_bgText);

        var _cardNum = new ccui.Text();
        _cardNum.setName("leftCardText");
        _cardNum.setColor(cc.color.WHITE);
        _cardNum.setFontSize(20);
        _cardNum.setFontName(MjClient.fzcyfont);
        _cardNum.setPosition(cc.p(_bgText.getContentSize().width/2,_bgText.getContentSize().height/2));
        _bgText.addChild(_cardNum);
    }


    if(_bgText)
    {
        _bgText.visible = true;
        var _cardTextNode = _bgText.getChildByName("leftCardText");
        _cardTextNode.setString("本局还剩余"+cardNum+"张牌");
        _bgText.runAction(cc.sequence(cc.delayTime(3),cc.fadeOut(1),cc.removeSelf(true)));
        _cardTextNode.runAction(cc.sequence(cc.delayTime(3),cc.fadeOut(1)));
    }
...
function SetUserVisible_xuzhou(node, off)
{
    //var sData = MjClient.data.sData;
    //return;
    cc.log("====================off======================" + off);
...
/*
    配子的上一张牌
 */
function getShowHunCard(realHunCard)
{
    var showHunCard = 0;
    //条万筒
    if(realHunCard < 30)
    {
        ...
        else if(realHunCard <= 9)
        {
            showHunCard = realHunCard - 1;
        }
        else if(realHunCard <= 11)
        {
            showHunCard = 19;
        }
        else if(realHunCard <= 19)
        {
            showHunCard = realHunCard - 1;
        }
        else if(realHunCard <= 21)
        {
            showHunCard = 29;
        }
        else if(realHunCard <= 29)
        {
            showHunCard = realHunCard - 1;
        }
    }
    else if(realHunCard <= 61)//东南西北
    {
        ...
        if(realHunCard === 31)
        {
            ...
            showHunCard = realHunCard - 10;
        }
    }
    else if (realHunCard <= 91)//中发白
    {
        if(realHunCard === 71)
        {
            showHunCard = 91
...
function InitUserHandUI_xuzhou(node, off)
{
    var sData = MjClient.data.sData;
    var tData = sData.tData;
    var pl = getUIPlayer(off);
    ...
    if(tData.tState == TableState.waitJiazhu && SelfUid() == pl.info.uid)
    {
        ...
        {
            cc.log("============jiaZhuXuzhouLayer_new===============");
            var layer = new jiaZhuXuzhouLayer_new();
    ...
    for(var i = 0; i < pl.mjpeng.length; i++)
    {
        ...
        var offIdx = getOffByIndex(pl.pengchigang.peng[i].pos, idx) - 1;//表示被碰的人和pl之间隔着几个人，如果是pl碰下家，则offIdx=0，pl碰上家，则offIdex=2
        var cdui = null;
        for(var j = 0; j < 3; j++)
        {
    ...
    for(var i = 0; i < pl.mjput.length; i++)
    {
        ...
        DealMJPut_xuzhou(node, msg, off, i);
    }
...
var PlayLayer_xuzhou = cc.Layer.extend({
    _btnPutCard:null,
    jsBind: {
        _event: {
            mjhand: function() {
            ...
            roundEnd: function() {
                ...
                function delayExe()
                {
                    ...
                    if ((tData.winner == -1) && (!MjClient.isDismiss)) {
                        ...
                        self.runAction(cc.sequence(cc.callFunc(function(){
                            ...
                            self.addChild(new EndOneView_xuzhou(),500);
                        })));
                ...
                {
                    //弹小结算之前显示其他人牌
                    this.runAction(cc.sequence(cc.delayTime(0.1),cc.callFunc(COMMON_UI.showMjhandBeforeEndOne),cc.delayTime(2),cc.callFunc(delayExe)));
                }
                else
                {
            ...
            initSceneData: function() {
                ...
                CheckRoomUiDelete();
                //TingVisibleCheckForxuzhou(MjClient.playui.eat._node);
            },
            ...
            roundnumAtlas: {
                ...
                _event: {
                    mjhand: function() {
                        var sData = MjClient.data.sData;
                        var tData = sData.tData;
                        if (tData) return this.setString("" + (tData.roundAll-tData.roundNum + 1)+"/"+tData.roundAll + "");
                    }
                }
            }
        },
                ...
                baidaImg: {
                    ...
                    _event: {
                        mjhand:function()
                        {
                            ...
                            var showMsg = getShowHunCard(HuncardMsg);
                            var that = this;
        ...
        //                 var tData = sData.tData;
        //
        //                 if (tData.curPlayer == (tData.uids.indexOf(SelfUid())+1 + tData.uids.length) % tData.uids.length )
        //                     return;
        //
        //                 this.stopAllActions();
        //                 stopEffect(playTimeUpEff);
        ...
        down: {
            ...
            _event: {
                ...
                mjhand: function(eD) {
                    if(MjClient.rePlayVideo != -1)//回放的时候不用斗牌
                    {
                        InitUserHandUI_xuzhou(this, 0);
                    }
                    else
                    {
                ...
                newCard: function(eD) {
                    // cdsNums++;
                    console.log("客户端发牌组合......0000000000000 ");
                    //cc.log(">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>newCard---------------");
                    ...
                    DealNewCard(this,eD.newCard,0,eD.isFistCard);// checkCanTing(eD);
                    if(!eD.isFistCard)
                    {
        ...
        right: {
            ...
            _event: {
                ...
                waitJiazhu:function (msg) {
                     ...
                     */
                    var pl = getUIPlayer(0)
                    var layer = new jiaZhuXuzhouLayer_new(function(){
                        //弹窗等待
                        MjClient.playui._jiazhuWait.visible = true;
            ...
            changeui: {
                ...
                changeuibg: {
                    ...
                    guobg: {
                        guo: {
                            _touch: function(btn, eT) {
                                if (eT == 2) MjClient.MJPass2NetForxuzhou();
                            }
                        },
                        fanhui: {
                            _touch: function(btn, et) {
    ...
    ctor: function() {
        ...
        var playui = ccs.load(res.Play_xuzhou_json);
        playMusic("bgFight");
...
function TagOrder_xz(na, nb)
{
    var aValue = 0;
    aValue = na.tag;
    if(na.tag === 91 &&  MjClient.data.sData.tData.hunCard !== 91)
    {
        aValue = na.getUserData();
    }

    var bValue = 0;
    bValue = nb.tag;
    if(nb.tag === 91 &&  MjClient.data.sData.tData.hunCard !== 91)
    {
        bValue = nb.getUserData();
    }
...
PlayLayer_xuzhou.prototype.CardLayoutRestore = function(node, off, isNewCard)
{
    ...
    //其他家新摸的牌动作
    if(isNewCard && !COMMON_UI3D.is3DUI())
    {
        if(off == 2)
        {
            orders[0].x = orders[0].x - upSize.width * upS *1;
            if(COMMON_UI3D.is3DUI()){
                orders[orders.length - 1].x = orders[orders.length - 1].x - upSize.width * upS * 0.8;
                COMMON_UI.newCardShakAni(ci,off);
            }
            else
            {
...
// 判断吃碰杠胡的状态
PlayLayer_xuzhou.prototype.EatVisibleCheck = function()
{
    var eat = MjClient.playui.jsBind.eat;
    ...
    if(tData.tState == TableState.waitPut && pl.mjState == TableState.waitPut)
    {
        if(IsTurnToMe())
        {
            ...
            // var huType = MjClient.majiang.canHu(pl.mjhand, 0, tData.hunCard);
            // if(pl.isNew && huType > 0)
            // {
            ...
            //天听
            if (!pl.isTing  && pl.mjput.length == 0 && pl.putCount == 0 && pl.mjhand.length >= 13) {
                cc.log("￥￥￥￥听天听牌监测");
                cc.log(pl.mjhand);
                MjClient.canTingCards = {};
            ...
            //碰后不能杠
            cc.log("1111111111111111111111111111开始判断是否能杠");
