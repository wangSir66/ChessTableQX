...
//向服务器发送 过消息
MjClient.MJPass2NetForkoudian = function()
{
   // console.log(">>>>>>>>>普通  过 <<<<<<<<");
...
function SetUserVisible_koudian(node, off)
{
    //var sData = MjClient.data.sData;
    //return;
    cc.log("====================off======================" + off);
...
//处理出牌,放一张牌，打牌动作
function DealMJPut_koudian(node, msg, off, outNum)
{
...
function InitUserHandUI_koudian(node, off)
{
    var sData = MjClient.data.sData;
    var tData = sData.tData;
    var pl = getUIPlayer(off);
    ...
    for(var i = 0; i < pl.mjput.length; i++)
    {
        ...
        DealMJPut_koudian(node, msg, off, i);
    }
...
var PlayLayer_koudian = cc.Layer.extend({
    jsBind: {
        _event: {
            mjhand: function() {
                ...
                function delayExe()
                {
                    ...
                    if ( tData.fieldId) {
                        self.addChild(new GoldEndOneLayer_MJ(),500)
                         ...
                         self.addChild(new EndOneView_koudian(),500);
                    }
                }
    ...
    ctor: function() {
        this._super();
        var playui = ccs.load(res.Play_koudian_json);
...
PlayLayer_koudian.prototype.CardLayoutRestore = function(node, off)
{
...
// 判断吃碰杠胡的状态
PlayLayer_koudian.prototype.EatVisibleCheck = function()
{
    var eat = MjClient.playui.jsBind.eat;
...
/*
    玩家ui，信息
 */
PlayLayer_koudian.prototype.Node_player = function(off)
{
    ...
    UIEventBind(null, _node, "initSceneData", function (eD) {
        SetUserVisible_koudian(_node, off);
    ...
    UIEventBind(null, _node, "mjhand", function (eD) {
        InitUserHandUI_koudian(_node, off);
    });
    ...
    UIEventBind(null, _node, "MJPut", function (eD) {
        ...
        // }
        DealMJPut_koudian(_node,eD,off);
        hideTingBtn();
        if(_tingCardNumNode) _tingCardNumNode.visible = false;
