function addWxHeadToEndUI_xiangtanPHZ(node,off)
{
...
function reInitarrCardVisible_xiangtanPHZ()
{
...
var EndOneView_xiangtanPHZ = cc.Layer.extend({
    _block:null,
	jsBind:{
		block:{
		    _run:function () {
		...
		parent:{
				...
				_click:function(btn,eT){
	                ...
	                if (MjClient.rePlayVideo >= 0 && MjClient.replayui) {
	                    ...
	                    if(MjClient.getAppType() == MjClient.APP_TYPE.HUNANWANGWANG || MjClient.getAppType() == MjClient.APP_TYPE.QXSYDTZ){
	                    	...
	                    	MjClient.MJPass2NetForxiangtanPHZ();
	                    }
	                }
					...
					reInitarrCardVisible_xiangtanPHZ();
				}
			},
			switchImg_zhuomian:{
				_run:function(){
				   	...
				   	_run:function(){
				   		...
				   		for(var index = 0;index<playerCount;index++){
						    ...
						    if(MjClient.getAppType() == MjClient.APP_TYPE.HUNANWANGWANG || MjClient.getAppType() == MjClient.APP_TYPE.QXSYDTZ){
						    	...
						    	addWxHeadToEndUI_xiangtanPHZ(cloneHead,showIndex);
						    }
				   		}
				   	},
				},
				showInfo:{
				   	...
				   	listView:{
				   		...
				   		_run:function(){
				   			...
				   			if(pl.hzdesc.dazihu){
				   				item = getItem();
				   				item.getChildByName("imgName").setString("大字胡");
				   				item.getChildByName("count").setString("×2");
				   			}
				   			if(pl.hzdesc.xiaozihu){
				   				item = getItem();
				   				item.getChildByName("imgName").setString("小字胡");
				   				item.getChildByName("count").setString("×2");
				   			}
				   			if(pl.hzdesc.pengpenghu){
				   				item = getItem();
				   				item.getChildByName("imgName").setString("碰碰胡");
				   				item.getChildByName("count").setString("×2");
				   			}
				   			if(pl.hzdesc.sanshihu){
				   				item = getItem();
				   				item.getChildByName("imgName").setString("30息");
				   				item.getChildByName("count").setString("×2");
				   			}
				   			if(pl.hzdesc.zimo && tData.areaSelectMode.zimoaddhuxi){
				   				item = getItem();
				   				item.getChildByName("imgName").setString("自摸");
				   				item.getChildByName("count").setString("+3");
				   			}
				   		}
				   	},
				   	cardInfo:{
			...
			main:{
				...
				leftPanel:{
					_run:function(){
						addWinMjHand_xiangtanPHZ(this,MjClient.MaxPlayerNum_xiangxiang - 1);
				...
				rightPanel:{
					_run:function(){
						...
						addWinMjHand_xiangtanPHZ(this,1);
						if(MjClient.rePlayVideo != -1){
	                    	this.visible = false;
	                    }
					}
...
//添加手牌
function addWinMjHand_xiangtanPHZ(_node,off) {
	var sData = MjClient.data.sData;
    var tData = sData.tData;
