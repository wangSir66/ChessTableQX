// 邵阳湘潭跑胡子字牌
var playPanel_xiangTanPHZ = playLayer_ziPai.extend({
    ctor: function() {
    ...
    getJsBind: function() {
        var jsBind = {
            ...
            img_banner: {
                ...
                btn_setting: {
                    _click: function() {
                        MjClient.Scene.addChild(new settingPanel_xiangTanPaoHuZi(), 6000);
...
//倒计时
playPanel_xiangTanPHZ.prototype.excCountDown = function(node) {
...
playPanel_xiangTanPHZ.prototype.showAndHideTrust = function(node){
    var off = this.getUIOffByNode(node);
...
// 打坨UI
playPanel_xiangTanPHZ.prototype.showDaTuoNode = function(node) {
    var off = this.getUIOffByNode(node);
...
//检测距离信息
playPanel_xiangTanPHZ.prototype.checkCanShowDistance = function(layoutData) {
...
playPanel_xiangTanPHZ.prototype.calculateHintPutList = function() {
    MjClient.hintPutList_ziPai = MjClient.huzi.hintPutCardsToTing();
};

//Override
playPanel_xiangTanPHZ.prototype.isNeedShuffle = function() {
    return false;
};

//规则说明
playPanel_xiangTanPHZ.prototype.getRuleInfo = function() {
    var tData = MjClient.data.sData.tData;
    var str = "";
...
//Override
playPanel_xiangTanPHZ.prototype.createEndOneLayer = function(type) {
    return MjClient.playui.isCoinField() ? (new EndOneView_ziPaiGold()) : (new EndOneView_xiangtanPHZ());
};

playPanel_xiangTanPHZ.prototype.getTingCards = function(sData, pl, putCard) {
    return MjClient.huzi.getTingCards(sData, pl, putCard);
};

//总结算面板
playPanel_xiangTanPHZ.prototype.createGameOverLayer = function(type) {
    return new GameOverLayer_syLoudi();
};

//字牌字体列表
playPanel_xiangTanPHZ.prototype.getCardFontList = function() {
    return ["type1", "type4", "type3"];
};

//Override 
playPanel_xiangTanPHZ.prototype.initSettingData = function() {
...
//Override
playPanel_xiangTanPHZ.prototype.isShowLongCard = function() {
    return true;
};

//Override
playPanel_xiangTanPHZ.prototype.isCheckTingStats = function() {
    return true;
}

//Override 
playPanel_xiangTanPHZ.prototype.getTingPaiType = function() {
    return 0;
};

//字牌字体idx
playPanel_xiangTanPHZ.prototype.getCardFontIdx = function() {
    return util.localStorageEncrypt.getNumberItem(MjClient.KEY_ZI_PAI_ZI_PAI_TYPE, 2);
};

//布局类型
playPanel_xiangTanPHZ.prototype.getLayoutType = function() {
    return util.localStorageEncrypt.getNumberItem(MjClient.KEY_ZI_PAI_PLAY_UI_LAYOUT, 0);
};

//Override
playPanel_xiangTanPHZ.prototype.getGameBgList = function() {
    return ["playing/gameTable/beijing_1.jpg", "playing/gameTable/beijing_2.jpg", "playing/gameTable/beijing_3.jpg"];
};

//Override
playPanel_xiangTanPHZ.prototype.getGameBgIdx = function() {
    return util.localStorageEncrypt.getNumberItem(MjClient.KEY_ZI_PAI_GAME_BG_TYPE, 0);
};

//Override
playPanel_xiangTanPHZ.prototype.getHuXiType = function() {
    var idx = util.localStorageEncrypt.getNumberItem(MjClient.KEY_ZI_PAI_HU_XI_TYPE, 0);
    return [1, 0][idx];
};

//Override
playPanel_xiangTanPHZ.prototype.getXuXianType = function() {
    return util.localStorageEncrypt.getNumberItem(MjClient.KEY_ZI_PAI_XU_XIAN_TYPE, 1);
};

//Override
playPanel_xiangTanPHZ.prototype.getSuDuType = function() {
...
//Override
playPanel_xiangTanPHZ.prototype.displayEatLabel = function(node, eatType, msg) {
    var off = this.getUIOffByNode(node);
...
//双击出牌tempData
playPanel_xiangTanPHZ.prototype.initDoubleTempData = function() {
...
//Override
playPanel_xiangTanPHZ.prototype.setCardTouchHandler = function(card, off) {
    var self = this;
    var pl = this.getUIPlayer(off);
    var dict = {};
    ...
    card.addTouchEventListener(function(btn, eventType) {
        ...
        if (eventType == ccui.Widget.TOUCH_ENDED || eventType == ccui.Widget.TOUCH_CANCELED) {
            ...
            // 出牌
            if (!MjClient.hasPut && IsTurnToMe() && tData.tState == TableState.waitPut && (!pl.canNotPutCard || pl.canNotPutCard.indexOf(btn.tag) == -1) && 
                (pos.y > MjClient.playui.jsBind.img_cutLine._node.y || self.doubleClickToPutCard)) {
                function doPut() {
...
playPanel_xiangTanPHZ.prototype.getCardShowType = function(card, off) {
    var indexInUids = this.getIndexInUids(off);
    var pl = MjClient.data.sData.players[MjClient.data.sData.tData.uids[indexInUids]];
    // 展示
    if (!(pl.mjHide && pl.mjHide.indexOf(card) >= 0) || pl.info.uid == SelfUid()) {
        return 2;
    }
...
//Override 收入亮张动画
playPanel_xiangTanPHZ.prototype.showPickCardAnimation = function(node) {
    var self = this;
    var off = this.getUIOffByNode(node);
    var tData = MjClient.data.sData.tData;
...
//Override
playPanel_xiangTanPHZ.prototype.updateHandCardByHZPickCard = function(node) {
    var off = this.getUIOffByNode(node);
    if (!this.isCurPlayer(off)) {
        if (off == 0) {
...
//Override
playPanel_xiangTanPHZ.prototype.updateHandCardByPick = function(node) {
    return;
};

//Override
playPanel_xiangTanPHZ.prototype.showPutCardAnimation = function(node) {
    var off = this.getUIOffByNode(node);
...
// // 出牌线&出牌手指动画
// playPanel_xiangTanPHZ.prototype.checkCutLineVisible = function(node) {
...
playPanel_xiangTanPHZ.prototype.showDefeatTip = function (uid) {
...
playPanel_xiangTanPHZ.prototype.removeDefeatTip = function () {
    var sData = MjClient.data.sData;
    for (var uid in sData.players) {
        var playerNode = this.getUINodeByUid(Number(uid));
        var playerHead = playerNode.getChildByName("layout_head");
...
// 更新展示牌(背光跟牌)
playPanel_xiangTanPHZ.prototype.updatePutCard = function(node, msg, isReconnect) {
    var tData = MjClient.data.sData.tData;
...
playPanel_xiangTanPHZ.prototype.isAniParallel = function() {
    return false;
};

// 发牌间隔
playPanel_xiangTanPHZ.prototype.getSendCardInterval = function() {
...
playPanel_xiangTanPHZ.prototype.changeEatBtnLayout = function(btnNode, len, idx) {
    var scale = 0.20;
    if(MjClient.getAppType() == MjClient.APP_TYPE.QXSYDTZ){
