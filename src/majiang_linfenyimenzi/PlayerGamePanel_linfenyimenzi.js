...
//向服务器发送 过消息
MjClient.MJPass2NetForlinfenyimenzi = function()
{
   // console.log(">>>>>>>>>普通  过 <<<<<<<<");
    cc.log("====================MJPass2NetForlinfenyimenzi======pass=====");
...
function SetUserVisible_linfenyimenzi(node, off)
{
    //var sData = MjClient.data.sData;
    //return;
...
//处理出牌,放一张牌，打牌动作
function DealMJPut_linfenyimenzi(node, msg, off, outNum)
{
...
//初始化手牌的位置
function InitUserHandUI_linfenyimenzi(node, off)
{
    var sData = MjClient.data.sData;
    var tData = sData.tData;
    var pl = getUIPlayer(off);
    ...
    for(var i = 0; i < pl.mjput.length; i++)
    {
        ...
        DealMJPut_linfenyimenzi(node, msg, off, i);
    }
...
function initFlower_linfenyimenzi() {
    initFlower(false, false);
}


var PlayLayer_linfenyimenzi = cc.Layer.extend({
    jsBind: {
        _event: {
            mjhand: function() {
            ...
            roundEnd: function() {
                if (MjClient.getAppType() == MjClient.APP_TYPE.TXLINFENMJ)
                {
                    ...
                    function delayExe()
                    {
                        ...
                        self.addChild(new EndOneView_linfenyimenzi(),500);
                    }
                    //hideCurrentTingNum();
            ...
            changeui: {
                ...
                changeuibg: {
                    ...
                    guobg: {
                        guo: {
                            _touch: function(btn, eT) {
                                if (eT == 2) MjClient.MJPass2NetForlinfenyimenzi();
                            }
                        },
                        fanhui: {
                            _touch: function(btn, et) {
    ...
    ctor: function() {
        this._super();
        var playui = ccs.load("Play_linfenyimenzi.json");
...
PlayLayer_linfenyimenzi.prototype.CardLayoutRestore = function(node, off)
{
...
// 判断吃碰杠胡的状态
PlayLayer_linfenyimenzi.prototype.EatVisibleCheck = function()
{
    var eat = MjClient.playui.jsBind.eat;
    ...
    if(tData.tState == TableState.waitPut && pl.mjState == TableState.waitPut)
    {
        if(IsTurnToMe())
        {
            ...
            var rtn = MjClient.majiang.canGang1(pl.mjpeng, pl.mjhand, pl.isTing, allCards);
            cc.log("$$$$杠牌监测"+JSON.stringify(rtn));
            if(rtn.length > 0 && pl.isNew)
            {
                var idx = 0;
                while (idx < rtn.length) {
                    if (calLack([rtn[idx]].concat(pl.mjpeng).concat(pl.mjgang0).concat(pl.mjgang1)) < 2) {
                        rtn.splice(idx, 1);
                    }
                    else {
                        idx++;
                ...
                if (rtn.length > 0) {
                    ...
                    if (!MjClient.GameTipMessageNode)
                        MjClient.showToastbyPos("碰杠超过一门，不能杠！", 0.3);
                }
            }
            if(vnode.length > 0)
            {
...
/*
    玩家ui，信息
 */
PlayLayer_linfenyimenzi.prototype.Node_player = function(off)
{
    ...
    UIEventBind(null, _node, "initSceneData", function (eD) {
        SetUserVisible_linfenyimenzi(_node, off);
    ...
    UIEventBind(null, _node, "mjhand", function (eD) {
        InitUserHandUI_linfenyimenzi(_node, off);
    });
    ...
    UIEventBind(null, _node, "MJPut", function (eD) {
        ...
        // }
        DealMJPut_linfenyimenzi(_node,eD,off);
        hideTingBtn();
        if(_tingCardNumNode) _tingCardNumNode.visible = false;
