...
//向服务器发送 过消息
MjClient.MJPass2NetForzhuohaozi = function()
{
    // console.log(">>>>>>>>>普通  过 <<<<<<<<");
    cc.log("====================MJPass2NetForlvliangmajiang======pass=====");
...
function SetUserVisible_zhuohaozi(node, off)
{
    //var sData = MjClient.data.sData;
    //return;
    cc.log("====================off======================" + off);
...
//处理出牌,放一张牌，打牌动作
function DealMJPut_zhuohaozi(node, msg, off, outNum)
{
    DealMJPut_shanXiApp(node, msg, off, outNum);
}

function InitUserHandUI_zhuohaozi(node, off)
{
    var sData = MjClient.data.sData;
    var tData = sData.tData;
    var pl = getUIPlayer(off);
    ...
    for(var i = 0; i < pl.mjput.length; i++)
    {
        ...
        DealMJPut_zhuohaozi(node, msg, off, i);
    }
...
function initFlower_zhuohaozi() {
    initFlower(false, false);
}


var PlayLayer_zhuohaozi = cc.Layer.extend({
    jsBind: {
        _event: {
            mjhand: function() {
            ...
            roundEnd: function(msg) {
                ...
                function delayExe()
                {
                    var sData = MjClient.data.sData;
                    var tData = sData.tData;
                    MjClient.data.sData.tData.guoNum=msg.guoNum
                    resetEatActionAnim();
                    ...
                    self.addChild(new EndOneView_zhuohaozi(),500);
                }
        ...
        banner: {
            ...
            hunPai:{
                ...
                baidaImg: {
                    ...
                    {
                        //baidaOject = this;
                        //this.setScale(this.getScale()*0.8);
                        this.setVisible(false);
                        var cardWith =  this.getSize().width * this.getScale();
                        this.name = "baidaImg";
                        //this.setPositionX(cardWith*0.1);
                    },
                    _event: {
                        ...
                        mjhand:function()
                        {
                            ...
                            if(HuncardMsg && HuncardMsg != -1)
                            {
                                ...
                                this.runAction(cc.sequence(cc.delayTime(0.5),
                                    cc.spawn(cc.scaleTo(0.6,this.getScale()),cc.moveTo(0.6,-cardWith*0.8,1.86)).easing(cc.easeQuinticActionOut()),
                                    func));
                            }
                            else
                            {
                            ...
                            if(HuncardMsg2 && HuncardMsg2 != -1)
                            {
                                ...
                                _hunCard2.name = "hunCard2";
                                var imgMing = new ccui.ImageView();
                                imgMing.loadTexture("playing/MJ/ming.png");
                                imgMing.setPosition(this.getContentSize().width/2,this.getContentSize().height/2);
                                _hunCard2.addChild(imgMing);
                                _hunCard2.runAction(cc.sequence(cc.delayTime(1.2),
                                    cc.spawn(cc.scaleTo(0.6,this.getScale()),cc.moveTo(0.6,cardWith*0.2,1.86)).easing(cc.easeQuinticActionOut()),
                                    func));
                            }
                        },
                        initSceneData:function()
                        {
                            ...
                            var HuncardMsg = MjClient.data.sData.tData.hunCard;
                            cc.log("=========initSceneData=====当前耗子======HuncardMsg = " + HuncardMsg);
                            if(HuncardMsg && HuncardMsg != -1)
                            {
                                if(!this.isSetted)
                                {
                                    this.setScale(this.getScale()*0.8);
                                    this.setPositionX(-cc.winSize.width*0.07);
                                    this.isSetted = true;
                                    var imgMing = new ccui.ImageView();
                                    imgMing.loadTexture("playing/MJ/ming.png");
                                    imgMing.setPosition(this.getContentSize().width/2,this.getContentSize().height/2);
                            ...
                            var HuncardMsg2 = MjClient.data.sData.tData.hunCard2;
                            cc.log("========initSceneData======当前耗子2======HuncardMsg = " + HuncardMsg2);
                            if( HuncardMsg2 && HuncardMsg2 != -1)
                            {
                                var _hunCard2 = this.getParent().getChildByName("hunCard2");
                                var cardWith =  this.getSize().width * this.getScale();
        ...
        eat: {
            ...
            ting: {
                ...
                _touch: function(btn, eT) {
                    if (eT == 2)
                    {
                        ...
                        var tingCards = MjClient.majiang.getCheckTingHuCards(currentCard,copyhand);
            ...
            guo: {
                ...
                _touch: function(btn, eT) {
                    if (eT == 2)
                    {
                        MjClient.MJPass2NetForzhuohaozi();
                    }
                },
                bgimg: {
                    _run: function() {
            ...
            changeui: {
                ...
                changeuibg: {
                    ...
                    guobg: {
                        guo: {
                            _touch: function(btn, eT) {
                                if (eT == 2) MjClient.MJPass2NetForzhuohaozi();
                            }
                        },
                        fanhui: {
                            _touch: function(btn, et) {
    ...
    ctor: function() {
        this._super();
        var playui = ccs.load("Play_zhuohaozi.json");
        ...
        if(pl && (pl.mjState == TableState.isReady || pl.mjState == TableState.roundFinish))
        {
            ...
            if(pl != null)
            {
                if (pl.isTing) {
                    ...
                    if (off == 0)
                    {
                        var tingSet = MjClient.majiang.getCheckTingHuCards(null,pl.mjhand);
...
PlayLayer_zhuohaozi.prototype.CardLayoutRestore = function(node, off)
{
...
// 判断吃碰杠胡的状态
PlayLayer_zhuohaozi.prototype.EatVisibleCheck = function()
{
    var eat = MjClient.playui.jsBind.eat;
    ...
    if(tData.tState == TableState.waitPut && pl.mjState == TableState.waitPut)
    {
        if(IsTurnToMe())
        {
            ...
            if (pl.isNew && pl.eatFlag & 8) {
                ...
                if (!pl.mustHu) {
                    if(MjClient.data.sData.tData.areaSelectMode["playType"]==0&&MjClient.data.sData.tData.areaSelectMode["biHu"]==true)
                    {
                        pl.mustHu = (tData.hunCard == pl.mjhand[pl.mjhand.length - 1] && MjClient.majiang.quantingpai(pl.mjhand.slice(0, -1), new Set([tData.hunCard, tData.hunCard2])));
                    }
                }
                if(pl.mustHu) pl.isZiMoHu = true;
                pl.mustHu = pl.haoDiao ? pl.haoDiao : pl.mustHu;
            }
            //听
            if (!pl.isTing) {
                // cc.log("￥￥￥￥听牌监测");
                ...
                for (var i = 0; i < pl.mjhand.length; i++) {
                    ...
                    if (MjClient.majiang.canTing(cardsAfterPut, pl.mjhand[i], new Set([tData.hunCard, tData.hunCard2]))) {
            ...
            var rtn = MjClient.majiang.canGang1(pl.mjpeng, pl.mjhand, pl.isTing, new Set([tData.hunCard, tData.hunCard2]));
            cc.log("$$$$杠牌监测"+JSON.stringify(rtn));
            if(rtn.length > 0 && pl.isNew && !pl.mustHu&&leftCard>0)
            {
    ...
    else if(tData.tState == TableState.waitEat)
    {
        ...
        if(!IsTurnToMe())
        {
            ...
            if(vnode.length > 0)
            {
                cc.log("bbbbbbbbbbbbbbbbb5555:"+pl.mustHu)
...
/*
    玩家ui，信息
 */
PlayLayer_zhuohaozi.prototype.Node_player = function(off)
{
    ...
    UIEventBind(null, _node, "initSceneData", function (eD) {
        var pl = getUIPlayer(off);
       // if(pl) MjClient.playui.haoDiao=pl.haoDiao;

//        if(pl&&pl.haoDiao&&MjClient.data.sData.tData.areaSelectMode["playType"]==2)
  //      {
    //        MjClient.playui.isHaoDiao=true;
      //  }
        //else
        //{
          //  MjClient.playui.isHaoDiao=false;
        //}
        SetUserVisible_zhuohaozi(_node, off);
    ...
    UIEventBind(null, _node, "mjhand", function (eD) {
        InitUserHandUI_zhuohaozi(_node, off);
    });
    ...
    UIEventBind(null, _node, "newCard", function (eD) {
        //MjClient.playui.haoDiao=eD.haoDiao;
        if(off == 0)
        {
            if (typeof(eD) == "number") {
                eD = {newCard: eD};
           ...
           // if(eD.haoDiao&&MjClient.data.sData.tData.areaSelectMode["playType"]==2)
            //{
              //  MjClient.playui.isHaoDiao=true;
            //}
            //else
    ...
    UIEventBind(null, _node, "MJPut", function (eD) {
        ...
        // }
        DealMJPut_zhuohaozi(_node,eD,off);
        hideTingBtn();
        if(_tingCardNumNode) _tingCardNumNode.visible = false;
