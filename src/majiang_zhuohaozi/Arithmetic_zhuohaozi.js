...
(function() {
    ...
    {
        return card == MjClient.data.sData.tData.hunCard || card == MjClient.data.sData.tData.hunCard2;
    }
    ...
    {   
        ...
        if(MjClient.data.sData.tData.areaSelectMode["haoZiDiaoJiangZiMo"] == 2){
            if (cd && cd < 30 && cd % 10 < 3) {
        ...
        if(this.is7Dui(cards, cd, hunCard)){
            if(MjClient.data.sData.tData.areaSelectMode["playType"]==1 || MjClient.data.sData.tData.areaSelectMode["playType"]==2){
                return true;
            }
        }
    ...
    majiang.getCheckTingHuCards = function (selectCard, mjhandCard) {
        ...
        tingSet = calTingSet(copyhand, new Set([tData.hunCard, tData.hunCard2]));

        var hunArry = [tData.hunCard,tData.hunCard2];
        var huncount = 0;
        for (var m = 0; m < hunArry.length; m++){
            for (var n = 0; n < copyhand.length; n++) {
                if (copyhand[n] == hunArry[m]) {
                    huncount++;
                }
            }
            if (huncount < 4){
    ...
    majiang.hasNotHunCard = function(pl,hunCard1,hunCard2){
        var HunCardCount = 0;
        for (var i = 0;i < pl.mjhand.length; i++){
            if ( hunCard1 && pl.mjhand[i] == hunCard1){
                HunCardCount ++;
            }else if (hunCard2 && pl.mjhand[i] == hunCard2){
                HunCardCount ++;
            }
        }
        if (HunCardCount == pl.mjhand.length) return true;
        return HunCardCount == 0;
    };
    ...
    {
        MjClient.majiang_zhuohaozi = majiang;
    }
    else
    {
