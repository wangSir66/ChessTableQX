...
function SetEndOneUserUI_hongtongwangpai(node,off)
{
    ...
    if(pl.isTing)
    {
        ...
        if(pl.putCardAfterTing >= 0 && pl.putCardAfterTing != -1)
        {
            ...
            var _cd = pl.putCardAfterTing;//pl.mjput[pl.putCardAfterTing]; todo。。。。。 这个不是idx
            cc.log("--------wangpai----_cd = " + _cd);
...
function cardLayoutRestoreForEndOne_hongtongwangpai(node, off)
{
    // node 是克隆新建的一个麻将节点 by sking

    var newC = null; //先创建麻将的UI节点
    var newVal = 0; //新牌的值，是几万，几筒，几条....为数字
    var pl = MjClient.getPlayerByIndex(off);; //player 信息
    var tData = MjClient.data.sData.tData;
        ...
        for (var i = 0; i < children.length; i++) {
            if (children[i].name == "mjhand") 
            {
                if (children[i].tag == pl.hunCard) {
                    ...
                    _wangzi.setPosition(pos.getContentSize().width/2+10,pos.getContentSize().height/2);
...
var EndOneView_hongtongwangpai = cc.Layer.extend({
    jsBind:{
        block: {
        ...
        back:{
            ...
            head0:{
                ...
                _run:function(){ SetEndOneUserUI_hongtongwangpai(this,0); },
            ...
            ,head1:{
                ...
                _run:function(){ SetEndOneUserUI_hongtongwangpai(this,1); }
            }
            ...
            ,head2:{
                ...
                _run:function(){ SetEndOneUserUI_hongtongwangpai(this,2); }
            }
            ,head3:{
                head:
                    {
                ...
                _run:function(){ SetEndOneUserUI_hongtongwangpai(this,3); }
            },
            count_down:{
                _visible :function()
                {
    ...
    ctor:function () {
        this._super();
        var endoneui = ccs.load("endOne_hongtongwangpai.json");
