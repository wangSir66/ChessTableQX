...
// 耒阳字牌全局方法
MjClient.MaxPlayerNum_YuanJiangGuiHuZi = 3;
MjClient.cardPath_YuanJiangGuiHuZi = "playing/paohuzi/";
var ActionType_YuanJiangGuiHuZi =  //图片提示
    {
        CHI:1,
        PENG:2,
        WEI:3,
...
//初始化玩家金币、名字、胡息
function InitUserCoinAndName_YuanJiangGuiHuZi(node, off){
...
//向服务器发送王闯王钓操作
function HZWangChuangToServer_YuanJiangGuiHuZi(type){
    cc.log("====================HZWangChuangToServer_YuanJiangGuiHuZi=================");
...
//向服务器发送打牌操作
function HZPutCardToServer_YuanJiangGuiHuZi(card){
    cc.log("====================HZPutCardToServer_YuanJiangGuiHuZi=================");
    MjClient.gamenet.request("pkroom.handler.tableMsg", {
        cmd: "MJPut",
        card: card
    });
...
// 向服务器发送吃牌
function HZChiToServer_YuanJiangGuiHuZi(eatCards, biCards){
...
function HZLiuToServer_YuanJiangGuiHuZi(card, type){
    if (MjClient.rePlayVideo != -1) {
        return; // 回放时候不能请求
    }
    var optFlag;
...
//像服务器发送碰牌
function HZPengToServer_YuanJiangGuiHuZi()
{
    if (MjClient.rePlayVideo != -1) return; // 回放时候不能请求
    cc.log("====================HZPengToServer_YuanJiangGuiHuZi=================");
    MjClient.gamenet.request("pkroom.handler.tableMsg", {
        cmd: "MJPeng"
    });
}
...
//向服务器发歪碰牌
function HZWaiToServer_YuanJiangGuiHuZi()
{
    if (MjClient.rePlayVideo != -1) return; // 回放时候不能请求
    cc.log("====================HZWaiToServer_YuanJiangGuiHuZi=================");
    MjClient.gamenet.request("pkroom.handler.tableMsg", {
        cmd: "HZWeiCard"
    });
}
...
// 向服务器发送过牌
function HZPassConfirmToServer_YuanJiangGuiHuZi() {
    if (MjClient.rePlayVideo != -1)  {
        return; // 回放时候不能请求
    }
    ...
    MjClient.gamenet.request("pkroom.handler.tableMsg", {
        cmd: "MJPass",
        eatFlag: EatFlag_YuanJiangGuiHuZi(),
        cardNext: tData.cardNext,
        card: tData.lastPutCard
    });
}
...
//发送跑牌命令
function HZGangToServer_YuanJiangGuiHuZi(type){
    if (MjClient.rePlayVideo != -1) return; // 回放时候不能请求
    cc.log("====================HZGangToServer_YuanJiangGuiHuZi=================");
    MjClient.gamenet.request("pkroom.handler.tableMsg", {
        cmd: "MJGang",
        type: type,
        eatFlag: EatFlag_YuanJiangGuiHuZi()
...
//发送偎牌命令
function HZWeiToServer_YuanJiangGuiHuZi(type){
    if (MjClient.rePlayVideo != -1) return; // 回放时候不能请求
    cc.log("====================HZWeiToServer_YuanJiangGuiHuZi=================");
    MjClient.gamenet.request("pkroom.handler.tableMsg", {
        cmd: "HZWeiCard"
    });
}
...
//向服务器发送发牌命令
function HZNewCardToServer_YuanJiangGuiHuZi(){
    if (MjClient.rePlayVideo != -1) return; // 回放时候不能请求
    cc.log("====================HZNewCardToServer_YuanJiangGuiHuZi=================");
    MjClient.gamenet.request("pkroom.handler.tableMsg", {
        cmd: "HZNewCard"
    });
}
...
//向服务器发送胡牌命令
function MJHuToServer_YuanJiangGuiHuZi(){
    if (MjClient.rePlayVideo != -1) return; // 回放时候不能请求
    cc.log("====================MJHuToServer=================");
    MjClient.gamenet.request("pkroom.handler.tableMsg", {
        cmd: "MJHu",
...
//添加吃的牌
function addEatCard_YuanJiangGuiHuZi(node,name,mjNum,off){
    //根据牌的类型获得需要添加的节点
    ...
    var type = 2;
    //设置牌
    var newCard = getNewCard_YuanJiangGuiHuZi(mjNum,type,off);
...
/**
 添加手牌(回放)
 */
function addHandCardReplay_YuanJiangGuiHuZi(tagName,index,mjNum,off){
    var _node = getNode_YuanJiangGuiHuZi(off);
    //根据牌的类型获得需要添加的节点
 ...
 */
function addHandCard_YuanJiangGuiHuZi(tagName,index,mjNum,off){
    if(off != 0){
        return;
    }
    var _node = getNode_YuanJiangGuiHuZi(off);
    ...
    //设置牌
    var newCard = getNewCard_YuanJiangGuiHuZi(mjNum,1,off);
    var scale_y = newCard.scaleY;
    //var parentCount = addNode.childrenCount;
    //首先根据name判断cpNode中是否已经添加
    var cardParent = addNode.getChildByTag(tagName);
    if(!cardParent){
        ...
        addNode.addChild(cardParent);
        // SetTouchCardHandler_YuanJiangGuiHuZi(cardParent);
    }
 ...
 **/
function getNewCard_YuanJiangGuiHuZi(mjNum, type, off, isTurn){
    var _node = getNode_YuanJiangGuiHuZi(off);
    var copyNode = null;
    if(type == 1){
        //手牌
    ...
    //cc.log("getNewCard_YuanJiangGuiHuZi:"+mjNum+","+type+","+off+","+isTurn);
    if(mjNum > 0){
        //创建一个带有麻将信息的麻将 cp为创建后的麻将
        setCardSprite_YuanJiangGuiHuZi(mjNode, mjNum, type, isTurn);
        if(type == 1){
            //只有手牌才会有点击事件
            SetTouchCardHandler_YuanJiangGuiHuZi(mjNode,off);
        }
    }
...
function commitEatCards_YuanJiangGuiHuZi(eatCards, biArr){
    if ((MjClient.data.sData.players[SelfUid()].eatFlag & 32) > 0) {
        MjClient.showMsg("吃牌后视为过胡，确定吃吗？", function() {
            setChiVisible_YuanJiangGuiHuZi();
            HZChiToServer_YuanJiangGuiHuZi(eatCards, biArr);
        }, function() {}, "1");
    } else {
        setChiVisible_YuanJiangGuiHuZi();
        HZChiToServer_YuanJiangGuiHuZi(eatCards, biArr);
...
function commitLiuCards_YuanJiangGuiHuZi(card, type){
    if ((MjClient.data.sData.players[SelfUid()].eatFlag & 32) > 0) {
        MjClient.showMsg(type == 0 ? "溜牌后视为过胡，确定吃吗？" : "飘牌后视为过胡，确定吃吗？", function() {
            setChiVisible_YuanJiangGuiHuZi();
            HZLiuToServer_YuanJiangGuiHuZi(card, type);
        }, function() {}, "1");
    } else {
        setChiVisible_YuanJiangGuiHuZi();
        HZLiuToServer_YuanJiangGuiHuZi(card, type);
...
/**
 ...
 type:类型，1:手牌 2：吃牌 3：打的牌
 */
function setCardSprite_YuanJiangGuiHuZi(mjNode, mjNum, type, isTurn){
    if(ziPai.getZiPaiType() == 0){
        MjClient.cardPath_YuanJiangGuiHuZi = "playing/paohuzi/";
    }else if(ziPai.getZiPaiType() == 1){
        MjClient.cardPath_YuanJiangGuiHuZi = "playing/paohuzi/MJBg4/";
    }else if(ziPai.getZiPaiType() == 2 && MjClient.getAppType() != MjClient.APP_TYPE.QXLYQP && MjClient.getAppType() != MjClient.APP_TYPE.QXXXGHZ) {
        MjClient.cardPath_YuanJiangGuiHuZi = "playing/paohuzi/MJBg2/";
    }else if(ziPai.getZiPaiType() == 3){
        MjClient.cardPath_YuanJiangGuiHuZi = "playing/paohuzi/MJBg3/";
    }else {
        MjClient.cardPath_YuanJiangGuiHuZi = "playing/paohuzi/";
    }
    if(type == 1){
        // cc.log("full path:" + (MjClient.cardPath_YuanJiangGuiHuZi+"hand" + mjNum + ".png"));
        mjNode.loadTexture(MjClient.cardPath_YuanJiangGuiHuZi+"hand" + mjNum + ".png");
    }
    if(type == 2){
        mjNode.loadTexture(MjClient.cardPath_YuanJiangGuiHuZi+"out" + mjNum + ".png");
        if(isTurn){
            mjNode.loadTexture(MjClient.cardPath_YuanJiangGuiHuZi+"huxiBG.png");
        }
    }
    if(type == 3 || type == 4){
        mjNode.loadTexture(MjClient.cardPath_YuanJiangGuiHuZi+"put" + mjNum + ".png");
        if(isTurn){
            mjNode.loadTexture(MjClient.cardPath_YuanJiangGuiHuZi+"normalBG.png");
        }
    }
    mjNode.tag = mjNum;
}
...
// 新的听牌
function newCheckTingCards_YuanJiangGuiHuZi(putCard){
    // return;
    // if(MjClient.getAppType() == MjClient.APP_TYPE.QXSYDTZ || MjClient.getAppType() == MjClient.APP_TYPE.BDHYZP){
        ...
        if(MjClient.data.sData.tData.tState == TableState.waitPut && curPlayerIsMe_YuanJiangGuiHuZi(0) && putCard === undefined){
            cc.log("is waitPut....");
             postEvent("showTingLayer", []);
            return;
        }
        if(putCard && (!curPlayerIsMe_YuanJiangGuiHuZi(0) || sData.tData.tState != TableState.waitPut)){
            putCard = undefined;
        }
        ...
        // var cards = MjClient.majiang_yuanJiangGuiHuZi.getTingCards(sData, pl, putCard);
        var textData = MjClient.majiang_yuanJiangGuiHuZi.getTingStats(sData, pl, putCard);

        // cc.log("==========checkTingCards_YuanJiangGuiHuZi===============");
...
function checkTingCards_YuanJiangGuiHuZi(putCard){
    // return;
    // if(MjClient.getAppType() == MjClient.APP_TYPE.QXSYDTZ || MjClient.getAppType() == MjClient.APP_TYPE.BDHYZP){
        //听牌检测
        var sData = MjClient.data.sData;
        ...
        if(pl.mjhand)
        {
            cards = MjClient.majiang_yuanJiangGuiHuZi.getTingCards(sData, pl, putCard);
        }
        // var textData = MjClient.majiang_yuanJiangGuiHuZi.getTingStats(sData, pl, putCard);
...
function getPut2TingStats_YuanJiangGuiHuZi() {
    return;

    var sData = MjClient.data.sData;
    var tData = sData.tData;

    if (tData.tState != TableState.waitPut || !curPlayerIsMe_YuanJiangGuiHuZi(0)){
        return;
    }

    var time1 = +new Date();
    var pl = sData.players[SelfUid()];
    var info = MjClient.majiang_yuanJiangGuiHuZi.put2TingStats(sData, pl);
    var time2 = +new Date();
    cc.log("getPut2TingStats_YuanJiangGuiHuZi time@@ ", time2 - time1);
    console.log("put2TingStats@@ ", JSON.stringify(info));
}

//刷新手牌移动时的坐标
var btn_oPos = null;
function updateBtnMovedPosition_YuanJiangGuiHuZi(btn, eventType){
...
function SetTouchCardHandler_YuanJiangGuiHuZi(card,off){
    ...
    var _node = getNode_YuanJiangGuiHuZi(off);
    // var movingCard_YuanJiangGuiHuZi = null;
    ...
    card.addTouchEventListener(function(btn,eventType){
        ...
        //禁止同时移动多个牌
        if(MjClient.movingCard_YuanJiangGuiHuZi !== null && cc.sys.isObjectValid(MjClient.movingCard_YuanJiangGuiHuZi) && MjClient.movingCard_YuanJiangGuiHuZi !== btn){
            return;
        }
        ...
        if(eventType == ccui.Widget.TOUCH_BEGAN){
            ...
            MjClient.movingCard_YuanJiangGuiHuZi = btn;
            //
            ...
            setCardSprite_YuanJiangGuiHuZi(cloneCard, cardTag, 1, false);
            cloneCard.opacity = 0;
            ...
            btn.x = touchPos.x;
            btn.y = touchPos.y;
            // updateBtnMovedPosition_YuanJiangGuiHuZi(btn, eventType);
            ShowPutCardIcon_YuanJiangGuiHuZi();

            newCheckTingCards_YuanJiangGuiHuZi(btn.tag);

            return true;
        }
        if(eventType == ccui.Widget.TOUCH_MOVED){
            if (MjClient.movingCard_YuanJiangGuiHuZi==null) return;
        ...
        if(eventType == ccui.Widget.TOUCH_ENDED ||
            ...
            var pl = getUIPlayer_YuanJiangGuiHuZi(0);
            var tData = MjClient.data.sData.tData;
            var isPut = false;
            ...
            var putCardCb = function () {
                ...
                if(!isWangBa && (!pl.canNotPutCard||pl.canNotPutCard.indexOf(btnTag)==-1 || MjClient.majiang.getCanPutCardNum(pl) == 0) &&
                    IsTurnToMe() && ((tData.tState == TableState.waitPut && pl.mjState == TableState.waitPut) || ( pl.eatFlag  == 16 && tData.doAfterCanLiuList[pl.info.uid]
                    ...
                    &&  !MjClient.isCommon && !MjClient.hasPut)
                {
                    ...
                    var putCard = getNewCard_YuanJiangGuiHuZi(btnTag, 3 ,off);
                    ...
                    HZPutCardToServer_YuanJiangGuiHuZi(btnTag);
                    MjClient.playui.ResetHandCard(MjClient.playui._downNode,off);
                    ShowPutCardIcon_YuanJiangGuiHuZi();
                }
                else
                {
                    ...
                    if(!isNoPut && !isWangBa && IsTurnToMe() && tData.tState == TableState.waitPut && pl.mjState == TableState.waitPut)
                    {
                        if(!MjClient.isCommon && cutLine.visible && endPos.y >= cutLine.y && !MjClient.hasPut)
                        {
                            ...
                            newCheckTingCards_YuanJiangGuiHuZi(btnTag);
                            MjClient.addGroupIndex = -1;    //新增列标志
                        }
                    }
                }
            ...
            if(!isPut || isWangBa )
            {
                ...
                if(selectParentTag == btnParent.tag){
                    ...
                    if(selectArr){
                        ...
                        MjClient.HandCardArr[btnParent.tag].splice(parseInt(btnName),1);
                        _fixArrIndex_YuanJiangGuiHuZi(selectArr, btnTag, btn);
                    }
                }
            ...
            MjClient.touchCell = null;
            MjClient.copyCell = null;
            if (endPos.y < cutLine.y && (!pl.canNotPutCard||pl.canNotPutCard.indexOf(btnTag)==-1)  && IsTurnToMe()
                && (tData.tState == TableState.waitPut && pl.mjState == TableState.waitPut)
        ...
        //     if (movingCard_YuanJiangGuiHuZi==null) return;
        //     movingCard_YuanJiangGuiHuZi = null;
        //     MjClient.playui.ResetHandCard(MjClient.playui._downNode,off);
        //     ShowPutCardIcon_YuanJiangGuiHuZi();
        // }
    });
}
function cleanCardNode_YuanJiangGuiHuZi(off) {
    //获取结点
    if(off != 0){
        return;
    }
...
//根据坐标Y，添加到数组对应的位置
function _fixArrIndex_YuanJiangGuiHuZi(arr, btnTag, btn){
    if(_isKan_YuanJiangGuiHuZi(arr)){
        arr.push(btnTag);
...
function resetCardNode_YuanJiangGuiHuZi(off) {
    if(off != 0){
        return;
    }
    var _node = getNode_YuanJiangGuiHuZi(off);
...
//改变数组元素的位置
function _changeArrIndex_YuanJiangGuiHuZi(arr, btnTag, btn){
    if(!_isKan_YuanJiangGuiHuZi(arr)){
        var len = arr.length;
        for(var i = 0; i < len; i++){
            var tag = arr[i];
        ...
        _fixArrIndex_YuanJiangGuiHuZi(arr, btnTag, btn);
    }
}

//数组里的是否为三个相同的牌
function _isKan_YuanJiangGuiHuZi(arr){
    var len = arr.length;
    if(len < 3){
        return false;
    }
...
//断线重连之后，显示之前玩家打的牌
function DealOffLineCard_YuanJiangGuiHuZi(node, off){
    cc.log("====================DealOffLineCard_YuanJiangGuiHuZi=================");
    cc.log("======DealOffLineCard_YuanJiangGuiHuZi======= " + off + " node = " + node.getName());
    ...
    if(typeof(tData.currCard) != "undefined" && tData.currCard != -1){
        var uids = tData.uids;
        var isNotShow = false;
        var selfIndex = (uids.indexOf(SelfUid()) + off) % MjClient.MaxPlayerNum_YuanJiangGuiHuZi;
        if(uids[selfIndex] == uids[tData.curPlayer]){
            if(uids[selfIndex] != SelfUid() && tData.mjHideCard[uids[selfIndex]].indexOf(tData.currCard) >= 0)
            {
                isNotShow = true;
            }
            var putCard = getNewCard_YuanJiangGuiHuZi(tData.currCard, 3, off, isNotShow);
...
//发牌
MjClient.isCommon = false;
function DealNewCard_YuanJiangGuiHuZi(node, msg, off){
    cc.log("======DealNewCard_YuanJiangGuiHuZi======= " + off + " node = " + node.getName());
    if(off == 0 && node.getName() != "down"){
        return;//2人玩法right节点同为玩家节点
    }
    //没有发牌过来的时候直接忽略
    if(!msg.newCard){
        RemovePutCardOut_YuanJiangGuiHuZi();
    ...
    if(uids[selfIndex] == msg.uid){
        ...
        if(lastPlayer != -1){
            //var lastOff = (off + MjClient.MaxPlayerNum_YuanJiangGuiHuZi - selfIndex) % MjClient.MaxPlayerNum_YuanJiangGuiHuZi;
            RemovePutCardOut_YuanJiangGuiHuZi();
        }
        ...
        var putCard = getNewCard_YuanJiangGuiHuZi(msg.newCard, 3, off, showPkBack);
        var putNode = node.getChildByName("put");
        ...
        if(!msg.isCommon){
            if(MjClient.rePlayVideo == -1){
                ...
                putNode.runAction(cc.sequence(
                    cc.delayTime(0.2),
                    cc.callFunc(function(){RemovePutCardOut_YuanJiangGuiHuZi();})
                ));
                for(var i = 0;i < cardArr.length;i++){
                    var tmpArr = cardArr[i];
                    if(tmpArr.length <= 2){
...
function DealPassWeiCard(node, msg, off){
    ...
    //清理之前玩家打的牌
    var pl = sData.players[uids[selfIndex]];
    var putCard = getNewCard_YuanJiangGuiHuZi(msg.lastPutCard, 3, off);
    putNode.stopAllActions();
    putNode.removeAllChildren();
    putCard.scaleX = putCard.width/putNode.width;
    putCard.scaleY = putCard.width/putNode.width;
...
//翻醒发牌
function DealFanXingNewCard_YuanJiangGuiHuZi(card){
    var putCard = getNewCard_YuanJiangGuiHuZi(card, 4, 0);
...
//王闯/王钓
function DealWangChuang_YuanJiangGuiHuZi(node,msg,off){
    var sData = MjClient.data.sData;
    var tData = sData.tData;
    ...
    if(uids[selfIndex] == msg.uid){
        ...
        if(pl.wangType == 1){
            var putCard = getNewCard_YuanJiangGuiHuZi(91, 3, off);
            var putNode = node.getChildByName("put");
            putCard.scaleX = putCard.width/putNode.width;
            putCard.scaleY = putCard.width/putNode.width;
            putCard.x = putCard.width/2;
            ...
            ShowEatActionAnim_YuanJiangGuiHuZi(node, ActionType_YuanJiangGuiHuZi.WANGDIAO, off);

            var delay = cc.delayTime(1.5);
            var callback = function(){
                RemovePutCardOut_YuanJiangGuiHuZi();
                //HZNewCardToServer_YuanJiangGuiHuZi();
            }
            //putNode.runAction(cc.sequence(delay,cc.callFunc(callback)));
        }else if(pl.wangType == 2){
            var putCard = getNewCard_YuanJiangGuiHuZi(91, 3, off);
            var putCard1 = getNewCard_YuanJiangGuiHuZi(91, 3, off);
            var putNode = node.getChildByName("put");
            putCard.scaleX = putCard.width/putNode.width;
            putCard.scaleY = putCard.width/putNode.width;
            ...
            ShowEatActionAnim_YuanJiangGuiHuZi(node, ActionType_YuanJiangGuiHuZi.WANGCHUANG, off);
            var delay = cc.delayTime(1.5);
            var callback = function(){
                RemovePutCardOut_YuanJiangGuiHuZi();
                //HZNewCardToServer_YuanJiangGuiHuZi();
            ...
            var putCard = getNewCard_YuanJiangGuiHuZi(91, 3, off);
            var putCard1 = getNewCard_YuanJiangGuiHuZi(91, 3, off);
            var putCard2 = getNewCard_YuanJiangGuiHuZi(91, 3, off);
            var putNode = node.getChildByName("put");
            putCard.scaleX = putCard.width/putNode.width;
            putCard.scaleY = putCard.width/putNode.width;
            putCard1.scaleX = putCard.width/putNode.width;
            ...
            ShowEatActionAnim_YuanJiangGuiHuZi(node, ActionType_YuanJiangGuiHuZi.WANGZHA, off);
            var delay = cc.delayTime(1.5);
            var callback = function(){
                RemovePutCardOut_YuanJiangGuiHuZi();
                //HZNewCardToServer_YuanJiangGuiHuZi();
...
//处理出牌
function DealPutCard_YuanJiangGuiHuZi(node, msg, off){
    cc.log("======DealPutCard_YuanJiangGuiHuZi======= " + off + " node = " + node.getName());
    if(off == 0 && node.getName() != "down"){
        return;//2人玩法right节点同为玩家节点
    }
    MjClient.hasPut = false;
    ...
    if(uids[selfIndex] == msg.uid)
    {
        ...
        if (SelfUid() != msg.uid || MjClient.rePlayVideo != -1)
        {
            ...
            putNode.stopAllActions();
            putNode.removeAllChildren();
            var putCard = getNewCard_YuanJiangGuiHuZi(msg.card, 3 ,off);
            putCard.scaleX = putCard.width/putNode.width;
            putCard.scaleY = putCard.width/putNode.width;
            putCard.x = putCard.width/2;
            putCard.y = putCard.height/2;
        ...
        if (off == 0) {
            removeTingSign_YuanJiangGuiHuZi(node);
        }

        if(MjClient.rePlayVideo != -1){
            RemoveHandCard_YuanJiangGuiHuZi(node,msg.card,off);
            MjClient.playui.ResetHandCard(node,off);
        }
    }
}

function checkCard_YuanJiangGuiHuZi(node, off){
    ...
    if(cardHandArr.length > cardHand.length){
        ...
        for(var i = 0;i < cardHandArr.length; i++){
            if(cardHandArr[i] != cardHand[i]){
                RemoveHandCard_YuanJiangGuiHuZi(node, cardHandArr[i], off);
...
function calculateHintPutList_YuanJiangGuiHuZi() {
    MjClient.hintPutList_ziPai = MjClient.majiang_yuanJiangGuiHuZi.hintPutCardsToTing();
    // cc.log('MjClient.hintPutList_ziPai@@ ', MjClient.hintPutList_ziPai);
}

function addTingSign_YuanJiangGuiHuZi(node) {
...
function removeTingSign_YuanJiangGuiHuZi(node) {
    var handNode = node.getChildByName("handNode");
    if (!handNode) {
        return;
    }
...
//吃牌
function DealChiCard_YuanJiangGuiHuZi(node, msg, off) {
    cc.log("======DealChiCard_YuanJiangGuiHuZi======= " + off + " node = " + node.getName());
    if(off == 0 && node.getName() != "down"){
        return;//2人玩法right节点同为玩家节点
    }
    var sData = MjClient.data.sData;
    ...
    if (tData.curPlayer == selfIndex) {
        ...
        for (var i = 0; i < tmpEatCards.length; i++) {
            RemoveHandCard_YuanJiangGuiHuZi(node, tmpEatCards[i], off);
        }
            ...
            for (var k = 0; k < biCards.length; k++) {
                ...
                for (var m = 0; m < biArr.length; m++) {
                    if (tmpBiArr.indexOf(biArr[m]) >= 0) {
                        ...
                        RemoveHandCard_YuanJiangGuiHuZi(node, biArr[m], off);
                    }
                }
            }
        }
        ...
        for (var i = 0; i < eatCards.length; i++) {
            var card = getNewCard_YuanJiangGuiHuZi(eatCards[i], 2, off);
            if (i == 2) {
            ...
            card.zIndex = 4 - i;
            card.x = 0;
            card.y = off_count * getOffY_YuanJiangGuiHuZi(node, card);
            setChiCardAnchorPoint_YuanJiangGuiHuZi(node, card);
        ...
        if (biCards) {
            ...
            for (var k = 0; k < biCards.length; k++) {
                var biArr = biCards[k];
                for (var m = 0; m < biArr.length; m++) {
                    var card = getNewCard_YuanJiangGuiHuZi(biArr[m], 2, off);
                    if (m == 2) {
                        card.setColor(cc.color(170, 170, 170));
                    } 
                    setChiCardAnchorPoint_YuanJiangGuiHuZi(node, card);
        ...
        var parentPos = getChiCardParentPosition_YuanJiangGuiHuZi(node, cardWidth, off, selfColCount);
        var moveX0 = 0;
        ...
        parent.x = moveX0;
        setCardParentPosY_YuanJiangGuiHuZi(node, parent);
        ...
        if (biCards) {
            ShowEatActionAnim_YuanJiangGuiHuZi(node, ActionType_YuanJiangGuiHuZi.XIAHUO, off);
        } else {
            ShowEatActionAnim_YuanJiangGuiHuZi(node, ActionType_YuanJiangGuiHuZi.CHI, off);
        }
    }
    //MjClient.playui.CardLayoutRestore(node,off);
}

function setChiCardAnchorPoint_YuanJiangGuiHuZi(node,newCard) {
    if(node.getName() == "down"){
        newCard.anchorX = 0;
        newCard.anchorY = 0;
    }else if(node.getName() == "right"){
...
function getOffY_YuanJiangGuiHuZi(node,newCard) {
    var off_y = 0;
    if(node.getName() == "down"){
        off_y = newCard.height;
    }else if(node.getName() == "right"){
...
function getChiCardParentPosition_YuanJiangGuiHuZi(node, newCardWidth, off, selfColCount, msg) {
    selfColCount = selfColCount || 1;
    var eatNode = node.getChildByName("eatNode");
    // var parentCount = eatNode.getChildrenCount() - 1;

    var pl = getUIPlayer_YuanJiangGuiHuZi(off);
     var totalColCount = pl.mjpeng.length + pl.mjwei.length + pl.mjgang0.length + pl.mjgang1.length + pl.mjgang2.length + pl.mjgang3.length;
...
function setCardParentPosY_YuanJiangGuiHuZi(node, cardParent) {
    var eatNode = node.getChildByName("eatNode");
    if(node.getName() == "down"){
        cardParent.y = 0;
    }else if (node.getName() == "right"){
...
function getShowCardIdx_YuanJiangGuiHuZi(node, chiTip) {
    var idx;
    switch(node.getName()) {
        case "down":
        case "xing":
...
// 处理碰
function DealPengCard_YuanJiangGuiHuZi(node, msg, off){
    cc.log("======DealPengCard_YuanJiangGuiHuZi======= " + off + " node = " + node.getName());
    if(off == 0 && node.getName() != "down"){
        return;//2人玩法right节点同为玩家节点
    }
    var sData = MjClient.data.sData;
    ...
    if(tData.curPlayer == selfIndex){
        ...
        for(var i = 0;i < 3; i++) {
            var card = getNewCard_YuanJiangGuiHuZi(pengCard,2,off);
            cardWidth = card.width//*eatNode.scaleX;
            var childCount = parent.childrenCount;
            card.zIndex = 4 - childCount;
            card.x = 0;
            card.y = childCount * getOffY_YuanJiangGuiHuZi(node, card);
            setChiCardAnchorPoint_YuanJiangGuiHuZi(node, card);
            parent.addChild(card);
        }
        eatNode.addChild(parent);

        RemoveHandCard_YuanJiangGuiHuZi(node,pengCard,off);
        RemoveHandCard_YuanJiangGuiHuZi(node,pengCard,off);
        MjClient.playui.ResetHandCard(node, off);
        ShowEatActionAnim_YuanJiangGuiHuZi(node, ActionType_YuanJiangGuiHuZi.PENG, off);
        ...
        var parentPos = getChiCardParentPosition_YuanJiangGuiHuZi(node, cardWidth, off, 1);
        var moveX0 = 0;
        var outNodeCount = eatNode.getChildrenCount();
        if(outNodeCount>0){
            if(node.getName() == "down" || node.getName() == "left"){
...
//偎牌
function DealWeiCard_YuanJiangGuiHuZi(node, msg, off) {
    cc.log("======DealWeiCard_YuanJiangGuiHuZi======= " + off + " node = " + node.getName());
    if(off == 0 && node.getName() != "down"){
        return;//2人玩法right节点同为玩家节点
    }
    var sData = MjClient.data.sData;
    ...
    if (tData.curPlayer == selfIndex) {
        RemovePutCardOut_YuanJiangGuiHuZi();
        ShowEatActionAnim_YuanJiangGuiHuZi(node, ActionType_YuanJiangGuiHuZi.WEI, off);
        ...
        for (var i = 0; i < 3; i++) {
            ...
            {
                if (i == getShowCardIdx_YuanJiangGuiHuZi(node, "wei")) {
                    isTurn = false;
                }
                var card = getNewCard_YuanJiangGuiHuZi(weiCard, 2, off, isTurn);
            } else if (MjClient.majiang.getCardShowType(tData.uids[tData.curPlayer], weiCard) == 1) {
                var card = getNewCard_YuanJiangGuiHuZi(weiCard, 2, off);
                ...
                var card = getNewCard_YuanJiangGuiHuZi(weiCard, 2, off, true);
            }
            cardWidth = card.width //*eatNode.scaleX;
            card.zIndex = 4 - i;
            card.x = 0;
            card.y = i * getOffY_YuanJiangGuiHuZi(node, card);
            setChiCardAnchorPoint_YuanJiangGuiHuZi(node, card);
            parent.addChild(card);
        }
        eatNode.addChild(parent);

        RemoveHandCard_YuanJiangGuiHuZi(node, weiCard, off);
        RemoveHandCard_YuanJiangGuiHuZi(node, weiCard, off);
        MjClient.playui.ResetHandCard(node, off);
...
// 跑牌或者提牌
function DealGangCard_YuanJiangGuiHuZi(node, msg, off) {
    cc.log("======DealGangCard_YuanJiangGuiHuZi======= " + off + " node = " + node.getName());
    if(off == 0 && node.getName() != "down"){
        return;//2人玩法right节点同为玩家节点
    }
    var sData = MjClient.data.sData;
    ...
    if (uids.indexOf(gangUid) == selfIndex) {
        if (msg.type == 2) {
            RemovePutCardOut_YuanJiangGuiHuZi();
            ShowEatActionAnim_YuanJiangGuiHuZi(node, ActionType_YuanJiangGuiHuZi.PAO, off);
        }else if (msg.type == 3) {
            RemovePutCardOut_YuanJiangGuiHuZi();
            ShowEatActionAnim_YuanJiangGuiHuZi(node, ActionType_YuanJiangGuiHuZi.ZHA, off);
        }else {
            if (!msg.isGangHand && msg.type == 1) {
                RemovePutCardOut_YuanJiangGuiHuZi();
            }
            ShowEatActionAnim_YuanJiangGuiHuZi(node, ActionType_YuanJiangGuiHuZi.TI, off);
        }
        ...
        for (var i = 0; i < 4; i++) {
            ...
            if (msg.type == 2) {
                isTurn = false;
                var card = getNewCard_YuanJiangGuiHuZi(gangCard, 2, off, isTurn);
            ...
            {
                if (i == getShowCardIdx_YuanJiangGuiHuZi(node, "ti")) {
                    isTurn = false;
                }
                var card = getNewCard_YuanJiangGuiHuZi(gangCard, 2, off, isTurn);
            } else if (MjClient.majiang.getCardShowType(tData.uids[tData.curPlayer], gangCard) == 1) {
                var card = getNewCard_YuanJiangGuiHuZi(gangCard, 2, off, false);
                var shade = new cc.Sprite("playing/paohuzi/huxiBG1.png");
                shade.opacity = 100;
                shade.x = shade.width / 2;
                shade.y = shade.height / 2;
                ...
                var card = getNewCard_YuanJiangGuiHuZi(gangCard, 2, off, true);
            }*/
            var isTurn = true;
            if (i == getShowCardIdx_YuanJiangGuiHuZi(node, "ti")) {
                if(tData.areaSelectMode["anWaiLiu"] || msg.isGangHand)
                {
            ...
            if(msg.type == 2 ||msg.type == 3){
                isTurn = false;
                card = getNewCard_YuanJiangGuiHuZi(gangCard, 2, off, isTurn);
            }
            // else {
        ...
        RemoveHandCard_YuanJiangGuiHuZi(node, gangCard, off);
        RemoveHandCard_YuanJiangGuiHuZi(node, gangCard, off);
        RemoveHandCard_YuanJiangGuiHuZi(node, gangCard, off);
        if (msg.isGangHand) {
            RemoveHandCard_YuanJiangGuiHuZi(node, gangCard, off);
        ...
        var parentPos = getChiCardParentPosition_YuanJiangGuiHuZi(node, cardWidth, off, 1, msg);
        var moveX0 = 0;
        var outNodeCount = eatNode.getChildrenCount();
        if (outNodeCount > 0) {
            if (node.getName() == "down" || node.getName() == "left") {
...
// 处理胡
function DealHu_YuanJiangGuiHuZi(node, msg, off){
    var sData = MjClient.data.sData;
    var tData = sData.tData;
    var selfIndex = (tData.uids.indexOf(SelfUid()) + off) % MjClient.MaxPlayerNum_YuanJiangGuiHuZi;
    var pl = getUIPlayer_YuanJiangGuiHuZi(off);
    ...
    if(pl){
        pl.eatFlag = 0;
        MjClient.playui.EatVisibleCheck();
        ShowEatActionAnim_YuanJiangGuiHuZi(node,ActionType_YuanJiangGuiHuZi.HU,off);
    }
}

function DealAddCard_YuanJiangGuiHuZi(node,msg, off){
    if(MjClient.rePlayVideo == -1 && off != 0){
        return;
    }
    var cardArr = MjClient.HandCardArr;
...
//清除打出的牌
function RemovePutCardOut_YuanJiangGuiHuZi(noAction){
...
//发手牌时再清理一遍
function clearMJHandCardUI_YuanJiangGuiHuZi() {
    // var handNode = MjClient.playui._downNode.getChildByName("handNode");
    ...
    if((MjClient.gameType == MjClient.GAME_TYPE.LUO_DI_SAO || MjClient.gameType == MjClient.GAME_TYPE.JIANG_YONG_15Z) && MjClient.MaxPlayerNum_YuanJiangGuiHuZi == 4){
    ...
    RemovePutCardOut_YuanJiangGuiHuZi(true);
}

//出牌表示状态
function ShowPutCardIcon_YuanJiangGuiHuZi() {
    ...
    if(MjClient.hasPut) {
        ...
        // if(MjClient.movingCard_YuanJiangGuiHuZi){
        //  cutLine.visible = status;
        // }
}

//清除手牌
function RemoveHandCard_YuanJiangGuiHuZi(node,card,off){
    if(MjClient.rePlayVideo == -1 && off != 0){
        return;
    }
    var cardArr = MjClient.HandCardArr;
...
// 初始化吃碰杠胡动作
function InitShowEatActionNode_YuanJiangGuiHuZi(plNode){
...
// 重置吃碰杠胡动作
function resetEatActionAnim_YuanJiangGuiHuZi()
{
    var jsBind = MjClient.playui.jsBind;
    ...
    var count = MjClient.MaxPlayerNum_YuanJiangGuiHuZi;
    if(count == 2){
    ...
    {
        if(ui[i])
            InitShowEatActionNode_YuanJiangGuiHuZi(ui[i]._node);
    }
}

//播放头像移动
function tableStartHeadMoveAction_YuanJiangGuiHuZi(node){
...
//重置3家头像位置
function reConectHeadLayout_YuanJiangGuiHuZi(node){
    var tData = MjClient.data.sData.tData;
    var down = node.getChildByName("down").getChildByName("head");
    var left = node.getChildByName("left").getChildByName("head");
    var right = node.getChildByName("right").getChildByName("head");
    ...
    resetEatActionAnim_YuanJiangGuiHuZi();
...
//根据偏移获得玩家node
function getNode_YuanJiangGuiHuZi(off){
    var _node = null;
    if(MjClient.gameType == MjClient.GAME_TYPE.PAO_HU_ZI_SR || MjClient.gameType == MjClient.GAME_TYPE.PAO_HU_ZI_SR_King){
        var tData = MjClient.data.sData.tData;
        var selfIndex = tData.uids.indexOf(SelfUid());
        if(selfIndex == tData.xingPlayer){
            // off = (tData.zhuang + 2 + off + MjClient.MaxPlayerNum_YuanJiangGuiHuZi) % MjClient.MaxPlayerNum_YuanJiangGuiHuZi;
            if(off == MjClient.MaxPlayerNum_YuanJiangGuiHuZi - 1){
    ...
    if(MjClient.MaxPlayerNum_YuanJiangGuiHuZi == 4 && (MjClient.gameType == MjClient.GAME_TYPE.LUO_DI_SAO || 
 ...
 * @return {object} 玩家数据
 */
function getUIPlayer_YuanJiangGuiHuZi(off){
    var sData = MjClient.data.sData;
    var tData = sData.tData;
    var uids = tData.uids;
    var selfIndex = uids.indexOf(SelfUid());
    selfIndex = (selfIndex + off) % MjClient.MaxPlayerNum_YuanJiangGuiHuZi;
...
// 获取ui头像，通过偏移值
function getUIHeadByOff_YuanJiangGuiHuZi(off){
    var pl = getUIPlayer_YuanJiangGuiHuZi(off);
    if(!pl)
    {
        return {};
 ...
 * 下载录音, 调用 播放函数
 * */
function downAndPlayVoice_YuanJiangGuiHuZi(uid, filePath){
    var index = getUiOffByUid_YuanJiangGuiHuZi(uid);
...
function getUiOffByUid_YuanJiangGuiHuZi(uid){
    var sData = MjClient.data.sData;
    var tData = sData.tData;
    var uids = tData.uids;
    var selfIndex = uids.indexOf(SelfUid());
    var targetIndex = uids.indexOf(uid);
    return (targetIndex - selfIndex + MjClient.MaxPlayerNum_YuanJiangGuiHuZi) % MjClient.MaxPlayerNum_YuanJiangGuiHuZi;
}

//设置微信头像
function setWxHead_YuanJiangGuiHuZi(node, d, off){
    if(d.uid == getUIHeadByOff_YuanJiangGuiHuZi(off).uid){
        ...
        COMMON_UI.addNobleHeadFrame(nobody,getUIPlayer_YuanJiangGuiHuZi(off))
    }
}

//显示玩家庄的ui
function showUserZhuangLogo_YuanJiangGuiHuZi(node, off){
    var tData = MjClient.data.sData.tData;
    var pl = getUIPlayer_YuanJiangGuiHuZi(off);
    node.zIndex = 100;
    if(tData && pl){
...
//显示房主  add by sking
function showFangzhuTagIcon_YuanJiangGuiHuZi(node,off)
{
    var pl = getUIPlayer_YuanJiangGuiHuZi(off);
    if(!pl) //位置上没人则删掉房主标签
    {
...
// 清理ui
function clearCardUI_YuanJiangGuiHuZi(node,off){
    mylog("clearCardUI_YuanJiangGuiHuZi");
    ...
    for(var i = 0; i < children.length; i++)
    {
        ...
        {
            InitShowEatActionNode_YuanJiangGuiHuZi(ni.getParent());
        }
    }
}

function EatFlag_YuanJiangGuiHuZi(){
    var eat = MjClient.playui.jsBind.eat;
    var eatFlag = 0;
...
function getCardSocre_YuanJiangGuiHuZi(cards) {
    //cc.log("chow", "getCardSocre_YuanJiangGuiHuZi " + JSON.stringify(cards));
    if(cards.length != 3){
        return 0;
    }
...
function huXiScore_YuanJiangGuiHuZi(type,card)
{
...
//刷新胡息
function UpdateHuXi_YuanJiangGuiHuZi(off){
    var sData = MjClient.data.sData;
    var tData = sData.tData;
    var score = 0;
    var pl = getUIPlayer_YuanJiangGuiHuZi(off);
    ...
    //跑牌
    var mjpao = pl.mjgang0.concat(pl.mjgang3);
    if(mjpao.length>0){
        for(var p=0;p<mjpao.length;p++){
            score += huXiScore_YuanJiangGuiHuZi("paoPai",mjpao[p]);
        }
    }
    ...
    if(mjpeng.length>0){
        for(var i=0;i<mjpeng.length;i++){
            score += huXiScore_YuanJiangGuiHuZi("peng",mjpeng[i]);
        }
    }
        ...
        var chiScore = function(cards){
            ...
            for(var k=0;k<chiXiArr.length;k++){
                ...
                if(cards.toString() == chiXiList.toString()){
                    score += huXiScore_YuanJiangGuiHuZi("chi",cards[0]);
                }
            }
        };
    ...
    if((MjClient.gameType == MjClient.GAME_TYPE.HY_LIU_HU_QIANG || MjClient.gameType == MjClient.GAME_TYPE.ML_HONG_ZI)&&pl.info.uid!=SelfUid()){
        ...
        if(mjti.length>0){
            for (var t = 0; t < mjti.length; t++) {
                if(MjClient.majiang.getCardShowType(pl.info.uid, mjti[t]) != 0){
                    score += huXiScore_YuanJiangGuiHuZi("tiPai", mjti[t]);
                }
            }
        }
        //偎牌
        ...
        //             score += huXiScore_YuanJiangGuiHuZi("weiPai", mjwei[w]);
        //         }
        //     }
        // }
    }else {
    ...
    if(tData.xingPlayer == selfIndex){
        pl = getUIPlayer_YuanJiangGuiHuZi(2);
        //提牌
        var mjti = pl.mjgang1;
        if(mjti.length>0){
            for(var t=0;t<mjti.length;t++){
...
//设置转盘显示状态
function IsArrowVisible_YuanJiangGuiHuZi(){
    var pl = getUIPlayer_YuanJiangGuiHuZi(0);
    if (!pl)
    {
        return;
...
// 显示吃碰杠胡动作
function ShowEatActionAnim_YuanJiangGuiHuZi(node, actType, off){
    var delayTime = 1;
    switch(actType)
    {
        case ActionType_YuanJiangGuiHuZi.FANXING:
            delayTime = 1.6;
            break;
        default:
            break;
    ...
    switch(actType)
    {
        case ActionType_YuanJiangGuiHuZi.CHI:
        ...
        case ActionType_YuanJiangGuiHuZi.PENG:
        ...
        case ActionType_YuanJiangGuiHuZi.WEI:
        ...
        case ActionType_YuanJiangGuiHuZi.HU:

            eatActionChild = eatActionNode.getChildByName("hu");
            eatActionChild.visible = true;
            break;
        case ActionType_YuanJiangGuiHuZi.PAO:
            eatActionChild = eatActionNode.getChildByName("pao");
            eatActionChild.loadTexture("playing/paohuzi/t_piao.png");
            eatActionChild.visible = true;
            eatActionChild.runAction(cc.sequence(cc.delayTime(delayTime), cc.callFunc(callback)));
            break;
        case ActionType_YuanJiangGuiHuZi.ZHA:
            eatActionChild = eatActionNode.getChildByName("zha");
            eatActionChild.visible = true;
            eatActionChild.runAction(cc.sequence(cc.delayTime(delayTime), cc.callFunc(callback)));
            break;
        case ActionType_YuanJiangGuiHuZi.TI:
            ...
            eatActionChild.runAction(cc.sequence(cc.delayTime(delayTime), cc.callFunc(callback)));
            break;
        case ActionType_YuanJiangGuiHuZi.WANGDIAO:
            eatActionChild = eatActionNode.getChildByName("wangDiao");
            eatActionChild.visible = true;
            eatActionChild.runAction(cc.sequence(cc.delayTime(delayTime), cc.callFunc(callback)));
            break;
        case ActionType_YuanJiangGuiHuZi.WANGCHUANG:
            eatActionChild = eatActionNode.getChildByName("wangChuang");
            eatActionChild.visible = true;
            eatActionChild.runAction(cc.sequence(cc.delayTime(delayTime), cc.callFunc(callback)));
            break;
        case ActionType_YuanJiangGuiHuZi.XIAHUO:
...
//显示玩家信息
function showPlayerInfo_YuanJiangGuiHuZi(off, node){
    //var tData = MjClient.data.sData.tData;
    var pl = getUIPlayer_YuanJiangGuiHuZi(off);
    if(pl){
...
function resetChiParam_YuanJiangGuiHuZi(){

}

function setChiVisible_YuanJiangGuiHuZi(){
    var eat = MjClient.playui.jsBind.eat;
    ...
    eat.zha._node.setTouchEnabled(false);
}

function showSelectEatCards_YuanJiangGuiHuZi(node,cardNum){
    var eat = MjClient.playui.jsBind.eat;
    eat.chi._node.visible = false;
    eat.guo._node.visible = false;
    eat.peng._node.visible = false;
    ...
    addEatCards_YuanJiangGuiHuZi(parent,cardArr,putCard, pl);
}

function showSelectLiuCards_YuanJiangGuiHuZi(node,type){
    var eat = MjClient.playui.jsBind.eat;
    eat.chi._node.visible = false;
    eat.guo._node.visible = false;
    eat.peng._node.visible = false;
    ...
    addLiuCards_YuanJiangGuiHuZi(parent, pl, type);
}

doMoveCenterAction_YuanJiangGuiHuZi = function(arr,isScale){
    arr.reverse();
    var gap = 5;
...
//添加可以吃的牌
function addEatCards_YuanJiangGuiHuZi(node,handCardArr,putCard, pl){
    ...
    for(var i = 0;i < chiSet.length;i++){
        ...
        for(var k = 0;k < cardList.length;k++){
            ...
            cardNode.loadTexture(MjClient.cardPath_YuanJiangGuiHuZi+"hand" + card + ".png");
        ...
        chiCloneBtn.addClickEventListener(function(chiBtn){
            ...
            //     setChiVisible_YuanJiangGuiHuZi();
            //     HZChiToServer_YuanJiangGuiHuZi(chiArr,null);
            // }else{
            //     var biSet = MjClient.majiang.getBiCards(tmpHandCardArr,putCard);
            //     if(biSet && biSet.length > 0){
            //         addFirstBiCards_YuanJiangGuiHuZi(node,biSet,putCard,tmpHandCardArr,chiArr);
            //     }
            // }
            // setChiVisible_YuanJiangGuiHuZi();
            // HZChiToServer_YuanJiangGuiHuZi(chiArr,null);
            commitEatCards_YuanJiangGuiHuZi(chiArr, null);
        });
    }

    if(ziPai.getUiLayoutType() == 0){//新布局
        doMoveCenterAction_YuanJiangGuiHuZi([chiBg], false);
    }
}

//添加可以溜的牌
function addLiuCards_YuanJiangGuiHuZi(node, pl, type){
    ...
    for(var i = 0; i < cards.length; i++){
        ...
        for(var j = 0; j < 4; j++){
            ...
            cardClone.loadTexture(MjClient.cardPath_YuanJiangGuiHuZi+"hand" + cards[i] + ".png");
        ...
        selectBtnClone.addClickEventListener(function(chiBtn){
            ...
            commitLiuCards_YuanJiangGuiHuZi(chiBtn.tag, type);
        });
    }
...
//添加第一个比牌
function addFirstBiCards_YuanJiangGuiHuZi(node,biSet,putCard,handCardArr,chiArr){
    ...
    for(var i = 0;i < biSet.length;i++){
        ...
        cloneBtn.addClickEventListener(function(btn){
            ...
            if(tmpHandCardArr.indexOf(putCard) >= 0){
                var biSet = MjClient.majiang.getBiCards(tmpHandCardArr,putCard);
                if(biSet && biSet.length > 0){
                    addSecBiCards_YuanJiangGuiHuZi(node,biSet,putCard,tmpHandCardArr,chiArr,biArr);
                }
            }else{
                setChiVisible_YuanJiangGuiHuZi();
                HZChiToServer_YuanJiangGuiHuZi(chiArr,[biArr]);
            }
        });
    }
...
//添加第二个比牌
function addSecBiCards_YuanJiangGuiHuZi(node,biSet,putCard,handCardArr,chiArr,firstBiArr){
    ...
    for(var i = 0;i < biSet.length;i++){
        ...
        cloneBtn.addClickEventListener(function(btn){
            ...
            btn.setBright(false);
            setChiVisible_YuanJiangGuiHuZi();
            HZChiToServer_YuanJiangGuiHuZi(chiArr,[firstBiArr,biArr]);
        });
    }
...
function GetReadyVisible_YuanJiangGuiHuZi(node, off) {
    if (off < 0) {
        node.visible = false;
        return false;
    }
...
//设置玩家掉线头像
function setUserOffline_YuanJiangGuiHuZi(node, off){
    var pl = getUIPlayer_YuanJiangGuiHuZi(off);
    if(!pl){
        return;
    }
...
//显示玩家文字
function showUserChat_YuanJiangGuiHuZi(node, off, msg){
    var tData = MjClient.data.sData.tData;
 ...
 */
function curPlayerIsMe_YuanJiangGuiHuZi(off){
    var sData = MjClient.data.sData;
    var tData = sData.tData;
...
function getVisablePlayerCount_YuanJiangGuiHuZi(){
    var sData = MjClient.data.sData;
    var tData = sData.tData;
    var uids = tData.uids;
    var count = 0;
 ...
 */
function getXingPlayerIndex_YuanJiangGuiHuZi(){
    if(MjClient.MaxPlayerNum_YuanJiangGuiHuZi != 4){
        return -1;
    }
    if(getVisablePlayerCount_YuanJiangGuiHuZi() < 3){
        return -1;
    }
    var sData = MjClient.data.sData;
    var tData = sData.tData;
    ...
    var xingIndex = (zhuang + 2 + MjClient.MaxPlayerNum_YuanJiangGuiHuZi) % MjClient.MaxPlayerNum_YuanJiangGuiHuZi;
    var selfIndex = tData.uids.indexOf(SelfUid());
    xingIndex = (xingIndex - selfIndex + MjClient.MaxPlayerNum_YuanJiangGuiHuZi) % MjClient.MaxPlayerNum_YuanJiangGuiHuZi;
    return xingIndex;
}
 ...
 *                  提供 getUIPlayer_YuanJiangGuiHuZi(resoff) 获取 off位置玩家数据
 */
function getOffByXing_YuanJiangGuiHuZi(off){
    var resOff = off
    if(MjClient.MaxPlayerNum_YuanJiangGuiHuZi != 4){
        return resOff;
    }
    ...
    if(selfIndex == xingIndex){ // xing为off固定为2， 参数off
        resOff = (2 + off) % MjClient.MaxPlayerNum_YuanJiangGuiHuZi;
    }else{
        // 如果我的下家是醒家，且是ui层传过来的是位置是1 则跳过
        if((selfIndex + 1 + MjClient.MaxPlayerNum_YuanJiangGuiHuZi) % MjClient.MaxPlayerNum_YuanJiangGuiHuZi == xingIndex && off == 1){
            resOff = off + 1;

            // 如果我的上家是醒家，且是ui层传过来的是位置是3 则跳过
        }else if((selfIndex -1 + MjClient.MaxPlayerNum_YuanJiangGuiHuZi) % MjClient.MaxPlayerNum_YuanJiangGuiHuZi == xingIndex
            && off == MjClient.MaxPlayerNum_YuanJiangGuiHuZi - 1){
            resOff = off - 1;

            // 如果我的上家是醒家, 且 传过来的是位置是2 , (发表情时发的是2）
        }else if( off==2 ){
            if((selfIndex + 1 + MjClient.MaxPlayerNum_YuanJiangGuiHuZi) % MjClient.MaxPlayerNum_YuanJiangGuiHuZi == xingIndex ){
                resOff = 1;
            }
            if((selfIndex - 1 + MjClient.MaxPlayerNum_YuanJiangGuiHuZi) % MjClient.MaxPlayerNum_YuanJiangGuiHuZi == xingIndex ){
                resOff = 3;
...
function resetHandAfterBegin_YuanJiangGuiHuZi(){
    var sData = MjClient.data.sData;
    var pl = sData.players[SelfUid() + ""];
...
/*
 设置弃胡状态
 */
function setQiHuState_YuanJiangGuiHuZi()
{
    var pl = getUIPlayer_YuanJiangGuiHuZi(0);
    if (pl.isQiHu) {
...
function changeMJBg_YuanJiangGuiHuZi(node, type)
{
    if (node.toString() == "[object ImageView]") {
        var oldFile = node.getRenderFile().file;
        var newFile = getNewMJBgFile_YuanJiangGuiHuZi(oldFile, type);
    ...
    for(var index in childArray)
    {
        ...
        changeMJBg_YuanJiangGuiHuZi(child, type);
    }
}

function getNewMJBgFile_YuanJiangGuiHuZi(oldFile, type)
{
    if (oldFile.indexOf("playing/paohuzi/") == -1)
        return oldFile;

    if (type == undefined)
        type = ziPai.getZiPaiType();//getCurrentMJBgType_YuanJiangGuiHuZi();
...
*/
function createRadioBoxForCheckBoxs_YuanJiangGuiHuZi(nodes, callback, defaultIndex){
    var newobj = {};
...
//是否显示离开按钮（包括准备阶段）
function isShowBackBtn_YuanJiangGuiHuZi(){
    if(IsInviteVisible()){
        return true;
    }else{
        var sData = MjClient.data.sData;
...
function _checkIsAddGroup_YuanJiangGuiHuZi(){
    var cardArr = MjClient.HandCardArr;
    var _node = getNode_YuanJiangGuiHuZi(0);
    //根据牌的类型获得需要添加的节点
    var addNode = _node.getChildByName("handNode");
    if(!addNode){
        return;
...
function _hasValueInArr_YuanJiangGuiHuZi(arr,value){
    var len = arr.length;
    for(var i = 0; i < len; i++){
        var temp = arr[i];
        if(temp == value){
...
function _checkGroupList_YuanJiangGuiHuZi(groupList,tagName){
    var _node = getNode_YuanJiangGuiHuZi(0);
    //根据牌的类型获得需要添加的节点
    var addNode = _node.getChildByName("handNode");
    if(!addNode){
    ...
    if(cardParent){
        ...
        for(var j = len; j < 4; j++){
            ...
            if(card && 
                (!_hasValueInArr_YuanJiangGuiHuZi(groupList,card.tag) || 
                    hasSameChild_YuanJiangGuiHuZi(cardParent) ))// || cardParent.childrenCount > groupList.length))
            {
...
//一列中是否是相同的牌
function hasSameChild_YuanJiangGuiHuZi(cardParent){
...
//通过名字和tag 查找节点
function findChildByNameAndIndex_YuanJiangGuiHuZi(cardParent, mjNum, index){
...
function refreshHandCard_YuanJiangGuiHuZi(tagName,index,mjNum,off){
    if(off != 0){
        return;
    }
    var _node = getNode_YuanJiangGuiHuZi(off);
    ...
    var newCard = findChildByNameAndIndex_YuanJiangGuiHuZi(cardParent, mjNum, index);
    ...
    if(newCard){
        ...
        //  newCard = null;
        //  newCard = getNewCard_YuanJiangGuiHuZi(mjNum,1,off);
        //  cardParent.addChild(newCard);
        // }else{
        //  // return;
        // }
...
function refreshNode_YuanJiangGuiHuZi(posNode,off,needAction){
    ...
    cc.log("执行动画前手牌 refreshNode_YuanJiangGuiHuZi=====:" , MjClient.HandCardArr);
    ...
    _checkIsAddGroup_YuanJiangGuiHuZi();

    var index = 0;
    for(var k = 0;k < oldLen;k++){
        var groupList = cardArr[index];

        if(_hasValueInArr_YuanJiangGuiHuZi(deleIndexArr, k)){
            continue;
        }

        _checkGroupList_YuanJiangGuiHuZi(groupList,k);
        for(var j = 0;j < groupList.length;j++){

            refreshHandCard_YuanJiangGuiHuZi(k,j,groupList[j],off);
        }
        index += 1;
    }
    ...
    for(var i = 0; i < oldLen; i++){
        ...
        if(addNode && _hasValueInArr_YuanJiangGuiHuZi(deleIndexArr, addNode.tag)){
    ...
    cc.log("执行动画后手牌 refreshNode_YuanJiangGuiHuZi=====:" , MjClient.HandCardArr);
...
function getRoundInfo_YuanJiangGuiHuZi() {
    var tData = MjClient.data.sData.tData;
    var str = getPlaySelectPara(MjClient.gameType, tData.areaSelectMode);
    if(MjClient.data.sData.tData.areaSelectMode.maiPai19 && MjClient.data.sData.tData.maxPlayer == 2){
        str  = str + "埋牌19张"
    }
    if (!MjClient.data.sData.tData.areaSelectMode.maiPai19) {
        str += tData.areaSelectMode.maiPaiNum == 1 ? "埋牌10张" : tData.areaSelectMode.maiPaiNum == 2 ? "埋牌19张" : "不埋牌";
    }
...
function addOrAdjustHandCard_YuanJiangGuiHuZi(tagName,index,mjNum,off){
    if(off != 0){
        return;
    }
    var _node = getNode_YuanJiangGuiHuZi(off);
    ...
    if (MjClient.movingCard_paohuzi == newCard) {
        setCardSprite_YuanJiangGuiHuZi(newCard, newCard.tag, 1, false);
    ...
    //newCard.opacity = 255;
    SetTouchCardHandler_YuanJiangGuiHuZi(newCard,off);
...
//裁剪字符串 超出以...表示
function sliceStrByLen_YuanJiangGuiHuZi(str,length) {
    var tempStr = str;
...
function setOfflineShow_YuanJiangGuiHuZi(node, pl) {
...
function setCardNumImage_YuanJiangGuiHuZi(node) {
    if(!IsArrowVisible_YuanJiangGuiHuZi()){
        return;
    }
    cc.log("chow", "setCardNumImage_YuanJiangGuiHuZi");
    node.visible = true;
    var sData = MjClient.data.sData;
    var tData = sData.tData;
    var next = MjClient.majiang.getAllCardsTotal(sData) - tData.cardNext;

    if(MjClient.MaxPlayerNum_YuanJiangGuiHuZi == 3){
...
function addPutFrame_YuanJiangGuiHuZi(node, flag){
    if(flag == 0){
...
function setSiShou_YuanJiangGuiHuZi(node, off) {
    var pl = getUIPlayer_YuanJiangGuiHuZi(off);
    if(!IsArrowVisible_YuanJiangGuiHuZi()){
        return;
    }
