...
//向服务器发送 过消息
MjClient.MJPass2NetForLingshibanmo = function()
{
   // console.log(">>>>>>>>>普通  过 <<<<<<<<");
    cc.log("====================MJPass2NetForlingshibanmo======pass=====");
...
function SetUserVisible_lingshibanmo(node, off)
{
    //var sData = MjClient.data.sData;
    //return;
    cc.log("====================off======================" + off);
...
//处理出牌,放一张牌，打牌动作
function DealMJPut_lingshibanmo(node, msg, off, outNum)
{
...
function InitUserHandUI_lingshibanmo(node, off)
{
    var sData = MjClient.data.sData;
    var tData = sData.tData;
    var pl = getUIPlayer(off);
    ...
    for(var i = 0; i < pl.mjput.length; i++)
    {
        ...
        DealMJPut_lingshibanmo(node, msg, off, i);
    }
...
function initFlower_lingshibanmo() {
    initFlower(false, false);
}


var PlayLayer_lingshibanmo = cc.Layer.extend({
    jsBind: {
        _event: {
            mjhand: function() {
            ...
            roundEnd: function() {
                ...
                function delayExe()
                {
                    ...
                    self.addChild(new EndOneView_lingshibanmo(),500);
                }
        ...
        eat: {
            ...
            guo: {
                ...
                _touch: function(btn, eT) {
                    if (eT == 2)
                    {
                        MjClient.MJPass2NetForLingshibanmo();
                    }
                },
                bgimg: {
                    _run: function() {
            ...
            changeui: {
                ...
                changeuibg: {
                    ...
                    guobg: {
                        guo: {
                            _touch: function(btn, eT) {
                                if (eT == 2) MjClient.MJPass2NetForLingshibanmo();
                            }
                        },
                        fanhui: {
                            _touch: function(btn, et) {
    ...
    ctor: function() {
        this._super();
        var playui = ccs.load("Play_lingshibanmo.json");
        ...
        if(pl && (pl.mjState == TableState.isReady || pl.mjState == TableState.roundFinish))
        {
            ...
            if(pl != null)
            {
                if (pl.isTing) {
                    ...
                    if (off == 0)
                    {
                        var tingSet = calTingSet(pl.mjhand, tData.areaSelectMode["qidui"]);
...
PlayLayer_lingshibanmo.prototype.CardLayoutRestore = function(node, off)
{
...
// 判断吃碰杠胡的状态
PlayLayer_lingshibanmo.prototype.EatVisibleCheck = function()
{
    var eat = MjClient.playui.jsBind.eat;
        ...
        if(IsTurnToMe())
        {
            ...
            if (!pl.isTing) {
                ...
                for (var i = 0; i < pl.mjhand.length; i++) {
                    ...
                    if (MjClient.majiang.canTing(cardsAfterPut, tData.areaSelectMode["qidui"])) {
            ...
            var rtn = MjClient.majiang.canGang1(pl.mjpeng, pl.mjhand, pl.isTing, tData.areaSelectMode["qidui"]);
...
/*
    玩家ui，信息
 */
PlayLayer_lingshibanmo.prototype.Node_player = function(off)
{
    ...
    UIEventBind(null, _node, "initSceneData", function (eD) {
        SetUserVisible_lingshibanmo(_node, off);
    ...
    UIEventBind(null, _node, "mjhand", function (eD) {
        InitUserHandUI_lingshibanmo(_node, off);
    });
    ...
    UIEventBind(null, _node, "MJPut", function (eD) {
        ...
        //     var tingSet = calTingSet(pl.mjhand, MjClient.data.sData.tData.areaSelectMode["qidui"]);
        //     if(_tingCards) setTingCards_JINZHONGMJ(_tingCards,tingSet);

        // }
        DealMJPut_lingshibanmo(_node,eD,off);
        hideTingBtn();
        if(_tingCardNumNode) _tingCardNumNode.visible = false;
