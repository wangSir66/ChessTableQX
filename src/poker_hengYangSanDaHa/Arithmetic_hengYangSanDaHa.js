...
PokerHengYangSanDaHa.prototype.CARDTPYE = PDK_CARDTPYE;
var PDK_CARDCOUNT = {};
...
PokerHengYangSanDaHa.prototype.randomCards = function(areaSelectMode, tData) {
    var cards = [];
    // 去掉3、4
    for (var i = 1; i <= 54; i++) {
...
PokerHengYangSanDaHa.prototype.cardlog = function(cards, zhuColorType) {
 ...
 */
PokerHengYangSanDaHa.prototype.countWinType = function(jiaofen, PfenPaiArr, isTouXiang, Pkoudifen) {
...
PokerHengYangSanDaHa.prototype.isWuZhu = function(zhuColorType) {
...
PokerHengYangSanDaHa.prototype.calPoint = function(num) {
    if (!num) return -1;
...
PokerHengYangSanDaHa.prototype.calPointBySdh = function(num, zhuColorType) {
    if (!num) return -1;
...
// 牌的花色
PokerHengYangSanDaHa.prototype.calColor = function(num) {
    if (!num) return -1;
    // 0:方块  1:梅花  2:梅花  3:黑桃
...
PokerHengYangSanDaHa.prototype.calColorSdh = function(num, zhuColorType) {
    if (!num) return -1;
...
// 判断此牌是否是主花
PokerHengYangSanDaHa.prototype.isZhuCard = function(num, zhuColorType) {
    var point = this.calPoint(num);
    var color = this.calColor(num);
...
PokerHengYangSanDaHa.prototype.getChangZhuCount = function(oCards) {
...
PokerHengYangSanDaHa.prototype.getRoundFirstCards = function(putCardsRecord, uid, maxPlayNum) {
...
PokerHengYangSanDaHa.prototype.getRoundCardsByUid = function(putCardsRecord, uid, maxPlayNum) {
    if(!putCardsRecord || typeof putCardsRecord == 'undefined') {
        return null;
    }
...
// 获取本轮最大的牌， 同点数先出比后出的大
PokerHengYangSanDaHa.prototype.getMaxCardPlayerUid = function(roundPutCards, firstPutUid, uids, zhuColorType,bteshutuolaji,bchou6) {
    var frontUid = -1;
    if(!roundPutCards || typeof roundPutCards == 'undefined') {
        return frontUid;
    }
...
PokerHengYangSanDaHa.prototype.getAllCardFen = function( fenPaiArr ) {
    var fen = 0;
...
// 获得牌的分  非分牌返回0分 
PokerHengYangSanDaHa.prototype.getCardFen = function(num) {
...
// 获取所有对应牌数的拖拉机组合
PokerHengYangSanDaHa.prototype.getLianDuiCards = function(oCards, cardNum, zhuColorType,bteshutuolaji,bchou6) {
...
// 获取所有对应牌数的拖拉机组合
PokerHengYangSanDaHa.prototype.getDuiZis = function(oCards, zhuColorType) {
...
PokerHengYangSanDaHa.prototype.countDuiNum = function(oCards, zhuColorType) {
    var num = 0;
    var cardNumByValue = {};
    for(var i in oCards){
...
PokerHengYangSanDaHa.prototype.isChangZhu = function (num) {
    var point = this.calPoint(num);
 ...
 * @return {bool}
 */
PokerHengYangSanDaHa.prototype.isLiandui = function(oCards, zhuColorType,bteshutuolaji,bchou6) { //oCards有序
    ...
    for(var i=0; i < cardCount - 1; i++) {
        ...
        if(i % 2 == 1){
            if(!bteshutuolaji){
                if (this.isChangZhu(oCards[i]) || this.isChangZhu(oCards[i+1])) { return false; }
...
PokerHengYangSanDaHa.prototype.isDuizi = function(oCards) {
 ...
 */
PokerHengYangSanDaHa.prototype.calType = function(cards, zhuColorType,bteshutuolaji,bchou6) {
 ...
 * @return {number}
 */
PokerHengYangSanDaHa.prototype.calCardsValue = function(cards, zhuColorType) {
...
// // 牌点比较函数
PokerHengYangSanDaHa.prototype.cardValueCmp = function(a, b, zhuColorType) {
 ...
 */
PokerHengYangSanDaHa.prototype.isBigger = function(beiYaCards, oLastCards, zhuColorType,bteshutuolaji,bchou6) {
...
PokerHengYangSanDaHa.prototype.checkPut = function(oHands, cards) {
 ...
 */
PokerHengYangSanDaHa.prototype.checkPutSdh = function(oHands, pOutCards, oBeiYaCard, zhuColorType, isCanShuaiPai,tData) {
...
PokerHengYangSanDaHa.prototype.findCardByType = function(oHands, oBeiYaCard, zhuColorType) {
 ...
 * @return {array} 提示的牌
 */
PokerHengYangSanDaHa.prototype.tipCards = function(oHands, oBeiYaCard, zhuColorType,bteshutuolaji,bchou6) {
 ...
 */
PokerHengYangSanDaHa.prototype.sortHandCards = function(oCards, zhuColorType) {
 ...
 * @return {array}
 */
PokerHengYangSanDaHa.prototype.findNSameCard = function(hands, point, n, zhuColorType) {
...
// 删除相应牌值(不算花色)
PokerHengYangSanDaHa.prototype.delPoint = function(hands, points){
    var arr = hands.slice();
...
// 删除相同的牌(花色相同，牌值相同)
PokerHengYangSanDaHa.prototype.delCards = function(hands, cards) {
    var arr = hands.slice();
    for(var i=0; i < cards.length; i++){
        for (var j = 0; j < arr.length; j++) {
            if( arr[j] == cards[i] ){
...
// 从牌组中获取指定花色
PokerHengYangSanDaHa.prototype.getColorCards = function(cards, colors, zhuColorType) {
    var arr = [];
    for(var i=0; i < cards.length; i++) {
        var num = cards[i];
...
PokerHengYangSanDaHa.prototype.formatCards = function(oCards, zhuColorType) {
    var info = {};
...
PokerHengYangSanDaHa.prototype.isSameCards = function(cards1, cards2, zhuColorType) {
...
// 求出牌型
PokerHengYangSanDaHa.prototype.cardsType = function(oCards, zhuColorType,bteshutuolaji,bchou6) {
    if (oCards) {
...
PokerHengYangSanDaHa.prototype.isAllZhuCard = function(cards, zhuColorType) {
    for (var i = 0; i < cards.length; i++) {
...
if(typeof(module)!="undefined" && module.exports)    
    module.exports = PokerHengYangSanDaHa;

if(typeof(MjClient)!="undefined")
    MjClient.poker_HengYangSanDaHa = new PokerHengYangSanDaHa();
