...
DoudizhuHA.prototype.CARDTPYE = CARDTPYE;
var CARDCOUNT = {};
...
DoudizhuHA.prototype.randomCards = function() {
    var cards = [];
...
DoudizhuHA.prototype.calPoint = calPoint;

DoudizhuHA.prototype.isShun = function(oCards) { //oCards有序
    if (!oCards) {
        return false;
    }
...
DoudizhuHA.prototype.isLiandui = function(oCards) { //oCards有序
    if (!oCards) {
        return false;
    }
    var cardCount = oCards.length;
...
DoudizhuHA.prototype.isSanshun = function(cards) { //cards有序
    if (!cards) {
        return false;
    }
...
DoudizhuHA.prototype.isFeiji = function(oCards) { //cards有序
    if (!oCards) {
        return false;
    }
...
// 计算牌型
DoudizhuHA.prototype.calType = function(cards) {
    var cardCount = cards.length;
...
DoudizhuHA.prototype.calCardsValue = function(cards, type) {
...
// 牌点比较函数
DoudizhuHA.prototype.cardValueCmp = function(a, b) {
    var pa = calPoint(a);
    var pb = calPoint(b);
    if (pa == pb) {
        return a - b;
...
// 牌是否能压上
DoudizhuHA.prototype.canPut = function(oCards, oLastCards, tData) {
    var cards = oCards.slice();
...
// 检查是否能出牌
DoudizhuHA.prototype.checkPut = function(oHands, oCards, lastCards, tData) {
...
DoudizhuHA.prototype.checkPutByOneTimes = function (oHands) {
...
// 对手牌排序，普通牌型和同花顺优先排序
DoudizhuHA.prototype.sortHandCards = function(oCards, sortType) {
    var cards = oCards.slice();
...
// 找N张点数一样的牌
DoudizhuHA.prototype.findNSameCard = function(hands, point, n) { 
...
DoudizhuHA.prototype.filterSort = function(hands, cardsArray, type) { 
...
// 找出type牌型的牌
DoudizhuHA.prototype.findCardByType = function(hands, type, cardCount) { 
    var rets = [];
...
// 是否可以包含炸弹
DoudizhuHA.prototype.isCanContainZhadan = function (type) {
    return type == CARDTPYE.zhadan;
};

// 不拆炸弹
DoudizhuHA.prototype.dismantleZhadan = function (hands, cardsArray, type) {
...
// 求解提示出牌
DoudizhuHA.prototype.tipCards = function(oHands, oLastCards) {
    var rets = [];
...
// 求出牌型
DoudizhuHA.prototype.cardsType = function(oCards) {
    if (oCards) {
        var cards = oCards.slice();
        cards.sort(this.cardValueCmp);
...
// 计算真实的点
DoudizhuHA.prototype.calRealPoint = function(num) {
    if (!num) {
        return -1;
    }
    var ceilNum = Math.ceil(num / 4);
...
DoudizhuHA.prototype.findMinPointAndMinZhaDanByHands = function (hands) {
...
// 找最小牌的顺子/连对
DoudizhuHA.prototype.findCardsByMinPointAndType = function (cardsMap, len, minPoint, cType) {
...
DoudizhuHA.prototype.findSanZhangByMinPoint = function (cardsMap, len, minPoint) {
...
// 找最小牌的对子
DoudizhuHA.prototype.findDuiZiByMinPoint = function (cardsMap, len, minPoint) {
...
DoudizhuHA.prototype.changeCardsToMap = function (hands) {
    var cardsMap = {}
    for (var i = 0; i < hands.length; i++) {
...
// 首出逻辑
DoudizhuHA.prototype.getAutoPutCardWithFirstOut = function(hands) {
    // 1. 从小到大出牌
...
// ==========================[[ 斗地主提牌 ]] ===================================
// 是否是三张系列
DoudizhuHA.prototype.is3ZhangType = function(cType) {
...
DoudizhuHA.prototype.findSelectCardForAutoUp = function(tipCards, selectValue) {
...
// 更换牌值
DoudizhuHA.prototype.changeSelectCardForAuto = function(tipCards, idx, selectValue) {
    tipCards[idx] = selectValue;
    return tipCards;
}

// 有足够的三张牌组
DoudizhuHA.prototype.isEnough3SameCards = function (upSelectCards, groupNum) { 
    var start = false;
    var tmpCards = [];
...
DoudizhuHA.prototype.findEnough3SameCard = function(oHands, upSelectCards, groupNum, zhadanMap) {
    var tmpSelect = [];
    var lastPoint = -1;
    for (var i = 0; i < upSelectCards.length; i++) {
...
// 找三张配合的牌
DoudizhuHA.prototype.findDaiCardsWithSanZhang = function(oHands, upSelectCards, cType, groupNum, zhadanMap) {
...
// 自动提牌-划动-三张系列
DoudizhuHA.prototype.handleAutoByTouchMoveSanZhang = function(upSelectCards, cType, oLastCards, oHands) {
    var groupNum = 1; 
...
DoudizhuHA.prototype.findTheRightTouchMoveCards = function(upSelectCards, cType, oLastCards) {
...
DoudizhuHA.prototype.findZhadanTouchMoveCards = function(upSelectCards) {
    var tCards = [];
    var lastPoint = -1;
...
// 自动提牌-划牌
DoudizhuHA.prototype.handleAutoByTouchMove = function (bTouchMove, upSelectCards, cType, tipCardsArray, oLastCards, oHands) {
...
// 自动提牌-点牌
DoudizhuHA.prototype.handleAutoByFirstCard = function(tipCardsArray, upSelectCard, cType) {
...
// 计算斗地主划牌结果
DoudizhuHA.prototype.calAutoPutCards = function (bTouchMove, tipCardsArray, upSelectCards, oLastCards, oHands) {
...
DoudizhuHA.prototype.getFindCardForFirstPut = function(cards, cType, count) {
    var realCards = null;
    var rateType = {};
...
DoudizhuHA.prototype.calAutoPutCardWithFirstOut = function(upSelectCards, hands) {
    var data = {hasCardToUp:false};
...
// 计算选择的牌,排除炸弹
DoudizhuHA.prototype.calSelectWithoutZhadan = function(upSelectCards, zhadanMap) {
...
// 计算牌组的炸弹
DoudizhuHA.prototype.getZhaDanMapByCards = function(oHands) {
    var hands = oHands.slice();
    hands.sort(this.cardValueCmp);
...
if(typeof(module)!="undefined" && module.exports)    
    module.exports = DoudizhuHA;
if(typeof(MjClient)!="undefined")
    MjClient.doudizhu_haian = new DoudizhuHA();
})();