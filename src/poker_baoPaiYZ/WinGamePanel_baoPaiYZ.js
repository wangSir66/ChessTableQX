function SetEndOneUserUI_baoPaiYZ(node, off) {
    ...
    node.getChildByName("jiaChuiTag").visible = tData.areaSelectMode.jiaChui && pl.jiaChui;

    if (pl.info.uid == SelfUid()) {
        if(Number(pl.winone) > 0)
        {
            node.loadTexture(isZhuan ? "gameOver_baoPaiYZ/zhuang_zheng_me.png" : "gameOver_baoPaiYZ/right_zheng_me.png");
        }else{
            node.loadTexture(isZhuan ? "gameOver_baoPaiYZ/zhuang_fu_me.png" : "gameOver_baoPaiYZ/right_fu_me.png");
        }
    }else{
        if(Number(pl.winone) > 0)
        {
            node.loadTexture(isZhuan ? "gameOver_baoPaiYZ/zhuang_zheng.png" : "gameOver_baoPaiYZ/right_zheng.png");
        }else{
            node.loadTexture(isZhuan ? "gameOver_baoPaiYZ/zhuang_fu.png" : "gameOver_baoPaiYZ/right_fu.png");
        }
    }
    ...
    var cardNumDi = MjClient.MaxPlayerNum == 3 ? 9 : 8 ;

    if (isZhuan) {
        var image_state = node.getChildByName("image_state");
        switch (tData.winType) {
            case -4: // 投降
                image_state.loadTexture("gameOver_baoPaiYZ/touxiang.png");
                break;
            case -3: // 大光
                image_state.loadTexture("gameOver_baoPaiYZ/daguang.png");
                break;
            case -2: // 小光
                image_state.loadTexture("gameOver_baoPaiYZ/xiaoguang.png");
                break;
            case -1: // 拉庄
                image_state.loadTexture("gameOver_baoPaiYZ/guozhuang.png");
                break;
            case 1: // 跨庄
                image_state.loadTexture("gameOver_baoPaiYZ/kuazhuang.png");
                break;
            case 2: // 小倒
                image_state.loadTexture("gameOver_baoPaiYZ/xiaodao.png");
                break;
            case 3: // 大倒
                image_state.loadTexture("gameOver_baoPaiYZ/dadao.png");
                break;
            case 4: // 大倒
                image_state.loadTexture("gameOver_baoPaiYZ/dadao.png");
                break;
        ...
        if (cards.length == cardNumDi) {
            for (var i = 0; i < cardNumDi; i++) {
...
var EndOneView_baoPaiYZ = cc.Layer.extend({
    jsBind: {
        block: {
            _layout: [
                [1, 1],
        ...
        back: {
            ...
            pingju: {
                ...
                _run: function() {
                    if (MjClient.isDismiss) {
                        this.loadTexture("gameOver_baoPaiYZ/jiesan.png");
                    }
                }
            },
            shareBtn: {
            ...
            image_player_zhuang: {
                _run: function() {
                    SetEndOneUserUI_baoPaiYZ(this, MjClient.endoneui.offs[0]);
                }
            },
            image_player_0: {
                _run: function() {
                    SetEndOneUserUI_baoPaiYZ(this, MjClient.endoneui.offs[1]);
                },
            },
            image_player_1: {
                _run: function() {
                    SetEndOneUserUI_baoPaiYZ(this, MjClient.endoneui.offs[2]);
                }
            },
            image_player_2: {
                _run: function() {
                    SetEndOneUserUI_baoPaiYZ(this, MjClient.endoneui.offs[3]);
                }
            },
        }
    },
    ctor: function() {
        ...
        var endoneui = ccs.load(res.EndOne_baoPaiYZ_json); 
