...
PokerbaoPaiYZ.prototype.CARDTPYE = PDK_CARDTPYE;
var PDK_CARDCOUNT = {};
...
PokerbaoPaiYZ.prototype.randomCards = function(areaSelectMode, tData) {
    var cards = [];
    // 去掉3、4、6
    for (var i = 1; i <= 54; i++) {
        var point = this.calPoint(i);
        if (point == 3 || point == 4 || point == 6) { continue; }
        cards.push(i);
    }
    // 两幅扑克牌
...
PokerbaoPaiYZ.prototype.cardlog = function(cards, zhuColorType) {
 ...
 */
PokerbaoPaiYZ.prototype.countWinType = function(jiaofen, PfenPaiArr, isTouXiang, Pkoudifen, daDaoBuFengDing) {
    if( isTouXiang ) {
        return -4;
    }
    ...
    var zhuangFen = 205 - jiaofen;
    var difffen = defen - zhuangFen;
    var winType = 0;
    ...
    if(difffen < 0 && defen == 0 ) { // 大光 每个闲家-3
        ...
        winType = 2;
         ...
         if (daDaoBuFengDing) {
            // 大倒不封顶
            winType = Math.floor((difffen - 70) / 30) + 3;
         }
    }
...
PokerbaoPaiYZ.prototype.isWuZhu = function(zhuColorType) {
...
// 是否有主牌
PokerbaoPaiYZ.prototype.isHaveZhuCards = function (cards, zhuColorType) {
...
PokerbaoPaiYZ.prototype.calPoint = function(num) {
    if (!num) return -1;
...
PokerbaoPaiYZ.prototype.calPointBySdh = function(num, zhuColorType) {
    if (!num) return -1;
...
// 牌的花色
PokerbaoPaiYZ.prototype.calColor = function(num) {
    if (!num) return -1;
    // 0:方块  1:梅花  2:梅花  3:黑桃
...
PokerbaoPaiYZ.prototype.calColorSdh = function(num, zhuColorType) {
    if (!num) return -1;
...
// 判断此牌是否是主花
PokerbaoPaiYZ.prototype.isZhuCard = function(num, zhuColorType) {
    var point = this.calPoint(num);
    var color = this.calColor(num);
...
PokerbaoPaiYZ.prototype.getRoundFirstCards = function(putCardsRecord, uid, maxPlayNum) {
...
PokerbaoPaiYZ.prototype.getRoundCardsByUid = function(putCardsRecord, uid, maxPlayNum) {
    if(!putCardsRecord || typeof putCardsRecord == 'undefined') {
        return null;
    }
...
// 获取本轮最大的牌， 同点数先出比后出的大
PokerbaoPaiYZ.prototype.getMaxCardPlayerUid = function(roundPutCards, firstPutUid, uids, zhuColorType) {
    var frontUid = -1;
    if(!roundPutCards || typeof roundPutCards == 'undefined') {
        return frontUid;
    }
...
PokerbaoPaiYZ.prototype.getAllCardFen = function( fenPaiArr ) {
    var fen = 0;
...
// 获得牌的分  非分牌返回0分 
PokerbaoPaiYZ.prototype.getCardFen = function(num) {
...
// 获取所有对应牌数的拖拉机组合
PokerbaoPaiYZ.prototype.getLianDuiCards = function(oCards, cardNum, zhuColorType) {
	// 补充逻辑：拖拉机去除单牌，解决类似两个黑桃2和两个梅花2中间夹一个红桃2不是拖拉机的问题
    var sourceCards = oCards.slice();
...
// 获取所有对应牌数的拖拉机组合
PokerbaoPaiYZ.prototype.getDuiZis = function(oCards, zhuColorType) {
...
PokerbaoPaiYZ.prototype.countDuiNum = function(oCards, zhuColorType) {
    var num = 0;
    var cardNumByValue = {};
    for(var i in oCards){
 ...
 * @return {bool}
 */
PokerbaoPaiYZ.prototype.isLiandui = function(oCards, zhuColorType) { //oCards有序
    ...
    for(var i=0; i < cardCount - 1; i++) {
        ...
        if(i % 2 == 1 && Math.abs(points[i] - points[i+1]) != 1 ){
            if (points[i]%100 == 5 && points[i+1]%100 == 3 || points[i]%100 == 3 && points[i+1]%100 == 5) { // 抽6下的 5和8
                continue;
            } else {
                return false;
            }
...
PokerbaoPaiYZ.prototype.isDuizi = function(oCards) {
 ...
 */
PokerbaoPaiYZ.prototype.calType = function(cards, zhuColorType) {
 ...
 * @return {number}
 */
PokerbaoPaiYZ.prototype.calCardsValue = function(cards, zhuColorType) {
...
// // 牌点比较函数
PokerbaoPaiYZ.prototype.cardValueCmp = function(a, b, zhuColorType) {
 ...
 */
PokerbaoPaiYZ.prototype.isBigger = function(beiYaCards, oLastCards, zhuColorType) {
...
PokerbaoPaiYZ.prototype.checkPut = function(oHands, cards) {
 ...
 */
PokerbaoPaiYZ.prototype.checkPutSdh = function(oHands, pOutCards, oBeiYaCard, zhuColorType, isCanShuaiPai, tData) {
...
PokerbaoPaiYZ.prototype.findCardByType = function(oHands, oBeiYaCard, zhuColorType) {
 ...
 * @return {array} 提示的牌
 */
PokerbaoPaiYZ.prototype.tipCards = function(oHands, oBeiYaCard, zhuColorType) {
 ...
 */
PokerbaoPaiYZ.prototype.sortHandCards = function(oCards, zhuColorType) {
 ...
 * @return {array}
 */
PokerbaoPaiYZ.prototype.findNSameCard = function(hands, point, n, zhuColorType) {
...
// 删除相应牌值(不算花色)
PokerbaoPaiYZ.prototype.delPoint = function(hands, points){
    var arr = hands.slice();
...
// 删除相同的牌(花色相同，牌值相同)
PokerbaoPaiYZ.prototype.delCards = function(hands, cards) {
    var arr = hands.slice();
    for(var i=0; i < cards.length; i++){
        for (var j = 0; j < arr.length; j++) {
            if( arr[j] == cards[i] ){
...
// 从牌组中获取指定花色
PokerbaoPaiYZ.prototype.getColorCards = function(cards, colors, zhuColorType) {
    var arr = [];
    for(var i=0; i < cards.length; i++) {
        var num = cards[i];
...
PokerbaoPaiYZ.prototype.formatCards = function(oCards, zhuColorType) {
    var info = {};
...
PokerbaoPaiYZ.prototype.isSameCards = function(cards1, cards2, zhuColorType) {
...
PokerbaoPaiYZ.prototype.isAllZhuCard = function(cards, zhuColorType) {
    for (var i = 0; i < cards.length; i++) {
...
// 求出牌型
PokerbaoPaiYZ.prototype.cardsType = function(oCards, zhuColorType) {
    if (oCards) {
...
// 检查是否是一次可以出完
PokerbaoPaiYZ.prototype.checkPutByOneTimes = function (oHands, zhuColorType) {
...
if(typeof(module)!="undefined" && module.exports)    
    module.exports = PokerbaoPaiYZ;

if(typeof(MjClient)!="undefined")
    MjClient.poker_baoPaiYZ = new PokerbaoPaiYZ();
