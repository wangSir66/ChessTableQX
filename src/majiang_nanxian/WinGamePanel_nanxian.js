...
function SetEndOneUserUI_nanxian(node,off)
{
...
var EndOneView_nanxian = cc.Layer.extend({
	jsBind:{
		block: {
		...
		head0:{
		   ...
		   _run:function(){ SetEndOneUserUI_nanxian(this,0); },
		...
		,head1:{ 
		  ...
		  _run:function(){ SetEndOneUserUI_nanxian(this,1); }
		}
		...
		,head2:{ 
		   ...
		   _run:function(){ SetEndOneUserUI_nanxian(this,2); }
		}
		,head3:{
			 head:
			 {
			...
			_run:function(){ SetEndOneUserUI_nanxian(this,3); }
			},
            count_down:{
                _visible :function()
                {
    ...
    ctor:function () {
        this._super();
        var endoneui = ccs.load("endOne_nanxian.json");
        ...
        for(var i = 0;i < cards.length;i++)
        {
            ...
            if (tData.niaoCards && tData.niaoCards.length > 0) 
            {
            	for (var j = 0; j < tData.niaoCards.length; j++) {
            		if (cards[i] == tData.niaoCards[j]) 
            		{
            			tData.niaoCards.splice(j,1);
            		}
            		else
            		{
