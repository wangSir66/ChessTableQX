...
var _isShowShaiziAni_nanxian = false; //为了控制mjhand之后播骰子动画时newCard的状态显示
//向服务器发送 过消息
MjClient.MJPass2NetFornanxian = function()
{
   // console.log(">>>>>>>>>普通  过 <<<<<<<<");
    cc.log("====================send======pass=====");
...
var showShaiziAni_nanxian = false;
function showShaiziAni_nanxian_old(poff)
{
    if(poff == 0) {
        ...
        if(MjClient.rePlayVideo != -1)//回放
        {
            for (var off = 0; off < 4; off++) {
                ...
                InitUserHandUI_nanxian(_node, off);
            }
            playEffect("shuffle");
        ...
        firstFrame.runAction(cc.sequence(playSoundFunc, animate,showPonitFunc,cc.delayTime(0.5),cc.callFunc(function(){
            //发牌
            _isShowShaiziAni_nanxian = false;
...
function updateHeadUI_nanxian(node, off)
{      
    cc.log('updateHeadUI_nanxian', off)
    var pl = getUIPlayer(off);
    if(!pl) return;
    showHandCard_nanxian(node, off);
    MjClient.playui.CardLayoutRestore(node, off);
    cc.log('updateHeadUI_nanxian update success', off)
...
function SetUserVisible_nanxian(node, off)
{
    if (_isShowShaiziAni_nanxian)  // 解决摇骰子时断线重连调用本函数，会手牌重复的bug by cyc
        return;
...
function showHandCard_nanxian(node, off)
{
    var sData = MjClient.data.sData;
    var tData = sData.tData;
    var pl = getUIPlayer(off);
    ...
    for(var i = 0; i < pl.mjgang1.length; i++)
    {
        for(var j = 0; j < 4; j++)
        {
            ...
            {   
                ...
                {
                    var GangCard = getNewCard(node, "down", "gang1", pl.mjgang1[i], off);
                    var imgNode = GangCard.getChildByName("imgNode");
                    if (imgNode)
                    {
...
function InitUserHandUI_nanxian(node, off)
{
    var sData = MjClient.data.sData;
    var tData = sData.tData;
    var pl = getUIPlayer(off);
...
function initFlower_nanxian() {
    initFlower(false, false);
}  

var PlayLayer_nanxian = cc.Layer.extend({
    jsBind: {
        _event: {
            mjhand: function() {
            ...
            roundEnd: function() {
                ...
                function showEndCards()
                {
                    ...
                    self.addChild(new EndOneView_nanxian(),500);
                }
                function delayExe()
                {
      ...
      //           }
        ...
        BtnPutCard:{ //add by  sking for put card button
            ...
            _event:{
                ...
                waitPut: function() {
                    if (_isShowShaiziAni_nanxian) return;
            ...
            changeui: {
                ...
                changeuibg: {
                    ...
                    guobg: {
                        guo: {
                            _touch: function(btn, eT) {
                                if (eT == 2) MjClient.MJPass2NetFornanxian();
                            }
                        },
                        fanhui: {
                            _touch: function(btn, et) {
    ...
    ctor: function() {
        this._super();
        var playui = ccs.load("Play_nanxian.json");
            ...
            if(pl != null)
            {
                if (pl.isTing) {
                    ...
                    if (off == 0 /*&& pl.mjhand.length%3 == 1*/)
                    {
                        ...
                        //cc.log("tingListAfterPut",tingListAfterPut);
...
PlayLayer_nanxian.prototype.willHuShowArrow = function()
{
    var tData = MjClient.data.sData.tData;
...
PlayLayer_nanxian.prototype.CardLayoutRestore = function(node, off, isNewCard)
{
    ...
    for(var i = 0; i < children.length; i++) //children 为 "down" 节点下的字节点
    {
        ...
        {
            if (off != 0 && ci.isgang4 !== true)//暗杠不显示牌面
            {
                if (pl.isTing)
                {
                    ci.loadTexture(getNewMJBgFile("playing/MJ/Mj_up_" + off + ".png"));
                ...
                else
                {
                    ...
                    {
                        ci.loadTexture(getNewMJBgFile("playing/MJ/Mj_06.png"));
                    }
                    var imgNode = ci.getChildByName("imgNode");
                    if (imgNode)
                    {
...
PlayLayer_nanxian.prototype.hideAllEat = function(){
    var eat = MjClient.playui.jsBind.eat;
    eat.chi0._node.visible = false;
    eat.chi1._node.visible = false;
    eat.chi2._node.visible = false;
...
PlayLayer_nanxian.prototype.EatBaoTingVisibleCheck = function(tData, vnode, eat, pl) 
{
    var eat = MjClient.playui.jsBind.eat;
    var sData = MjClient.data.sData;
    var tData = sData.tData;
...
// 判断吃碰杠胡的状态
PlayLayer_nanxian.prototype.EatVisibleCheck = function()
{
    var eat = MjClient.playui.jsBind.eat;
...
PlayLayer_nanxian.prototype.jiaozhuToPiaoniao = function(jiazhuLayer, msg) {
    var tData = MjClient.data.sData.tData;
...
PlayLayer_nanxian.prototype.resetcardcolorforqsting = function (node) {
    var _node = node;
    var children = node.children;
    for (var i = 0; i < children.length; i++) {
        if (children[i].name == "mjhand") 
