...
DoudizhuZERO.prototype.CARDTPYE = CARDTPYE;
var CARDCOUNT = {};
...
DoudizhuZERO.prototype.randomCards = function() {
    var cards = [];
...
DoudizhuZERO.prototype.calPoint = calPoint;

// 牌的花色
DoudizhuZERO.prototype.calColor = function(num) {
    if (!num) return -1;
    // 0:方块  1:梅花  2:梅花  3:黑桃
...
DoudizhuZERO.prototype.isShun = function(oCards) { //oCards有序
    if (!oCards) {
        return false;
    }
...
DoudizhuZERO.prototype.isLiandui = function(oCards) { //oCards有序
    if (!oCards) {
        return false;
    }
    var cardCount = oCards.length;
...
DoudizhuZERO.prototype.isSanshun = function(cards) { //cards有序
    if (!cards) {
        return false;
    }
...
DoudizhuZERO.prototype.isFeiji2 = function(oCards) { //cards有序
    if (!oCards) {
        return false;
    }
...
DoudizhuZERO.prototype.isFeiji = function(oCards) { //cards有序
    if (!oCards) {
        return false;
    }
    var cards = oCards.slice();
...
// 计算牌型
DoudizhuZERO.prototype.calType = function(cards) {
    var cardCount = cards.length;
...
DoudizhuZERO.prototype.calCardsValue = function(cards, type) {
...
// 牌点比较函数
DoudizhuZERO.prototype.cardValueCmp = function(a, b) {
    var pa = calPoint(a);
    var pb = calPoint(b);
    if (pa == pb) {
        return a - b;
...
// 牌是否能压上
DoudizhuZERO.prototype.canPut = function(oCards, oLastCards) {
    var cards = oCards.slice();
...
// 检查是否能出牌
DoudizhuZERO.prototype.checkPut = function(oHands, oCards, lastCards) {
...
DoudizhuZERO.prototype.checkPutByOneTimes = function (oHands) {
...
// 对手牌排序，普通牌型和同花顺优先排序
DoudizhuZERO.prototype.sortHandCards = function(oCards, sortType) {
    var cards = oCards.slice();
...
// 找N张点数一样的牌
DoudizhuZERO.prototype.findNSameCard = function(hands, point, n) { 
...
DoudizhuZERO.prototype.filterSort = function(hands, cardsArray, type) { 
...
// 找出type牌型的牌
DoudizhuZERO.prototype.findCardByType = function(hands, type, cardCount) { 
    var rets = [];
...
// 是否可以包含炸弹
DoudizhuZERO.prototype.isCanContainZhadan = function (type) {
...
// 不拆炸弹
DoudizhuZERO.prototype.dismantleZhadan = function (hands, cardsArray, type) {
...
// 求解提示出牌
DoudizhuZERO.prototype.tipCards = function(oHands, oLastCards) {
    var rets = [];
...
// 求出牌型
DoudizhuZERO.prototype.cardsType = function(oCards) {
    if (oCards) {
        var cards = oCards.slice();
        cards.sort(this.cardValueCmp);
...
// 计算真实的点
DoudizhuZERO.prototype.calRealPoint = function(num) {
    if (!num) {
        return -1;
    }
    var ceilNum = Math.ceil(num / 4);
...
// 是否是三张系列
DoudizhuZERO.prototype.is3ZhangType = function(cType) {
...
DoudizhuZERO.prototype.findSelectCardForAutoUp = function(tipCards, selectValue) {
...
// 更换牌值
DoudizhuZERO.prototype.changeSelectCardForAuto = function(tipCards, idx, selectValue) {
    tipCards[idx] = selectValue;
    return tipCards;
}

// 有足够的三张牌组
DoudizhuZERO.prototype.isEnough3SameCards = function (upSelectCards, groupNum) {
    var start = false;
    var tmpCards = [];
...
DoudizhuZERO.prototype.findEnough3SameCard = function(oHands, upSelectCards, groupNum, zhadanMap) {
    var tmpSelect = [];
    var lastPoint = -1;
    for (var i = 0; i < upSelectCards.length; i++) {
...
// 找三张配合的牌
DoudizhuZERO.prototype.findDaiCardsWithSanZhang = function(oHands, upSelectCards, cType, groupNum, zhadanMap) {
...
// 自动提牌-划动-三张系列
DoudizhuZERO.prototype.handleAutoByTouchMoveSanZhang = function(upSelectCards, cType, oLastCards, oHands) {
    var groupNum = 1;
...
DoudizhuZERO.prototype.findTheRightTouchMoveCards = function(upSelectCards, cType, oLastCards) {
...
DoudizhuZERO.prototype.findZhadanTouchMoveCards = function(upSelectCards) {
    var tCards = [];
    var lastPoint = -1;
...
// 自动提牌-划牌
DoudizhuZERO.prototype.handleAutoByTouchMove = function (bTouchMove, upSelectCards, cType, tipCardsArray, oLastCards, oHands) {
...
// 自动提牌-点牌
DoudizhuZERO.prototype.handleAutoByFirstCard = function(tipCardsArray, upSelectCard, cType) {
...
// 计算斗地主划牌结果
DoudizhuZERO.prototype.calAutoPutCards = function (bTouchMove, tipCardsArray, upSelectCards, oLastCards, oHands) {
...
DoudizhuZERO.prototype.getFindCardForFirstPut = function(cards, cType, count) {
    var realCards = null;
    var rateType = {};
...
DoudizhuZERO.prototype.calAutoPutCardWithFirstOut = function(upSelectCards, hands) {
    var data = {hasCardToUp:false};
...
// 计算选择的牌,排除炸弹
DoudizhuZERO.prototype.calSelectWithoutZhadan = function(upSelectCards, zhadanMap) {
...
// 计算牌组的炸弹
DoudizhuZERO.prototype.getZhaDanMapByCards = function(oHands) {
    var hands = oHands.slice();
    hands.sort(this.cardValueCmp);
...
// ======================= [[ 金币场相关 ]] ===========================================
DoudizhuZERO.prototype.getDiCardsRateAndDesc = function(oDiCards, isJinBiChang) {
...
DoudizhuZERO.prototype.findMinPointAndMinZhaDanByHands = function (hands, isExcludeZhaDan) {
...
// 找最小牌的顺子/连对
DoudizhuZERO.prototype.findCardsByMinPointAndType = function (cardsMap, len, minPoint, cType) {
...
DoudizhuZERO.prototype.findSanZhangByMinPoint = function (cardsMap, len, minPoint) {
...
// 找最小牌的对子
DoudizhuZERO.prototype.findDuiZiByMinPoint = function (cardsMap, len, minPoint) {
...
DoudizhuZERO.prototype.changeCardsToMap = function (hands) {
    var cardsMap = {}
    for (var i = 0; i < hands.length; i++) {
...
// 首出逻辑
DoudizhuZERO.prototype.getAutoPutCardWithFirstOut = function(hands, isExcludeZhaDan) {
    // 1. 从小到大出牌
...
DoudizhuZERO.prototype.getAutoPutCardWithNext = function(hands, lastPutCard, isRobot, isEnemy) {
...
if(typeof(module)!="undefined" && module.exports)    
    module.exports = DoudizhuZERO;
if(typeof(MjClient)!="undefined")
    MjClient.doudizhu_zero = new DoudizhuZERO();
})();