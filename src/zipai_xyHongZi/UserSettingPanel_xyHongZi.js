...
MjClient.KEY_voiceType_xyHongZi = "KEY_voiceType_xyHongZi";
MjClient.KEY_MJBgType_xyHongZi  = "KEY_MJBgType_xyHongZi";
MjClient.KEY_gameBgType_xyHongZi = "KEY_gameBgType_xyHongZi";  

MjClient.KEY_bgMusicType = "BG_MUSIC_TYPE"

//选项选中时的颜色处理  统一处理
var XYHZ_SETTNG_COLOR_1 = cc.color(255,119,28);
var XYHZ_SETTNG_COLOR_2 = cc.color(118,99,42);

/*
    获取语言语音,返回值 0 表示普通话，1 表示本地话
 */
function getCurrentVoiceType_xyHongZi()
{
    return util.localStorageEncrypt.getNumberItem(MjClient.KEY_voiceType_xyHongZi, 0);
}

var UserSetting_MJBgType_xyHongZi = -1;
var UserSetting_GameBgType_xyHongZi = -1;
 ...
 * @return {Number}
 */
function getCurrentGameBgType_xyHongZi()
{
    if (UserSetting_GameBgType_xyHongZi != -1)
        return UserSetting_GameBgType_xyHongZi;
    else
        return util.localStorageEncrypt.getNumberItem(MjClient.KEY_gameBgType_xyHongZi, 0);
}

function setCurrentGameBgType_xyHongZi(gameBgType)
{
	UserSetting_GameBgType_xyHongZi = gameBgType;
    util.localStorageEncrypt.setNumberItem(MjClient.KEY_gameBgType_xyHongZi, gameBgType);
}

function setCurrentGameBgTypeToNext_xyHongZi()
{
    ...
    var current = getCurrentGameBgType_xyHongZi();
    if (current < max)
        current ++;
    else
        current = 0;

    setCurrentGameBgType_xyHongZi(current);
}
 ...
 * @return {Number}
 */
function getCurrentMJBgType_xyHongZi()
{
    return ziPai.getZiPaiType();
    // if (UserSetting_MJBgType_xyHongZi != -1)
    //     return UserSetting_MJBgType_xyHongZi;
    // else
    //     return util.localStorageEncrypt.getNumberItem(MjClient.KEY_MJBgType_xyHongZi, 1);
}

var SettingView_xyHongZi = cc.Layer.extend({
	jsBind:{
		block:{
    ...
    ctor:function () {
        ...
        if(MjClient.getAppType() == MjClient.APP_TYPE.TXLINFENMJ){
            XYHZ_SETTNG_COLOR_1 = cc.color(94,119,199);
            XYHZ_SETTNG_COLOR_2 = cc.color(91,95,128);
        }
        ...
        puTongHua_txt.setTextColor(XYHZ_SETTNG_COLOR_1);
        benDiHua_txt.setTextColor(XYHZ_SETTNG_COLOR_2);
        benDiHua_txt.ignoreContentAdaptWithSize(true);
        puTongHua_txt.ignoreContentAdaptWithSize(true);
		var voiceType = getCurrentVoiceType_xyHongZi();
        if(voiceType  == 0)
        {
            puTongHua.setSelected(true);
            benDiHua.setSelected(false);
            ...
            benDiHua_txt.setTextColor(XYHZ_SETTNG_COLOR_1);
            puTongHua_txt.setTextColor(XYHZ_SETTNG_COLOR_2);
        }
        ...
        benDiHua.addEventListener(function(sender,type)
        {
            switch (type) {
                ...
                case ccui.CheckBox.EVENT_UNSELECTED:
                    ...
                    benDiHua_txt.setTextColor(XYHZ_SETTNG_COLOR_1);
                    puTongHua_txt.setTextColor(XYHZ_SETTNG_COLOR_2);
                    util.localStorageEncrypt.setNumberItem(MjClient.KEY_voiceType_xyHongZi, 1);
                    break;
            }
        },this);
        ...
        puTongHua.addEventListener(function(sender,type)
        {
            switch (type) {
                ...
                case ccui.CheckBox.EVENT_UNSELECTED:
                    ...
                    puTongHua_txt.setTextColor(XYHZ_SETTNG_COLOR_1);
                    benDiHua_txt.setTextColor(XYHZ_SETTNG_COLOR_2);
                    util.localStorageEncrypt.setNumberItem(MjClient.KEY_voiceType_xyHongZi, 0);
                    break;
            }
        },this);
        ...
        var gameBgEventCb = function (sender, type) {
            if (type == ccui.CheckBox.EVENT_SELECTED || type == ccui.CheckBox.EVENT_UNSELECTED)
            {
                ...
                setCurrentGameBgType_xyHongZi(gameBgType);
                postEvent("changeGameBgEvent");
            }
        }

        var gameBgType = getCurrentGameBgType_xyHongZi();
        for (var i = 0; i < nodeListgameBg.length; i ++)
        {
        ...
        var MJBgEventCb = function (sender, type) {
            if (type == ccui.CheckBox.EVENT_SELECTED || type == ccui.CheckBox.EVENT_UNSELECTED)
            {
                ...
                util.localStorageEncrypt.setNumberItem(MjClient.KEY_MJBgType_xyHongZi, type);
                UserSetting_MJBgType_xyHongZi = type;
                postEvent("changeMJBgEvent", type);
            }
        }

        var MJBgType = getCurrentMJBgType_xyHongZi();
            ...
            onTouchEnded: function (touch, event) { 
                ...
                if (listnode) {
                    ...
                    if (radio == that._playNode_Hua_radio) {
                        ...
                        if (number == 0) {
                            that.puTongHua_txt.setTextColor(XYHZ_SETTNG_COLOR_1);
                            that.benDiHua_txt.setTextColor(XYHZ_SETTNG_COLOR_2);
                            util.localStorageEncrypt.setNumberItem(MjClient.KEY_voiceType_xyHongZi, 0);
                        }
                        if (number == 1) {
                            that.benDiHua_txt.setTextColor(XYHZ_SETTNG_COLOR_1);
                            that.puTongHua_txt.setTextColor(XYHZ_SETTNG_COLOR_2);
                            util.localStorageEncrypt.setNumberItem(MjClient.KEY_voiceType_xyHongZi, 1);
                        }
                    }
                    ...
                    that.bgMusic1_txt.setTextColor(bgMusicType  == 0 ? XYHZ_SETTNG_COLOR_1 : XYHZ_SETTNG_COLOR_2);
                    that.bgMusic2_txt.setTextColor(bgMusicType  == 1 ? XYHZ_SETTNG_COLOR_1 : XYHZ_SETTNG_COLOR_2);
                    that.bgMusic3_txt.setTextColor(bgMusicType  == 2 ? XYHZ_SETTNG_COLOR_1 : XYHZ_SETTNG_COLOR_2);
