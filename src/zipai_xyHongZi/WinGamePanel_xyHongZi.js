...
function setOutCardRes_xyHongZi(){
    MjClient.cardPath_xyHongZi = "playing/paohuzi/";
    if (MjClient.playui && MjClient.playui.getCardFilePath) {
        MjClient.cardPath_xyHongZi = MjClient.playui.getCardFilePath();
    }
}

var EndOneView_xyHongZi = cc.Layer.extend({
    jsBind: {
        block: {
                ...
                hutip: {
                    _run : function(){
                        ...
                        var nameObj = {heiHu:"黑胡", dianHu: "点胡", shiHong: "十红", xiaoYiSe: "小一色", 
                                        daYiSe: "大一色", qiDui: "7对", pengPengHu: "碰碰胡", juJuHong: "花胡",
                                        siPengDanDiao: "四碰单吊"}; 
                    ...
                    _run: function() {
                        ...
                        for (var i = 0; i < pl.mjchi.length; i++) {
                            ...
                            for (var j = 0; j < 3; j++) {
                                ...
                                cc.log("%%%%%%% chi card:", MjClient.cardPath_xyHongZi + "out" + card + ".png");
                                node.loadTexture( MjClient.cardPath_xyHongZi + "out" + card + ".png");
                                this.addChild(node);
                            }
                            idx++;
                        }
                        ...
                        for (var i = 0; i < pl.mjpeng.length; i++) {
                            ...
                            for (var j = 0; j < 3; j++) {
                                ...
                                node.visible = true;
                                node.x += idx * node.getContentSize().width;
                                cc.log("%%%%%%% mjpeng card:", MjClient.cardPath_xyHongZi + "out" + card + ".png");
                                node.loadTexture(MjClient.cardPath_xyHongZi + "out" + card + ".png");
                                this.addChild(node);
                            }
                            idx++;
                        ...
                        for (var i = 0; i < handSort.length; i++) {
                            ...
                            for (var j = 0; j < row.length; j++) {
                                ...
                                node.visible = true;
                                node.x += idx * node.getContentSize().width;
                                cc.log("%%%%%%%card:", MjClient.cardPath_xyHongZi + "out" + card + ".png");
                                node.loadTexture(MjClient.cardPath_xyHongZi + "out" + card + ".png");
                                this.addChild(node);   
    ...
    setItemInfoByPl : function(pl, cloneItem){
        ...
        setRoundEndUserOffline_xyHongZi(headIcon,pl);

        var dissTypeText = cloneItem.getChildByName("dissType");
        dissTypeText.ignoreContentAdaptWithSize(true);
        dissTypeText.setVisible(MjClient.isDismiss);
        cc.log("ssssssssssssssssssssssssssd = ",MjClient.isDismiss);
        if(MjClient.isDismiss){
            dissTypeText.setString(getDismissTypeName(pl));
        }
    ...
    setMsg : function(){
        ...
        //wanfa.ignoreContentAdaptWithSize(true);
