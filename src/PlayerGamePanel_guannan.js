...
//向服务器发送 过消息
MjClient.MJPass2NetForGuannan = function()
{
   // console.log(">>>>>>>>>普通  过 <<<<<<<<");
    cc.log("====================send======pass=====");
...
function SetUserVisible_guannan(node, off)
{
    //var sData = MjClient.data.sData;
    //return;
    cc.log("====================off======================" + off);
...
function InitUserHandUI_guannan(node, off)
{
    var sData = MjClient.data.sData;
    var tData = sData.tData;
    var pl = getUIPlayer(off);
...
function initFlower_guannan() {
    initFlower(false, false);
}


var PlayLayer_guannan = cc.Layer.extend({
    jsBind: {
        _event: {
            mjhand: function() {
            ...
            roundEnd: function() {
                ...
                function delayExe()
                {
                    ...
                    self.addChild(new EndOneView_guannan(),500);
                }
        ...
        banner: {
            ...
            Button_1: {
                _visible : true,
                _click: function() {
                    MjClient.openWeb({url:MjClient.GAME_TYPE.GUAN_NAN,help:true});
                }
            },
            hunPai:{
                baidaBg:{
        ...
        down: {
            ...
            _event: {
                ...
                MJPut: function(eD) {
                    ...
                    cc.log(">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>MJPut---------------tingSet = " + JSON.stringify(tingSet));
                ...
                MJChi: function(eD) {
                    ...
                    var _tingCards = this.getChildByName("tingCardsNode");
                    var tingSet = MjClient.majiang.calTingSetsplice(pl.mjhand);
                    cc.log(">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>MJChi---------------tingSet = " + JSON.stringify(tingSet));
                    setTingCards_showCount(_tingCards,tingSet);
                    setUserOffline(this, 0);
                },
                MJGang: function(eD) {
                    ...
                    var _tingCards = this.getChildByName("tingCardsNode");
                    var tingSet = MjClient.majiang.calTingSetsplice(pl.mjhand);
                    cc.log(">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>MJGang---------------tingSet = " + JSON.stringify(tingSet));
                    setTingCards_showCount(_tingCards,tingSet);
                    setUserOffline(this, 0);
                },
                MJPeng: function(eD) {
                    ...
                    var _tingCards = this.getChildByName("tingCardsNode");
                    var tingSet = MjClient.majiang.calTingSetsplice(pl.mjhand);
                    cc.log(">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>MJPeng---------------tingSet = " + JSON.stringify(tingSet));
                    setTingCards_showCount(_tingCards,tingSet);
                    setUserOffline(this, 0);
                },
                MJHu: function(eD) {
            ...
            ting: {
                ...
                _touch: function(btn, eT) {
                    if (eT == 2)
                    {
                         ...
                         */
                        // var pl = getUIPlayer(0);
                        // var tingSet = getCheckTingHuCards(cardui.tag,pl.mjhand);
                        //cc.log(">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>MJChi---------------tingSet = " + JSON.stringify(tingSet));
                        MjClient.playui._tingCardsNode.visible = false;
                        //setTingCards_showCount(MjClient.playui._tingCardsNode,tingCards);
                    }
                }
            },
            noTing : {
            ...
            changeui: {
                ...
                changeuibg: {
                    ...
                    guobg: {
                        guo: {
                            _touch: function(btn, eT) {
                                if (eT == 2) MjClient.MJPass2NetForGuannan();
                            }
                        },
                        fanhui: {
                            _touch: function(btn, et) {
    ...
    ctor: function() {
        this._super();
        var playui = ccs.load(res.Play_guannan_json);
    ...
    */
    ...
    getCahtText:function(num) {
        switch(num){
            case 0:
                return "你看看，打什么来什么";
                break;
            case 1:
                return "要他他不来，不要的一个劲的来";
                break;
            case 2:
                return "还有比这个牌差的不";
                break;
...
PlayLayer_guannan.prototype.CardLayoutRestore = function(node, off)
{
            ...
            if(off == 0)
            {
                    ...
                    if (ci.name == "mjhand")
                    {
                        if(MjClient.canTingCards[ci.tag])
                        {
                            ...
                            if (!hasUp) {
                                ...
                                hasUp = true;
                                var pl = getUIPlayer(0);
                                var tingSet = getCheckTingHuCards(ci.tag,pl.mjhand);
                                setTingCards_showCount(MjClient.playui._tingCardsNode,tingSet);
                            }
                        }
                        else {
                            ci.setColor(cc.color(190, 190, 190));
                ...
                else if(i == orders.length - 1)
                {
                    ...
                    if(newC)
                    {
                        ...
                        var pl = getUIPlayer(0);
                        var tingSet = MjClient.majiang.getCheckTingHuCardsSplice(ci.tag,pl.mjhand);
                        setTingCards_showCount(MjClient.playui._tingCardsNode,tingSet);
...
PlayLayer_guannan.prototype.willHuShowArrow = function()
{
...
// 判断吃碰杠胡的状态
PlayLayer_guannan.prototype.EatVisibleCheck = function()
{
    var eat = MjClient.playui.jsBind.eat;
            ...
            for(var i=cduis.length-1;i>=0;i--)
            {
                if(cduis[i].name == "mjhand" && MjClient.majiang.isCardFlower(cduis[i].tag))
                {
                    var closeCallBack = function(){
                        ...
                        cduis[index].runAction(cc.sequence(cc.delayTime(0.3), cc.callFunc(callback)));
                    }
