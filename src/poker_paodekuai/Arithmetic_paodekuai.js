...
Paodekuai.prototype.CARDTPYE = PDK_CARDTPYE;
var PDK_CARDCOUNT = {};
...
var PDK_CARD_VALUE = {};
PDK_CARD_VALUE[PDK_CARDTPYE.sangeA] = 2;
PDK_CARD_VALUE[PDK_CARDTPYE.sizha] = 1;
...
var pdk_allBuChaiType = [
    ...
    PDK_CARDTPYE.sanshun,
    PDK_CARDTPYE.feiji,
    PDK_CARDTPYE.sizha,
    PDK_CARDTPYE.sangeA
]

Paodekuai.prototype.allTipsNoOrder = [
    [PDK_CARDTPYE.danpai, []],
    [PDK_CARDTPYE.duizi, []],
 ...
 * @return {number}
 */
Paodekuai.prototype.calPoint = function(num) {
    if (!num)
        return -1;
 ...
 * @return {boll} 
 */
Paodekuai.prototype.isSanShun = function(oCards){
...
// console.log( Paodekuai.prototype.isSanShun([3,3,3]) );
 ...
 */
Paodekuai.prototype.getSanShunAndSanPai = function(oCards){
    var countSanSame = 0;
    // 记录所有3张的数值
    var sanSameList = [];
    // 记录散牌
...
// console.log( Paodekuai.prototype.getSanShunAndSanPai(handCards) );
 ...
 */
Paodekuai.prototype.isFeiJi = function(oCards){
 ...
 * @return {bool}
 */
Paodekuai.prototype.isShun = function(oCards) {
...
Paodekuai.prototype.isHadZhaDan = function(cards) {
    ...
    if(this.findNSameCard(cards, PDK_APOINT, 3) && cards.length > 3)
        return true;
 ...
 * @return {bool}
 */
Paodekuai.prototype.isLiandui = function(oCards) { //oCards有序
...
Paodekuai.prototype.calType = function(pCards) {
    ...
    if (cardCount == 3 && allSame && this.calPoint(cards[0]) == PDK_APOINT)
...
Paodekuai.prototype.cardsType = function(cards) {
...
// 牌点比较函数
Paodekuai.prototype.cardValueCmp = function(a, b) {
...
// 排掉从大到小排序
Paodekuai.prototype.cardSortBigToSmall = function(a, b) {
    var pa = this.calPoint(a);
    var pb = this.calPoint(b);
    if (pa == pb)
        return b - a;
 ...
 * @return {number} 返回癞子数
 */
Paodekuai.prototype.transformAndGetLaizi = function(oCards, cards) {
    var laizi = 0;
    for (var i = 0; i < oCards.length; i++) {
 ...
 * @return {array}
 */
Paodekuai.prototype.findNSameCard = function(hands, point, n) {
 ...
 */
Paodekuai.prototype.lieju = function(parr, num){
    var tmps = [];
...
Paodekuai.prototype.formatCards = function(oCards) {
    var info = {};
...
Paodekuai.prototype.getAllMaxCard = function(oCards){
...
// Paodekuai.prototype.getAllCardType = function(oHands, oLastCards){
//     var hands = [];
//     var handLaizi = this.transformAndGetLaizi(oHands, hands);


//     for(var i=0; i<PDK_CARDTPYE.length; i++){
//         var cardType = PDK_CARDTPYE[i];
//         this.findCardByType(hands, 0, lastCardsType, oLastCards);

//     }
// }

Paodekuai.prototype.findDaiPai = function(oCards, type, findNum, same, buChaiTypes) {
    var ret = [];
...
Paodekuai.prototype.findPutTipCards = function(hands, lastCards, isMustPutHongTaoSan, isNextPlayerOneCard, putOrder, allBuChaiTypes) {
    ...
    //当勾“先出红桃三”选项时，提示出红桃三
    if (isMustPutHongTaoSan && hands.indexOf(HONG_TAO_3) >=0){
        lastType = 99;
        includeCard = HONG_TAO_3;
 ...
 */
Paodekuai.prototype.IsSametipCards = function(tipscardsArray, areaSelectMode)
{
...
Paodekuai.prototype.delDaipai = function(Cards, areaSelectMode)
{
    ...
    if( cardtype == PDK_CARDTPYE.sandaiyi)
