...
function SetUserVisible_paodekuai(node, off)
{
    ...
    if(pl)
    {
        ...
        InitUserHandUI_paodekuai(node, off);
...
function InitUserHandUI_paodekuai(node, off, needSort)
{
...
function cardsSort_paodekuai(cards)
{
    var pointCounts = {};
    for (var i = 0; i < cards.length; i++) {
        var p = MjClient.majiang_paodekuai.calPoint(cards[i]);
        if (pointCounts[p])
            pointCounts[p] ++;
        else
            pointCounts[p] = 1;
    ...
    var commonCmp = function (a, b) {
        var c1 = pointCounts[MjClient.majiang_paodekuai.calPoint(a)];
        var c2 = pointCounts[MjClient.majiang_paodekuai.calPoint(b)];
        if (c1 == c2)
            return MjClient.majiang_paodekuai.cardValueCmp(a, b);
        else
            return c1 - c2;
    }
...
var PlayLayer_paodekuai = cc.Layer.extend({
    _btnPutCard:null,
    jsBind: {
        _run: function() {
        ...
        banner: {
            ...
            setting: {
                ...
                _run:function() {
                    //setWgtLayout(this,[0.12, 0.12],[0.94,0.04],[0,0]);
                }
            },
            gps_btn: {
                _run: function() {
        ...
        BtnPutCard:{ //add by  sking for put card button
            ...
            _click: function(btn) {
                //cc.log("BtnPutCard");
				cardsSort_paodekuai(MjClient.selectCards_card);
        ...
        down: {
            ...
            _event: {
                ...
                initSceneData: function(eD) {
                    SetUserVisible_paodekuai(this, 0);
                ...
                mjhand: function(eD) {
                    InitUserHandUI_paodekuai(this, 0);
        ...
        right: {
            ...
            _event: {
                ...
                initSceneData: function(eD) {
                    SetUserVisible_paodekuai(this, 1);
                },
                addPlayer: function(eD) {
                    SetUserVisible_paodekuai(this, 1);
                },
                ...
                mjhand: function(eD) {
                    InitUserHandUI_paodekuai(this, 1);
                },
                roundEnd: function() {
        ...
        top: {
            ...
            _event: {
                ...
                initSceneData: function(eD) {
                    SetUserVisible_paodekuai(this, 2);
                },
                addPlayer: function(eD) {
                    SetUserVisible_paodekuai(this, 2);
                },
                ...
                mjhand: function(eD) {
                    InitUserHandUI_paodekuai(this, 2);
                },
                roundEnd: function() {
        ...
        left: {
            ...
            _event: {
                ...
                initSceneData: function(eD) {
                    SetUserVisible_paodekuai(this, 3);
                },
                addPlayer: function(eD) {
                    SetUserVisible_paodekuai(this, 3);
                },
                ...
                mjhand: function(eD) {
                    InitUserHandUI_paodekuai(this, 3);
                },
                roundEnd: function() {
        ...
        chat_btn: {
            ...
            _click: function() {
                // var bWin = false;
                // var cl = (bWin==true)? cc.color(0xff,0xfe,0xd6):cc.color(0xd4,0xfe,0xff);
                // cc.log("================", cl.r, cl.g, cl.b);
    ...
    ctor: function() {
        this._super();
        var playui = ccs.load(res.Play_paodekuai_json);
...
PlayLayer_paodekuai.prototype.InitC_Data = function() {
...
PlayLayer_paodekuai.prototype.cannotOutCardGrey = function()
{
...
PlayLayer_paodekuai.prototype.recoverCannotOutCard = function()
{
    var children = this._downNode.children;
    for (var i = 0; i < children.length; i++)
    {
...
PlayLayer_paodekuai.prototype.clockNumberUpdate = function(node, endFunc)
{
    return arrowbkNumberUpdate(node, endFunc);
}

PlayLayer_paodekuai.prototype.updateClockPosition = function(arrowNode)
{
    var tData = MjClient.data.sData.tData;
    var uids = tData.uids;
...
PlayLayer_paodekuai.prototype.CardLayoutRestore = function(node, off, needSort)
{
    // 如果正在发牌 不排序
...
PlayLayer_paodekuai.prototype.CardLayoutDesk = function(node,cards,off)
{
    //if(off != 0) return;
    ...
    var sort = function (node)
    {
        ...
        for (var i = 0; i < node.length; i++) {
            var p = MjClient.majiang_paodekuai.calPoint(node[i].tag);
            if (pointCounts[p])
                pointCounts[p] ++;
            else
                pointCounts[p] = 1;
        ...
        var commonCmp = function (a, b) {
            var c1 = pointCounts[MjClient.majiang_paodekuai.calPoint(a.tag)];
            var c2 = pointCounts[MjClient.majiang_paodekuai.calPoint(b.tag)];
            if (c1 == c2)
                return MjClient.majiang_paodekuai.cardValueCmp(a.tag, b.tag);
            else
                return c1 - c2;
        }
...
//横向摆放《正常》
PlayLayer_paodekuai.prototype.horSort = function(node, off, needSort)
{
    var pl; //player 信息
...
//纵向摆放
PlayLayer_paodekuai.prototype.verSort = function(node, off)
{
    if(off != 0)
    {
        return;
