...
//向服务器发送 过消息
MjClient.MJPass2NetForlinfenjixian = function()
{
   // console.log(">>>>>>>>>普通  过 <<<<<<<<");
    cc.log("====================MJPass2NetForlinfenjixian======pass=====");
...
function SetUserVisible_linfenjixian(node, off)
{
    //var sData = MjClient.data.sData;
    //return;
...
//处理出牌,放一张牌，打牌动作
function DealMJPut_linfenjixian(node, msg, off, outNum)
{
    DealMJPut_shanXiApp(node, msg, off, outNum);
}

//初始化手牌的位置
function InitUserHandUI_linfenjixian(node, off)
{
    var sData = MjClient.data.sData;
    var tData = sData.tData;
    var pl = getUIPlayer(off);
    ...
    for(var i = 0; i < pl.mjput.length; i++)
    {
        ...
        DealMJPut_linfenjixian(node, msg, off, i);
    }
...
function initFlower_linfenjixian() {
    initFlower(false, false);
}


var PlayLayer_linfenjixian = cc.Layer.extend({
    jsBind: {
        _event: {
            mjhand: function() {
            ...
            roundEnd: function() {
                if (MjClient.getAppType() == MjClient.APP_TYPE.TXLINFENMJ)
                {
                    ...
                    function delayExe()
                    {
                        ...
                        self.addChild(new EndOneView_linfenjixian(),500);
                    }
                    //hideCurrentTingNum();
            ...
            changeui: {
                ...
                changeuibg: {
                    ...
                    guobg: {
                        guo: {
                            _touch: function(btn, eT) {
                                if (eT == 2) MjClient.MJPass2NetForlinfenjixian();
                            }
                        },
                        fanhui: {
                            _touch: function(btn, et) {
    ...
    ctor: function() {
        this._super();
        var playui = ccs.load("Play_linfenjixian.json");
...
PlayLayer_linfenjixian.prototype.CardLayoutRestore = function(node, off)
{
...
// 判断吃碰杠胡的状态
PlayLayer_linfenjixian.prototype.EatVisibleCheck = function()
{
    var eat = MjClient.playui.jsBind.eat;
...
/*
    玩家ui，信息
 */
PlayLayer_linfenjixian.prototype.Node_player = function(off)
{
    ...
    UIEventBind(null, _node, "initSceneData", function (eD) {
        SetUserVisible_linfenjixian(_node, off);
    ...
    UIEventBind(null, _node, "mjhand", function (eD) {
        InitUserHandUI_linfenjixian(_node, off);
    });
    ...
    UIEventBind(null, _node, "MJPut", function (eD) {
        ...
        // }
        DealMJPut_linfenjixian(_node,eD,off);
        hideTingBtn();
        if(_tingCardNumNode) _tingCardNumNode.visible = false;
