...
//向服务器发送 过消息
MjClient.MJPass2NetForLingshibianlong = function()
{
   // console.log(">>>>>>>>>普通  过 <<<<<<<<");
    cc.log("====================MJPass2NetForLingshibianlong======pass=====");
...
function SetUserVisible_lingshibianlong(node, off)
{
    //var sData = MjClient.data.sData;
    //return;
    cc.log("====================off======================" + off);
...
//处理出牌,放一张牌，打牌动作
function DealMJPut_lingshibianlong(node, msg, off, outNum)
{
    DealMJPut_shanXiApp(node, msg, off, outNum);
}

function InitUserHandUI_lingshibianlong(node, off)
{
    var sData = MjClient.data.sData;
    var tData = sData.tData;
    var pl = getUIPlayer(off);
    ...
    for(var i = 0; i < pl.mjput.length; i++)
    {
        ...
        DealMJPut_lingshibianlong(node, msg, off, i);
    }
...
function initFlower_lingshibianlong() {
    initFlower(false, false);
}


var PlayLayer_lingshibianlong = cc.Layer.extend({
    jsBind: {
        _event: {
            mjhand: function() {
            ...
            roundEnd: function() {
                ...
                function delayExe()
                {
                    ...
                    self.addChild(new EndOneView_lingshibianlong(),500);
                }
        ...
        banner: {
            ...
            Button_1: {
                _visible : true,
                _click: function() {
                    MjClient.openWeb({url:MjClient.GAME_TYPE.LING_SHI_BIAN_LONG,help:true});
                }
            },
            hunPai:{
                baidaBg:{
        ...
        eat: {
            ...
            guo: {
                ...
                _touch: function(btn, eT) {
                    if (eT == 2)
                    {
                        MjClient.MJPass2NetForLingshibianlong();
                    }
                },
                bgimg: {
                    _run: function() {
            ...
            changeui: {
                ...
                changeuibg: {
                    ...
                    guobg: {
                        guo: {
                            _touch: function(btn, eT) {
                                if (eT == 2) MjClient.MJPass2NetForLingshibianlong();
                            }
                        },
                        fanhui: {
                            _touch: function(btn, et) {
    ...
    ctor: function() {
        this._super();
        var playui = ccs.load(res.Play_lingshibianlong_json);
...
PlayLayer_lingshibianlong.prototype.CardLayoutRestore = function(node, off)
{
...
// 判断吃碰杠胡的状态
PlayLayer_lingshibianlong.prototype.EatVisibleCheck = function()
{
    var eat = MjClient.playui.jsBind.eat;
    ...
    if(tData.tState == TableState.waitPut && pl.mjState == TableState.waitPut)
    {
        if(IsTurnToMe())
        {
            ...
            if(rtn.length > 0 && pl.isNew)
            {
                ...
                MjClient.playui.CardLayoutRestorePengGang();
    ...
    else if(tData.tState == TableState.waitEat)
    {
        ...
        if(!IsTurnToMe())
        {
            ...
            if (pl.eatFlag & 2) {
                ...
                MjClient.pengCards = [tData.lastPutCard];
...
/*
    玩家ui，信息
 */
PlayLayer_lingshibianlong.prototype.Node_player = function(off)
{
    ...
    UIEventBind(null, _node, "initSceneData", function (eD) {
        SetUserVisible_lingshibianlong(_node, off);
    ...
    UIEventBind(null, _node, "mjhand", function (eD) {
        InitUserHandUI_lingshibianlong(_node, off);
    });
    ...
    UIEventBind(null, _node, "MJPut", function (eD) {
        ...
        // }
        DealMJPut_lingshibianlong(_node,eD,off);
        hideTingBtn();
        if(_tingCardNumNode) _tingCardNumNode.visible = false;
 ...
 * @constructor
 */
PlayLayer_lingshibianlong.prototype.CardLayoutRestorePengGang = function () {

    var moveY = 5;
    var pl = getUIPlayer(0);
    var myCards = MjClient.playui._downNode.children;
    var handCard = pl.mjhand.slice();
    var pengCards = MjClient.pengCards || [];
    var gangCards = MjClient.gangCards || [];
    if(!pl || !handCard) return;

    var needControlCards = [];
    for(var i = 0; i < myCards.length; i++){
        if(myCards[i].getName() === "mjhand"){
            var cardTag = myCards[i].tag;
            if(pengCards.indexOf(cardTag) > -1 || gangCards.indexOf(cardTag) > -1){
                needControlCards.push(myCards[i]);
                var originY = myCards[i].y;
            }
        }
    }

    var setCardPosition = function (cardNode) {
        cardNode.setPositionY(originY);
    };

    if(handCard.length % 3 === 2) needControlCards.pop();    // 新牌不用操作
    for(var j = 0; j < needControlCards.length; j++){
        var cardNode = needControlCards[j];
        var action = cc.sequence(
            cc.moveBy(0.3, 0, moveY),
            cc.delayTime(0.8),
            cc.moveBy(0.1, 0, -moveY),
            cc.callFunc(setCardPosition)
        );
