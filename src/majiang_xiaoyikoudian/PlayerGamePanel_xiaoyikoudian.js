...
//向服务器发送 过消息
MjClient.MJPass2NetForxiaoyikoudian = function()
{
   // console.log(">>>>>>>>>普通  过 <<<<<<<<");
    cc.log("====================MJPass2NetForxiaoyikoudian======pass=====");
...
function SetUserVisible_xiaoyikoudian(node, off)
{
    //var sData = MjClient.data.sData;
    //return;
...
//处理出牌,放一张牌，打牌动作
function DealMJPut_xiaoyikoudian(node, msg, off, outNum)
{
    DealMJPut_shanXiApp(node, msg, off, outNum);
}


function InitUserHandUI_xiaoyikoudian(node, off)
{
    var sData = MjClient.data.sData;
    var tData = sData.tData;
    var pl = getUIPlayer(off);
    ...
    for(var i = 0; i < pl.mjput.length; i++)
    {
        ...
        DealMJPut_xiaoyikoudian(node, msg, off, i);
    }
...
function initFlower_xiaoyikoudian() {
    initFlower(false, false);
}


var PlayLayer_xiaoyikoudian = cc.Layer.extend({
    jsBind: {
        _event: {
            mjhand: function() {
            ...
            roundEnd: function() {
                ...
                function delayExe()
                {
                    ...
                    self.addChild(new EndOneView_xiaoyikoudian(),500);
                }
        ...
        eat: {
            ...
            guo: {
                ...
                _touch: function(btn, eT) {
                    if (eT == 2)
                    {
                        MjClient.MJPass2NetForxiaoyikoudian();
                    }
                },
                bgimg: {
                    _run: function() {
            ...
            changeui: {
                ...
                changeuibg: {
                    ...
                    guobg: {
                        guo: {
                            _touch: function(btn, eT) {
                                if (eT == 2) MjClient.MJPass2NetForxiaoyikoudian();
                            }
                        },
                        fanhui: {
                            _touch: function(btn, et) {
    ...
    ctor: function() {
        this._super();
        var playui = ccs.load("Play_xiaoyikoudian.json");
...
PlayLayer_xiaoyikoudian.prototype.CardLayoutRestore = function(node, off)
{
...
// 判断吃碰杠胡的状态
PlayLayer_xiaoyikoudian.prototype.EatVisibleCheck = function()
{
    var eat = MjClient.playui.jsBind.eat;
    ...
    if(tData.tState == TableState.waitPut && pl.mjState == TableState.waitPut)
    {
        if(IsTurnToMe())
        {
            cc.log("-----------canTingCards666666666666666--------------");
            //检测补花
            ...
            if (!pl.isTing) {
                ...
                cc.log("-----------canTingCards000000000000000--------------");
                for (var i = 0; i < pl.mjhand.length; i++) {
...
/*
    玩家ui，信息
 */
PlayLayer_xiaoyikoudian.prototype.Node_player = function(off)
{
    ...
    UIEventBind(null, _node, "initSceneData", function (eD) {
        SetUserVisible_xiaoyikoudian(_node, off);
    ...
    UIEventBind(null, _node, "mjhand", function (eD) {
        InitUserHandUI_xiaoyikoudian(_node, off);
    });
    ...
    UIEventBind(null, _node, "MJPut", function (eD) {
        ...
        // }
        DealMJPut_xiaoyikoudian(_node,eD,off);
        hideTingBtn();
        if(_tingCardNumNode) _tingCardNumNode.visible = false;
    ...
    switch (off)
    {
        ...
        case 1:
            ...
            setWgtLayout(_out1, [0, 0.055], [0.94, 0.5], [-3.9, -4.0]);
            if(_out2) setWgtLayout(_out2, [0, 0.055], [0.94, 0.5], [-2.6, -4.0]);
        ...
        case 2:
            ...
            setWgtLayout(_out0, [0, 0.08], [0.55, 1], [4.1, -3.5]);
            setWgtLayout(_out1, [0, 0.08], [0.55, 1], [4.1, -2.3]);
            if(_out2) setWgtLayout(_out2, [0, 0.08], [0.55, 1], [4.1, -1.1]);
        ...
        case 3:
            ...
            setWgtLayout(_out0, [0, 0.055], [0.06, 0.5], [5.2, 4.2]);
            setWgtLayout(_out1, [0, 0.055], [0.06, 0.5], [3.9, 4.2]);
