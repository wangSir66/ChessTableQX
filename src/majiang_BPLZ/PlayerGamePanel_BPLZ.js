...
//向服务器发送 过消息
MjClient.MJPass2NetForBPLZ = function()
{
   // console.log(">>>>>>>>>普通  过 <<<<<<<<");
    cc.log("====================send======pass=====");
...
function SetUserVisible_BPLZ(node, off)
{
    //var sData = MjClient.data.sData;
    //return;
...
function InitUserHandUI_BPLZ(node, off)
{
    var sData = MjClient.data.sData;
    var tData = sData.tData;
    var pl = getUIPlayer_changpai(off);
    ...
    cc.log("off-------------- pl.mjhand 8888 = " + pl.mjhand);
    ...
    cc.log(off + " = off-------------- pl.mjhand 8888 = " + pl.mjhand);
...
var PlayLayer_BPLZ = cc.Layer.extend({
    jsBind: {
        _event: {
            mjhand: function() {
            ...
            roundEnd: function() {
                ...
                function delayExe()
                {
                    ...
                    self.addChild(new EndOneView_BPLZ(),500);
                }
        ...
        down: {
            ...
            _event: {
                ...
                RGLong: function (eD) { //不罗龙
                    ...
                    PlayLayer_BPLZ.prototype.CardLayoutRestore(_downNode,0);
                },
            ...
            changeui: {
                ...
                changeuibg: {
                    ...
                    guobg: {
                        guo: {
                            _touch: function(btn, eT) {
                                if (eT == 2) MjClient.MJPass2NetForBPLZ();
                            }
                        },
                        fanhui: {
                            _touch: function(btn, et) {
    ...
    ctor: function() {
        this._super();
        var playui = ccs.load(res.Play_BPLZ_json);
        playMusic("bgFight");
...
PlayLayer_BPLZ.prototype.CardLayoutRestore = function(node, off)
{
    ...
    if(off == 0)
    {
        ...
        for(var i = 0;i < uiArray0.length;i++)
        {
            ...
            for(var j = 0;j < _uis.length;j++)
            {
                ...
                {
                    //cc.log("===========gang= " + ci.tag);
                    // cc.log(_idx+"--------杠是四张牌---------1111 = " + ci.tag);
                    // cc.log("--------杠是四张牌----------00000000 =  " + MjClient.data.sData.tData.hunCard);
                    if(_idx == 4)
                    {
                        _idx = 0;
                        leftStartX += _uiDx;
...
// 判断吃碰杠胡的状态
PlayLayer_BPLZ.prototype.EatVisibleCheck = function()
{
    var eat = MjClient.playui.jsBind.eat;
...
/*
    显示洛龙，不洛龙的按钮
 */
PlayLayer_BPLZ.prototype.longTouch = function(cd)
{
    var eat = MjClient.playui.jsBind.eat;
    //sk 为啥要隐藏掉？   //因为一开始是不可见的，隐藏根节点 by sking
    MjClient.playui.jsBind.eat.changeui.changeuibg._node.visible = false;
...
/*
    获取当前牌张数
 */
PlayLayer_BPLZ.prototype.setLeftCardCount = function(off)
{
    var pl = getUIPlayer_changpai(off);
    var sData = MjClient.data.sData;
    var tData = sData.tData;
