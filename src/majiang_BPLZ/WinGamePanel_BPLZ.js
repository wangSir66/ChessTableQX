...
function addWxHeadToEndUI_BPLZ(node,off)
{
...
function SetEndOneUserUI_BPLZ(node,off)
{
		...
		head: {
			...
			up: {
				...
				, _run: function () {
					...
					CardLayoutRestore_changpai_BPLZ(node, pl);
    ...
    addWxHeadToEndUI_BPLZ(uibind.head._node,off);
	//uibind.winNum._node.y=uibind.head._node.y;
}

CardLayoutRestore_changpai_BPLZ = function(node, pl)
{
    if(!pl) return;
...
var EndOneView_BPLZ = cc.Layer.extend({
	jsBind:{
        block: {
		...
		head0:{
		   ...
		   _run:function(){ SetEndOneUserUI_BPLZ(this,0); },
		...
		,head1:{
		  ...
		  _run:function(){ SetEndOneUserUI_BPLZ(this,1); }
		}
		,head2:{
		   head:{
			   zhuang:{_visible:false}
		   ...
		   _run:function(){ SetEndOneUserUI_BPLZ(this,2); }
		}
		}
	},
    ctor:function () {
        this._super();
        var endoneui = ccs.load(res.EndOne_BPLZ_json);
