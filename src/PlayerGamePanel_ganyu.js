...
//向服务器发送 过消息
MjClient.MJPass2NetForganyu = function()
{
   // console.log(">>>>>>>>>普通  过 <<<<<<<<");
    cc.log("====================send======pass=====");
...
function SetUserVisible_ganyu(node, off)
{
    //var sData = MjClient.data.sData;
    //return;
    cc.log("====================off======================" + off);
...
function InitUserHandUI_ganyu(node, off)
{
    var sData = MjClient.data.sData;
    var tData = sData.tData;
    var pl = getUIPlayer(off);
...
function initFlower_ganyu() {
    initFlower(false, false);
}


var PlayLayer_ganyu = cc.Layer.extend({
    jsBind: {
        _event: {
            mjhand: function() {
            ...
            roundEnd: function() {
                ...
                function delayExe()
                {
                    ...
                    self.addChild(new EndOneView_ganyu(),500);
                }
        ...
        banner: {
            ...
            Button_1: {
                _visible : true,
                _click: function() {
                    MjClient.openWeb({url:MjClient.GAME_TYPE.GAN_YU,help:true});
                }
            },
            hunPai:{
                baidaBg:{
        ...
        BtnPutCard:{ //add by  sking for put card button
            ...
            _event:{
                //拿倒一张牌的时候，出牌按钮亮起，其他状态隐藏，by sking
                mjhand: function() {
                    this.visible = false;
            ...
            changeui: {
                ...
                changeuibg: {
                    ...
                    guobg: {
                        guo: {
                            _touch: function(btn, eT) {
                                if (eT == 2) MjClient.MJPass2NetForganyu();
                            }
                        },
                        fanhui: {
                            _touch: function(btn, et) {
    ...
    ctor: function() {
        this._super();
        var playui = ccs.load(res.Play_ganyu_json);
...
PlayLayer_ganyu.prototype.CardLayoutRestore = function(node, off)
{
    // node 是克隆新建的一个麻将节点 by sking

    var newC = null; //先创建麻将的UI节点
    var newVal = 0; //新牌的值，是几万，几筒，几条....为数字
    var pl = getUIPlayer(off);//获取玩家信息.off 为0 ，就是自己得信息，能看倒自己摸牌 by sking
    var mjhandNum = 0;
    ...
    var orders = []; //重新排序后装倒数组里 by sking
...
// 判断吃碰杠胡的状态
PlayLayer_ganyu.prototype.EatVisibleCheck = function()
{
    var eat = MjClient.playui.jsBind.eat;
...
/*            var cduis=MjClient.playui.jsBind.down._node.children;
            ...
            ///tData.areaSelectMode.tingType !== TingCardType.noTing &&
    ...
    else if(tData.tState == TableState.waitEat)
    {
        ...
        if(!IsTurnToMe())
        {
            ...
            if (pl.eatFlag & 2) {
                ...
                eat.peng._node.visible = true;
                vnode.push(eat.peng._node);
            }

            //吃
            if (tData.areaSelectMode.canChi && pl.eatFlag & 1) {
            ...
            // cc.log("==================chi=吃======111========");
            // //吃
            // if (leftCard > 0 && tData.uids[(tData.curPlayer + 1) % 4] == SelfUid() && !pl.isTing)
            //
            // {
            //     var eatpos = mj.canChi(pl.mjhand, tData.lastPutCard);
            //     cc.log("==================chi=吃============= length =" + eatpos.length);
            //     MjClient.eatpos = eatpos;
            //     if (eatpos.length > 0)
            //     {
            //         vnode.push(eat.chi0._node);
