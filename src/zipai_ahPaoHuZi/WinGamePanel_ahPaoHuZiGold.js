function showUserInfoGold_ahPaoHuZi(node,off)
{
	...
	if(pl.winone >= 0){
		if(scoreLabel.setProperty){
			scoreLabel.setProperty("+" + pl.winone, "game_picture/gold/end/phz/redNum.png", 28, 43, "+");
		}else{
		...
		if(scoreLabel.setProperty){
			scoreLabel.setProperty(pl.winone, "game_picture/gold/end/phz/blueNum.png", 28, 43, "+");
		}else{
...
var EndOneView_ahPaoHuZiGold = cc.Layer.extend({
    _block:null,
    _endoneuiNode:null, 
	jsBind:{
		block: {
		...
		uiNode: {
			...
			huang : {
				...
				head: {
					_run : function(){
                        showUserInfoGold_ahPaoHuZi(this, 2);//上家
						...
						while(index < 2){
							var clone = this.clone();
			   				...
			   				showUserInfoGold_ahPaoHuZi(clone, index);
			   				index += 1;
						}
					}
				}
						...
						var sData = MjClient.data.sData;
			   			...
			   			var cloneFunc = function(cardNum){
			   				if(cloneNum != 0){
			   					...
			   					cloneCard.loadTexture(getOutCardRes_ahPaoHuZi(cardNum));
			   					cloneNum++;
			   					return cloneCard;
			   				}   
			   				cloneNum++;
			   				var cardImg = self.getChildByName("card");
			   				cardImg.loadTexture(getOutCardRes_ahPaoHuZi(cardNum));
			   				return cardImg;
			   			}
    ...
    setWinCard: function(){ 
    	...
    	var cloneCardsSetData = function(cardNode,i,num){
			...
			cardImg.loadTexture(getOutCardRes_ahPaoHuZi(num));
			return cardImg;
    	}
    ...
    setXingCardData: function(tData,num){
    	...
    	card_img.loadTexture(getOutCardRes_ahPaoHuZi(tData.hunCard));
