function addWxHeadToEndUI_ahPaoHuZi_GO(node, off) {
...
function SetGameOverLayer_ahPaoHuZi(node,off,isNewUi){
    ...
    addWxHeadToEndUI_ahPaoHuZi_GO(wxNode,off);
    ...
    if(MjClient.gameType == MjClient.GAME_TYPE.SHAO_YANG_BO_PI)
        ...
        if(pl.winall2 != undefined){
            scoreContnet = pl.winall2;
            if(tData.areaSelectMode.scoreNeedEnough == 1){
                if(pl.winall < 0 && pl.winall < pl.winall2){
...
var GameOverLayer_ahPaoHuZi = cc.Layer.extend({
    sprite:null,
    jsBind:{
        back:{
            close :{
    ...
    ctor:function () {
        this._super();
        var endallui = ccs.load("endAll_AnHuaPaoHuZi.json");
        ...
        for(var i = 0 ; i < playerCount ; i++){
            ...
            SetGameOverLayer_ahPaoHuZi(clone,i, false);
