...
//向服务器发送 过消息
MjClient.MJPass2NetForlvliangkoudian = function()
{
   // console.log(">>>>>>>>>普通  过 <<<<<<<<");
    cc.log("====================MJPass2NetForlvliangkoudian======pass=====");
...
function SetUserVisible_lvliangkoudian(node, off)
{
    //var sData = MjClient.data.sData;
    //return;
    cc.log("====================off======================" + off);
    ...
    if(pl)
    {
        ...
        InitUserHandUI_lvliangkoudian(node, off);
...
//处理出牌,放一张牌，打牌动作
function DealMJPut_lvliangkoudian(node, msg, off, outNum)
{
    DealMJPut_shanXiApp(node, msg, off, outNum);
}

function InitUserHandUI_lvliangkoudian(node, off)
{
    var sData = MjClient.data.sData;
    var tData = sData.tData;
    var pl = getUIPlayer(off);
    ...
    for(var i = 0; i < pl.mjput.length; i++)
    {
        ...
        DealMJPut_lvliangkoudian(node, msg, off, i);
    }
...
function initFlower_lvliangkoudian() {
    initFlower(false, false);
}


var PlayLayer_lvliangkoudian = cc.Layer.extend({
    jsBind: {
        _event: {
            mjhand: function() {
            ...
            roundEnd: function() {
                ...
                function delayExe()
                {
                    ...
                    self.addChild(new EndOneView_lvliangkoudian(),500);
                }
                    ...
                    _event: {
                        mjhand:function()
                        {
                            ...
                            if(HuncardMsg && HuncardMsg != -1)
                            {
                                ...
                                imgMing.loadTexture("playing/MJ/haozi.png");
        ...
        eat: {
            ...
            guo: {
                ...
                _touch: function(btn, eT) {
                    if (eT == 2)
                    {
                        MjClient.MJPass2NetForlvliangkoudian();
                    }
                },
                bgimg: {
                    _run: function() {
            ...
            changeui: {
                ...
                changeuibg: {
                    ...
                    guobg: {
                        guo: {
                            _touch: function(btn, eT) {
                                if (eT == 2) MjClient.MJPass2NetForlvliangkoudian();
                            }
                        },
                        fanhui: {
                            _touch: function(btn, et) {
            ...
            _event: {
                ...
                sendVoice: function(fullFilePath) {
                    ...
                    else
                    {
                        ...
                        fileName = extensionName[extensionName.length - 1];
                    }
    ...
    ctor: function() {
        this._super();
        var playui = ccs.load("Play_lvliangkoudian.json");
...
PlayLayer_lvliangkoudian.prototype.CardLayoutRestore = function(node, off)
{
...
// 判断吃碰杠胡的状态
PlayLayer_lvliangkoudian.prototype.EatVisibleCheck = function()
{
    var eat = MjClient.playui.jsBind.eat;
...
/*
    玩家ui，信息
 */
PlayLayer_lvliangkoudian.prototype.Node_player = function(off)
{
    ...
    UIEventBind(null, _node, "initSceneData", function (eD) {
        SetUserVisible_lvliangkoudian(_node, off);
    ...
    UIEventBind(null, _node, "mjhand", function (eD) {
        InitUserHandUI_lvliangkoudian(_node, off);
    });
    ...
    UIEventBind(null, _node, "MJPut", function (eD) {
        ...
        // }
        DealMJPut_lvliangkoudian(_node,eD,off);
        hideTingBtn();
        if(_tingCardNumNode) _tingCardNumNode.visible = false;
