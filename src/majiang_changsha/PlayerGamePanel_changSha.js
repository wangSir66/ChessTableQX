...
var _isShowShaiziAni_changSha = false; //为了控制mjhand之后播骰子动画时newCard的状态显示
//向服务器发送 过消息
MjClient.MJPass2NetForChangSha = function()
{
   // console.log(">>>>>>>>>普通  过 <<<<<<<<");
    cc.log("====================send======pass=====");
    ...
    if(IsTurnToMe() && tData.tState == TableState.waitPut)
    {
        ...
        var commitFunc = function () {
            ...
            if (eat.hu._node.visible && tData.curPlayer == selfIndex) {
                showSkipHuTip_changSha();
            }
...
function showSkipHuTip_changSha() {
    var tipsImageView = new ccui.ImageView("png/tips.png");
    var textView = new ccui.Text();

    textView.setString("你已放弃胡牌");
    textView.setFontSize(22);
    textView.setPosition(tipsImageView.width / 2, tipsImageView.height / 2);
    tipsImageView.addChild(textView);

    MjClient.playui.addChild(tipsImageView, 99999);

    setWgtLayout(tipsImageView, [0.35, 0.5], [0.5, -0.1], [0, 0]);
    tipsImageView.runAction(cc.moveBy(0.5, 0, cc.director.getVisibleSize().height * 0.45));

    function removeTip() {
        if (tipsImageView && cc.sys.isObjectValid(tipsImageView))
            tipsImageView.removeFromParent(true);
    }

    UIEventBind(null, MjClient.playui, "initSceneData", function(rtn) {
        removeTip();
    });

    UIEventBind(null, MjClient.playui, "roundEnd", function(rtn) {
        removeTip();
    });

    UIEventBind(null, MjClient.playui, "MJHu", function(rtn) {
        removeTip();
    });

    UIEventBind(null, MjClient.playui, "MJGang", function(rtn) {
        removeTip();
    });

    UIEventBind(null, MjClient.playui, "MJPut", function(rtn) {
        removeTip();
    });
}

var showShaiziAni_changSha = false
function showShaiziAni_changSha_old(poff)
{
    if(poff == 0) {
        ...
        if(MjClient.rePlayVideo != -1)//回放
        {
            for (var off = 0; off < 4; off++) {
                ...
                InitUserHandUI_changSha(_node, off);
            }
            playEffect("shuffle");
        ...
        _isShowShaiziAni_changSha = true;
        ...
        firstFrame.runAction(cc.sequence(playSoundFunc, animate,showPonitFunc,cc.delayTime(0.8),cc.callFunc(function(){
            //发牌
            _isShowShaiziAni_changSha = false;
...
function updateHeadUI_changSha(node, off , actionType)
{      
    cc.log('updateHeadUI_changSha', off)
    var pl = getUIPlayer(off);
    if(!pl) return;
    // showHandCard_MLHZ(node, off);
    showHandCard_changSha(node,off);
    MjClient.playui.CardLayoutRestore(node, off);
    cc.log('updateHeadUI_changSha update success', off)
...
function SetUserVisible_changSha(node, off)
{
    if (_isShowShaiziAni_changSha)  // 解决摇骰子时断线重连调用本函数，会手牌重复的bug by cyc
        return;
...
function showHandCard_changSha(node, off)
{
    var sData = MjClient.data.sData;
    var tData = sData.tData;
    var pl = getUIPlayer(off);
...
function InitUserHandUI_changSha(node, off)
{
    var sData = MjClient.data.sData;
    var tData = sData.tData;
    var pl = getUIPlayer(off);
    ...
    // }
     ...
     if (SelfUid() == pl.info.uid) {
        if (tData.tState == TableState.waitJiazhu) {
            cc.log("===== tData.tState , TableState.waitJiazhu ,pl.mjState",tData.tState , TableState.waitJiazhu,pl.mjState)
	        if (pl.mjState == TableState.waitJiazhu) {
                if(!MjClient.playui.getChildByName("jiaZhuCS")){
    ...
    if (off == 0) {
        ...
        if ((pl.tPutCard || Object.keys(tingSet).length > 0) && !downNode.getChildByName("AutoPut_btn")) 
        {
            ...
            var isErRen = "isErRen" in tData.areaSelectMode ? tData.areaSelectMode.isErRen : false;
            if(MjClient.getAppType() == MjClient.APP_TYPE.QXYYQP && !isErRen){
    ...
    if (kaiGangCardLen > 0) {
        var msg = {
            card1: kaiGangCardLen > 0 ? tData.lastPutCard : -1,
            card2: kaiGangCardLen > 1 ? tData.lastPutCard2 : -1,
            card3: kaiGangCardLen > 2 ? tData.lastPutCard3 : -1,
            card4: kaiGangCardLen > 3 ? tData.lastPutCard4 : -1,
            uid: pl.info.uid
        };
...
var PlayLayer_changSha = cc.Layer.extend({
    jsBind: {
        _event: {
            mjhand: function() {
            ...
            roundEnd: function() {
                ...
                function showEndCards()
                {
                    ...
                    self.addChild(new EndOneView_changSha(),500);
                }
                function delayExe()
                {
            ...
            initSceneData: function() {
                ...
                cc.log("initSceneData -----------------changsha = " + JSON.stringify(this));
                CheckRoomUiDelete();
            ...
            MJHu: function(d) {
                MjClient.playui.checkTipDuoHu(d);
            }
        },
        roundnumImg: {
            _run:function () {
        ...
        gameName:{
            _run: function() {
                ...
                if(isYongZhouProject())
                {
                    ...
                    setWgtLayout(this, [0.22, 0], [0.5, 0.74], [0, 1.0])
                }
                var self = this;
                this.schedule(function(){
                    var sData = MjClient.data.sData;
                    var areaSelectMode = sData.tData.areaSelectMode;
                    if(areaSelectMode.isErRen){
                        self.loadTexture("playing/gameTable/game_changsha_er.png");
                    }else{
                        self.loadTexture("playing/gameTable/game_changsha.png");
                    }
                });
            },
            // _event: {
        ...
        tableid:{
            _run: function() {
                ...
                if(isYongZhouProject())
                {
                    setWgtLayout(this,[0.15, 0.08], [0.101, 0.95],[0, 0])
                }
                else{
                    setWgtLayout(this,[0.115, 0], [0.5, 0.735],[0, 0])
                }
            },
            _event: {
                initSceneData: function() {
                    ...
                    this.setString(name + MjClient.data.sData.tData.tableid);
                }
            }
        },
        roundInfo:{
        ...
        banner: {
            ...
            setting: {
                _click: function() {
                    ...
                    if(isYongZhouProject()){
                        settringLayer = new RoomMaJiangSetting();
                    }else{
                        settringLayer = new SettingView();
                    }
        ...
        wait: {
            ...
            wxinvite: {
                _run:function(){
                    if(isYongZhouProject())
                    {
                        setWgtLayout(this,[0.2, 0.2],[0.65, 0.2],[0, 0]);
                    }
                    else {
        ...
        BtnPutCard:{ //add by  sking for put card button
            ...
            _event:{
                ...
                waitPut: function() {
                    if (_isShowShaiziAni_changSha) return;
                    var pl = getUIPlayer(0);
                    var eat = MjClient.playui.jsBind.eat;
                    if (MjClient.rePlayVideo == -1 && IsTurnToMe() && pl.isTing && !eat.hu._node.visible && !eat.gang0._node.visible && !eat.gang1._node.visible && !eat.gang2._node.visible) {
                        cc.log("*********自动出牌*********");
                        var downNode = MjClient.playui._downNode;
                        var AutoPut_btn = downNode.getChildByName("AutoPut_btn");
                        if (AutoPut_btn) AutoPut_btn.visible = false;
        ...
        down: {
            ...
            out1: {
                _run: function () {
                    ...
                    else if (MjClient.size.width / MjClient.size.height >= 1.5)
                    {
                        setWgtLayout(this, [0.0, 0.0763], [0.53, 0], [-7, 4.85]);
                    }
                    else
            ...
            _event: {
                ...
                initSceneData: function(eD) {
                    SetUserVisible_changSha(this, 0);
                },
                addPlayer: function(eD) {
                    SetUserVisible_changSha(this, 0);
                },
                ...
                mjhand: function(eD) {
					if (showShaiziAni_changSha)
                    	showShaiziAni_changSha(0);
					else
						InitUserHandUI_changSha(this, 0);
                },
                roundEnd: function() {
                ...
                MJPut: function(eD) {
                    ...
                    if (eD.uid == SelfUid() && MjClient.data.sData.tData.areaSelectMode.isOpenTingTip)
                    {
                        ...
                        var tingSet = MjClient.majiang.calTingSet(pl.mjhand);
                        setTingCards(_tingCards,tingSet);
                        var isErRen = "isErRen" in MjClient.data.sData.tData.areaSelectMode ? MjClient.data.sData.tData.areaSelectMode.isErRen : false;
                        if (MjClient.getAppType() == MjClient.APP_TYPE.QXYYQP && !isErRen) {
                ...
                MJChi: function(eD) {
                    // DealMJChi(this, eD, 0);
                    updateHeadUI_changSha(this, 0 , ActionType.CHI);
                    setUserOffline(this, 0);
                    var downNode = MjClient.playui._downNode;
                    if (MjClient.getAppType() == MjClient.APP_TYPE.QXYYQP) {
                        var AutoPut_btn = downNode.getChildByName("AutoPut_btn");
                ...
                MJPeng: function(eD) {
                    ...
                    if (MjClient.rePlayVideo != -1) {
                        ...
                        updateHeadUI_changSha(this, 0, ActionType.PENG);
                    }
                    setUserOffline(this, 0);
                    MjClient.playui.EatVisibleCheck();
                    if (MjClient.getAppType() == MjClient.APP_TYPE.QXYYQP) {
                ...
                MJHu: function(eD) {
                    ...
                    if (MjClient.getAppType() == MjClient.APP_TYPE.QXYYQP) {
                        var AutoPut_btn = this.getChildByName("AutoPut_btn");
                        if (AutoPut_btn && eD.cpginfo && eD.cpginfo.id == SelfUid()) {
                            AutoPut_btn.removeFromParent();
                            MjClient.gamenet.request("pkroom.handler.tableMsg", {
                                cmd: "MJTouchPutCard",
                                tPutCard: false
        ...
        right: {
            ...
            _event: {
                ...
                initSceneData: function(eD) {
                    SetUserVisible_changSha(this, 1);
                },
                addPlayer: function(eD) {
                    SetUserVisible_changSha(this, 1);
                },
                ...
                mjhand: function(eD) {
					if (!showShaiziAni_changSha)
                    	InitUserHandUI_changSha(this, 1);
                },
                roundEnd: function() {
                ...
                MJChi: function(eD) {
                    if (MjClient.rePlayVideo != -1)
                        ...
                        updateHeadUI_changSha(this, 1, ActionType.CHI);
                    setUserOffline(this, 1);
                },
                MJGang: function(eD) {
                ...
                MJPeng: function(eD) {
                    if (MjClient.rePlayVideo != -1)
                        ...
                        updateHeadUI_changSha(this, 1, ActionType.PENG);
                    setUserOffline(this, 1);
                },
                MJHu: function(eD) {
        ...
        top: {
            ...
            _event: {
                ...
                initSceneData: function(eD) {
                    SetUserVisible_changSha(this, 2);
                },
                addPlayer: function(eD) {
                    SetUserVisible_changSha(this, 2);
                },
                ...
                mjhand: function(eD) {
					if (!showShaiziAni_changSha)
                    	InitUserHandUI_changSha(this, 2);
                },
                roundEnd: function() {
                ...
                MJChi: function(eD) {
                    if (MjClient.rePlayVideo != -1)
                        ...
                        updateHeadUI_changSha(this, 2, ActionType.CHI);
                    setUserOffline(this, 2);
                },
                MJGang: function(eD) {
                ...
                MJPeng: function(eD) {
                    if (MjClient.rePlayVideo != -1)
                        ...
                        updateHeadUI_changSha(this, 2, ActionType.PENG);
                    setUserOffline(this, 2);
                },
                MJHu: function(eD) {
        ...
        left: {
            ...
            _event: {
                ...
                initSceneData: function(eD) {
                    SetUserVisible_changSha(this, 3);
                },
                addPlayer: function(eD) {
                    SetUserVisible_changSha(this, 3);
                },
                ...
                mjhand: function(eD) {
					if (!showShaiziAni_changSha)
                   		InitUserHandUI_changSha(this, 3);
                },
                roundEnd: function() {
                ...
                MJChi: function(eD) {
                    if (MjClient.rePlayVideo != -1)
                        ...
                        updateHeadUI_changSha(this, 3, ActionType.CHI);
                    setUserOffline(this, 3);
                },
                MJGang: function(eD) {
                ...
                MJPeng: function(eD) {
                    if (MjClient.rePlayVideo != -1)
                        ...
                        updateHeadUI_changSha(this, 3, ActionType.PENG);
                    setUserOffline(this, 3);
                },
                MJHu: function(eD) {
            ...
            chi0: {
                ...
                _touch: function(btn, eT) {
                    if (eT == 2) 
                    {
                        ...
                        if (MjClient.playui.jsBind.eat.hu._node.visible)
                            MjClient.showMsg("确认 不胡 吗?", func, function() {}, "1");
                        else
                            func();
                    }
                },
            ...
            changeui: {
                ...
                changeuibg: {
                    ...
                    guobg: {
                        guo: {
                            _touch: function(btn, eT) {
                                if (eT == 2) MjClient.MJPass2NetForChangSha();
                            }
                        },
                        fanhui: {
                            _touch: function(btn, et) {
        ...
        hu_btn: {
            ...
            _touch: function(btn, eT) {
                if (eT == 2) {
                    //显示起手胡
                    var layer = new showQiShouHuLayer();
    ...
    ctor: function() {
        ...
        initSceneFunc();
        if(isYongZhouProject() && !COMMON_UI3D.isCanChangTo3D()) COMMON_UI3D.InitSetArrowbk();
        changeMJBg(this, getCurrentMJBgType());
        ...
        if(isYongZhouProject())
        {
            addClub_BackHallBtn(true, [[0.11, 0.11], [0.05, 0.78], [0, 0]], [[0.11, 0.11], [0.1, 0.78], [0, 0]]);
        }
        MjClient.playui.refreshHeadLight(false);
        return true;
    },
    ...
    {
        var tData = MjClient.data.sData.tData;
        if (!tData.areaSelectMode.xianPiao && !tData.areaSelectMode.piaoFen)
            return;
    ...
    getGameInfoString: function(param)
    {
        ...
        */
        ...
        if (tData.areaSelectMode.menQing_no258)
            str += "门清不要258将,";
        if (tData.areaSelectMode.queYiMen)
            str += "缺一门,";
        if (tData.areaSelectMode.kai4Gang)
            str += "开4杠,";
...
PlayLayer_changSha.prototype.CardLayoutRestore = function(node, off, isNewCard)
{
...
// 提示等待起手胡
PlayLayer_changSha.prototype.ShowQiShouHuTip = function()
{
    var eat = MjClient.playui.jsBind.eat;
    var sData = MjClient.data.sData;
...
PlayLayer_changSha.prototype.haveQiShouHu = function()
{
    var sData = MjClient.data.sData;
    for (var uid in sData.players) {
        var p = sData.players[uid];
...
PlayLayer_changSha.prototype.EatKaiGangVisibleCheck = function(tData, vnode, eat, pl) 
{
    var isKaiGangAfterSelfAnGang = tData.uids[tData.curPlayer] == pl.info.uid;
    for (var i = 0; i < 4; i++) {
        var eatFlag = i == 0 ? pl.eatFlag : pl["eatFlag" + (i + 1)];
...
PlayLayer_changSha.prototype.showKaiGangBigCards = function() {
    var tData = MjClient.data.sData.tData;
    cc.log("showKaiGangBigCards: tData.lastPutCard=" + tData.lastPutCard + " tData.lastPutCard2=" + tData.lastPutCard2 + 
        " tData.lastPutCard3=" + tData.lastPutCard3 + " tData.lastPutCard4=" + tData.lastPutCard4);

    if (tData.lastPutCard == -1 && tData.lastPutCard2 == -1 && tData.lastPutCard3 == -1 && tData.lastPutCard4 == -1)
        return;
    ...
    for (var i = 0; i < MjClient.MaxPlayerNum; i ++)
    {
        ...
        if (pl.eatFlag > 0 || pl.eatFlag2 > 0 || pl.eatFlag3 > 0 || pl.eatFlag4 > 0)
        {
            all_0 = false;
            break;
        }
    ...
    if (all_0 && !COMMON_UI3D.is3DUI())
        return;

    var kaiGangKuang;
    if (tData.lastPutCard3 != -1)
        kaiGangKuang = MjClient.playui.jsBind.eat.kaiGangKuang2._node;
    else
        kaiGangKuang = MjClient.playui.jsBind.eat.kaiGangKuang._node;
    ...
    if (tData.lastPutCard3 != -1) {
        ...
        if (tData.lastPutCard3 != -1)
            setCardSprite(cardNode3, tData.lastPutCard3, 0);
        if (tData.lastPutCard4 != -1)
            setCardSprite(cardNode4, tData.lastPutCard4, 0);
    }
}

PlayLayer_changSha.prototype.hideAllEat = function(){
    var eat = MjClient.playui.jsBind.eat;
    ...
    eat.kaiGangKuang2._node.visible = false;
 ...
 */
var QiShouHuNode = {};
PlayLayer_changSha.prototype.playQiShouHuAnim = function(node, eD, off){
    var uidHu = eD.uid;
    var pl = getUIPlayer(off);
    ...
    var qshuNode1 = node.getChildByName("out_qshu_layout");
    ...
    this.showQiShouHuScore(eD.score, off);

    return true;
}

PlayLayer_changSha.prototype.showQiShouHuScore = function(score, off) {
    if (!score)
        return;

    for (var i = 0; i < MjClient.MaxPlayerNum; i++) {
        var curOff = getOffForPlayerNum(i);
        var node = getNode(curOff);

        node.removeChildByName("qiShouHuScore");

        var iconImg = new cc.Sprite(off == curOff ? "playground/gang_addIcon.png" : "playground/gang_subIcon.png");
        var tempScore = off == curOff ? score : score/(MjClient.MaxPlayerNum - 1);
        var scoreText = new ccui.TextAtlas(tempScore + "", off == curOff ? "playground/gang_addText.png" : "playground/gang_subText.png", 50, 73, ".");
...
// 判断吃碰杠胡的状态
PlayLayer_changSha.prototype.EatVisibleCheck = function() {
    var eat = MjClient.playui.jsBind.eat;
    var sData = MjClient.data.sData;
    var tData = sData.tData;
    ...
    if ((tData.tState == TableState.waitEat || tData.tState == TableState.waitCard) && tData.lastPutCard2 && pl.mjState != TableState.roundFinish)    //开杠
    {
    ...
    if (pl.haiDiLaoState == 1 || MjClient.majiang.getAllCardsTotal() - tData.cardNext <= 0)
    {
        ...
        {
            if (pl.eatFlag & 8)
                MjClient.native.umengEvent4CountWithProperty("changSha_debug", {uid:SelfUid(), message: tData.tableid + "_" + tData.tState + "_" + pl.mjState});
            vnode = [];
            eat.hu._node.stopActionByTag(1);
            eat._node.stopActionByTag(1);
    ...
    if(vnode.length > 0)
    {
        var isShowCard = MjClient.size.width / MjClient.size.height > 1.68;
        for(var i = 0; i < vnode.length; i++)
        {
            ...
            if (btnName == "hu") {
                if (MjClient.huCards.length >= 1)
                    cards = MjClient.huCards;
                else if(IsTurnToMe())
                    cards = [pl.mjhand[pl.mjhand.length - 1]];
            }
            else if (btnName == "gang1") {
                if (vnode[i].cards && vnode[i].cards.length > 0)
                    cards = vnode[i].cards;
                else if (MjClient.kaiGangCards.length > 0)
                    cards = MjClient.kaiGangCards;
            }
            else if (btnName == "gang0") { 
                if (vnode[i].cards && vnode[i].cards.length > 0)
                    cards = vnode[i].cards;
                else if (MjClient.gangCards.length > 0)
                    cards = MjClient.gangCards;
            }
            else if (btnName == "peng") {
                if (MjClient.pengCards.length > 0)
                    cards = MjClient.pengCards;
            }
            else if (btnName == "chi0") {
                if (MjClient.chiCards.length >= 1)
                    cards = MjClient.chiCards;
            }
            else {
                cards = [];
            }
            ...
            for (var j = 0; j < 4; j ++) {
                var cardNode = vnode[i].getChildByName("card" + (j + 1));
                if (j > cards.length - 1 || !isShowCard) {
                    if (cardNode)
                ...
                if (!cardNode) {
                    if(!vnode[i].getChildByName("card" + j)) continue;
                    
                    cardNode = vnode[i].getChildByName("card" + j).clone();
                    cardNode.setName("card" + (j + 1));
                    vnode[i].addChild(cardNode);
                }
                
                cardNode.x = vnode[i].width + cardNode.width*cardNode.scaleX/2;
                cardNode.y = vnode[i].height/2 + j * cardNode.height*cardNode.scaleY;

                cardNode.visible = true;
                setCardSprite(cardNode, cards[j], 0);
            }
            ...
            var off = 0;
            if(isYongZhouProject() && vnode.length > 5)
            {
                off = 0.4
            }
            if (isShowCard) {
                setWgtLayout(vnode[i], [0, 0.16], [0.75, 0], [(i - vnode.length + 1) * (1.6 - off), 1.8], false, false);
            }
            else {
                setWgtLayout(vnode[i], [0, 0.16], [0.75, 0], [(i - vnode.length + 1) * (1.1 - off), 1.8], false, false);
            }
    ...
    showTingCarsSignChangSha();
}

PlayLayer_changSha.prototype.RemoveNewCardOut = function(node, card)
{
...
// @Override 牌背
PlayLayer_changSha.prototype.getCardBgList = function(is3D) {
    if(is3D){
...
PlayLayer_changSha.prototype.getMaJiangBgType = function(){
...
// 获得3D类型
PlayLayer_changSha.prototype.get3DType = function(){
...
PlayLayer_changSha.prototype.DealKaiGang = function(node, msg, off)
{
...
PlayLayer_changSha.prototype.getIsZhongBird = function(cd,birdArr){
    return MjClient.data.sData.tData.niaoCards.indexOf(cd) >= 0;
}

PlayLayer_changSha.prototype.playQiShouHuEffect = function(qshuName) {
    var soundName = null;
...
PlayLayer_changSha.prototype.playHuEffect = function(winType, huWords) {
    huWords = huWords || [];
...
PlayLayer_changSha.prototype.checkTipDuoHu = function(d) {
    var isDuoHu = false;
    for (var i = 0; i < MjClient.MaxPlayerNum; i++) {
        var pl = getUIPlayer(getOffForPlayerNum(i));
        if (!pl)
        ...
        if (d.uid != pl.info.uid && pl.eatFlag >= 8) {
            isDuoHu = true;
            break;
        }
    }
    ...
    if (isDuoHu) {
        var eatActionNode = getNode(0).getChildByName("play_tips");
...
function addTingCardsSignChangSha(children, tingCard) {
    for (var i = 0; i < children.length; i++) {
        if (children[i].name == "mjhand")
        {
...
function removeTingCardsSignChangSha(children) {
    cc.log("get in ---------");
    for (var i = 0; i < children.length; i++) {
        if (children[i].name == "mjhand") {
...
function getTingCardsChangSha(children, card) {
    var canTingCards = {};
    for (var i = 0; i < children.length; i++) {
        if (children[i].name == "mjhand") {
...
function showTingCarsSignChangSha() {
    if (!MjClient.data.sData.tData.areaSelectMode.isOpenTingTip)
        return;
