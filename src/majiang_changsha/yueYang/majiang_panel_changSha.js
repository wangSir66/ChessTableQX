...
(function () {
    majiang_panel_changSha = majiang_panel_yueyang.extend({
                        ...
                        _run: function() {
                            ...
                            this.schedule(function () {
                                ...
                                if (areaSelectMode.isErRen) {
                                    self.setString("二人缺门长麻");
                                } else {
                                    self.setString(GameCnName[MjClient.gameType]);
                                }
                            });
                        }
                    },
        ...
        *  海底捞出牌 */
        ...
        updatePlayerEatBtn: function(){
            ...
            var pos = this.isIPad() ? 0.85 : 0.85;
            var space = this.isIPad() ? 1.4 : 1.5;
            var off_y = this.isIPad() ? 1.7 : 2;
            for(var i = 0;i < eatNodeArr.length;i++){
                var btn = eatNodeArr[i];
                btn.visible = true;
                setWgtLayout(btn, [0, pct], [pos, 0], [(i - eatNodeArr.length + 1) * space*1.1, off_y], false, false);
            }
            ...
            for (var i = 0; i < 4; i++) {
                ...
                if (eatFlag & 8) {
                    ...
                    if (player.isTing && isKaiGangAfterSelfAnGang){ // 开杠后可以自摸则不显示【过】
                        mustHu = true;
                        // MjClient.playui.sendHuToServer(lastPutCard);
        ...
        eatMoreCards: function (node, cards) {
            ...
            for (var j = 0; j < 4; j++) {
                ...
                cardNode.x = node.width * 1 + cardNode.width * cardNode.scaleX / 2;
        ...
        checkKaiGangBtn: function (player) {
            this.kaiGangCardArray = [];
            var tData = MjClient.data.sData.tData;
            var menQing = tData.areaSelectMode.menQing; 
            var menQing_no258 = tData.areaSelectMode.menQing_no258;
            ...
            cc.log("==== this.kaiGangCardArray ",this.kaiGangCardArray)
        ...
        resetPutCards:function(node){
            ...
            if(!player) return;
            cc.log("===== node.getName()  player.mjput",node.getName(),player.mjput)
...
//码牌动画
majiang_panel_changSha.prototype.playShuffleEffect = function(){
    ...
    var shuffleNode = new majiang_mapai_Layer(MjClient.majiang.getAllCardsTotal(tData), sData.players[uid], function(){
