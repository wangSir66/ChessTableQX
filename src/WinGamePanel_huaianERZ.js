...
function SetEndOneUserUI_huaianERZ(node,off)
{
    ...
    node.getChildByName("huaX").visible = true;
    node.getChildByName("huaIcon").visible = true;

    var huaCount = node.getChildByName("huaCount");
    huaCount.zIndex = 11;
    huaCount.visible = true;
	...
	var uibind= {
		head: {
			...
			up: {
				...
				, _run: function () {
					...
					// 添加花牌 灌南最多只有4张花牌 
					// for (var i = 0; i < pl.mjflower.length && i < 4; i++) {

					// 	arry.push(getNewCard(node, "up", "flower", pl.mjflower[i], 0));
					// }
     ...
     //                    if(arry[i].name == 'flower'){
     //                    	var size = arry[i].getSize();
     //                    	arry[i].setScale(arry[i].getScale()*0.8);
     //                    	var scale = arry[i].getScale();
     //                    	var width = size.width*scale*0.2;
     //                    	var height = size.height*scale*0.3;
     //                    	var row = flowerCount % 2;
     //                    	var low =  Math.floor( flowerCount / 2 );
     //                    	arry[i].x = arry[i].x - 5.8 * width +  4.4 * row * width;
     //                    	arry[i].y = arry[i].y + 2.2 * height - 2.8 * low * height;
     //                    	flowerCount++;
     //                    }
					// }
...
var EndOneView_huaianERZ = cc.Layer.extend({
	jsBind:{
		block: {
		...
		head0:{
		   ...
		   _run:function(){ SetEndOneUserUI_huaianERZ(this,0); },
		...
		,head1:{ 
		  ...
		  _run:function(){ SetEndOneUserUI_huaianERZ(this,1); }
		}
		...
		,head2:{ 
		   ...
		   _run:function(){ SetEndOneUserUI_huaianERZ(this,2); }
		}
		,head3:{
			 head:
			 {
			...
			_run:function(){ SetEndOneUserUI_huaianERZ(this,3); }
			},
            count_down:{
                _visible :function()
                {
    ...
    ctor:function () {
        this._super();
        var endoneui = ccs.load(res.EndOne_huaianERZ_json);
