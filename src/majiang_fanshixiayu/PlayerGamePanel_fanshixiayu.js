...
//向服务器发送 过消息
MjClient.MJPass2NetForfanshixiayu = function()
{
   // console.log(">>>>>>>>>普通  过 <<<<<<<<");
    cc.log("====================MJPass2NetForjinzhong======pass=====");
...
function SetUserVisible_fanshixiayu(node, off)
{
    //var sData = MjClient.data.sData;
    //return;
    ...
    if(pl)
    {
        ...
        InitUserHandUI_fanshixiayu(node, off);
...
function DealMJPut_fanshixiayu(node, msg, off, outNum)
{
    DealMJPut_shanXiApp(node, msg, off, outNum);
}

function InitUserHandUI_fanshixiayu(node, off)
{
    var sData = MjClient.data.sData;
    var tData = sData.tData;
    var pl = getUIPlayer(off);
    ...
    for(var i = 0; i < pl.mjput.length; i++)
    {
        ...
        DealMJPut_fanshixiayu(node, msg, off, i);
    }
...
function initFlower_fanshixiayu() {
    initFlower(false, false);
}


var PlayLayer_fanshixiayu = cc.Layer.extend({
    jsBind: {
        _event: {
            mjhand: function() {
            ...
            roundEnd: function() {
                ...
                function delayExe()
                {
                    ...
                    self.addChild(new EndOneView_fanshixiayu(),500);
                }
        ...
        eat: {
            ...
            guo: {
                ...
                _touch: function(btn, eT) {
                    if (eT == 2)
                    {
                        MjClient.MJPass2NetForfanshixiayu();
                    }
                },
                bgimg: {
                    _run: function() {
            ...
            changeui: {
                ...
                changeuibg: {
                    ...
                    guobg: {
                        guo: {
                            _touch: function(btn, eT) {
                                if (eT == 2) MjClient.MJPass2NetForfanshixiayu();
                            }
                        },
                        fanhui: {
                            _touch: function(btn, et) {
    ...
    ctor: function() {
        this._super();
        var playui = ccs.load("Play_xinzhoufanshi.json");
...
PlayLayer_fanshixiayu.prototype.CardLayoutRestore = function(node, off)
{
...
// 判断吃碰杠胡的状态
PlayLayer_fanshixiayu.prototype.EatVisibleCheck = function()
{
    var eat = MjClient.playui.jsBind.eat;
    ...
    // 如果碰杠花色已经有两种，第三种碰杠时需要提示：手牌超过2个花色，不能胡牌！
    var newCard = pl.newCd ? [pl.newCd] : [];
    var lack = calLack(newCard.concat(pl.mjpeng).concat(pl.mjgang0).concat(pl.mjgang1));
    ...
    if(tData.tState == TableState.waitPut && pl.mjState == TableState.waitPut)
    {
        if(IsTurnToMe())
        {
            ...
            //杠
            cc.log("=== pl.newSendCard :  " + newCard);
            cc.log("=== lack :  " + lack);
            cc.log("=== pl.mjgang0 :  " + pl.mjgang0);
            cc.log("=== pl.mjgang1 :  " + pl.mjgang1);
            cc.log("=== pl.mjpeng :  " + pl.mjpeng);
            cc.log("=== pl.mjhand :  " + pl.mjhand);
            cc.log("=== pl.isTing :  " + pl.isTing);
            cc.log("=== tData.huangNum :  " + tData.huangNum);
            cc.log("=== tData.cardNext :  " + tData.cardNext);
            var leftCard = MjClient.majiang.getAllCardsTotal() - tData.huangNum - tData.cardNext;
            var rtn = leftCard > 0 && MjClient.majiang.canGang1(pl.mjpeng, pl.mjhand, pl.isTing);
            cc.log("$$$$杠牌监测"+JSON.stringify(rtn));
            if(rtn.length > 0 && pl.isNew)
            {
                var idx = 0;
                while (idx < rtn.length) {
                    if (calLack([rtn[idx]].concat(pl.mjpeng).concat(pl.mjgang0).concat(pl.mjgang1)) == 0) {
                        rtn.splice(idx, 1);
                    }
                    else {
                        idx++;
                ...
                if (rtn.length > 0) {
                    ...
                    if (!MjClient.GameTipMessageNode)
                        MjClient.showToastbyPos("碰杠超过2个花色，不能杠！", 0.3);
                }
            }
            if(vnode.length > 0)
            {
    ...
    else if(tData.tState == TableState.waitEat)
    {
        ...
        if(!IsTurnToMe())
        {
            ...
            if (pl.eatFlag & 4) {
                if(lack && lack < 1) MjClient.showToastbyPos("手牌超过2个花色，不能胡牌！", 0.3);
                vnode.push(eat.gang0._node);
...
/*
    玩家ui，信息
 */
PlayLayer_fanshixiayu.prototype.Node_player = function(off)
{
    ...
    UIEventBind(null, _node, "initSceneData", function (eD) {
        var tData = MjClient.data.sData.tData;
        SetUserVisible_fanshixiayu(_node, off);
        ...
        // 准备出牌的人，没有出过牌的人，初始化先清除听提示
        if(off == 0 && pl && pl.mjhand && pl.putCount && (pl.mjhand.length % 3 == 2 || pl.putCount == 0)) {
        ...
        {
            _skipHuIconTag && _skipHuIconTag.setVisible(false);
            _skipPengIconTag && _skipPengIconTag.setVisible(false);
            return;
        }
    });
    ...
    UIEventBind(null, _node, "mjhand", function (eD) {
        InitUserHandUI_fanshixiayu(_node, off);
    });
    ...
    UIEventBind(null, _node, "MJPut", function (eD) {
        ...
        var pl = getUIPlayer(off);
        if (pl && eD.uid == pl.info.uid && off == 0 && pl.isTing)
        {
        ...
        DealMJPut_fanshixiayu(_node,eD,off);
        hideTingBtn();
        if(_tingCardNumNode) _tingCardNumNode.visible = false;
