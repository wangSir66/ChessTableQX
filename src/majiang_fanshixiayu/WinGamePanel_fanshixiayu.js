...
function CardLayoutRestoreForEndOne_fanshixiayu(node, endonepl)
{
...
function SetEndOneUserUI_fanshixiayu(node,off)
{
	var sData=MjClient.data.sData;
	var tData=sData.tData;
    ...
    else if (tData.areaSelectMode.baoting) //非报听模式下，小结算界面不要显示叫听张，以及叫听状态
    {
...
var EndOneView_fanshixiayu = cc.Layer.extend({
	jsBind:{
		block: {
		...
		btnShowHeap:{
                ...
                _click:function()
                {
                	if(MjClient.endoneui){
                        var fanshixiayuCardsHeap = new PopupShowCardsHeap();
                        fanshixiayuCardsHeap.setName("fanshixiayuCardsHeap");
                        MjClient.endoneui.addChild(fanshixiayuCardsHeap);
                    }
                },
            },
		dir: {
		...
		head0:{
		   ...
		   _run:function(){ SetEndOneUserUI_fanshixiayu(this,0); },
		...
		head1:{
		  ...
		  _run:function(){ SetEndOneUserUI_fanshixiayu(this,1); }
		},
		head2:{
		   head:{
			   zhuang:{_visible:false}
		   ...
		   _run:function(){ SetEndOneUserUI_fanshixiayu(this,2); }
		},
		head3:{
			 head:
			 {
			...
			_run:function(){ SetEndOneUserUI_fanshixiayu(this,3); }
			},
		count_down:{
                _visible :function()
                {
    ...
    ctor:function () {
        this._super();
        var endoneui = ccs.load("endOne_xinzhoufanshi.json");
