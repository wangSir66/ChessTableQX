...
(function() {
    ...
    majiang.calCardBaseScore = function(tData, card, type) {
        if (tData.hunCard2 in laojiangPoint) { // 翻两个大将
        ...
        else if (tData.hunCard in laojiangPoint && !this.isCardFlower(tData.hunCard2)) { // 翻一个大将、一个普通牌
        ...
        else if (!this.isCardFlower(tData.hunCard) && !this.isCardFlower(tData.hunCard2)) {
            if (tData.hunCard == tData.hunCard2) { // 翻在同一张普通牌上
                if (card == tData.hunCard) {
            ...
            else if (tData.hunCard % 10 == tData.hunCard2 % 10) { // 翻将在不同的普通牌上、但是点数一样
                if (card == tData.hunCard || card == tData.hunCard2) {
                    return [16, 48, 0, 48, 64][type];
                }
                if (card in laojiangPoint) {
                ...
                if (card % 10 == tData.hunCard % 10 || card % 10 == tData.hunCard2 % 10) {
                    return [4, 8, 16, 24, 32][type];
                }
            }
        }
        ...
        else if (tData.hunCard in laojiangPoint && this.isCardFlower(tData.hunCard2)) { // 翻将一张喜，一张大将
            if (card == tData.hunCard) {
                return [32, 96, 0, 96, 128][type];
            }
            if (card in laojiangPoint) {
        ...
        else if (this.isCardFlower(tData.hunCard) || this.isCardFlower(tData.hunCard2)) { // 翻将一张喜，一张普通牌
            var jiang = this.isCardFlower(tData.hunCard) ? tData.hunCard2 : tData.hunCard;
            if (card == jiang) {
                return [8, 24, 0, 24, 32][type];
            }
            if (card in laojiangPoint) {
    ...
    {
        MjClient.majiang_rugao_SJ = majiang;
    }
    else
    {
