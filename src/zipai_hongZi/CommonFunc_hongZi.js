...
// 耒阳字牌全局方法
MjClient.MaxPlayerNum_hongZi = 3;
MjClient.cardPath_hongZi = "playing/paohuzi/";
var ActionType_hongZi =  //图片提示
    {
        CHI:1,
        PENG:2,
        WEI:3,
...
//初始化玩家金币、名字、胡息
function InitUserCoinAndName_hongZi(node, off){
...
//向服务器发送王闯王钓操作
function HZWangChuangToServer_hongZi(type){
    cc.log("====================HZWangChuangToServer_hongZi=================");
...
//向服务器发送打牌操作
function HZPutCardToServer_hongZi(card){
    cc.log("====================HZPutCardToServer_hongZi=================");
    MjClient.gamenet.request("pkroom.handler.tableMsg", {
        cmd: "MJPut",
        card: card
    });
...
// 向服务器发送吃牌
function HZChiToServer_hongZi(eatCards, biCards){
...
//像服务器发送碰牌
function HZPengToServer_hongZi()
{
    if (MjClient.rePlayVideo != -1) return; // 回放时候不能请求
    cc.log("====================HZPengToServer_hongZi=================");
    MjClient.gamenet.request("pkroom.handler.tableMsg", {
        cmd: "MJPeng"
    });
}
...
// 向服务器发送过牌
function HZPassConfirmToServer_hongZi() {
    if (MjClient.rePlayVideo != -1)  {
        return; // 回放时候不能请求
    }
...
//发送跑牌命令
function HZGangToServer_hongZi(type){
    if (MjClient.rePlayVideo != -1) return; // 回放时候不能请求
    cc.log("====================HZGangToServer_hongZi=================");
    MjClient.gamenet.request("pkroom.handler.tableMsg", {
        cmd: "MJGang",
        type: type,
        eatFlag: EatFlag_hongZi()
...
//发送偎牌命令
function HZWeiToServer_hongZi(type){
    if (MjClient.rePlayVideo != -1) return; // 回放时候不能请求
    cc.log("====================HZWeiToServer_hongZi=================");
    MjClient.gamenet.request("pkroom.handler.tableMsg", {
        cmd: "HZWeiCard"
    });
}
...
//向服务器发送发牌命令
function HZNewCardToServer_hongZi(){
    if (MjClient.rePlayVideo != -1) return; // 回放时候不能请求
    cc.log("====================HZNewCardToServer_hongZi=================");
    MjClient.gamenet.request("pkroom.handler.tableMsg", {
        cmd: "HZNewCard"
    });
}
...
//添加吃的牌
function addEatCard_hongZi(node,name,mjNum,off){
    //根据牌的类型获得需要添加的节点
    ...
    var type = 2;
    //设置牌
    var newCard = getNewCard_hongZi(mjNum,type,off);
...
/**
 添加手牌(回放)
 */
function addHandCardReplay_hongZi(tagName,index,mjNum,off){
    var _node = getNode_hongZi(off);
    //根据牌的类型获得需要添加的节点
 ...
 */
function addHandCard_hongZi(tagName,index,mjNum,off){
    if(off != 0){
        return;
    }
    var _node = getNode_hongZi(off);
    ...
    //设置牌
    var newCard = getNewCard_hongZi(mjNum,1,off);
    var scale_y = newCard.scaleY;
    //var parentCount = addNode.childrenCount;
    //首先根据name判断cpNode中是否已经添加
    var cardParent = addNode.getChildByTag(tagName);
    if(!cardParent){
        ...
        addNode.addChild(cardParent);
        // SetTouchCardHandler_hongZi(cardParent);
    }
 ...
 **/
function getNewCard_hongZi(mjNum, type, off, isTurn){
    var _node = getNode_hongZi(off);
    var copyNode = null;
    if(type == 1){
        //手牌
    ...
    //cc.log("getNewCard_hongZi:"+mjNum+","+type+","+off+","+isTurn);
    if(mjNum > 0){
        //创建一个带有麻将信息的麻将 cp为创建后的麻将
        setCardSprite_hongZi(mjNode, mjNum, type, isTurn);
        if(type == 1){
            //只有手牌才会有点击事件
            SetTouchCardHandler_hongZi(mjNode,off);
        }
    }
...
/**
 ...
 type:类型，1:手牌 2：吃牌 3：打的牌
 */
function setCardSprite_hongZi(mjNode, mjNum, type, isTurn){
    if(getCurrentMJBgType_HongZi() == 0){
        MjClient.cardPath_hongZi = "playing/paohuzi/";
    }else if(getCurrentMJBgType_HongZi() == 1){
        MjClient.cardPath_hongZi = "playing/paohuzi/MJBg1/";
    }else if(getCurrentMJBgType_HongZi() == 2 && MjClient.getAppType() != MjClient.APP_TYPE.QXLYQP && MjClient.getAppType() != MjClient.APP_TYPE.QXXXGHZ){
        MjClient.cardPath_hongZi = "playing/paohuzi/MJBg2/";
    }else {
        MjClient.cardPath_hongZi = "playing/paohuzi/";
    }
    if(type == 1){
        // cc.log("full path:" + (MjClient.cardPath_hongZi+"hand" + mjNum + ".png"));
        mjNode.loadTexture(MjClient.cardPath_hongZi+"hand" + mjNum + ".png");
    }
    if(type == 2){
        mjNode.loadTexture(MjClient.cardPath_hongZi+"out" + mjNum + ".png");
        if(isTurn){
            mjNode.loadTexture(MjClient.cardPath_hongZi+"huxiBG.png");
        }
    }
    if(type == 3 || type == 4){
        mjNode.loadTexture(MjClient.cardPath_hongZi+"put" + mjNum + ".png");
        if(isTurn){
            mjNode.loadTexture(MjClient.cardPath_hongZi+"normalBG.png");
        }
    }
    mjNode.tag = mjNum;
}
...
// 新的听牌
function newCheckTingCards_hongZi(putCard){
    // return;
    // if(MjClient.getAppType() == MjClient.APP_TYPE.QXSYDTZ || MjClient.getAppType() == MjClient.APP_TYPE.BDHYZP){
        //听牌检测
        var sData = MjClient.data.sData;

        if(MjClient.data.sData.tData.tState == TableState.waitPut && curPlayerIsMe_hongZi(0) && putCard === undefined){
            cc.log("is waitPut....");
             postEvent("showTingLayer", []);
            return;
        }

        if(putCard && (!curPlayerIsMe_hongZi(0) || sData.tData.tState != TableState.waitPut)){
            putCard = undefined;
        }
        ...
        // var cards = MjClient.majiang_hongZi.getTingCards(sData, pl, putCard);
        var textData = MjClient.majiang_hongZi.getTingStats(sData, pl, putCard);

        // cc.log("==========checkTingCards_hongZi===============");
...
function checkTingCards_hongZi(putCard){
    // return;
    // if(MjClient.getAppType() == MjClient.APP_TYPE.QXSYDTZ || MjClient.getAppType() == MjClient.APP_TYPE.BDHYZP){
        //听牌检测
        var sData = MjClient.data.sData;
        ...
        if(pl.mjhand)
        {
            cards = MjClient.majiang_hongZi.getTingCards(sData, pl, putCard);
        }
        // var textData = MjClient.majiang_hongZi.getTingStats(sData, pl, putCard);
...
function getPut2TingStats_hongZi() {
    return;

    var sData = MjClient.data.sData;
    var tData = sData.tData;

    if (tData.tState != TableState.waitPut || !curPlayerIsMe_hongZi(0)){
        return;
    }

    var time1 = +new Date();
    var pl = sData.players[SelfUid()];
    var info = MjClient.majiang_hongZi.put2TingStats(sData, pl);
    var time2 = +new Date();
    cc.log("getPut2TingStats_hongZi time@@ ", time2 - time1);
    console.log("put2TingStats@@ ", JSON.stringify(info));
}

//刷新手牌移动时的坐标
var btn_oPos = null;
function updateBtnMovedPosition_hongZi(btn, eventType){
...
function SetTouchCardHandler_hongZi(card,off){
    ...
    card.addTouchEventListener(function(btn,eventType){
        ...
        //禁止同时移动多个牌
        if(MjClient.movingCard_hongZi !== null && cc.sys.isObjectValid(MjClient.movingCard_hongZi) && MjClient.movingCard_hongZi !== btn){
            return;
        }
        ...
        if(eventType == ccui.Widget.TOUCH_BEGAN){
            ...
            MjClient.movingCard_hongZi = btn;
            //
            ...
            setCardSprite_hongZi(cloneCard, cardTag, 1, false);
            cloneCard.opacity = 0;
            ...
            btn.x = touchPos.x;
            btn.y = touchPos.y;
            // updateBtnMovedPosition_hongZi(btn, eventType);
            ShowPutCardIcon_hongZi();

            newCheckTingCards_hongZi(btn.tag);

            return true;
        }
        if(eventType == ccui.Widget.TOUCH_MOVED){
            if (MjClient.movingCard_hongZi==null) return;
        ...
        if(eventType == ccui.Widget.TOUCH_ENDED || 
                ...
                !MjClient.isCommon && cutLine.visible && endPos.y >= cutLine.y && !MjClient.hasPut)
            {
                ...
                var putCard = getNewCard_hongZi(btnTag, 3 ,off);
                ...
                HZPutCardToServer_hongZi(btnTag);
                MjClient.playui.ResetHandCard(MjClient.playui._downNode,off);
                ShowPutCardIcon_hongZi();
            }
            else
            {
                ...
                if(!isNoPut && !isWangBa && IsTurnToMe() && tData.tState == TableState.waitPut && pl.mjState == TableState.waitPut)
                {
                    if(!MjClient.isCommon && cutLine.visible && endPos.y >= cutLine.y && !MjClient.hasPut)
                    {
                        ...
                        newCheckTingCards_hongZi(btnTag);
                ...
                if(!isPut || isWangBa || isNoPut)
                {
                    ...
                    if(selectParentTag == btnParent.tag){
                        ...
                        _changeArrIndex_hongZi(selectArr, btnTag, btn);
                    }
                        ...
                        if(selectArr.length >= cardNum){
                            ...
                            //todo 千千
                            _fixArrIndex_hongZi(selectArr, btnTag, btn);
                            MjClient.HandCardArr[btnParent.tag].splice(parseInt(btnName),1);
                        }
                    }
                    else{
        ...
        //     if (movingCard_hongZi==null) return;
        //     movingCard_hongZi = null;
        //     MjClient.playui.ResetHandCard(MjClient.playui._downNode,off);
        //     ShowPutCardIcon_hongZi();
        // }
    });
...
//根据坐标Y，添加到数组对应的位置
function _fixArrIndex_hongZi(arr, btnTag, btn){
    if(_isKan_hongZi(arr)){
        arr.push(btnTag);
    }else{
...
//改变数组元素的位置
function _changeArrIndex_hongZi(arr, btnTag, btn){
    if(!_isKan_hongZi(arr)){
        var len = arr.length;
        for(var i = 0; i < len; i++){
            var tag = arr[i];
        ...
        _fixArrIndex_hongZi(arr, btnTag, btn);
    }
}

//数组里的是否为三个相同的牌
function _isKan_hongZi(arr){
    var len = arr.length;
    if(len < 3){
        return false;
    }
...
//断线重连之后，显示之前玩家打的牌
function DealOffLineCard_hongZi(node, off){
    if(off == 1 && node.getName() == "right" && MjClient.MaxPlayerNum_hongZi == 2){
        return;//2人玩法right节点同为玩家节点
    }
    cc.log("====================DealOffLineCard_hongZi=================");
    var sData = MjClient.data.sData;
    var tData = sData.tData;
    ...
    if(typeof(tData.currCard) != "undefined" && tData.currCard != -1){
        var uids = tData.uids;
        var selfIndex = (uids.indexOf(SelfUid()) + off) % MjClient.MaxPlayerNum_hongZi;
        if(uids[selfIndex] == uids[tData.curPlayer]){
            var putCard = getNewCard_hongZi(tData.currCard, 3, off);
...
//发牌
MjClient.isCommon = false;
function DealNewCard_hongZi(node, msg, off){
    cc.log("======DealNewCard_hongZi======= " + off);
    if(off == 1 && node.getName() == "right" && MjClient.MaxPlayerNum_hongZi == 2){
        return;//2人玩法right节点同为玩家节点
    }
    //没有发牌过来的时候直接忽略
    if(!msg.newCard){
        RemovePutCardOut_hongZi();
    ...
    if(uids[selfIndex] == msg.uid){
        ...
        if(lastPlayer != -1){
            //var lastOff = (off + MjClient.MaxPlayerNum_hongZi - selfIndex) % MjClient.MaxPlayerNum_hongZi;
            RemovePutCardOut_hongZi();
        }
        ...
        var putCard = getNewCard_hongZi(msg.newCard, 3, off, isTurn);
        var putNode = node.getChildByName("put");
        putNode.removeAllChildren();
        putCard.scaleX = putCard.width/putNode.width - 0.16;
        putCard.scaleY = putCard.width/putNode.width - 0.06;
        ...
        if(!msg.isCommon){
            if(MjClient.rePlayVideo == -1){
                ...
                putNode.runAction(cc.sequence(
                    cc.delayTime(0.2),
                    cc.callFunc(function(){RemovePutCardOut_hongZi();})
                ));
                for(var i = 0;i < cardArr.length;i++){
                    var tmpArr = cardArr[i];
                    if(tmpArr.length <= 2){
...
//翻醒发牌
function DealFanXingNewCard_hongZi(card){
    if(off == 1 && node.getName() == "right" && MjClient.MaxPlayerNum_hongZi == 2){
        return;//2人玩法right节点同为玩家节点
    }
    var putCard = getNewCard_hongZi(card, 4, 0);
...
//王闯/王钓
function DealWangChuang_hongZi(node,msg,off){
    if(off == 1 && node.getName() == "right" && MjClient.MaxPlayerNum_hongZi == 2){
        return;//2人玩法right节点同为玩家节点
    }
    var sData = MjClient.data.sData;
    ...
    if(uids[selfIndex] == msg.uid){
        ...
        if(pl.wangType == 1){
            var putCard = getNewCard_hongZi(91, 3, off);
            var putNode = node.getChildByName("put");
            putCard.scaleX = putCard.width/putNode.width - 0.16;
            putCard.scaleY = putCard.width/putNode.width - 0.06;
            putCard.x = putCard.width/2;
            ...
            ShowEatActionAnim_hongZi(node, ActionType_hongZi.WANGDIAO, off);

            var delay = cc.delayTime(1.5);
            var callback = function(){
                RemovePutCardOut_hongZi();
                //HZNewCardToServer_hongZi();
            }
            //putNode.runAction(cc.sequence(delay,cc.callFunc(callback)));
        }else if(pl.wangType == 2){
            var putCard = getNewCard_hongZi(91, 3, off);
            var putCard1 = getNewCard_hongZi(91, 3, off);
            var putNode = node.getChildByName("put");
            putCard.scaleX = putCard.width/putNode.width - 0.16;
            putCard.scaleY = putCard.width/putNode.width - 0.06;
            ...
            ShowEatActionAnim_hongZi(node, ActionType_hongZi.WANGCHUANG, off);
            var delay = cc.delayTime(1.5);
            var callback = function(){
                RemovePutCardOut_hongZi();
                //HZNewCardToServer_hongZi();
            ...
            var putCard = getNewCard_hongZi(91, 3, off);
            var putCard1 = getNewCard_hongZi(91, 3, off);
            var putCard2 = getNewCard_hongZi(91, 3, off);
            var putNode = node.getChildByName("put");
            putCard.scaleX = putCard.width/putNode.width - 0.16;
            putCard.scaleY = putCard.width/putNode.width - 0.06;
            putCard1.scaleX = putCard.width/putNode.width - 0.16;
            ...
            ShowEatActionAnim_hongZi(node, ActionType_hongZi.WANGZHA, off);
            var delay = cc.delayTime(1.5);
            var callback = function(){
                RemovePutCardOut_hongZi();
                //HZNewCardToServer_hongZi();
...
//处理出牌
function DealPutCard_hongZi(node, msg, off){
    cc.log("======DealPutCard_hongZi======= " + off);
    ...
    if(uids[selfIndex] == msg.uid)
    {
        ...
        if (SelfUid() != msg.uid || MjClient.rePlayVideo != -1)
        {
            var putNode = node.getChildByName("put");
            putNode.removeAllChildren();
            var putCard = getNewCard_hongZi(msg.card, 3 ,off);
            putCard.scaleX = putCard.width/putNode.width - 0.16;
            putCard.scaleY = putCard.width/putNode.width - 0.06;
            putCard.x = putCard.width/2;
            putCard.y = putCard.height/2;
        ...
        if (off == 0) {
            removeTingSign_hongZi(node);
        }

        if(MjClient.rePlayVideo != -1){
            RemoveHandCard_hongZi(node,msg.card,off);
            MjClient.playui.ResetHandCard(node,off);
        }
    }
}

function checkCard_hongZi(node, off){
    ...
    if(cardHandArr.length > cardHand.length){
        ...
        for(var i = 0;i < cardHandArr.length; i++){
            if(cardHandArr[i] != cardHand[i]){
                RemoveHandCard_hongZi(node, cardHandArr[i], off);
...
function calculateHintPutList_hongZi() {
    MjClient.hintPutList_ziPai = MjClient.majiang_hongZi.hintPutCardsToTing();
    // cc.log('MjClient.hintPutList_ziPai@@ ', MjClient.hintPutList_ziPai);
}

function addTingSign_hongZi(node) {
...
function removeTingSign_hongZi(node) {
    var handNode = node.getChildByName("handNode");
    if (!handNode) {
        return;
    }
...
//吃牌
function DealChiCard_hongZi(node, msg, off) {
    if(off == 1 && node.getName() == "right" && MjClient.MaxPlayerNum_hongZi == 2){
        return;//2人玩法right节点同为玩家节点
    }
    cc.log("======DealChiCard_hongZi======= " + off);
    var sData = MjClient.data.sData;
    var tData = sData.tData;
    var uids = tData.uids;
    var selfIndex = (uids.indexOf(SelfUid()) + off) % MjClient.MaxPlayerNum_hongZi;
    if (tData.curPlayer == selfIndex) {
        ...
        for (var i = 0; i < tmpEatCards.length; i++) {
            RemoveHandCard_hongZi(node, tmpEatCards[i], off);
        }
            ...
            for (var k = 0; k < biCards.length; k++) {
                ...
                for (var m = 0; m < biArr.length; m++) {
                    if (tmpBiArr.indexOf(biArr[m]) >= 0) {
                        ...
                        RemoveHandCard_hongZi(node, biArr[m], off);
                    }
                }
            }
        }
        ...
        for (var i = 0; i < eatCards.length; i++) {
            var card = getNewCard_hongZi(eatCards[i], 2, off);
            if (i == 2) {
            ...
            card.zIndex = 4 - i;
            card.x = 0;
            card.y = off_count * getOffY_hongZi(node, card);
            setChiCardAnchorPoint_hongZi(node, card);
        ...
        if (biCards) {
            ...
            for (var k = 0; k < biCards.length; k++) {
                var biArr = biCards[k];
                for (var m = 0; m < biArr.length; m++) {
                    var card = getNewCard_hongZi(biArr[m], 2, off);
                    if (m == 2) {
                        card.setColor(cc.color(170, 170, 170));
                    } 
                    setChiCardAnchorPoint_hongZi(node, card);
        ...
        var parentPos = getChiCardParentPosition_hongZi(node, cardWidth, off, selfColCount);
        var moveX0 = 0;
        ...
        parent.x = moveX0;
        setCardParentPosY_hongZi(node, parent);
        ...
        if (biCards) {
            ShowEatActionAnim_hongZi(node, ActionType_hongZi.XIAHUO, off);
        } else {
            ShowEatActionAnim_hongZi(node, ActionType_hongZi.CHI, off);
        }
    }
    //MjClient.playui.CardLayoutRestore(node,off);
}

function setChiCardAnchorPoint_hongZi(node,newCard) {
    if(node.getName() == "down"){
        newCard.anchorX = 0;
        newCard.anchorY = 0;
    }else if(node.getName() == "right"){
...
function getOffY_hongZi(node,newCard) {
    var off_y = 0;
    if(node.getName() == "down"){
        off_y = newCard.height;
    }else if(node.getName() == "right"){
...
function getChiCardParentPosition_hongZi(node, newCardWidth, off, selfColCount, orignColPosX) {
...
function setCardParentPosY_hongZi(node, cardParent) {
    var eatNode = node.getChildByName("eatNode");
    if(node.getName() == "down"){
        cardParent.y = 0;
    }else if (node.getName() == "right"){
...
function getShowCardIdx_hongZi(node, chiTip) {
    var idx;
    switch(node.getName()) {
        case "down":
        case "xing":
...
// 处理碰
function DealPengCard_hongZi(node, msg, off){
    cc.log("======DealChiCard_hongZi======= ");
    if(off == 1 && node.getName() == "right" && MjClient.MaxPlayerNum_hongZi == 2){
        return;//2人玩法right节点同为玩家节点
    }
    var sData = MjClient.data.sData;
    ...
    if(tData.curPlayer == selfIndex){
        ...
        for(var i = 0;i < 3; i++) {
            var card = getNewCard_hongZi(pengCard,2,off);
            cardWidth = card.width//*eatNode.scaleX;
            var childCount = parent.childrenCount;
            card.zIndex = 4 - childCount;
            card.x = 0;
            card.y = childCount * getOffY_hongZi(node, card);
            setChiCardAnchorPoint_hongZi(node, card);
            parent.addChild(card);
        }
        eatNode.addChild(parent);

        RemoveHandCard_hongZi(node,pengCard,off);
        RemoveHandCard_hongZi(node,pengCard,off);
        MjClient.playui.ResetHandCard(node, off);
        ShowEatActionAnim_hongZi(node, ActionType_hongZi.PENG, off);
        ...
        var parentPos = getChiCardParentPosition_hongZi(node, cardWidth, off, 1);
        var moveX0 = 0;
        var outNodeCount = eatNode.getChildrenCount();
        if(outNodeCount>0){
            if(node.getName() == "down" || node.getName() == "left"){
...
//偎牌
function DealWeiCard_hongZi(node, msg, off) {
    cc.log("======DealWeiCard_hongZi======= ");
    if(off == 1 && node.getName() == "right" && MjClient.MaxPlayerNum_hongZi == 2){
        return;//2人玩法right节点同为玩家节点
    }
    var sData = MjClient.data.sData;
    ...
    if (tData.curPlayer == selfIndex) {
        RemovePutCardOut_hongZi();
        ShowEatActionAnim_hongZi(node, ActionType_hongZi.WEI, off);
        ...
        for (var i = 0; i < 3; i++) {
            ...
            {
                if (i == getShowCardIdx_hongZi(node, "wei")) {
                    isTurn = false;
                }
                var card = getNewCard_hongZi(weiCard, 2, off, isTurn);
            } else if (MjClient.majiang.getCardShowType(tData.uids[tData.curPlayer], weiCard) == 1) {
                var card = getNewCard_hongZi(weiCard, 2, off);
                ...
                var card = getNewCard_hongZi(weiCard, 2, off, true);
            }
            cardWidth = card.width //*eatNode.scaleX;
            card.zIndex = 4 - i;
            card.x = 0;
            card.y = i * getOffY_hongZi(node, card);
            setChiCardAnchorPoint_hongZi(node, card);
            parent.addChild(card);
        }
        eatNode.addChild(parent);

        RemoveHandCard_hongZi(node, weiCard, off);
        RemoveHandCard_hongZi(node, weiCard, off);
        MjClient.playui.ResetHandCard(node, off);
...
// 跑牌或者提牌
function DealGangCard_hongZi(node, msg, off) {
    if(off == 1 && node.getName() == "right" && MjClient.MaxPlayerNum_hongZi == 2){
        return;//2人玩法right节点同为玩家节点
    }
    var sData = MjClient.data.sData;
    ...
    if (uids.indexOf(gangUid) == selfIndex) {
        if (msg.type == 2) {
            RemovePutCardOut_hongZi();
            ShowEatActionAnim_hongZi(node, ActionType_hongZi.PAO, off);
        } else {
            if (!msg.isGangHand && msg.type == 1) {
                RemovePutCardOut_hongZi();
            }
            ShowEatActionAnim_hongZi(node, ActionType_hongZi.TI, off);
        }
        ...
        for (var i = 0; i < 4; i++) {
            ...
            if (msg.type == 2) {
                isTurn = false;
                var card = getNewCard_hongZi(gangCard, 2, off, isTurn);
            ...
            {
                if (i == getShowCardIdx_hongZi(node, "ti")) {
                    isTurn = false;
                }
                var card = getNewCard_hongZi(gangCard, 2, off, isTurn);
            } else if (MjClient.majiang.getCardShowType(tData.uids[tData.curPlayer], gangCard) == 1) {
                var card = getNewCard_hongZi(gangCard, 2, off, false);
                var shade = new cc.Sprite("playing/paohuzi/huxiBG1.png");
                shade.opacity = 100;
                shade.x = shade.width / 2;
                shade.y = shade.height / 2;
                ...
                var card = getNewCard_hongZi(gangCard, 2, off, true);
            }
            cardWidth = card.width //*eatNode.scaleX;
            card.zIndex = 4 - i;
            card.x = 0;
        ...
        RemoveHandCard_hongZi(node, gangCard, off);
        RemoveHandCard_hongZi(node, gangCard, off);
        RemoveHandCard_hongZi(node, gangCard, off);
        if (msg.isGangHand) {
            RemoveHandCard_hongZi(node, gangCard, off);
        ...
        var parentPos = getChiCardParentPosition_hongZi(node, cardWidth, off, 1, orignColPosX);
        var moveX0 = 0;
        var outNodeCount = eatNode.getChildrenCount();
        if (outNodeCount > 0) {
            if (node.getName() == "down" || node.getName() == "left") {
...
// 处理胡
function DealHu_hongZi(node, msg, off){
    if(off == 1 && node.getName() == "right" && MjClient.MaxPlayerNum_hongZi == 2){
        return;//2人玩法right节点同为玩家节点
    }
    var sData = MjClient.data.sData;
    var tData = sData.tData;
    var selfIndex = (tData.uids.indexOf(SelfUid()) + off) % MjClient.MaxPlayerNum_hongZi;
    var pl = getUIPlayer_hongZi(off);
    ...
    if(pl){
        pl.eatFlag = 0;
        MjClient.playui.EatVisibleCheck();
        ShowEatActionAnim_hongZi(node,ActionType_hongZi.HU,off);
    }
}

function DealAddCard_hongZi(node,msg, off){
    if(off == 1 && node.getName() == "right" && MjClient.MaxPlayerNum_hongZi == 2){
        return;//2人玩法right节点同为玩家节点
    }
    if(MjClient.rePlayVideo == -1 && off != 0){
...
//清除打出的牌
function RemovePutCardOut_hongZi(noAction){
    ...
    for(var i = 0;i < uiList.length;i++){
        ...
        if (MjClient.gameType == MjClient.GAME_TYPE.PAO_HU_ZI_SR || MjClient.gameType == MjClient.GAME_TYPE.PAO_HU_ZI_SR_King)
        {
            pl = getUIPlayer_hongZi(getOffByXing_hongZi(i));
        }
...
//发手牌时再清理一遍
function clearMJHandCardUI_hongZi() {
    // var handNode = MjClient.playui._downNode.getChildByName("handNode");
    ...
    if((MjClient.gameType == MjClient.GAME_TYPE.LUO_DI_SAO || MjClient.gameType == MjClient.GAME_TYPE.JIANG_YONG_15Z) && MjClient.MaxPlayerNum_hongZi == 4){
...
//出牌表示状态
function ShowPutCardIcon_hongZi() {
...
//清除手牌
function RemoveHandCard_hongZi(node,card,off){
    if(MjClient.rePlayVideo == -1 && off != 0){
        return;
    }
    var cardArr = MjClient.HandCardArr;
...
// 初始化吃碰杠胡动作
function InitShowEatActionNode_hongZi(plNode){
...
// 重置吃碰杠胡动作
function resetEatActionAnim_hongZi()
{
    var jsBind = MjClient.playui.jsBind;
    ...
    var count = MjClient.MaxPlayerNum_hongZi;
    cc.log("count:", count);
    for(var i = 0; i < count; i++)
    {
        if(ui[i])
            InitShowEatActionNode_hongZi(ui[i]._node);
    }
}

//播放头像移动
function tableStartHeadMoveAction_hongZi(node){
...
//重置3家头像位置
function reConectHeadLayout_hongZi(node){
    var tData = MjClient.data.sData.tData;
    var down = node.getChildByName("down").getChildByName("head");
    var left = node.getChildByName("left").getChildByName("head");
    var right = node.getChildByName("right").getChildByName("head");
...
//根据偏移获得玩家node
function getNode_hongZi(off){
    var _node = null;
    if(MjClient.gameType == MjClient.GAME_TYPE.PAO_HU_ZI_SR || MjClient.gameType == MjClient.GAME_TYPE.PAO_HU_ZI_SR_King){
        var tData = MjClient.data.sData.tData;
        var selfIndex = tData.uids.indexOf(SelfUid());
        if(selfIndex == tData.xingPlayer){
            // off = (tData.zhuang + 2 + off + MjClient.MaxPlayerNum_hongZi) % MjClient.MaxPlayerNum_hongZi;
            if(off == MjClient.MaxPlayerNum_hongZi - 1){
    ...
    if(MjClient.MaxPlayerNum_hongZi == 4 && (MjClient.gameType == MjClient.GAME_TYPE.LUO_DI_SAO || 
 ...
 * @return {object} 玩家数据
 */
function getUIPlayer_hongZi(off){
    var sData = MjClient.data.sData;
    var tData = sData.tData;
    var uids = tData.uids;
    var selfIndex = uids.indexOf(SelfUid());
    selfIndex = (selfIndex + off) % MjClient.MaxPlayerNum_hongZi;
...
// 获取ui头像，通过偏移值
function getUIHeadByOff_hongZi(off){
    var pl = getUIPlayer_hongZi(off);
    if(!pl)
    {
        return {};
 ...
 * 下载录音, 调用 播放函数
 * */
function downAndPlayVoice_hongZi(uid, filePath){
    var index = getUiOffByUid_hongZi(uid);
...
function getUiOffByUid_hongZi(uid){
    var sData = MjClient.data.sData;
    var tData = sData.tData;
    var uids = tData.uids;
    var selfIndex = uids.indexOf(SelfUid());
    var targetIndex = uids.indexOf(uid);
    return (targetIndex - selfIndex + MjClient.MaxPlayerNum_hongZi) % MjClient.MaxPlayerNum_hongZi;
}

//设置微信头像
function setWxHead_hongZi(node, d, off){
    if(d.uid == getUIHeadByOff_hongZi(off).uid){
        ...
        COMMON_UI.addNobleHeadFrame(nobody,getUIPlayer_hongZi(off))
    }
}

//显示玩家庄的ui
function showUserZhuangLogo_hongZi(node, off){
    var tData = MjClient.data.sData.tData;
    var pl = getUIPlayer_hongZi(off);
    node.zIndex = 100;
    if(tData && pl){
...
//显示房主  add by sking
function showFangzhuTagIcon_hongZi(node,off)
{
    var pl = getUIPlayer_hongZi(off);
    if(!pl) //位置上没人则删掉房主标签
    {
...
// 清理ui
function clearCardUI_hongZi(node,off){
    mylog("clearCardUI_hongZi");
    ...
    for(var i = 0; i < children.length; i++)
    {
        ...
        {
            InitShowEatActionNode_hongZi(ni.getParent());
        }
    }
}

function EatFlag_hongZi(){
    var eat = MjClient.playui.jsBind.eat;
    var eatFlag = 0;
...
function huXiScore_hongZi(type,card)
{
...
//刷新胡息
function UpdateHuXi_hongZi(off){
    var sData = MjClient.data.sData;
    var tData = sData.tData;
    var score = 0;
    var pl = getUIPlayer_hongZi(off);
    ...
    if(mjpao.length>0){
        for(var p=0;p<mjpao.length;p++){
            score += huXiScore_hongZi("paoPai",mjpao[p]);
        }
    }
    ...
    if(mjpeng.length>0){
        for(var i=0;i<mjpeng.length;i++){
            score += huXiScore_hongZi("peng",mjpeng[i]);
        }
    }
        ...
        var chiScore = function(cards){
            ...
            for(var k=0;k<chiXiArr.length;k++){
                ...
                if(cards.toString() == chiXiList.toString()){
                    score += huXiScore_hongZi("chi",cards[0]);
                }
            }
        };
    ...
    if((MjClient.gameType == MjClient.GAME_TYPE.HY_LIU_HU_QIANG || MjClient.gameType == MjClient.GAME_TYPE.ML_HONG_ZI)&&pl.info.uid!=SelfUid()){
        ...
        if(mjti.length>0){
            for (var t = 0; t < mjti.length; t++) {
                if(MjClient.majiang.getCardShowType(pl.info.uid, mjti[t]) != 0){
                    score += huXiScore_hongZi("tiPai", mjti[t]);
                }
            }
        }
        //偎牌
        ...
        //             score += huXiScore_hongZi("weiPai", mjwei[w]);
        //         }
        //     }
        // }
    }else {
...
//设置转盘显示状态
function IsArrowVisible_hongZi(){
    var pl = getUIPlayer_hongZi(0);
    if (!pl)
    {
        return;
...
// 显示吃碰杠胡动作
function ShowEatActionAnim_hongZi(node, actType, off){
    var delayTime = 1;
    switch(actType)
    {
        case ActionType_hongZi.FANXING:
            delayTime = 1.6;
            break;
        default:
            break;
...
//显示玩家信息
function showPlayerInfo_hongZi(off, node){
    //var tData = MjClient.data.sData.tData;
    var pl = getUIPlayer_hongZi(off);
    if(pl){
...
function showSelectEatCards_hongZi(node,cardNum){
    var eat = MjClient.playui.jsBind.eat;
    eat.chi._node.visible = false;
    eat.guo._node.visible = false;
    eat.peng._node.visible = false;
    ...
    addEatCards_hongZi(parent,cardArr,putCard, pl);
}

doMoveCenterAction_hongZi = function(arr,isScale){
    arr.reverse();
    var gap = 5;
...
//添加可以吃的牌
function addEatCards_hongZi(node,handCardArr,putCard, pl){
    ...
    for(var i = 0;i < chiSet.length;i++){
        ...
        for(var k = 0;k < cardList.length;k++){
            ...
            cardNode.loadTexture(MjClient.cardPath_hongZi+"hand" + card + ".png");
        ...
        chiCloneBtn.addClickEventListener(function(chiBtn){
            ...
            //     setChiVisible_hongZi();
            //     HZChiToServer_hongZi(chiArr,null);
            // }else{
            //     var biSet = MjClient.majiang.getBiCards(tmpHandCardArr,putCard);
            //     if(biSet && biSet.length > 0){
            //         addFirstBiCards_hongZi(node,biSet,putCard,tmpHandCardArr,chiArr);
            //     }
            // }
            // setChiVisible_hongZi();
            // HZChiToServer_hongZi(chiArr,null);
            commitEatCards_hongZi(chiArr, null);
        });
    }

    if(ziPai.getUiLayoutType() == 0){//新布局
        doMoveCenterAction_hongZi([chiBg], false);
    }
}

//添加第一个比牌
function addFirstBiCards_hongZi(node,biSet,putCard,handCardArr,chiArr){
    ...
    for(var i = 0;i < biSet.length;i++){
        ...
        cloneBtn.addClickEventListener(function(btn){
            ...
            if(tmpHandCardArr.indexOf(putCard) >= 0){
                var biSet = MjClient.majiang.getBiCards(tmpHandCardArr,putCard);
                if(biSet && biSet.length > 0){
                    addSecBiCards_hongZi(node,biSet,putCard,tmpHandCardArr,chiArr,biArr);
                }
            }else{
                setChiVisible_hongZi();
                HZChiToServer_hongZi(chiArr,[biArr]);
            }
        });
    }
...
//添加第二个比牌
function addSecBiCards_hongZi(node,biSet,putCard,handCardArr,chiArr,firstBiArr){
    ...
    for(var i = 0;i < biSet.length;i++){
        ...
        cloneBtn.addClickEventListener(function(btn){
            ...
            btn.setBright(false);
            setChiVisible_hongZi();
            HZChiToServer_hongZi(chiArr,[firstBiArr,biArr]);
        });
    }
...
function GetReadyVisible_hongZi(node, off) {
    if (off < 0) {
        node.visible = false;
        return false;
    }
...
//设置玩家掉线头像
function setUserOffline_hongZi(node, off){
    var pl = getUIPlayer_hongZi(off);
    if(!pl){
        return;
    }
...
//显示玩家文字
function showUserChat_hongZi(node, off, msg){
    var tData = MjClient.data.sData.tData;
 ...
 */
function curPlayerIsMe_hongZi(off){
    var sData = MjClient.data.sData;
    var tData = sData.tData;
...
function getVisablePlayerCount_hongZi(){
    var sData = MjClient.data.sData;
    var tData = sData.tData;
    var uids = tData.uids;
    var count = 0;
 ...
 */
function getXingPlayerIndex_hongZi(){
    if(MjClient.MaxPlayerNum_hongZi != 4){
        return -1;
    }
    if(getVisablePlayerCount_hongZi() < 3){
        return -1;
    }
    var sData = MjClient.data.sData;
    var tData = sData.tData;
    ...
    var xingIndex = (zhuang + 2 + MjClient.MaxPlayerNum_hongZi) % MjClient.MaxPlayerNum_hongZi;
    var selfIndex = tData.uids.indexOf(SelfUid());
    xingIndex = (xingIndex - selfIndex + MjClient.MaxPlayerNum_hongZi) % MjClient.MaxPlayerNum_hongZi;
    return xingIndex;
}
 ...
 *                  提供 getUIPlayer_hongZi(resoff) 获取 off位置玩家数据
 */
function getOffByXing_hongZi(off){
    var resOff = off
    if(MjClient.MaxPlayerNum_hongZi != 4){
        return resOff;
    }
    ...
    if(selfIndex == xingIndex){ // xing为off固定为2， 参数off
        resOff = (2 + off) % MjClient.MaxPlayerNum_hongZi;
    }else{
        // 如果我的下家是醒家，且是ui层传过来的是位置是1 则跳过
        if((selfIndex + 1 + MjClient.MaxPlayerNum_hongZi) % MjClient.MaxPlayerNum_hongZi == xingIndex && off == 1){
            resOff = off + 1;

            // 如果我的上家是醒家，且是ui层传过来的是位置是3 则跳过
        }else if((selfIndex -1 + MjClient.MaxPlayerNum_hongZi) % MjClient.MaxPlayerNum_hongZi == xingIndex
            && off == MjClient.MaxPlayerNum_hongZi - 1){
            resOff = off - 1;

            // 如果我的上家是醒家, 且 传过来的是位置是2 , (发表情时发的是2）
        }else if( off==2 ){
            if((selfIndex + 1 + MjClient.MaxPlayerNum_hongZi) % MjClient.MaxPlayerNum_hongZi == xingIndex ){
                resOff = 1;
            }
            if((selfIndex - 1 + MjClient.MaxPlayerNum_hongZi) % MjClient.MaxPlayerNum_hongZi == xingIndex ){
                resOff = 3;
...
function resetHandAfterBegin_hongZi(){
    var sData = MjClient.data.sData;
    var pl = sData.players[SelfUid() + ""];
...
function changeMJBg_hongZi(node, type)
{
    if (node.toString() == "[object ImageView]") {
        var oldFile = node.getRenderFile().file;
        var newFile = getNewMJBgFile_hongZi(oldFile, type);
    ...
    for(var index in childArray)
    {
        ...
        changeMJBg_hongZi(child, type);
    }
}

function getNewMJBgFile_hongZi(oldFile, type)
{
    if (oldFile.indexOf("playing/paohuzi/") == -1)
        return oldFile;

    if (type == undefined)
        type = ziPai.getZiPaiType();//getCurrentMJBgType_HongZi();
...
*/
function createRadioBoxForCheckBoxs_hongZi(nodes, callback, defaultIndex){
    var newobj = {};
...
function _checkIsAddGroup_hongZi(){
    var cardArr = MjClient.HandCardArr;
    var _node = getNode_hongZi(0);
    //根据牌的类型获得需要添加的节点
    var addNode = _node.getChildByName("handNode");
    if(!addNode){
        return;
...
function _hasValueInArr_hongZi(arr,value){
    var len = arr.length;
    for(var i = 0; i < len; i++){
        var temp = arr[i];
        if(temp == value){
...
function _checkGroupList_hongZi(groupList,tagName){
    var _node = getNode_hongZi(0);
    //根据牌的类型获得需要添加的节点
    var addNode = _node.getChildByName("handNode");
    if(!addNode){
    ...
    if(cardParent){
        ...
        for(var j = len; j < 4; j++){
            ...
            if(card && 
                (!_hasValueInArr_hongZi(groupList,card.tag) || 
                    hasSameChild_hongZi(cardParent) ))// || cardParent.childrenCount > groupList.length))
            {
...
//一列中是否是相同的牌
function hasSameChild_hongZi(cardParent){
...
//通过名字和tag 查找节点
function findChildByNameAndIndex_hongZi(cardParent, mjNum, index){
...
function refreshHandCard_hongZi(tagName,index,mjNum,off){
    if(off != 0){
        return;
    }
    var _node = getNode_hongZi(off);
    ...
    var newCard = findChildByNameAndIndex_hongZi(cardParent, mjNum, index);
    ...
    if(newCard){
        ...
        //  newCard = null;
        //  newCard = getNewCard_hongZi(mjNum,1,off);
        //  cardParent.addChild(newCard);
        // }else{
        //  // return;
        // }
...
function refreshNode_hongZi(posNode,off,needAction){
    ...
    cc.log("执行动画前手牌 refreshNode_hongZi=====:" , MjClient.HandCardArr);
    ...
    _checkIsAddGroup_hongZi();

    var index = 0;
    for(var k = 0;k < oldLen;k++){
        var groupList = cardArr[index];

        if(_hasValueInArr_hongZi(deleIndexArr, k)){
            continue;
        }

        _checkGroupList_hongZi(groupList,k);
        for(var j = 0;j < groupList.length;j++){

            refreshHandCard_hongZi(k,j,groupList[j],off);
        }
        index += 1;
    }
    ...
    for(var i = 0; i < oldLen; i++){
        ...
        if(addNode && _hasValueInArr_hongZi(deleIndexArr, addNode.tag)){
    ...
    cc.log("执行动画后手牌 refreshNode_hongZi=====:" , MjClient.HandCardArr);
