...
var topOff = 2;
//向服务器发送 过消息
MjClient.MJPass2NetForHZTDH = function() {
    // console.log(">>>>>>>>>普通  过 <<<<<<<<");
    cc.log("====================send======pass=====");
...
function SetUserVisible_HZTDH(node, off) {
    //var sData = MjClient.data.sData;
    //return;
    cc.log("====================off======================" + off);
...
function InitUserHandUI_HZTDH(node, off) {
    var sData = MjClient.data.sData;
    var tData = sData.tData;
    var pl = getUIPlayer(off);
    if (!pl) {
    ...
    cc.log("_________________InitUserHandUI_HZTDH_______________" + JSON.stringify(pl));

    if (tData.tState == TableState.waitJiazhu && SelfUid() == pl.info.uid) {
        var zhuangUid = tData.uids[tData.zhuang];
        var selfIsZhuang = zhuangUid == SelfUid();
        var isDingZhuang = false;   // 是否轮到顶庄
        var laZhuangCount = 0;      // 拉庄人数
        for(var i in sData.players){
            var tmpPlayerInfo = sData.players[i];
            if(tmpPlayerInfo.uid != zhuangUid && tmpPlayerInfo.jiazhuNum >= 0){
                laZhuangCount++;
            }
        }

        // 庄家外 所有人已表态是否拉庄， 且我是庄家
        if(laZhuangCount == (MjClient.MaxPlayerNum - 1) && selfIsZhuang){
            isDingZhuang = true;
        }

        if (pl.mjState == TableState.waitJiazhu && (!selfIsZhuang || isDingZhuang) ) {

            var layer = new LazhuangSelectLayer(null, 4, isDingZhuang); // 自己是庄家表示要顶庄啦。
...
function initFlower_HZTDH() {
    initFlower(false, false);
}


var PlayLayer_HZTDH = cc.Layer.extend({
    jsBind: {
        _event: {
            mjhand: function() {
            ...
            roundEnd: function() {
                ...
                function delayExe() {
                    ...
                    self.addChild(new EndOneView_HZTDH(), 500);
                }
            ...
            moveHead: function() {
                ...
                MjClient.playui._btnFlower.visible = isshow;
                initFlower_HZTDH();
            },
        ...
        roundnumImg: {
            _run: function() {
                ...
                // else
                //     setWgtLayout(this, [0.1, 0.1], [0.5, 0.65], [-1.2, 1.0]);
            },
            _event: {
        ...
        gameName: {
            _run: function() {
                // if (MjClient.MaxPlayerNum != 3)
                    setWgtLayout(this, [0.16, 0.16], [0.5, 0.62], [0, 1.0]);
                // else
                //     setWgtLayout(this, [0.16, 0.16], [0.5, 0.77], [0, 1.0]);
            }
        },
        roundInfo: {
            _run: function() {
                // if (MjClient.MaxPlayerNum != 3)
                    setWgtLayout(this, [0.12, 0.12], [0.5, 0.38], [0, 1.0]);
                // else
                //     setWgtLayout(this, [0.12, 0.12], [0.5, 0.53], [0, 1.0]);

                var tData = MjClient.data.sData.tData;
                this.ignoreContentAdaptWithSize(true);
                this.setString(MjClient.playui.getGameInfoString("roundInfo"));
                showPlayUI_roundInfo(MjClient.playui.getGameInfoString("roundInfo"), tData.tableid);

                //比赛场
                var BSStr = "";
                if (tData.matchId) {
                    BSStr = ",10秒出牌";
                    this.setString(GameCnName[MjClient.gameType]+","+MjClient.playui.getGameInfoString("roundInfo") + BSStr);
                }
        ...
        banner: {
            ...
            Button_1: {
                _visible: true,
                _click: function() {
                    MjClient.openWeb({ url: MjClient.GAME_TYPE.HZ_TUI_DAO_HU, help: true });
                }
            },
            hunPai: {
                baidaBg: {
        ...
        //     _run: function() {
        //         if (MjClient.MaxPlayerNum != 3)
        //             setWgtLayout(this, [0.2, 0.2], [0.5, 0.5], [0, 0.25]);
        //         else
        //             setWgtLayout(this, [0.2, 0.2], [0.5, 0.65], [0, 0.25]);
        //
        //         MjClient.arrowbkNode = this;
        //         setDirVisible(this, MjClient.MaxPlayerNum != 3);
        ...
        wait: {
            getRoomNum: {
                _run:function(){
                    setWgtLayout(this, [0.18, 0.18],[0.75, 0.12],[0, 0]);
                },
                _visible: function() {
                ...
                _click: function() {
                     ...
                     */
                    var tData = MjClient.data.sData.tData;
                    var str1 = GameCnName[MjClient.gameType] + ",";
                    var str2 = MjClient.playui.getGameInfoString("getRoomNum") + ",";
                    var str3;
                    if (MjClient.MaxPlayerNum == 4)
                        str3 = tData.roundNum + "局,三缺一,";
                    else if (MjClient.MaxPlayerNum == 3)
                        str3 = tData.roundNum + "局,二缺一,";
                    else
                        str3 = tData.roundNum + "局,缺一,";
            ...
            wxinvite: {
                _run:function(){
                    setWgtLayout(this, [0.18, 0.18],[0.25, 0.12],[0, 0]);
                    var qipao = new ccui.ImageView("playing/gameTable/paopao_kaizhuo.png"),
                        pos = cc.p(this.width/2, this.height);
                    this.addChild(qipao);
                    qipao.setAnchorPoint(cc.p(0.5, 0));
            ...
            delroom: {
                _run:function(){
                    setWgtLayout(this, [0.12, 0.12],[0.05, 0.45],[0, 0]);
                    if(isIPhoneX()) setWgtLayout(this, [0.12, 0.12],[0.1, 0.45],[0, 0]);
                },
                _click: function() {
            ...
            backHomebtn: {
                _run:function(){
                    setWgtLayout(this, [0.12, 0.12],[0.05, 0.6],[0, 0]);
                    if(isIPhoneX()) setWgtLayout(this, [0.12, 0.12],[0.1, 0.6],[0, 0]);
                },
                _click: function(btn) {
            ...
            _event: {
                ...
                MJPut: function(eD) {
                    ...
                    if (eD.uid == SelfUid()) {
                        ...
                        if (pl.isTing && pl.putCardAfterTing)
                            _tingCards.getChildByName("icon").loadTexture("playing/gameTable/icon_jiating.png");
                        else
                            _tingCards.getChildByName("icon").loadTexture("playing/gameTable/icon_ting.png");
                    }
                ...
                MJGang: function(eD) {
                    ...
                    hideTingBtn();
                    if (MjClient.data.sData.tData.areaSelectMode.getGangScore)
                        dealGangLiJiJieSuan(this, eD, 0);
                    setUserOffline(this, 0);
                },
                MJPeng: function(eD) {
                ...
                waitJiazhu: function(msg) {
                    ...
                    if (tData.uids[msg.zhuang] != SelfUid()) {
                        var layer = new LazhuangSelectLayer(function() {
                            //弹窗等待
                            MjClient.playui._jiazhuWait.visible = true;
                            var pl = getUIPlayer(0);
                            pl.mjState = TableState.isReady;
                            cc.log("==============LazhuangSelectLayer=========TableState.isReady;= ");
                ...
                waitDingZhuang: function(msg) {
                    var layer = new LazhuangSelectLayer(function() {}, 4, true);
                    MjClient.playui.addChild(layer, 99);
                    if (MjClient.webViewLayer != null) {
                        MjClient.webViewLayer.close();
                    }
        ...
        top: {
            ...
            head: {
                tuoguan: {
                    ...
                    _event: {
                        beTrust: function(msg) {
                            if (getUIPlayer(topOff)&&getUIPlayer(topOff).info.uid == msg.uid) {
                                this.visible = true;
                            }
                        },
                        cancelTrust: function(msg) {
                ...
                zhuang: {
                    ...
                    _event: {
                        waitPut: function() {
                            showUserZhuangLogo(this, topOff);
                        },
                        initSceneData: function() {
                            if (IsArrowVisible()) showUserZhuangLogo(this, topOff);
                        }
                    }
                },
                chatbg: {
                    ...
                    chattext: {
                        _event: {
                            ...
                            MJChat: function(msg) {
                                showUserChat(this, topOff, msg);
                            },
                            playVoice: function(voicePath) {
                                MjClient.data._tempMessage.msg = voicePath;
                                showUserChat(this, topOff, MjClient.data._tempMessage);
                            }
                        }
                    }
                },
                _click: function(btn) {
                    showPlayerInfo(topOff, btn);
                },
                _event: {
                    loadWxHead: function(d) {
                        setWxHead(this, d, topOff);
                    },
                    addPlayer: function(eD) {
                        showFangzhuTagIcon(this, topOff);
                    },
                    removePlayer: function(eD) {
                        showFangzhuTagIcon(this, topOff);
                    }
                ...
                tingCard: {
                    ...
                    _event: {
                        MJTing: function(eD) {
                            if (MjClient.playui.isPlayerPutCard(eD, topOff)) {
                                var pl = getUIPlayer(topOff);
                                pl.putCardAfterTing = eD.putCardAfterTing;
                                MjClient.playui.setTingCardInfo(this, eD, topOff);
                            }
                        },
                        clearCardUI: function(eD) {
                            cc.log("ready to----- clear sking ----");
                ...
                tingIcon: {
                    ...
                    _event: {
                        ...
                        moveHead: function() {
                            MjClient.playui.tingIconVisible(this, topOff);
                        },
                        onlinePlayer: function(eD) {
                            //MjClient.playui.tingIconVisible(this,topOff);
                        },
            ...
            _event: {
                clearCardUI: function() {
                    clearCardUI(this, topOff);
                },
                initSceneData: function(eD) {
                    SetUserVisible_HZTDH(this, topOff);
                },
                addPlayer: function(eD) {
                    SetUserVisible_HZTDH(this, topOff);
                },
                ...
                mjhand: function(eD) {
                    InitUserHandUI_HZTDH(this, topOff);
                },
                roundEnd: function() {
                    InitUserCoinAndName(this, topOff);

                },
                waitPut: function(eD) {
                    DealWaitPut(this, eD, topOff);
                },
                MJPut: function(eD) {
                    DealMJPut(this, eD, topOff);
                    if (eD.uid != SelfUid()) {
                        hideTingBtn();
                    }
                    setUserOffline(this, topOff);
                },
                MJChi: function(eD) {
                    DealMJChi(this, eD, topOff);
                    setUserOffline(this, topOff);
                },
                MJGang: function(eD) {
                    DealMJGang(this, eD, topOff);
                    if (MjClient.data.sData.tData.areaSelectMode.getGangScore)
                        dealGangLiJiJieSuan(this, eD, topOff);
                    setUserOffline(this, topOff);
                },
                MJPeng: function(eD) {
                    DealMJPeng(this, eD, topOff);
                    setUserOffline(this, topOff);
                },
                MJHu: function(eD) {
                    HandleMJHu(this, eD, topOff);
                    setUserOffline(this, topOff);
                },
                onlinePlayer: function(eD) {
                    setUserOffline(this, topOff);
                ...
                MJFlower: function(eD) {
                    HandleMJFlower(this, eD, topOff);
                },
                MJTing: function(eD) {
                    HandleMJTing(this, eD, topOff);
                }
            }
        },
        left: {
            ...
            changeui: {
                ...
                changeuibg: {
                    ...
                    guobg: {
                        guo: {
                            _touch: function(btn, eT) {
                                if (eT == 2) MjClient.MJPass2NetForHZTDH();
                            }
                        },
                        fanhui: {
                            _touch: function(btn, et) {
    ...
    ctor: function() {
        ...
        var playui = ccs.load(res.Play_HZTDH_json);
        playMusic("bgFight");
    ...
    getGameInfoString: function(param) {
        ...
        str += tData.areaSelectMode.lazhuang ? "拉庄," : "不拉庄,";

        if (tData.areaSelectMode.canGangScore) {
            if (tData.areaSelectMode.getGangScore)
                str += "杠牌直接得分,";
            else
                str += "杠牌有分,";
        }

        if (tData.areaSelectMode.liu8zhang)
            str += "留8张牌,";

        if (tData.areaSelectMode.difen) {
            str += "积分底分" + Number(tData.areaSelectMode.difen) + ",";
        }
    ...
    /**
     * @function 设置“花x0"是否显示
     * @param {Node} parentNode 父节点
     */
    setPlayerHuaValueShow: function(parentNode) {
        var tData = MjClient.data.sData.tData;
        var isShow = tData.areaSelectMode.zhongfabai != 3 && !IsInviteVisible();
        ...
        if (pl && (pl.mjState == TableState.isReady || pl.mjState == TableState.roundFinish)) {
            ...
            if (pl != null) {
                if (off == 0)
                {
                    ...
                    node.visible = this._tingCardsNode.visible;
                    if (pl.isTing && pl.putCardAfterTing)
                        this._tingCardsNode.getChildByName("icon").loadTexture("playing/gameTable/icon_jiating.png");
                    else
                        this._tingCardsNode.getChildByName("icon").loadTexture("playing/gameTable/icon_ting.png");
                }
            }
        }
        return node.visible;
    ...
    */
    ...
    isPlayerPutCard: function(eD, off) {
        ...
        offidx = getOffForPlayerNum(offidx);
...
PlayLayer_HZTDH.prototype.CardLayoutRestore = function(node, off) {
...
// 判断吃碰杠胡的状态
PlayLayer_HZTDH.prototype.EatVisibleCheck = function() {
    var eat = MjClient.playui.jsBind.eat;
        ...
        if (IsTurnToMe()) {
            ...
            for (var i = cduis.length - 1; i >= 0; i--) {
                if (cduis[i].name == "mjhand" && MjClient.majiang.isCardFlower(cduis[i].tag)) {
                    var index = i;
                    var callback = function() {
                        PutOutCard(cduis[index], cduis[index].tag);
                    };
            ...
            // if(pl.isNew && MjClient.majiang.canHu(pl.mjhand, 0)) {
            //     if (pl.isTing || Object.keys(calTingSet(pl.mjhand)).length > 1) { //听牌或者听多张牌才能胡
            //         vnode.push(eat.hu._node);
            //     }
            // }
            //听
    ...
    //别人点
        ...
        if (!IsTurnToMe()) {
            //选了点炮胡玩法才能点炮胡
            //if (tData.playType.indexOf(MjClient.PLAY_TYPE.DIAN_PAO_HU) >= 0 )
            //{
            // cc.log("diao pao hu-=================================================" + "!IsTurnToMe()");
            if (pl.eatFlag & 8) {
                vnode.push(eat.hu._node);
            }
            ...
            // var canHu = MjClient.majiang.canHu(pl.mjhand, tData.lastPutCard);
            // if(canHu && !pl.skipHu)
            // {
            //     if (pl.isTing || Object.keys(calTingSet(pl.mjhand)).length > 1) { //听牌或者听多张牌才能胡
            //         vnode.push(eat.hu._node);
            //     }
            // }
            // cc.log("tData.putType" + tData.putType + "---------tData.putType" + tData.putType);
            // if((tData.putType == 0 || tData.putType == 4))
            // {
            //     //杠
            //     // cc.log("rtnrtnrtnrtnrtnrtn" + JSON.stringify(MjClient.majiang.canGang0(pl.mjhand, tData.lastPutCard, pl.isTing)));
            //     if(leftCard > 0 && MjClient.majiang.canGang0(pl.mjhand, tData.lastPutCard, pl.isTing))
            //     {
            //         vnode.push(eat.gang0._node);
            //         MjClient.gangCards = [tData.lastPutCard];
            ...
            //     //碰
            //     if(leftCard > 0 && mj.canPeng(pl.mjhand, tData.lastPutCard) && !pl.isTing)
            //     {
            //         vnode.push(eat.peng._node);
            //     }
            // }
    ...
    if (vnode.length > 0) {
        ...
        for (var i = 0; i < vnode.length; i++) {
            ...
            setWgtLayout(vnode[i], [0, 0.2], [0.5, 0], [(1 - vnode.length) / 1.8 + i * 1.4, 1.8], false, false);
        }
    }
    if (eat.hu._node.visible) {
