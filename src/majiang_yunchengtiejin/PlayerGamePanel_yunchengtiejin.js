...
//向服务器发送 过消息
MjClient.MJPass2NetForyunchengtiejin = function()
{
   // console.log(">>>>>>>>>普通  过 <<<<<<<<");
    cc.log("====================MJPass2NetForyunchengtiejin======pass=====");
...
function SetUserVisible_yunchengtiejin(node, off)
{
    //var sData = MjClient.data.sData;
    //return;
    cc.log("====================off======================" + off);
...
function InitUserHandUI_yunchengtiejin(node, off)
{
    var sData = MjClient.data.sData;
    var tData = sData.tData;
    var pl = getUIPlayer(off);
    ...
    for(var i = 0; i < pl.mjput.length; i++)
    {
        ...
        MjClient.playui.DealMJPut_yunchengtiejin(node, msg, off, i);
    }
...
function initFlower_yunchengtiejin() {
    initFlower(false, false);
}


var PlayLayer_yunchengtiejin = cc.Layer.extend({
    jsBind: {
        _event: {
            mjhand: function() {
            ...
            roundEnd: function() {
                ...
                function delayExe()
                {
                    ...
                    self.addChild(new EndOneView_yunchengtiejin(),500);
                }
        ...
        eat: {
            ...
            guo: {
                ...
                _touch: function(btn, eT) {
                    if (eT == 2)
                    {
                        MjClient.MJPass2NetForyunchengtiejin();
                    }
                },
                bgimg: {
                    _run: function() {
            ...
            changeui: {
                ...
                changeuibg: {
                    ...
                    guobg: {
                        guo: {
                            _touch: function(btn, eT) {
                                if (eT == 2) MjClient.MJPass2NetForyunchengtiejin();
                            }
                        },
                        fanhui: {
                            _touch: function(btn, et) {
    ...
    ctor: function() {
        this._super();
        var playui = ccs.load("Play_yunchengtiejin.json");
        ...
        cc.log("playui.node 是空？？ ",playui.node);
...
PlayLayer_yunchengtiejin.prototype.CardLayoutRestore = function(node, off)
{
...
// 判断吃碰杠胡的状态
PlayLayer_yunchengtiejin.prototype.EatVisibleCheck = function()
{
    var eat = MjClient.playui.jsBind.eat;
...
/*
    玩家ui，信息
 */
PlayLayer_yunchengtiejin.prototype.Node_player = function(off)
{
    var _node = getNode(off);
    cc.log("off  ",off,_node);
    ...
    UIEventBind(null, _node, "initSceneData", function (eD) {
        SetUserVisible_yunchengtiejin(_node, off);
        ...
        if(_tingCardsNode) MjClient.playui.tingIconVisible(_tingIcon,off);
    ...
    UIEventBind(null, _node, "mjhand", function (eD) {
        cc.log("??????  ");
        InitUserHandUI_yunchengtiejin(_node, off);
    });
    ...
    UIEventBind(null, _node, "MJPut", function (eD) {
        ...
        var tData = MjClient.data.sData.tData;
        {
            MjClient.playui.DealMJPut_yunchengtiejin(_node,eD,off);
        }
        hideTingBtn();
        var pl = getUIPlayer(off);
    ...
    UIEventBind(null, _node, "MJTing", function (eD) {
        ...
        if(pl && eD.uid == getUIPlayer(off).info.uid)
        {
            ...
            //MjClient.playui.setTingCardInfo(_tingIcon,eD,off);
            _tingCard.visible = false;
        }
    });
...
PlayLayer_yunchengtiejin.prototype.willHuShowArrow = function()
{
    var tData = MjClient.data.sData.tData;
...
//处理出牌,放一张牌，打牌动作
PlayLayer_yunchengtiejin.prototype.DealMJPut_yunchengtiejin = function(node, msg, off, outNum)
{
...
PlayLayer_yunchengtiejin.prototype.changeHuaImg = function(node,off)
{
    var _HuaImg = node;
    var tData = MjClient.data.sData.tData;
    var pl = getUIPlayer(off);
