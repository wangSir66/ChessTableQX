...
(function() {
    ...
    majiang.getHongZhongNum = function(cards){
    var hongzhongNum = 0;
        for(var i in cards){
            if( 71 == cards[i] ){
                hongzhongNum++;
    ...
    majiang.canGang1 = function(peng, hand, isTing)
    {
        ...
        // 杠后只剩下红中， 不能杠
        if( this.getHongZhongNum(hand) >= (hand.length-4) )
        {
            return false;
        }
        ...
        {
            var cd = hand[i];
            if(majiang.isEqualHunCard(cd) || 71 == cd)
            {
                continue;
            }
            var num = cnum[cd];
    ...
    {
        var idDaihongzhong = MjClient.data.sData.tData.areaSelectMode.daihongzhong;
        if(idDaihongzhong){
            return 112;
        }else{
            return 108
