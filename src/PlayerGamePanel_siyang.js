...
//向服务器发送 过消息
MjClient.MJPass2NetForSiyang = function()
{
   // console.log(">>>>>>>>>普通  过 <<<<<<<<");
    cc.log("====================send======pass=====");
...
function SetUserVisible_siyang(node, off)
{
    //var sData = MjClient.data.sData;
    //return;
    cc.log("====================off======================" + off);
...
function InitUserHandUI_siyang(node, off)
{
    var sData = MjClient.data.sData;
    var tData = sData.tData;
    var pl = getUIPlayer(off);
    ...
    for(var i = 0; i < pl.mjgang0.length; i++)
    {
        ...
        //}
        //else {
        //    offIdx = (pl.pengchigang.pgang[i-pl.pengchigang.gang.length].pos - idx + MjClient.MaxPlayerNum) % MjClient.MaxPlayerNum - 1;
        //}
        var offIdx = null;
...
function initFlower_siyang() {
    initFlower(false, false);
}


var PlayLayer_siyang = cc.Layer.extend({
    jsBind: {
        _event: {
            mjhand: function() {
            ...
            roundEnd: function() {
                ...
                function delayExe()
                {
                    ...
                    self.addChild(new EndOneView_siyang(),500);
                }
        ...
        banner: {
            ...
            Button_1: {
                _visible : true,
                _click: function() {
                    MjClient.openWeb({url:MjClient.GAME_TYPE.SI_YANG,help:true});
                }
            },
            hunPai:{
                baidaBg:{
        ...
        BtnPutCard:{ //add by  sking for put card button
            ...
            _click: function(btn) {
                ...
                for(var i = 0; i < children.length; i++)
                {
                    ...
                    {
                        if(children[i].y > standUI.y + 10) //!MjClient.majiang.isCardFlower(children[i].tag) &&
                        {
            ...
            _event:{
                ...
                waitPut: function() {
                    ...
                    var speed = 0.8;
                    if (MjClient.data.sData.tData.areaSelectMode.quickly == 1) speed = 0.35;
                    var eat = MjClient.playui.jsBind.eat;
                    if (IsTurnToMe() && pl.isTing && !eat.hu._node.visible && !eat.gang0._node.visible && !eat.gang1._node.visible && !eat.gang2._node.visible && !MjClient.majiang.isCardFlower(pl.newCd)) {
                        cc.log("*********自动出牌*********");
                        this.runAction(cc.sequence(cc.delayTime(speed),
            ...
            changeui: {
                ...
                changeuibg: {
                    ...
                    guobg: {
                        guo: {
                            _touch: function(btn, eT) {
                                if (eT == 2) MjClient.MJPass2NetForSiyang();
                            }
                        },
                        fanhui: {
                            _touch: function(btn, et) {
    ...
    ctor: function() {
        ...
        var playui = ccs.load(res.Play_siyang_json);
        playMusic("bgFight");
...
PlayLayer_siyang.prototype.CardLayoutRestore = function(node, off)
{
...
// 判断吃碰杠胡的状态
PlayLayer_siyang.prototype.EatVisibleCheck = function()
{
    var eat = MjClient.playui.jsBind.eat;
    ...
    if(tData.tState == TableState.waitPut && pl.mjState == TableState.waitPut)
    {
        if(IsTurnToMe())
        {
            ...
            //听
            pl.hua = pl.hua || 0;
            cc.log("￥￥￥￥听牌监测91111199999 pl.isTing = " + pl.isTing);
            cc.log("￥￥￥￥听牌监测91111199999 pl.mjpeng = " + pl.mjpeng.length);
            cc.log("￥￥￥￥听牌监测91111199999 pl.mjgang0 = " + pl.mjgang0.length);
            cc.log("￥￥￥￥听牌监测91111199999 pl.hua = " + pl.hua);
            cc.log("￥￥￥￥听牌监测91111199999 pl.mjflower = " + pl.mjflower.length);
            cc.log("￥￥￥￥听牌监测9999999999999");
            MjClient.canTingCards = {};
            if (!pl.isTing && (pl.mjpeng.length == 0 && pl.mjgang0.length == 0 || pl.hua + pl.mjflower.length >=3)) {
                for (var i = 0; i < pl.mjhand.length; i++) {
                ...
                cc.log("Tom --------------------￥￥￥￥听牌监测9999999999999---------   _canTingCards ", JSON.stringify(MjClient.canTingCards));
            }
            //杠
