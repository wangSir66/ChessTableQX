...
//向服务器发送 过消息
MjClient.MJPass2NetForfenxiyingkou = function()
{
   // console.log(">>>>>>>>>普通  过 <<<<<<<<");
    cc.log("====================MJPass2NetForfenxiyingkou======pass=====");
...
function SetUserVisible_fenxiyingkou(node, off)
{
    //var sData = MjClient.data.sData;
    //return;
...
function DealMJPut_fenxiyingkou(node, msg, off, outNum)
{
    DealMJPut_shanXiApp(node, msg, off, outNum);
}

function InitUserHandUI_fenxiyingkou(node, off)
{
    var sData = MjClient.data.sData;
    var tData = sData.tData;
    var pl = getUIPlayer(off);
    ...
    MjClient.playui.jsBind.banner.hunPai._node.visible=true;
    ...
    for(var i = 0; i < pl.mjgang1.length; i++)
    {
        for(var j = 0; j < 4; j++)
        {
            ...
            {
                if(tData.areaSelectMode.angangbukejian && off != 0)
                {
                    cc.log("暗杠不可见！！！！！");
    ...
    for(var i = 0; i < pl.mjput.length; i++)
    {
        ...
        DealMJPut_fenxiyingkou(node, msg, off, i);
    }
...
function initFlower_fenxiyingkou() {
    initFlower(false, false);
}

function DealMJGang_FENXIYINGKOU(node, msg, off)
{
    cc.log("================DealMJGang = " + JSON.stringify(msg));
        ...
        if(msg.gang == 3)//暗杠
        {
            if (MjClient.gameType == MjClient.GAME_TYPE.TONG_HUA)
            {
                ...
                else
                {
                    if(j == 3)
                    {
                        _cardNode = getNewCard(node, "down", "gang1", 0, off, "isgang4").tag = msg.card;
                    }
                    else
                    {
                        _cardNode = getNewCard(node, "down", "gang1", 0, off).tag = msg.card;
                    }
                }
            }
            else if(MjClient.gameType == MjClient.GAME_TYPE.FEN_XI_YING_KOU) //汾西硬扣暗杠不可见
            {
                if(j == 3)
                {
                    _cardNode = getNewCard(node, "down", "gang1", 0, off, "isgang4").tag = msg.card;
...
var PlayLayer_fenxiyingkou = cc.Layer.extend({
    jsBind: {
        _event: {
            mjhand: function() {
            ...
            roundEnd: function() {
                ...
                if (MjClient.getAppType() == MjClient.APP_TYPE.TXLINFENMJ)
                {
                    ...
                    function delayExe()
                    {
                        ...
                        self.addChild(new EndOneView_fenxiyingkou(),500);
                    }
                    //hideCurrentTingNum();
                ...
                mingCardImg: {
                    ...
                    _event: {
                        mjhand: function () {
                            ...
                            var mingCardMsg = MjClient.data.sData.tData.hunCard;
                            this.visible = false;
                            cc.log('wwwwwwwwwwwwwwwwwdddddddd:'+JSON.stringify(MjClient.data.sData.tData))
                            cc.log("-------------------mjhand----------------- " + mingCardMsg);
                            if (mingCardMsg) {
                                this.visible = true;
                                setCardSprite(this, parseInt(mingCardMsg), 4);
                        ...
                        initSceneData: function () {
                            var mingCardMsg = MjClient.data.sData.tData.hunCard;
                            cc.log("=============明牌======HuncardMsg = " + JSON.stringify(MjClient.data.sData));
                            if (mingCardMsg) {
                                this.visible = true;
                                setCardSprite(this, parseInt(mingCardMsg), 4);
                                var imgMing = new ccui.ImageView();
            ...
            ting: {
                ...
                _touch: function(btn, eT) {
                    if (eT == 2)
                    {
                        ...
                        tingCards = MjClient.majiang.getCheckTingHuCards(currentCard,pl.mjhand);
            ...
            changeui: {
                ...
                changeuibg: {
                    ...
                    guobg: {
                        guo: {
                            _touch: function(btn, eT) {
                                if (eT == 2) MjClient.MJPass2NetForfenxiyingkou();
                            }
                        },
                        fanhui: {
                            _touch: function(btn, et) {
    ...
    ctor: function() {
        this._super();
        var playui = ccs.load("Play_fenxiyingkou.json");
...
PlayLayer_fenxiyingkou.prototype.CardLayoutRestore = function(node, off)
{
...
// 判断吃碰杠胡的状态
PlayLayer_fenxiyingkou.prototype.EatVisibleCheck = function()
{
    var eat = MjClient.playui.jsBind.eat;
        ...
        if(IsTurnToMe())
        {
            ...
            if (!pl.isTing) {
                ...
                for (var i = 0; i < pl.mjhand.length; i++) {
                    ...
                    if (MjClient.majiang.canTing(cardsAfterPut, cardsAfterPut.concat(pl.mjpeng).concat(pl.mjgang1).concat(pl.mjgang0))) {
    ...
    if(vnode.length > 0)
    {
        ...
        for(var i = 0; i < vnode.length; i++)
        {
            ...
            if(btnName == "gang0" && MjClient.majiang.isHuHua()==true)
            {
                vnode[i].loadTextureNormal("playing/gameTable/youxizhong-2_54.png");
                // vnode[i].loadTexturePressed("playing/gameTable/youxizhong-2_04.png");
            }
            ...
            if(i == 0)
            {
                ...
                else if(btnName == "gang0")
                {
                    ...
                    if(MjClient.majiang.isHuHua()==true)
                    {
                        vnode[i].loadTextureNormal("playing/gameTable/youxizhong-2_54.png");
                        vnode[i].loadTexturePressed("playing/gameTable/youxizhong-2_04.png");
                    }
    ...
    if (eat.hu._node.visible && eat.gang0._node.visible && MjClient.majiang.isHuHua()==true)//同时存在胡，杠开按钮，隐藏胡按钮
    {
        eat.hu._node.visible = false;
    }
...
/*
    玩家ui，信息
 */
PlayLayer_fenxiyingkou.prototype.Node_player = function(off)
{
    ...
    UIEventBind(null, _node, "initSceneData", function (eD) {
        SetUserVisible_fenxiyingkou(_node, off);
    ...
    UIEventBind(null, _node, "mjhand", function (eD) {
        InitUserHandUI_fenxiyingkou(_node, off);
    ...
    UIEventBind(null, _node, "MJPut", function (eD) {
        DealMJPut_fenxiyingkou(_node,eD,off);
        hideTingBtn();
        if(_tingCardNumNode) _tingCardNumNode.visible = false;
    ...
    UIEventBind(null, _node, "MJGang", function (eD) {
        DealMJGang_FENXIYINGKOU(_node, eD, off);
        hideTingBtn();
        if(_skipPengIconTag) _skipPengIconTag.visible = false;
        setUserOffline(_node, off);
    });
    ...
    switch (off)
    {
        ...
        case 2:
            ...
            setWgtLayout(_down,[0, 0.07], [0.5, 0.95], [6, -0.7]);
