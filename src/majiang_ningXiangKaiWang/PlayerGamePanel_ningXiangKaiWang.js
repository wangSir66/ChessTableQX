...
//向服务器发送 过消息
MjClient.MJPass2NetForningXiangKaiWang = function()
{
   // console.log(">>>>>>>>>普通  过 <<<<<<<<");
    cc.log("====================send======pass=====");
...
function SetUserVisible_ningXiangKaiWang(node, off)
{
    //var sData = MjClient.data.sData;
    //return;
    cc.log("====================off======================" + off);
    ...
    if(pl)
    {
        ...
        InitUserHandUI_ningXiangKaiWang(node, off);
...
function InitUserHandUI_ningXiangKaiWang(node, off)
{
    var sData = MjClient.data.sData;
    var tData = sData.tData;
    var pl = getUIPlayer(off);
    ...
    if (pl.haiDiStatus == 2 && SelfUid() == pl.info.uid)
    {
        //弹窗选择是否捞一把
        var pl = getUIPlayer(0);
        if(pl.fengDong){
            ...
            MjClient.gamenet.request("pkroom.handler.tableMsg", {
                cmd: "MJPickHaiDi",
                haiDiStatus: pl.haiDiStatus,
            });
        }else{
            var layer = new laZhangLayer(function(select) {
                pl.haiDiStatus = select ? 1 : 0;
                MjClient.gamenet.request("pkroom.handler.tableMsg", {
                    cmd: "MJPickHaiDi",
                    haiDiStatus: pl.haiDiStatus,
                });
...
function initFlower_ningXiangKaiWang() {
    initFlower(false, false);
}


var PlayLayer_ningXiangKaiWang = cc.Layer.extend({
    jsBind: {
        _event: {
            mjhand: function() {
            ...
            roundEnd: function() {
                ...
                function showEndCards()
                {
                    ...
                    self.addChild(new EndOneView_ningXiangKaiWang(),500);
                }
                function delayExe()
                {
...
/*                MjClient.Scene.addChild(new HAHZ_showCardLayer(showCards,function(){
            ...
            cardnumAtlas: {
                ...
                _event: {
                    ...
                    MJZhiTouZi: function(eD){
                        var tData = MjClient.data.sData.tData;
                        if (tData) this.setString(MjClient.majiang.getAllCardsTotal(tData) - eD.cardNext);
                    }
                }
            }
        },
        ...
        roundInfo:{
            ...
            {
                var tData = MjClient.data.sData.tData;
                // var trustTime = {"-1": "", 0: "", 60: "1分钟后托管", 120: "2分钟后托管", 180: "3分钟后托管", 300: "5分钟后托管" };
                // var str = "";

                // str += tData.areaSelectMode.difen > 0 ? "积分底分" + tData.areaSelectMode.difen + "," : ""; 
                // str += "坐压" + tData.areaSelectMode.zuoYa + "分,"; 
                // str += tData.areaSelectMode.fengding      == 0 ? "大胡3番封顶"  : "大胡不封顶";
                // str += tData.areaSelectMode.mustzimo      == 1 ? ",只能自摸胡"  : "";
                // str += tData.areaSelectMode.wangCount     == 1 ? ",开单王"      : ",开双王";
                // str += tData.areaSelectMode.huCount       == 2 ? ",开杠可胡2张" : "";
                ...
                //         break;
                // }
                // str += tData.areaSelectMode.zhuaniaoNum == 0 ? "" : tData.areaSelectMode.niaoScoreType == 1 ? ",中鸟加倍"   : ",中鸟加分";
                // str += tData.areaSelectMode.piaoniao ? ",飘鸟"     : "";  
                // str += tData.areaSelectMode.canChi   ? ",可以吃"   : "";  
                // str += tData.areaSelectMode.chouPai  ? ",抽牌40张," : ""; 
                // tData.areaSelectMode.trustTime = 300; 
                // str += trustTime[tData.areaSelectMode.trustTime];  
                var str = getPlayingRoomInfo(0);
            ...
            _event: {
                MJCanBaoTing: function(){
                    ...
                    for (var uid in sData.players) {
                        ...
                        if (pl.tingStatus == 2 || tData.firstCard && (pl.eatFlag & 8) == 8) {
                            this.visible = true;
                            break ;
                        }
                    }
        ...
        touziNode:{
            ...
            _event:{
                MJZhiTouZi:function(data){
                    ...
                    this.visible = !data.isHaiDi && addnewcard.length == 2;
                    ...
                    firstFrame.visible = !data.isHaiDi && addnewcard.length == 2;
                    ...
                    if(data.isHaiDi || addnewcard.length != 2){
                        postEvent("showCardsOfKaiGang", addnewcard);
                    }else{
        ...
        BtnPutCard:{ //add by  sking for put card button
            ...
            _event:{
                ...
                waitPut: function() {
                    var pl = getUIPlayer(0);
                    var eat = MjClient.playui.jsBind.eat;
                    if (IsTurnToMe() && (pl.isTing || pl.fengDong) && !eat.hu._node.visible && !eat.gang0._node.visible && !eat.gang1._node.visible && !eat.gang2._node.visible) {
                        cc.log("*********自动出牌*********");
                ...
                // MJFengDong: function(msg){
                //     var tData = MjClient.data.sData.tData;
                //     var pl = getUIPlayer(0);
                //     if(SelfUid() == msg.uid && IsTurnToMe() && !tData.firstCard && pl.mjState == TableState.waitPut){
                //         cc.log("玩家封东，自动打出新摸的牌...");
                //         var downNode = MjClient.playui._downNode;
                //         var children = downNode.children;

                //         for(var i = 0; i < children.length; i++)
                //         {
                //             if(children[i].name == "mjhand" && children[i + 1].name != "mjhand"){
                //                 PutOutCard(children[i], children[i].tag); 
                //                 break;
                //             }
                //         }
                //     }
...
/*                        waitHaiDiLao: function()
            ...
            _event: {
                ...
                initSceneData: function(eD) {
                    SetUserVisible_ningXiangKaiWang(this, 0);
                    MjClient.playui.ShowAllTingTips();
                    MjClient.playui.dealFengDong(this, 0, true);
                ...
                mjhand: function(eD) {
                    ...
                    //{
                        InitUserHandUI_ningXiangKaiWang(this, 0);
                    //}
                    //else
                    //{
                ...
                roundEnd: function() {
                    ...
                    //setTaiInfo("");
                    if(MjClient.playui.getChildByName("fengDongTip0")){
                        MjClient.playui.getChildByName("fengDongTip0").removeFromParent();
                    }
                },
                newCard: function(eD) {
                    // cdsNums++;
                ...
                MJPickHaiDi: function(msg) {
                    if(msg.uid == SelfUid() && MjClient.playui.getChildByName("laZhangLayer")){
                        MjClient.playui.removeChildByName("laZhangLayer");
                    }
                },
                MJFengDong: function(msg){
                    if(SelfUid() == msg.uid){
                        MjClient.playui.dealFengDong(this, 0);
                    }
                },
            }
        },
...
/*                        waitHaiDiLao: function()
           ...
           out0: {
            ...
            _event: {
                ...
                initSceneData: function(eD) {
                    SetUserVisible_ningXiangKaiWang(this, 1);
                    MjClient.playui.dealFengDong(this, 1, true);
                },
                addPlayer: function(eD) {
                    SetUserVisible_ningXiangKaiWang(this, 1);
                },
                ...
                showTableEvent: function(isShow){ 
                    InitUserHandUI_ningXiangKaiWang(this, 1);
                },
                mjhand: function(eD) {
                    InitUserHandUI_ningXiangKaiWang(this, 1);
                },
                roundEnd: function() {
                    ...
                    if(MjClient.playui.getChildByName("fengDongTip1")){
                        MjClient.playui.getChildByName("fengDongTip1").removeFromParent();
                    }
                },
                waitPut: function() {
                    var tData = MjClient.data.sData.tData;
                ...
                MJFengDong: function(msg){
                    var tData = MjClient.data.sData.tData;
                    var playerIndex = getPlayerIndex(1);
                    if(tData.uids[playerIndex] == msg.uid){
                        MjClient.playui.dealFengDong(this, 1);
                    }
                },
            }
        },
...
/*                        waitHaiDiLao: function()
           ...
           out0: {
                _run: function () {
                    if (MjClient.size.width / MjClient.size.height >= 1.5)
                    {
                        setWgtLayout(this, [0, 0.0763],[0.5, 1],[6.8, -2.7]);
                    }
                    else
                    {
                        setWgtLayout(this, [0, 0.07],[0.5, 1],[4.8, -2.7]);
                    }
            ...
            out1: {
                _run: function () {
                    if (MjClient.size.width / MjClient.size.height >= 1.5)
                    {
                        setWgtLayout(this, [0, 0.0763],[0.5, 1],[6.8, -3.75]);
                    }
                    else
                    {
                        setWgtLayout(this, [0, 0.07],[0.5, 1],[4.8, -3.75]);
                    }
            ...
            _event: {
                ...
                initSceneData: function(eD) {
                    SetUserVisible_ningXiangKaiWang(this, 2);
                    MjClient.playui.dealFengDong(this, 2, true);
                },
                addPlayer: function(eD) {
                    SetUserVisible_ningXiangKaiWang(this, 2);
                },
                ...
                mjhand: function(eD) {
                    InitUserHandUI_ningXiangKaiWang(this, 2);
                },
                showTableEvent: function(isShow){ 
                    InitUserHandUI_ningXiangKaiWang(this, 2);
                },
                roundEnd: function() {
                    ...
                    if(MjClient.playui.getChildByName("fengDongTip2")){
                        MjClient.playui.getChildByName("fengDongTip2").removeFromParent();
                    }
                },
                waitPut: function() {
                    var tData = MjClient.data.sData.tData;
                ...
                MJFengDong: function(msg){
                    var tData = MjClient.data.sData.tData;
                    var playerIndex = getPlayerIndex(2);
                    if(tData.uids[playerIndex] == msg.uid){
                        MjClient.playui.dealFengDong(this, 2);
                    }
                },
            }
        },
...
/*                        waitHaiDiLao: function()
            ...
            _event: {
                ...
                initSceneData: function(eD) {
                    SetUserVisible_ningXiangKaiWang(this, 3);
                    MjClient.playui.dealFengDong(this, 3, true);
                },
                addPlayer: function(eD) {
                    SetUserVisible_ningXiangKaiWang(this, 3);
                },
                ...
                mjhand: function(eD) {
                    InitUserHandUI_ningXiangKaiWang(this, 3);
                },
                showTableEvent: function(isShow){ 
                    InitUserHandUI_ningXiangKaiWang(this, 3);
                },
                roundEnd: function() {
                    ...
                    if(MjClient.playui.getChildByName("fengDongTip3")){
                        MjClient.playui.getChildByName("fengDongTip3").removeFromParent();
                    }
                },
                waitPut: function() {
                    var tData = MjClient.data.sData.tData;
                ...
                MJFengDong: function(msg){
                    var tData = MjClient.data.sData.tData;
                    var playerIndex = getPlayerIndex(3);
                    if(tData.uids[playerIndex] == msg.uid){
                        MjClient.playui.dealFengDong(this, 3);
                    }
                },
            }
        },
        eat: {
            ...
            kaiGangKuang: {
                ...
                _event:{
                    ...
                    putCardAfterGang: function(eD){
                        ...
                        if(cardNode1.visible == cardNode2.visible && cardNode2.visible == false){
                            this.visible = false;
            ...
            ting: {
                ...
                _touch: function(btn, eT) {
                    if (eT == 2)
                    {
                        ...
                        if((pl.eatFlag & 8) == 8){
                            ...
                            MjClient.showMsg(msg, function()
                            {
                                btn.getParent().getChildByName("hu_qishou").visible = false;
                                btn.getParent().getChildByName("gang0").visible = false;
                                hideTingBtn();
                ...
                _event: {
                    mjhand: function(eD) {
                        setWgtLayout(this,[0, 0.16], [0.54, 0], [0, 1.8]);
                    },
                    MJCanBaoTing: function(eD){
                        ...
                        if(eD.uid == SelfUid() && eD.tingStatus == 2){
                            this.visible = true;
                            if((eD.eatFlag & 8) != 8){
                                this.setPosition(this.getParent().getChildByName("hu_qishou"));
                            }
                        }
                    },
                    MJBaoTing: function(eD){
                        if(eD.uid == SelfUid()){
                            this.visible = false;
                            MjClient.playui.jsBind.eat.gang0._node.visible = false;
                        }
                    },
                    initSceneData: function(){
                        if (!IsArrowVisible()){
                        ...
                        if(tData.firstCard && (pl.eatFlag & 8) != 8){
                            this.setPosition(this.getParent().getChildByName("hu_qishou"));
                        }
                    },
                }
            ...
            hu_qishou: {
                ...
                _event: {
                    MJCanBaoTing: function(eD){
                        if(eD.uid == SelfUid() && (eD.eatFlag & 8) == 8){
                            this.visible = true;
                        }
                    },
                    MJBaoTing: function(eD){
                    ...
                    initSceneData: function(){
                        ...
                        var tData = sData.tData;
                        var pl = sData.players[SelfUid()];
                        if((pl.eatFlag & 8) == 8 && tData.firstCard){
                            this.visible = true;
                        }
                    },
                    clearCardUI: function(eD) {
            ...
            noTing: {
                ...
                _event: {
                    MJCanBaoTing: function(eD){
                        if(eD.uid == SelfUid() && (eD.tingStatus == 2 || (eD.eatFlag & 8) == 8)){
                            this.visible = true;
                        }
                    },
                    MJBaoTing: function(eD){
                    ...
                    initSceneData: function(){
                        ...
                        if(pl.isTing && tData.gangAddCard && tData.gangAddCard.length != 0){
                            for(var index in tData.gangAddCard){
                                if(MjClient.majiang.isHunCard(tData.gangAddCard[index], tData.hunCard)){
                                    isShow = false;   //起手杠到纯王不需要显示过
                                    break ;
                                }
                            }
                        }

                        if(isShow && (pl.tingStatus == 2 || ((pl.eatFlag & 8) == 8 && tData.firstCard))){ 
                            this.visible = true;
                        }
                    },
                    MJFengDong: function(msg){
                        ...
                        var tData = MjClient.data.sData.tData;
                        var pl = getUIPlayer(0);
                        if(pl.tingStatus == 2 || ((pl.eatFlag & 8) == 8 && tData.firstCard)){
                            this.getParent().getChildByName("hu_qishou").visible = false;
                            hideTingBtn();
                            MjClient.majiang.MJTingToServer(SelfUid(), false);
                        }
                    },
            ...
            gang0: {
                ...
                _touch: function(btn, eT) {
                    if (eT == 2) {
                        ...
                        if(MjClient.playui.jsBind.eat.hu._node.visible || MjClient.playui.jsBind.eat.hu_qishou._node.visible)
                        {
                            MjClient.showMsg("确认不胡吗?", function(){
            ...
            guo: {
                ...
                _touch: function(btn, eT) {
                    if (eT == 2)
                    {
                        MjClient.MJPass2NetForningXiangKaiWang();
                    }
                },
                bgimg: {
                    _run: function() {
                ...
                _event: {
                    MJFengDong: function(msg){
                        ...
                        var tData = MjClient.data.sData.tData;
                        var pl = getUIPlayer(0);
                        if(SelfUid() == msg.uid && pl.eatFlag != 0 && !tData.firstCard){
                            MJPassConfirmToServer();
                        }
                    },
                }
            ...
            changeui: {
                ...
                changeuibg: {
                        ...
                        this.gangTouch = function(btn, et) {
                            if (et == 2){
                                if(isTouZi_huaihuaMJ){
                                    ...
                                    MjClient.playui.jsBind.eat.hu_qishou._node.visible = false;
                                    hideTingBtn();
                                }else{
                    ...
                    guobg: {
                        guo: {
                            _touch: function(btn, eT) {
                                if (eT == 2) MjClient.MJPass2NetForningXiangKaiWang();
                            }
                        },
                        fanhui: {
                            _touch: function(btn, et) {
            ...
            _event: {
                ...
                MJOtherGang: function(eD) {
                    console.log("HHH : MJOtherGang------");
                    MjClient.playui.EatVisibleCheck();
                },
                MJHu: function (eD) {
                    hideTingBtn();
        ...
        fengdong_btn: {
            _visible: false,
            _layout: [[0.08, 0.08], [0.97, 0], [0, 3.2]],
            _click: function() {
                MjClient.showMsg("您确认封东吗?",
                function() {
                    MjClient.gamenet.request("pkroom.handler.tableMsg", {
                        cmd: "MJFengDong",
                    }); 
            ...
            _event:{
                ...
                initSceneData: function(){
                    ...
                    if(pl.fengDong){
                        this.visible = false;
                    }else if(TableState.waitPut == pl.mjState || TableState.waitEat == pl.mjState || TableState.waitCard == pl.mjState){
                        this.visible = true;
                    }
                },
            }
    ...
    ctor: function() {
        ...
        var playui = ccs.load("Play_ningXiangKaiWang.json");
        playMusic("bgFight");
...
//封东处理
PlayLayer_ningXiangKaiWang.prototype.dealFengDong = function(node, off, isInitScene) {
    var pl = getUIPlayer(off);

    if(pl && pl.fengDong){
        if(pl.mjState != TableState.waitPut && pl.mjState != TableState.waitEat && pl.mjState != TableState.waitCard){
            return ;
        }

        var fengDongTip = new cc.Sprite("playing/gameTable/fendonging.png");
        fengDongTip.setScale(cc.winSize.height / 720);
        switch(off){
            case 0:
                fengDongTip.setName("fengDongTip0");
                fengDongTip.x = cc.winSize.width / 2;
                fengDongTip.y = node.getChildByName("stand").y;
                break;
            case 1:
                fengDongTip.setName("fengDongTip1");
                fengDongTip.setRotation(-90);
                fengDongTip.x = node.getChildByName("stand").x;
                fengDongTip.y = cc.winSize.height / 1.7;
                break;
            case 2:
                fengDongTip.setName("fengDongTip2");
                fengDongTip.x = cc.winSize.width / 2;
                fengDongTip.y = node.getChildByName("stand").y;
                break;
            case 3:
                fengDongTip.setName("fengDongTip3");
                fengDongTip.setRotation(90);
                fengDongTip.x = node.getChildByName("stand").x;
                fengDongTip.y = cc.winSize.height / 1.7;
                break;    
        }
        MjClient.playui.addChild(fengDongTip);
        MjClient.playui.EatVisibleCheck();
        MjClient.playui.dealCardsAfterFengDong(node, off, isInitScene);
    }
}

//封东处理--->手牌和打出去的牌
PlayLayer_ningXiangKaiWang.prototype.dealCardsAfterFengDong = function(node, off, isInitScene) {
    var pl = getUIPlayer(off);
    if(!pl || !pl.fengDong){
        return ;
    }
    ...
    {
        var card = children[i];
        if(card.name == "out" && isInitScene){
            outCradCount ++;
            if(outCradCount > pl.putIndex){
                if(off == 0 || off == 2){
                    card.loadTexture(getNewMJBgFile("playing/MJ/Mj_04.png"));
                }else{
                    card.loadTexture(getNewMJBgFile("playing/MJ/Mj_06.png"));
                }
                card.removeAllChildren();
            }
        }

        if(card.name == "mjhand" && off == 0){
           card.loadTexture(getNewMJBgFile("playing/MJ/Mj_04.png"));
           card.removeAllChildren();
        }
    }
...
//展示杠开的牌
PlayLayer_ningXiangKaiWang.prototype.showKaiGangBigCards = function(kaiGangKuang, addnewcard) {
    var tData = MjClient.data.sData.tData;
    var isKaiGang = true;
    ...
    if(addnewcard.length == 1){
        cardNode1.x = cardNode2.x - cardNode2.width * 0.5;
        cardNode2.setVisible(false);
    }

    if(addnewcard.length == 2){
        cardNode1.x = cardNode2.x - cardNode2.width;
    }
...
// 显示当前局数增加的分数
PlayLayer_ningXiangKaiWang.prototype.NowRoundScore = function(node,off,isShow) { 
    var pl = getUIPlayer(off);
    if(!pl)return; 

    if (MjClient.data.sData.tData.areaSelectMode.piaoniao)
        return;
...
PlayLayer_ningXiangKaiWang.prototype.CardLayoutRestore = function(node, off, isNewCard)
{
    ...
    for(var i = 0; i < orders.length; i++)
    {
        ...
        if(off % 2 == 0)//自己或者对家
        {
            ...
            if(off == 0)
            {
                ...
                var isGray =  pl.isTing && ci.name == "mjhand";
                var isFengDong = pl.fengDong && ci.name == "mjhand";
                //if(ci.name == "mjhand")
                ...
                else if(i == orders.length - 1)
                {
                    ...
                    if(newC)
                    {
                        ...
                        if (isGray || isFengDong) // 岳阳APP不自动提起新牌
                            ci.y += 20;
...
// 判断吃碰杠胡的状态
PlayLayer_ningXiangKaiWang.prototype.EatVisibleCheck = function(isKaiGang)
{
    isTouZi_huaihuaMJ = false;
    var eat = MjClient.playui.jsBind.eat;
    ...
    if(tData.tState == TableState.waitPut && pl.mjState == TableState.waitPut)
    {
        if(IsTurnToMe())
        {
            ...
            //检测死手
            var isSiShou = true;
            for(var mjhandIndex in pl.mjhand){
                if(!MjClient.majiang.isHunCard(pl.mjhand[mjhandIndex], tData.hunCard)){
                    isSiShou = false;
                    break ;
                }
            }
            if(pl.isTing && MjClient.majiang.isHunCard(pl.newCd, tData.hunCard)){
                isSiShou = true;
            }
            if(pl.isTing && isKaiGang){
                for(var index in tData.gangAddCard){
                    if(MjClient.majiang.isHunCard(tData.gangAddCard[index], tData.hunCard)){
                        isSiShou = true;
                        break ;
            ...
            //胡
            if (!tData.firstCard && pl.isNew && (pl.eatFlag & 8)) {
                pl.isZiMoHu = true;
                _bIsZiMo = true;
                vnode.push(eat.hu._node);
            }
            if(qiShouBtnNum == 0 && vnode.length > 0 && !isSiShou)
            {
                //if(!_bIsZiMo) 
    ...
    else if(tData.tState == TableState.waitEat)
    {
        ...
        if(!IsTurnToMe())
        {
            ...
            if (!tData.firstCard && (pl.eatFlag & 8)) {
                vnode.push(eat.hu._node);
    ...
    //庄家起手的杠位置特殊处理
    if(tData.firstCard && SelfUid() == tData.uids[tData.zhuang] && vnode.length == 1 && qiShouBtnNum != 0) {
        vnode[0].x = MjClient.playui.jsBind.eat.hu_qishou._node.x
        vnode[0].x -= (MjClient.playui.jsBind.eat.noTing._node.x - MjClient.playui.jsBind.eat.hu_qishou._node.x) * qiShouBtnNum;
    }
    ...
    MjClient.playui.ShowAllTingTips();
    MjClient.playui.refreshNewOut();
}

//刷新被隐藏掉的新打出的牌
PlayLayer_ningXiangKaiWang.prototype.refreshNewOut = function ()
{
    var children1 = MjClient.playui._downNode.children;
    var children2 = MjClient.playui._rightNode.children;
    var children3 = MjClient.playui._topNode.children;
    var children4 = MjClient.playui._leftNode.children;
    var children = children1.concat(children2,children3,children4);
...
PlayLayer_ningXiangKaiWang.prototype.ShowAllTingTips = function ()
{
    if (!MjClient.data.sData.tData.areaSelectMode.isOpenTingTip)
        return;
    ...
    var children = downNode.children;
    var pl = getUIPlayer(0);
    if(pl.fengDong) return;

    var canTingCardsByTag = {};
    var canTingCardsByNode = [];
    for (var i = 0; i < children.length; i++)
    {
        if(children[i].name == "mjhand")
        {
            ...
            var temp_tag = card.tag;
            var tingCards = MjClient.majiang.getCheckTingHuCards(temp_tag, pl.mjhand, true);
            if (Object.keys(tingCards).length > 0) {
                canTingCardsByTag[temp_tag] = 1;
                canTingCardsByNode.push(children[i]);
            }
        }
    }

    var tingMostCard = calculateTotalCardsOfTingHu(canTingCardsByTag, pl.mjhand);
    for (var i = 0; i < canTingCardsByNode.length; i++) {
        var card = canTingCardsByNode[i];
        var ting_tips = new ccui.ImageView();
        if (tingMostCard[card.tag] > 0) {
            ting_tips.loadTexture("playing/other/tingcardmost.png");
        } else {
            ting_tips.loadTexture("playing/MJ/ting_tips.png");
        }
