...
function SetEndOneUserUI_ningXiangKaiWang(node,off)
{
	var sData=MjClient.data.sData;
	var tData=sData.tData;
		...
		, winNum: {
            ...
            niao_card: {
                ...
                _run:function(){
                    ...
                    var zhongCountStartX = 0;
                    for(var i = 0;i < cards.length;i++)
                    {
                        var zhongNiao = cards[i] % 10 % tData.maxPlayer;
                        if(zhongNiao == 0){
                            zhongNiao = tData.maxPlayer;
                        }

                        if (zhongNiao == (off - tData.winner + tData.maxPlayer) % tData.maxPlayer + 1)
                        {
                            zhongCountStartX++;
                        }
                    }
                    ...
                    var startX = this.getParent().getChildByName("hu").x - slotwith * 3 - ((slotwith * zhongCountStartX + 1) / 2);

                    for(var i = 0;i < cards.length;i++)
                    {
                        var cardToNiao = cards[i] % 10 % tData.maxPlayer;
                        if(cardToNiao == 0){
                            cardToNiao = tData.maxPlayer;
                        }

                        if (cardToNiao == (off - tData.winner + tData.maxPlayer) % tData.maxPlayer + 1)
                        {
                            zhongCount++;
                        }else{
                            continue ;
                        ...
                        _node.visible = true;
                        _node.setPosition(cc.p(startX + slotwith*(zhongCount - 1),this.y));
                    ...
                    niao_count.x = startX + zhongCountStartX / 2 * slotwith;
                }
            },
		}
	}
...
var EndOneView_ningXiangKaiWang = cc.Layer.extend({
	jsBind:{
		block: {
		...
		head0:{
		   ...
		   _run:function(){ SetEndOneUserUI_ningXiangKaiWang(this,0); },
		...
		,head1:{ 
		  ...
		  _run:function(){ SetEndOneUserUI_ningXiangKaiWang(this,1); }
		}
		...
		,head2:{ 
		   ...
		   _run:function(){ SetEndOneUserUI_ningXiangKaiWang(this,2); }
		}
		,head3:{
			 head:
			 {
			...
			_run:function(){ SetEndOneUserUI_ningXiangKaiWang(this,3); }
			},
            count_down:{
                _visible :function()
                {
    ...
    ctor:function () {
        this._super();
        var endoneui = ccs.load("endOne_ningXiangKaiWang.json");
