...
(function() {
    ...
    majiang.isHunCard = function(cardTag){
        var tData = MjClient.data.sData.tData;
        if(cardTag != tData.hunCard){
            return false;
        }
        return true;
    };
    ...
    majiang.calTingSet  = function (oCds, hun, isReturn)
    {
        ...
        if(tData.areaSelectMode.hongzhong){
    ...
    majiang.getIsShowTingTips = function(tingCards, mjhand, isHunCard){
        ...
        if(typeof(tingCards) == "number") return true;

        // 没有可听牌
        if(Object.keys(tingCards).length <= 0) return false;

        // 是赖子，不显示
        if(isHunCard) return false;
