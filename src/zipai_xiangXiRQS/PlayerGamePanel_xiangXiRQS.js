...
function SetUserVisible_xiangXiRQS(node, off){
    ...
    if(pl){
        ...
        setUserOffline_xiangXiRQS(node, off);
...
function InitUserHandUI_xiangXiRQS(node, off){
    ...
    if (pl.jiazhuNum == 2 && MjClient.rePlayVideo == -1)
    {
        ...
        MjClient.majiang.setJiaZhuNum(node, getUIPlayer_xiangXiRQS(off));
    }
...
var PlayLayer_xiangXiRQS = cc.Layer.extend({
    jsBind: {
        _event: {
            ...
            endRoom: function(msg) {
                ...
                if (msg.showEnd){
                    this.addChild(new GameOverLayer_xiangXiRQS(),500);
                }else{
            ...
            roundEnd: function() {
                ...
                if(!MjClient.isDismiss && tData.hunCard && tData.hunCard != -1 && tData.areaSelectMode.xingType==2){
                    ...
                    ShowEatActionAnim_xiangXiRQS(MjClient.playui._downNode, ActionType_paohuzi.FANXING, 0);
                    DealFanXingNewCard_xiangXiRQS(hunCard);
                }
                ...
                function delayExe(){
                    ...
                    if (sData.tData.roundNum <= 0){
                        var layer = new GameOverLayer_xiangXiRQS();
                        layer.setVisible(false);
                        self.addChild(layer,500);
                    }
                    if(!MjClient.endoneui){
                        self.addChild(new EndOneView_xiangXiRQS(),500);
                    }
            ...
            moveHead: function() {
                ...
                //tableStartHeadMoveAction_xiangXiRQS(this);   //不涉及到头像移动动作
            },
            initSceneData: function() {
                ...
                reConectHeadLayout_xiangXiRQS(this);        //不涉及到头像移动动作
                CheckRoomUiDelete();
                tableStartHeadMoveAction_xiangXiRQS(this);   //不涉及到头像移动动作
            ...
            changeMJBgEvent: function() {
                changeMJBg_xiangXiRQS(this, ziPai.getZiPaiType());
            },
            MJPeng: function() {
                MjClient.playui.showAndHideHeadEffect();
            },
        ...
        cardNumImg: {
            ...
            _event: {
                initSceneData: function(eD) {
                    var sData = MjClient.data.sData;
                    var tData = sData.tData;
                    this.visible = IsArrowVisible_xiangXiRQS() && TableState.waitJiazhu != tData.tState;
                },
                mjhand: function(eD) {
                    var sData = MjClient.data.sData;
                    var tData = sData.tData;
        ...
        banner: {
            ...
            getRoomNum: {
                ...
                _click: function() {
                    ...
                    if (tData.areaSelectMode.zimoAddFen){
                        infos.push("自摸加1");
                    }

                    if (MjClient.GAME_TYPE.XIANG_XI_2710 == MjClient.gameType){
                        if (item = ["无庄分", "2分", "5分", "10分"][tData.areaSelectMode.zhuangFen]){
                            infos.push(item);
                        }
                    }
                    if(tData.areaSelectMode.jieSuanDiFen)
                        infos.push("底分x"+tData.areaSelectMode.jieSuanDiFen);
                    ...
                    var sData = MjClient.data.sData;
                    var str7 = " ";
                    if (Object.keys(sData.players).length > 0 && MjClient.MaxPlayerNum_xiangXiRQS > Object.keys(sData.players).length){
                        str7 += ["一","二","三"][Object.keys(sData.players).length - 1] + "缺" + ["一","二","三"][MjClient.MaxPlayerNum_xiangXiRQS - Object.keys(sData.players).length - 1];
                    }
            ...
            gps_btn: {
                ...
                _click: function() {
                    if(MjClient.MaxPlayerNum_xiangXiRQS == 3){
            ...
            roundnumImg: {
                ...
                _event: {
                    initSceneData: function(eD) {
                        this.visible = IsArrowVisible_xiangXiRQS();
                    },
                    mjhand: function(eD) {
                        this.visible = IsArrowVisible_xiangXiRQS();
                    },
        ...
        wait: {
            ...
            backHomebtn: {
                ...
                _event: {
                    ...
                    initSceneData: function(eD) {
                        this.visible = isShowBackBtn_xiangXiRQS();
                    },
                    removePlayer: function(eD) {
                        this.visible = isShowBackBtn_xiangXiRQS();
                    },
        ...
        btns_piao:{
            ...
            _event: {
                initSceneData: function() {
                    ...
                    var tData = MjClient.data.sData.tData;
                    if(!pl) return;
                    if (tData.tState == TableState.waitJiazhu && pl.piaoFen == -1 && tData.areaSelectMode.chongFen) {
                        this.visible = true;
                        var chooseArr = [0,2,5,10];
                        for (var i = 0; i < 4; ++ i){
                            var btn = this.getChildByName("btn_piao" + i);
                            if (btn){
                                btn.loadTextureNormal("playing/chenzhouzipai/chong" + chooseArr[i] + ".png");
                                btn.loadTexturePressed("playing/chenzhouzipai/chong" + chooseArr[i] + "_s.png");
                            }
                        }
                    }
                },
                waitJiazhu:function (msg) {
                    var tData = MjClient.data.sData.tData;
                    if ( tData.tState == TableState.waitJiazhu && tData.areaSelectMode.chongFen) {
                        this.visible = true;
                        var chooseArr = [0,2,5,10];
                        for (var i = 0; i < 4; ++ i){
                            var btn = this.getChildByName("btn_piao" + i);
        ...
        down: {
            head: {
                chuizi_tips : {
                    _run: function(){
                        setJiachuiImgTips_xiangXiRQS(this, 0);
                    },
                    _event: {
                        initSceneData: function() {
                            setJiachuiImgTips_xiangXiRQS(this, 0);
                ...
                jiachuiText_tips : {
                    _run: function() {
                        setJiachuiTextTips_xiangXiRQS(this, 0);
                    },
                    _event:{
                        initSceneData: function() {
                            setJiachuiTextTips_xiangXiRQS(this, 0);
                ...
                zhuang: {
                    ...
                    _event: {
                        waitPut: function(){
                            showUserZhuangLogo_xiangXiRQS(this, 0);
                        },
                        initSceneData: function(){
                            if (IsArrowVisible_xiangXiRQS()) showUserZhuangLogo_xiangXiRQS(this, 0);
                        },
                        mjhand: function(){
                            if (IsArrowVisible_xiangXiRQS()) showUserZhuangLogo_xiangXiRQS(this, 0);
                        }
                    ...
                    chattext: {
                        _event: {
                            ...
                            MJChat: function(msg) {
                                ...
                                showUserChat_xiangXiRQS(this, 0, msg);
                            },
                            playVoice: function(voicePath) {
                                MjClient.data._tempMessage.msg = voicePath;
                                showUserChat_xiangXiRQS(this, 0, MjClient.data._tempMessage);
                            }
                        }
                    }
                },
                _click: function(btn) {
                    showPlayerInfo_xiangXiRQS(0, btn);
                },
                _event: {
                    loadWxHead: function(d) {
                        setWxHead_xiangXiRQS(this, d, 0);
                    },
                    addPlayer: function(eD) {
                        showFangzhuTagIcon_xiangXiRQS(this,0);
                    },
                    removePlayer: function(eD) {
                        showFangzhuTagIcon_xiangXiRQS(this,0);
                    }
                ...
                huxi:{
                    ...
                    _event:{
                        ...
                        initSceneData:function(){
                            // this.visible = true;
                            // var huXi = UpdateHuXi_xiangXiRQS(0);
                            // if(!IsArrowVisible_xiangXiRQS()){
                            //     huXi = 0;
                            //     this.visible = false;
                            // }
                ...
                timeImg:{
                    ...
                    // _event:{
                    //     initSceneData: function(eD) {
                    //         this.visible = curPlayerIsMe_xiangXiRQS(0);
                    //     },
                    //     mjhand: function(eD) {
                    //         this.visible = curPlayerIsMe_xiangXiRQS(0);
                    //     },
                    //     onlinePlayer: function(eD) {
                    //         this.visible = curPlayerIsMe_xiangXiRQS(0) && IsArrowVisible_xiangXiRQS();
                    //     },
                    //     waitPut: function(eD) {
                    //         this.visible = curPlayerIsMe_xiangXiRQS(0);
                    //     },
                    ...
                    //             playTimeUpEff = null;
                    //             var pl = getUIPlayer_xiangXiRQS(0);
                    //             if(pl && pl.onLine && curPlayerIsMe_xiangXiRQS(0)){
                    //                 arrowbkNumberUpdate(this);
                    //             }
                    //         },
                    //         roundEnd: function() {
            ...
            ready: {
                ...
                _run: function() {
                    GetReadyVisible_xiangXiRQS(this, 0);
                },
                _event: {
                    startShuffleCards:function (d) {
                        GetReadyVisible_xiangXiRQS(this, -1);
                    },
                    moveHead: function() {
                        GetReadyVisible_xiangXiRQS(this, -1);
                    },
                    ...
                    addPlayer: function() {
                        GetReadyVisible_xiangXiRQS(this, 0);//根据状态设置ready 是否可见 add by sking
                    },
                    removePlayer: function() {
                        GetReadyVisible_xiangXiRQS(this, 0);
                    },
            ...
            handNode: {
                ...
                _event:  {
                    initSceneData: function() {
                        calculateHintPutList_xiangXiRQS();
                    },
                    HZPickCard: function() {
                        calculateHintPutList_xiangXiRQS();
                    },
                    ...
                    MJPass: function() { // 天胡 提 偎 跑后过胡
                        calculateHintPutList_xiangXiRQS();
                        addTingSign_xiangXiRQS(MjClient.playui._downNode);
                    },
                    HZCheckRaise: function() {
                        calculateHintPutList_xiangXiRQS();
                        addTingSign_xiangXiRQS(MjClient.playui._downNode);
            ...
            outNode_New: {
                _visible: false,
                _layout : [[0.14, 0.14], [1-0.005, 0.005], [0, 0]],
                _run: function(){
                    if(MjClient.MaxPlayerNum_xiangXiRQS == 2){
                        var qiPaiNode = this.getChildByName("qiPaiNode");
                        qiPaiNode.setBackGroundImage("playing/gameTable/qiPaiBGLeft_Two.png");
                        qiPaiNode.height += qiPaiNode.height / 2;
                        //qiPaiNode.y += qiPaiNode.height / 3;
                        this.getChildByName("Text_qi").y = qiPaiNode.y + qiPaiNode.height / 2;

                        // var chuPaiNode = this.getChildByName("chuPaiNode");
                        // chuPaiNode.setBackGroundImage("playing/gameTable/qiPaiBGLeft_Two.png");
                        // chuPaiNode.height += chuPaiNode.height / 2;
                        // this.getChildByName("Text_chu").y = chuPaiNode.y + chuPaiNode.height / 2;
                    }
                },
                _event: {
                    mjhand: function(){
                        this.getChildByName("qiPaiNode").removeAllChildren();
                        this.getChildByName("chuPaiNode").removeAllChildren();
                        this.visible = true;
                    },
                    clearCardUI: function(){
                        this.getChildByName("qiPaiNode").removeAllChildren();
            ...
            _event: {
                ...
                initSceneData: function(eD) {
                    SetUserVisible_xiangXiRQS(this, 0);
                    InitUserHandUI_xiangXiRQS(this, 0);
                    InitUserCoinAndName_xiangXiRQS(this, 0);
                    MjClient.hasPut = false; // 重连回来重置打掉牌标志（打牌时候断线)
                    ShowPutCardIcon_xiangXiRQS();
                    DealOffLineCard_xiangXiRQS(this,0);
                },
                addPlayer: function(eD) {
                    SetUserVisible_xiangXiRQS(this, 0);
                    InitUserCoinAndName_xiangXiRQS(this, 0);
                ...
                mjhand: function(eD) {
                    ...
                    var uid = getUIPlayer_xiangXiRQS(0).info.uid;
                    // if (eD.jiazhuNums)
                    // {
                ...
                HZNewCard: function(eD) {
                    ...
                    DealNewCard_xiangXiRQS(this,eD,0);
                },
                MJPut: function(eD) {
                    DealPutCard_xiangXiRQS(this,eD,0);
                },
                HZChiCard: function(eD) {
                    DealChiCard_xiangXiRQS(this, eD, 0);
                },
                HZWeiCard: function(eD){
                    DealWeiCard_xiangXiRQS(this,eD,0);
                },
                HZGangCard: function(eD) {
                    DealGangCard_xiangXiRQS(this, eD, 0);
                },
                MJPeng: function(eD) {
                    DealPengCard_xiangXiRQS(this, eD, 0);
                },
                MJHu: function(eD) {
                    DealHu_xiangXiRQS(this, eD, 0);
                },
                onlinePlayer: function(eD) {
                    resetHandAfterBegin_xiangXiRQS();
                    setUserOffline_xiangXiRQS(this, 0);
                },
                ...
                MJJiazhu: function(msg) {
                    ...
                    MjClient.majiang.setJiaZhuNum(MjClient.playui._downNode, getUIPlayer_xiangXiRQS(0));
                    MjClient.majiang.setJiaZhuNum(MjClient.playui._rightNode, getUIPlayer_xiangXiRQS(1));
                    MjClient.majiang.setJiaZhuNum(MjClient.playui._topNode, getUIPlayer_xiangXiRQS(2));
                },
                HZPickCard:function (eD) {
                    ...
                    DealShowLastCard_xiangXiRQS(this, eD, 0);
                },
                HZAddCards:function (eD) {
                    DealAddCard_xiangXiRQS(this,eD, 0);
                },
        ...
        right: {
            ...
            head: {
                chuizi_tips : {
                    _run: function(){
                        ...
                        setJiachuiImgTips_xiangXiRQS(this, changeUIOff_xiangXiRQS(2));
                    },
                    _event: {
                        initSceneData: function() {
                            if(MjClient.MaxPlayerNum_xiangXiRQS == 2) {
                ...
                jiachuiText_tips : {
                    _run:function() {
                        ...
                        setJiachuiTextTips_xiangXiRQS(this, changeUIOff_xiangXiRQS(2));
                    },
                    _event:{
                        initSceneData: function() {
                            if(MjClient.MaxPlayerNum_xiangXiRQS == 2) {
                        ...
                        MJJiazhu: function(msg) {
                            ...
                            var pl = getUIPlayer_xiangXiRQS(changeUIOff_xiangXiRQS(2))
                            if (msg.uid == pl.info.uid) {
                                setJiachuiTextTips_xiangXiRQS(this, changeUIOff_xiangXiRQS(2));
                            }
                        },
                ...
                zhuang: {
                    ...
                    _event: {
                        waitPut: function() {
                            ...
                            showUserZhuangLogo_xiangXiRQS(this, changeUIOff_xiangXiRQS(2));
                        },
                        initSceneData: function() {
                            if(MjClient.MaxPlayerNum_xiangXiRQS == 2) {
                                return ;
                            }
                            if (IsArrowVisible_xiangXiRQS()) showUserZhuangLogo_xiangXiRQS(this, changeUIOff_xiangXiRQS(2));
                        },
                        mjhand: function(){
                            if(MjClient.MaxPlayerNum_xiangXiRQS == 2) {
                                return ;
                    ...
                    chattext: {
                        _event: {
                            MJChat: function(msg) {
                                ...
                                showUserChat_xiangXiRQS(this, changeUIOff_xiangXiRQS(2), msg);
                            },
                            playVoice: function(voicePath) {
                                if(MjClient.MaxPlayerNum_xiangXiRQS == 2) {
                                    return ;
                                ...
                                MjClient.data._tempMessage.msg = voicePath;
                                showUserChat_xiangXiRQS(this, changeUIOff_xiangXiRQS(2), MjClient.data._tempMessage);
                            }
                        }
                    }
                },
                _click: function(btn) {
                    ...
                    showPlayerInfo_xiangXiRQS(changeUIOff_xiangXiRQS(2), btn);
                },
                _event: {
                    loadWxHead: function(d) {
                        if(MjClient.MaxPlayerNum_xiangXiRQS == 2) {
                        ...
                        setWxHead_xiangXiRQS(this, d, changeUIOff_xiangXiRQS(2));
                    },
                    addPlayer: function(eD) {
                        if(MjClient.MaxPlayerNum_xiangXiRQS == 2) {
                            return ;
                        }
                        showFangzhuTagIcon_xiangXiRQS(this,changeUIOff_xiangXiRQS(2));
                    },
                    removePlayer: function(eD) {
                        if(MjClient.MaxPlayerNum_xiangXiRQS == 2) {
                            return ;
                ...
                huxi:{
                    ...
                    _event:{
                        ...
                        initSceneData:function(){
                            // this.visible = true;
                            // var huXi = UpdateHuXi_xiangXiRQS(changeUIOff_xiangXiRQS(2));
                            // if(!IsArrowVisible_xiangXiRQS()){
                            //     huXi = 0;
                            //     this.visible = false;
                            // }
                ...
                timeImg:{
                    ...
                    // _event:{
                    //     initSceneData: function(eD) {
                    //         this.visible = curPlayerIsMe_xiangXiRQS(1);
                    //     },
                    //     mjhand: function(eD) {
                    //         this.visible = curPlayerIsMe_xiangXiRQS(1);
                    //     },
                    //     onlinePlayer: function(eD) {
                    //         this.visible = curPlayerIsMe_xiangXiRQS(1) && IsArrowVisible_xiangXiRQS();
                    //     },
                    //     waitPut: function(eD) {
                    //         this.visible = curPlayerIsMe_xiangXiRQS(1);
                    //     },
                    ...
                    //             stopEffect(playTimeUpEff);
                    //             playTimeUpEff = null;
                    //             var pl = getUIPlayer_xiangXiRQS(1);
                    //             if(pl && pl.onLine && curPlayerIsMe_xiangXiRQS(1)){
                    //                 arrowbkNumberUpdate(this);
                    //             }
                    //         },
                    //         roundEnd: function() {
            ...
            ready: {
                ...
                _run: function() {
                    ...
                    GetReadyVisible_xiangXiRQS(this, changeUIOff_xiangXiRQS(2));
                },
                _event: {
                    startShuffleCards:function (d) {
                        if(MjClient.MaxPlayerNum_xiangXiRQS == 2) {
            ...
            _event: {
                clearCardUI: function() {
                    ...
                    clearCardUI_xiangXiRQS(this, changeUIOff_xiangXiRQS(2));
                },
                clearCardArr: function(){
                    if(MjClient.MaxPlayerNum_xiangXiRQS == 2) {
                        return ;
                ...
                initSceneData: function(eD) {
                    ...
                    SetUserVisible_xiangXiRQS(this, changeUIOff_xiangXiRQS(2));
                    InitUserHandUI_xiangXiRQS(this, changeUIOff_xiangXiRQS(2));
                    InitUserCoinAndName_xiangXiRQS(this, changeUIOff_xiangXiRQS(2));
                    DealOffLineCard_xiangXiRQS(this,changeUIOff_xiangXiRQS(2));
                },
                addPlayer: function(eD) {
                    if(MjClient.MaxPlayerNum_xiangXiRQS == 2) {
                        return ;
                ...
                HZNewCard: function(eD) {
                    ...
                    DealNewCard_xiangXiRQS(this,eD,changeUIOff_xiangXiRQS(2));
                },
                roundEnd: function() {
                    if(MjClient.MaxPlayerNum_xiangXiRQS == 2) {
                        return ;
                    ...
                    InitUserCoinAndName_xiangXiRQS(this, changeUIOff_xiangXiRQS(2));
                    MjClient.playui.ResetOtherCard(this, changeUIOff_xiangXiRQS(2));
                },
                waitPut: function(eD) {
                    if(MjClient.MaxPlayerNum_xiangXiRQS == 2) {
                        return ;
                ...
                MJPut: function(eD) {
                    ...
                    DealPutCard_xiangXiRQS(this, eD, changeUIOff_xiangXiRQS(2));
                },
                HZChiCard: function(eD) {
                    if(MjClient.MaxPlayerNum_xiangXiRQS == 2) {
                        return ;
                    ...
                    DealChiCard_xiangXiRQS(this, eD, changeUIOff_xiangXiRQS(2));
                },
                HZGangCard: function(eD) {
                    if(MjClient.MaxPlayerNum_xiangXiRQS == 2) {
                        return ;
                    ...
                    DealGangCard_xiangXiRQS(this, eD, changeUIOff_xiangXiRQS(2));
                },
                MJPeng: function(eD) {
                    if(MjClient.MaxPlayerNum_xiangXiRQS == 2) {
                        return ;
                    ...
                    DealPengCard_xiangXiRQS(this, eD, changeUIOff_xiangXiRQS(2));
                },
                HZWeiCard: function(eD){
                    if(MjClient.MaxPlayerNum_xiangXiRQS == 2) {
                        return ;
                    ...
                    DealWeiCard_xiangXiRQS(this, eD, changeUIOff_xiangXiRQS(2));
                },
                MJHu: function(eD) {
                    if(MjClient.MaxPlayerNum_xiangXiRQS == 2) {
                        return ;
                    ...
                    DealHu_xiangXiRQS(this, eD, changeUIOff_xiangXiRQS(2));
                },
                onlinePlayer: function(eD) {
                    if(MjClient.MaxPlayerNum_xiangXiRQS == 2) {
                        return ;
                    ...
                    setUserOffline_xiangXiRQS(this, changeUIOff_xiangXiRQS(2));
                },
                playerStatusChange: function(eD) {
                    if(MjClient.MaxPlayerNum_xiangXiRQS == 2) {
                        return ;
                ...
                MJFlower: function(eD) {
                    ...
                    HandleMJFlower(this, eD, changeUIOff_xiangXiRQS(2));
                },
                MJTing: function (eD) {
                    if(MjClient.MaxPlayerNum_xiangXiRQS == 2) {
                        return ;
                    ...
                    HandleMJTing(this, eD, changeUIOff_xiangXiRQS(2));
                },
                HZPickCard:function (eD) {
                    if(MjClient.MaxPlayerNum_xiangXiRQS == 2) {
                        return ;
                    ...
                    if(MjClient.rePlayVideo != -1){
                        MjClient.playui._doPickResetHandCardForPlayBack(eD, changeUIOff_xiangXiRQS(2));
                    }

                    // RemovePutCardOut_xiangXiRQS(true);
                    // MjClient.playui.ResetHandCard(this,1);

                    DealShowLastCard_xiangXiRQS(this, eD, changeUIOff_xiangXiRQS(2));
                },
                HZAddCards:function (eD) {
                    if(MjClient.MaxPlayerNum_xiangXiRQS == 2) {
                        return ;
                    ...
                    DealAddCard_xiangXiRQS(this,eD, changeUIOff_xiangXiRQS(2));
                },
                HZCheckRaise:function (msg) {
                    if(MjClient.MaxPlayerNum_xiangXiRQS == 2) {
                        return ;
        ...
        left: {
            head: {
                chuizi_tips : {
                    _run: function(){
                        setJiachuiImgTips_xiangXiRQS(this, changeUIOff_xiangXiRQS(3));
                    },
                    _event: {
                        initSceneData: function() {
                            setJiachuiImgTips_xiangXiRQS(this, changeUIOff_xiangXiRQS(3));
                ...
                jiachuiText_tips : {
                    _visible:false,
                    _run:function() {
                        setJiachuiTextTips_xiangXiRQS(this, changeUIOff_xiangXiRQS(3));
                    },
                    _event:{
                        initSceneData: function() {
                            setJiachuiTextTips_xiangXiRQS(this, changeUIOff_xiangXiRQS(3));
                        ...
                        MJJiazhu: function(msg) {
                            var pl = getUIPlayer_xiangXiRQS(changeUIOff_xiangXiRQS(3))
                            if (msg.uid == pl.info.uid) {
                                setJiachuiTextTips_xiangXiRQS(this, changeUIOff_xiangXiRQS(3));
                            }
                        },
                ...
                zhuang: {
                    ...
                    _event: {
                        waitPut: function() {
                            showUserZhuangLogo_xiangXiRQS(this, changeUIOff_xiangXiRQS(3));
                        },
                        initSceneData: function() {
                            if (IsArrowVisible_xiangXiRQS()) showUserZhuangLogo_xiangXiRQS(this, changeUIOff_xiangXiRQS(3));
                        },
                        mjhand: function(){
                            if (IsArrowVisible_xiangXiRQS()) showUserZhuangLogo_xiangXiRQS(this, changeUIOff_xiangXiRQS(3));
                        }
                ...
                chatbg: {
                    ...
                    chattext: {
                        _event: {
                            MJChat: function(msg) {
                                showUserChat_xiangXiRQS(this, changeUIOff_xiangXiRQS(3), msg);
                            },
                            playVoice: function(voicePath) {
                                MjClient.data._tempMessage.msg = voicePath;
                                showUserChat_xiangXiRQS(this, changeUIOff_xiangXiRQS(3), MjClient.data._tempMessage);
                            }
                        }
                    }
                },
                _click: function(btn) {
                    showPlayerInfo_xiangXiRQS(changeUIOff_xiangXiRQS(3), btn);
                },
                _event: {
                    loadWxHead: function(d) {
                        setWxHead_xiangXiRQS(this, d, changeUIOff_xiangXiRQS(3));
                    },
                    addPlayer: function(eD) {
                        showFangzhuTagIcon_xiangXiRQS(this,changeUIOff_xiangXiRQS(3));
                    },
                    removePlayer: function(eD) {
                        showFangzhuTagIcon_xiangXiRQS(this,changeUIOff_xiangXiRQS(3));
                    }
                ...
                huxi:{
                    ...
                    _event:{
                        ...
                        initSceneData:function(){
                            // this.visible = true;
                            // var huXi = UpdateHuXi_xiangXiRQS(changeUIOff_xiangXiRQS(3));
                            // if(!IsArrowVisible_xiangXiRQS()){
                            //     huXi = 0;
                            //     this.visible = false;
                            // }
                ...
                timeImg:{
                    ...
                    // _event:{
                    //     initSceneData: function(eD) {
                    //         this.visible = curPlayerIsMe_xiangXiRQS(2);
                    //     },
                    //     mjhand: function(eD) {
                    //         this.visible = curPlayerIsMe_xiangXiRQS(2);
                    //     },
                    //     onlinePlayer: function(eD) {
                    //         this.visible = curPlayerIsMe_xiangXiRQS(2) && IsArrowVisible_xiangXiRQS();
                    //     },
                    //     waitPut: function(eD) {
                    //         this.visible = curPlayerIsMe_xiangXiRQS(2);
                    //     },
                    ...
                    //             stopEffect(playTimeUpEff);
                    //             playTimeUpEff = null;
                    //             var pl = getUIPlayer_xiangXiRQS(2);
                    //             if(pl && pl.onLine && curPlayerIsMe_xiangXiRQS(2)){
                    //                 arrowbkNumberUpdate(this);
                    //             }
                    //         },
                    //         roundEnd: function() {
            ...
            ready: {
                _layout: [[0.07, 0.07],[0.5, 0.5],[-2.5, 2.5]],
                _run: function() {
                    GetReadyVisible_xiangXiRQS(this, changeUIOff_xiangXiRQS(3));
                },
                _event: {
                    startShuffleCards:function (d) {
                        GetReadyVisible_xiangXiRQS(this, -1);
            ...
            outNode_New: {
                ...
                _run: function(){
                    if(MjClient.MaxPlayerNum_xiangXiRQS == 2){
                        var qiPaiNode = this.getChildByName("qiPaiNode");
                        qiPaiNode.setBackGroundImage("playing/gameTable/qiPaiBGRight_Two.png");
                        qiPaiNode.height += qiPaiNode.height / 2;
                        this.getChildByName("Text_qi").y = qiPaiNode.y - qiPaiNode.height / 2;

                        var chuPaiNode = this.getChildByName("chuPaiNode");
                        //chuPaiNode.setBackGroundImage("playing/gameTable/qiPaiBGRight_Two.png");
                        //chuPaiNode.height += chuPaiNode.height / 2;
                        chuPaiNode.y -= qiPaiNode.height / 3;
                        this.getChildByName("Text_chu").y = chuPaiNode.y - chuPaiNode.height / 2;
                    }
                },
                _event: {
            ...
            _event: {
                clearCardUI: function() {
                    clearCardUI_xiangXiRQS(this, changeUIOff_xiangXiRQS(3));
                },
                clearCardArr: function(){
                    var eatNode = this.getChildByName("eatNode");
                    eatNode.removeAllChildren();
                ...
                initSceneData: function(eD) {
                    SetUserVisible_xiangXiRQS(this, changeUIOff_xiangXiRQS(3));
                    InitUserHandUI_xiangXiRQS(this, changeUIOff_xiangXiRQS(3));
                    InitUserCoinAndName_xiangXiRQS(this, changeUIOff_xiangXiRQS(3));
                    DealOffLineCard_xiangXiRQS(this,changeUIOff_xiangXiRQS(3));
                },
                addPlayer: function(eD) {
                    SetUserVisible_xiangXiRQS(this, changeUIOff_xiangXiRQS(3));
                    InitUserCoinAndName_xiangXiRQS(this, changeUIOff_xiangXiRQS(3));
                ...
                HZNewCard: function(eD) {
                    ...
                    DealNewCard_xiangXiRQS(this,eD,changeUIOff_xiangXiRQS(3));
                },
                roundEnd: function() {
                    InitUserCoinAndName_xiangXiRQS(this,changeUIOff_xiangXiRQS(3));
                    MjClient.playui.ResetOtherCard(this, changeUIOff_xiangXiRQS(3));
                },
                waitPut: function(eD) {
                    DealWaitPut(this, eD, changeUIOff_xiangXiRQS(3));
                },
                MJPut: function(eD) {
                    DealPutCard_xiangXiRQS(this, eD, changeUIOff_xiangXiRQS(3));
                },
                HZChiCard: function(eD) {
                    DealChiCard_xiangXiRQS(this, eD, changeUIOff_xiangXiRQS(3));
                },
                HZGangCard: function(eD) {
                    DealGangCard_xiangXiRQS(this, eD, changeUIOff_xiangXiRQS(3));
                },
                MJPeng: function(eD) {
                    DealPengCard_xiangXiRQS(this, eD, changeUIOff_xiangXiRQS(3));
                },
                HZWeiCard: function(eD){
                    DealWeiCard_xiangXiRQS(this, eD, changeUIOff_xiangXiRQS(3));
                },
                MJHu: function(eD) {
                    DealHu_xiangXiRQS(this, eD, changeUIOff_xiangXiRQS(3));
                },
                onlinePlayer: function(eD) {
                    setUserOffline_xiangXiRQS(this, changeUIOff_xiangXiRQS(3));
                },
                playerStatusChange: function(eD) {
                    setUserOffline_xiangXiRQS(this, changeUIOff_xiangXiRQS(3));
                },
                MJFlower: function(eD) {
                    HandleMJFlower(this, eD, changeUIOff_xiangXiRQS(3));
                },
                MJTing: function (eD) {
                    HandleMJTing(this, eD, changeUIOff_xiangXiRQS(3));
                },
                HZPickCard:function (eD) {
                    if(MjClient.rePlayVideo != -1) {
                        MjClient.playui._doPickResetHandCardForPlayBack(eD, changeUIOff_xiangXiRQS(3));
                    }
                    // RemovePutCardOut_xiangXiRQS(true);
                    // MjClient.playui.ResetHandCard(this,2);

                    DealShowLastCard_xiangXiRQS(this, eD, changeUIOff_xiangXiRQS(3));
                },
                HZAddCards:function (eD) {
                    DealAddCard_xiangXiRQS(this,eD, changeUIOff_xiangXiRQS(3));
                },
                HZCheckRaise:function (msg) {
                    ShowPutCardIcon_xiangXiRQS();
                    //MjClient.playui.EatVisibleCheck();
        ...
        xing: {
            ...
            head: {
                chuizi_tips : {
                    _run: function(){
                        setJiachuiImgTips_xiangXiRQS(this, changeUIOff_xiangXiRQS(1));
                    },
                    _event: {
                        initSceneData: function() {
                            setJiachuiImgTips_xiangXiRQS(this, changeUIOff_xiangXiRQS(1));
                ...
                jiachuiText_tips : {
                    _run:function() {
                        setJiachuiTextTips_xiangXiRQS(this, changeUIOff_xiangXiRQS(1));
                    },
                    _event:{
                        initSceneData: function() {
                            setJiachuiTextTips_xiangXiRQS(this, changeUIOff_xiangXiRQS(1));
                        ...
                        MJJiazhu: function(msg) {
                            var pl = getUIPlayer_xiangXiRQS(changeUIOff_xiangXiRQS(1))
                            if (msg.uid == pl.info.uid) {
                                setJiachuiTextTips_xiangXiRQS(this, changeUIOff_xiangXiRQS(1));
                            }
                        },
                ...
                zhuang: {
                    ...
                    _event: {
                        waitPut: function() {
                            if(MjClient.MaxPlayerNum_xiangXiRQS == 4 ) {
                                showUserZhuangLogo_xiangXiRQS(this, changeUIOff_xiangXiRQS(1));
                            }
                        },
                        initSceneData: function() {
                            if(MjClient.MaxPlayerNum_xiangXiRQS == 4 ) {
                                if (IsArrowVisible_xiangXiRQS()) showUserZhuangLogo_xiangXiRQS(this, changeUIOff_xiangXiRQS(1));
                            }
                        },
                        mjhand: function(){
                            if(MjClient.MaxPlayerNum_xiangXiRQS == 4 ) {
                ...
                chatbg: {
                    ...
                    chattext: {
                        _event: {
                            MJChat: function(msg) {
                                if(MjClient.MaxPlayerNum_xiangXiRQS == 4) {
                                    showUserChat_xiangXiRQS(this, changeUIOff_xiangXiRQS(1), msg);
                                }
                            },
                            playVoice: function(voicePath) {
                                if(MjClient.MaxPlayerNum_xiangXiRQS == 4) {
                                    MjClient.data._tempMessage.msg = voicePath;
                                    showUserChat_xiangXiRQS(this, changeUIOff_xiangXiRQS(1), MjClient.data._tempMessage);
                                }
                            }
                        }
                    }
                ...
                _click: function(btn) {
                    if(MjClient.MaxPlayerNum_xiangXiRQS == 4) {
                        showPlayerInfo_xiangXiRQS(changeUIOff_xiangXiRQS(1), btn);
                    }
                },
                _event: {
                    loadWxHead: function(d) {
                        if(MjClient.MaxPlayerNum_xiangXiRQS == 4) {
                            setWxHead_xiangXiRQS(this, d, changeUIOff_xiangXiRQS(1));
                        }
                    },
                    addPlayer: function(eD) {
                        showFangzhuTagIcon_xiangXiRQS(this,changeUIOff_xiangXiRQS(1));
                    },
                    removePlayer: function(eD) {
                        showFangzhuTagIcon_xiangXiRQS(this,changeUIOff_xiangXiRQS(1));
                    }
                ...
                huxi:{
                    ...
                    _event:{
                        ...
                        initSceneData:function(){
                            // if(MjClient.MaxPlayerNum_xiangXiRQS == 4) {
                            //     // this.visible = true;
                            //     var huXi = UpdateHuXi_xiangXiRQS(changeUIOff_xiangXiRQS(1));
                            //     if (!IsArrowVisible_xiangXiRQS()) {
                            //         huXi = 0;
                            //         this.visible = false;
                            //     }
            ...
            ready: {
                ...
                _run: function() {
                    if(MjClient.MaxPlayerNum_xiangXiRQS == 4) {
                        GetReadyVisible_xiangXiRQS(this, changeUIOff_xiangXiRQS(1));
                    }
                },
                _event: {
                    startShuffleCards:function (d) {
            ...
            _event: {
                clearCardUI: function() {
                    if(MjClient.MaxPlayerNum_xiangXiRQS == 4) {
                        clearCardUI_xiangXiRQS(this, changeUIOff_xiangXiRQS(1));
                    }
                },
                clearCardArr: function(){
                    if(MjClient.MaxPlayerNum_xiangXiRQS == 4) {
                ...
                initSceneData: function(eD) {
                    if(MjClient.MaxPlayerNum_xiangXiRQS == 4) {
                        SetUserVisible_xiangXiRQS(this, changeUIOff_xiangXiRQS(1));
                        InitUserHandUI_xiangXiRQS(this, changeUIOff_xiangXiRQS(1));
                        InitUserCoinAndName_xiangXiRQS(this, changeUIOff_xiangXiRQS(1));
                        DealOffLineCard_xiangXiRQS(this, changeUIOff_xiangXiRQS(1));
                    }
                },
                addPlayer: function(eD) {
                    if(MjClient.MaxPlayerNum_xiangXiRQS == 4) {
                ...
                HZNewCard: function(eD) {
                    if(MjClient.MaxPlayerNum_xiangXiRQS == 4) {
                        ...
                        DealNewCard_xiangXiRQS(this, eD, changeUIOff_xiangXiRQS(1));
                    }
                },
                roundEnd: function() {
                    if(MjClient.MaxPlayerNum_xiangXiRQS == 4) {
                        InitUserCoinAndName_xiangXiRQS(this, changeUIOff_xiangXiRQS(1));
                        MjClient.playui.ResetOtherCard(this, changeUIOff_xiangXiRQS(1));
                    }
                },
                waitPut: function(eD) {
                    if(MjClient.MaxPlayerNum_xiangXiRQS == 4) {
                        DealWaitPut(this, eD, changeUIOff_xiangXiRQS(1));
                    }
                },
                MJPut: function(eD) {
                    if(MjClient.MaxPlayerNum_xiangXiRQS == 4) {
                        DealPutCard_xiangXiRQS(this, eD, changeUIOff_xiangXiRQS(1));
                    }
                },
                HZChiCard: function(eD) {
                    if(MjClient.MaxPlayerNum_xiangXiRQS == 4) {
                        DealChiCard_xiangXiRQS(this, eD, changeUIOff_xiangXiRQS(1));
                    }
                },
                HZGangCard: function(eD) {
                    if(MjClient.MaxPlayerNum_xiangXiRQS == 4) {
                        DealGangCard_xiangXiRQS(this, eD, changeUIOff_xiangXiRQS(1));
                    }
                },
                MJPeng: function(eD) {
                    if(MjClient.MaxPlayerNum_xiangXiRQS == 4) {
                        DealPengCard_xiangXiRQS(this, eD, changeUIOff_xiangXiRQS(1));
                    }
                },
                HZWeiCard: function(eD){
                    if(MjClient.MaxPlayerNum_xiangXiRQS == 4) {
                        DealWeiCard_xiangXiRQS(this, eD, changeUIOff_xiangXiRQS(1));
                    }
                },
                MJHu: function(eD) {
                    if(MjClient.MaxPlayerNum_xiangXiRQS == 4) {
                        DealHu_xiangXiRQS(this, eD, changeUIOff_xiangXiRQS(1));
                    }
                },
                onlinePlayer: function(eD) {
                    if(MjClient.MaxPlayerNum_xiangXiRQS == 4) {
                        setUserOffline_xiangXiRQS(this, changeUIOff_xiangXiRQS(1));
                    }
                },
                playerStatusChange: function(eD) {
                    if(MjClient.MaxPlayerNum_xiangXiRQS == 4) {
                ...
                MJFlower: function(eD) {
                    if(MjClient.MaxPlayerNum_xiangXiRQS == 4) {
                        HandleMJFlower(this, eD, changeUIOff_xiangXiRQS(1));
                    }
                },
                MJTing: function (eD) {
                    if(MjClient.MaxPlayerNum_xiangXiRQS == 4) {
                        HandleMJTing(this, eD, changeUIOff_xiangXiRQS(1));
                    }
                },
                HZPickCard:function (eD) {
                    if(MjClient.MaxPlayerNum_xiangXiRQS == 4) {
                        if (MjClient.rePlayVideo != -1 ) {
                            var pl = getUIPlayer_xiangXiRQS(changeUIOff_xiangXiRQS(1));
                            if (eD.uid == pl.info.uid) {
                                MjClient.OtherHandArr[changeUIOff_xiangXiRQS(1)] = MjClient.majiang.sortHandCardSpecial(pl.mjhand)
                            }
                        }
                        DealShowLastCard_xiangXiRQS(this, eD, changeUIOff_xiangXiRQS(1));
                    }
                },
                HZAddCards:function (eD) {
                    if(MjClient.MaxPlayerNum_xiangXiRQS == 4) {
                        DealAddCard_xiangXiRQS(this, eD, changeUIOff_xiangXiRQS(1));
                    }
                },
                HZCheckRaise:function (msg) {
                    if(MjClient.MaxPlayerNum_xiangXiRQS == 4) {
        ...
        eat: {
            chi: {
                ...
                _touch: function(btn, eT) {
                    if (eT == 2){
                        if(ziPai.getHasFastEat()){
                            ...
                            if (chiSet.length == 1) {
                                ...
                                if (pl.mjhand.indexOf(putCard) < 0 || copy.indexOf(putCard) >= 0) {
                                    ...
                                    commitEatCards_xiangXiRQS(eatCards, null);
                        ...
                        showSelectEatCards_xiangXiRQS(this,btn.tag);
            ...
            hu: {
                ...
                _touch: function(btn, eT) {
                    if (eT == 2) {
                        MJHuToServer_xiangXiRQS();
                        setChiVisible_xiangXiRQS();//add by maoyu
                    }
                }
            },
            cancel: {
            ...
            wangDiao:{
                ...
                _touch: function(btn, eT) {
                    console.log(">>>> lf，点击碰按钮");
                    if (eT == 2) {
                        HZWangChuangToServer_xiangXiRQS(1);
                        setChiVisible_xiangXiRQS();//add by maoyu
                    }
                }
            },
            wangChuang:{
                ...
                _touch: function(btn, eT) {
                    console.log(">>>> lf，点击碰按钮");
                    if (eT == 2) {
                        HZWangChuangToServer_xiangXiRQS(2);
                        setChiVisible_xiangXiRQS();//add by maoyu
                    }
                }
            },
            wangZha:{
                ...
                _touch: function(btn, eT) {
                    console.log(">>>> lf，点击碰按钮");
                    if (eT == 2) {
                        HZWangChuangToServer_xiangXiRQS(4);
                        setChiVisible_xiangXiRQS();//add by maoyu
                    }
                }
            },
                    ...
                    this.adaptChiLayout = function() {
                        ...
                        if (cards_option.length == 0) { // 吃牌选好了
                            if ((MjClient.data.sData.players[SelfUid()].eatFlag & 32) > 0) {
                                MjClient.showMsg("选择吃后视为过胡，确定吃吗？", function() {
                                    ...
                                    HZChiToServer_xiangXiRQS(cards_select[0], biCards);
                                    this.visible = false;
                                }.bind(this), function() {
                                    cards_select.pop();
                                    this.adaptChiLayout();
                        ...
                        for (var i = 0; i < cards_select.length; i++) {
                            ...
                            for (var j = 0; j < row.length; j++) {
                                ...
                                chiCardImg.loadTexture(MjClient.cardPath_xiangXiRQS + "out" + row[j] + ".png");
        ...
        voice_btn: {
            ...
            _event: {
                ...
                downAndPlayVoice: function(msg) {
                    ...
                    downAndPlayVoice_xiangXiRQS(msg.uid, msg.msg);
                }
            }
        },
        cutLine:{
    ...
    ctor: function() {
        this._super();
        ziPai.initZiPaiData();
        cc.log("==========xaiongxierqishi=======");
        var playui = ccs.load("Play_xiangXiRQS.json");
        playMusic("bgFight");
        var tData = MjClient.data.sData.tData;
        MjClient.MaxPlayerNum_xiangXiRQS = tData.maxPlayer;
        ...
        //三人玩时恢复
            ...
            this.jsBind.right = PlayLayer_xiangXiRQS.jsBindRightCopy;
        }
        ...
        this.tingLayer = new xiangXiRQSTingLayer();
    ...
    resetJiaZhuTip: function()
    {
        ...
        if(MjClient.MaxPlayerNum_xiangXiRQS == 2){
            nodes = [this._downNode, this._topNode];
        }

        for (var i = 0; i < MjClient.MaxPlayerNum_xiangXiRQS; i ++)
        {
            var pl = getUIPlayer_xiangXiRQS(i);
            if (!pl) 
                continue;
...
PlayLayer_xiangXiRQS.jsBindRightCopy = PlayLayer_xiangXiRQS.prototype.jsBind.right;

//重置手牌的顺序
PlayLayer_xiangXiRQS.prototype.ResetHandCard = function(posNode,off, needAction, isDelay){

    needAction = needAction === undefined ? false : needAction;
    // needAction = false;

    if(off == 0){
        resetHandCard_xiangXiRQSEx(posNode,off, needAction, isDelay);
...
//回放时其他家补牌重置手牌的处理
PlayLayer_xiangXiRQS.prototype._doPickResetHandCardForPlayBack = function(msg, off){
    var pl = getUIPlayer_xiangXiRQS(off);
...
PlayLayer_xiangXiRQS.prototype._doMovetoAction = function(node,endP){
    node.stopAllActions();
    var ac = cc.moveTo(0.3,endP);
    node.runAction(ac);
};

PlayLayer_xiangXiRQS.prototype.ResetOtherCard = function(posNode, off) {
    //添加吃、碰、跑、偎、提
    var sData = MjClient.data.sData;
    var tData = null;
    if (sData) {
    ...
    if (mjSortArr) {
            ...
            for (var i = 0; i < childList.length; i++) {
                ...
                if(tData.maxPlayer == 4){
                    if((off <= 1 && i != childList.length - 1) ||
                        (off >= 2 && i != 0)){
                        child.loadTexture(MjClient.cardPath_xiangXiRQS + "huxiBG.png");
                    }
                }else{
        ...
        for (var k = 0; k < mjSortArr.length; k++) {
            ...
            if (name == "mjgang1") {
                ...
                for (var i = 0; i < 4; i++) {
                    addEatCard_xiangXiRQS(posNode, "mjgang1" + k, cardNum, off);
                }
            ...
            if (name == "mjwei") {
                var cardNum = pl.mjwei[pos];
                for (var i = 0; i < 3; i++) {
                    addEatCard_xiangXiRQS(posNode, "mjwei" + k, cardNum, off);
                }
            ...
            if (name == "mjgang0") {
                ...
                for (var i = 0; i < 4; i++) {
                    addEatCard_xiangXiRQS(posNode, "mjgang0" + k, cardNum, off);
                }
            }
            //碰
            if (name == "mjpeng") {
                ...
                for (var i = 0; i < 3; i++) {
                    addEatCard_xiangXiRQS(posNode, "mjpeng" + k, cardNum, off);
                }
            }
            //吃
            if (name == "mjchi") {
                ...
                for (var i = 0; i < eatCards.length; i++) {
                    addEatCard_xiangXiRQS(posNode, "mjchi" + k, eatCards[i], off);
                }
                ...
                if (biCards && biCards.length > 0) {
                    for (var i = 0; i < biCards.length; i++) {
                        ...
                        for (var m = 0; m < biArr.length; m++) {
                            addEatCard_xiangXiRQS(posNode, "mjbi" + k + i, biArr[m], off);
                        }
                    }
                }
    ...
    //    var outCard = getNewCard_xiangXiRQS(pl.mjput[i], 2, off);
    //    if(off == 0){
    //        outCard.anchorX = 1;
    //        outCard.anchorY = 0;
...
PlayLayer_xiangXiRQS.prototype.ResetPutCard = function(posNode,off){
    //添加打出的牌
    var pl = getUIPlayer_xiangXiRQS(off);
    if (!pl) return;
    ...
    var outNode = posNode.getChildByName("outNode_New").getChildByName("chuPaiNode");
    var qiNode = posNode.getChildByName("outNode_New").getChildByName("qiPaiNode");

    outNode.visible = true;
    qiNode.visible = true;
    outNode.removeAllChildren();
    qiNode.removeAllChildren();

    for (var i = 0; i < pl.mjput.length; i++) {
        if(pl.info.uid == curUId && i == pl.mjput.length - 1 && tData.currCard == pl.mjput[i]){
            AddPutCard_xiangXiRQS(posNode, null, off);
            continue;
        }
        var outCard = getNewCard_xiangXiRQS(pl.mjput[i], 2, off);
        outNode.addChild(outCard);

        setPutCardPos_xiangXiRQS(outNode, outCard, i, off);
    }

    if(pl.qiPai){
        for (var j = 0; j < pl.qiPai.length; j++) {
            var qiCard = getNewCard_xiangXiRQS(pl.qiPai[j], 2, off);
            qiNode.addChild(qiCard);

            setPutCardPos_xiangXiRQS(qiNode, qiCard, j, off);
        }
    }

    if(pl.info.uid == curUId){
        MjClient.lastFallArea = pl.fallArea;
    }
};

//重置牌的顺序
PlayLayer_xiangXiRQS.prototype.CardLayoutRestore = function(posNode,off){
    if(MjClient.rePlayVideo == -1){
...
// 判断吃、碰、跑、偎、提、胡的状态
PlayLayer_xiangXiRQS.prototype.EatVisibleCheck = function(off){
    var eat = MjClient.playui.jsBind.eat;
...
PlayLayer_xiangXiRQS.prototype.showAndHideHeadEffect = function(){
    ...
    else if (curPlayerIsMe_xiangXiRQS(1)){
        curPlayerNode = arr[1].getChildByName("head");
    }else if (curPlayerIsMe_xiangXiRQS(2)){
        curPlayerNode = arr[2].getChildByName("head");
    }else if (curPlayerIsMe_xiangXiRQS(3)){
