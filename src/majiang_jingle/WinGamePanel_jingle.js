...
function SetEndOneUserUI_jingle(node,off)
{
					        ...
					        for(var i = 0; i < pl.fgang.length; i++)
					        {
					            ...
					            if (fgangcards.pos != -1) // 明杠
					            {
					                ...
					                for(var j = 0; j < 4; j++)
					                {
					                    if(j < 3)
					                    {
					                        getNewCard(node, "up", "gang0", dnxb[j], 0);
					                    }
					                    else
					                    {
					                        var cdui = getNewCard(node, "up", "gang0", fgangcards.card, 0, "isgang4");//最后一张牌放上面
					            ...
					            { 
					                for(var j = 0; j < 4; j++)
					                {
					                    if(j == 3)
					                    {
					                        getNewCard(node, "down", "gang1", 0, 0, "isgang4").tag = dnxb[j];
					                    }
					                    else
					                    {
					                        getNewCard(node, "up", "gang1", dnxb[j], 0);
					                    }
					                }
					            }
					        }
...
var EndOneView_jingle = cc.Layer.extend({
	jsBind:{
		block: {
		...
		head0:{
		   ...
		   _run:function(){ SetEndOneUserUI_jingle(this,0); },
		...
		,head1:{ 
		  ...
		  _run:function(){ SetEndOneUserUI_jingle(this,1); }
		}
		...
		,head2:{ 
		   ...
		   _run:function(){ SetEndOneUserUI_jingle(this,2); }
		}
		,head3:{
			 head:
			 {
			...
			_run:function(){ SetEndOneUserUI_jingle(this,3); }
			},
            count_down:{
                _visible :function()
                {
    ...
    ctor:function () {
        this._super();
        var endoneui = ccs.load("endOne_jingle.json");
        ...
        var _ming =  _back.getChildByName("Image_ming").getChildByName("card");
        var HuncardMsg = MjClient.data.sData.tData.hunCard;

        if(HuncardMsg && HuncardMsg != -1)
        {
            setCardSprite(_ming, parseInt(HuncardMsg), 4);
        }
        else
        {
            _back.getChildByName("Image_ming").visible = false;
        }
        ...
        {
            var _hunCard2 = _ming.clone();
            //_hunCard2.loadTexture(_ming.getRenderFile().file);
            _ming.getParent().addChild(_hunCard2);
            _hunCard2.setScale(_ming.getScale());
            var cardWith =  _ming.getSize().width * _ming.getScale();
            setCardSprite(_hunCard2, parseInt(HuncardMsg2), 4);
            _hunCard2.setPosition(_ming.getPositionX()+ cardWith ,_ming.getPositionY());
        }
