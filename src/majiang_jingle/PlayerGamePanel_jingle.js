...
//向服务器发送 过消息
MjClient.MJPass2NetForjingle = function()
{
   // console.log(">>>>>>>>>普通  过 <<<<<<<<");
    cc.log("====================MJPass2NetForjingle======pass=====");
...
function SetUserVisible_jingle(node, off)
{
    //var sData = MjClient.data.sData;
    //return;
    cc.log("====================off======================" + off);
...
//处理出牌,放一张牌，打牌动作
function DealMJPut_jingle(node, msg, off, outNum)
{
...
function InitUserHandUI_jingle(node, off)
{
    var sData = MjClient.data.sData;
    var tData = sData.tData;
    var pl = getUIPlayer(off);
    ...
    //添加风杠
    cc.log("------------添加风杠-----------" + pl.fgang);
    if (typeof(pl.fgang) != "undefined") 
    {
        for(var i = 0; i < pl.fgang.length; i++)
        {
            var fgangcards = pl.fgang[i];
            var dnxb = [31,41,51,61];
            if (fgangcards.pos != -1) // 明杠
            {

                var index = dnxb.indexOf(fgangcards.card);//排除风杠牌
                dnxb.splice(index,1);
                var bIsPengGang = false;

                var idx = tData.uids.indexOf(pl.info.uid);

                var offIdx = getOffByIndex(pl.fgang[i].pos, idx) - 1;

                if (offIdx == null)
                {
                    cc.log("InitUserHandUI:offIdx == null!!!!  fenggang");
                    offIdx = 0;
                }
                ...
                for(var j = 0; j < 4; j++)
                {
                    if(j < 3)
                    {
                        if( (j % 3 == 2 - offIdx && off % 3 == 0) || (j % 3 == offIdx && off % 3 != 0) )
                        {
                            var cdui = getNewCard(node, "up", "gang0", dnxb[j], off, "heng", "heng");
                            if(bIsPengGang) offIdx = 3;
                            setCardArrow(cdui, offIdx, off);
                            if (j==1)
                            {
                        ...
                        {
                            getNewCard(node, "up", "gang0", dnxb[j], off);
                        }
                    }
                    else
                    {
                        var cdui = getNewCard(node, "up", "gang0", fgangcards.card, off, "isgang4");//最后一张牌放上面
                        cdui.tag = fgangcards.card;
                        if (setCardArrowOnGang4)
                        {
                            if(bIsPengGang) offIdx = 3;
                            setCardArrow(cdui, offIdx, off);
            ...
            { 
                for(var j = 0; j < 4; j++)
                {
                    if(j == 3)
                    {
                        getNewCard(node, "down", "gang1", 0, off, "isgang4").tag = dnxb[j];
                    }
                    else
                    {
                        getNewCard(node, "up", "gang1", dnxb[j], off);
                    }
                }
            }
        }
    ...
    for(var i = 0; i < pl.mjput.length; i++)
    {
        ...
        DealMJPut_jingle(node, msg, off, i);
    }
    ...
    if(MjClient.rePlayVideo == -1)//表示正常游戏
    {
        ...
        else
        {
            ...
            if (typeof (pl.fgang) != "undefined") 
            {
                upCardCount = CardCount - ((pl.mjpeng.length + pl.mjgang0.length + pl.mjgang1.length + pl.fgang.length) * 3 + pl.mjchi.length);
            }
...
function initFlower_jingle() {
    initFlower(false, false);
}


var PlayLayer_jingle = cc.Layer.extend({
    jsBind: {
        _event: {
            mjhand: function() {
            ...
            roundEnd: function() {
                ...
                function delayExe()
                {
                    ...
                    self.addChild(new EndOneView_jingle(),500);
                }
            ...
            ting: {
                ...
                _touch: function(btn, eT) {
                    if (eT == 2)
                    {
                        ...
                        var tData = MjClient.data.sData.tData;
                        var tingCards = MjClient.majiang.calTingSet(copyhand, new Set([tData.hunCard, tData.hunCard2]),pl.fgang);
            ...
            changeui: {
                ...
                changeuibg: {
                    ...
                    _run: function() {
                        ...
                        this.gangTouch = function(btn, et) {
                            if (et == 2)
                            {
                                ...
                                if (btn.fgang && btn.fgang != -1 &&
                                    MjClient.majiang.hasDnxb(pl.mjhand)) 
                                {
                                    MjClient.playui.MJGangToServer(btn.fgang,true);
                                }
                                else
                                {
                                    MjClient.playui.MJGangToServer(btn.tag);
                                }
                            }
                    ...
                    guobg: {
                        guo: {
                            _touch: function(btn, eT) {
                                if (eT == 2) MjClient.MJPass2NetForjingle();
                            }
                        },
                        fanhui: {
                            _touch: function(btn, et) {
    ...
    ctor: function() {
        this._super();
        var playui = ccs.load("Play_jingle.json");
        ...
        if(pl && (pl.mjState == TableState.isReady || pl.mjState == TableState.roundFinish))
        {
            ...
            if(pl != null)
            {
                if (pl.isTing) {
                    ...
                    if (off == 0)
                    {
                        var tingSet = MjClient.majiang.calTingSet(pl.mjhand, new Set([tData.hunCard, tData.hunCard2]),pl.fgang);
...
PlayLayer_jingle.prototype.CardLayoutRestore = function(node, off)
{
    ...
    //排序麻将的位置 by sking
    cc.log("手牌----------------   11 ",pl.mjhand,pl.isNew);
    if (pl.mjhand && pl.mjhand.length > 0)
    {
        var count = tempMaJiang.CardCount(pl);
        cc.log("mjhandNum   --",mjhandNum,"   ",count,"   ",pl.mjhand.length);
...
// 判断吃碰杠胡的状态
PlayLayer_jingle.prototype.EatVisibleCheck = function()
{
    var eat = MjClient.playui.jsBind.eat;
    ...
    cc.log("检测pl数据    ",JSON.stringify(pl));
        ...
        if(IsTurnToMe())
        {
            ...
            if (!pl.isTing) {
                ...
                for (var i = 0; i < pl.mjhand.length; i++) {
                    ...
                    if (MjClient.majiang.canTing(cardsAfterPut, pl.mjhand[i], new Set([tData.hunCard, tData.hunCard2]),pl.fgang)) {
            ...
            var rtn = MjClient.majiang.canGang1(pl.mjpeng, pl.mjhand, pl.isTing, new Set([tData.hunCard, tData.hunCard2]), pl.fgang, pl.mjgang0.concat(pl.mjgang1));
    ...
    else if(tData.tState == TableState.waitEat)
    {
        ...
        if(!IsTurnToMe())
        {
            ...
            if (pl.eatFlag & 4 ) {
                ...
                {
                    vnode.push(eat.gang0._node);
                    MjClient.gangCards = MjClient.majiang.getGang0(pl.mjhand,tData.lastPutCard);
                    cc.log("显示杠牌    ", MjClient.gangCards,tData.lastPutCard);
                    eat.gang0._node.visible = true;
                    setCardSprite(eat.gang0.card1._node, MjClient.fgang, 0);
                }
            }
            if (pl.eatFlag & 2) {
    ...
    if (pl.eatFlag & 8 && tData.areaSelectMode["bihu"]) 
    {
        eat.gang0._node.visible = false;
        eat.guo._node.visible = false;
        eat.peng._node.visible = false;//如果有胡，且选中必胡选项，其他按钮都不可见
    }
...
/*
    玩家ui，信息
 */
PlayLayer_jingle.prototype.Node_player = function(off)
{
    ...
    UIEventBind(null, _node, "initSceneData", function (eD) {
        SetUserVisible_jingle(_node, off);
    ...
    UIEventBind(null, _node, "mjhand", function (eD) {
        InitUserHandUI_jingle(_node, off);
    });
    ...
    UIEventBind(null, _node, "MJGang", function (eD) {
        MjClient.playui.DealMJGang_Jingle(_node, eD, off);
        hideTingBtn();
        if(_skipPengIconTag) _skipPengIconTag.visible = false;
        setUserOffline(_node, off);
        MjClient.fgang = 0;
...
PlayLayer_jingle.prototype.MJGangToServer = function(cd,isfgang)
{
    var fgnag = false;
    if (isfgang) 
    {
    ...
    cc.log("====================MJGangToServer=====for self============ SKING:", cd,isfgang);
    MjClient.gamenet.request("pkroom.handler.tableMsg", {
        cmd: "MJGang",
        card: cd,
        eatFlag: EatFlag(),
...
PlayLayer_jingle.prototype.DealMJGang_Jingle = function(node, msg, off)
{
    if (!msg.isFenggang)
    {
        DealMJGang(node, msg, off);
        return;
    }

    cc.log("DealfengGang: begin");
    ...
    else if (msg.gang == 4) //明杠风杠
    {
        var fromOff = [];
        var dnxb = [31, 41, 51, 61];
        var index = dnxb.indexOf(msg.card);//排除当前选择的一张牌
        dnxb.splice(index,1);
        var fromBind = GetUIBind(msg.from, fromOff);
        var fnode = fromBind._node;
        ...
        if(off == 0)
        {
            for (var i = 0; i < dnxb.length; i++) {
                RemoveNodeBack(node, "mjhand", 1, dnxb[i]);
            }
        }

        MjClient.playui.CardLayoutRestore(fnode, fromOff[0]);
    }
    else if (msg.gang == 5) //暗杠风杠
    {
        var dnxb = [31, 41, 51, 61];
        if(off == 0)
        {
        ...
        if(off == 3)
        {
            ...
            else if (msg.gang == 4) //明杠风杠
            {
                ...
                else
                {
                    ...
                    for (var i = 0; i < dnxb.length; i++) {
                        RemoveNodeBack(node, "mjhand_replay", 1, dnxb[i]);
                    }
                }
            }
            else if (msg.gang == 5) //暗杠风杠
    ...
    for(var j = 0; j < 4; j++)
    {
        ...
        else if (msg.gang == 4) 
        {
            for (var i=0; i<pl.fgang.length; i++)
            {
                if (pl.fgang[i].card == msg.card)
                {
                    offIdx = getOffByIndex(pl.fgang[i].pos, idx) - 1;
                    break;
                }
            }
            ...
            if(j < 3)
            {
                if( (j % 3 == 2 - offIdx && off % 3 == 0) || (j % 3 == offIdx && off % 3 != 0) )
                {
                    _cardNode = getNewCard(node, "up", "gang0", dnxb[j], off, "heng", "heng");
                ...
                {
                    _cardNode = getNewCard(node, "up", "gang0", dnxb[j], off);
                }
            }
            else
            {
        ...
        else if (msg.gang == 5) 
        {
            ...
            {
                _cardNode = getNewCard(node, "down", "gang1", 0, off, "isgang4");
                _cardNode.tag = dnxb[3];
            }
            else
            {
                _cardNode = getNewCard(node, "up", "gang1", dnxb[j], off);
            }
        }
