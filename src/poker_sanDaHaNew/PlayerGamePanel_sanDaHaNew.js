...
var PlayLayer_sanDaHaNew = cc.Layer.extend({
    _btnPutCard: null,
    jsBind: {
        _event: {
            mjhand: function() {
            ...
            roundEnd: function() {
                ...
                function delayExe() {
                    ...
                    self.addChild(new EndOneView_sanDaHaNew(), 500);
                }
    ...
    ctor: function() {
        this._super();
        var playui = ccs.load(res.Play_sanDaHaNew_json);
...
PlayLayer_sanDaHaNew.prototype.reConnectShowDeskCard = function() {
    var tData = MjClient.data.sData.tData;
...
PlayLayer_sanDaHaNew.prototype.cannotOutCardGrey = function() {
...
PlayLayer_sanDaHaNew.prototype.recoverCannotOutCard = function() {
    var children = this._downNode.children;
    for (var i = 0; i < children.length; i++) {
        if (children[i].name != "mjhand")
            continue;
...
PlayLayer_sanDaHaNew.prototype.clockNumberUpdate = function(node, endFunc) {
    return arrowbkNumberUpdate(node, endFunc);
}

PlayLayer_sanDaHaNew.prototype.updateClockPosition = function(arrowNode) {
    var tData = MjClient.data.sData.tData;
    var uids = tData.uids;
 ...
 * @return {String}
 */
PlayLayer_sanDaHaNew.prototype.getGameInfoString = function(param) {
    var tData = MjClient.data.sData.tData;
...
PlayLayer_sanDaHaNew.prototype.updateOperateBtnPosition = function() {
...
// 拖拉机动画
PlayLayer_sanDaHaNew.prototype.playCardAni_liandui = function() {
...
// 闲家得分达到条件时动画、投降
PlayLayer_sanDaHaNew.prototype.playWinAni = function(winType) {
...
// 刷新得分栏
PlayLayer_sanDaHaNew.prototype.refreshScoreBanner = function(shouldPlayAnim) {
    var tData = MjClient.data.sData.tData;
...
// 判断是否有不同意投降的玩家
PlayLayer_sanDaHaNew.prototype.haveNotAgreeTouXiang = function()
{
    var sData = MjClient.data.sData;
    var tData = MjClient.data.sData.tData;
...
// 重置玩家同意投降状态
PlayLayer_sanDaHaNew.prototype.initAgreeTouXiangStatus = function()
{
    var sData = MjClient.data.sData;
    var tData = MjClient.data.sData.tData;
    for (var i = 0; i < tData.uids.length; i++)
...
PlayLayer_sanDaHaNew.prototype.showWaitTip = function(bMaiPai) {
    var sData = MjClient.data.sData;
    var tData = sData.tData;
...
// 显示各玩家叫分
PlayLayer_sanDaHaNew.prototype.showJiaoFenTag = function(off) {
    var tData = MjClient.data.sData.tData;
...
// 隐藏各玩家叫分
PlayLayer_sanDaHaNew.prototype.hideJiaoFenTag = function() {
...
// 显示叫分面板
PlayLayer_sanDaHaNew.prototype.showJiaoFenPanel = function() {
    if (MjClient.rePlayVideo != -1)
    {
        MjClient.playui.jsBind.panel.jiaoFenPanel._node.setVisible(false);
        return;
    ...
    var func = function(isPaiFen) {
        ...
        var path = "playing/sanDaHa/jiaoFen/";
        if (MjClient.getAppType() == MjClient.APP_TYPE.QXYYQP)
            path = "playing/sanDaHa/jiaoFen_new/";
        ...
        for (var i = 5; i <= 80; i += 5) {
            ...
            {
                if (tData.areaSelectMode.qiJiao60) {
                    if(i >= 45 && i<= 60) {
                        btn.loadTextures(path + "red_" + i + ".png","",path + "gray_" + i + ".png");
                    }
                    else if(i >= 25 && i<= 40) {
        ...
        if (tData.areaSelectMode.jiaoFenJiaPai) {
...
// 把底牌添加到手牌中
PlayLayer_sanDaHaNew.prototype.addDiPaiToHand = function() {
    var tData = MjClient.data.sData.tData;
...
// 从手牌中移除埋的牌
PlayLayer_sanDaHaNew.prototype.removeMaiPaiFromHand = function() {
    var tData = MjClient.data.sData.tData;
    var off = getOffByIndex(tData.curPlayer);
...
// 显示投降同意选择框
PlayLayer_sanDaHaNew.prototype.showTouXuangDialog = function() {
     if (MjClient.rePlayVideo != -1)
        return;
...
// 显示叫主面板
PlayLayer_sanDaHaNew.prototype.showJiaoZhuPanel = function() {
    var tData = MjClient.data.sData.tData;
    ...
    if (tData.areaSelectMode.daWuZhu)
        jiaoZhuPanel.getChildByName("Btn_4").setEnabled(false);
}

// 显示埋牌、投降按扭
PlayLayer_sanDaHaNew.prototype.showMaiPaiBtn = function() {
    var tData = MjClient.data.sData.tData;
...
// 显示上一轮面板
PlayLayer_sanDaHaNew.prototype.showShangYiLunPanel = function(shangYiLunPanel) {
    var tData = MjClient.data.sData.tData;
...
// 显示查看底牌面板
PlayLayer_sanDaHaNew.prototype.showDiPaiPanel = function(diPaiPanel) {
    var tData = MjClient.data.sData.tData;
...
// 显示查牌面板
PlayLayer_sanDaHaNew.prototype.showChaPaiPanel = function(chaPaiPanel) {
    var tData = MjClient.data.sData.tData;
    var records = tData.putCardsRecord;
...
// 显示得分栏上的具体分牌
PlayLayer_sanDaHaNew.prototype.showScoreCards = function(panel, cards) {
...
// 给手牌中的主牌设置角标
PlayLayer_sanDaHaNew.prototype.setCardSprite_ZhuTag = function(node) {
    var tData = MjClient.data.sData.tData;
...
// 计算各牌色数量
PlayLayer_sanDaHaNew.prototype.getCardNumByColor = function(cards, colorIndex) {
    var num = 0;
    var types = [1, 2, 3, 0, 4]
    for (var i = 0; i < cards.length; i++) {
...
// 清空桌面
PlayLayer_sanDaHaNew.prototype.clearDesk = function() {
    cc.log("clearDesk")
...
PlayLayer_sanDaHaNew.prototype.CardLayoutRestore = function(node, off) {
    cc.log("横向排序 off=" + off);
    MjClient.playui.horSort(node, off);
};

PlayLayer_sanDaHaNew.prototype.CardLayoutDesk = function(node, cards, off) {
    //if(off != 0) return;
    var children = node.children;
	...
	var tData = MjClient.data.sData.tData;
    var cardType = MjClient.majiang.cardsType(cards, tData.zhuPaiType, tData.areaSelectMode.chou6);
...
//横向摆放《正常》
PlayLayer_sanDaHaNew.prototype.horSort = function(node, off) {
    var pl; //player 信息
...
PlayLayer_sanDaHaNew.prototype.DealCardEffect = function(uid, off)
{
    var tData = MjClient.data.sData.tData;
    ...
    //接牌
    var bChou6 = tData.areaSelectMode.chou6;
    var maxCardUid = MjClient.majiang.getMaxCardPlayerUid(roundPutCards, tData.firstPutCardUid, tData.uids, tData.zhuPaiType, bChou6);
...
PlayLayer_sanDaHaNew.prototype.getHandCount = function() {
    var tData = MjClient.data.sData.tData;
    var isFour = MjClient.MaxPlayerNum == 4;
...
// 埋完牌展示投降按钮
PlayLayer_sanDaHaNew.prototype.allowTouXiang = function() {
    if (MjClient.rePlayVideo != -1)
        return;
