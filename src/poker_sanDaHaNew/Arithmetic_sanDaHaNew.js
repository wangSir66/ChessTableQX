...
PokerSanDaHaNew.prototype.CARDTPYE = PDK_CARDTPYE;
var PDK_CARDCOUNT = {};
...
PokerSanDaHaNew.prototype.randomCards = function(areaSelectMode, tData) {
    var cards = [];
    // 去掉3、4
    for (var i = 1; i <= 54; i++) {
...
PokerSanDaHaNew.prototype.cardlog = function(cards, zhuColorType) {
 ...
 */
PokerSanDaHaNew.prototype.countWinType = function(jiaofen, PfenPaiArr, isTouXiang, Pkoudifen, xiaoGuangFen) {
    if( isTouXiang ) {
        return -4;
    }
...
PokerSanDaHaNew.prototype.countDiFen = function (jiaoFen, maxPlayer, qiJiao60) {
    if (maxPlayer == 3 && qiJiao60) {
...
PokerSanDaHaNew.prototype.isWuZhu = function(zhuColorType) {
...
// 是否有主牌
PokerSanDaHaNew.prototype.isHaveZhuCards = function (cards, zhuColorType) {
...
PokerSanDaHaNew.prototype.getSortZhuCard = function(oHands) {
    var cards = [];
...
PokerSanDaHaNew.prototype.isFirstOneHandBigger = function(firstHand, secondHand) {
...
// 获取常主牌数组
PokerSanDaHaNew.prototype.getChangZhuCardNum = function (cards) {
    var num = 0;
    for (var i = 0; i < cards.length; i++) {
...
PokerSanDaHaNew.prototype.calPoint = function(num) {
    if (!num) return -1;
...
PokerSanDaHaNew.prototype.calPointBySdh = function(num, zhuColorType) {
    if (!num) return -1;
...
// 牌的花色
PokerSanDaHaNew.prototype.calColor = function(num) {
    if (!num) return -1;
    // 0:方块  1:梅花  2:梅花  3:黑桃
...
PokerSanDaHaNew.prototype.calColorSdh = function(num, zhuColorType) {
    if (!num) return -1;
...
// 判断此牌是否是主花
PokerSanDaHaNew.prototype.isZhuCard = function(num, zhuColorType) {
    var point = this.calPoint(num);
    var color = this.calColor(num);
...
PokerSanDaHaNew.prototype.getRoundFirstCards = function(putCardsRecord, uid, maxPlayNum) {
...
PokerSanDaHaNew.prototype.getRoundCardsByUid = function(putCardsRecord, uid, maxPlayNum) {
    if(!putCardsRecord || typeof putCardsRecord == 'undefined') {
        return null;
    }
...
// 获取本轮最大的牌， 同点数先出比后出的大
PokerSanDaHaNew.prototype.getMaxCardPlayerUid = function(roundPutCards, firstPutUid, uids, zhuColorType,bchou6) {
    var frontUid = -1;
    if(!roundPutCards || typeof roundPutCards == 'undefined') {
        return frontUid;
    }
...
PokerSanDaHaNew.prototype.getAllCardFen = function( fenPaiArr ) {
    var fen = 0;
...
// 获得牌的分  非分牌返回0分 
PokerSanDaHaNew.prototype.getCardFen = function(num) {
...
// 获取所有对应牌数的拖拉机组合
PokerSanDaHaNew.prototype.getLianDuiCards = function(oCards, cardNum, zhuColorType, bchou6) {
...
// 获取所有对应牌数的拖拉机组合
PokerSanDaHaNew.prototype.getDuiZis = function(oCards, zhuColorType) {
...
PokerSanDaHaNew.prototype.countDuiNum = function(oCards, zhuColorType) {
    var num = 0;
    var cardNumByValue = {};
    for(var i in oCards){
 ...
 * @return {bool}
 */
PokerSanDaHaNew.prototype.isLiandui = function(oCards, zhuColorType,bchou6) { //oCards有序
...
PokerSanDaHaNew.prototype.isDuizi = function(oCards) {
 ...
 */
PokerSanDaHaNew.prototype.calType = function(cards, zhuColorType,bchou6) {
 ...
 * @return {number}
 */
PokerSanDaHaNew.prototype.calCardsValue = function(cards, zhuColorType) {
...
// // 牌点比较函数
PokerSanDaHaNew.prototype.cardValueCmp = function(a, b, zhuColorType) {
 ...
 */
PokerSanDaHaNew.prototype.isBigger = function(beiYaCards, oLastCards, zhuColorType, bchou6) {
...
PokerSanDaHaNew.prototype.checkPut = function(oHands, cards) {
 ...
 */
PokerSanDaHaNew.prototype.checkPutSdh = function(oHands, pOutCards, oBeiYaCard, zhuColorType, isCanShuaiPai, tData) {
...
PokerSanDaHaNew.prototype.findCardByType = function(oHands, oBeiYaCard, zhuColorType) {
 ...
 * @return {array} 提示的牌
 */
PokerSanDaHaNew.prototype.tipCards = function(oHands, oBeiYaCard, zhuColorType,bchou6) {
 ...
 */
PokerSanDaHaNew.prototype.sortHandCards = function(oCards, zhuColorType) {
 ...
 * @return {array}
 */
PokerSanDaHaNew.prototype.findNSameCard = function(hands, point, n, zhuColorType) {
...
// 删除相应牌值(不算花色)
PokerSanDaHaNew.prototype.delPoint = function(hands, points){
    var arr = hands.slice();
...
// 删除相同的牌(花色相同，牌值相同)
PokerSanDaHaNew.prototype.delCards = function(hands, cards) {
    var arr = hands.slice();
    for(var i=0; i < cards.length; i++){
        for (var j = 0; j < arr.length; j++) {
            if( arr[j] == cards[i] ){
...
// 从牌组中获取指定花色
PokerSanDaHaNew.prototype.getColorCards = function(cards, colors, zhuColorType) {
    var arr = [];
    for(var i=0; i < cards.length; i++) {
        var num = cards[i];
...
PokerSanDaHaNew.prototype.formatCards = function(oCards, zhuColorType) {
    var info = {};
...
PokerSanDaHaNew.prototype.isSameCards = function(cards1, cards2, zhuColorType) {
...
PokerSanDaHaNew.prototype.isAllZhuCard = function(cards, zhuColorType) {
    for (var i = 0; i < cards.length; i++) {
...
// 求出牌型
PokerSanDaHaNew.prototype.cardsType = function(oCards, zhuColorType,bchou6) {
    if (oCards) {
...
// 检查是否是一次可以出完
PokerSanDaHaNew.prototype.checkPutByOneTimes = function (oHands, zhuColorType, bchou6) {
...
if(typeof(module)!="undefined" && module.exports)    
    module.exports = PokerSanDaHaNew;

if(typeof(MjClient)!="undefined")
    MjClient.poker_SanDaHaNew = new PokerSanDaHaNew();
