...
//向服务器发送 过消息
MjClient.MJPass2NetForGuanyun = function()
{
   // console.log(">>>>>>>>>普通  过 <<<<<<<<");
    cc.log("====================send======pass=====");
...
function SetUserVisible_guanyun(node, off)
{
    //var sData = MjClient.data.sData;
    //return;
    cc.log("====================off======================" + off);
...
function InitUserHandUI_guanyun(node, off)
{
    var sData = MjClient.data.sData;
    var tData = sData.tData;
    var pl = getUIPlayer(off);
...
function initFlower_guanyun() {
    initFlower(false, false);
}


var PlayLayer_guanyun = cc.Layer.extend({
    jsBind: {
        _event: {
            mjhand: function() {
            ...
            roundEnd: function() {
                ...
                function delayExe()
                {
                    ...
                    self.addChild(new EndOneView_guanyun(),500);
                }
            ...
            _event: {
                ...
                MJPut: function(eD) {
                    ...
                    if (eD.uid === SelfUid())
                    {
                        ...
                        var tingSet = MjClient.majiang.calTingSetsplice(pl.mjhand);
            ...
            changeui: {
                ...
                changeuibg: {
                    ...
                    guobg: {
                        guo: {
                            _touch: function(btn, eT) {
                                if (eT == 2) MjClient.MJPass2NetForGuanyun();
                            }
                        },
                        fanhui: {
                            _touch: function(btn, et) {
    ...
    ctor: function() {
        this._super();
        var playui = ccs.load(res.Play_guanyun_json);
        playMusic("bgFight");
...
PlayLayer_guanyun.prototype.CardLayoutRestore = function(node, off)
{
...
PlayLayer_guanyun.prototype.willHuShowArrow = function()
{
    ...
    var _canTingCardsSplice = {};

    for(var i = 0; i < pl.mjhand.length; i++)
    {
        var afterPutCards = pl.mjhand.slice();
        afterPutCards.splice(i, 1);
        if(MjClient.majiang.canTing && MjClient.majiang.canTing(afterPutCards))
        {
            _canTingCards[pl.mjhand[i]] = 1;
        }
        if(MjClient.majiang.canTingSplice && MjClient.majiang.canTingSplice(afterPutCards)) {
            _canTingCardsSplice[pl.mjhand[i]] = 1;
        }
    }
    //  去掉箭头对象中多余的牌：防止听0张加箭头
    _canTingCards = COMMON_UI.deleteCanTingCardsForShowArrow(pl.mjhand, _canTingCards);
    _canTingCardsSplice = COMMON_UI.deleteCanTingCardsForShowArrow(pl.mjhand, _canTingCardsSplice);
    MjClient.canTingCards = _canTingCards;

    // 添加听牌标识
    if(Object.keys(_canTingCardsSplice).length <= 0) return;
    for(var i = 0; i < myCards.length; i++)
    {
        if((myCards[i].name === "mjhand") && (myCards[i].tag in _canTingCardsSplice))
        {
...
// 判断吃碰杠胡的状态
PlayLayer_guanyun.prototype.EatVisibleCheck = function()
{
    var eat = MjClient.playui.jsBind.eat;
...
/*            var cduis=MjClient.playui.jsBind.down._node.children;
            ...
            if (!pl.isTing) {
                ...
                {
                    cc.log("闲家起手听 不是庄家---");
                    MjClient.canTingCards = calTingSet(cardsAfterPut);
                    if (MjClient.majiang.canTing(cardsAfterPut)) {
                        cc.log("----可以听牌--- pl.putCount = " + pl.putCount);
                        cc.log("----可以听牌--- pl.isTianting= " + pl.isTianting);
                        if (vnode.indexOf(eat.ting._node) < 0 && pl.isTianting) {
                    ...
                    for (var i = 0; i < pl.mjhand.length; i++) {
                        cardsAfterPut = pl.mjhand.slice();
                        cardsAfterPut.splice(i,1); //依次去掉某张牌看能不能听
                        if (MjClient.majiang.canTing(cardsAfterPut)) {
                            MjClient.canTingCards[pl.mjhand[i]] = 1;
                        }

                        if(MjClient.majiang.canTingForTingBtn(cardsAfterPut)) {
                            if (vnode.indexOf(eat.ting._node) < 0) {
                                vnode.push(eat.ting._node);
                            }
                        }
                    }
                    cc.log("==============正常听牌============== MjClient.canTingCards = " + JSON.stringify(MjClient.canTingCards));
                }
            }
            ...
            if(vnode.length > 0)
            {
                if(vnode.indexOf(eat.ting._node) >= 0  && vnode.length == 1 && MjClient.fistTing)
                {
                    //闲家天听。。。
                    cc.log("========闲家天听......======");
                }
