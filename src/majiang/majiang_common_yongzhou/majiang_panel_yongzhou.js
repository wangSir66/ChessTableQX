 ...
 * @type {void | Class | *}
 */
var majiang_panel_yongzhou;
(function() {
    majiang_panel_yongzhou = majiang_panel.extend({
        getJsBind: function(){
            var jsBind = {
                img_roomInfo3D: {
                    img_hunpaiBg: {
        ...
        ctor: function(subObj, jsonFile){
            this._super(majiang_panel_yongzhou, jsonFile);

            subObj.jsBind = subObj.prototype.getJsBind();
            util.assign(subObj.jsBind, majiang_panel_yongzhou.jsBind);
        ...
        createEndOnePanel: function(){
            return new majiang_winGamePanel_yongzhou();
        },

        // @Override 显示大结算
        createGameOverPanel: function(){
            return new majiang_gameOver_yongzhou();
        },

        // @Override 显示设置界面
        createSettingView: function(){
            return new majiang_settingPanel_yongzhou();
        }
    });
    ...
    // @Override 3D吃碰牌特效 开启
    majiang_panel_yongzhou.prototype.isNeedEatActionEffect3D = function() {
        return true;
    };

    // @Override 摸牌动画 开启
    majiang_panel_yongzhou.prototype.isCanPlayNewCardAction = function() {
        return true;
    };

    // @Override 开启出牌放大特效
    majiang_panel_yongzhou.prototype.isOpenPutOutCardAnima = function(){
        return this.getPutCardScaleConfig();
    };

    // @Override 插牌动画 开启
    majiang_panel_yongzhou.prototype.isCanInsertcard = function() {
        return true;
    };

    // @Override 插牌动画倾斜 开启
    majiang_panel_yongzhou.prototype.isNeedCardRotateAction = function(){
        return true;
    };

    // @Override 设置癞子牌颜色 关闭
    majiang_panel_yongzhou.prototype.setLaiZiColor = function(cardNode) {
        return false;
    };

    // @Override top位置玩家出的牌是否相对down玩家旋转180 开启
    majiang_panel_yongzhou.prototype.isCardRotationOfTopPlayer = function(){
        return true;
    };

    // 开局检测距离 开启
    majiang_panel_yongzhou.prototype.isCheckDistance = function() {
        return true;
    };
    ...
    // @Override 初始化游戏数据
    majiang_panel_yongzhou.prototype.initGameData = function() {
    ...
    // @Override 游戏桌面背景
    majiang_panel_yongzhou.prototype.getGameBgList = function(is3D) {
        if(is3D){ // 3dBG资源
    ...
    // @Override 游戏桌面背景名称
    majiang_panel_yongzhou.prototype.getGameBgNameList = function(is3D) {
        if(is3D){
            return ["经典","碧绿","翡翠","海蓝"];
        }
        return ["土黄","翠绿","墨绿","湖蓝"];
    ...
    // @Override 获取语音名字
    majiang_panel_yongzhou.prototype.getVoiceNameList = function() {
        return ["普通话","本地话"];
    };

    // @Override 牌背
    majiang_panel_yongzhou.prototype.getCardBgList = function(is3D) {
        if(is3D){
    ...
    // @Override 牌背名字
    majiang_panel_yongzhou.prototype.getCardBgNameList = function(is3D) {
        if(is3D){
            return ["经典", "流行"];
        }
    ...
    // @Override 牌面
    majiang_panel_yongzhou.prototype.getCardFrontList = function(is3D) {
        if(is3D){
    ...
    // @Override 展示游戏内置聊天信息
    majiang_panel_yongzhou.prototype.showGameSoundChatMessage = function(node, playerNodeName, msg) {
    ...
    // @Override 设置东南西北的方位
    majiang_panel_yongzhou.prototype.updateArrowIconDirection = function(arrowNode) {
    ...
    // @Override 中间的小转盘，刷新
    majiang_panel_yongzhou.prototype.updateArrowRotation2D = function(arrowNode, nextPlayer) {
        if(!this.isInGame()){
            return;
        }
        var tData = MjClient.data.sData.tData;
    ...
    majiang_panel_yongzhou.prototype.setInGameHeadLayout = function(nodeName,layout_head) {
    ...
    // @Override 设置初始值头像位置
    majiang_panel_yongzhou.prototype.setInitHeadLayout = function(nodeName,layout_head) {
        if(!layout_head || !nodeName) return;
    ...
    // @Override 设置房主的标识
    majiang_panel_yongzhou.prototype.updateFangZhuIconStatus = function(headNode) {
        var tData = MjClient.data.sData.tData;
    ...
    // @Override 获得癞子标识
    majiang_panel_yongzhou.prototype.getLaiZiIcon2D = function(){
    ...
    // @Override 添加癞子标识2D
    majiang_panel_yongzhou.prototype.addLaiZiIcon2D = function(cardNode){
    ...
    // @Override 添加癞子标识3D
    majiang_panel_yongzhou.prototype.addLaiZiIcon3D = function(cardNode) {
        if(!this.isCanAddLaiZiIcon(cardNode.tag)){
            return;
        }
     ...
     * 根据麻将类型进行缩放
     */
    majiang_panel_yongzhou.prototype.updateCardNodeScale = function(cardNode){

    };

    // @Override 获得癞子标识的坐标
    majiang_panel_yongzhou.prototype.getHunIconPosition2D = function() {
        return [[60, 77], [52, 70], [40, 84], [74, 68], [61, 78], [61, 78], [22, 30]];
    };

    // @Override 刷新手牌大小
    majiang_panel_yongzhou.prototype.updateHandCardSize = function(node) {
    ...
    // @Override 设置其他牌大小
    majiang_panel_yongzhou.prototype.updateOtherCardSize = function(node) {
        var is3D = MjClient.playui.is3DStyle();
        var maxPlayer = MjClient.playui.getMaxPlayer();
        var playNode = node.getParent();
        var _ds = 0;
        ...
        if(playNode.getName() == "node_left"){
            if(node.getName() == "img_eatFrontCard"){
                if(!is3D){
                    setWgtLayout(node, [0, 0.058], [0, 1], [3.5, -1]);
                }else{
                    _ds = this.isIPad() ? -0.01 : 0;
                    if(maxPlayer == 4){
    ...
    majiang_panel_yongzhou.prototype.getEatCardSpace = function() {
        return cc.winSize.height / 36;
    };

    // @Override 获得手牌之间的缩放比
    majiang_panel_yongzhou.prototype.getHandCardSpaceScale = function() {
     ...
     **/
    majiang_panel_yongzhou.prototype.getDownAndTopNodeEatCardScale = function(mjBgType){
        var scale = 0.93;
        if(mjBgType == 0){
            scale = 0.91;
        }else if(mjBgType == 1){
     ...
     **/
    majiang_panel_yongzhou.prototype.getRightAndLeftNodeEatCardScale = function(mjBgType){
        var scale = 0.65;
        if(mjBgType == 0){
            scale = 0.66;
        }else if(mjBgType == 1){
     ...
     **/
    majiang_panel_yongzhou.prototype.getDownAndTopPutOutCardScale = function(mjBgType){
     ...
     **/
    majiang_panel_yongzhou.prototype.getRightAndLeftPutOutCardScale = function(mjBgType){
    ...
    majiang_panel_yongzhou.prototype.getHandCardFacePosition2D = function(mjBgType) {
     ...
     *  是否开启显示最多听牌标识
     **/
    majiang_panel_yongzhou.prototype.isShowMaxTingCards = function(){
        return true;
    };


    // @Override 获得打出的牌牌背和牌面对应的坐标
    majiang_panel_yongzhou.prototype.getPutCardFacePosition2D = function(mjBgType) {
    ...
    majiang_panel_yongzhou.prototype.getScaleByMjType2D = function(mjBgType) {
    ...
    // @Override 吃牌的坐标
    majiang_panel_yongzhou.prototype.setChiPosition2D = function(node, cardArr, pos, index, chiFirstIndex) {
    ...
    // 断线重连时将所吃的牌放在中间
    majiang_panel_yongzhou.prototype.resetChiCardPos = function(cardArr, index, chiCard) {
    ...
    /** ----------------------------------------  游戏操作  ------------------------------------------**/
    // @Override 关闭自动摸打
    majiang_panel_yongzhou.prototype.isCanAutoPut = function() {
        return false;
    };

    // @Override 发送过的命令给服务器
    majiang_panel_yongzhou.prototype.clickPass = function() {
        var that = this;
    ...
    //Override
    majiang_panel_yongzhou.prototype.updatePlayerEatBtn = function(){
    ...
    //Override
    majiang_panel_yongzhou.prototype.handlerWhenCardTouchEnded = function(cardNode, cardTag){
        var that = this;
        var player = MjClient.playui.getPlayerInfoByName("node_down");
    ...
    //胡牌后牌型贴图展示
    majiang_panel_yongzhou.prototype.showHuTypeImage = function(node, player, playerNodeName) {
        var imgNodes = [];
