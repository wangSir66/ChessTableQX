...
//向服务器发送 过消息
MjClient.MJPass2NetForDaozhouMJ = function()
{
   // console.log(">>>>>>>>>普通  过 <<<<<<<<");
    ...
    else
    {
        if(MjClient.playui.jsBind.eat.hu._node.visible)
        {
            MjClient.showMsg("确认不胡吗?", function(){
                if(MjClient.APP_TYPE.QXSYDTZ == MjClient.getAppType() && MjClient.playui.jsBind.eat.hu._node.visible){
                    MjClient.showToast("你选择了过，暂时放弃胡牌");
                }
                MJPassConfirmToServer();
            }, function() {}, "1");
...
function SetUserVisible_DaozhouMJ(node, off)
{
...
function InitUserHandUI_DaozhouMJ(node, off)
{
    var sData = MjClient.data.sData;
    var tData = sData.tData;
    var pl = getUIPlayer(off);
    ...
    if((tData.tState == TableState.waitJiazhu || tData.tState == TableState.roundFinish) && SelfUid() == pl.info.uid)
    {
        if(pl.mjState == TableState.waitJiazhu || pl.mjState == TableState.roundFinish)
        {
            MjClient.playui.ResetBtnPiao();
        }
        else
        {
            //弹窗等待其他玩家加注
...
var PlayLayer_DaozhouMJ = cc.Layer.extend({
    jsBind: {
        _event: {
            mjhand: function() {
            ...
            roundEnd: function() {
                ...
                function delayExe()
                {
                    ...
                    self.addChild(new EndOneView_DaozhouMJ(),500);
                    var sData = MjClient.data.sData;
                    var tData = sData.tData;
        ...
        down: {
                    ...
                    _event:{
                        ...
                        MJJiazhu:function(msg){
                            ...
                            if(pl.info.uid == msg.uid){
                                MjClient.playui.ResetBtnPiao();
                                this.visible = true;
                                var str = pl.jiazhuNum == 1?"不飘":pl.jiazhuNum == 2?"飘2":pl.jiazhuNum == 3?"飘3":"飘5";
                                this.setString(str);
                            }
                        },
                        mjhand:function(msg){
                            var tData = MjClient.data.sData.tData;
                            var pl = getUIPlayer(0);
                            if(!pl || !tData.areaSelectMode.kepiao){
                                return;
                            }
                            MjClient.playui.ResetBtnPiao();
                            var str = pl.jiazhuNum == 1?"不飘":pl.jiazhuNum == 2?"飘2":pl.jiazhuNum == 3?"飘3":"飘5";
                        ...
                        initSceneData:function(){
                            ...
                            if((tData.tState == TableState.waitJiazhu && pl.mjState == TableState.isReady) || IsArrowVisible())
                            {   
                                var str;
                                if(IsArrowVisible() || SelfUid() == pl.info.uid){
                                    str = pl.jiazhuNum == 1?"不飘":pl.jiazhuNum == 2?"飘2":pl.jiazhuNum == 3?"飘3":"飘5";
                                }else{
                                    str = "已下飘";
                                }
                                this.visible = true;
            ...
            _event: {
                ...
                initSceneData: function(eD) {
                    SetUserVisible_DaozhouMJ(this, 0);
                },
                addPlayer: function(eD) {
                    SetUserVisible_DaozhouMJ(this, 0);
                },
                ...
                mjhand: function(eD) {
                    InitUserHandUI_DaozhouMJ(this, 0);
                },
                roundEnd: function() {
        ...
        right: {
            ...
            _event: {
                ...
                initSceneData: function(eD) {
                    SetUserVisible_DaozhouMJ(this, 1);
                },
                addPlayer: function(eD) {
                    SetUserVisible_DaozhouMJ(this, 1);
                },
        ...
        top: {
            ...
            _event: {
                ...
                initSceneData: function(eD) {
                    SetUserVisible_DaozhouMJ(this, 2);
                },
                addPlayer: function(eD) {
                    SetUserVisible_DaozhouMJ(this, 2);
                },
        ...
        left: {
            ...
            _event: {
                ...
                initSceneData: function(eD) {
                    SetUserVisible_DaozhouMJ(this, 3);
                },
                addPlayer: function(eD) {
                    SetUserVisible_DaozhouMJ(this, 3);
                },
        ...
        eat: {
            ...
            guo: {
                ...
                _touch: function(btn, eT) {
                    if (eT == 2)
                    {
                        MjClient.MJPass2NetForDaozhouMJ();
                    }
                },
                bgimg: {
                    _run: function() {
            ...
            changeui: {
                ...
                changeuibg: {
                    ...
                    guobg: {
                        guo: {
                            _touch: function(btn, eT) {
                                if (eT == 2) MjClient.MJPass2NetForDaozhouMJ();
                            }
                        },
                        fanhui: {
                            _touch: function(btn, et) {
    ...
    ctor: function() {
        ...
        var playui = ccs.load("Play_daoZhouMJ.json");
        playMusic("bgFight");
...
PlayLayer_DaozhouMJ.prototype.CardLayoutRestore = function(node, off)
{
...
//飘按钮的状态
PlayLayer_DaozhouMJ.prototype.ResetBtnPiao = function(){
    var eat = MjClient.playui.jsBind.eat;
    ...
    eat.piao_0._node.setTouchEnabled(true);
    eat.piao_1._node.setTouchEnabled(true);
    eat.piao_2._node.setTouchEnabled(true);
    eat.piao_3._node.setTouchEnabled(true);

    var sData = MjClient.data.sData;
    var tData = sData.tData;
    var selfPlayer = sData.players[SelfUid() + ""];
    if(!tData.areaSelectMode.kepiao){
        return;
    }

    if(selfPlayer.jiazhuNum > 0){
        return;
    }
    ...
    for(var uid in sData.players){
        ...
        if(p.mjState == TableState.waitJiazhu || p.mjState == TableState.isReady/* || p.mjState == TableState.roundFinish*/){
            count++;
        }
    }
    ...
    // if((tData.tState == TableState.waitJiazhu || tData.tState == TableState.roundFinish) && 
    //     !(TableState.waitPut == selfPlayer.mjState || TableState.waitEat == selfPlayer.mjState || TableState.waitCard == selfPlayer.mjState))
    // {
        var btnArr = [];
        btnArr.push(eat.piao_0._node);
        btnArr.push(eat.piao_1._node);
        btnArr.push(eat.piao_2._node);
        btnArr.push(eat.piao_3._node);

        for(var i = 0;i < btnArr.length;i++){
            btnArr[i].visible = true;
            setWgtLayout(btnArr[i], [0, 0.1], [0.5, 0], [(1 - btnArr.length) / 1.6+ i * 1.3, 3.6], false, false);
        }
    // } 
};

// 判断吃碰杠胡的状态
PlayLayer_DaozhouMJ.prototype.EatVisibleCheck = function()
{
    var eat = MjClient.playui.jsBind.eat;
