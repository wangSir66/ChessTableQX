...
function addWxHeadToEndUI_hanShouPaoHuZi(node,off)
{
    ...
    setRoundEndUserOffline_hanShouPaoHuZi(frame, pl);
...
function reInitarrCardVisible_hanShouPaoHuZi()
{
...
//添加手牌
function addWinMjHand_hanShouPaoHuZi(_node,off) {
    var sData = MjClient.data.sData;
    var tData = sData.tData;
    ...
    for(var i = 0;i < sortArr.length;i++){
        ...
        for(var j = 0;j < childSort.length;j++){
            ...
            clone.visible = true;
            clone.loadTexture(MjClient.cardPath_hanShouPaoHuZi+"out" + card +".png");
            var off_y = 0;
            if(_node.getName() == "rightPanel"){
                off_y = clone.height;
                clone.anchorX = 1;
...
var EndOneView_hanShouPaoHuZi = cc.Layer.extend({
    _block:null,
    _endoneuiNode:null,
    jsBind:{
        block: {
        ...
        btn_ready:{
            ...
            _click: function(btn,eT){
                ...
                reInitarrCardVisible_hanShouPaoHuZi();
            }
        },
        btn_logic: {
            _run: function(){
                ...
                _run:function(){
                    ...
                    if(tData.cardNext < cards.length){
                        ...
                        for(var i = tData.cardNext;i < cards.length;i++){
                            ...
                            cloneCard.loadTexture(MjClient.cardPath_hanShouPaoHuZi+"out" + card +".png");
            ...
            leftPanel:{
                _run:function(){
                   if(MjClient.rePlayVideo != -1){
                    ...
                    addWinMjHand_hanShouPaoHuZi(this, changeUIOff_hanShouPaoHuZi(3));
                }
            },
            rightPanel:{
                _run:function(){
                    ...
                    addWinMjHand_hanShouPaoHuZi(this, changeUIOff_hanShouPaoHuZi(2));
                }
            },
            xingPanel:{
                _run:function(){
                    ...
                    addWinMjHand_hanShouPaoHuZi(this, changeUIOff_hanShouPaoHuZi(1));
                }
            }
        },
        uiNode: {
            ...
            diPaiNode: {
                _run: function(){
                    ...
                    var cloneFunc = function(cardNum){
                        ...
                        cloneCard.loadTexture(MjClient.cardPath_hanShouPaoHuZi+"out" + cardNum + ".png");
                        return cloneCard;
                    }


                    var sumCardNum = cards.length - tData.cardNext;//牌池剩余牌(y埋牌的情况下，包括埋牌)
                    var oneLine = true;// 一行
                    var maiPaiStartIndex = 80;//埋牌开始索引
                    var maxColumnCount = 21;
                    //安化暂时没有埋牌
                    if(tData.maxPlayer == 2 && tData.areaSelectMode.isMaiPai){
                        // 两人有埋牌
                        var maiType = tData.areaSelectMode.hasOwnProperty("maiPaiType") ? tData.areaSelectMode.maiPaiType : 2;
                        maiPaiStartIndex = cards.length - maiType * 10;
                        // 加一个埋牌文本
                    ...
                    startPosX -= 10;
                    var scrollView;
                    //有埋牌的情况下，牌池数量超过单行最大牌数maxColumnCount时，将底牌放入滚动容器中
                    if(tData.maxPlayer == 2 && tData.areaSelectMode.isMaiPai && (maiPaiStartIndex - tData.cardNext) > maxColumnCount){
                        oneLine = false;
                        var maiType = tData.areaSelectMode.hasOwnProperty("maiPaiType") ? tData.areaSelectMode.maiPaiType : 2;
                        var cardModel = self.getChildByName("card");
                        scrollView = ccui.ScrollView.create();
                        scrollView.setContentSize(cc.size(45 *20 + cardModel.width, cardModel.height));
                        scrollView.setDirection(ccui.ScrollView.DIR_HORIZONTAL);
                        scrollView.setBounceEnabled(true);
                        scrollView.setInnerContainerSize(cc.size(45 * (sumCardNum - maiType * 10), cardModel.height));
                        var pos = cc.pSub(cc.p(startPosX,66),cc.p(cardModel.width/2, cardModel.height/2));
                        pos =  self.convertToWorldSpace(pos);
                        scrollView.setPosition(this.convertToNodeSpace(pos));
                    ...
                    for (var i = tData.cardNext; i < cards.length; i++) {
                        ...
                        var posX, posY;
                        if(i >= maiPaiStartIndex){//埋的牌
                            posX = startPosX + (i - maiPaiStartIndex) * 45  ;
                            posY = 20;
                            var pos = self.convertToWorldSpace(cc.p(posX, posY));
                            pos = this.convertToNodeSpace(pos);
                            cardImg.setPosition(pos);
                            this.addChild(cardImg);
                            continue;
                        }

                        //埋牌情况下，单行最大牌数maxColumnCount
                        if(scrollView && cc.sys.isObjectValid(scrollView)){
                            cardImg.setPosition(cc.p(cardImg.width/2 + (i - tData.cardNext) * 45, cardImg.height/2));
                            scrollView.addChild(cardImg);
                            continue;
                        }

                        //埋牌情况下，底牌不超过单行最大牌数maxColumnCount
                        if(tData.areaSelectMode.isMaiPai){
                            posX = startPosX + (i - tData.cardNext) * 45;
                            posY = 66;
                            cardImg.setPosition(this.convertToNodeSpace(self.convertToWorldSpace(cc.p(posX, posY))));
                            this.addChild(cardImg);
                            continue;
                        }

                        //没有埋牌的情况下
                        posX = startPosX + Math.floor((i - tData.cardNext) % maxColumnCount) * 45;
                        posY =  Math.floor((i - tData.cardNext) / maxColumnCount) >= 1 ? 20 : 66;
                        cardImg.setPosition(this.convertToNodeSpace(self.convertToWorldSpace(cc.p(posX, posY))));
                        this.addChild(cardImg);
                    }
            ...
            uiBg:{
                headNode: {
                    _run: function(){
                        ...
                        if(tData.winner != -1)
                            winOff = getUiOffByUid_hanShouPaoHuZi(pl.info.uid);

                        addWxHeadToEndUI_hanShouPaoHuZi(this,winOff);
                        ...
                        for(var index = 0;index<playerCount;index++){
                            if(index != winOff){
                                ...
                                addWxHeadToEndUI_hanShouPaoHuZi(cloneThis,index);
                                cloneNum++;
                            }
                        }
                    }
    ...
    ctor:function () {
        ...
        if (MjClient.playui.getCardFilePath) {
            //兼容新版
            MjClient.cardPath_hanShouPaoHuZi = MjClient.playui.getCardFilePath();
        }
    ...
    setWinCard: function(){
        ...
        var cloneCardsSetData = function(cardNode,i,num){
            ...
            cardImg.loadTexture(MjClient.cardPath_hanShouPaoHuZi+"out" + num + ".png");
            return cardImg;
        }
                ...
                var getHuXi = function(cards){
                    ...
                    for(var t = 0;t < normal.length;t++){
                        if(normal[t].toString() == cards.toString()){
                            var huxi = huXiScore_hanShouPaoHuZi("chi",cards[0]);
                            return huxi;
                        }
                    }
                    return 0;
        ...
        // 手里的牌的名字
        var myYCNName = {kan: "坎",ti: "提", peng : "碰", dan : "", men : ""};
        // 设置位置，设置资源
        for (var index in handSortArr) {
            ...
            if(myYCNName[cardData.name] || myYCNName[cardData.name] === "") {
    ...
    setHuNameContnet: function(){
        ...
        for (var index in pl.hzdesc) {
            ...
            if(index == "huXi" || index == "totalFan" || index == "totalTun" || index == "piaoFen"){
    ...
    setXingCardData: function(tData,num){
        ...
        card_img.loadTexture(MjClient.cardPath_hanShouPaoHuZi+"out" + tData.hunCard +".png");
    ...
    getMingTangInfo: function(name,tData){
        ...
        var table = {hongHu: "红胡", dianHu: "点胡", jiaHong: "夹红", wuHu: "乌胡", duiZiHu: "对子胡",sanTiWuKan:"三提五坎",};
