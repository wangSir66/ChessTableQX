...
var _isShowShaiziAni_chaogu = false; //为了控制mjhand之后播骰子动画时newCard的状态显示
//向服务器发送 过消息
MjClient.MJPass2NetForchaogu = function()
{
   // console.log(">>>>>>>>>普通  过 <<<<<<<<");
    cc.log("====================send======pass=====");
...
var showShaiziAni_chaogu = false;
function showShaiziAni_chaogu_old(poff)
{
    if(poff == 0) {
        ...
        if(MjClient.rePlayVideo != -1)//回放
        {
            for (var off = 0; off < 4; off++) {
                ...
                InitUserHandUI_chaogu(_node, off);
            }
            playEffect("shuffle");
...
function updateHeadUI_chaogu(node, off)
{      
    cc.log('updateHeadUI_chaogu', off)
    var pl = getUIPlayer(off);
    if(!pl) return;
    showHandCard_chaogu(node, off);
    MjClient.playui.CardLayoutRestore(node, off);
    cc.log('updateHeadUI_chaogu update success', off)
...
function SetUserVisible_chaogu(node, off)
{
    if (_isShowShaiziAni_chaogu)  // 解决摇骰子时断线重连调用本函数，会手牌重复的bug by cyc
        return;
...
function showHandCard_chaogu(node, off)
{
    var sData = MjClient.data.sData;
    var tData = sData.tData;
    var pl = getUIPlayer(off);
...
function InitUserHandUI_chaogu(node, off)
{
    var sData = MjClient.data.sData;
    var tData = sData.tData;
    var pl = getUIPlayer(off);
    ...
    if (SelfUid() == pl.info.uid) {
        if (tData.tState == TableState.waitJiazhu && pl.jiazhuNum < 0) {
            if (pl.mjState == TableState.waitJiazhu && MjClient.playui.getChildByName("JiaZhu") == null) {
                ...
                var type = tData.areaSelectMode["piaofen"] == -1 ? 4 : 1;
                var layer = new PiaoFen4ChaoGu(type);
...
function initFlower_chaogu() {
    initFlower(false, false);
}


var PlayLayer_chaogu = cc.Layer.extend({
    jsBind: {
        _event: {
            mjhand: function() {
            ...
            roundEnd: function() {
                ...
                function showEndCards()
                {
                    ...
                    self.addChild(new EndOneView_chaogu(),500);
                }
                function delayExe()
                {
      ...
      //           }
        ...
        BtnPutCard:{ //add by  sking for put card button
            ...
            _event:{
                ...
                waitPut: function() {
                    if (_isShowShaiziAni_chaogu) return;
        ...
        down: {
            ...
            _event: {
                ...
                waitJiazhu: function(msg) {
                    ...
                    cc.log("piaofenType",tData.areaSelectMode["piaofen"]);
                    //弹窗选择是否飘分
                    var layer = new PiaoFen4ChaoGu(type,function(){
                        //弹窗等待
                        MjClient.playui._jiazhuWait.visible = true;
                    });
                    if (layer.isVisible())
            ...
            changeui: {
                ...
                changeuibg: {
                    ...
                    guobg: {
                        guo: {
                            _touch: function(btn, eT) {
                                if (eT == 2) MjClient.MJPass2NetForchaogu();
                            }
                        },
                        fanhui: {
                            _touch: function(btn, et) {
    ...
    ctor: function() {
        this._super();
        var playui = ccs.load("Play_chaogu.json");
...
PlayLayer_chaogu.prototype.willHuShowArrow = function()
{
    var tData = MjClient.data.sData.tData;
...
PlayLayer_chaogu.prototype.CardLayoutRestore = function(node, off, isNewCard)
{
...
// 判断吃碰杠胡的状态
PlayLayer_chaogu.prototype.EatVisibleCheck = function()
{
    var eat = MjClient.playui.jsBind.eat;
...
PlayLayer_chaogu.prototype.jiaozhuToPiaoniao = function(jiazhuLayer, msg) {
    var tData = MjClient.data.sData.tData;
