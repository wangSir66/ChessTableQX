...
//向服务器发送 过消息
MjClient.MJPass2NetForDonghai = function()
{
...
function SetUserVisible_donghai(node, off)
{
    //var sData = MjClient.data.sData;
    //return;
...
function InitUserHandUI_donghai(node, off)
{
    var sData = MjClient.data.sData;
    var tData = sData.tData;
    var pl = getUIPlayer(off);
    ...
    for(var i = 0; i < pl.mjchi.length; i++)
    {
        ...
        //by sking 为了，碰的牌都放在中间
        if(pl.mjchiCard[chiIdx-1] == pl.mjchi[i])//吃的横牌表示吃的是哪张牌
        {
                var _currentChiCard = pl.mjchiCard[chiIdx-1]; //当前吃的那张牌
                var _currentChiArray = [];//当前吃牌的数组
                for(var n = 0;n < 3;n++) _currentChiArray.push(pl.mjchi[3*(chiIdx-1) + n]);
                _currentChiArray.splice(_currentChiArray.indexOf(_currentChiCard),1);
                getNewCard(node, "up", "chi", _currentChiArray[0], off);
                cdui = getNewCard(node, "up", "chi", _currentChiCard, off, "heng");
                getNewCard(node, "up", "chi", _currentChiArray[1], off);
        }
...
/*        ShowEatActionAnim(node,ActionType.FLOWER,off);
    ...
    if (pl.jiazhuNum > 0)
    {
        MjClient.majiang.setJiaZhuNum(node, pl);
    }

    MjClient.playui.CardLayoutRestore(node, off);
}

function initFlower_donghai() {
    initFlower(false, false);
}


var PlayLayer_donghai = cc.Layer.extend({
    jsBind: {
        _event: {
            mjhand: function() {
            ...
            roundEnd: function() {
                ...
                function delayExe()
                {
                    ...
                    self.addChild(new EndOneView_donghai(),500);
                }
        ...
        banner: {
            ...
            Button_1: {
                _visible : true,
                _click: function() {
                    MjClient.openWeb({url:MjClient.GAME_TYPE.DONG_HAI,help:true});
                }
            },
            hunPai:{
                baidaBg:{
            ...
            _event: {
                ...
                waitJiazhu:function (eD) {
                    ...
                    /*
                        弹窗加注
                     */
                    var layer = new jiaZhuLayer(function(){
                        //弹窗等待
                        MjClient.playui._jiazhuWait.visible = true;
                        var pl = getUIPlayer(0)
            ...
            changeui: {
                ...
                changeuibg: {
                    ...
                    guobg: {
                        guo: {
                            _touch: function(btn, eT) {
                                if (eT == 2) MjClient.MJPass2NetForDonghai();
                            }
                        },
                        fanhui: {
                            _touch: function(btn, et) {
    ...
    ctor: function() {
        ...
        var playui = ccs.load(res.Play_donghai_json);
        playMusic("bgFight");
        ...
        MjClient.playui._waitLayer = null;
        //添加滚动通知 by sking
...
PlayLayer_donghai.prototype.CardLayoutRestore = function(node, off)
{
    ...
    for(var i = 0; i < orders.length; i++)
    {
        ...
        if(off % 2 == 0)//自己或者对家
        {
            ...
            if(ci.name == "chi")
            {
                cc.log("-----------------cardlayour chi = " + ci.tag);
                if(ci.heng)
                {
                    cc.log("==========================cardlayour chi heng = " + ci.tag);
                }
            }
...
// 判断吃碰杠胡的状态
PlayLayer_donghai.prototype.EatVisibleCheck = function()
{
    var eat = MjClient.playui.jsBind.eat;
