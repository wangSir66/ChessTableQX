(function() {
    ...
    PokerWuXue510K.prototype.CARDTPYE = CARDTPYE;
    ...
    //两副牌，4人，每人27张
    PokerWuXue510K.prototype.randomCards = function(areaSelectMode, tData) {
        var cards = [];
    ...
    PokerWuXue510K.prototype.isLaizi = function(card) {
     ...
     * @return {number}
     */
    PokerWuXue510K.prototype.calPoint = function(num) {
        if (!num) return -1;
     ...
     * @return {number}
     */
    PokerWuXue510K.prototype.calFlower = function(num) {
        num = this.removeLaiziSign(num);
        return (num + 3) % 4;
    }
     ...
     * @return {bool}
     */
    PokerWuXue510K.prototype.isShun = function(oCards) {
     ...
     * @return {bool}
     */
    PokerWuXue510K.prototype.isTongHua = function(oCards) {
     ...
     * @return {bool}
     */
    PokerWuXue510K.prototype.is510K = function(oCards) {
        if (!oCards || oCards.length != 3)
            return false;
     ...
     * @return {bool}
     */
    PokerWuXue510K.prototype.isTongHua510K = function(oCards) {
     ...
     * @return {bool}
     */
    PokerWuXue510K.prototype.isLiandui = function(oCards) { //oCards有序
     ...
     * @return {bool}
     */
    PokerWuXue510K.prototype.isAllLaizi = function(oCards) { //oCards有序
        if (!oCards || oCards.length <= 0)
            return false;
     ...
     */
    PokerWuXue510K.prototype.calType = function(oCards) {
        if (!oCards)
            return -1;
    ...
    PokerWuXue510K.prototype.cardsType = function(cards) {
        return this.calType(cards)
    };
    
    // 计算牌型点数
    PokerWuXue510K.prototype.calCardsValue = function (oCards) {
        if (!oCards || oCards.length == 0) {
            return -1;
        }
    ...
    // 牌点比较函数（从小到大）
    PokerWuXue510K.prototype.cardValueCmp = function(a, b) {
        var pa = this.calPoint(a);
        var pb = this.calPoint(b);
        if (pa == pb) return this.removeLaiziSign(a) - this.removeLaiziSign(b);
        return pa - pb;
    };
     ...
     * @return {number} 返回癞子数
     */
    PokerWuXue510K.prototype.transformAndGetLaizi = function(oCards, cards) {
        var laizis = [];
        for (var i = 0; i < oCards.length; i++) {
            if (this.isLaizi(oCards[i]))
     ...
     */
    PokerWuXue510K.prototype.canPut = function(oCards, oLastCards, areaSelectMode) {
        var cards = oCards.slice();
    ...
    // 检查是否能出牌
    PokerWuXue510K.prototype.checkPut = function (oHands, oCards, lastPutCard, areaSelectMode) {
     ...
     */
    PokerWuXue510K.prototype.sortHandCards = function(oCards, sortType) {
        var cards = oCards.slice();
    ...
    // 找N张点数一样的牌
    PokerWuXue510K.prototype.findNSameCard = function (hands, point, n) {
    ...
    PokerWuXue510K.prototype.findAllSameCard = function(hands, point) {
        var sameCards = [];
    ...
    PokerWuXue510K.prototype.indexOfCards = function(cardList, cards) {
    ...
    PokerWuXue510K.prototype.isSameCards = function(cards1, cards2) {
     ...
     */
    PokerWuXue510K.prototype.lieju = function(parr, num){
        var tmps = [];
     ...
     */
    PokerWuXue510K.prototype.findCardByType = function(hands, laizis, type, cardCount, areaSelectMode, extras) {
        var rets = [];
    ...
    PokerWuXue510K.prototype.findFirstPutCards = function(oHands, areaSelectMode) {
    ...
    // 压牌提示
    PokerWuXue510K.prototype.findTipsForBackHand = function(oHands, oLastCards, areaSelectMode) {
        var hands = [];
        var handLaizis = this.transformAndGetLaizi(oHands, hands);

        // 固定带癞子牌
        areaSelectMode.laiziOption = 1;
        ...
        for (var i = 0; i < hands.length; i++) {
            ...
            pointCounts[p] = pointCounts[p] ? pointCounts[p] + 1 : 1;
        }
        ...
        var findRets = function(laizis) {
            ...
            if (laiziCount === 0) {
                // 手牌没有大小王的情况（无赖子情况）
                // 3炸 < 4炸 < 杂510K < 5炸 < 方510K < 梅510K < 红510K < 黑510K < 6炸 < 7炸 < 8炸
                var rets = [];
                putOrder = [
                ...
                if (lastCardType < CARDTPYE.big) {
                    ...
                    else if(minZhaDanCount === 5 && laiziCards.length <= 0) {
                        for (var i = putOrder.length - 1; i >= 0; i--) {
                ...
                // 手牌有大小王的情况（带癞子）
                // 3炸 < 4炸 < 杂510K < 5炸 < 方510K < 梅510K < 红510K < 黑510K < 6炸 < 两个王 < 7炸 < 8炸 < 9炸
                putOrder = [
                    {cardType: CARDTPYE.zhadan, cardCount: 3},
                    {cardType: CARDTPYE.zhadan, cardCount: 4},
                    {cardType: CARDTPYE.z_510k, cardCount: 3},
                ...
                if (laiziCount === 1) {
                    ...
                    putOrder.push({cardType: CARDTPYE.zhadan, cardCount: 9});
                }
                else if (laiziCount === 2) {
                    putOrder.push({cardType: CARDTPYE.wangzha, cardCount: 2});   //双王炸
                    putOrder.push({cardType: CARDTPYE.zhadan, cardCount: 7});
                    putOrder.push({cardType: CARDTPYE.zhadan, cardCount: 8});
                    putOrder.push({cardType: CARDTPYE.zhadan, cardCount: 9});
                    putOrder.push({cardType: CARDTPYE.zhadan, cardCount: 10});
                }
                else if (laiziCount === 3) {
                    putOrder.push({cardType: CARDTPYE.zhadan, cardCount: 7});
                    putOrder.push({cardType: CARDTPYE.wangzha, cardCount: 3});   //三王炸
                    putOrder.push({cardType: CARDTPYE.zhadan, cardCount: 8});
                    putOrder.push({cardType: CARDTPYE.zhadan, cardCount: 9});
                    putOrder.push({cardType: CARDTPYE.zhadan, cardCount: 10});
                    putOrder.push({cardType: CARDTPYE.zhadan, cardCount: 11});
                }
                    ...
                    putOrder.push({cardType: CARDTPYE.zhadan, cardCount: 10});
                    putOrder.push({cardType: CARDTPYE.zhadan, cardCount: 11});
                    putOrder.push({cardType: CARDTPYE.zhadan, cardCount: 12});
                    putOrder.push({cardType: CARDTPYE.wangzha, cardCount: 4});  //四王炸
                }
                ...
                if (putOrder.length > 0) {
                    ...
                    var validCounts = [];
                    for (var p in pointCounts) {
                        var newCount = pointCounts[p] + laiziCount;
                    ...
                    for (var i = putOrder.length - 1; i >= 0; i--) {
                        ...
                        if (validCounts.indexOf(putOrder[i].cardCount) < 0) {
                            putOrder.splice(i,1);
                        }
                    }
                    ...
                    for (var i = 0; i < putOrder.length; i++) {
                        ...
                        tempRets = this.findCardByType(hands, laizis, curType, cardCount, areaSelectMode, curType == CARDTPYE.f_510k ? {flower: putOrder[i].flower} : null);
                        for (var j = 0; j < tempRets.length; j++) {
                            if (curType == CARDTPYE.zhadan && pointCounts[this.calPoint(tempRets[j][0])] + laiziCount > cardCount) {
                                // 存在该点数更长的炸弹，则丢弃短的结果
                                continue;
                            }
                            if (this.canPut(tempRets[j], oLastCards, areaSelectMode)) {
     ...
     * @return {array} 提示的牌
     */
    PokerWuXue510K.prototype.tipCards = function(oHands, oLastCards, areaSelectMode) {
        ...
        if (!oLastCards || oLastCards.length <= 0) {
            ...
            cc.log("Tom ------------------  压牌 ----- " + JSON.stringify(this.findTipsForBackHand(oHands, oLastCards, areaSelectMode)));
     ...
     */
    PokerWuXue510K.prototype.getLaiziCardsFromRet = function(ret) {
        if (!ret || ret.length <= 0)
            return [];
     ...
     */
    PokerWuXue510K.prototype.getRealLaizi = function(laiziCard) {
        if (!laiziCard)
            return -1;
     ...
     * @param {array} rets 结果
     */
     PokerWuXue510K.prototype.removeSameResults = function(rets) {
        var pointsCache = {};
     ...
     * @cardValue {number} 
     */
    PokerWuXue510K.prototype.addLaiziSign = function(cardValue,laiziValue) {
     ...
     * @cardValue {number} 
     */
    PokerWuXue510K.prototype.removeLaiziSign = function(cardValue) {
        return cardValue % LAI_ZI_SIGN;
    };
     ...
     * @cardValue {number} 
     */
    PokerWuXue510K.prototype.haveLaiziSign = function(cardValue) {
        return cardValue > LAI_ZI_SIGN;
    };
     ...
     */
    PokerWuXue510K.prototype.replaceLaizi = function(laiziValue,point,cards) {
    ...
    if(typeof(module)!="undefined" && module.exports)    
        module.exports = PokerWuXue510K;
    
    if(typeof(MjClient) != "undefined")
        MjClient.poker_WuXue510K = new PokerWuXue510K();
