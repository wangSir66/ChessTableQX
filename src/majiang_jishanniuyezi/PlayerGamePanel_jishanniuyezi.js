...
//向服务器发送 过消息
MjClient.MJPass2NetForJiShanNiuYeZi = function()
{
   // console.log(">>>>>>>>>普通  过 <<<<<<<<");
    cc.log("====================MJPass2NetForJiShanNiuYeZi======pass=====");
...
function SetUserVisible_jiShanNiuYeZi(node, off)
{
    //var sData = MjClient.data.sData;
    //return;
...
function DealMJPut_jiShanNiuYeZi(node, msg, off, outNum)
{
    DealMJPut_shanXiApp(node, msg, off, outNum);
}

function InitUserHandUI_jiShanNiuYeZi(node, off)
{
    var sData = MjClient.data.sData;
    var tData = sData.tData;
    var pl = getUIPlayer(off);
    ...
    for(var i = 0; i < pl.mjput.length; i++)
    {
        ...
        DealMJPut_jiShanNiuYeZi(node, msg, off, i);
    }
...
function initFlower_jiShanNiuYeZi() {
    initFlower(false, false);
}


var PlayLayer_jiShanNiuYeZi = cc.Layer.extend({
    jsBind: {
        _event: {
            mjhand: function() {
            ...
            roundEnd: function() {
                ...
                function delayExe()
                {
                    ...
                    self.addChild(new EndOneView_jiShanNiuYeZi(),500);
                }
        ...
        banner: {
            ...
            Button_1: {
                _visible : true,
                _click: function() {
                    MjClient.openWeb({url:MjClient.GAME_TYPE.JI_SHAN_NIU_YE_ZI,help:true});
                }
            },
            hunPai:{
                baidaBg:{
        ...
        eat: {
            ...
            guo: {
                ...
                _touch: function(btn, eT) {
                    if (eT == 2)
                    {
                        MjClient.MJPass2NetForJiShanNiuYeZi();
                    }
                },
                bgimg: {
                    _run: function() {
            ...
            changeui: {
                ...
                changeuibg: {
                    ...
                    guobg: {
                        guo: {
                            _touch: function(btn, eT) {
                                if (eT == 2) MjClient.MJPass2NetForJiShanNiuYeZi();
                            }
                        },
                        fanhui: {
                            _touch: function(btn, et) {
    ...
    ctor: function() {
        this._super();
        var playui = ccs.load("Play_jishanniuyezi.json");
...
PlayLayer_jiShanNiuYeZi.prototype.CardLayoutRestore = function(node, off)
{
...
// 判断吃碰杠胡的状态
PlayLayer_jiShanNiuYeZi.prototype.EatVisibleCheck = function()
{
    var eat = MjClient.playui.jsBind.eat;
        ...
        if(IsTurnToMe())
        {
            ...
            if (!pl.isTing) {
                ...
                for (var i = 0; i < pl.mjhand.length; i++) {
                    ...
                    if (MjClient.majiang.canTing(cardsAfterPut, pl)) {
            ...
            var rtn = leftCard > 0 && MjClient.majiang.canGang1(pl.mjpeng, pl.mjhand, pl.isTing, pl);
...
/*
    玩家ui，信息
 */
PlayLayer_jiShanNiuYeZi.prototype.Node_player = function(off)
{
    ...
    UIEventBind(null, _node, "initSceneData", function (eD) {
        SetUserVisible_jiShanNiuYeZi(_node, off);
    ...
    UIEventBind(null, _node, "mjhand", function (eD) {
        InitUserHandUI_jiShanNiuYeZi(_node, off);
    });
    ...
    UIEventBind(null, _node, "MJPut", function (eD) {
        ...
        DealMJPut_jiShanNiuYeZi(_node,eD,off);
        hideTingBtn();
        if(_tingCardNumNode) _tingCardNumNode.visible = false;
