...
function SetEndOneUserUI_jiShanNiuYeZi(node,off)
{
	...
	if(pl.isTing)
    {
        ...
        if(pl.putCardAfterTing >= 0 && pl.putCardAfterTing != -1)
        {
			...
			if(tData.areaSelectMode["tingPaiBuKou"] && pl.istingcard){
            	_cd = pl.istingcard;
			}
...
var EndOneView_jiShanNiuYeZi = cc.Layer.extend({
	jsBind:{
		block: {
		...
		dir: {
			...
			_text: function () {
				...
				var str1 = arr[1];
                var str = str1.replace(GameCnName[MjClient.GAME_TYPE.DA_TONG_GUAI_SAN_JIAO]+",", "");
				return (str);
			}
		},
		dir_0: {
		...
		head0:{
		   ...
		   _run:function(){ SetEndOneUserUI_jiShanNiuYeZi(this,0); },
		...
		head1:{
		  ...
		  _run:function(){ SetEndOneUserUI_jiShanNiuYeZi(this,1); }
		},
		head2:{
		   head:{
			   zhuang:{_visible:false}
		   ...
		   _run:function(){ SetEndOneUserUI_jiShanNiuYeZi(this,2); }
		},
		head3:{
			 head:
			 {
			...
			_run:function(){ SetEndOneUserUI_jiShanNiuYeZi(this,3); }
			},
		count_down:{
                _visible :function()
                {
    ...
    ctor:function () {
        this._super();
        var endoneui = ccs.load("endOne_jishanniuyezi.json");
