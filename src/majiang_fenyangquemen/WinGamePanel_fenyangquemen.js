...
function CardLayoutRestoreForEndOne_fenyangquemen(node, endonepl)
{
    ...
    {
        cc.log("mjhand has resort------------");
        uistand.sort(TagOrder);//7.06 汾西硬扣结算时候取消花牌排在最前面
    }
...
function SetEndOneUserUI_fenyangquemen(node,off)
{
	var sData=MjClient.data.sData;
	var tData=sData.tData;
...
var EndOneView_fenyangquemen = cc.Layer.extend({
	jsBind:{
		block: {
		...
		btnShowHeap:{
                ...
                _click:function()
                {
                	if(MjClient.endoneui){
                        var fenyangquemenCardsHeap = new PopupShowCardsHeap();
                        fenyangquemenCardsHeap.setName("fenyangquemenCardsHeap");
                        MjClient.endoneui.addChild(fenyangquemenCardsHeap);
                    }
                },
            },
        dir: {
		...
		head0:{
		   ...
		   _run:function(){ SetEndOneUserUI_fenyangquemen(this,0); },
		...
		head1:{
		  ...
		  _run:function(){ SetEndOneUserUI_fenyangquemen(this,1); }
		},
		head2:{
		   head:{
			   zhuang:{_visible:false}
		   ...
		   _run:function(){ SetEndOneUserUI_fenyangquemen(this,2); }
		},
		head3:{
			 head:
			 {
			...
			_run:function(){ SetEndOneUserUI_fenyangquemen(this,3); }
			},
		count_down:{
                _visible :function()
                {
    ...
    ctor:function () {
        this._super();
        var endoneui = ccs.load("endOne_fenyangquemen.json");
