...
//向服务器发送 过消息
MjClient.MJPass2NetForfenyangquemen = function()
{
   // console.log(">>>>>>>>>普通  过 <<<<<<<<");
    cc.log("====================MJPass2NetForfenyangquemen======pass=====");
...
function SetUserVisible_fenyangquemen(node, off)
{
    //var sData = MjClient.data.sData;
    //return;
...
//处理出牌,放一张牌，打牌动作
function DealMJPut_fenyangquemen(node, msg, off, outNum)
{
...
function InitUserHandUI_fenyangquemen(node, off)
{
    var sData = MjClient.data.sData;
    var tData = sData.tData;
    var pl = getUIPlayer(off);
    ...
    for(var i = 0; i < pl.mjput.length; i++)
    {
        ...
        DealMJPut_fenyangquemen(node, msg, off, i);
    }
...
function initFlower_fenyangquemen() {
    initFlower(false, false);
}


var PlayLayer_fenyangquemen = cc.Layer.extend({
    jsBind: {
        _event: {
            mjhand: function() {
            ...
            roundEnd: function() {
                ...
                function delayExe()
                {
                    ...
                    self.addChild(new EndOneView_fenyangquemen(),500);
                }
        ...
        eat: {
            ...
            ting: {
                ...
                _touch: function(btn, eT) {
                    if (eT == 2)
                    {
                        ...
                        var tingCards = MjClient.majiang.getCheckTingHuCards(currentCard,pl.mjhand,pl);
            ...
            guo: {
                ...
                _touch: function(btn, eT) {
                    if (eT == 2)
                    {
                        MjClient.MJPass2NetForfenyangquemen();
                    }
                },
                bgimg: {
                    _run: function() {
            ...
            changeui: {
                ...
                changeuibg: {
                    ...
                    guobg: {
                        guo: {
                            _touch: function(btn, eT) {
                                if (eT == 2) MjClient.MJPass2NetForfenyangquemen();
                            }
                        },
                        fanhui: {
                            _touch: function(btn, et) {
            ...
            _event: {
                ...
                waitPut: function(eD) {
                    ...
                    MjClient.playui.EatVisibleCheck();
                    var tData = MjClient.data.sData.tData;
                    if (!tData.areaSelectMode["baoTing"]) 
                    {
    ...
    ctor: function() {
        this._super();
        var playui = ccs.load("Play_fenyangquemen.json");
        ...
        if(pl && (pl.mjState == TableState.isReady || pl.mjState == TableState.roundFinish))
        {
            ...
            if(pl != null)
            {
                if (pl.isTing) {
                    ...
                    if (off == 0)
                    {
                        var tingSet = MjClient.majiang.calTingSet(pl.mjhand,pl);
...
PlayLayer_fenyangquemen.prototype.CardLayoutRestore = function(node, off)
{
...
// 判断吃碰杠胡的状态
PlayLayer_fenyangquemen.prototype.EatVisibleCheck = function()
{
    var eat = MjClient.playui.jsBind.eat;
    ...
    var sData = MjClient.data.sData;
    var tData = sData.tData;
    var leftCard = MjClient.majiang.getAllCardsTotal() - tData.cardNext - tData.huangNum;
...
/*            //检测补花
            ...
            if(rtn.length > 0 && leftCard > 0/*&& pl.isNew*/)
            {
...
/*
    玩家ui，信息
 */
PlayLayer_fenyangquemen.prototype.Node_player = function(off)
{
    ...
    UIEventBind(null, _node, "initSceneData", function (eD) {
        SetUserVisible_fenyangquemen(_node, off);
    ...
    UIEventBind(null, _node, "mjhand", function (eD) {
        InitUserHandUI_fenyangquemen(_node, off);
    });
    ...
    UIEventBind(null, _node, "MJPut", function (eD) {
        ...
        if (pl && eD.uid == pl.info.uid)
        {
            ...
            if (tData.areaSelectMode["baoTing"]) 
            {
                if(_tingCards && pl.isTing) setTingCards_JINZHONGMJ(_tingCards,tingSet);
            }
            else
            {
        ...
        DealMJPut_fenyangquemen(_node,eD,off);
        hideTingBtn();
        if(_tingCardNumNode) _tingCardNumNode.visible = false;
