...
var CreateRoomNode_TYZZ = CreateRoomNode.extend({
    setKey: function() {
        ...
        this.localStorageKey.KEY_TYZZ_XX_tuoguan      =  "_TYZZ_XX_TUO_GUAN";              //湘乡的托管
    },
    ...
    initPlayNode: function() {
        ...
        this._playNode_niaoType_2 = _play.getChildByName("zhuaniao_1");
        this._playNode_niaoType_3 = _play.getChildByName("zhuaniao_2");
        this._playNode_niaoType_4 = _play.getChildByName("zhuaniao_3");
        this._playNode_niaoType_5 = _play.getChildByName("zhuaniao_4");    // 上中下鸟
        this._playNode_niaoType_6 = _play.getChildByName("zhuaniao_5");    // 一鸟全中
        //邵阳、湘乡 转转加抓8鸟
        this._playNode_niaoType_7 = _play.getChildByName("zhuaniao_6"); 
        if(MjClient.getAppType() == MjClient.APP_TYPE.HUNANWANGWANG || MjClient.getAppType() == MjClient.APP_TYPE.QXSYDTZ){
            this._playNode_niaoType_9 = _play.getChildByName("zhuaniao_9");
        }

        var nodeList1 = [];
        nodeList1.push(_play.getChildByName("zhuaniao_0"));
        nodeList1.push(_play.getChildByName("zhuaniao_1"));
        nodeList1.push(_play.getChildByName("zhuaniao_2"));
        nodeList1.push(_play.getChildByName("zhuaniao_3"));
        nodeList1.push(_play.getChildByName("zhuaniao_4"));
        nodeList1.push(_play.getChildByName("zhuaniao_5"));
        if(this._playNode_niaoType_7){
            nodeList1.push(_play.getChildByName("zhuaniao_6"));
        }
        if(this._playNode_niaoType_9){
            nodeList1.push(_play.getChildByName("zhuaniao_9"));
        }
        ...
        if(MjClient.getAppType() == MjClient.APP_TYPE.HUNANWANGWANG || MjClient.getAppType() == MjClient.APP_TYPE.QXSYDTZ){
            // 不中算全中
            this._playNode_zhongNiao = _play.getChildByName("buZhongNiao");
            ...
            // 全中翻倍
            this._playNode_quanZhongFanBei = _play.getChildByName("quanZhongFanBei");
        ...
        this._playNode_anZhuang = _play.getChildByName("play_anzhuang");
        this._playNode_buLunKong = _play.getChildByName("play_buLunKong");
        if(this._playNode_6niaowanfa){
        ...
        if(MjClient.getAppType() == MjClient.APP_TYPE.HUNANWANGWANG || MjClient.getAppType() == MjClient.APP_TYPE.QXSYDTZ || MjClient.getAppType() == MjClient.APP_TYPE.BDHYZP ){
            ...
            addBtn.addTouchEventListener(function(sender, type) {
                if (type == 2) {
                    var num = parseInt(score.getString());
                    num++;
                    if(num > 10) num = 1;
                    score.setString(num);
                    this.setRoomCardModeFree(num);
                }
            }, this);
    ...
    setPlayNodeCurrentSelect: function(isClub) {
        ...
        if(MjClient.getAppType() == MjClient.APP_TYPE.QXXXGHZ){
            ...
            if (isClub)
                ...
                _trustTime = util.localStorageEncrypt.getNumberItem(this.localStorageKey.KEY_TYZZ_XX_tuoguan, 0);
            this._playNode_tuoguanType_0.setSelected(false);
            this._playNode_tuoguanType_1.setSelected(false);
            this._playNode_tuoguanType_2.setSelected(false);
            this._playNode_tuoguanType_3.setSelected(false);
    ...
    getSelectedPara: function() {
        ...
        if(this._playNode_queyimen){
            // 二人玩法，缺一门 条子
            if(this._playNode_player_type_radio2.getSelectIndex() == 3 || this._playNode_player_type_radio2.getSelectIndex() == 2){
        ...
        if(this._playNode_buLunKong){
            para.buLunKong = this._playNode_buLunKong.visible && this._playNode_buLunKong.isSelected();  
        }
        ...
        if(score){
            para.jieSuanDiFen = parseInt(score.getString());
    ...
    savePara : function(para){
        if (!this._isFriendCard){
            ...
            if(MjClient.getAppType() == MjClient.APP_TYPE.QXXXGHZ)
            {
                util.localStorageEncrypt.setNumberItem(this.localStorageKey.KEY_TYZZ_XX_tuoguan, para.trustTime);
            }
