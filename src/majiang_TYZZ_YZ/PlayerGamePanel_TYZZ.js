    ...
    jsBind: {
        _event: {
            ...
            roundEnd: function() {
                ...
                function delayExe()
                {
                    ...
                    {
                        //弹小结算之前显示其他人牌
                        that.runAction(cc.sequence(cc.delayTime(0.1),cc.callFunc(showMjhandBeforeEndOne),cc.delayTime(1.5),cc.callFunc(showEndCards)));
                    }
                    else
                    {
            ...
            initSceneData: function() {
                ...
                //解散房间
                var sData = MjClient.data.sData;
                if(sData.tData.delEnd != 0 && MjClient.delroomui && cc.sys.isObjectValid(MjClient.delroomui)){
        ...
        roundInfo:{
            ...
            _run:function()
            {
                ...
                str += tData.areaSelectMode.zuoJiang258 ? ",258做将" : "";
                str += tData.areaSelectMode.huangZhuangGang ? ",荒庄荒杠" : "";
                str += tData.areaSelectMode.piaoniao ? ",围一飘鸟" : "";

                if(MjClient.getAppType() == MjClient.APP_TYPE.HUNANWANGWANG || MjClient.APP_TYPE.QXSYDTZ == MjClient.getAppType()){
                    if(tData.areaSelectMode.piaoType == 5)
                        str += ",首局定飘";
                    else if(tData.areaSelectMode.piaoType == 4)
                        str += ",自由下飘";
                    else if(tData.areaSelectMode.piaoType < 3)
                        str += ",飘"+ (tData.areaSelectMode.piaoType + 1) + "分";
        ...
        wait: {
            ...
            getRoomNum: {
                ...
                _click: function() {
                    ...
                    cc.log("tData.areaSelectMode  ZZ = " + JSON.stringify(tData.areaSelectMode));
            ...
            delroom: {
                _run:function(){
                    ...
                    //湘乡告胡子APP添加ipad适配
                    if((MjClient.getAppType() === MjClient.APP_TYPE.QXXXGHZ || MjClient.getAppType() === MjClient.APP_TYPE.BDHYZP) 
                        && isIpadSize() && MjClient.MaxPlayerNum != 2){
                        setWgtLayout(this, [0.11, 0.11],[0.15, 0.65],[0, 0]);
                    }
                },
                _click: function() {
        ...
        down: {
            ...
            _event: {
                ...
                mjhand: function(eD) {
                    ...
                    if(MjClient.rePlayVideo != -1 || (MjClient.getAppType() != MjClient.APP_TYPE.HUNANWANGWANG && MjClient.APP_TYPE.QXSYDTZ != MjClient.getAppType()) || COMMON_UI3D.is3DUI()){
                ...
                newCard: function(eD) {
                    ...
                    cc.log(">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>eD.isFistCard = " + eD.isFistCard);
                    if(!eD.isFistCard)
                    {
        ...
        eat: {
            ...
            chi0: {
                ...
                _touch: function(btn, eT) {
                    if (eT == 2) {
                        var tData = MjClient.data.sData.tData;
                        if(tData.areaSelectMode.bihuType && MjClient.playui.jsBind.eat.hu._node.visible){
                            return MjClient.showToast("有胡必胡");
                        }
                        if(MjClient.playui.jsBind.eat.hu._node.visible)
                        {
            ...
            gang0: {
                ...
                _touch: function(btn, eT) {
                    if (eT == 2) {
                        var tData = MjClient.data.sData.tData;
                        // var pl = getUIPlayer(0);
                        // var isTingAll = this.majiang.canHuAfterGang(pl.mjhand, MjClient.gangCards[0], tData.hunCard);
                        // if(tData.areaSelectMode.bihuType && MjClient.playui.jsBind.eat.hu._node.visible && !isTingAll){
                        //     return MjClient.showToast("有胡必胡");
                        // }
                        if(!tData.areaSelectMode.bihuType && MjClient.playui.jsBind.eat.hu._node.visible)
                        {
...
PlayLayer_TYZZ.prototype.hideEat= function() {
    if (MjClient.getAppType() != MjClient.APP_TYPE.HUNANWANGWANG && MjClient.APP_TYPE.QXSYDTZ != MjClient.getAppType()) {
        return ;
    }
...
// 显示当前局数增加的分数
PlayLayer_TYZZ.prototype.NowRoundScore = function(node,off,isShow) { 
    var tData = MjClient.data.sData.tData;
    if(!tData || tData.areaSelectMode.jiapiao){
        return;
    }
    var pl = getUIPlayer(off);
    if(!pl)return; 
                        ...
                        else
                        {
                            if(off == 0)
                            {
                                ...
                                if(MjClient.getAppType() == MjClient.APP_TYPE.BDHYZP){
                                    ci.x = orders[i - 1].x + upSize.width * upS; // 衡阳地区的牌资源大小不太一样
                                }else{
...
PlayLayer_TYZZ.prototype.ShowAllTingTips = function ()
{
    if(!MjClient.majiang.getIsInTheGame()) return;
    var downNode = MjClient.playui._downNode;
    var children = downNode.children;
    var pl = getUIPlayer(0);
...
// 判断是否显示听
PlayLayer_TYZZ.prototype.CheckTingIsShow = function()
{
    if(!MjClient.majiang.getIsInTheGame()) return;
    var eat = MjClient.playui.jsBind.eat;
    eat.ting._node.visible = false;
    ...
    var pl = getUIPlayer(0);
    var currentCard = CurrentPutCardMsg();
    var tingCardsNode = MjClient.playui._tingCardsNode;
    tingCardsNode.visible = false;
    if(currentCard == -1){
        return;
    }
...
PlayLayer_TYZZ.prototype.updateGangScore = function(node, data, off){
    ...
    selectNode.setString(Math.abs(score));
