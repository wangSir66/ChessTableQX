...
PaodekuaiLyg.prototype = Object.create(PaodekuaiBase.prototype);
PaodekuaiLyg.prototype.constructor = PaodekuaiLyg
...
PaodekuaiLyg.prototype.calType = function(pCards, areaSelectMode, skipSzdaipai) {
    ...
    // 三带二
    if (areaSelectMode.can3dai2 && cardCount == this.PDK_CARDCOUNT[this.CARDTPYE.sandaier] && maxCount == 3 ) {
...
PaodekuaiLyg.prototype.canPut = function(oCards, oLastCards, handsNum, areaSelectMode) {
    ...
    if (cardsType == this.CARDTPYE.szdaipai && lastCardsType == this.CARDTPYE.shunzi) {
...
PaodekuaiLyg.prototype.findPutTipCards = function(hands, lastCards, areaSelectMode, isNextPlayerOneCard, isFirstRound, putOrder, allBuChaiTypes) {
    ...
    for (var order = 0; order < putOrder.length; order++) {
        ...
        for (var typeIndex = 0; typeIndex < typeArray.length; typeIndex++) {
            ...
            var cards;
            if (this.pdk_allZhaDan.indexOf(type) >= 0 && lastType != type)
