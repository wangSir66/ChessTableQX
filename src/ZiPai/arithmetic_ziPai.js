...
(function() {
	...
	huzi.prototype.canHu = function(tb, pl, card) {
        if(tb.tData.gameType == MjClient.GAME_TYPE.DA_ZI_BO_PI){
            if(card >= 1 && card <= 10){
                return {huMatrix: [], maxWinOne: 0};
            }
        }
		var huMatrix = [];
    ...
    huzi.prototype.getTingStats = function(tb, pl, putCard) {
        if (pl.canNotPutCard && pl.canNotPutCard.indexOf(putCard) >= 0/* || putCard == KING*/) {
            return {};
        }
        ...
        for (var uid in tb.players) {
            ...
            for (var i = 0; i < p.mjgang1.length; i++) {
                if(tb.tData.gameType == MjClient.GAME_TYPE.HY_LIU_HU_QIANG && tb.tData.areaSelectMode.isMingwei == false){
                    if (p.info.uid == pl.info.uid || !(p.mjHide && p.mjHide.indexOf(p.mjgang1[i]) >= 0)){
                        add(p.mjgang1[i], 4);
                    }
                }else{
                    add(p.mjgang1[i], 4);
            ...
            for (var i = 0; i < p.mjwei.length; i++) {
                ...
                if (tb.tData.gameType == MjClient.GAME_TYPE.HUAI_HUA_HONG_GUAI_WAN
                    ...
                    || (tb.tData.gameType == MjClient.GAME_TYPE.HY_LIU_HU_QIANG && tb.tData.areaSelectMode.isMingwei == false)
