MjClient.MaxPlayerNum_yuanLingPaoHuZi = 3;
MjClient.cardPath_yuanLingPaoHuZi = "playing/paohuzi/";
var ActionType_yuanLingPaoHuZi =  //图片提示
    {
        CHI:1,
        PENG:2,
        WEI:3,
...
//初始化玩家金币、名字、胡息
function InitUserCoinAndName_yuanLingPaoHuZi(node, off){
...
//向服务器发送王闯王钓操作
function HZWangChuangToServer_yuanLingPaoHuZi(type){
    cc.log("====================HZWangChuangToServer_yuanLingPaoHuZi=================");
...
//向服务器发送打牌操作
function HZPutCardToServer_yuanLingPaoHuZi(card){
    cc.log("====================HZPutCardToServer_yuanLingPaoHuZi=================");
    MjClient.gamenet.request("pkroom.handler.tableMsg", {
        cmd: "MJPut",
        card: card
    });
...
// 向服务器发送吃牌
function HZChiToServer_yuanLingPaoHuZi(eatCards, biCards){
...
//像服务器发送碰牌
function HZPengToServer_yuanLingPaoHuZi()
{
    if (MjClient.rePlayVideo != -1) return; // 回放时候不能请求
    cc.log("====================HZPengToServer_yuanLingPaoHuZi=================");
    MjClient.gamenet.request("pkroom.handler.tableMsg", {
        cmd: "MJPeng"
    });
}

function MJPass2Net_yuanLingPaoHuZi(){
    cc.log("====================send======pass=====");
...
// 向服务器发送过牌
function HZPassConfirmToServer_yuanLingPaoHuZi() {
    if (MjClient.rePlayVideo != -1)  {
        return; // 回放时候不能请求
    }
    ...
    MjClient.gamenet.request("pkroom.handler.tableMsg", {
        cmd: "MJPass",
        eatFlag: EatFlag_yuanLingPaoHuZi(),
        cardNext: tData.cardNext,
        card: tData.lastPutCard
    });
}
...
//发送跑牌命令
function HZGangToServer_yuanLingPaoHuZi(type){
    if (MjClient.rePlayVideo != -1) return; // 回放时候不能请求
    cc.log("====================HZGangToServer_yuanLingPaoHuZi=================");
    MjClient.gamenet.request("pkroom.handler.tableMsg", {
        cmd: "MJGang",
        type: type,
        eatFlag: EatFlag_yuanLingPaoHuZi()
...
//发送偎牌命令
function HZWeiToServer_yuanLingPaoHuZi(type){
    if (MjClient.rePlayVideo != -1) return; // 回放时候不能请求
    cc.log("====================HZWeiToServer_yuanLingPaoHuZi=================");
    MjClient.gamenet.request("pkroom.handler.tableMsg", {
        cmd: "HZWeiCard"
    });
}
...
//向服务器发送发牌命令
function HZNewCardToServer_yuanLingPaoHuZi(){
    if (MjClient.rePlayVideo != -1) return; // 回放时候不能请求
    cc.log("====================HZNewCardToServer_yuanLingPaoHuZi=================");
    MjClient.gamenet.request("pkroom.handler.tableMsg", {
        cmd: "HZNewCard"
    });
}
...
//向服务器发送胡牌命令
function MJHuToServer_yuanLingPaoHuZi(){
    if (MjClient.rePlayVideo != -1) return; // 回放时候不能请求
    cc.log("====================MJHuToServer=================");
    MjClient.gamenet.request("pkroom.handler.tableMsg", {
        cmd: "MJHu",
...
//添加吃的牌
function addEatCard_yuanLingPaoHuZi(node,name,mjNum,off){
    //根据牌的类型获得需要添加的节点
    ...
    var type = 2;
    //设置牌
    var newCard = getNewCard_yuanLingPaoHuZi(mjNum,type,off);
...
/**
 添加手牌(回放)
 */
function addHandCardReplay_yuanLingPaoHuZi(tagName,index,mjNum,off){
    var _node = getNode_yuanLingPaoHuZi(off);
    //根据牌的类型获得需要添加的节点
...
function cleanCardNode_yuanLingPaoHuZi(off) {
    //获取结点
    if(off != 0){
        return;
    }
...
function resetCardNode_yuanLingPaoHuZi(off) {
    if(off != 0){
        return;
    }
    var _node = getNode_yuanLingPaoHuZi(off);
...
function addOrAdjustHandCard_yuanLingPaoHuZi(tagName,index,mjNum,off){
    if(off != 0){
        return;
    }
    var _node = getNode_yuanLingPaoHuZi(off);
    ...
    if(!newCard){
        cc.log("chow", "newCard");
        newCard = getNewCard_yuanLingPaoHuZi(mjNum,1,off);
    }else{
        cc.log("chow", "getCard");
    }
    var scale_y = newCard.scaleY;
    ...
    if (MjClient.movingCard_paohuzi == newCard) {
        setCardSprite_yuanLingPaoHuZi(newCard, newCard.tag, 1, false);
    ...
    if(newCard.lastPosition){
        ...
        doMovetoAction_yuanLingPaoHuZi(newCard, cc.p(beginPoint.x, beginPoint.y + cardCount * off_y));
    }else{
        newCard.x = beginPoint.x;
        newCard.y = beginPoint.y + cardCount * off_y;
    }
    //newCard.opacity = 255;
    SetTouchCardHandler_yuanLingPaoHuZi(newCard,off);
    ...
    //cc.log("chow", "addOrAdjustHandCard_yuanLingPaoHuZi: name=" + newCard.getName() + " num=" + newCard.num + " tag =" + newCard.tag);
}

function doMovetoAction_yuanLingPaoHuZi(node,endP){
    node.stopAllActions();
    var action = cc.moveTo(0.15,endP);
    node.runAction(action);
};

function resetHandCard_yuanLingPaoHuZi(posNode,off, needAction, isDelay){
    if(MjClient.rePlayVideo == -1){
        if(off == 0) {
            var handNode = posNode.getChildByName("handNode");
            handNode.visible = true;
            cleanCardNode_yuanLingPaoHuZi(off);
            ...
            for (var k = 0; k < cardArr.length; k++) {
                ...
                for (var j = 0; j < groupList.length; j++) {
                    addOrAdjustHandCard_yuanLingPaoHuZi(k, j, groupList[j], off);
                }
            }
            resetCardNode_yuanLingPaoHuZi(off);

            addTingSign_yuanLingPaoHuZi(posNode); // 添加听牌角标
            ...
            for (var i = 0; i < cardArr.length; i++) {
                ...
                if(addNode.lastPosition){
                    ...
                    doMovetoAction_yuanLingPaoHuZi(addNode, cc.p((winSize.width - totalWidth) / 2 + i * width * scale_x, 0));
                }else{
        ...
        var handCard = null;
        if(off == 0){
            if(MjClient.MaxPlayerNum_yuanLingPaoHuZi == 3 || MjClient.MaxPlayerNum_yuanLingPaoHuZi == 2){
        ...
        for(var k = 0;k < cardArr.length;k++){
            ...
            for(var j = 0;j < groupList.length;j++){
                if(off == 0){
                    if(MjClient.MaxPlayerNum_yuanLingPaoHuZi == 3 || MjClient.MaxPlayerNum_yuanLingPaoHuZi == 2){
                        addHandCard_yuanLingPaoHuZi(k,j,groupList[j],off);
                    }else if(MjClient.MaxPlayerNum_yuanLingPaoHuZi == 4){
        ...
        if(off == 0 && (MjClient.MaxPlayerNum_yuanLingPaoHuZi == 3 || MjClient.MaxPlayerNum_yuanLingPaoHuZi == 2)) {
            var scale_x = handCard.scaleX;
            var winSize = MjClient.size;
...
function checkCard_yuanLingPaoHuZi(node, off){
    ...
    if(cardHandArr.length > cardHand.length){
        ...
        for(var i = 0;i < cardHandArr.length; i++){
            if(cardHandArr[i] != cardHand[i]){
                RemoveHandCard_yuanLingPaoHuZi(node, cardHandArr[i], off);
...
function resetHandCard_yuanLingPaoHuZiEx(posNode,off, needAction, isDelay){
    if(off == 0) {
        ...
        checkCard_yuanLingPaoHuZi(posNode, off);
        var handNode = posNode.getChildByName("handNode");
        handNode.visible = true;
        cleanCardNode_yuanLingPaoHuZi(off);
        handNode.removeAllChildren();
 ...
 */
function addHandCard_yuanLingPaoHuZi(tagName,index,mjNum,off){
    if(off != 0){
        return;
    }
    var _node = getNode_yuanLingPaoHuZi(off);
    ...
    //设置牌
    var newCard = getNewCard_yuanLingPaoHuZi(mjNum,1,off);
    var scale_y = newCard.scaleY;
    //var parentCount = addNode.childrenCount;
    //首先根据name判断cpNode中是否已经添加
    var cardParent = addNode.getChildByTag(tagName);
    if(!cardParent){
        ...
        cardParent.width = newCard.width;
        addNode.addChild(cardParent);
        // SetTouchCardHandler_yuanLingPaoHuZi(cardParent);
    }
    ...
    cc.log("chow", "addHandCard_yuanLingPaoHuZi: name=" + newCard.getName() + " num=" + newCard.num + " tag =" + newCard.tag);
}
 ...
 **/
function getNewCard_yuanLingPaoHuZi(mjNum, type, off, isTurn){
    var _node = getNode_yuanLingPaoHuZi(off);
    var copyNode = null;
    if(type == 1){
        //手牌
    ...
    //cc.log("getNewCard_yuanLingPaoHuZi:"+mjNum+","+type+","+off+","+isTurn);
    if(mjNum > 0){
        //创建一个带有麻将信息的麻将 cp为创建后的麻将
        setCardSprite_yuanLingPaoHuZi(mjNode, mjNum, type, isTurn);
        if(type == 1){
            //只有手牌才会有点击事件
            SetTouchCardHandler_yuanLingPaoHuZi(mjNode,off);
        }
    }
...
/**
 ...
 type:类型，1:手牌 2：吃牌 3：打的牌
 */
function setCardSprite_yuanLingPaoHuZi(mjNode, mjNum, type, isTurn){
    MjClient.cardPath_yuanLingPaoHuZi = ziPai.getCardFilePath();
    if(ziPai.getZiPaiType() == 3){
        MjClient.cardPath_yuanLingPaoHuZi = "playing/paohuzi/MJBg3/";
    }

    if(type == 1){
        mjNode.loadTexture(MjClient.cardPath_yuanLingPaoHuZi+"hand" + mjNum + ".png");
    }
    if(type == 2){
        mjNode.loadTexture(MjClient.cardPath_yuanLingPaoHuZi+"out" + mjNum + ".png");
        if(isTurn){
            mjNode.loadTexture(MjClient.cardPath_yuanLingPaoHuZi+"huxiBG.png");
        }
    }
    if(type == 3 || type == 4){
        mjNode.loadTexture(MjClient.cardPath_yuanLingPaoHuZi+"put" + mjNum + ".png");
        if(isTurn){
            mjNode.loadTexture(MjClient.cardPath_yuanLingPaoHuZi+"normalBG.png");
        }
    }
    mjNode.tag = mjNum;
}

function checkTingCards_yuanLingPaoHuZi(putCard){
    //听牌检测
    var sData = MjClient.data.sData;

    if(MjClient.data.sData.tData.tState == TableState.waitPut && curPlayerIsMe_yuanLingPaoHuZi(0) && putCard === undefined){
        postEvent("showTingLayer", []);
        return;
    }

    if(putCard && (!curPlayerIsMe_yuanLingPaoHuZi(0) || sData.tData.tState != TableState.waitPut)){
        putCard = undefined;
    }
...
//刷新手牌移动时的坐标
var btn_oPos = null;
function updateBtnMovedPosition_yuanLingPaoHuZi(btn, eventType){
...
//MjClient.movingCard_yuanLingPaoHuZi = null;
//MjClient.cloneCard = null;
//MjClient.putCard = null;
MjClient.hasPut = false;
function SetTouchCardHandler_yuanLingPaoHuZi(card,off){
    ...
    card.addTouchEventListener(function(btn,eventType){
        ...
        if(eventType == ccui.Widget.TOUCH_BEGAN){
            ...
            setCardSprite_yuanLingPaoHuZi(cloneCard, cardTag, 1, false);
            ...
            btn.x = touchPos.x;
            btn.y = touchPos.y;
            // updateBtnMovedPosition_yuanLingPaoHuZi(btn, eventType);
            ShowPutCardIcon_yuanLingPaoHuZi();

            checkTingCards_yuanLingPaoHuZi(btn.tag);
            var alignWidth = btn.scale * btn.width;
            setCardSprite_yuanLingPaoHuZi(btn, btn.tag, 3, false);
        ...
        if(eventType == ccui.Widget.TOUCH_ENDED ||
            ...
            setCardSprite_yuanLingPaoHuZi(btn, btn.tag, 1, false);
            ...
            var putCardCb = function () {
                ...
                if(((!pl.canNotPutCard||pl.canNotPutCard.indexOf(btnTag)==-1) &&(getOtherWeiCards_yuanLingPaoHuZi(btnTag)||(pl.limitHuPutCard && pl.limitHuPutCard.indexOf(btnTag) != -1))) &&
                    ...
                    !MjClient.isCommon&& !MjClient.hasPut)
                {
                    ...
                    var putCard = getNewCard_yuanLingPaoHuZi(btnTag, 3 ,off);
                    ...
                    if(getOtherWeiCards_yuanLingPaoHuZi(btnTag)){
                                ...
                                if(pl.limitHuPutCard && pl.limitHuPutCard.indexOf(btnTag) != -1){
                                    MjClient.showMsg("吃边打边，这局将只能胡"+ MjClient.majiang.getLimitHuDesc(btnTag, tData)+"，是否确定？",
                                        function(){
                                            ...
                                            HZPutCardToServer_yuanLingPaoHuZi(btnTag);
                                            MjClient.playui.ResetHandCard(MjClient.playui._downNode,0);

                                            checkTingCards_yuanLingPaoHuZi(btnTag);
                                        },
                                        function(){
                                            MjClient.playui.ResetHandCard(MjClient.playui._downNode,0);
                                            ShowPutCardIcon_yuanLingPaoHuZi();
            ...
            if(!isPut || isWangBa)
            {
                ...
                if(selectParentTag == btnParent.tag){
                    ...
                    if(selectArr){
                        //_changeArrIndex_yuanLingPaoHuZi(selectArr, btnTag, btn);
                        MjClient.HandCardArr[btnParent.tag].splice(parseInt(btnName),1);
                        _fixArrIndex_yuanLingPaoHuZi(selectArr, btnTag, btn);
                    }
                }
...
//根据坐标Y，添加到数组对应的位置
function _fixArrIndex_yuanLingPaoHuZi(arr, btnTag, btn){
    if(arr){
        if(_isKan_yuanLingPaoHuZi(arr)){
            arr.push(btnTag);
        ...
        cc.log('_fixArrIndex_yuanLingPaoHuZi====:',arr);
    }

}

//改变数组元素的位置
function _changeArrIndex_yuanLingPaoHuZi(arr, btnTag, btn){
    if(arr && !_isKan_yuanLingPaoHuZi(arr)){
        var len = arr.length;
        for(var i = 0; i < len; i++){
            var tag = arr[i];
            if(tag == btnTag){
        ...
        _fixArrIndex_yuanLingPaoHuZi(arr, btnTag, btn);
    }
}

//数组里的是否为三个相同的牌
function _isKan_yuanLingPaoHuZi(arr){
    if(arr){
        var len = arr.length;
        if(len < 3){
            return false;
...
//断线重连之后，显示之前玩家打的牌
function DealOffLineCard_yuanLingPaoHuZi(node, off) {
    cc.log("====================DealOffLineCard_yuanLingPaoHuZi=================", off);
    var sData = MjClient.data.sData;
    var tData = sData.tData;
    ...
    if(typeof(tData.currCard) != "undefined" && tData.currCard != -1){
        var uids = tData.uids;
        var selfIndex = (uids.indexOf(SelfUid()) + off) % MjClient.MaxPlayerNum_yuanLingPaoHuZi;
        if(uids[selfIndex] == uids[tData.curPlayer]){
            var putCard = getNewCard_yuanLingPaoHuZi(tData.currCard, 3, off);
    ...
    cc.log("====================DealOffLineCard_yuanLingPaoHuZi================= end", off);
}

// 通过玩家idx获取UIBind（down left right xing）
function getUIBindByIndex_yuanLingPaoHuZi(idx) {
    var sData = MjClient.data.sData;
    var tData = sData.tData;
...
// 落牌动画
function fall_yuanLingPaoHuZi() {
    var sData = MjClient.data.sData;
    var tData = sData.tData;

    // tData.lastPlayer没同步!
    var lastPlayer = (tData.curPlayer - 1 + tData.maxPlayer) % tData.maxPlayer;
    cc.log("chow", "fall_yuanLingPaoHuZi"+ " lastPlayer = " + lastPlayer);
    ...
    if (out) {
        ...
        cc.log("chow", "fall_yuanLingPaoHuZi" + JSON.stringify(out.getPosition()));
...
//发牌
MjClient.isCommon = false;
function DealNewCard_yuanLingPaoHuZi(node, msg, off) {
    //没有发牌过来的时候直接忽略
    if (!msg.newCard) {
        RemovePutCardOut_yuanLingPaoHuZi();
        MjClient.playui.EatVisibleCheck();
        return;
    }

    AddPutCard_yuanLingPaoHuZi(node, msg, off); // 注意各个玩法验证！
    //存在发牌
    var sData = MjClient.data.sData;
    var tData = sData.tData;
    var uids = tData.uids;
    ...
    if (uids[selfIndex] == msg.uid) {
        cc.log("DealNewCard_yuanLingPaoHuZi  newCard@@ ", msg.newCard);
        //清理之前玩家打的牌
        var lastPlayer = tData.lastPlayer;
        if (lastPlayer != -1) {
            //var lastOff = (off + MjClient.MaxPlayerNum_yuanLingPaoHuZi - selfIndex) % MjClient.MaxPlayerNum_yuanLingPaoHuZi;
            // RemovePutCardOut_yuanLingPaoHuZi();

            fall_yuanLingPaoHuZi();
        }
        ...
        var putCard = getNewCard_yuanLingPaoHuZi(msg.newCard, 3, off, showPkBack);
        var putNode = node.getChildByName("put");
        ...
        if (!msg.isCommon) {
            if (MjClient.rePlayVideo == -1) {
                ...
                for (var i = 0; i < cardArr.length; i++) {
                    ...
                    if (tmpArr.length <= 2) {
                        isAdd = true;
                        //addHandCard_yuanLingPaoHuZi(i,tmpArr.length.length + 1,msg.newCard,off);
...
function setPutCardPos_yuanLingPaoHuZi(outNode, outCard, idx, off) {
    var tData = MjClient.data.sData.tData;
    var maxPlayer = tData.maxPlayer;
    var type = ziPai.getUiLayoutType();
    var isZuoXing = false;
    cc.log("setPutCardPos_yuanLingPaoHuZi:" + isZuoXing);
...
function setJiachuiTextTips_yuanLingPaoHuZi(node, off) {
    node.visible = false;
    var tData = MjClient.data.sData.tData;
    var pl = getUIPlayer_yuanLingPaoHuZi(off);
...
function setJiachuiImgTips_yuanLingPaoHuZi(node, off) {
    node.visible = false;
    var tData = MjClient.data.sData.tData;
    var pl = getUIPlayer_yuanLingPaoHuZi(off);
...
function AddPutCard_yuanLingPaoHuZi(node, msg, off) {
    var sData = MjClient.data.sData;
    var tData = sData.tData;
    var off_index = (tData.uids.indexOf(SelfUid()) + off) % MjClient.MaxPlayerNum_yuanLingPaoHuZi;
    if (off_index == tData.curPlayer) { // off对应玩家为当前玩家
        ...
        var out = getNewCard_yuanLingPaoHuZi(card, 2, off);
        var count = pl.mjput.length - 1;
        var width = out.width;
        var height = out.height;
        setPutCardPos_yuanLingPaoHuZi(outNode, out, pl.mjput.length - 1, off);
...
//翻醒发牌
function DealFanXingNewCard_yuanLingPaoHuZi(card){
    var putCard = getNewCard_yuanLingPaoHuZi(card, 4, 0);
...
//王闯/王钓
function DealWangChuang_yuanLingPaoHuZi(node,msg,off){
    var sData = MjClient.data.sData;
    var tData = sData.tData;
    ...
    if(uids[selfIndex] == msg.uid){
        ...
        if(pl.wangType == 1){
            var putCard = getNewCard_yuanLingPaoHuZi(91, 3, off);
            var putNode = node.getChildByName("put");
            putCard.scaleX = putCard.width/putNode.width - 0.16;
            putCard.scaleY = putCard.width/putNode.width - 0.06;
            putCard.x = putCard.width/2;
            ...
            ShowEatActionAnim_yuanLingPaoHuZi(node, ActionType_yuanLingPaoHuZi.WANGDIAO, off);

            var delay = cc.delayTime(1.5);
            var callback = function(){
                RemovePutCardOut_yuanLingPaoHuZi();
                //HZNewCardToServer_yuanLingPaoHuZi();
            }
            //putNode.runAction(cc.sequence(delay,cc.callFunc(callback)));
        }else if(pl.wangType == 2){
            var putCard = getNewCard_yuanLingPaoHuZi(91, 3, off);
            var putCard1 = getNewCard_yuanLingPaoHuZi(91, 3, off);
            var putNode = node.getChildByName("put");
            putCard.scaleX = putCard.width/putNode.width - 0.16;
            putCard.scaleY = putCard.width/putNode.width - 0.06;
            ...
            ShowEatActionAnim_yuanLingPaoHuZi(node, ActionType_yuanLingPaoHuZi.WANGCHUANG, off);
            var delay = cc.delayTime(1.5);
            var callback = function(){
                RemovePutCardOut_yuanLingPaoHuZi();
                //HZNewCardToServer_yuanLingPaoHuZi();
            ...
            var putCard = getNewCard_yuanLingPaoHuZi(91, 3, off);
            var putCard1 = getNewCard_yuanLingPaoHuZi(91, 3, off);
            var putCard2 = getNewCard_yuanLingPaoHuZi(91, 3, off);
            var putNode = node.getChildByName("put");
            putCard.scaleX = putCard.width/putNode.width - 0.16;
            putCard.scaleY = putCard.width/putNode.width - 0.06;
            putCard1.scaleX = putCard.width/putNode.width - 0.16;
            ...
            ShowEatActionAnim_yuanLingPaoHuZi(node, ActionType_yuanLingPaoHuZi.WANGZHA, off);
            var delay = cc.delayTime(1.5);
            var callback = function(){
                RemovePutCardOut_yuanLingPaoHuZi();
                //HZNewCardToServer_yuanLingPaoHuZi();
...
//处理摸最后一张牌
function DealShowLastCard_yuanLingPaoHuZi(node, msg, off){
    cc.log("======DealShowLastCard_yuanLingPaoHuZi======= " + off);
    MjClient.hasPut = true;
    var sData = MjClient.data.sData;
    var tData = sData.tData;
    var uids = tData.uids;
...
//处理出牌
function DealPutCard_yuanLingPaoHuZi(node, msg, off) {
    AddPutCard_yuanLingPaoHuZi(node, msg, off); // 注意各个玩法验证！
    ...
    if (uids[selfIndex] == msg.uid) {
        ...
        if (SelfUid() != msg.uid || MjClient.rePlayVideo != -1) {
            ...
            putNode.stopAllActions();
            putNode.removeAllChildren();
            var putCard = getNewCard_yuanLingPaoHuZi(msg.card, 3, off);
            putCard.scaleX = putCard.width / putNode.width - 0.16;
            putCard.scaleY = putCard.width / putNode.width - 0.06;
            putCard.x = putCard.width / 2;
            putCard.y = putCard.height / 2;
        ...
        if (off == 0) {
            removeTingSign_yuanLingPaoHuZi(node);
        }

        if (MjClient.rePlayVideo != -1) {
            RemoveHandCard_yuanLingPaoHuZi(node, msg.card, off);
            MjClient.playui.ResetHandCard(node, off);
        }
    }
}

function calculateHintPutList_yuanLingPaoHuZi() {
...
function addTingSign_yuanLingPaoHuZi(node) {
...
function removeTingSign_yuanLingPaoHuZi(node) {
    var handNode = node.getChildByName("handNode");
    if (!handNode) {
        return;
    }
...
//吃牌
function DealChiCard_yuanLingPaoHuZi(node, msg, off) {
    cc.log("======DealChiCard_yuanLingPaoHuZi======= " + off);
    var sData = MjClient.data.sData;
    var tData = sData.tData;
    var uids = tData.uids;
    var selfIndex = (uids.indexOf(SelfUid()) + off) % MjClient.MaxPlayerNum_yuanLingPaoHuZi;
    if (tData.curPlayer == selfIndex) {
        ...
        for (var i = 0; i < tmpEatCards.length; i++) {
            RemoveHandCard_yuanLingPaoHuZi(node, tmpEatCards[i], off);
        }
            ...
            for (var k = 0; k < biCards.length; k++) {
                ...
                for (var m = 0; m < biArr.length; m++) {
                    if (tmpBiArr.indexOf(biArr[m]) >= 0) {
                        ...
                        RemoveHandCard_yuanLingPaoHuZi(node, biArr[m], off);
                    }
                }
            }
        }
        ...
        for (var i = 0; i < eatCards.length; i++) {
            var card = getNewCard_yuanLingPaoHuZi(eatCards[i], 2, off);
            if (i == 2) {
                card.setColor(cc.color(170, 170, 170));
            }
            ...
            card.zIndex = 4 - i;
            card.x = 0;
            card.y = off_count * getOffY_yuanLingPaoHuZi(node, card);
            setChiCardAnchorPoint_yuanLingPaoHuZi(node, card);
        ...
        if (biCards) {
            ...
            for (var k = 0; k < biCards.length; k++) {
                var biArr = biCards[k];
                for (var m = 0; m < biArr.length; m++) {
                    var card = getNewCard_yuanLingPaoHuZi(biArr[m], 2, off);
                    if (m == 2) {
                        card.setColor(cc.color(170, 170, 170));
                    }
                    setChiCardAnchorPoint_yuanLingPaoHuZi(node, card);
        ...
        var parentPos = getChiCardParentPosition_yuanLingPaoHuZi(node, cardWidth, off, selfColCount);
        var moveX0 = 0;
        ...
        parent.x = moveX0;
        setCardParentPosY_yuanLingPaoHuZi(node, parent);
        ...
        if (biCards) {
            ShowEatActionAnim_yuanLingPaoHuZi(node, ActionType_yuanLingPaoHuZi.XIAHUO, off);
        } else {
            ShowEatActionAnim_yuanLingPaoHuZi(node, ActionType_yuanLingPaoHuZi.CHI, off);
        }
...
function setChiCardAnchorPoint_yuanLingPaoHuZi(node,newCard) {
    if(node.getName() == "down"){
        newCard.anchorX = 0;
        newCard.anchorY = 0;
    }else if(node.getName() == "right"){
...
function getOffY_yuanLingPaoHuZi(node,newCard) {
    var off_y = 0;
    if(node.getName() == "down"){
        off_y = newCard.height;
    }else if(node.getName() == "right"){
...
function getChiCardParentPosition_yuanLingPaoHuZi(node, newCardWidth, off, selfColCount, orignColPosX) {
...
function setCardParentPosY_yuanLingPaoHuZi(node, cardParent) {
    var eatNode = node.getChildByName("eatNode");
    if(node.getName() == "down"){
        cardParent.y = 0;
    }else if (node.getName() == "right"){
...
function getShowCardIdx_yuanLingPaoHuZi(node, chiTip) {
    var idx;
    switch(node.getName()) {
        case "down":
        case "xing":
...
// 处理碰
function DealPengCard_yuanLingPaoHuZi(node, msg, off){
    cc.log("======DealChiCard_yuanLingPaoHuZi======= ");
    var sData = MjClient.data.sData;
    var tData = sData.tData;
    var uids = tData.uids;
    var selfIndex = (uids.indexOf(SelfUid()) + off) % MjClient.MaxPlayerNum_yuanLingPaoHuZi;
    if(tData.curPlayer == selfIndex){
        ...
        for(var i = 0;i < 3; i++) {
            var card = getNewCard_yuanLingPaoHuZi(pengCard,2,off);
            cardWidth = card.width//*eatNode.scaleX;
            var childCount = parent.childrenCount;
            card.zIndex = 4 - childCount;
            card.x = 0;
            card.y = childCount * getOffY_yuanLingPaoHuZi(node, card);
            setChiCardAnchorPoint_yuanLingPaoHuZi(node, card);
            parent.addChild(card);
        }
        eatNode.addChild(parent);

        RemoveHandCard_yuanLingPaoHuZi(node,pengCard,off);
        RemoveHandCard_yuanLingPaoHuZi(node,pengCard,off);
        MjClient.playui.ResetHandCard(node, off);
        ShowEatActionAnim_yuanLingPaoHuZi(node, ActionType_yuanLingPaoHuZi.PENG, off);
        ...
        var parentPos = getChiCardParentPosition_yuanLingPaoHuZi(node, cardWidth, off, 1);
        var moveX0 = 0;
        var outNodeCount = eatNode.getChildrenCount();
        if(outNodeCount>0){
            if(node.getName() == "down" || node.getName() == "left"){
...
//偎牌
function DealWeiCard_yuanLingPaoHuZi(node, msg, off) {
    cc.log("======DealWeiCard_yuanLingPaoHuZi======= ");
    var sData = MjClient.data.sData;
    var tData = sData.tData;
    var uids = tData.uids;
    var selfIndex = (uids.indexOf(SelfUid()) + off) % MjClient.MaxPlayerNum_yuanLingPaoHuZi;
    if (tData.curPlayer == selfIndex) {
        ...
        RemovePutCardOut_yuanLingPaoHuZi();
        ShowEatActionAnim_yuanLingPaoHuZi(node, ActionType_yuanLingPaoHuZi.WEI, off);
        ...
        for (var i = 0; i < 3; i++) {
            ...
            {
                if (i == getShowCardIdx_yuanLingPaoHuZi(node, "wei")) {
                    isTurn = false;
                }
                var card = getNewCard_yuanLingPaoHuZi(weiCard, 2, off, isTurn);
            } else if (MjClient.majiang.getCardShowType(tData.uids[tData.curPlayer], weiCard) == 1) {
                var card = getNewCard_yuanLingPaoHuZi(weiCard, 2, off);
                ...
                var card = getNewCard_yuanLingPaoHuZi(weiCard, 2, off, true);
            }
            cardWidth = card.width //*eatNode.scaleX;
            card.zIndex = 4 - i;
            card.x = 0;
            card.y = i * getOffY_yuanLingPaoHuZi(node, card);
            setChiCardAnchorPoint_yuanLingPaoHuZi(node, card);
            parent.addChild(card);
        }
        eatNode.addChild(parent);

        RemoveHandCard_yuanLingPaoHuZi(node, weiCard, off);
        RemoveHandCard_yuanLingPaoHuZi(node, weiCard, off);
        MjClient.playui.ResetHandCard(node, off);
...
// 跑牌或者提牌
function DealGangCard_yuanLingPaoHuZi(node, msg, off) {
    var sData = MjClient.data.sData;
    var tData = sData.tData;
    var uids = tData.uids;
    var selfIndex = (uids.indexOf(SelfUid()) + off) % MjClient.MaxPlayerNum_yuanLingPaoHuZi;
    ...
    if (uids.indexOf(gangUid) == selfIndex) {
        ...
        if (msg.type == 2) {
            RemovePutCardOut_yuanLingPaoHuZi();
            ShowEatActionAnim_yuanLingPaoHuZi(node, ActionType_yuanLingPaoHuZi.PAO, off);
        } else {
            if (!msg.isGangHand && msg.type == 1) {
                RemovePutCardOut_yuanLingPaoHuZi();
            }
            ShowEatActionAnim_yuanLingPaoHuZi(node, ActionType_yuanLingPaoHuZi.TI, off);
        }
        ...
        for (var i = 0; i < 4; i++) {
            ...
            if (msg.type == 2) {
                isTurn = false;
                var card = getNewCard_yuanLingPaoHuZi(gangCard, 2, off, isTurn);
            }
            else {
                if (i == getShowCardIdx_yuanLingPaoHuZi(node, "ti"))
                {
                    isTurn = false;
                }
                var card = getNewCard_yuanLingPaoHuZi(gangCard, 2, off, isTurn);
        ...
        RemoveHandCard_yuanLingPaoHuZi(node, gangCard, off);
        RemoveHandCard_yuanLingPaoHuZi(node, gangCard, off);
        RemoveHandCard_yuanLingPaoHuZi(node, gangCard, off);
        if (msg.isGangHand) {
            RemoveHandCard_yuanLingPaoHuZi(node, gangCard, off);
        ...
        MjClient.playui.ResetHandCard(node, off);
        // addTingSign_yuanLingPaoHuZi(node); // 添加听牌角标 (提 跑可能手牌未变动)
        ...
        var parentPos = getChiCardParentPosition_yuanLingPaoHuZi(node, cardWidth, off, 1, orignColPosX);
        var moveX0 = 0;
        var outNodeCount = eatNode.getChildrenCount();
        if (outNodeCount > 0) {
            if (node.getName() == "down" || node.getName() == "left") {
...
// 处理胡
function DealHu_yuanLingPaoHuZi(node, msg, off){
    var sData = MjClient.data.sData;
    var tData = sData.tData;
    var selfIndex = (tData.uids.indexOf(SelfUid()) + off) % MjClient.MaxPlayerNum_yuanLingPaoHuZi;
    var pl = getUIPlayer_yuanLingPaoHuZi(off);
    ...
    if(pl){
        pl.eatFlag = 0;
        MjClient.playui.EatVisibleCheck();
        ShowEatActionAnim_yuanLingPaoHuZi(node,ActionType_yuanLingPaoHuZi.HU,off);
    }
}

function DealAddCard_yuanLingPaoHuZi(node,msg, off){
    if(MjClient.rePlayVideo == -1 && off != 0){
        return;
    }
    var cardArr = MjClient.HandCardArr;
...
//清除打出的牌
function RemovePutCardOut_yuanLingPaoHuZi(noAction){
...
//发手牌时再清理一遍
function clearMJHandCardUI_yuanLingPaoHuZi() {
    // var handNode = MjClient.playui._downNode.getChildByName("handNode");
    // handNode.removeAllChildren();
...
// //出牌表示状态
// function ShowPutCardIcon_yuanLingPaoHuZi() {
...
//清除手牌
function RemoveHandCard_yuanLingPaoHuZi(node,card,off){
    if(MjClient.rePlayVideo == -1 && off != 0){
        return;
    }
    var cardArr = MjClient.HandCardArr;
...
// 初始化吃碰杠胡动作
function InitShowEatActionNode_yuanLingPaoHuZi(plNode){
...
// 重置吃碰杠胡动作
function resetEatActionAnim_yuanLingPaoHuZi()
{
    var jsBind = MjClient.playui.jsBind;
    ...
    var count = MjClient.MaxPlayerNum_yuanLingPaoHuZi;
    for(var i = 0; i < count; i++)
    {
        InitShowEatActionNode_yuanLingPaoHuZi(ui[i]._node);
    }
}

//播放头像移动
function tableStartHeadMoveAction_yuanLingPaoHuZi(node){
...
//重置3家头像位置
function reConectHeadLayout_yuanLingPaoHuZi(node){
    var tData = MjClient.data.sData.tData;
    var down = node.getChildByName("down").getChildByName("head");
    var left = node.getChildByName("left").getChildByName("head");
    var right = node.getChildByName("right").getChildByName("head");
...
//根据偏移获得玩家node
function getNode_yuanLingPaoHuZi(off){
    var _node = null;
    var tData = MjClient.data.sData.tData;
    if (tData.maxPlayer != 4){
        if (off == 1 || off == 2){
 ...
 * @return {object} 玩家数据
 */
function getUIPlayer_yuanLingPaoHuZi(off){
    var sData = MjClient.data.sData;
    var tData = sData.tData;
    var uids = tData.uids;
    var selfIndex = uids.indexOf(SelfUid());
    selfIndex = (selfIndex + off) % MjClient.MaxPlayerNum_yuanLingPaoHuZi;
...
// 获取ui头像，通过偏移值
function getUIHeadByOff_yuanLingPaoHuZi(off){
    var pl = getUIPlayer_yuanLingPaoHuZi(off);
    if(!pl)
    {
        return {};
 ...
 * 下载录音, 调用 播放函数
 * */
function downAndPlayVoice_yuanLingPaoHuZi(uid, filePath){
    var index = getUiOffByUid_yuanLingPaoHuZi(uid);
...
function getUiOffByUid_yuanLingPaoHuZi(uid){
    var sData = MjClient.data.sData;
    var tData = sData.tData;
    var uids = tData.uids;
    var selfIndex = uids.indexOf(SelfUid());
    var targetIndex = uids.indexOf(uid);
    return (targetIndex - selfIndex + MjClient.MaxPlayerNum_yuanLingPaoHuZi) % MjClient.MaxPlayerNum_yuanLingPaoHuZi;
}

//设置微信头像
function setWxHead_yuanLingPaoHuZi(node, d, off){
    if(d.uid == getUIHeadByOff_yuanLingPaoHuZi(off).uid){
        ...
        COMMON_UI.addNobleHeadFrame(nobody,getUIPlayer_yuanLingPaoHuZi(off))
    }
}

//显示玩家庄的ui
function showUserZhuangLogo_yuanLingPaoHuZi(node, off){
    var tData = MjClient.data.sData.tData;
    var pl = getUIPlayer_yuanLingPaoHuZi(off);
    node.zIndex = 100;
    if(tData && pl){
...
function showUserZhuangLogoZuoXing_yuanLingPaoHuZi(node, off){
    var tData = MjClient.data.sData.tData;
    var pl = getUIPlayer_yuanLingPaoHuZi(off);
    node.zIndex = 100;
    if(tData && pl){
...
//显示房主  add by sking
function showFangzhuTagIcon_yuanLingPaoHuZi(node,off)
{
    var pl = getUIPlayer_yuanLingPaoHuZi(off);
    if(!pl) //位置上没人则删掉房主标签
    {
...
// 清理ui
function clearCardUI_yuanLingPaoHuZi(node,off){
    mylog("clearCardUI_yuanLingPaoHuZi");
    ...
    for(var i = 0; i < children.length; i++)
    {
        ...
        {
            InitShowEatActionNode_yuanLingPaoHuZi(ni.getParent());
        }
    }
}

function EatFlag_yuanLingPaoHuZi(){
    var eat = MjClient.playui.jsBind.eat;
    var eatFlag = 0;
...
function huXiScore_yuanLingPaoHuZi(type,card)
{
...
//刷新胡息
function UpdateHuXi_yuanLingPaoHuZi(off){
    var sData = MjClient.data.sData;
    var tData = sData.tData;
    var score = 0;
    var pl = getUIPlayer_yuanLingPaoHuZi(off);
    ...
    if(mjpao.length>0){
        for(var p=0;p<mjpao.length;p++){
            score += huXiScore_yuanLingPaoHuZi("paoPai",mjpao[p]);
        }
    }
    ...
    if(mjpeng.length>0){
        for(var i=0;i<mjpeng.length;i++){
            score += huXiScore_yuanLingPaoHuZi("peng",mjpeng[i]);
        }
    }
        ...
        var chiScore = function(cards){
            ...
            for(var k=0;k<chiXiArr.length;k++){
                ...
                if(cards.toString() == chiXiList.toString()){
                    score += huXiScore_yuanLingPaoHuZi("chi",cards[0]);
                }
            }
        };
    ...
    if(mjti.length>0){
        for (var t = 0; t < mjti.length; t++) {
            score += huXiScore_yuanLingPaoHuZi("tiPai", mjti[t]);
        }
    }
    ...
    if(mjwei && mjwei.length>0){
        for (var w = 0; w < mjwei.length; w++) {
            if(MjClient.majiang.getCardShowType(pl.info.uid, mjwei[w]) != 0) {
                score += huXiScore_yuanLingPaoHuZi("weiPai", mjwei[w]);
            }
        }
    }
    ...
    if(tData.xingPlayer == selfIndex){
        pl = getUIPlayer_yuanLingPaoHuZi(2);
        //提牌
        var mjti = pl.mjgang1;
        if(mjti.length>0){
            for(var t=0;t<mjti.length;t++){
...
//设置转盘显示状态
function IsArrowVisible_yuanLingPaoHuZi(){
    var pl = getUIPlayer_yuanLingPaoHuZi(0);
    if (!pl)
    {
        return;
...
// 显示吃碰杠胡动作
function ShowEatActionAnim_yuanLingPaoHuZi(node, actType, off){
    var delayTime = 1;
    switch(actType)
    {
        case ActionType_yuanLingPaoHuZi.FANXING:
            delayTime = 1.6;
            break;
        default:
            break;
    ...
    switch(actType)
    {
        ...
        case ActionType_yuanLingPaoHuZi.TI:
            ...
            break;
        case ActionType_yuanLingPaoHuZi.WANGDIAO:
            ...
            break;
        case ActionType_yuanLingPaoHuZi.WANGCHUANG:
            eatActionChild = eatActionNode.getChildByName("wangChuang");
            eatActionChild.visible = true;
            eatActionChild.runAction(cc.sequence(cc.delayTime(delayTime), cc.callFunc(callback)));
            break;
        case ActionType_yuanLingPaoHuZi.XIAHUO:
...
//显示玩家信息
function showPlayerInfo_yuanLingPaoHuZi(off, node){
    //var tData = MjClient.data.sData.tData;
    var pl = getUIPlayer_yuanLingPaoHuZi(off);
    if(pl){
...
function resetChiParam_yuanLingPaoHuZi(){

}

function setChiVisible_yuanLingPaoHuZi(){
    var eat = MjClient.playui.jsBind.eat;
...
function showSelectEatCards_yuanLingPaoHuZi(node,cardNum){
    var eat = MjClient.playui.jsBind.eat;
    eat.chi._node.visible = false;
    eat.guo._node.visible = false;
    eat.peng._node.visible = false;
    ...
    addEatCards_yuanLingPaoHuZi(parent,cardArr,putCard);
}

//居中动画,数组里的节点居中排列 只关心x轴
doMoveCenterAction_yuanLingPaoHuZi = function(arr,isScale){
    arr.reverse();
    var gap = 5;
...
//添加可以吃的牌
function addEatCards_yuanLingPaoHuZi(node,handCardArr,putCard){
    var cardPath = MjClient.cardPath_yuanLingPaoHuZi;
    ...
    for(var i = 0;i < chiSet.length;i++){
        ...
        chiCloneBtn.addClickEventListener(function(chiBtn){
            ...
            if(tmpHandCardArr.indexOf(putCard) < 0){
                // setChiVisible_yuanLingPaoHuZi();
                // HZChiToServer_yuanLingPaoHuZi(chiArr,null);

                commitEatCards_yuanLingPaoHuZi(chiArr, null);
            }else{
                var biSet = MjClient.majiang.getBiCards(tmpHandCardArr,putCard);
                if(biSet && biSet.length > 0){
                    addFirstBiCards_yuanLingPaoHuZi(node,biSet,putCard,tmpHandCardArr,chiArr);
                }
            }
        });
    }
    if(ziPai.getUiLayoutType() == 0){//新布局
        doMoveCenterAction_yuanLingPaoHuZi([chiBg], false);
    }
}

//添加第一个比牌
function addFirstBiCards_yuanLingPaoHuZi(node,biSet,putCard,handCardArr,chiArr){
    var cardPath = MjClient.cardPath_yuanLingPaoHuZi;
    cardPath = ziPai.getCardFilePath();
    ...
    for(var i = 0;i < biSet.length;i++){
        ...
        cloneBtn.addClickEventListener(function(btn){
            ...
            if(tmpHandCardArr.indexOf(putCard) >= 0){
                var biSet = MjClient.majiang.getBiCards(tmpHandCardArr,putCard);
                if(biSet && biSet.length > 0){
                    addSecBiCards_yuanLingPaoHuZi(node,biSet,putCard,tmpHandCardArr,chiArr,biArr);
                }
            }else{
                // setChiVisible_yuanLingPaoHuZi();
                // HZChiToServer_yuanLingPaoHuZi(chiArr,[biArr]);
                commitEatCards_yuanLingPaoHuZi(chiArr,[biArr]);
            }
        });
    }
    ...
    if(ziPai.getUiLayoutType() == 0){//新布局
        ...
        doMoveCenterAction_yuanLingPaoHuZi([chiBg,biBg],isScale);
    }else{
        biBg.scale = chiBg.scale;
        doMoveCenterAction_yuanLingPaoHuZi2([chiBg,biBg],isScale);
    }
}

//居中动画,数组里的节点居中排列 只关心x轴
doMoveCenterAction_yuanLingPaoHuZi2 = function(arr,isScale){
    isScale = isScale === undefined ? true : isScale;
    var gap = 5;
    if(arr && arr.length > 0){
        var len = arr.length;
...
//添加第二个比牌
function addSecBiCards_yuanLingPaoHuZi(node,biSet,putCard,handCardArr,chiArr,firstBiArr){
    var cardPath = MjClient.cardPath_yuanLingPaoHuZi;
    cardPath = ziPai.getCardFilePath();
    ...
    for(var i = 0;i < biSet.length;i++){
        ...
        cloneBtn.addClickEventListener(function(btn){
            ...
            btn.setBright(false);
            // setChiVisible_yuanLingPaoHuZi();
            // HZChiToServer_yuanLingPaoHuZi(chiArr,[firstBiArr,biArr]);

            commitEatCards_yuanLingPaoHuZi(chiArr,[firstBiArr,biArr]);
        });
    }
    ...
    if(ziPai.getUiLayoutType() == 0){//新布局
        ...
        doMoveCenterAction_yuanLingPaoHuZi([chiBg,biBg0,biBg],isScale);
    }else{
        biBg.scale = chiBg.scale;
        biBg0.scale = chiBg.scale;
        doMoveCenterAction_yuanLingPaoHuZi2([chiBg,biBg0,biBg],isScale);
    }
}

function GetReadyVisible_yuanLingPaoHuZi(node, off) {
    if (off < 0) {
        node.visible = false;
        return false;
    }
...
//设置玩家掉线头像
function setUserOffline_yuanLingPaoHuZi(node, off){
...
//设置玩家掉线头像（邵阳新版大小结算专用）
function setRoundEndUserOffline_yuanLingPaoHuZi(frame, pl){
    if(!pl){
        return;
    }
...
//显示玩家文字
function showUserChat_yuanLingPaoHuZi(node, off, msg){
    var tData = MjClient.data.sData.tData;
 ...
 */
function curPlayerIsMe_yuanLingPaoHuZi(off){
    var sData = MjClient.data.sData;
    var tData = sData.tData;
...
function getVisablePlayerCount_yuanLingPaoHuZi(){
    var sData = MjClient.data.sData;
    var tData = sData.tData;
    var uids = tData.uids;
    var count = 0;
 ...
 */
function getXingPlayerIndex_yuanLingPaoHuZi(){
    if(MjClient.MaxPlayerNum_yuanLingPaoHuZi != 4){
        return -1;
    }
    if(getVisablePlayerCount_yuanLingPaoHuZi() < 3){
        return -1;
    }
    var sData = MjClient.data.sData;
    var tData = sData.tData;
    ...
    var xingIndex = (zhuang + 2 + MjClient.MaxPlayerNum_yuanLingPaoHuZi) % MjClient.MaxPlayerNum_yuanLingPaoHuZi;
    var selfIndex = tData.uids.indexOf(SelfUid());
    xingIndex = (xingIndex - selfIndex + MjClient.MaxPlayerNum_yuanLingPaoHuZi) % MjClient.MaxPlayerNum_yuanLingPaoHuZi;
    return xingIndex;
}


//重新开始后，重置MjClient.HandCardArr
function resetHandAfterBegin_yuanLingPaoHuZi(){
    var sData = MjClient.data.sData;
    var pl = sData.players[SelfUid() + ""];
...
/*
 设置弃胡状态
 */
function setQiHuState_yuanLingPaoHuZi()
{
    var pl = getUIPlayer_yuanLingPaoHuZi(0);
    if (pl.isQiHu) {
...
function changeMJBg_yuanLingPaoHuZi(node, type)
{
    if (node.toString() == "[object ImageView]") {
        var oldFile = node.getRenderFile().file;
        var newFile = getNewMJBgFile_yuanLingPaoHuZi(oldFile, type);
    ...
    for(var index in childArray)
    {
        ...
        changeMJBg_yuanLingPaoHuZi(child, type);
    }
}

function getNewMJBgFile_yuanLingPaoHuZi(oldFile, type)
{
    ...
    if (type == 0) {
        ...
        if (oldFile.indexOf("/MJBg1") != -1)
            ...
            MjClient.cardPath_yuanLingPaoHuZi = "playing/paohuzi/MJBg1/";
        }
        ...
        if (oldFile.indexOf("/MJBg2") != -1)
            ...
            MjClient.cardPath_yuanLingPaoHuZi = "playing/paohuzi/MJBg2/";
        }
        else if (oldFile.indexOf("playing/paohuzi/") != -1) {
...
function getOtherWeiCards_yuanLingPaoHuZi(card) {
    var sData = MjClient.data.sData;
 ...
 */
function createRadioBoxForCheckBoxs_yuanLingPaoHuZi(nodes, callback, defaultIndex){
    var newobj = {};
 ...
 *  off:玩家的实际off
 **/
function changeUIOff_yuanLingPaoHuZi(off){
    if (MjClient.MaxPlayerNum_yuanLingPaoHuZi != 4 && (off == 2 || off == 3)){
        return off - 1;
    }
    return off;
}
...
//是否显示离开按钮（包括准备阶段）
function isShowBackBtn_yuanLingPaoHuZi(){
    if(IsInviteVisible()){
        return true;
    }else{
        var sData = MjClient.data.sData;
 ...
 */
function getAnimate_yuanLingPaoHuZi(preName, len, delaytime, playCount, startIndex){
...
function paioFenToServer_yuanLingPaoHuZi(index) {
    var tData = MjClient.data.sData.tData;
