...
//向服务器发送 过消息
MjClient.MJPass2NetForXNMJ = function()
{
...
function SetUserVisible_XNMJ(node, off)
{
...
function InitUserHandUI_XNMJ(node, off)
{
    var sData = MjClient.data.sData;
    var tData = sData.tData;
    var pl = getUIPlayer(off);
...
var PlayLayer_XNMJ = cc.Layer.extend({
    jsBind: {
        _event: {
            mjhand: function() {
            ...
            roundEnd: function() {
                ...
                function showEndCards()
                {
                    ...
                    self.addChild(new EndOneView_XNMJ(),500);
                }
                ...
                var isQxsydtz = MjClient.getAppType() == MjClient.APP_TYPE.HUNANWANGWANG || MjClient.getAppType() == MjClient.APP_TYPE.QXSYDTZ;
                //如果没有
        ...
        roundInfo:{
            ...
            _run:function()
            {
                ...
                var tData = MjClient.data.sData.tData;
                var str = tData.areaSelectMode.maxPlayer + "人场";
                str += tData.areaSelectMode.isCanChi ? ",可吃牌":"";
                str += tData.areaSelectMode.isDaiFeng ? ",带风":"";
                str += tData.areaSelectMode.zhiTouZi ? ",开杠听掷骰子":"";
                str += tData.areaSelectMode.isJiaChui ? ",加锤":"";
                str += (tData.areaSelectMode.isLianChui && tData.areaSelectMode.isJiaChui) ? ",连庄不可拆锤" : "";
                str += (tData.areaSelectMode.isZhuangShangChui && tData.areaSelectMode.isJiaChui) ? ",庄上锤" : "";
                str += tData.areaSelectMode.huType == 0 ? ",一炮多响" : tData.areaSelectMode.huType == 1 ? ",一炮一响" : ",不支持抢杠胡";
                str += tData.areaSelectMode.zhuaniao == 0 ? ",不抓鸟" : "," + tData.areaSelectMode.zhuaniao + "鸟";
        ...
        touziNode:{
            ...
            _event:{
                MJZhiTouZi:function(data){
                    ...
                    MjClient.playui.CardLayoutRestore();
                }
            }
        },
        wait: {
            ...
            changeui: {
                ...
                changeuibg: {
                    ...
                    guobg: {
                        guo: {
                            _touch: function(btn, eT) {
                                if (eT == 2) MjClient.MJPass2NetForXNMJ();
                            }
                        },
                        fanhui: {
                            _touch: function(btn, et) {
        ...
        btnJiaChui_node: {
            ...
            _event:{
                ...
                waitJiazhu:function(ed){     
                    ...
                    var tData = MjClient.data.sData.tData;
                    if(pl.mjState == TableState.waitJiazhu && tData.tState == TableState.waitJiazhu && pl.jiachuiNum == -1){  
                        this.visible = true;
                    }else{
                        this.visible = false;
                    }
    ...
    ctor: function() {
        ...
        var playui = ccs.load("Play_XinNingMJ.json");
        playMusic("bgFight");
...
PlayLayer_XNMJ.prototype.hideEat= function() {
...
// 显示当前局数增加的分数
PlayLayer_XNMJ.prototype.NowRoundScore = function(node,off,isShow) { 
    var pl = getUIPlayer(off);
    if(!pl)return; 
    var score = null;
...
PlayLayer_XNMJ.prototype.CardLayoutRestore = function(node, off,isNewCard)
{
    ...
    if (off != 0 && MjClient.rePlayVideo != -1 && HandCardsLength > 0 && HandCardsLength % 3 == 2 
        && (MjClient.getAppType() == MjClient.APP_TYPE.HUNANWANGWANG || MjClient.getAppType() == MjClient.APP_TYPE.QXSYDTZ)) // 回放或倒牌
    {
...
// 三人的将中间部分内容向上移动
PlayLayer_XNMJ.prototype.MoveContnet = function(){
...
PlayLayer_XNMJ.prototype.ShowCardTingTip = function(){
    var downNode = MjClient.playui._downNode;
    var children = downNode.children;
    for(var i = 0; i < children.length; i++){
        var tipNode = children[i].getChildByName("ting_tips");
        if(tipNode){
            tipNode.removeFromParent(true);
        }
    }
    ...
    if(IsTurnToMe()){
        ...
        for(var i = 0; i < children.length; i++){
            ...
            var tipNode = card.getChildByName("ting_tips");
            var cardNum = card.tag; 
            tingCards = MjClient.majiang.getCheckTingHuCards(cardNum,pl.mjhand, true);
            cc.log("card:" + cardNum + "=======tingCards:" + tingCards);
            if(Object.keys(tingCards).length == 0){
                continue;
            }

            tipNode = new ccui.ImageView("playing/MJ/ting_tips.png"); 
            tipNode.setName("ting_tips");
            var cardSize = card.getContentSize(); 
            tipNode.setPosition(cc.p(cardSize.width / 2,cardSize.height *1.1));
            card.addChild(tipNode,10);
        }        
    }
};

// 判断吃碰杠胡的状态
PlayLayer_XNMJ.prototype.EatVisibleCheck = function()
{ 
    var eat = MjClient.playui.jsBind.eat;
    ...
    eat.touzi._node.setTouchEnabled(true);
    ...
    if((tData.tState == TableState.waitPut && pl.mjState == TableState.waitPut) ||
        (pl.mjState == TableState.waitEat && pl.touzi))
    {
        if(IsTurnToMe())
        {
            //胡
