...
function SetEndOneUserUI_XNMJ(node,off)
{
...
var EndOneView_XNMJ = cc.Layer.extend({
	jsBind:{
		block: {
		...
		dir:
		{
			...
			_text: function () {
                ...
                str += (tData.areaSelectMode.isJiaChui && tData.areaSelectMode.isLianChui) ? ",连庄不可拆锤":"";
                str += (tData.areaSelectMode.isJiaChui && tData.areaSelectMode.isZhuangShangChui) ? ",庄上锤":"";
		...
		head0:{
		   ...
		   _run:function(){ SetEndOneUserUI_XNMJ(this,0); },
		...
		,head1:{ 
		  ...
		  _run:function(){ SetEndOneUserUI_XNMJ(this,1); }
		}
		...
		,head2:{ 
		   ...
		   _run:function(){ SetEndOneUserUI_XNMJ(this,2); }
		}
		,head3:{
			 head:
			 {
			...
			_run:function(){ SetEndOneUserUI_XNMJ(this,3); }
			},
            count_down:{
                _visible :function()
                {
    ...
    ctor:function () {
        this._super();
        var endoneui = ccs.load("endOne_XinNingMJ.json");
        ...
        { 
            ...
            setCardSprite(_node, moCard[i],0); 
            ...
            if(huNum > 1){
            	// 多胡
            	var niaoData = MjClient.majiang.getNiaoIsShow(moCard[i],4); 
            	if(niaoData != -1){
            	...
            	// 159 + 风牌
            	var dataNum = moCard[i] % 10;
        		if(dataNum == 1 || dataNum == 5 || dataNum == 9 || moCard[i] > 30 ){
        			_node.setColor(cc.color(255,255,255));  
        			zhongNiaoNum++;
        		}
            }  
