...
PokerShaoYangSanDaHa.prototype.CARDTPYE = PDK_CARDTPYE;
var PDK_CARDCOUNT = {};
PDK_CARDCOUNT[PDK_CARDTPYE.liandui] = 4;
PDK_CARDCOUNT[PDK_CARDTPYE.duizi] = 2;
PDK_CARDCOUNT[PDK_CARDTPYE.danpai] = 1;

// 54 - 12 =42*2=84-8=76/4=19
PokerShaoYangSanDaHa.prototype.randomCards = function(areaSelectMode, tData) {
    var cards = [];
    // 去掉3、4
    for (var i = 1; i <= 54; i++) {
        if( this.calPoint(i) != 3 && this.calPoint(i) != 4 ) {
            if (areaSelectMode["chou6"] && this.calPoint(i) == 6) { continue; }
            cards.push(i);
        }

    }
    this.handCount = areaSelectMode["chou6"] ? 19 : 21;
    // 两幅扑克牌
...
PokerShaoYangSanDaHa.prototype.cardlog = function(cards, zhuColorType) {
 ...
 */
PokerShaoYangSanDaHa.prototype.countWinType = function(jiaofen, PfenPaiArr, isTouXiang, Pkoudifen) {
...
PokerShaoYangSanDaHa.prototype.isWuZhu = function(zhuColorType) {
...
PokerShaoYangSanDaHa.prototype.calPoint = function(num) {
    if (!num) return -1;
...
PokerShaoYangSanDaHa.prototype.calPointBySdh = function(num, zhuColorType) {
    if (!num) return -1;
...
// 牌的花色
PokerShaoYangSanDaHa.prototype.calColor = function(num) {
    if (!num) return -1;
    // 0:方块  1:梅花  2:梅花  3:黑桃
...
PokerShaoYangSanDaHa.prototype.calColorSdh = function(num, zhuColorType) {
    if (!num) return -1;
...
// 判断此牌是否是主花
PokerShaoYangSanDaHa.prototype.isZhuCard = function(num, zhuColorType) {
    var point = this.calPoint(num);
    var color = this.calColor(num);
...
PokerShaoYangSanDaHa.prototype.getChangZhuCount = function(oCards) {
    if(!oCards || typeof oCards == 'undefined') {
        return 0;
    }
    var num = 0;
    for (var i = 0; i < oCards.length; i++) {
        var point = this.calPoint(oCards[i]);
        if (this.cardCfg.changZhuByPoint[point]) {
            num++
        }
    }
    return num;
...
PokerShaoYangSanDaHa.prototype.getRoundFirstCards = function(putCardsRecord, uid, maxPlayNum) {
...
PokerShaoYangSanDaHa.prototype.getRoundCardsByUid = function(putCardsRecord, uid, maxPlayNum) {
    if(!putCardsRecord || typeof putCardsRecord == 'undefined') {
        return null;
    }
...
// 获取本轮最大的牌， 同点数先出比后出的大
PokerShaoYangSanDaHa.prototype.getMaxCardPlayerUid = function(roundPutCards, firstPutUid, uids, zhuColorType,bteshutuolaji,bchou6) {
    var frontUid = -1;
    if(!roundPutCards || typeof roundPutCards == 'undefined') {
        return frontUid;
    }
    ...
    for(var i=0; i<uids.length; i++) {
        ...
        if(frontUid > 0) {
            ...
            if(cards && tmpcards && this.isBigger(tmpcards, cards, zhuColorType,bteshutuolaji,bchou6) > 0) {
...
PokerShaoYangSanDaHa.prototype.getAllCardFen = function( fenPaiArr ) {
    var fen = 0;
...
// 获得牌的分  非分牌返回0分 
PokerShaoYangSanDaHa.prototype.getCardFen = function(num) {
...
// 获取所有对应牌数的拖拉机组合
PokerShaoYangSanDaHa.prototype.getLianDuiCards = function(oCards, cardNum, zhuColorType,bteshutuolaji,bchou6) {
    if(oCards.length < cardNum) return false;
    var resCards = [];
    for(var i=0; i < oCards.length - cardNum + 1; i++) {
        var cards = oCards.slice(i, i + cardNum);
        if( this.isLiandui(cards, zhuColorType,bteshutuolaji,bchou6) )
            resCards.push(cards);
    }
...
// 获取所有对应牌数的拖拉机组合
PokerShaoYangSanDaHa.prototype.getDuiZis = function(oCards, zhuColorType) {
...
PokerShaoYangSanDaHa.prototype.countDuiNum = function(oCards, zhuColorType) {
    var num = 0;
    var cardNumByValue = {};
    for(var i in oCards){
...
PokerShaoYangSanDaHa.prototype.isChangZhu = function (num) {
    var point = this.calPoint(num);
    return this.cardCfg.changZhuByPoint[point] != null;
}
 ...
 * @return {bool}
 */
PokerShaoYangSanDaHa.prototype.isLiandui = function(oCards, zhuColorType,bteshutuolaji,bchou6) { //oCards有序
    ...
    var cardsSorted = this.sortHandCards(oCards, zhuColorType);
    
    var cardCount = cardsSorted.length;
    var points = [];
    var colors = [];
    var colorsSdh = [];
    for (var i = 0; i < cardsSorted.length; i++) {
     
        points.push(this.calPointBySdh(cardsSorted[i], zhuColorType));

        colors.push(this.calColor(cardsSorted[i]));
        colorsSdh.push(this.calColorSdh(cardsSorted[i], zhuColorType));
    }
    ...
    for(var i=0; i < cardCount - 1; i++) {
        ...
        if(i % 2 == 1){
            if(!bteshutuolaji){
                if (this.isChangZhu(cardsSorted[i]) || this.isChangZhu(cardsSorted[i+1])) { return false; }
                if (points[i] - points[i+1] != 1) { return false; } 
                if (points[i+1]%100 == 4 && points[i]%100 == 5) { return false;} 
            }else{
                //5588算连对，去掉6的条件下
                if(bchou6){
                    //6688不算连对
                    if((points.indexOf(5)!=-1 && points.indexOf(4)!=-1)
                        ||(points.indexOf(105)!=-1 && points.indexOf(104)!=-1))
                    {
                        if((points[i] == 5 && points[i+1] == 4)||(points[i] == 105 && points[i+1] == 104))
                        {
                            i+=1;
                            return false;
                        }
                    ...
                    //5588算连对
                    if((points.indexOf(5)!=-1 && points.indexOf(3)!=-1)
...
PokerShaoYangSanDaHa.prototype.isDuizi = function(oCards) {
 ...
 */
PokerShaoYangSanDaHa.prototype.calType = function(cards, zhuColorType,bteshutuolaji,bchou6) {
    var cardCount = cards.length;

    // 连对，2对起
    if (cardCount >= PDK_CARDCOUNT[PDK_CARDTPYE.liandui]  && this.isLiandui(cards, zhuColorType,bteshutuolaji,bchou6))
 ...
 * @return {number}
 */
PokerShaoYangSanDaHa.prototype.calCardsValue = function(cards, zhuColorType) {
...
// // 牌点比较函数
PokerShaoYangSanDaHa.prototype.cardValueCmp = function(a, b, zhuColorType) {
 ...
 */
PokerShaoYangSanDaHa.prototype.isBigger = function(beiYaCards, oLastCards, zhuColorType,bteshutuolaji,bchou6) {

    var cardsType = this.calType(beiYaCards, zhuColorType,bteshutuolaji,bchou6);
    ...
    var lastCardsType = this.calType(oLastCards, zhuColorType,bteshutuolaji,bchou6);
...
PokerShaoYangSanDaHa.prototype.checkPut = function(oHands, cards) {
 ...
 */
PokerShaoYangSanDaHa.prototype.checkPutSdh = function(oHands, pOutCards, oBeiYaCard, zhuColorType, isCanShuaiPai,tData) {
    ...
    if(!oBeiYaCard || oBeiYaCard.length <= 0) {
        ...
        var outCardType = this.calType(outCards, zhuColorType,bteshutuolaji,bchou6);
    ...
    // 检测是否打出同花色的牌
        ...
        if(outCards.length <= handColorSameNum &&  outCardsSameColorNum == outCards.length) {
            var oBeiYaType = this.calType(oBeiYaCard, zhuColorType,bteshutuolaji,bchou6);
            var outCardType = this.calType(outCards, zhuColorType,bteshutuolaji,bchou6);
            // 被压的是拖拉机， 
            if(outCardType != oBeiYaType && oBeiYaType == PDK_CARDTPYE.liandui) {
                ...
                if( this.getLianDuiCards(handColorSameCards, oBeiYaCard.length, zhuColorType,bteshutuolaji,bchou6) && !this.isLiandui(outCards, zhuColorType,bteshutuolaji,bchou6) )
                    needPutLianDui = true;
...
PokerShaoYangSanDaHa.prototype.findCardByType = function(oHands, oBeiYaCard, zhuColorType) {
 ...
 * @return {array} 提示的牌
 */
PokerShaoYangSanDaHa.prototype.tipCards = function(oHands, oBeiYaCard, zhuColorType,bteshutuolaji,bchou6) {
    ...
    if(!oBeiYaCard) {
        ...
        var beiyaType = this.calType(oBeiYaCard, zhuColorType,bteshutuolaji,bchou6)
        var tiparr = [];
        ...
        if( handsColorCards.length >= cardNum ) {
            ...
            if(beiyaType == PDK_CARDTPYE.liandui) {
                ...
                var lianduis = this.getLianDuiCards(sourceCards, cardNum, zhuColorType,bteshutuolaji,bchou6);
                if( lianduis ) {
 ...
 */
PokerShaoYangSanDaHa.prototype.sortHandCards = function(oCards, zhuColorType) {
 ...
 * @return {array}
 */
PokerShaoYangSanDaHa.prototype.findNSameCard = function(hands, point, n, zhuColorType) {
...
// 删除相应牌值(不算花色)
PokerShaoYangSanDaHa.prototype.delPoint = function(hands, points){
    var arr = hands.slice();
...
// 删除相同的牌(花色相同，牌值相同)
PokerShaoYangSanDaHa.prototype.delCards = function(hands, cards) {
    var arr = hands.slice();
    for(var i=0; i < cards.length; i++){
        for (var j = 0; j < arr.length; j++) {
            if( arr[j] == cards[i] ){
...
// 从牌组中获取指定花色
PokerShaoYangSanDaHa.prototype.getColorCards = function(cards, colors, zhuColorType) {
    var arr = [];
    for(var i=0; i < cards.length; i++) {
        var num = cards[i];
...
PokerShaoYangSanDaHa.prototype.formatCards = function(oCards, zhuColorType) {
    var info = {};
...
PokerShaoYangSanDaHa.prototype.isSameCards = function(cards1, cards2, zhuColorType) {
...
// 求出牌型
PokerShaoYangSanDaHa.prototype.cardsType = function(oCards, zhuColorType,bteshutuolaji,bchou6) {
    if (oCards) {
        return this.calType(oCards, zhuColorType,bteshutuolaji,bchou6);
    }
    return -1;
};

PokerShaoYangSanDaHa.prototype.isAllZhuCard = function(cards, zhuColorType) {
    for (var i = 0; i < cards.length; i++) {
...
if(typeof(module)!="undefined" && module.exports)    
    module.exports = PokerShaoYangSanDaHa;

if(typeof(MjClient)!="undefined")
    MjClient.poker_ShaoYangSanDaHa = new PokerShaoYangSanDaHa();
