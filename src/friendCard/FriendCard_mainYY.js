...
var FriendCard_main = cc.Layer.extend({
	...
	initOther: function(back){
        ...
        _btnwanfa = back.getParent().getChildByName("btn_liebiao");
        _btnwanfa.setTouchEnabled(true);
        setWgtLayout(_btnwanfa, [0.15, 0.15], [0.9, 0.9], [0, 0]);
        this._panelwanfa = _btnwanfa.getChildByName("Panel_wanfa");
        this._panelwanfa.visible = false;
        this._listView_rule = this._panelwanfa.getChildByName("ListView_rule");

        if(_btnwanfa) {
            _btnwanfa.addTouchEventListener(function(sender, type) {
                if (type == 2)
                {
                    cc.log("---------玩法列表的按钮-------------11111111111");
                    this._panelwanfa.visible = !this._panelwanfa.isVisible();
                    cc.log("---------玩法列表的按钮-------------");
                }
            }, this);
        }

        // var _btncloselist =  this._panelwanfa.getChildByName("btn_close");
        // _btncloselist.addTouchEventListener(function(sender, type) {
        //     if (type == 2)
        //     {
        //         this._panelwanfa.visible = !this._panelwanfa.isVisible();
        //     }
		...
		if (this.fangkaBG) {
            ...
            //房卡管理
            var that = this;
            var fangkaGuanli = this._image_bottom.getChildByName("btnList").getChildByName("btn_fangka");
                fangkaGuanli.addTouchEventListener(function(sender, type) {
                    if (type == 2) {
                    	var _layerFangka = new FriendCard_fangkazhuanzeng();
                        MjClient.FriendCard_main_ui.addChild(_layerFangka);
                        // gotoStoreFunc();
                        // that.closeClubList();
	...
	setDeskBGImg: function(desk, skinType,skinCfg) {
		...
		for (var k = 1; k < 5; k++) {
			...
			if (yizi)
				yizi.loadTexture(path + "yizi_1.png");
		}
	...
	refreshInfo: function() {
		...
		if (this.fangkaBG) {
			...
			if(this.data.info.type == 1){
				...
				this.text_fangka_type.setString("我的黄金");
		...
		this._btn_webZhanji.setVisible(true);//MjClient.systemConfig.openUserInfoShare + "" == "true" && (!this._btn_FCM || !this._btn_FCM.isVisible() || !this._btn_match.visible));
	...
	updateDeskNum: function(ruleRoomData) {
		...
		this.text_deskNum.visible = true;
	},
	closeBtns:function(isClose){
		...
		if(this._listView_rule){
			...
			if(isClose){
				...
				this._btn_all_rule.visible = false;//true;
				this._btn_outline_rule.visible = false;//true;
	...
	refreshRuleList: function() {
		...
		btn_outline_rule.visible = false;//FriendCard_Common.isManager();
...
//房卡转曾
var FriendCard_fangkazhuanzeng = cc.Layer.extend({
    ...
    ctor: function (clubData, clubId, matchId, subClubId) {
        ...
        var that = this;
        var self = this;
        var node = ccs.load("friendCard_fangka.json").node;
        this.addChild(node);
        this.node = node;
        ...
        this.listView_shangji = back.getChildByName("listView_shangji");
        this.listView_shangji.removeAllItems();



        MjClient.block();
        MjClient.gamenet.request("pkplayer.handler.userList",{uid:SelfUid(),pageNum:10,order:3,index:1},function(rtn){
            cc.log("pkplayer.handler.userList = "+JSON.stringify(rtn))
            MjClient.unblock();
            if(rtn.code == 0) {
            	cc.log("!!!Success!!!!!");
            	that._userData = rtn.data.list;
            	that.addItem(that._userData);
            }else {
    ...
    createItem: function (itemData, i) {
        ...
        text_name.setString(unescape(itemData.userNickname));
        ...
        //     _id2 = _id;
        // }
        text_id.setString(_id);
        var data = itemData;
        
        var _btnZhuanzeng = item.getChildByName("Button_1");
        _btnZhuanzeng.setTag(Number(_id));
        _btnZhuanzeng.addTouchEventListener(function(sender, type) {
            if(type == 2)
            {
                ////成功后，加粒子效果
                ...
                var _zengSongID = sender.getTag(); //赠送给谁的ID
                var itagID = SelfUid(); //自己的ID
                if (MjClient.systemConfig.recharge.length > 1) {
                    ...
                    MjClient.Scene.addChild(new payWayLayer(function(platform){
                        MjClient.recharge(itagID, parseInt(platform), null, null, {receiverId:_zengSongID});
                    ...
                    MjClient.recharge(itagID, parseInt(MjClient.systemConfig.recharge[0].platform), null, null, 
                        {receiverId:_zengSongID})
                }
            }
            });
        img_hd.needScale = 8;
        img_hd.isMask = true;
        COMMON_UI.refreshHead(this, data.userAvatar ? data.userAvatar : "png/default_headpic.png", img_hd);
    ...
    addItem: function (data) {
        ...
        var _scroll = this.listView_shangji;//this._type == 1 ? this.listView_shangji : this._type == 3 ? this.listView_join : this.listView_win;
        for (var i = 0; i < data.length; i++) {
        	cc.log("========================list data = " + JSON.stringify(data[i]));
