...
var FriendCard_info = cc.Layer.extend({
        ...
        for (var key in MjClient.RuleParam) {
            ...
            if (rule == 'delete') {
                for (let _i = 0; _i < this.wanfaBtns.length; _i++) {
                    ...
                    if (wf.wanFaIndex === rInx && !wf.isAddButton) {
    ...
    initRightNode: function () {
        ...
        let rules = data ? FriendCard_Common.getRuleParm(data.info) : [],
            rLen = rules.length;
        var hang = Math.ceil(rules.length / 2) || 1;
        ...
        let endFun = () => {
            //最有一个
            rLen > 0 && (this.wanfaBtns[rLen - 1].tempP = this["btn_pos_" + rLen]);
            this.checkAddBtn();
            //移动玩法
