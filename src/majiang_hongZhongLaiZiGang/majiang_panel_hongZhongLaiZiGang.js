var majiang_panel_hongZhongLaiZiGang = majiang_panel_hubei.extend({
    jsonFile: "Play_hongZhongLaiZiGang.json",
    ctor: function(){
        this.LAI_ZI_CARD_TYPE = 1;//癞子牌类型
        ...
        //动画名称映射
        this.ExpandEatActionType = {
            LaiZiGang:          "lazigang",//癞子杠
            JinDing:          "jinding",//金顶
            ZuanShiDing:          "zhuanshiding",//钻石顶

        };
        this._super(majiang_panel_hongZhongLaiZiGang, this.jsonFile);
    },
                ...
                node_animation:{
                    _event:{
                        MJHu: function(data){
                            ...
                            var is2DAni = false;
                            if(player.huWord){
                                if(player.huWord.indexOf("gangkai") > -1){
                                    actType = "gangshanghua";
                                    is2DAni = true;
                                }else if(player.huWord.indexOf("qiangganghu") > -1){
                                    actType = "qiangganghu";
                                    is2DAni = true;
                                }
                            }
                            MjClient.playui.showEatActionAnim(this.getParent(), actType,is2DAni);
...
//动画展示癞子牌
majiang_panel_hongZhongLaiZiGang.prototype.showPiZiAndLaiZi = function(node){
...
//Override 是否能添加癞子标识
majiang_panel_hongZhongLaiZiGang.prototype.isCanAddLaiZiIcon = function(cardTag){
    var tData = MjClient.data.sData.tData;
...
//Override
majiang_panel_hongZhongLaiZiGang.prototype.getLaiZiIcon2D = function(cardTag){
...
//Override 是否显示飘分
majiang_panel_hongZhongLaiZiGang.prototype.isShowPiao = function(){
	return false;
};

majiang_panel_hongZhongLaiZiGang.prototype.getHunIconPosition2D = function(){
    return [[60, 107], [52, 70], [40, 104], [74, 68], [40, 78], [61, 77], [22, 30]];
};

//Override 癞子位置偏移比
majiang_panel_hongZhongLaiZiGang.prototype.getEatAndHandCardsLaiZiIconPosSpcae3D = function(){
    return cc.p(0.88, 1.1);
};


//Override [createEndOnePanel 创建小结算界面]
majiang_panel_hongZhongLaiZiGang.prototype.createEndOnePanel = function(){
    return new majiang_winGamePanel_hongZhongLaiZiGang();
};

//Override 所有的牌进行排序
majiang_panel_hongZhongLaiZiGang.prototype.getCardSortArray2D = function(node, newCardTag){
    var tData = MjClient.data.sData.tData;
    ...
    for(i = 0;i < children.length;i++){
        ...
        if(child.name == this.HandleCardType.Hand || (child.name == this.HandleCardType.Chi && child.isCut)){
            ...
            if(this.isSpecialGang(child.tag)){
                laiZiArr.push(child);
                continue;
            }
            handArr.push(child);
...
//Override 所有的牌进行排序
majiang_panel_hongZhongLaiZiGang.prototype.getCardSortArray3D = function(node, newCardTag){
    var tData = MjClient.data.sData.tData;
...
//获取手牌中的痞子牌或癞子牌
majiang_panel_hongZhongLaiZiGang.prototype.getSpecilaCardsOfHand = function(type){
    var cardArr = [];
...
//设置其他牌大小
majiang_panel_hongZhongLaiZiGang.prototype.updateOtherCardSize = function(node){
    var is3D = MjClient.playui.is3DStyle();
            ...
            if(!is3D){
                if(MjClient.size.width / MjClient.size.height >= 1.5){
                    if(maxPlayer === 2)
                        ...
                        setWgtLayout(node, [0, 0.088], [0.5, 1], [5.5, -3]);
    ...
    if(playNode.getName() == "node_left"){
        ...
        if(node.getName() == "img_putCardOne"){
            if(!is3D){
                if(MjClient.size.width / MjClient.size.height >= 1.5){
                    setWgtLayout(node, [0, 0.063], [0.05, 0.5], [5.4, 4.2]);
                }else if(this.isIPad()){
...
//Override 计算打牌插牌位置
majiang_panel_hongZhongLaiZiGang.prototype.getInsertCardPosIndex = function (handcardUI, index) {
    ...
    //发财
    if(this.newCardNode && this.newCardNode.tag == 81){
        if(tagValue == 71){
            return -1;
        }
    ...
    //不是特殊牌
    if(this.newCardNode){
        if(this.isSpecialGang(tagValue)){
            return -1;
        }
 ...
 *  Override 常量定义
 **/
majiang_panel_hongZhongLaiZiGang.prototype.initCardTypeName = function(){
    majiang_panel.prototype.initCardTypeName.call(this);
    this.CsdDefaultCardType["PiZiGang"] = "img_piZiGang";
    this.HandleCardType["HongZhongGang"] = "HongZhongGang";
    this.HandleCardType["LaiZiGang"] = "LaiZiGang";
    this.HandleCardType["FaCaiGang"] = "FaCaiGang";
};

/**
 *  Override 刷新手牌大小
 **/
majiang_panel_hongZhongLaiZiGang.prototype.updateHandCardSize = function(node){
    var playNode = node.getParent();
 ...
 **/
majiang_panel_hongZhongLaiZiGang.prototype.getMoveDistance2D = function(node, cardArr, index, chiFirstIndex){
    if(index == 0){
        return 0;
    }
 ...
 *  Override刷新牌
 **/
majiang_panel_hongZhongLaiZiGang.prototype.resetCardLayout2D = function(node){
    ...
    for(i = 0;i < children.length;i++){
        ...
        if(child.name == this.HandleCardType.FaCaiGang || child.name == this.HandleCardType.LaiZiGang || child.name == this.HandleCardType.HongZhongGang){
 ...
 *  Override 刷新牌,待完成
 **/
majiang_panel_hongZhongLaiZiGang.prototype.resetCardLayout3D = function(node){
...
//更新node_down玩家的特殊杠
majiang_panel_hongZhongLaiZiGang.prototype.updateDownPiZiGangCards3D = function(node, piZiGangArr){
...
//更新node_right玩家的特殊杠
majiang_panel_hongZhongLaiZiGang.prototype.updateRightPiZiGangCards3D = function(node, piZiGangArr){
    var templetCard = node.getChildByName(this.CsdDefaultCardType.PiZiGang);
    var scale = templetCard.getScale() * 1.3;
    ...
    for(var k = 0;k < piZiGangArr.length;k++){
        ...
        cardNode.setPosition(pos_x - 20, pos_y);
...
//更新node_top玩家的特殊杠
majiang_panel_hongZhongLaiZiGang.prototype.updateTopPiZiGangCards3D = function(node, piZiGangArr){
    var templetCard = node.getChildByName(this.CsdDefaultCardType.PiZiGang);
    var scale = templetCard.getScale() * 1.3;
...
//更新node_left玩家的特殊杠
majiang_panel_hongZhongLaiZiGang.prototype.updateLeftPiZiGangCards3D = function(node, piZiGangArr){
    var templetCard = node.getChildByName(this.CsdDefaultCardType.PiZiGang);
    var scale = templetCard.getScale() * 1.3;
...
//Override 更新玩家的牌
majiang_panel_hongZhongLaiZiGang.prototype.updatePlayerCards = function (node){
    var player = this.getPlayerInfoByName(node.getName());
    if(!player){
        return;
    }
    ...
    if(player.mjPiZiGang){
        for(i = 0; i < player.mjPiZiGang.length; i++){
            var gangType = this.GANG_TYPE.LAI_ZI_GANG;
            var gangName = this.HandleCardType.LaiZiGang;
            if(player.mjPiZiGang[i] == 71){
                gangType = this.GANG_TYPE.HONG_ZHONG_GANG;
                gangName = this.HandleCardType.LaiZiGang;
            }else if(player.mjPiZiGang[i] == 81){
                gangType = this.GANG_TYPE.FA_CAI_GANG;
                gangName = this.HandleCardType.FaCaiGang;
            }
 ...
 *  添加癞子标识
 **/
majiang_panel_hongZhongLaiZiGang.prototype.addLaiZiIcon2D = function(cardNode){
...
//获取癞子杠的图标偏移量
majiang_panel_hongZhongLaiZiGang.prototype.getPiZiGangIconPositionOffset2D = function(cardNode){
    var playerNodeName = cardNode.getParent().getName();
 ...
 *  Override 添加癞子标识
 **/
majiang_panel_hongZhongLaiZiGang.prototype.addLaiZiIcon3D = function(cardNode){
    if(!this.isCanAddLaiZiIcon(cardNode.tag)){
        return;
    }
 ...
 **/
majiang_panel_hongZhongLaiZiGang.prototype.getHandCardFacePosition3D = function(mjBgType){
 ...
 *  return {Array}
 **/
majiang_panel_hongZhongLaiZiGang.prototype.getCardFacePositon2D = function(mjBgType, cardNode){
    ...
    switch(cardTypeName){
        ...
        case this.HandleCardType.FaCaiGang:
        case this.HandleCardType.HongZhongGang:
...
//Override 
majiang_panel_hongZhongLaiZiGang.prototype.getPutCardsLaiZiIconPosSpcae3D = function(cardNode){
...
//获取3d玩家吃碰杠手牌的起始位置
majiang_panel_hongZhongLaiZiGang.prototype.getEatAndHandCardsStartPos3D = function(node_name, firstCardType){
 ...
 **/
majiang_panel_hongZhongLaiZiGang.prototype.updateChiGangCards3D = function(cardNode, cardTag){
...
// 设置游戏中，结束位置的头像
majiang_panel_hongZhongLaiZiGang.prototype.setInGameHeadLayout = function(nodeName, layout_head){
...
//更新翻倍
majiang_panel_hongZhongLaiZiGang.prototype.updatePlayerBeiShu = function(node, data){
    if(!node) return;
...
//翻倍动画
majiang_panel_hongZhongLaiZiGang.prototype.showPlayerFanBeiAnim = function(node, data){
    node.visible = false;
    if(!node || !data) return;
...
//扩展的eat动画
majiang_panel_hongZhongLaiZiGang.prototype.showExpandEatActionAnim = function(playerNode, actType, param){
    param = param || {};
...
majiang_panel_hongZhongLaiZiGang.prototype.isSpecialGang = function(card){
...
//Override
majiang_panel_hongZhongLaiZiGang.prototype.dealSpecialGang = function(playerNode, gangType,card){
    //发财，红中，癞子杠
    if(gangType){
        switch (gangType) {
            case 1:
                this.showEatActionAnim(playerNode, "gang");
                gangType = this.HandleCardType.HongZhongGang;
                break;
            case 2:
                this.showEatActionAnim(playerNode, "gang");
                gangType = this.HandleCardType.FaCaiGang;
                break;
            case 3:
                this.showExpandEatActionAnim(playerNode, this.ExpandEatActionType.LaiZiGang);
                gangType = this.HandleCardType.LaiZiGang;
                break;
        }
    }
    this.createCard(playerNode, this.CsdDefaultCardType.PiZiGang, gangType, card);
    this.resetCardLayout(playerNode);
};
majiang_panel_hongZhongLaiZiGang.prototype.dealPut = function(playerNode, msg){
    var tData = MjClient.data.sData.tData;
    ...
    if(this.isSpecialGang(msg.card)){
        var player = this.getPlayerInfoByName(playerNode.getName());
        if(!msg || msg.uid != player.info.uid) return;
        this.dealSpecialGang(playerNode,msg.gangType,msg.card);
...
majiang_panel_hongZhongLaiZiGang.prototype.putOutCard = function (cardNode, cardTag, autoput){
    ...
    if(this.isSpecialGang(cardTag)){
        ...
        switch (cardTag) {
            ...
            case 81:
                gangType = 2;
                break;
            case MjClient.data.sData.tData.hunCard:
                gangType = 3;
                break;
        }
        var uidIndex = MjClient.playui.getUidIndex(this.getSelfUid())
        var node = MjClient.playui.getUIBind(uidIndex);
        this.dealSpecialGang(node,gangType,cardTag)
        return;
    }
 ...
 *	处理结算
 **/
majiang_panel_hongZhongLaiZiGang.prototype.handleRoundEnd = function(){
    var tData = MjClient.data.sData.tData;
    ...
    for(var i = 0; i < tData.uids.length; i++){
        ...
        if(player.fengDingType > 0 && player.winone < 0){
            fengDingPlayerCount += 1;
        }
        if(MjClient.majiang.isPlayerKaiKou(player)){
            kaiKouPlayerCount += 1;
        }
    }

    var showEndCards = function(){
        if(winnerUid && tData.maxPlayer != 2 && fengDingPlayerCount == tData.maxPlayer - 1){//金顶 钻石顶
            var atlasSrc = "spine/yipierlai/chipenggang.atlas";
            var jsonSrc = "spine/yipierlai/chipenggang.json";
            var playUiNode = MjClient.playui.getChildByName("playui");
            var projNode = playUiNode.getChildByName("dingAnim");
            if(projNode){
            ...
            var dingAni = kaiKouPlayerCount == tData.maxPlayer - 1 ? self.ExpandEatActionType.ZuanShiDing : self.ExpandEatActionType.JinDing
            projNode.setAnimation(0, dingAni, false);
            projNode.setTimeScale(1.5);
            projNode.setScale(scale);
            projNode.setName("dingAnim");
...
//设置禁胡标记
majiang_panel_hongZhongLaiZiGang.prototype.setJinHuTip = function(node, msg){
    return;
    var sData = MjClient.data.sData;
 ...
 **/
majiang_panel_hongZhongLaiZiGang.prototype.checkWhenTouchBegan = function(cardNode){
    var tData = MjClient.data.sData.tData;
    if (MjClient.majiang.getAllCardsTotal(tData) - tData.cardNext < (MjClient.MaxPlayerNum + 10)){
...
//Override 
majiang_panel_hongZhongLaiZiGang.prototype.handlerWhenCardTouchEnded = function(cardNode, cardTag){
    var sData = MjClient.data.sData;
    var tData = sData.tData;
    if (MjClient.majiang.getAllCardsTotal(tData) - tData.cardNext < (MjClient.MaxPlayerNum + 10)){
...
majiang_panel_hongZhongLaiZiGang.prototype.getHandAndEatCardSpcae3D = function(){
    return 1.7;
};

//Override
majiang_panel_hongZhongLaiZiGang.prototype.getAllTouchCards = function(){
...
majiang_panel_hongZhongLaiZiGang.prototype.getDisScaleOfFirstCutDownBetweenEatCard2D = function(){
    return 1;
};
majiang_panel_hongZhongLaiZiGang.prototype.createFenZhangAni = function(aniNode){
 ...
 *  吃碰杠胡动画
 **/
majiang_panel_hongZhongLaiZiGang.prototype.showEatActionAnim = function(playerNode, actType,is2DAni) {
    var delayTime = 2;
    var animateNode = playerNode.getChildByName("node_animation");
    ...
    var _scale = 1;
    if(is2DAni){
        var aniUrl = "spine/" + actType + "/" + actType + ".png";
        projNode = new cc.Sprite(aniUrl);
    }else if (isOpenEffect == 1 || !this.isNeedEatActionEffect3D()) {
