...
//向服务器发送 过消息
MjClient.MJPass2NetForloudifpf = function(){
    cc.log("====================send======pass=====");
...
function SetUserVisible_loudifpf(node, off){
    ...
    if(pl){
        ...
        // InitUserHandUI_loudifpf(node, off);
...
function InitUserHandUI_loudifpf(node, off){
...
var PlayLayer_loudifpf = cc.Layer.extend({
    jsBind: {
        _event: {
            mjhand: function() {
            ...
            roundEnd: function() {
                ...
                function delayExe(){
                    ...
                    if (sData.tData.roundNum <= 0){
                        var layer = new endAll_xxghz();
                        layer.setVisible(false);
                        self.addChild(layer,500);
                    }
                    if(!MjClient.endoneui){
                        self.addChild(new EndOneView_ldfpf(),500);//EndOneView_xxghz()
                    }
                }
        ...
        roundInfo:{
            ...
            _run:function(){
                var tData = MjClient.data.sData.tData;
                var str = "";
                log("fsdfsd" + tData.areaSelectMode.isfending);
                ...
                str += (tData.areaSelectMode.piaohu == true) ? "/飘胡" :"" ;
        ...
        down: {
            ...
            handCard: {
                _run:function () {
                    ...
                    }else{ // 如果本类只有湘乡地区在用，就可以把handcard下面的else都去掉
                         var sizeType = util.localStorageEncrypt.getNumberItem(MjClient.KEY_ZI_PAI_HAND_SIZE_TYPE,0);
                        var mjBgType = ziPai.getZiPaiType();

                        if(sizeType == 0){
                            setWgtLayout(this, [87 / 1280, 0], [0.27, 0.75], [0, 0]);
                        }else if(sizeType == 1){
                            setWgtLayout(this, [75 / 1280, 0], [0.27, 0.75], [0, 0]);
                        }
                            
                        if(mjBgType == 0){
                            setWgtLayout(this, [95 / 1280, 137 / 720], [0.27, 0.75], [0, 0]);
                        }
                    }
            ...
            _event: {
                ...
                initSceneData: function(eD) {
                    SetUserVisible_loudifpf(this, 0);
                    InitUserHandUI_loudifpf(this, 0);
...
PlayLayer_loudifpf.prototype.showAndHideTrust = function(node, uiOff){
    var tData = MjClient.data.sData.tData;
...
//庄家打出第一张牌之后，刷新提垅的牌
PlayLayer_loudifpf.prototype.UpdateEatCardsAfterFirst = function(posNode,off){
    var sData = MjClient.data.sData;
    var tData = MjClient.data.sData.tData;
...
//重置手牌的顺序
PlayLayer_loudifpf.prototype.ResetHandCard = function(posNode,off, needAction, isDelay){
...
PlayLayer_loudifpf.prototype._doMovetoAction = function(node,endP){
    node.stopAllActions();
    var ac = cc.moveTo(0.3,endP);
    node.runAction(ac);
};

PlayLayer_loudifpf.prototype.ResetOtherCard = function(posNode,off){
    //添加吃、碰、跑、偎、提
    var sData = MjClient.data.sData;
    var tData = null;
    if(sData){
...
PlayLayer_loudifpf.prototype.ResetPutCard = function(posNode,off){
    //添加打出的牌
    var pl = getUIPlayer_xiangxiang(off);
    if (!pl) return;
...
//重置牌的顺序
PlayLayer_loudifpf.prototype.CardLayoutRestore = function(posNode,off){
    if(MjClient.rePlayVideo == -1){
...
// 判断吃、碰、跑、偎、提、胡的状态
PlayLayer_loudifpf.prototype.EatVisibleCheck = function(off){
    var eat = MjClient.playui.jsBind.eat;
...
PlayLayer_loudifpf.prototype.showAndHideHeadEffect = function(){
...
PlayLayer_loudifpf.prototype.showCountdown = function(node){
