function addWxHeadToEndUI_ldfpf(node,off)
{
...
function reInitarrCardVisible_ldfpf()
{
...
var EndOneView_ldfpf = cc.Layer.extend({
    _block:null,
	jsBind:{
		block:{
		    _run:function () {
					...
					head:{
				   	...
				   	_run:function(){
				   		...
				   		for(var index = 0;index<playerCount;index++){
						    ...
						    addWxHeadToEndUI_ldfpf(cloneHead,showIndex);
				   		}
				   	},
				},
				showInfo:{
				   		...
				   		_run:function(){
                            ...
                            if(tData.cardNext < cards.length){
                                ...
                                if(tData.maxPlayer == 2 && tData.areaSelectMode.maipai){ 
					   			...
					   			var nextLine = (tData.maxPlayer == 2 && tData.areaSelectMode.maipai) ? cards.length - 20 : tData.cardNext + 20;
			...
			main:{
				...
				leftPanel:{
					_run:function(){
						...
						addWinMjHand_ldfpf(this,MjClient.MaxPlayerNum_xiangxiang - 1);
				...
				rightPanel:{
					_run:function(){
						if(MjClient.gameType == MjClient.GAME_TYPE.HY_LIU_HU_QIANG){
							addWinMjHand_ldfpf(this,MjClient.MaxPlayerNum_xiangxiang - 2);
						}else{
							addWinMjHand_ldfpf(this,1);
						}
						if(MjClient.rePlayVideo != -1){
	                    	this.visible = false;
	                    }
...
//添加手牌
function addWinMjHand_ldfpf(_node,off) {
	var sData = MjClient.data.sData;
    var tData = sData.tData;
