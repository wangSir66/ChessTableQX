 ...
 */
var actionZindex_pingjiangzhaniao = 1000;
var _isShowShaiziAni_pingjiangzhaniao = false; //为了控制mjhand之后播骰子动画时newCard的状态显示
MjClient.AllKaiGangBox = [];
MjClient.CanClean = false;
var ClickBu = false;
//向服务器发送 过消息
MjClient.MJPass2NetForpingjiangzhaniao = function()
{
   // console.log(">>>>>>>>>普通  过 <<<<<<<<");
    cc.log("====================send======pass=====Forpingjiangzhaniao");
    ...
    if(IsTurnToMe() && tData.tState == TableState.waitPut)
    {
        ...
        var commitFunc = function () {
            ...
            MjClient.majiang.MJPassConfirmToServer_new();
        }
        cc.log("自己回合！！ ");
    ...
    else
    {
        if(MjClient.playui.jsBind.eat.hu._node.visible)
        {
            ...
            if (MjClient.getAppType() == MjClient.APP_TYPE.QXYYQP|| MjClient.getAppType() == MjClient.APP_TYPE.YLHUNANMJ) {
                ...
                var saveRoomMsgValueH = util.localStorageEncrypt.getStringItem("IGNORE_H_TIP","");
                //cc.log("忽略过胡提示！！ ",saveRoomMsgValueH.length,saveRoomMsgValueH);
                if(saveRoomMsgValueH.length > 0 && saveRoomMsgValueH == roomMsgValue){
                    //MJPassConfirmToServer();
                    MjClient.majiang.MJPassConfirmToServer_new();
                    return;
            ...
            }, (MjClient.getAppType() == MjClient.APP_TYPE.QXYYQP|| MjClient.getAppType() == MjClient.APP_TYPE.YLHUNANMJ) ? "3":"1");
        }
        else
        {
            //cc.log("不存在胡按钮！！ ");
...
MjClient.MJPass3NetForpingjiangzhaniao = function(eatFlag, eatFlag2)
{
    cc.log("====================send======pass=====");
...
var showShaiziAni_pingjiangzhaniao = false;
function showShaiziAni_pingjiangzhaniao_old(poff)
{
    if(poff == 0) {
        ...
        if(MjClient.rePlayVideo != -1)//回放
        {
            for (var off = 0; off < 4; off++) {
                ...
                InitUserHandUI_pingjiangzhaniao(_node, off);
            }
            playEffect("shuffle");
            MjClient.playui.EatVisibleCheck();
            return;
        ...
        _isShowShaiziAni_pingjiangzhaniao = true;
        ...
        firstFrame.runAction(cc.sequence(playSoundFunc, animate,showPonitFunc,cc.delayTime(0.8),cc.callFunc(function(){
            //发牌
            _isShowShaiziAni_pingjiangzhaniao = false;
            ...
            if( !MjClient.playui.ShowQiShouHuBtn() ){
                MjClient.playui.EatVisibleCheck();
            }
        }),cc.removeSelf()));
...
function showShaiziAni_pingjiangzhaniao_dami(poff)
{
    if(poff == 0) {
...
function updateHeadUI_pingjiangzhaniao(node, off , actionType)
{      
    cc.log('updateHeadUI_pingjiangzhaniao', off)
    var pl = getUIPlayer(off);
    if(!pl) return;
    // showHandCard_MLHZ(node, off);
    showHandCard_pingjiangzhaniao(node,off);
    MjClient.playui.CardLayoutRestore(node, off);
    cc.log('updateHeadUI_pingjiangzhaniao update success', off)
...
function SetUserVisible_pingjiangzhaniao(node, off)
{
    if (_isShowShaiziAni_pingjiangzhaniao)  // 解决摇骰子时断线重连调用本函数，会手牌重复的bug by cyc
        return;
...
function showHandCard_pingjiangzhaniao(node, off)
{
    var sData = MjClient.data.sData;
    var tData = sData.tData;
    var pl = getUIPlayer(off);
   ...
   // 把吃的牌放中间 begin
    if ((MjClient.getAppType() == MjClient.APP_TYPE.QXYYQP|| MjClient.getAppType() == MjClient.APP_TYPE.YLHUNANMJ) && pl.mjchi.length > 0){
...
function InitUserHandUI_pingjiangzhaniao(node, off)
{
    var sData = MjClient.data.sData;
    var tData = sData.tData;
    var pl = getUIPlayer(off);
    ...
    // }
     ...
     if (tData.tState == TableState.waitJiazhu && SelfUid() == pl.info.uid) {
        if (pl.mjState == TableState.waitJiazhu) {
            var img1 = "playing/other/bupiaofen.png";
            var img2 = "playing/other/piaofen.png";
    ...
    if (pl.haiDiLaoState == 2 && SelfUid() == pl.info.uid && MjClient.rePlayVideo == -1)
    {
    ...
    { 
        ...
        if ((pl.tPutCard || Object.keys(tingSet).length > 0) && !downNode.getChildByName("AutoPut_btn") && !pl.isTing) 
        { 
...
var PlayLayer_pingjiangzhaniao = cc.Layer.extend({
    jsBind: {
        _event: {
            mjhand: function() {
            ...
            roundEnd: function() {
                ...
                var delayTime_touzi = 0
                if (tData.shaizi && tData.shaizi.length > 0) 
                {
                    delayTime_touzi = 1;
                }
                ...
                {
                    var action = cc.sequence(cc.delayTime(2 * delayTime_touzi + 0.5),cc.callFunc(function(){
                        ...
                        self.addChild(new EndOneView_pingjiangzhaniao(),500);
                    }));
                    action.setTag(201801204);
                    self.runAction(action);
                }
                function delayExe()
                {
            ...
            initSceneData: function() {
                ...
                cc.log("initSceneData -----------------pingjiangzhaniao = " + JSON.stringify(this));
                CheckRoomUiDelete();
        ...
        BtnPutCard:{ //add by  sking for put card button
            ...
            _event:{
                ...
                waitPut: function() {
                    if (_isShowShaiziAni_pingjiangzhaniao) return;
                    var pl = getUIPlayer(0);
                    var eat = MjClient.playui.jsBind.eat;
                    cc.log("isTing---------------",pl.isTing);
...
/*                        waitHaiDiLao: function()
                ...
                tingIcon: {
                    ...
                    _run:function(){
                        this.visible = false;
                        var pl = getUIPlayer(0);
                        if (pl && pl.jiangjianghuLists && Object.keys(pl.jiangjianghuLists).length > 0) 
                        {
                            this.loadTexture("playing/gameTable/icon_jiang.png");
                        }
                    ...
                    _event: {
                        ...
                        roundEnd: function(){
                            // cc.log("end rounde------------------------")w;
                            this.loadTexture("playing/gameTable/icon_ting.png");
                            this.visible = false;
                        }
                    }
                },
            ...
            play_tips: {
                ...
                _run: function () {
                    this.zIndex = actionZindex_pingjiangzhaniao;
                },
                _visible:false,
            },
            tai_layout:{
            ...
            out1: {
                _run: function () {
                    ...
                    {  
                        setWgtLayout(this, [0.0, 0.07], [0.53, 0], [-7, 4.85]);
                    }
                    ...
                    if (MjClient.MaxPlayerNum == 2) 
                    {
                        if (MjClient.size.width / MjClient.size.height >= 1.5)
                        {
                            setWgtLayout(this, [0.0, 0.0763], [0.39, 0], [-7, 4.85]);
                        }
                        else
                        {
                            setWgtLayout(this, [0.0, 0.07], [0.39, 0], [-7, 4.85]);
                        }
                    }
            ...
            out0: {
                _run: function () {
                    ...
                    {
                        setWgtLayout(this, [0.0, 0.07], [0.53, 0], [-7, 3.8]);
                    }
                    if (MjClient.MaxPlayerNum == 3)
                        this.x -= this.width * this.scale * 1;
                    ...
                    if (MjClient.MaxPlayerNum == 2) 
                    {
                        if (MjClient.size.width / MjClient.size.height >= 1.5)
                        {
                            setWgtLayout(this, [0.0, 0.0763], [0.39, 0], [-7, 3.8]);
                        }
                        else
                        {
                            setWgtLayout(this, [0.0, 0.07], [0.39, 0], [-7, 3.8]);
                        }
                    }
            ...
            tingCardsNode: {
                ...
                _event: {
                    ...
                    initSceneData:function(eD)
                    {
                        ...
                        var pl = getUIPlayer(0);
                        var tData = MjClient.data.sData.tData;
                        if(pl && (tData.tState == TableState.waitEat || tData.tState == TableState.waitCard))
                        {
                            var tingSet = {};
                            if (pl.tingListAfterPut) 
                            {
            ...
            _event: {
                ...
                initSceneData: function(eD) {
                    SetUserVisible_pingjiangzhaniao(this, 0);
                    var pl = getUIPlayer(0);
                    if (pl.isBBHu) 
                    {
                        MjClient.playui.EatBBHuVisibleCheck();
                    }
                ...
                mjhand: function(eD) {
                    if (showShaiziAni_pingjiangzhaniao)
                        showShaiziAni_pingjiangzhaniao(0);
                    else
                        InitUserHandUI_pingjiangzhaniao(this, 0);
                },
                roundEnd: function() {
                    var self = this;
                    var parent_node = this.getParent();
                    function InitUserCoinAndNameafterdelay()
                    {
                        InitUserCoinAndName(self, 0);
                    }
                    parent_node.runAction(cc.sequence(cc.DelayTime(5),cc.callFunc(InitUserCoinAndNameafterdelay)));
                ...
                sendKaiGangCard: function(eD) {
                    cc.log("  开杠      ")
                    MjClient.playui.DealKaiGang(this, eD, 0,true);
                    MjClient.playui.showKaiGangBigCards(eD.uid);
                    var pl = getUIPlayer(0);
                    if (eD.uid == SelfUid() && MjClient.data.sData.tData.areaSelectMode.isOpenTingTip)
                    {
                        var _tingCards = this.getChildByName("tingCardsNode");
                        var tingSet = {};
                        cc.log("听牌数据   ",JSON.stringify(pl.tingListAfterPut));
                        if (pl.tingListAfterPut) 
                        {
                            tingSet = pl.tingListAfterPut;
                        }
                ...
                sendKaiGangCard2:function(eD)
                {
                    cc.log("CLEAN ------------------ ",MjClient.CanClean);
                    MjClient.playui.DealKaiGang(this, eD, 0,false);
                    var pl = getUIPlayer(0);
                    if (eD.uid == SelfUid() && MjClient.data.sData.tData.areaSelectMode.isOpenTingTip)
                    {
                        var _tingCards = this.getChildByName("tingCardsNode");
                    ...
                    /*MjClient.playui.cleanKaiGangBigCards();
                    MjClient.playui.refreshKaiGangBigCards();*/
                    MjClient.playui.showKaiGangBigCards(eD.uid);
                },
                MJBBhu: function(eD) {
                    cc.log("  板板胡      ")
                    showShaiziAni_pingjiangzhaniao_dami(0);
                    MjClient.playui.DealDaMi(this,eD,0);
                    if (eD.uid == SelfUid()) 
                    {
                        MjClient.playui.hideAllEat();
                    }
                ...
                doQiShouHu: function(eD) {
                    ...
                    if(eD.isNoQsHu) {
                        ...
                        MjClient.playui.ShowQiShouHuBtn()
                    }
                },
                waitBanbanhu:function(ed)
                {
                    var pl = getUIPlayer(0);
                    if (pl.isBBHu) 
                    {
                    ...
                    if (eD.uid == SelfUid() && MjClient.data.sData.tData.areaSelectMode.isOpenTingTip)
                    {
                        ...
                        if(eD.uid == SelfUid()){
                            if (this.getChildByName("AutoPut_btn")) {
                                ...
                                if (Object.keys(tingSet).length > 0 && !pl.isTing) 
                                {
                                    var AutoPut_btn = AddAutoPutCheckBox();
                                    this.addChild(AutoPut_btn,999);
                                    AutoPut_btn.setSelected(pl.tPutCard);
                ...
                MJChi: function(eD) {
                    ...
                    //MjClient.playui.cleanKaiGangBigCards();
                    MjClient.playui.showKaiGangBigCards();
                    updateHeadUI_pingjiangzhaniao(this, 0 , ActionType.CHI);
                    setUserOffline(this, 0);
                ...
                MJPeng: function(eD) {
                    ...
                    if (MjClient.rePlayVideo != -1) {
                        ...
                        updateHeadUI_pingjiangzhaniao(this, 0, ActionType.PENG);
                    }
                    setUserOffline(this, 0);
                    MjClient.playui.EatVisibleCheck();
                    if (MjClient.data.sData.tData.curPlayer == getPlayerIndex(0))
...
/*                    //弹窗选择是否飘分
                    var img1 = "playing/other/bupiaofen.png";
                    var img2 = "playing/other/piaofen.png";
                    var layer = new jiaZhuComLayer(3, img1, img2, function(){
                        //弹窗等待
                        MjClient.playui._jiazhuWait.visible = true;
                    });
                    MjClient.playui.addChild(layer, 99);
                ...
                MJSelectHaiDiLao: function(msg) {
                    ...
                    var pl = getUIPlayer(0);
                    pl.haiDiLaoState = msg.haiDiLaoState;
                },
                MJPass:function(eD)
                {
                    if (eD.touchCard && eD.uid == SelfUid() && IsTurnToMe()) 
                    {
...
/*                        waitHaiDiLao: function()
            ...
            _event: {
                ...
                initSceneData: function(eD) {
                    SetUserVisible_pingjiangzhaniao(this, 1);
                },
                addPlayer: function(eD) {
                    SetUserVisible_pingjiangzhaniao(this, 1);
                },
                ...
                mjhand: function(eD) {
                    if (!showShaiziAni_pingjiangzhaniao)
                        InitUserHandUI_pingjiangzhaniao(this, 1);
                },
                roundEnd: function() {
                    var parent_node = this.getParent();
                    var self = this;
                ...
                sendKaiGangCard: function(eD) {
                    MjClient.playui.DealKaiGang(this, eD, 1,true);
                },
                sendKaiGangCard2: function(eD) {
                    MjClient.playui.DealKaiGang(this, eD, 1,false);
                },   
                MJBBhu: function(eD) {
                    cc.log("  板板胡      ")
                    MjClient.playui.DealDaMi(this,eD,1);
                },         
                doQiShouHu: function(eD) {
                ...
                MJChi: function(eD) {
                    if (MjClient.rePlayVideo != -1)
                        ...
                        updateHeadUI_pingjiangzhaniao(this, 1, ActionType.CHI);
                    setUserOffline(this, 1);
                },
                MJGang: function(eD) {
                ...
                MJPeng: function(eD) {
                    if (MjClient.rePlayVideo != -1)
                        ...
                        updateHeadUI_pingjiangzhaniao(this, 1, ActionType.PENG);
                    setUserOffline(this, 1);
                },
                MJHu: function(eD) {
...
/*                        waitHaiDiLao: function()
            ...
            out0: {
                _run: function () {
                    ...
                    {
                        setWgtLayout(this, [0, 0.07],[0.5, 1],[6.8, -2.3]);
                    }
                    ...
                    if (MjClient.MaxPlayerNum == 2) 
                    {
                        if (MjClient.size.width / MjClient.size.height >= 1.5)
                        {
                            setWgtLayout(this, [0, 0.0763],[0.63, 1],[6.8, -2.3]);
                        }
                        else
                        {
                            setWgtLayout(this, [0, 0.07],[0.63, 1],[6.8, -2.3]);
                        }
                    }
            ...
            out1: {
                _run: function () {
                    ...
                    {
                        setWgtLayout(this, [0, 0.07],[0.5, 1],[6.8, -3.35]);
                    }
                    ...
                    if (MjClient.MaxPlayerNum == 2) 
                    {
                        if (MjClient.size.width / MjClient.size.height >= 1.5)
                        {
                            setWgtLayout(this, [0, 0.0763],[0.63, 1],[6.8, -3.35]);
                        }
                        else
                        {
                            setWgtLayout(this, [0, 0.07],[0.63, 1],[6.8, -3.35]);
                        }
                    }
                },
            ...
            _event: {
                ...
                initSceneData: function(eD) {
                    SetUserVisible_pingjiangzhaniao(this, 2);
                },
                addPlayer: function(eD) {
                    SetUserVisible_pingjiangzhaniao(this, 2);
                },
                ...
                mjhand: function(eD) {
                    if (!showShaiziAni_pingjiangzhaniao)
                        InitUserHandUI_pingjiangzhaniao(this, 2);
                },
                roundEnd: function() {
                    var parent_node = this.getParent();
                    var self = this;
                ...
                sendKaiGangCard: function(eD) {
                    MjClient.playui.DealKaiGang(this, eD, 2,true);
                },
                sendKaiGangCard2: function(eD) {
                    MjClient.playui.DealKaiGang(this, eD, 2,false);
                },
                MJBBhu: function(eD) {
                    cc.log("  板板胡      ")
                    MjClient.playui.DealDaMi(this,eD,2);
                },
                doQiShouHu: function(eD) {
                ...
                MJChi: function(eD) {
                    if (MjClient.rePlayVideo != -1)
                        ...
                        updateHeadUI_pingjiangzhaniao(this, 2, ActionType.CHI);
                    setUserOffline(this, 2);
                },
                MJGang: function(eD) {
                ...
                MJPeng: function(eD) {
                    if (MjClient.rePlayVideo != -1)
                        ...
                        updateHeadUI_pingjiangzhaniao(this, 2, ActionType.PENG);
                    setUserOffline(this, 2);
                },
                MJHu: function(eD) {
...
/*                        waitHaiDiLao: function()
            ...
            out1: {
                _run: function () {
                    if (MjClient.size.width / MjClient.size.height >= 1.5)
                    {
                        setWgtLayout(this, [0, 0.0545],[0, 0.5],[5.7 - 0.075, 4.8]);
                    }
                    else
                    {
                        setWgtLayout(this, [0, 0.05],[0, 0.5],[5.7 - 0.075, 4.8]);
                    }
                    if (MjClient.MaxPlayerNum == 3)
                        this.y += this.height * this.scale * 2;
            ...
            _event: {
                ...
                initSceneData: function(eD) {
                    SetUserVisible_pingjiangzhaniao(this, 3);
                },
                addPlayer: function(eD) {
                    SetUserVisible_pingjiangzhaniao(this, 3);
                },
                ...
                mjhand: function(eD) {
                    if (!showShaiziAni_pingjiangzhaniao)
                        InitUserHandUI_pingjiangzhaniao(this, 3);
                },
                roundEnd: function() {
                    var parent_node = this.getParent();
                    var self = this;
                ...
                sendKaiGangCard: function(eD) {
                    MjClient.playui.DealKaiGang(this, eD, 3,true);
                },
                sendKaiGangCard2: function(eD) {
                    MjClient.playui.DealKaiGang(this, eD, 3,false);
                },
                MJBBhu: function(eD) {
                    cc.log("  板板胡      ")
                    MjClient.playui.DealDaMi(this,eD,3);
                },
                doQiShouHu: function(eD) {
                ...
                MJChi: function(eD) {
                    if (MjClient.rePlayVideo != -1)
                        ...
                        updateHeadUI_pingjiangzhaniao(this, 3, ActionType.CHI);
                    setUserOffline(this, 3);
                },
                MJGang: function(eD) {
                ...
                MJPeng: function(eD) {
                    if (MjClient.rePlayVideo != -1)
                        ...
                        updateHeadUI_pingjiangzhaniao(this, 3, ActionType.PENG);
                    setUserOffline(this, 3);
                },
                MJHu: function(eD) {
        ...
        eat: {
            outBig: {
                ...
                _event:
                {
                    ...
                    {
                        changeMJBg(this,getCurrentMJBgType());
                        cc.log("changeMJBg" , this.getRenderFile().file);
                    }
                }
            },
            niaopai: {
                ...
                _event: 
                {
                    LeaveGame: function(ed) {
                        this.visible = false;
                        setWgtLayout(this,[0.13, 0.13], [0.5, 0.5], [0, 0.4]);
                    },
                    initSceneData: function(ed) {
                        this.visible = false;
                        setWgtLayout(this,[0.13, 0.13], [0.5, 0.5], [0, 0.4]);
            ...
            chi0: {
                ...
                _touch: function(btn, eT) {
                    ...
                    {
                        if (MjClient.playui.jsBind.eat.hu._node.visible) {
                            MjClient.showMsg("确认 不胡 吗?",function()
                            {
                                if (MjClient.chiCards.length >= 1)
                                    MjClient.playui.MJChiCardchange_kaiGang();
                                else
            ...
            ting: {
                ...
                _touch: function(btn, eT) {
                    if (eT == 2)
                    {
                        ...
                        var tingCards_self = {};
                        if (pl.jiangjianghuLists && pl.jiangjianghuLists[currentCard]) 
                        {
                            var curtingset = pl.jiangjianghuLists[currentCard];
                        ...
                        if (MjClient.playui.jsBind.eat.hu._node.visible) {
                            MjClient.showMsg("确认 不胡 吗?",function()
                            {
                                if (MjClient.pengCards.length > 1)
                                    MjClient.playui.MJPengCardchange_kaiGang();
            ...
            hu: {
                ...
                _touch: function(btn, eT) {
                    if (eT == 2) 
                    {
                        ...
                        {
                            if (MjClient.huCards.length > 1)//杠开胡两张 直接胡
            ...
            changeui: {
                ...
                changeuibg: {
                    ...
                    guobg: {
                        guo: {
                            _touch: function(btn, eT) {
                                if (eT == 2) MjClient.MJPass2NetForpingjiangzhaniao();
                            }
                        },
                        fanhui: {
                            _touch: function(btn, et) {
            ...
            _event: {
                ...
                sendKaiGangCard: function(eD) {
                    ...
                    MjClient.playui.EatKaiGangVisibleCheck();
                },
                sendKaiGangCard2: function(eD) {
                    console.log("HHH :sendKaiGangCard2------");
                    MjClient.playui.EatKaiGangVisibleCheck();
                },
                MJPut: function(eD) {
                ...
                roundEnd: function(eD) {
                    ...
                    MjClient.playui.hideAllEat();
                    var tData = MjClient.data.sData.tData;
                    var delayTime = tData.mopai.length > 0 ? 1 : 0;
                    this.runAction(cc.sequence(cc.delayTime(3 * delayTime),cc.callFunc(function(){
                        MjClient.playui.showShaiziAni_pingjiangzhaniao_nor(0);
                    })));
                },
    ...
    ctor: function() {
        ...
        var playui = ccs.load("Play_pingjiangzhaniao.json");
        ...
        MjClient.playui._chiTiShi = playui.node.getChildByName("chi_tishi");
        ...
        if(pl && (pl.mjState == TableState.isReady || pl.mjState == TableState.roundFinish))
        {
            ...
            if(pl != null)
            {
                if (pl.isTing) {
                    ...
                    if (Object.keys(pl.jiangjianghuLists).length > 0) 
                    {
                        var _jiangIcon1 = getNode(off).getChildByName("head").getChildByName("tingIcon");
...
PlayLayer_pingjiangzhaniao.prototype.CardLayoutRestore = function(node, off, isNewCard)
{
    ...
    for(var i = 0; i < orders.length; i++)
    {
        ...
        if(off % 2 == 0)//自己或者对家
        {
            ...
            else
            {
                ...
                if (ci.name == "mjhand" && (pl.isTing || MjClient.clickTing && !MjClient.JJHcanTingCards[ci.tag]))
                    ci.setColor(cc.color(190, 190, 190));
                else
            ...
            if(off == 0)
            {
                ...
                if(MjClient.clickTing)
                {
                    if (ci.name == "mjhand")
                    {
                        if(MjClient.JJHcanTingCards[ci.tag])
                        {
                            ci.setColor(cc.color(255, 255, 255));
                            if (!hasUp) {
                                ci.y += 20;
...
PlayLayer_pingjiangzhaniao.prototype.willHuShowArrow = function()
{
    var tData = MjClient.data.sData.tData;
...
// 提示等待起手胡
PlayLayer_pingjiangzhaniao.prototype.ShowQiShouHuTip = function()
{
    var eat = MjClient.playui.jsBind.eat;
    var sData = MjClient.data.sData;
    var pl = sData.players[SelfUid()];
    ...
    if(pl && pl.qiShouHu && Object.keys(pl.qiShouHu).length >0 ){
        waitOtherQsHu = false;
    }
...
PlayLayer_pingjiangzhaniao.prototype.ShowQiShouHuBtn = function()
{
    var eat = MjClient.playui.jsBind.eat;
    var sData = MjClient.data.sData;
    var pl = sData.players[SelfUid()];
    ...
    if(!qshu_node.qshuBtnArr) qshu_node.qshuBtnArr = []

    var clearnUI = function(){
        for(var i in qshu_node.qshuBtnArr){
            if(cc.sys.isObjectValid(qshu_node.qshuBtnArr[i]))
                qshu_node.qshuBtnArr[i].removeFromParent();
        }
        qshu_node.qshuBtnArr = []
    }

    if(pl && pl.qiShouHu && Object.keys(pl.qiShouHu).length >0){
        this.hideAllEat();
        qshu_node.setVisible(true);

        var btnCount = 0;
        // for(var i=0; i < 7; i++){
        for(var qsHuName in pl.qiShouHu){
            // var qsHuName = 'liuliushun';
            var imgPath = "playing/gameTable/qs_" + qsHuName + ".png";
            var qshuBtn = qshu_node.getChildByName('qshu_base').clone();
            var pscale = qshuBtn.getScale()
            var psize = qshuBtn.getSize()
            qshuBtn.loadTextureNormal(imgPath);
            qshuBtn.setVisible(true);
            qshuBtn.setPositionX( qshuBtn.getPositionX() - (btnCount%2)*psize.width*pscale - 20);
            qshuBtn.setPositionY( qshuBtn.getPositionY() + Math.floor(btnCount/2)*psize.height*pscale);
            qshuBtn.addTouchEventListener(function(btn,eT){
                if(eT==2) {
                    MJQsHuToServer(btn.getName());   
                    clearnUI();
                }
            }, qshuBtn);
            qshuBtn.setName(qsHuName);
            qshu_node.addChild(qshuBtn, 1, qsHuName);
            qshu_node.qshuBtnArr.push(qshuBtn);
            isShowBtn = true;
            btnCount++;
        }

        var cancel_qs = qshu_node.getChildByName('cancel_qs');
        cancel_qs.setVisible(true);
        cancel_qs.addTouchEventListener(function(btn,eT){
            if(eT==2) {
                qshu_node.setVisible(false);   
                MJPassQsHuToServer();
                clearnUI();
            }
        }.bind(this), cancel_qs);
...
PlayLayer_pingjiangzhaniao.prototype.EatKaiGangVisibleCheck = function() 
{
    ...
    if ((eatFlag & 4) && pl.kaigangList.length > 0) {
    ...
    if (eatFlag & 2) {
        ...
        if (MjClient.pengCards.indexOf(lastPutCard) == -1)
            MjClient.pengCards.push(lastPutCard);
    }

    if (eatFlag & 1) {
        var eatpos = pl.canEat || [];
        if (eatpos.length > 0) {
    ...
    if (pl.haiDiLaoState == 1 )
    {
        ...
        if (vnode.indexOf(eat.hu._node) != -1)
        {
            ...
            cc.log("自动胡  ",pl.haiDiLaoState);
            //eat.hu._node.runAction(cc.sequence(cc.delayTime(0.8), cc.callFunc(MJHuToServer))).tag = 1;
            eat.hu._node.runAction(cc.sequence(cc.delayTime(0.8), cc.callFunc(function(){
                MjClient.majiang.MJHuToServer(tData.lastPutCard);
            }))).tag = 1;
        }
    ...
    for (var i = 0; i < vnode.length; i++) {
        ...
        if(i == 0)
        {
            ...
            if(cardVal && cardVal > 0)
            {
                ...
                vnode[0].getChildByName("card1").visible = false;
            }
        ...
        //eat.hu._node.visible = true;
        var space = isIPad() ? 1.1 : 1.4;
        setWgtLayout(vnode[i], [0, 0.16], [0.70, 0], [(i - vnode.length + 1) * space, 1.8], false, false);
    }
}
PlayLayer_pingjiangzhaniao.prototype.EatBBHuVisibleCheck = function(tData, vnode, eat, pl) 
{
    var eat = MjClient.playui.jsBind.eat;
    var sData = MjClient.data.sData;
    var tData = sData.tData;
    ...
    for (var i = rtn.length - 1; i >= 0; i--) {
        if (pl.mjchi && pl.mjchi.length > 0 && Math.floor(pl.mjchi[0] / 10) != Math.floor(rtn[i] / 10)) {
            rtn.splice(i, 1); // 吃牌之后不能杠
        }
    }
...
PlayLayer_pingjiangzhaniao.prototype.showKaiGangBigCards = function(uid) {
    var tData = MjClient.data.sData.tData;
    ...
    if (!kaigangsArry || kaigangsArry.length <= 0)
        return;
    var pl = getUIPlayer(0);
    ...
    for (var i = 0; i < kaigangsArry.length; i++) {
        var newKaiGangbox = kaiGangKuang.clone();
        ...
        for (var j = 0; j < curcards.length; j++) {
            if (j <= kaigangsArry[i].fanIndex) 
            {
                ...
                setCardSprite(newCardNode, curcards[j], 0);
        ...
        /*var cardNode1 = newKaiGangbox.getChildByName("card1");
        setCardSprite(cardNode1, kaigangsArry[i].card1, 0);
        var cardNode2 = newKaiGangbox.getChildByName("card2");
        if (kaigangsArry[i].fan2) 
        {
            cardNode2.loadTexture(cardNode1.getRenderFile().file);
            setCardSprite(cardNode2, kaigangsArry[i].card2, 0);
        }*/
        node.addChild(newKaiGangbox);
        MjClient.AllKaiGangBox.push(newKaiGangbox);
    }
    if (pl.eatFlag == 8 && typeof(uid) != "undefined" && uid == pl.info.uid)
    {
...
PlayLayer_pingjiangzhaniao.prototype.refreshKaiGangBigCards = function() {
    /*var tData = MjClient.data.sData.tData;
    var kaigangsArry = MjClient.AllKaiGangBox;
    var kaigangs = tData.kaigangs;

    if (kaigangsArry.length <= 0 || kaigangs.length <= 0)
        return;
    
    var kaiGangKuang = kaigangsArry[kaigangsArry.length - 1];
    var node = kaiGangKuang.getParent();
    var cardNode1 = kaiGangKuang.getChildByName("card1");
    var cardNode2 = kaiGangKuang.getChildByName("card2");
    cardNode2.loadTexture(cardNode1.getRenderFile().file);
    cc.log("第二张牌,",kaigangs[kaigangs.length -1].card2);
    setCardSprite(cardNode1, kaigangs[kaigangs.length -1].card1, 0);   
    setCardSprite(cardNode2, kaigangs[kaigangs.length -1].card2, 0);   
    MjClient.CanClean = true;*/
}

PlayLayer_pingjiangzhaniao.prototype.cleanKaiGangBigCards = function() {
/*    if (ClickBu) 
    {
        MjClient.CanClean = false;
        ClickBu = false;
    }
    if (!MjClient.CanClean) 
    {
        return;
    }
    var tData = MjClient.data.sData.tData;
    var kaigangsArry = MjClient.AllKaiGangBox;
    var kaigangs = tData.kaigangs;
    cc.log("剩余开杠框cln",kaigangsArry.length,tData.kaigangs);
    if (kaigangsArry.length <= 0)
    {
        MjClient.CanClean = false;
        return;
    }
    ...
    var kaiGangKuang = kaigangsArry[kaigangsArry.length - 1];
    kaiGangKuang.removeFromParent();
    kaigangsArry.pop();
    MjClient.CanClean = false;*/
}

PlayLayer_pingjiangzhaniao.prototype.showhaidipai = function(haidiCard)
{
    var tData = MjClient.data.sData.tData;
    ...
    var haidipaikuang = MjClient.playui.jsBind.eat.haidipaikuang._node;
    haidipaikuang.setVisible(true);
    var card = haidipaikuang.getChildByName("card");
    card.setVisible(true);
    setCardSprite(card,haidiCard,0);

    if (tData.winner == -1)
     {
        var huangzhuangsign = new ccui.ImageView();
        huangzhuangsign.loadTexture("playing/gameTable/huangzhuang.png");
        huangzhuangsign.setName("huangzhuangsign");
        //tingsign.setScale(0);
        huangzhuangsign.setPosition(haidipaikuang.getContentSize().width/2,haidipaikuang.getContentSize().height + huangzhuangsign.getContentSize().height/2 +7);
        haidipaikuang.addChild(huangzhuangsign,20); 
     }

}
PlayLayer_pingjiangzhaniao.prototype.showzhuaniao = function(showCards)
{
    cc.log("addzhuaniao");
    var niaopai = MjClient.playui.jsBind.eat.niaopai._node;

    niaopai.setVisible(true);
    setCardSprite(niaopai,showCards[0],0);
    //判断哪个玩家中鸟
    var indexoff = -1;
    for (var i = 0; i < 4; i++) {
        cc.log("i============"+i);
...
PlayLayer_pingjiangzhaniao.prototype.hideAllEat = function(){
    var eat = MjClient.playui.jsBind.eat;
    ...
    eat.haidipaikuang._node.visible = false;
    MjClient.playui._chiTiShi.visible = false;
    cc.log("hideAllEat---------------------")
}

// 判断吃碰杠胡的状态
PlayLayer_pingjiangzhaniao.prototype.EatVisibleCheck = function()
{
...
/*    if (tData.kaigangs &&tData.kaigangs.length > 0) 
    ...
    if (pl.isBBHu && (tData.tState == TableState.waitEat || tData.tState == TableState.waitPut)) 
    {
        return this.EatBBHuVisibleCheck();
    }
    cc.log("  tData.isKaigang ",tData.isKaigang ," tData.kaigangs ",tData.kaigangs);
    if (tData.kaigangs && tData.kaigangs.length > 0 && tData.isKaigang)    //开杠
    {
        return this.EatKaiGangVisibleCheck();
    }

    cc.log("当前玩家   ",tData.curPlayer,pl.mjState);
    ...
    if(tData.tState == TableState.waitPut && pl.mjState == TableState.waitPut)
    {
        if(IsTurnToMe())
        {
            ...
            MjClient.canTingCards = {};
            MjClient.JJHcanTingCards = {};
            if (pl.tingLists ) 
            {
                for (var i in pl.tingLists) {
                    MjClient.canTingCards[i] = 1;
/*                    if (vnode.indexOf(eat.ting._node) < 0 && pl.needTing) {
                        vnode.push(eat.ting._node);
                    }*/
                }
            }

            if (pl.jiangjianghuLists && !pl.isTing) 
            {
                for (var i in pl.jiangjianghuLists) {
                    MjClient.JJHcanTingCards[i] = 1;
            ...
            if(rtn.length > 0 && !pl.isPass)
            {
                for (var i = 0; i < rtn.length; i ++)
                {
                    if (pl.kaigangList.length > 0)
                    {
                        MjClient.kaiGangCards = pl.kaigangList;
                        if (vnode.indexOf(eat.gang1._node) == -1) {
                            vnode.push(eat.gang1._node);
                        }
                    }
                    ...
                    // if (pl.mjpeng.indexOf(rtn[i]) == -1) { // 暗杠
                    //     if (!pl.isTing) { // 开杠之后不能补
                    //         MjClient.gangCards.push(rtn[i]);
                    //         if (vnode.indexOf(eat.gang0._node) == -1) {
                    //             vnode.push(eat.gang0._node); 
                    //         }
                    //     }
                    // }
                    // else if (pl.isNew && !pl.isTing) { // 补
                    //     if (pl.mjhand[pl.mjhand.length - 1] == rtn[i]) {
                    //         MjClient.gangCards.push(rtn[i]);
                    //         if (vnode.indexOf(eat.gang0._node) == -1) {
                    //             vnode.push(eat.gang0._node); 
                    //         }
    ...
    {
        // 
        cc.log("diao pao hu-=================================================",tData.tState);
        if(!IsTurnToMe())
        {
            cc.log("diao pao hu-=================================================");
            if (pl.eatFlag & 8) {
            ...
            if (pl.eatFlag & 4) {
                // if (MjClient.majiang.canKaiGang(pl.mjhand, tData.lastPutCard, false, null, false, tData)) {
                //     vnode.push(eat.gang1._node);
            ...
            if(pl.eatFlag & 1)
            {
                ...
                // if(pl.mjchi.length > 0){//初次吃需要提示
                //     MjClient.playui._chiTiShi.visible = false;
                // }else{
                //     MjClient.playui._chiTiShi.visible = false;//7.02  删除【吃后只能胡清一色】贴图
                // }
    ...
    if(vnode.length > 0)
    {
        ...
        for(var i = 0; i < vnode.length; i++)
        {
            ...
            if(btnName == "peng")
            {
                cc.log("peng  lastPutCard   ",cardVal);
                cardVal = tData.lastPutCard;
            }
            else if(btnName == "chi0")
            {
            ...
            if(cardVal && cardVal > 0)
            {
                setCardSprite(vnode[i].getChildByName("card1"), cardVal, 0);
                vnode[i].getChildByName("card1").visible = true;
                vnode[i].card = cardVal;
            }

            if(vnode[i].getChildByName("bgground"))
            {
                vnode[i].getChildByName("bgground").zIndex = -1;
                vnode[i].getChildByName("bgground").visible = true;
            }
...
PlayLayer_pingjiangzhaniao.prototype.RemoveNewCardOut = function(node, card)
{
...
PlayLayer_pingjiangzhaniao.prototype.DealDaMi = function(node, msg, off)
{
    cc.log("======DealDaMi======= " + off);
    //cardPutted = false;
    var sData = MjClient.data.sData;
    ...
    var cards = msg.showCards;
    if(COMMON_UI3D.is3DUI())
    {
        for (var ii = 0; ii < cards.length; ii ++)
        {
            var msg = {card:cards[ii],uid:msg.uid};
            var outnum = -1;
            if (ii == 0) 
            {
                outnum = cards[ii+1] != -1 ? pl.mjput.length - 2 : pl.mjput.length - 1;
            }
            else
            {
                outnum = pl.mjput.length - 1;
            }
            COMMON_UI3D.DealMJPut_3D(node, msg, off,outnum);
        }
        return;
    }
    ...
    for (var ii = 0; ii < 2; ii ++)
    {
        ...
        if (ii == 0)
        {
            putnum = cards[ii+1] != -1 ? pl.mjput.length - 2 : pl.mjput.length - 1;
            card = cards[ii];
        }
        else 
        {
            cc.log("--msg.card2=" + msg.card2);
            if (cards[ii] == -1)
                break;

            putnum = pl.mjput.length - 1;
            card = cards[ii];
        }
        ...
        {
            endPoint.y = out.y;
            endPoint.x = out.x + oSize.width * arg * oSc * putnum*0.91;
        }
        else if (off == 1)
        {
            cc.log("DealMJPut remove card  = " + card);
            endPoint.y = (out.y + oSize.height * arg * oSc) + oSize.height * arg * oSc * putnum * 0.7;
            endPoint.x = out.x;
    ...
    function RemovePutCard(visibleOut, card)
    {
        ...
        for (var i = 0; i <= outActionList.length - 1; i ++)
        {
            ...
            else
            {
                ...
                {
                    outActionList[i + 1].setPosition(outActionList[i].getPosition());
                    outActionList[i + 1].out.setPosition(outActionList[i].out.getPosition());
                    outActionList[i + 1].zIndex = outActionList[i].zIndex;
                    outActionList[i + 1].out.zIndex = outActionList[i].out.zIndex;
                }
...
PlayLayer_pingjiangzhaniao.prototype.DealKaiGang = function(node, msg, off,isfrist)
{
    ...
    //3D模式下的特殊UI by sking 2018.9.27
    var cards = [];
    cards = [tData.lastPutCard];

    //var cards = [msg.card1,msg.card2];
    if(COMMON_UI3D.is3DUI())
    {
        for (var ii = 0; ii < cards.length; ii ++)
        {
            var msg = {card:cards[ii],uid:msg.uid};
            COMMON_UI3D.DealMJPut_3D(node, msg, off,pl.mjput.length - cards.length + ii);
        }
        return;
    }
    ...
    //for (var ii = 0; ii < 2; ii ++)
    {
        ...
        /*if (ii == 0)
        {
            putnum = msg.card2 != -1 ? pl.mjput.length - 2 : pl.mjput.length - 1;
            card = msg.card1;
        }
        else 
        {
        ...
        {
            cc.log("DealMJPut remove card  = " + card);
            endPoint.y = out.y + oSize.height * arg * oSc * putnum * 0.7;
            endPoint.x = out.x;
            out.zIndex = 100 - putnum;
        }
        else if(off == 2)
    ...
    function RemovePutCard(visibleOut, card)
    {
        ...
        //for (var i = 0; i <= outActionList.length - 1; i ++)
        {
            var isSame = outAction.tag == card;
            if ((visibleOut != null && visibleOut) || (visibleOut == null && (a || !isSame)))
            {
                outAction.out.visible = true;
                //outActionList[i].out.zIndex = outActionList[i].zIndex;
            }
            /*else
            {
...
PlayLayer_pingjiangzhaniao.prototype.MJChiCardchange_kaiGang = function(){
...
PlayLayer_pingjiangzhaniao.prototype.MJPengCardchange_kaiGang = function() {
...
PlayLayer_pingjiangzhaniao.prototype.showShaiziAni_pingjiangzhaniao_nor = function(poff)
{
    if(poff == 0) {
