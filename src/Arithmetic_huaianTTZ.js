...
(function() {
    ...
    //是否可以杠
    majiang.canGang1 = function(peng, hand, isTing, skipGang)
    {
        var rtn = [];
        for(var i = 0; i < peng.length; i++)
        {
            // 跳过杠的牌不能再杠
            if(hand.indexOf(peng[i]) >= 0 && skipGang.indexOf(peng[i]) < 0 && (!isTing || majiang.canGangWhenTing(hand, peng[i])))
            {
                //手牌里的牌Push到rtn中
                rtn.push(peng[i]);
            }
    ...
    majiang.getAllCardsTotal = function ()
    {
        ...
        if (tData.areaSelectMode.flowerType == WithFlowerType.flower4)
        {
            return 112;
        }
        return 116;
    ...
    {
        MjClient.majiang_huaianTTZ = majiang;
    }
    else
    {
