//邵阳永州扯胡子小结算面板
function addWxHeadToEndUI_shaoYangLuoDiSao(node,off){
...
function reInitarrCardVisible_shaoYangLuoDiSao()
{
...
//添加手牌
function addWinMjHand_shaoYangLuoDiSao(_node,off){
...
var EndOneView_shaoYangLuoDiSao = cc.Layer.extend({
    _block:null,
	jsBind:{
		block:{
		    _run:function () {
		...
		parent:{
			...
			readyBtn:{
				...
				_click:function(btn,eT){
					...
					reInitarrCardVisible_shaoYangLuoDiSao();
				}
			},
			switchImg:{
				_run: function(){
			...
			balance:{
				...
				tableLabel:{
					...
					_text:function(){
						...
						var tData=sData.tData;
						var text = "落地扫,";
						var wangArray = ["无王,","单王,","双王,","三王,"];
						text += wangArray[tData.areaSelectMode.kingNum];
				   		...
				   		for(var index = 0;index<playerCount;index++){
                            ...
                            if(tData.areaSelectMode.scoreNeedEnough == 1){
                                if(pl.winone < 0 && pl.winone < pl.winone2){
                                    ...
                                    //bg.set//ContentSize(cc.size(110, 30));
						    ...
						    addWxHeadToEndUI_shaoYangLuoDiSao(cloneHead,index);
				   		}
				   	},
				},
				showInfo:{
			...
			main:{
				...
				leftPanel:{
					_run:function(){
						...
						addWinMjHand_shaoYangLuoDiSao(this,MjClient.MaxPlayerNum_paohuzi - 1);
				...
				rightPanel:{
					_run:function(){
						...
						addWinMjHand_shaoYangLuoDiSao(this,index);
						if(MjClient.rePlayVideo != -1){
	                    	this.visible = false;
	                    }
					}
				...
				xingPanel:{
                    _run:function(){
                    	if(MjClient.MaxPlayerNum_paohuzi != 4){
                    		...
                    		addWinMjHand_shaoYangLuoDiSao(this,1);
                    	}
                    	if(MjClient.rePlayVideo != -1){
	                    	this.visible = false;
	                    }
