...
function SetEndOneUserUI_doudizhuLF(node, off) {
    ...
    var tiNode=node.getChildByName("ti");
    tiNode.setVisible(MjClient.playui.jiazhuNum[off] > 0);
    tiNode.zIndex=10;
    //add by sking
        ...
        head: {
            ...
            stand: {
                ...
                _run: function () {
                    ...
                    CardLayoutRestoreForEndOne_doudizhuLF(node, pl);
                }
            },
            standDi:
                {
                    ...
                    _run: function () {
                        ...
                        {
                            var _cards    = MjClient.playui.diCards;
                            var arry = [];
                            //添加手牌
                            for (var i = 0; i < _cards.length; i++) {
                                arry.push(getNewCard_card(node, "standDi", "mjhand", _cards[i], 0));
                            }
                            var standDiNode = node.getChildByName("standDi");
                            var start;
                            start = standDiNode;
                            var upSize = standDiNode.getSize();
                            var upS = standDiNode.scale;
                            for (var i = 0; i < arry.length; i++) {
                                arry[i].visible = true;
                                arry[i].enabled = false;
                                arry[i].setScale(arry[i].getScale() * 0.75);
                                if(i>0)
                                    arry[i].x = arry[i - 1].x + upSize.width * upS * 0.4;//调牌的距离的
                            }
                        }
                    }
                },
...
function CardLayoutRestoreForEndOne_doudizhuLF(node, plNode) {
...
var EndOneView_doudizhuLF = cc.Layer.extend({
    jsBind: {
        block: {
        ...
        back: {
            ...
            dir_0: {
                ...
                _text: function () {
                    ...
                    text += tData.areaSelectMode.scoreType==0 ? "翻番," : "加分,";
                    if (tData.areaSelectMode.zhafengding == 0){
                        text += "不封顶,";
                    }else{
                        text +=  tData.areaSelectMode.zhafengding   + "炸封顶,";
                    }
                    if (tData.areaSelectMode.sidaisi) {
                        text += "四带两对,";
                    }
                    if (tData.areaSelectMode.zhaDanSanGeSan) {
                        text += "3个3算两炸,";
                    }

                    text += (tData.areaSelectMode.daiti && tData.areaSelectMode.ti_byOrder)? "按顺序踢," : "";

                    text += (tData.areaSelectMode.daiti && tData.areaSelectMode.zhafengding != 0)? "踢计入封顶" : "";
            ...
            head0: {
                ...
                _run: function () { SetEndOneUserUI_doudizhuLF(this, 0); },
    		...
    		, head1: {
    		    ...
    		    _run: function () { SetEndOneUserUI_doudizhuLF(this, 1); }
    		}
    		...
    		, head2: {
    		    ...
    		    _run: function () { SetEndOneUserUI_doudizhuLF(this, 2); }
    		}
            , head3: {
                head: {
                    ti:{_visible:false}
                ...
                _run: function () { SetEndOneUserUI_doudizhuLF(this, 3); }
            }
        }
    },
    ctor: function (endInfo) {
        this._super();
        var endoneui = ccs.load("endOne_doudizhuLF.json");
        MjClient.playui.diCards = endInfo.diCards;
        MjClient.playui.zhuang = endInfo.zhuang;
        MjClient.playui.jiazhuNum = endInfo.jiazhuNum;
