function SetGameOverLayer_fulushou(node,off,isNewUi){
    ...
    var uibind={
        ...
        listView:{
            _run:function(){
                ...
                var nameText = ["胡牌次数","门清次数","滚滚","碰碰胡","杠上花","海底捞","杠上炮"];

                var describeMsg = pl.roomStatistics;  
                // cc.log("fffffffffffffff = ",listView.children.length);
                ...
                for (var i = 0; i < describeMsg.length; i++) {
                    ...
                    var cloneItem = item.clone();
                    this.pushBackCustomItem(cloneItem);

                    var itemName = cloneItem.getChildByName("title");
                    itemName.setString(nameText[i]);
                    itemName.ignoreContentAdaptWithSize(true);
                    var scoreText = cloneItem.getChildByName("score");
...
/*
    福禄寿大结算
 */
var GameOverLayer_FuLuShou = cc.Layer.extend({
    sprite:null,
    jsBind:{
        back:{
            share:{
    ...
    ctor:function () {
        ...
        var endallui = ccs.load("endAll_FuLuShou.json");
        ...
        if(playerCount == 2) {
            off_x = (infoBgSize.width - 2*bgSize.width)/3;
            startPos = cc.p(bgSize.width/2 + off_x,infoBgSize.height/2);
        }else {
            off_x = (infoBgSize.width - playerCount*bgSize.width)/(playerCount-1);
            startPos = cc.p(bgSize.width/2,infoBgSize.height/2);
        }
        ...
        for(var i = 0 ; i < playerCount ; i++){
            ...
            SetGameOverLayer_fulushou(clone,i, isNewUi);

            if(playerCount == 2) {
                startPos = cc.p(startPos.x + bgSize.width/2 + bgSize.width /2+ off_x,startPos.y);
            } else {
