...
var PlayLayer_paohuzi_ER = cc.Layer.extend({
    jsBind: {
        _event: {
        ...
        roundInfo:{
            ...
            _run:function(){
                ...
                str += tData.areaSelectMode.minHu ? tData.areaSelectMode.minHu + "胡起胡," : "";
        ...
        left: {
            ...
            //     _run:function()
            //     {
            //         setWgtLayout(this, [0.30, 0.30],[0.27,0.75],[0,0]);
            //         var userData = {scale:this.getScale(), pos:this.getPosition()};
            //         this.setUserData(userData);
            //     }
            // },
...
//重置手牌的顺序
PlayLayer_paohuzi_ER.prototype.ResetHandCard = function(posNode,off,needAction,isDelay){
...
PlayLayer_paohuzi_ER.prototype._doMovetoAction = function(node,endP){
    node.stopAllActions();
    var ac = cc.moveTo(0.3,endP);
    node.runAction(ac);
};

PlayLayer_paohuzi_ER.prototype.ResetEatCardsEnd = function(posNode,off){
    var pl = getUIPlayer_paohuzi(off);
    if (!pl) return;
    //提、偎牌的特殊处理
...
PlayLayer_paohuzi_ER.prototype.ResetOtherCard = function(posNode,off){
    //添加吃、碰、跑、偎、提
    var sData = MjClient.data.sData;
    var tData = null;
    if(sData){
...
PlayLayer_paohuzi_ER.prototype.ResetPutCard = function(posNode,off){
    //添加打出的牌
    var pl = getUIPlayer_paohuzi(off);
    if (!pl) return;
...
//重置牌的顺序
PlayLayer_paohuzi_ER.prototype.CardLayoutRestore = function(posNode,off){
    if(MjClient.rePlayVideo == -1){
...
// 判断吃、碰、跑、偎、提、胡的状态
PlayLayer_paohuzi_ER.prototype.EatVisibleCheck = function(off){
    var eat = MjClient.playui.jsBind.eat;
    var sData = MjClient.data.sData;
    var tData = sData.tData;
