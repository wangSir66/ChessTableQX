...
function SetUserVisible_liuHuQiang(node, off){
    ...
    if(pl){
        ...
        setUserOffline_liuHuQiang(node, off);
...
function InitUserHandUI_liuHuQiang(node, off){
    ...
    if (pl.jiazhuNum == 2 && MjClient.rePlayVideo == -1)
    {
        ...
        MjClient.majiang.setJiaZhuNum(node, getUIPlayer_liuHuQiang(off));
    }
...
var getRightNodeOff_ER = function(){
    var tData = MjClient.data.sData.tData;
    var off = 1;
    if(tData.maxPlayer == 2){
        off = 1;
    }else{
        off = tData.maxPlayer - 2;
    }
    return off;
}

var PlayLayer_liuHuQiang = cc.Layer.extend({
    jsBind: {
        _event: {
            ...
            endRoom: function(msg) {
                ...
                if (msg.showEnd){
                    this.addChild(new GameOverLayer_liuHuQiang(),500);
                }else{
            ...
            roundEnd: function() {
                ...
                if(!MjClient.isDismiss && tData.hunCard && tData.hunCard != -1 && tData.areaSelectMode.xingType==2){
                    ...
                    ShowEatActionAnim_liuHuQiang(MjClient.playui._downNode, ActionType_paohuzi.FANXING, 0);
                    DealFanXingNewCard_liuHuQiang(hunCard);
                }
                ...
                function delayExe(){
                    ...
                    if (sData.tData.roundNum <= 0){
                        var layer = new GameOverLayer_liuHuQiang();
                        layer.setVisible(false);
                        self.addChild(layer,500);
                    }
                    if(!MjClient.endoneui){
                        self.addChild(new EndOneView_liuHuQiang(),500);//EndOneView_syZiPai
                    }
            ...
            moveHead: function() {
                ...
                //tableStartHeadMoveAction_liuHuQiang(this);   //不涉及到头像移动动作
            },
            initSceneData: function() {
                ...
                reConectHeadLayout_liuHuQiang(this);        //不涉及到头像移动动作
                CheckRoomUiDelete();
                tableStartHeadMoveAction_liuHuQiang(this);   //不涉及到头像移动动作
            ...
            changeMJBgEvent: function() {
                changeMJBg_liuHuQiang(this, ziPai.getZiPaiType());
            },
            MJPeng: function() {
                MjClient.playui.showAndHideHeadEffect();
            },
            ...
            EZP_layout : function(){
                if(MjClient.data.sData.tData.maxPlayer < 4){
                    ...
                    MjClient.playui.ResetPutCard(MjClient.playui._rightNode, MjClient.MaxPlayerNum_liuHuQiang - 2);
                    MjClient.playui.ResetPutCard(MjClient.playui._topNode, MjClient.MaxPlayerNum_liuHuQiang - 1);
                }
        ...
        cardNumImg: {
            ...
            _event: {
                initSceneData: function(eD) {
                    this.visible = IsArrowVisible_liuHuQiang();
                },
                mjhand: function(eD) {
                    this.visible = IsArrowVisible_liuHuQiang();
                },
        ...
        roundInfo:{
            ...
            _run:function(){
                if(MjClient.getAppType() == MjClient.APP_TYPE.YLHUNANMJ || MjClient.getAppType() == MjClient.APP_TYPE.QXYYQP) {
                    ...
                    var tData = MjClient.data.sData.tData;
                    var str = "";
                    //str += (tData.areaSelectMode.xiNum == 6) ? "6息," : (tData.areaSelectMode.xiNum == 9) ? "9息," : "10息," ;
                    str += tData.areaSelectMode.xiNum + "息";
                    str += (tData.areaSelectMode.suanfenType == 0) ? "3息1囤(1底分)," : (tData.areaSelectMode.suanfenType == 1) ? "3息1囤(2底分)," : "1息1囤,";
                    str += (tData.areaSelectMode.isMingwei) ? "明偎," : "" ;
                    ...
                    str += (tData.areaSelectMode.is21Zhang) ? "21张," : "" ;
                    str += (tData.areaSelectMode.bihuType == 0) ? "有胡必胡," : tData.areaSelectMode.bihuType == 1 ? "点炮必胡," : "" ;
                    str += (tData.areaSelectMode.xingType == 0) ? "不带醒" : (tData.areaSelectMode.xingType == 2) ? "翻醒" : "随醒" ;
                    str += tData.areaSelectMode.diFen > 0 ? ",小于" + tData.areaSelectMode.diFen + "分翻倍" : ",不翻倍";
        ...
        banner: {
            ...
            getRoomNum: {
                ...
                _click: function() {
                    ...
                    var sData = MjClient.data.sData;
                    var str7 = "  二缺一";
                    if(MjClient.MaxPlayerNum_liuHuQiang == 3){
                        if((MjClient.MaxPlayerNum_liuHuQiang - Object.keys(sData.players).length) == 2){
                            str7 = "  一缺二";
                        }
                    }else if(MjClient.MaxPlayerNum_liuHuQiang == 4){
                        if((MjClient.MaxPlayerNum_liuHuQiang - Object.keys(sData.players).length) == 3){
                            str7 = "  一缺三";
                        }else if((MjClient.MaxPlayerNum_liuHuQiang - Object.keys(sData.players).length) == 2){
                            str7 = "二缺二";
                        }else  if((MjClient.MaxPlayerNum_liuHuQiang - Object.keys(sData.players).length) == 1){
                            str7 = "三缺一";
                        }
                    }
        ...
        wait: {
            ...
            delroom: {
                ...
                // _event: {
                //     initSceneData: function(eD) {
                //         this.visible = isShowBackBtn_liuHuQiang();
                //     },
                //     removePlayer: function(eD) {
                //         this.visible = isShowBackBtn_liuHuQiang();
                //     },
        ...
        down: {
            head: {
                zhuang: {
                    ...
                    _event: {
                        waitPut: function(){
                            showUserZhuangLogo_liuHuQiang(this, 0);
                        },
                        initSceneData: function(){
                            if (IsArrowVisible_liuHuQiang()) showUserZhuangLogo_liuHuQiang(this, 0);
                        },
                        mjhand: function(){
                            if (IsArrowVisible_liuHuQiang()) showUserZhuangLogo_liuHuQiang(this, 0);
                        }
                    ...
                    chattext: {
                        _event: {
                            ...
                            MJChat: function(msg) {
                                ...
                                showUserChat_liuHuQiang(this, 0, msg);
                            },
                            playVoice: function(voicePath) {
                                MjClient.data._tempMessage.msg = voicePath;
                                showUserChat_liuHuQiang(this, 0, MjClient.data._tempMessage);
                            }
                        }
                    }
                },
                _click: function(btn) {
                    showPlayerInfo_liuHuQiang(0, btn);
                },
                _event: {
                    loadWxHead: function(d) {
                        setWxHead_liuHuQiang(this, d, 0);
                    },
                    addPlayer: function(eD) {
                        showFangzhuTagIcon_liuHuQiang(this,0);
                    },
                    removePlayer: function(eD) {
                        showFangzhuTagIcon_liuHuQiang(this,0);
                    }
                ...
                huxi:{
                    ...
                    _event:{
                        ...
                        initSceneData:function(){
                            this.visible = true;
                            var huXi = UpdateHuXi_liuHuQiang(0);
                            if(!IsArrowVisible_liuHuQiang()){
                                huXi = 0;
                                this.visible = false;
                            }
                ...
                timeImg:{
                    ...
                    // _event:{
                    //     initSceneData: function(eD) {
                    //         this.visible = curPlayerIsMe_liuHuQiang(0);
                    //     },
                    //     mjhand: function(eD) {
                    //         this.visible = curPlayerIsMe_liuHuQiang(0);
                    //     },
                    //     onlinePlayer: function(eD) {
                    //         this.visible = curPlayerIsMe_liuHuQiang(0) && IsArrowVisible_liuHuQiang();
                    //     },
                    //     waitPut: function(eD) {
                    //         this.visible = curPlayerIsMe_liuHuQiang(0);
                    //     },
                    ...
                    //             if(pl && pl.onLine && curPlayerIsMe_liuHuQiang(0)){
                    //                 arrowbkNumberUpdate(this);
                    //             }
                    //         },
                    //         roundEnd: function() {
            ...
            ready: {
                ...
                _run: function() {
                    GetReadyVisible_liuHuQiang(this, 0);
                },
                _event: {
                    startShuffleCards:function (d) {
                        GetReadyVisible_liuHuQiang(this, -1);
                    },
                    moveHead: function() {
                        GetReadyVisible_liuHuQiang(this, -1);
                    },
                    addPlayer: function() {
                        GetReadyVisible_liuHuQiang(this, 0);//根据状态设置ready 是否可见 add by sking
                    },
                    removePlayer: function() {
                        this.visible = false;
                        // GetReadyVisible_liuHuQiang(this, 0);
            ...
            handNode: {
                ...
                _event:  {
                    initSceneData: function() {
                        calculateHintPutList_liuHuQiang();
                    },
                    HZPickCard: function() {
                        calculateHintPutList_liuHuQiang();
                    },
                    ...
                    MJPass: function() { // 天胡 提 偎 跑后过胡
                        calculateHintPutList_liuHuQiang();
                        addTingSign_liuHuQiang(MjClient.playui._downNode);
                    },
            ...
            _event: {
                ...
                initSceneData: function(eD) {
                    SetUserVisible_liuHuQiang(this, 0);
                    InitUserHandUI_liuHuQiang(this, 0);
                    InitUserCoinAndName_liuHuQiang(this, 0);
                    MjClient.hasPut = false; // 重连回来重置打掉牌标志（打牌时候断线)
                    ShowPutCardIcon();
                    DealOffLineCard_liuHuQiang(this,0);
                },
                addPlayer: function(eD) {
                    SetUserVisible_liuHuQiang(this, 0);
                    InitUserCoinAndName_liuHuQiang(this, 0);
                ...
                mjhand: function(eD) {
                    ...
                    var uid = getUIPlayer_liuHuQiang(0).info.uid;
                    if (eD.jiazhuNums)
                    {
                ...
                HZNewCard: function(eD) {
                    ...
                    DealNewCard_liuHuQiang(this,eD,0);
                },
                MJPut: function(eD) {
                    DealPutCard_liuHuQiang(this,eD,0);
                },
                HZChiCard: function(eD) {
                    DealChiCard_liuHuQiang(this, eD, 0);
                },
                HZWeiCard: function(eD){
                    DealWeiCard_liuHuQiang(this,eD,0);
                },
                HZGangCard: function(eD) {
                    DealGangCard_liuHuQiang(this, eD, 0);
                },
                MJPeng: function(eD) {
                    DealPengCard_liuHuQiang(this, eD, 0);
                },
                MJHu: function(eD) {
                    DealHu_liuHuQiang(this, eD, 0);
                },
                onlinePlayer: function(eD) {
                    resetHandAfterBegin_liuHuQiang();
                    setUserOffline_liuHuQiang(this, 0);
                },
                ...
                MJJiazhu: function(msg) {
                    ...
                    MjClient.majiang.setJiaZhuNum(MjClient.playui._downNode, getUIPlayer_liuHuQiang(0));
                    MjClient.majiang.setJiaZhuNum(MjClient.playui._rightNode, getUIPlayer_liuHuQiang(1));
                    MjClient.majiang.setJiaZhuNum(MjClient.playui._topNode, getUIPlayer_liuHuQiang(2));
                },
                ...
                HZAddCards:function (eD) {
                    DealAddCard_liuHuQiang(this,eD, 0);
                },
        ...
        xing: {
            _run:function () {
                if(MjClient.MaxPlayerNum_liuHuQiang == 3 || MjClient.MaxPlayerNum_liuHuQiang == 2){
                    this.setVisible(false);
                }
            },
            head: {
                zhuang: {
                    ...
                    _event: {
                        waitPut: function() {
                            if(MjClient.MaxPlayerNum_liuHuQiang == 4) {
                                showUserZhuangLogo_liuHuQiang(this, 1);
                            }
                        },
                        initSceneData: function() {
                            if(MjClient.MaxPlayerNum_liuHuQiang == 4) {
                                if (IsArrowVisible_liuHuQiang()) showUserZhuangLogo_liuHuQiang(this, 1);
                            }
                        },
                        mjhand: function(){
                            if(MjClient.MaxPlayerNum_liuHuQiang == 4) {
                ...
                chatbg: {
                    ...
                    chattext: {
                        _event: {
                            MJChat: function(msg) {
                                if(MjClient.MaxPlayerNum_liuHuQiang == 4) {
                                    showUserChat_liuHuQiang(this, 1, msg);
                                }
                            },
                            playVoice: function(voicePath) {
                                if(MjClient.MaxPlayerNum_liuHuQiang == 4) {
                                    MjClient.data._tempMessage.msg = voicePath;
                                    showUserChat_liuHuQiang(this, 1, MjClient.data._tempMessage);
                                }
                            }
                        }
                    }
                ...
                _click: function(btn) {
                    if(MjClient.MaxPlayerNum_liuHuQiang == 4) {
                        showPlayerInfo_liuHuQiang(1, btn);
                    }
                },
                _event: {
                    loadWxHead: function(d) {
                        if(MjClient.MaxPlayerNum_liuHuQiang == 4) {
                            setWxHead_liuHuQiang(this, d, 1);
                        }
                    },
                    addPlayer: function(eD) {
                        if(MjClient.MaxPlayerNum_liuHuQiang == 4) {
                            showFangzhuTagIcon_liuHuQiang(this, 1);
                        }
                    },
                    removePlayer: function(eD) {
                        if(MjClient.MaxPlayerNum_liuHuQiang == 4) {
                ...
                huxi:{
                    ...
                    _event:{
                        ...
                        initSceneData:function(){
                            if(MjClient.MaxPlayerNum_liuHuQiang == 4) {
                                this.visible = true;
                                var huXi = UpdateHuXi_liuHuQiang(1);
                                if (!IsArrowVisible_liuHuQiang()) {
                                    huXi = 0;
                                    this.visible = false;
                                }
                ...
                timeImg:{
                    ...
                    //     initSceneData: function(eD) {
                    //         if(MjClient.MaxPlayerNum_liuHuQiang == 4) {
                    //             this.visible = curPlayerIsMe_liuHuQiang(1);
                    //         }
                    //     },
                    //     mjhand: function(eD) {
                    //         if(MjClient.MaxPlayerNum_liuHuQiang == 4) {
                    ...
                    //     onlinePlayer: function(eD) {
                    //         if(MjClient.MaxPlayerNum_liuHuQiang == 4) {
                    //             this.visible = curPlayerIsMe_liuHuQiang(1) && IsArrowVisible_liuHuQiang();
                    //         }
                    //     },
                    //     waitPut: function(eD) {
                    //         if(MjClient.MaxPlayerNum_liuHuQiang == 4) {
                    ...
                    //                 stopEffect(playTimeUpEff);
                    //                 playTimeUpEff = null;
                    //                 var pl = getUIPlayer_liuHuQiang(1);
                    //                 if (pl && pl.onLine && curPlayerIsMe_liuHuQiang(1)) {
                    //                     arrowbkNumberUpdate(this);
                    //                 }
                    //             }
                    //         },
            ...
            _event: {
                ...
                initSceneData: function(eD) {
                    if(MjClient.MaxPlayerNum_liuHuQiang == 4) {
                        SetUserVisible_liuHuQiang(this, 1);
                        InitUserHandUI_liuHuQiang(this, 1);
                        InitUserCoinAndName_liuHuQiang(this, 1);
                        DealOffLineCard_liuHuQiang(this, 1);
                    }
                },
                addPlayer: function(eD) {
                    if(MjClient.MaxPlayerNum_liuHuQiang == 4) {
                ...
                HZNewCard: function(eD) {
                    if(MjClient.MaxPlayerNum_liuHuQiang == 4) {
                        ...
                        DealNewCard_liuHuQiang(this, eD, 1);
                    }
                },
                roundEnd: function() {
                    if(MjClient.MaxPlayerNum_liuHuQiang == 4) {
                ...
                MJPut: function(eD) {
                    if(MjClient.MaxPlayerNum_liuHuQiang == 4) {
                        DealPutCard_liuHuQiang(this, eD, 1);
                    }
                },
                HZChiCard: function(eD) {
                    if(MjClient.MaxPlayerNum_liuHuQiang == 4) {
                        DealChiCard_liuHuQiang(this, eD, 1);
                    }
                },
                HZGangCard: function(eD) {
                    if(MjClient.MaxPlayerNum_liuHuQiang == 4) {
                        DealGangCard_liuHuQiang(this, eD, 1);
                    }
                },
                MJPeng: function(eD) {
                    if(MjClient.MaxPlayerNum_liuHuQiang == 4) {
                        DealPengCard_liuHuQiang(this, eD, 1);
                    }
                },
                HZWeiCard: function(eD){
                    if(MjClient.MaxPlayerNum_liuHuQiang == 4) {
                        DealWeiCard_liuHuQiang(this, eD, 1);
                    }
                },
                MJHu: function(eD) {
                    if(MjClient.MaxPlayerNum_liuHuQiang == 4) {
                        DealHu_liuHuQiang(this, eD, 1);
                    }
                },
                onlinePlayer: function(eD) {
                    if(MjClient.MaxPlayerNum_liuHuQiang == 4) {
                        setUserOffline_liuHuQiang(this, 1);
                    }
                },
                playerStatusChange: function(eD) {
                    if(MjClient.MaxPlayerNum_liuHuQiang == 4) {
                ...
                HZAddCards:function (eD) {
                    if(MjClient.MaxPlayerNum_liuHuQiang == 4) {
                        DealAddCard_liuHuQiang(this, eD, 1);
                    }
                },
                HZCheckRaise:function (msg) {
                    if(MjClient.MaxPlayerNum_liuHuQiang == 4) {
        ...
        right: {
            ...
            head: {
                zhuang: {
                    ...
                    _event: {
                        waitPut: function() {
                            showUserZhuangLogo_liuHuQiang(this, getRightNodeOff_ER());
                        },
                        initSceneData: function() {
                            if (IsArrowVisible_liuHuQiang()) showUserZhuangLogo_liuHuQiang(this, getRightNodeOff_ER());
                        },
                        mjhand: function(){
                            if (IsArrowVisible_liuHuQiang()) showUserZhuangLogo_liuHuQiang(this, getRightNodeOff_ER());
                        }
                ...
                chatbg: {
                    ...
                    chattext: {
                        _event: {
                            MJChat: function(msg) {
                                showUserChat_liuHuQiang(this, getRightNodeOff_ER(), msg);
                            },
                            playVoice: function(voicePath) {
                                MjClient.data._tempMessage.msg = voicePath;
                                showUserChat_liuHuQiang(this, getRightNodeOff_ER(), MjClient.data._tempMessage);
                            }
                        }
                    }
                },
                _click: function(btn) {
                    showPlayerInfo_liuHuQiang(getRightNodeOff_ER(), btn);
                },
                _event: {
                    loadWxHead: function(d) {
                        setWxHead_liuHuQiang(this, d, getRightNodeOff_ER());
                    },
                    addPlayer: function(eD) {
                        showFangzhuTagIcon_liuHuQiang(this,getRightNodeOff_ER());
                    },
                    removePlayer: function(eD) {
                        showFangzhuTagIcon_liuHuQiang(this,getRightNodeOff_ER());
                    }
                ...
                huxi:{
                    ...
                    _event:{
                        ...
                        initSceneData:function(){
                            this.visible = true;
                            var huXi = UpdateHuXi_liuHuQiang(getRightNodeOff_ER());
                            if(!IsArrowVisible_liuHuQiang()){
                                huXi = 0;
                                this.visible = false;
                            }
                ...
                timeImg:{
                    ...
                    // _event:{
                    //     initSceneData: function(eD) {
                    //         this.visible = curPlayerIsMe_liuHuQiang(getRightNodeOff_ER());
                    //     },
                    //     mjhand: function(eD) {
                    //         this.visible = curPlayerIsMe_liuHuQiang(getRightNodeOff_ER());
                    //     },
                    //     onlinePlayer: function(eD) {
                    //         this.visible = curPlayerIsMe_liuHuQiang(getRightNodeOff_ER()) && IsArrowVisible_liuHuQiang();
                    //     },
                    //     waitPut: function(eD) {
                    //         this.visible = curPlayerIsMe_liuHuQiang(getRightNodeOff_ER());
                    //     },
                    ...
                    //             stopEffect(playTimeUpEff);
                    //             playTimeUpEff = null;
                    //             if(curPlayerIsMe_liuHuQiang(getRightNodeOff_ER())){
                    //                 arrowbkNumberUpdate(this);
                    //                 this.setString("00");
                    //             }
                    //         },
                    ...
                    //             stopEffect(playTimeUpEff);
                    //             playTimeUpEff = null;
                    //             var pl = getUIPlayer_liuHuQiang(getRightNodeOff_ER());
                    //             if(pl && pl.onLine && curPlayerIsMe_liuHuQiang(getRightNodeOff_ER())){
                    //                 arrowbkNumberUpdate(this);
                    //             }
                    //         },
                    //         roundEnd: function() {
            ...
            ready: {
                _layout: [[0.07, 0.07],[0.5, 0.5],[2.5, 2.5]],
                _run: function() {
                    GetReadyVisible_liuHuQiang(this, getRightNodeOff_ER());
                },
                _event: {
                    startShuffleCards:function (d) {
                        GetReadyVisible_liuHuQiang(this, -1);
            ...
            _event: {
                clearCardUI: function() {
                    clearCardUI_liuHuQiang(this, getRightNodeOff_ER());
                },
                clearCardArr: function(){
                    var eatNode = this.getChildByName("eatNode");
                    eatNode.removeAllChildren();
                ...
                initSceneData: function(eD) {
                    SetUserVisible_liuHuQiang(this, getRightNodeOff_ER());
                    InitUserHandUI_liuHuQiang(this, getRightNodeOff_ER());
                    InitUserCoinAndName_liuHuQiang(this, getRightNodeOff_ER());
                    DealOffLineCard_liuHuQiang(this,getRightNodeOff_ER());
                },
                addPlayer: function(eD) {
                    SetUserVisible_liuHuQiang(this, getRightNodeOff_ER());
                    InitUserCoinAndName_liuHuQiang(this, getRightNodeOff_ER());
                ...
                HZNewCard: function(eD) {
                    ...
                    DealNewCard_liuHuQiang(this,eD,getRightNodeOff_ER());
                },
                roundEnd: function() {
                    InitUserCoinAndName_liuHuQiang(this, getRightNodeOff_ER());
                    MjClient.playui.ResetOtherCard(this, getRightNodeOff_ER());
                },
                waitPut: function(eD) {
                    DealWaitPut(this, eD, getRightNodeOff_ER());
                },
                MJPut: function(eD) {
                    DealPutCard_liuHuQiang(this, eD, getRightNodeOff_ER());
                },
                HZChiCard: function(eD) {
                    DealChiCard_liuHuQiang(this, eD, getRightNodeOff_ER());
                },
                HZGangCard: function(eD) {
                    DealGangCard_liuHuQiang(this, eD, getRightNodeOff_ER());
                },
                MJPeng: function(eD) {
                    DealPengCard_liuHuQiang(this, eD, getRightNodeOff_ER());
                },
                HZWeiCard: function(eD){
                    DealWeiCard_liuHuQiang(this, eD, getRightNodeOff_ER());
                },
                MJHu: function(eD) {
                    DealHu_liuHuQiang(this, eD, getRightNodeOff_ER());
                },
                onlinePlayer: function(eD) {
                    setUserOffline_liuHuQiang(this, getRightNodeOff_ER());
                },
                playerStatusChange: function(eD) {
                    setUserOffline_liuHuQiang(this, getRightNodeOff_ER());
                },
                MJFlower: function(eD) {
                    HandleMJFlower(this, eD, getRightNodeOff_ER());
                },
                MJTing: function (eD) {
                    HandleMJTing(this, eD, getRightNodeOff_ER());
                },
                waitJiazhu:function (msg) {
                    postEvent("returnPlayerLayer");
                    /*
                ...
                HZPickCard:function (eD) {
                    if(MjClient.rePlayVideo != -1){
                        var pl = getUIPlayer_liuHuQiang(getRightNodeOff_ER());
                        if(eD.uid == pl.info.uid) {
                            MjClient.OtherHandArr[getRightNodeOff_ER()] = MjClient.majiang.sortHandCardSpecial(pl.mjhand)
                        }
                    }

                    RemovePutCardOut_liuHuQiang(true);
                    MjClient.playui.ResetHandCard(this,getRightNodeOff_ER());
                },
                HZAddCards:function (eD) {
                    DealAddCard_liuHuQiang(this,eD, getRightNodeOff_ER());
                },
                HZCheckRaise:function (msg) {
                    ShowPutCardIcon();
                    //MjClient.playui.EatVisibleCheck();
        ...
        left: {
            head: {
                zhuang: {
                    ...
                    _event: {
                        waitPut: function() {
                            showUserZhuangLogo_liuHuQiang(this, MjClient.MaxPlayerNum_liuHuQiang - 1);
                        },
                        initSceneData: function() {
                            if (IsArrowVisible_liuHuQiang()) showUserZhuangLogo_liuHuQiang(this, MjClient.MaxPlayerNum_liuHuQiang - 1);
                        },
                        mjhand: function(){
                            if (IsArrowVisible_liuHuQiang()) showUserZhuangLogo_liuHuQiang(this, MjClient.MaxPlayerNum_liuHuQiang - 1);
                        }
                ...
                chatbg: {
                    ...
                    chattext: {
                        _event: {
                            MJChat: function(msg) {
                                showUserChat_liuHuQiang(this, MjClient.MaxPlayerNum_liuHuQiang - 1, msg);
                            },
                            playVoice: function(voicePath) {
                                MjClient.data._tempMessage.msg = voicePath;
                                showUserChat_liuHuQiang(this, MjClient.MaxPlayerNum_liuHuQiang - 1, MjClient.data._tempMessage);
                            }
                        }
                    }
                },
                _click: function(btn) {
                    showPlayerInfo_liuHuQiang(MjClient.MaxPlayerNum_liuHuQiang - 1, btn);
                },
                _event: {
                    loadWxHead: function(d) {
                        setWxHead_liuHuQiang(this, d, MjClient.MaxPlayerNum_liuHuQiang - 1);
                    },
                    addPlayer: function(eD) {
                        showFangzhuTagIcon_liuHuQiang(this,MjClient.MaxPlayerNum_liuHuQiang - 1);
                    },
                    removePlayer: function(eD) {
                        showFangzhuTagIcon_liuHuQiang(this,MjClient.MaxPlayerNum_liuHuQiang - 1);
                    }
                ...
                huxi:{
                    ...
                    _event:{
                        ...
                        initSceneData:function(){
                            this.visible = true;
                            var huXi = UpdateHuXi_liuHuQiang(MjClient.MaxPlayerNum_liuHuQiang - 1);
                            if(!IsArrowVisible_liuHuQiang()){
                                huXi = 0;
                                this.visible = false;
                            }
                ...
                timeImg:{
                    ...
                    // _event:{
                    //     initSceneData: function(eD) {
                    //         this.visible = curPlayerIsMe_liuHuQiang(MjClient.MaxPlayerNum_liuHuQiang - 1);
                    //     },
                    //     mjhand: function(eD) {
                    //         this.visible = curPlayerIsMe_liuHuQiang(MjClient.MaxPlayerNum_liuHuQiang - 1);
                    //     },
                    //     onlinePlayer: function(eD) {
                    //         this.visible = curPlayerIsMe_liuHuQiang(MjClient.MaxPlayerNum_liuHuQiang - 1) && IsArrowVisible_liuHuQiang();
                    //     },
                    //     waitPut: function(eD) {
                    //         this.visible = curPlayerIsMe_liuHuQiang(MjClient.MaxPlayerNum_liuHuQiang - 1);
                    //     },
                    ...
                    //             stopEffect(playTimeUpEff);
                    //             playTimeUpEff = null;
                    //             if(curPlayerIsMe_liuHuQiang(MjClient.MaxPlayerNum_liuHuQiang - 1)){
                    //                 arrowbkNumberUpdate(this);
                    //                 this.setString("00");
                    //             }
                    //         },
                    ...
                    //             stopEffect(playTimeUpEff);
                    //             playTimeUpEff = null;
                    //             var pl = getUIPlayer_liuHuQiang(MjClient.MaxPlayerNum_liuHuQiang - 1);
                    //             if(pl && pl.onLine && curPlayerIsMe_liuHuQiang(MjClient.MaxPlayerNum_liuHuQiang - 1)){
                    //                 arrowbkNumberUpdate(this);
                    //             }
                    //         },
                    //         roundEnd: function() {
            ...
            ready: {
                _layout: [[0.07, 0.07],[0.5, 0.5],[-2.5, 2.5]],
                _run: function() {
                    GetReadyVisible_liuHuQiang(this, MjClient.MaxPlayerNum_liuHuQiang - 1);
                },
                _event: {
                    startShuffleCards:function (d) {
                        GetReadyVisible_liuHuQiang(this, -1);
            ...
            _event: {
                clearCardUI: function() {
                    clearCardUI_liuHuQiang(this, MjClient.MaxPlayerNum_liuHuQiang - 1);
                },
                clearCardArr: function(){
                    var eatNode = this.getChildByName("eatNode");
                    eatNode.removeAllChildren();
                ...
                initSceneData: function(eD) {
                    SetUserVisible_liuHuQiang(this, MjClient.MaxPlayerNum_liuHuQiang - 1);
                    InitUserHandUI_liuHuQiang(this, MjClient.MaxPlayerNum_liuHuQiang - 1);
                    InitUserCoinAndName_liuHuQiang(this, MjClient.MaxPlayerNum_liuHuQiang - 1);
                    DealOffLineCard_liuHuQiang(this,MjClient.MaxPlayerNum_liuHuQiang - 1);
                },
                addPlayer: function(eD) {
                    SetUserVisible_liuHuQiang(this, MjClient.MaxPlayerNum_liuHuQiang - 1);
                    InitUserCoinAndName_liuHuQiang(this, MjClient.MaxPlayerNum_liuHuQiang - 1);
                ...
                HZNewCard: function(eD) {
                    ...
                    DealNewCard_liuHuQiang(this,eD,MjClient.MaxPlayerNum_liuHuQiang - 1);
                },
                roundEnd: function() {
                    InitUserCoinAndName_liuHuQiang(this, MjClient.MaxPlayerNum_liuHuQiang - 1);
                    MjClient.playui.ResetOtherCard(this, MjClient.MaxPlayerNum_liuHuQiang - 1);
                },
                waitPut: function(eD) {
                    DealWaitPut(this, eD, MjClient.MaxPlayerNum_liuHuQiang - 1);
                },
                MJPut: function(eD) {
                    DealPutCard_liuHuQiang(this, eD, MjClient.MaxPlayerNum_liuHuQiang - 1);
                },
                HZChiCard: function(eD) {
                    DealChiCard_liuHuQiang(this, eD, MjClient.MaxPlayerNum_liuHuQiang - 1);
                },
                HZPaoCard: function(eD) {
                    DealPaoCard_liuHuQiang(this, eD, MjClient.MaxPlayerNum_liuHuQiang - 1);
                },
                HZGangCard: function(eD) {
                    DealGangCard_liuHuQiang(this, eD, MjClient.MaxPlayerNum_liuHuQiang - 1);
                },
                MJPeng: function(eD) {
                    DealPengCard_liuHuQiang(this, eD, MjClient.MaxPlayerNum_liuHuQiang - 1);
                },
                HZWeiCard: function(eD){
                    DealWeiCard_liuHuQiang(this, eD, MjClient.MaxPlayerNum_liuHuQiang - 1);
                },
                MJHu: function(eD) {
                    DealHu_liuHuQiang(this, eD, MjClient.MaxPlayerNum_liuHuQiang - 1);
                },
                onlinePlayer: function(eD) {
                    setUserOffline_liuHuQiang(this, MjClient.MaxPlayerNum_liuHuQiang - 1);
                },
                playerStatusChange: function(eD) {
                    setUserOffline_liuHuQiang(this, MjClient.MaxPlayerNum_liuHuQiang - 1);
                },
                MJFlower: function(eD) {
                    HandleMJFlower(this, eD, MjClient.MaxPlayerNum_liuHuQiang - 1);
                },
                MJTing: function (eD) {
                    HandleMJTing(this, eD, MjClient.MaxPlayerNum_liuHuQiang - 1);
                },
                HZPickCard:function (eD) {
                    if(MjClient.rePlayVideo != -1) {
                        var pl = getUIPlayer_liuHuQiang(MjClient.MaxPlayerNum_liuHuQiang - 1);
                        if(eD.uid == pl.info.uid){
                            MjClient.OtherHandArr[MjClient.MaxPlayerNum_liuHuQiang - 1] = MjClient.majiang.sortHandCardSpecial(pl.mjhand)
                        }
                    }
                    RemovePutCardOut_liuHuQiang(true);
                    MjClient.playui.ResetHandCard(this,MjClient.MaxPlayerNum_liuHuQiang - 1);
                },
                HZAddCards:function (eD) {
                    DealAddCard_liuHuQiang(this,eD, MjClient.MaxPlayerNum_liuHuQiang - 1);
                },
                HZCheckRaise:function (msg) {
                    ShowPutCardIcon();
                    //MjClient.playui.EatVisibleCheck();
        ...
        eat: {
                         ...
                         if(ziPai.getHasFastEat()){
                            ...
                            if (chiSet.length == 1) {
                                ...
                                if (pl.mjhand.indexOf(putCard) < 0 || copy.indexOf(putCard) >= 0) {
                                    commitEatCards_liuHuQiang(eatCards, null);
                                    return;
                                }
                            }
                        }
                        showSelectEatCards_liuHuQiang(this,btn.tag);
                    }
                }
            },
            noTing : {
            ...
            hu: {
                ...
                _touch: function(btn, eT) {
                    if (eT == 2) {
                        MJHuToServer_liuHuQiang();
                        setChiVisible_liuHuQiang();//add by maoyu
                    }
                }
            },
            cancel: {
            ...
            wangDiao:{
                ...
                _touch: function(btn, eT) {
                    console.log(">>>> lf，点击碰按钮");
                    if (eT == 2) {
                        HZWangChuangToServer_liuHuQiang(1);
                        setChiVisible_liuHuQiang();//add by maoyu
                    }
                }
            },
            wangChuang:{
                ...
                _touch: function(btn, eT) {
                    console.log(">>>> lf，点击碰按钮");
                    if (eT == 2) {
                        HZWangChuangToServer_liuHuQiang(2);
                        setChiVisible_liuHuQiang();//add by maoyu
                    }
                }
            },
            wangZha:{
                ...
                _touch: function(btn, eT) {
                    console.log(">>>> lf，点击碰按钮");
                    if (eT == 2) {
                        HZWangChuangToServer_liuHuQiang(4);
                        setChiVisible_liuHuQiang();//add by maoyu
                    }
                }
            },
        ...
        voice_btn: {
            ...
            _event: {
                ...
                downAndPlayVoice: function(msg) {
                    ...
                    downAndPlayVoice_liuHuQiang(msg.uid, msg.msg);
                }
            }
        },
        cutLine:{
    ...
    ctor: function() {
        this._super();
        ziPai.initZiPaiData();
        var playui = ccs.load("Play_liuHuQiang.json");
        playMusic("bgFight");
        var tData = MjClient.data.sData.tData;
        MjClient.MaxPlayerNum_liuHuQiang = tData.maxPlayer;
        cc.log(MjClient.MaxPlayerNum_liuHuQiang);
        ...
        //三人玩时恢复
            ...
            this.jsBind.right = PlayLayer_liuHuQiang.jsBindLeftCopy;
        }
        ...
        this.tingLayer = new LiuHuQiangPhzTingLayer();
    ...
    resetJiaZhuTip: function()
    {
        ...
        for (var i = 0; i < MjClient.MaxPlayerNum_liuHuQiang; i ++)
        {
...
PlayLayer_liuHuQiang.jsBindLeftCopy = PlayLayer_liuHuQiang.prototype.jsBind.right;
PlayLayer_liuHuQiang.prototype.ResetHandCard = function(posNode,off, needAction, isDelay){
    if(off == 0){
        resetHandCard_liuHuQiangEx(posNode,off, needAction, isDelay);
    }else{
        resetHandCard_liuHuQiang(posNode,off, needAction, isDelay);
    }
...
PlayLayer_liuHuQiang.prototype.ResetHandCard = function(posNode,off, needAction, isDelay){
    ...
    if(MjClient.rePlayVideo == -1){
        if(off == 0) {
            ...
            //if(MjClient.movingCard_liuHuQiang && MjClient.movingCard_liuHuQiang.getParent()){
            //    MjClient.movingCard_liuHuQiang.removeFromParent(true);
        ...
        if(off == 0 && (MjClient.MaxPlayerNum_liuHuQiang == 3 || MjClient.MaxPlayerNum_liuHuQiang == 2)) {
            var scale_x = handCard.scaleX;
            var winSize = MjClient.size;
...
*/
PlayLayer_liuHuQiang.prototype._doMovetoAction = function(node,endP){
    node.stopAllActions();
    var ac = cc.moveTo(0.3,endP);
    node.runAction(ac);
};

PlayLayer_liuHuQiang.prototype.ResetOtherCard = function(posNode,off){
    //添加吃、碰、跑、偎、提
    var sData = MjClient.data.sData;
    var tData = null;
    if(sData){
    ...
    if(mjSortArr){
        ...
        var callback = function(childList){
            ...
            for(var i = 0;i < childList.length;i++){
                ...
                var maxPlayer = MjClient.data.sData.tData.maxPlayer || 3;
                ...
                if ((i == 0 && maxPlayer - off <= 2) || (i == childList.length - 1 && maxPlayer - off > 2)) {
                    ...
                    child.loadTexture(MjClient.cardPath_liuHuQiang+"huxiBG.png");
                }
            }
        };
        ...
        for(var k = 0;k < mjSortArr.length;k++){
            ...
            if(name == "mjgang1"){
                ...
                for(var i = 0;i < 4;i++){
                    addEatCard_liuHuQiang(posNode,"mjgang1" + k,cardNum,off);
                }
            ...
            if(name == "mjwei"){
                var cardNum = pl.mjwei[pos];
                for(var i = 0;i < 3;i++){
                    addEatCard_liuHuQiang(posNode,"mjwei" + k,cardNum,off);
                }
            ...
            if(name == "mjgang0"){
                ...
                for(var i = 0;i < 4;i++){
                    addEatCard_liuHuQiang(posNode,"mjgang0" + k,cardNum,off);
                }
            }
            //碰
            if(name == "mjpeng"){
                ...
                for(var i = 0;i < 3;i++){
                    addEatCard_liuHuQiang(posNode,"mjpeng" + k,cardNum,off);
                }
            }
            //吃
            if(name == "mjchi"){
                ...
                for(var i = 0;i < eatCards.length;i++){
                    addEatCard_liuHuQiang(posNode,"mjchi" + k,eatCards[i],off);
                }
                ...
                if(biCards && biCards.length > 0){
                    for(var i = 0;i < biCards.length;i++){
                        ...
                        for(var m = 0;m<biArr.length;m++){
                            addEatCard_liuHuQiang(posNode,"mjbi" + k + i,biArr[m],off);
                        }
                    }        
                }
    ...
    //    var outCard = getNewCard_liuHuQiang(pl.mjput[i], 2, off);
    //    if(off == 0){
    //        outCard.anchorX = 1;
    //        outCard.anchorY = 0;
...
PlayLayer_liuHuQiang.prototype.ResetPutCard = function(posNode,off){
    //添加打出的牌
    var pl = getUIPlayer_liuHuQiang(off);
    if (!pl) return;
    var outNode = posNode.getChildByName("outNode");
    ...
    for (var i = 0; i < pl.mjput.length; i++) {
        ...
        var outCard = getNewCard_liuHuQiang(pl.mjput[i], 2, off);
        outNode.addChild(outCard);
        setPutCardPos_liuHuQiang(outNode, outCard, i, off);
...
//重置牌的顺序
PlayLayer_liuHuQiang.prototype.CardLayoutRestore = function(posNode,off){
    if(MjClient.rePlayVideo == -1){
...
// 判断吃、碰、跑、偎、提、胡的状态
PlayLayer_liuHuQiang.prototype.EatVisibleCheck = function(off){
    var eat = MjClient.playui.jsBind.eat;
    ...
    if(vnode.length > 0){
        ...
        for(var i = 0; i < vnode.length; i++){
            ...
            if(vnode[i] == eat.hu._node){
                setWgtLayout(vnode[i], [0, 0.25], [0.5, 0.10], [(1 - vnode.length) / 1.6+ i * 1.3, 1.8], false, false);
            }
...
PlayLayer_liuHuQiang.prototype.showAndHideHeadEffect = function(){
    ...
    else if (curPlayerIsMe_liuHuQiang(1)){
        curPlayerNode = arr[1].getChildByName("head");
    }else if (curPlayerIsMe_liuHuQiang(2)){
        curPlayerNode = arr[2].getChildByName("head");
    }else if(curPlayerIsMe_liuHuQiang(3)){
    ...
    {   
        var tag = 2018322;
        if(this._downNode.getChildByName("head").getChildByTag(tag))
            this._downNode.getChildByName("head").removeChildByTag(tag, true);
        if(this._rightNode.getChildByName("head").getChildByTag(tag))
            this._rightNode.getChildByName("head").removeChildByTag(tag, true);
        if(this._topNode.getChildByName("head").getChildByTag(tag))
            this._topNode.getChildByName("head").removeChildByTag(tag, true);
        if(this._xingNode.getChildByName("head").getChildByTag(tag))
