...
function addWxHeadToEndUI_liuHuQiang(node,off)
{
    ...
    setRoundEndUserOffline_liuHuQiang(frame, pl);
...
//添加手牌
function addWinMjHand_liuHuQiang(_node,off) {
	var sData = MjClient.data.sData;
    var tData = sData.tData;
    if(MjClient.GAME_TYPE.PAO_HU_ZI_SR == MjClient.gameType){
    	off = getOffByXing_liuHuQiang(off);
    }
	...
	for(var i = 0;i < sortArr.length;i++){
        ...
        for(var j = 0;j < childSort.length;j++){
			...
			clone.visible = true;
			clone.loadTexture(MjClient.cardPath_liuHuQiang+"out" + card +".png");
			var off_y = 0;
			if(_node.getName() == "rightPanel"){
				off_y = clone.height;
				clone.anchorX = 1;
...
function reInitarrCardVisible_liuHuQiang()
{
...
var EndOneView_liuHuQiang = cc.Layer.extend({
    _block:null,
    _endoneuiNode:null, 
	jsBind:{
		block: {
		...
		btn_ready:{
			...
			_click: function(btn,eT){
				...
				reInitarrCardVisible_liuHuQiang();
			}
		},
		btn_logic: {
			_run: function(){
		   			...
		   			if(tData.cardNext < cards.length){
                        ...
                        var startPos = cc.p((size.width - totalWidth)/2 + cardObj.width/2,size.height/2);
		   				...
		   				for(var i = tData.cardNext;i < cards.length;i++){
		   					...
		   					cloneCard.loadTexture(MjClient.cardPath_liuHuQiang+"out" + card +".png");
			...
			leftPanel:{
				_run:function(){
                    if ((MjClient.gameType == MjClient.GAME_TYPE.SHAO_YANG_BO_PI || 
                    	...
                    	addWinMjHand_liuHuQiang(this, 1);
                    }else{
                        addWinMjHand_liuHuQiang(this, MjClient.data.sData.tData.maxPlayer - 1);
                    }
			...
			rightPanel:{
				_run:function(){
                    if ((MjClient.gameType == MjClient.GAME_TYPE.SHAO_YANG_BO_PI || 
                        ...
                        addWinMjHand_liuHuQiang(this, MjClient.data.sData.tData.maxPlayer - 2);
                    }
                    if(MjClient.rePlayVideo != -1){
                    	this.visible = false;
                    }
					...
					var sData = MjClient.data.sData;
		   			...
		   			var cloneFunc = function(cardNum){
		   				if(cloneNum != 0){
		   					...
		   					cloneCard.loadTexture(MjClient.cardPath_liuHuQiang+"out" + cardNum + ".png");
		   					cloneNum++;
		   					return cloneCard;
		   				}   
		   				cloneNum++;
		   				var cardImg = self.getChildByName("card");
		   				cardImg.loadTexture(MjClient.cardPath_liuHuQiang+"out" + cardNum + ".png");
		   				return cardImg;
		   			}
			...
			uiBg:{
				headNode: {
					_run: function(){ 
						...
						if(tData.winner != -1)
							winOff = getUiOffByUid_liuHuQiang(pl.info.uid); 

						addWxHeadToEndUI_liuHuQiang(this,winOff); 
						...
						var cloneNum = 2;
				   		...
				   		for(var index = 0;index<playerCount;index++){
				   			if(index != winOff){
				   				...
				   				addWxHeadToEndUI_liuHuQiang(cloneThis,index);
				   				cloneNum++;
				   			}
				   		}
					}
    ...
    ctor:function () {
        ...
        if (MjClient.playui.getCardFilePath) {
            //兼容新版
            MjClient.cardPath_liuHuQiang = MjClient.playui.getCardFilePath();
        }
    ...
    setWinCard: function(){ 
    	...
    	var cloneCardsSetData = function(cardNode,i,num){
			...
			cardImg.loadTexture(MjClient.cardPath_liuHuQiang+"out" + num + ".png"); 
			return cardImg;
    	}
	    		...
	    		var getHuXi = function(cards){
	        		...
	        		for(var t = 0;t < normal.length;t++){
	        			if(normal[t].toString() == cards.toString()){
	        				var huxi = huXiScore_liuHuQiang("chi",cards[0]);
	        				return huxi;
	        			}
	        		} 
	        		return 0;			        		
    ...
    setXingCardData: function(tData,num){
    	...
    	card_img.loadTexture(MjClient.cardPath_liuHuQiang+"out" + tData.hunCard +".png");
