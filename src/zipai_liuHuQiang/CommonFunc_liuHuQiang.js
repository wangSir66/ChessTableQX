...
// 耒阳字牌全局方法
MjClient.MaxPlayerNum_liuHuQiang = 3;
MjClient.cardPath_liuHuQiang = "playing/ziPai/";
var ActionType_liuHuQiang =  //图片提示
    {
        CHI:1,
        PENG:2,
        WEI:3,
...
//初始化玩家金币、名字、胡息
function InitUserCoinAndName_liuHuQiang(node, off){
    var pl = getUIPlayer_liuHuQiang(off);
    if(!pl){
        return;
    }
...
//向服务器发送王闯王钓操作
function HZWangChuangToServer_liuHuQiang(type){
    cc.log("====================HZWangChuangToServer_liuHuQiang=================");
...
//向服务器发送打牌操作
function HZPutCardToServer_liuHuQiang(card){
    cc.log("====================HZPutCardToServer_liuHuQiang=================");
    MjClient.gamenet.request("pkroom.handler.tableMsg", {
        cmd: "MJPut",
        card: card
    });
...
// 向服务器发送吃牌
function HZChiToServer_liuHuQiang(eatCards, biCards){
...
//像服务器发送碰牌
function HZPengToServer_liuHuQiang()
{
    if (MjClient.rePlayVideo != -1) return; // 回放时候不能请求
    cc.log("====================HZPengToServer_liuHuQiang=================");
    MjClient.gamenet.request("pkroom.handler.tableMsg", {
        cmd: "MJPeng"
    });
}

function MJPass2Net_liuHuQiang(){
    cc.log("====================send======pass=====");
    ...
    if(IsTurnToMe() && tData.tState == TableState.waitPut){
        ...
        HZPassConfirmToServer_liuHuQiang();
    }
};

// 向服务器发送过牌
function HZPassConfirmToServer_liuHuQiang() {
    if (MjClient.rePlayVideo != -1)  {
        return; // 回放时候不能请求
    }
...
//发送跑牌命令
function HZGangToServer_liuHuQiang(type){
    if (MjClient.rePlayVideo != -1) return; // 回放时候不能请求
    cc.log("====================HZGangToServer_liuHuQiang=================");
    MjClient.gamenet.request("pkroom.handler.tableMsg", {
        cmd: "MJGang",
        type: type,
        eatFlag: EatFlag_liuHuQiang()
...
//发送偎牌命令
function HZWeiToServer_liuHuQiang(type){
    if (MjClient.rePlayVideo != -1) return; // 回放时候不能请求
    cc.log("====================HZWeiToServer_liuHuQiang=================");
    MjClient.gamenet.request("pkroom.handler.tableMsg", {
        cmd: "HZWeiCard"
    });
}
...
//向服务器发送发牌命令
function HZNewCardToServer_liuHuQiang(){
    if (MjClient.rePlayVideo != -1) return; // 回放时候不能请求
    cc.log("====================HZNewCardToServer_liuHuQiang=================");
    MjClient.gamenet.request("pkroom.handler.tableMsg", {
        cmd: "HZNewCard"
    });
}
...
//向服务器发送胡牌命令
function MJHuToServer_liuHuQiang(){
    if (MjClient.rePlayVideo != -1) return; // 回放时候不能请求
    cc.log("====================MJHuToServer=================");
    MjClient.gamenet.request("pkroom.handler.tableMsg", {
        cmd: "MJHu",
...
//添加吃的牌
function addEatCard_liuHuQiang(node,name,mjNum,off){
    //根据牌的类型获得需要添加的节点
    ...
    var type = 2;
    //设置牌
    var newCard = getNewCard_liuHuQiang(mjNum,type,off);
     ...
     (MjClient.gameType == MjClient.GAME_TYPE.AN_HUA_PAO_HU_ZI ||
      ...
      MjClient.gameType == MjClient.GAME_TYPE.SHAO_YANG_ZI_PAI)){
        if(MjClient.data.sData.tData.maxPlayer == 2 &&
            (node.getName() == "left" || node.getName() == "down")) {
            newCard.y -= 32;
        }
    }
...
/**
 添加手牌(回放)
 */
function addHandCardReplay_liuHuQiang(tagName,index,mjNum,off){
    var _node = getNode_liuHuQiang(off);
    //根据牌的类型获得需要添加的节点
...
function getEmptyIndex_liuHuQiang(cardArray) {
    var empty = [];
...
function cleanCardNode_liuHuQiang(off) {
    //获取结点
    if(off != 0){
        return;
    }
    ...
    /*for(var k = 0; k < addNode.cardList.length; k++){
        cc.log("chow", "cleanCardNode_liuHuQiang cardList: k = " + k + " tag = " + addNode.cardList[k].tag + " name = " + addNode.cardList[k].name + " parentTag =" + addNode.cardList[k].parent.tag);
    }*/
    //查找当前移动的牌 并更新移动位置
    if(MjClient.moveCard){
        cc.log("chow", "cleanCardNode_liuHuQiang" + " moveCard curCIndex = " + MjClient.moveCard.curCIndex + " curRIndex = " + MjClient.moveCard.curRIndex
        ...
        for(var i = 0; i < addNode.cardList.length; i++){
            if(addNode.cardList[i].parent.tag == MjClient.moveCard.curCIndex && addNode.cardList[i].name == MjClient.moveCard.curRIndex){
                //cc.log("chow", "cleanCardNode_liuHuQiang" + " addNode i = " + i + " tag = " + addNode.cardList[i].parent.tag + " name = " + addNode.cardList[i].name);
                oldIndex = i;
        ...
        if(oldIndex != addNode.cardList.length){
            ...
            if(MjClient.moveCard.curCIndex != MjClient.moveCard.nexCIndex
                ...
                //cc.log("chow", "cleanCardNode_liuHuQiang" + " moveCard tag = " + moveCard[0].tag + " name = " + moveCard[0].name);
                var newIndex = addNode.cardList.length;
                //查到当前移动牌移动的目标位置
                for(var i = 0; i < addNode.cardList.length; i++){
                    //cc.log("chow", "cleanCardNode_liuHuQiang" + " addNode.cardList tag = " + addNode.cardList[i].parent.tag + " name = " + addNode.cardList[i].name);
                ...
                cc.log("chow", "cleanCardNode_liuHuQiang" + " newIndex = " + newIndex);
    ...
    //将按数据排序好的牌复用 后续直接取
    cc.log("chow cleanCardNode_liuHuQiang cardList.length = ", addNode.cardList.length + "");
    /*for(var k = 0; k < addNode.cardList.length; k++){
        cc.log("chow", "cleanCardNode_liuHuQiang cardList: k = " + k + " tag = " + addNode.cardList[k].tag + " name = " + addNode.cardList[k].name + " parentTag =" + addNode.cardList[k].parent.tag);
    }*/
    ...
    for(var k = 0; k < addNode.cardList.length; k++){
        ...
        cc.log("chow", "cleanCardNode_liuHuQiang cardList: k = " + k + " tag = " + addNode.cardList[k].tag + " pos = " + JSON.stringify(addNode.cardList[k].lastPosition), null, "  ");
    }
    ...
    //复用列节点 ,后续直接查找使用
    cc.log("chow cleanCardNode_liuHuQiang cardRoot.length = ", addNode.cardRoot.length + "");
    for(var k = 0; k < addNode.cardRoot.length; k++){
        //cc.log("chow", "cleanCardNode_liuHuQiang cardRoot: k = " + k + " : " + addNode.cardRoot[k].tag);
    ...
    //查找数据中被删除的列索引
    cc.log("chow", "cleanCardNode_liuHuQiang : MjClient.HandCardArr = "  + JSON.stringify(MjClient.HandCardArr));
    var empty = getEmptyIndex_liuHuQiang(MjClient.HandCardArr);
    for(var i = 0; i < empty.length; i++){
        cc.log("chow", "cleanCardNode_liuHuQiang empty[" + i + "] = " + empty[i]);
    }

    //区分左右新建列和无变化
    cc.log("chow", "cleanCardNode_liuHuQiang addGroupIndex:" + MjClient.addGroupIndex);
    if(MjClient.addGroupIndex == 0){
        ...
        if(addNode.cardRoot.length > 0){
            ...
            cc.log("chow", "cleanCardNode_liuHuQiang" + " addRootCount = " + addRootCount);
...
function resetCardNode_liuHuQiang(off) {
    if(off != 0){
        return;
    }
    var _node = getNode_liuHuQiang(off);
    ...
    cc.log("chow resetCardNode_liuHuQiang cardList.length", addNode.cardList.length + "");
    for(var i = 0; i < addNode.cardList.length; i++){
        addNode.cardList[i].release();
    };

    cc.log("chow resetCardNode_liuHuQiang cardRoot.length", addNode.cardRoot.length + "");
    for(var i = 0; i < addNode.cardRoot.length; i++){
        addNode.cardRoot[i].release();
    }
}

function getCardNode_liuHuQiang(cardList, mjNum){
    for(var i = 0; i < cardList.length; i++){
        ...
        if (cardList[i].tag == mjNum && cardList[i].isSelect == false && cardList[i].opacity == 255) {
            ...
            //cc.log("chow", "getCardNode_liuHuQiang : " + i);
            return cardList[i];
        }
    }
    return null;
...
function getCardRoot_liuHuQiang(cardRoot, tagName){
    for(var i = 0; i < cardRoot.length; i++){
        if (cardRoot[i].tag == tagName) {
            //cc.log("chow", "getCardRoot_liuHuQiang : " + i);
            return cardRoot[i];
        }
    }
    return null;
...
function addOrAdjustHandCard_liuHuQiang(tagName,index,mjNum,off){
    if(off != 0){
        return;
    }
    var _node = getNode_liuHuQiang(off);
    ...
    //取一个对应的牌节点
    var newCard = getCardNode_liuHuQiang(addNode.cardList, mjNum);
    if(!newCard){
        cc.log("chow", "newCard");
        newCard = getNewCard_liuHuQiang(mjNum,1,off);
    }else{
        cc.log("chow", "getCard");
    }
    var scale_y = newCard.scaleY;
    ...
    var cardParent = addNode.getChildByTag(tagName);
    if(!cardParent){
        cardParent = getCardRoot_liuHuQiang(addNode.cardRoot, tagName);
        if(!cardParent){
            cc.log("chow", "newRoot");
            cardParent = new cc.Node();
            cardParent.tag = tagName;
    ...
    if (MjClient.movingCard_paohuzi == newCard) {
        setCardSprite_liuHuQiang(newCard, newCard.tag, 1, false);
    ...
    if(newCard.lastPosition){
        ...
        doMovetoAction_liuHuQiang(newCard, cc.p(beginPoint.x, beginPoint.y + cardCount * off_y));
    }else{
        newCard.x = beginPoint.x;
        newCard.y = beginPoint.y + cardCount * off_y;
    }
    //newCard.opacity = 255;
    SetTouchCardHandler_liuHuQiang(newCard,off);
    ...
    //cc.log("chow", "addOrAdjustHandCard_liuHuQiang: name=" + newCard.getName() + " num=" + newCard.num + " tag =" + newCard.tag);
}

function doMovetoAction_liuHuQiang(node,endP){
    node.stopAllActions();
    var action = cc.moveTo(0.15,endP);
    node.runAction(action);
};

function resetHandCard_liuHuQiang(posNode,off, needAction, isDelay){
    if(MjClient.rePlayVideo == -1){
        if(off == 0) {
            var handNode = posNode.getChildByName("handNode");
            handNode.visible = true;
            cleanCardNode_liuHuQiang(off);
            ...
            for (var k = 0; k < cardArr.length; k++) {
                ...
                for (var j = 0; j < groupList.length; j++) {
                    addOrAdjustHandCard_liuHuQiang(k, j, groupList[j], off);
                }
            }
            resetCardNode_liuHuQiang(off);

            addTingSign_liuHuQiang(posNode); // 添加听牌角标
            ...
            for (var i = 0; i < cardArr.length; i++) {
                ...
                if(addNode.lastPosition){
                    ...
                    doMovetoAction_liuHuQiang(addNode, cc.p((winSize.width - totalWidth) / 2 + i * width * scale_x, 0));
                }else{
        ...
        for(var k = 0;k < cardArr.length;k++){
            ...
            for(var j = 0;j < groupList.length;j++){
                if(off == 0){
                    if(MjClient.data.sData.tData.maxPlayer == 3 || MjClient.data.sData.tData.maxPlayer == 2){
                        addHandCard_liuHuQiang(k,j,groupList[j],off);
                    }else if(MjClient.data.sData.tData.maxPlayer == 4){
                        addHandCardReplay_liuHuQiang(k,j,groupList[j],off);
                    }
                }else {
                    addHandCardReplay_liuHuQiang(k,j,groupList[j],off);
                }
...
function checkCard_liuHuQiang(node, off){
    ...
    if(cardHandArr.length > cardHand.length){
        ...
        for(var i = 0;i < cardHandArr.length; i++){
            if(cardHandArr[i] != cardHand[i]){
                RemoveHandCard_liuHuQiang(node, cardHandArr[i], off);
...
function resetHandCard_liuHuQiangEx(posNode,off, needAction, isDelay){
    if(off == 0) {
        checkCard_liuHuQiang(posNode, off);
        var handNode = posNode.getChildByName("handNode");
        handNode.visible = true;
        cleanCardNode_liuHuQiang(off);
        handNode.removeAllChildren();
 ...
 */
function addHandCard_liuHuQiang(tagName,index,mjNum,off){
    if(off != 0){
        return;
    }
    var _node = getNode_liuHuQiang(off);
    ...
    //设置牌
    var newCard = getNewCard_liuHuQiang(mjNum,1,off);
    var scale_y = newCard.scaleY;
    //var parentCount = addNode.childrenCount;
    //首先根据name判断cpNode中是否已经添加
    var cardParent = addNode.getChildByTag(tagName);
    if(!cardParent){
        ...
        cardParent.width = newCard.width;
        addNode.addChild(cardParent);
        // SetTouchCardHandler_liuHuQiang(cardParent);
    }
    ...
    cc.log("chow", "addHandCard_liuHuQiang: name=" + newCard.getName() + " num=" + newCard.num + " tag =" + newCard.tag);
}
 ...
 **/
function getNewCard_liuHuQiang(mjNum, type, off, isTurn){
    var _node = getNode_liuHuQiang(off);
    var copyNode = null;
    if(type == 1){
        //手牌
    ...
    //cc.log("getNewCard_liuHuQiang:"+mjNum+","+type+","+off+","+isTurn);
    if(mjNum > 0){
        //创建一个带有麻将信息的麻将 cp为创建后的麻将
        setCardSprite_liuHuQiang(mjNode, mjNum, type, isTurn);
        if(type == 1){
            //只有手牌才会有点击事件
            SetTouchCardHandler_liuHuQiang(mjNode,off);
        }
    }
...
/**
 ...
 type:类型，1:手牌 2：吃牌 3：打的牌
 */
function setCardSprite_liuHuQiang(mjNode, mjNum, type, isTurn){
    MjClient.cardPath_liuHuQiang = ziPai.getCardFilePath();
    if(type == 1){
        mjNode.loadTexture(MjClient.cardPath_liuHuQiang+"hand" + mjNum + ".png");
        console.log(MjClient.cardPath_liuHuQiang+"hand" + mjNum + ".png");
    }
    if(type == 2){
        mjNode.loadTexture(MjClient.cardPath_liuHuQiang+"out" + mjNum + ".png");
        if(isTurn){
            mjNode.loadTexture(MjClient.cardPath_liuHuQiang+"huxiBG.png");
        }
    }
    if(type == 3 || type == 4){
        mjNode.loadTexture(MjClient.cardPath_liuHuQiang+"put" + mjNum + ".png");
        if(isTurn){
            mjNode.loadTexture(MjClient.cardPath_liuHuQiang+"normalBG.png");
        }
    }
    mjNode.tag = mjNum;
}

function checkTingCards_liuHuQiang(putCard){
    //听牌检测
    var sData = MjClient.data.sData;
    ...
    if(MjClient.data.sData.tData.tState == TableState.waitPut && curPlayerIsMe_liuHuQiang(0) && putCard === undefined){
        postEvent("showTingLayer", []);
        return;
    }

    if(putCard && (!curPlayerIsMe_liuHuQiang(0) || sData.tData.tState != TableState.waitPut)){
        putCard = undefined;
    }
...
//刷新手牌移动时的坐标
var btn_oPos = null;
function updateBtnMovedPosition_liuHuQiang(btn, eventType){
...
function SetTouchCardHandler_liuHuQiang(card,off){
    ...
    card.addTouchEventListener(function(btn,eventType){
        ...
        if(eventType == ccui.Widget.TOUCH_BEGAN){
            ...
            setCardSprite_liuHuQiang(cloneCard, cardTag, 1, false);
            cloneCard.opacity = 0;
            ...
            btn.x = touchPos.x;
            btn.y = touchPos.y;
            // updateBtnMovedPosition_liuHuQiang(btn, eventType);
            ShowPutCardIcon_liuHuQiang();

            checkTingCards_liuHuQiang(btn.tag);

            var alignWidth = btn.scale * btn.width;
            setCardSprite_liuHuQiang(btn, btn.tag, 3, false);
        ...
        if(eventType == ccui.Widget.TOUCH_ENDED || 
            ...
            var pl = getUIPlayer_liuHuQiang(0);
            var tData = MjClient.data.sData.tData;
            var isPut = false;
            var cutLine = MjClient.playui.jsBind.cutLine._node;

            setCardSprite_liuHuQiang(btn, btn.tag, 1, false);
            ...
            if(((!pl.canNotPutCard||pl.canNotPutCard.indexOf(btnTag)==-1) &&(getOtherWeiCards_liuHuQiang(btnTag)||(pl.limitHuPutCard && pl.limitHuPutCard.indexOf(btnTag) != -1))) &&
                ...
                !MjClient.isCommon && endPos.y >= cutLine.y && !MjClient.hasPut)
            {
                ...
                var putCard = getNewCard_liuHuQiang(btnTag, 3 ,off);
                ...
                */
                if(getOtherWeiCards_liuHuQiang(btnTag)){
                            ...
                            if(pl.limitHuPutCard && pl.limitHuPutCard.indexOf(btnTag) != -1){
                                MjClient.showMsg("吃边打边，这局将只能胡"+ MjClient.majiang.getLimitHuDesc(btnTag, tData)+"，是否确定？",
                                    function(){
                                        ...
                                        HZPutCardToServer_liuHuQiang(btnTag);
                                        MjClient.playui.ResetHandCard(MjClient.playui._downNode,0);

                                        checkTingCards_liuHuQiang(btnTag);
                                    },
                                    function(){
                                        MjClient.playui.ResetHandCard(MjClient.playui._downNode,0);
                                        ShowPutCardIcon_liuHuQiang();
            ...
            else
            {
                ...
                if(!isPut || isWangBa || isNoPut)
                {
                    ...
                    if(selectParentTag == btnParent.tag){
                        ...
                        if(selectArr){
                            //_changeArrIndex_liuHuQiang(selectArr, btnTag, btn);
                            MjClient.HandCardArr[btnParent.tag].splice(parseInt(btnName),1);
                            _fixArrIndex_liuHuQiang(selectArr, btnTag, btn);
                        }
                    }
...
//根据坐标Y，添加到数组对应的位置
function _fixArrIndex_liuHuQiang(arr, btnTag, btn){
    if(arr){
        if(_isKan_liuHuQiang(arr)){
            arr.push(btnTag);
        ...
        cc.log('_fixArrIndex_liuHuQiang====:',arr);
    }

}

//改变数组元素的位置
function _changeArrIndex_liuHuQiang(arr, btnTag, btn){
    if(arr && !_isKan_liuHuQiang(arr)){
        var len = arr.length;
        for(var i = 0; i < len; i++){
            var tag = arr[i];
            if(tag == btnTag){
        ...
        _fixArrIndex_liuHuQiang(arr, btnTag, btn);
    }
}

//数组里的是否为三个相同的牌
function _isKan_liuHuQiang(arr){
    if(arr){
        var len = arr.length;
        if(len < 3){
            return false;
...
//断线重连之后，显示之前玩家打的牌
function DealOffLineCard_liuHuQiang(node, off) {
    cc.log("====================DealOffLineCard_liuHuQiang=================");
    var sData = MjClient.data.sData;
    var tData = sData.tData;
    ...
    if(typeof(tData.currCard) != "undefined" && tData.currCard != -1){
        ...
        if(uids[selfIndex] == uids[tData.curPlayer]){
            var putCard = getNewCard_liuHuQiang(tData.currCard, 3, off);
...
function getUIBindByIndex_liuHuQiang(idx) {  
    var sData = MjClient.data.sData;
    var tData = sData.tData;
...
// 落牌动画
function fall_liuHuQiang() {
    var sData = MjClient.data.sData;
    var tData = sData.tData;

    // tData.lastPlayer没同步!
    var lastPlayer = (tData.curPlayer - 1 + tData.maxPlayer) % tData.maxPlayer;
    cc.log("chow", "fall_liuHuQiang"+ " lastPlayer = " + lastPlayer);
    ...
    if (out) {
        ...
        cc.log("chow", "fall_liuHuQiang" + JSON.stringify(out.getPosition()));
...
//发牌
MjClient.isCommon = false;
function DealNewCard_liuHuQiang(node, msg, off) {
    //没有发牌过来的时候直接忽略
    if (!msg.newCard) {
        RemovePutCardOut_liuHuQiang();
        MjClient.playui.EatVisibleCheck();
        return;
    }

    AddPutCard_liuHuQiang(node, msg, off); // 注意各个玩法验证！
    //存在发牌
    var sData = MjClient.data.sData;
    var tData = sData.tData;
    var uids = tData.uids;
    ...
    if (uids[selfIndex] == msg.uid) {
        cc.log("DealNewCard_liuHuQiang  newCard@@ ", msg.newCard);
        //清理之前玩家打的牌
        ...
        var putCard = getNewCard_liuHuQiang(msg.newCard, 3, off, showPkBack);
        var putNode = node.getChildByName("put");
        putNode.removeAllChildren();
        putCard.scaleX = putCard.width / putNode.width - 0.16;
        putCard.scaleY = putCard.width / putNode.width - 0.06;
        ...
        if (!msg.isCommon) {
            if (MjClient.rePlayVideo == -1) {
                ...
                for (var i = 0; i < cardArr.length; i++) {
                    ...
                    if (tmpArr.length <= 2) {
                        isAdd = true;
                        //addHandCard_liuHuQiang(i,tmpArr.length.length + 1,msg.newCard,off);
...
function setPutCardPos_liuHuQiang(outNode, outCard, idx, off) {
    var tData = MjClient.data.sData.tData;
    var maxPlayer = tData.maxPlayer;
    var type = ziPai.getUiLayoutType();
    var isZuoXing = false;
    cc.log("setPutCardPos_liuHuQiang:" + isZuoXing);
...
function setJiachuiTextTips_liuHuQiang(node, off) {
    node.visible = false;
    var tData = MjClient.data.sData.tData;
    var pl = getUIPlayer_liuHuQiang(off);
    if (!pl) {
...
function setJiachuiImgTips_liuHuQiang(node, off) {
...
function AddPutCard_liuHuQiang(node, msg, off) {
    var sData = MjClient.data.sData;
    var tData = sData.tData;
    ...
    if (off_index == tData.curPlayer) { // off对应玩家为当前玩家
        ...
        var out = getNewCard_liuHuQiang(card, 2, off);
        var count = pl.mjput.length - 1;
        var width = out.width;
        var height = out.height;
        setPutCardPos_liuHuQiang(outNode, out, pl.mjput.length - 1, off);
...
//翻醒发牌
function DealFanXingNewCard_liuHuQiang(card){
    var putCard = getNewCard_liuHuQiang(card, 4, 0);
...
//王闯/王钓
function DealWangChuang_liuHuQiang(node,msg,off){
    var sData = MjClient.data.sData;
    var tData = sData.tData;
    ...
    if(uids[selfIndex] == msg.uid){
        ...
        if(pl.wangType == 1){
            var putCard = getNewCard_liuHuQiang(91, 3, off);
            var putNode = node.getChildByName("put");
            putCard.scaleX = putCard.width/putNode.width - 0.16;
            putCard.scaleY = putCard.width/putNode.width - 0.06;
            putCard.x = putCard.width/2;
            ...
            ShowEatActionAnim_liuHuQiang(node, ActionType_liuHuQiang.WANGDIAO, off);

            var delay = cc.delayTime(1.5);
            var callback = function(){
                RemovePutCardOut_liuHuQiang();
                //HZNewCardToServer_liuHuQiang();
            }
            //putNode.runAction(cc.sequence(delay,cc.callFunc(callback)));
        }else if(pl.wangType == 2){
            var putCard = getNewCard_liuHuQiang(91, 3, off);
            var putCard1 = getNewCard_liuHuQiang(91, 3, off);
            var putNode = node.getChildByName("put");
            putCard.scaleX = putCard.width/putNode.width - 0.16;
            putCard.scaleY = putCard.width/putNode.width - 0.06;
            ...
            ShowEatActionAnim_liuHuQiang(node, ActionType_liuHuQiang.WANGCHUANG, off);
            var delay = cc.delayTime(1.5);
            var callback = function(){
                RemovePutCardOut_liuHuQiang();
                //HZNewCardToServer_liuHuQiang();
            ...
            var putCard = getNewCard_liuHuQiang(91, 3, off);
            var putCard1 = getNewCard_liuHuQiang(91, 3, off);
            var putCard2 = getNewCard_liuHuQiang(91, 3, off);
            var putNode = node.getChildByName("put");
            putCard.scaleX = putCard.width/putNode.width - 0.16;
            putCard.scaleY = putCard.width/putNode.width - 0.06;
            putCard1.scaleX = putCard.width/putNode.width - 0.16;
            ...
            ShowEatActionAnim_liuHuQiang(node, ActionType_liuHuQiang.WANGZHA, off);
            var delay = cc.delayTime(1.5);
            var callback = function(){
                RemovePutCardOut_liuHuQiang();
                //HZNewCardToServer_liuHuQiang();
...
//处理摸最后一张牌
function DealShowLastCard_liuHuQiang(node, msg, off){
    cc.log("======DealShowLastCard_liuHuQiang======= " + off);
    MjClient.hasPut = true;
    var sData = MjClient.data.sData;
    var tData = sData.tData;
    var uids = tData.uids;
    ...
    if(uids[selfIndex] == msg.uid)
    {
        ...
        setTimeout(function(){
            RemovePutCardOut_liuHuQiang(true);
            var pl = getUIPlayer_liuHuQiang(0);
...
//处理出牌
function DealPutCard_liuHuQiang(node, msg, off) {
    AddPutCard_liuHuQiang(node, msg, off); // 注意各个玩法验证！
    ...
    if (uids[selfIndex] == msg.uid) {
        //别人出牌或者回放时，添加动作
        if (SelfUid() != msg.uid || MjClient.rePlayVideo != -1 || sData.players[msg.uid].trust) {
            var putNode = node.getChildByName("put");
            putNode.removeAllChildren();
            var putCard = getNewCard_liuHuQiang(msg.card, 3, off);
            putCard.scaleX = putCard.width / putNode.width - 0.16;
            putCard.scaleY = putCard.width / putNode.width - 0.06;
            putCard.x = putCard.width / 2;
            putCard.y = putCard.height / 2;
        ...
        if (MjClient.rePlayVideo == -1 && off == 0) {
            if(sData.players[msg.uid].trust){
                RemoveHandCard_liuHuQiang(node, msg.card, off);
            }
            MjClient.playui.ResetHandCard(node, off);
        ...
        if (off == 0) {
            removeTingSign_liuHuQiang(node);
        }
...
function removeTingSign_liuHuQiang(node) {
    var handNode = node.getChildByName("handNode");
    if (!handNode) {
        return;
    }
...
//吃牌
function DealChiCard_liuHuQiang(node, msg, off) {
    cc.log("======DealChiCard_liuHuQiang======= " + off);
    var sData = MjClient.data.sData;
    var tData = sData.tData;
    var uids = tData.uids;
    var selfIndex = (uids.indexOf(SelfUid()) + off) % MjClient.data.sData.tData.maxPlayer;
    if (tData.curPlayer == selfIndex) {
        ...
        for (var i = 0; i < tmpEatCards.length; i++) {
            RemoveHandCard_liuHuQiang(node, tmpEatCards[i], off);
        }
            ...
            for (var k = 0; k < biCards.length; k++) {
                ...
                for (var m = 0; m < biArr.length; m++) {
                    if (tmpBiArr.indexOf(biArr[m]) >= 0) {
                        ...
                        RemoveHandCard_liuHuQiang(node, biArr[m], off);
                    }
                }
            }
        }
        ...
        for (var i = 0; i < eatCards.length; i++) {
            var card = getNewCard_liuHuQiang(eatCards[i], 2, off);
            if (i == 2) {
                card.setColor(cc.color(170, 170, 170));
            }
            ...
            card.zIndex = 4 - i;
            card.x = 0;
            card.y = off_count * getOffY_liuHuQiang(node, card);
            setChiCardAnchorPoint_liuHuQiang(node, card);
        ...
        if (biCards) {
            ...
            for (var k = 0; k < biCards.length; k++) {
                var biArr = biCards[k];
                for (var m = 0; m < biArr.length; m++) {
                    var card = getNewCard_liuHuQiang(biArr[m], 2, off);
                    if (m == 2) {
                        card.setColor(cc.color(170, 170, 170));
                    } 
                    setChiCardAnchorPoint_liuHuQiang(node, card);
        ...
        var parentPos = getChiCardParentPosition_liuHuQiang(node, cardWidth, off, selfColCount);
        var moveX0 = 0;
        ...
        parent.x = moveX0;
        setCardParentPosY_liuHuQiang(node, parent);
        ...
        if (biCards) {
            ShowEatActionAnim_liuHuQiang(node, ActionType_liuHuQiang.XIAHUO, off);
        } else {
            ShowEatActionAnim_liuHuQiang(node, ActionType_liuHuQiang.CHI, off);
        }
...
function setChiCardAnchorPoint_liuHuQiang(node,newCard) {
    if(node.getName() == "down"){
        newCard.anchorX = 0;
        newCard.anchorY = 0;
    }else if(node.getName() == "right"){
...
function getOffY_liuHuQiang(node,newCard) {
    var off_y = 0;
    if(node.getName() == "down"){
        off_y = newCard.height;
    }else if(node.getName() == "right"){
...
function getChiCardParentPosition_liuHuQiang(node, newCardWidth, off, selfColCount, orignColPosX) {
...
function setCardParentPosY_liuHuQiang(node, cardParent) {
    var eatNode = node.getChildByName("eatNode");
    if(node.getName() == "down"){
        cardParent.y = 0;
    }else if (node.getName() == "right"){
...
function getShowCardIdx_liuHuQiang(node, chiTip) {
    var idx;
    switch(node.getName()) {
        case "down":
        case "xing":
...
// 处理碰
function DealPengCard_liuHuQiang(node, msg, off){
    cc.log("======DealChiCard_liuHuQiang======= ");
    var sData = MjClient.data.sData;
    var tData = sData.tData;
    var uids = tData.uids;
    var selfIndex = (uids.indexOf(SelfUid()) + off) % MjClient.data.sData.tData.maxPlayer;
    if(tData.curPlayer == selfIndex){
        ...
        for(var i = 0;i < 3; i++) {
            var card = getNewCard_liuHuQiang(pengCard,2,off);
            cardWidth = card.width//*eatNode.scaleX;
            var childCount = parent.childrenCount;
            card.zIndex = 4 - childCount;
            card.x = 0;
            card.y = childCount * getOffY_liuHuQiang(node, card);
            setChiCardAnchorPoint_liuHuQiang(node, card);
            parent.addChild(card);
        }
        eatNode.addChild(parent);

        RemoveHandCard_liuHuQiang(node,pengCard,off);
        RemoveHandCard_liuHuQiang(node,pengCard,off);
        MjClient.playui.ResetHandCard(node, off);
        ShowEatActionAnim_liuHuQiang(node, ActionType_liuHuQiang.PENG, off);
        ...
        var parentPos = getChiCardParentPosition_liuHuQiang(node, cardWidth, off, 1);
        var moveX0 = 0;
        var outNodeCount = eatNode.getChildrenCount();
        if(outNodeCount>0){
            if(node.getName() == "down" || node.getName() == "left"){
...
//偎牌
function DealWeiCard_liuHuQiang(node, msg, off) {
    cc.log("======DealWeiCard_liuHuQiang======= ");
    var sData = MjClient.data.sData;
    var tData = sData.tData;
    var uids = tData.uids;
    var selfIndex = (uids.indexOf(SelfUid()) + off) % MjClient.data.sData.tData.maxPlayer;
    if (tData.curPlayer == selfIndex) {
        ...
        RemovePutCardOut_liuHuQiang();
        ShowEatActionAnim_liuHuQiang(node, ActionType_liuHuQiang.WEI, off);
        ...
        for (var i = 0; i < 3; i++) {
                ...
                MjClient.majiang.getCardShowType(tData.uids[tData.curPlayer], weiCard) == 2) 
            {
                if(MjClient.gameType === MjClient.GAME_TYPE.SHAO_YANG_FANG_PAO_FA){
                    ...
                    if (i == getShowCardIdx_liuHuQiang(node, "wei")) {
                        isTurn = false;
                    }
                    var card = getNewCard_liuHuQiang(weiCard, 2, off, isTurn);
                }
            } else if (MjClient.majiang.getCardShowType(tData.uids[tData.curPlayer], weiCard) == 1) {
                var card = getNewCard_liuHuQiang(weiCard, 2, off);
                ...
                var card = getNewCard_liuHuQiang(weiCard, 2, off, true);
            }
            cardWidth = card.width //*eatNode.scaleX;
            card.zIndex = 4 - i;
            card.x = 0;
            card.y = i * getOffY_liuHuQiang(node, card);
            setChiCardAnchorPoint_liuHuQiang(node, card);
            parent.addChild(card);
        }
        eatNode.addChild(parent);

        RemoveHandCard_liuHuQiang(node, weiCard, off);
        RemoveHandCard_liuHuQiang(node, weiCard, off);
        MjClient.playui.ResetHandCard(node, off);
...
// 跑牌或者提牌
function DealGangCard_liuHuQiang(node, msg, off) {
    var sData = MjClient.data.sData;
    var tData = sData.tData;
    var uids = tData.uids;
    var selfIndex = (uids.indexOf(SelfUid()) + off) % MjClient.data.sData.tData.maxPlayer;
    ...
    if (uids.indexOf(gangUid) == selfIndex) {
        ...
        if (msg.type == 2) {
            RemovePutCardOut_liuHuQiang();
            ShowEatActionAnim_liuHuQiang(node, ActionType_liuHuQiang.PAO, off);
        } else {
            if (!msg.isGangHand && msg.type == 1) {
                RemovePutCardOut_liuHuQiang();
            }
            ShowEatActionAnim_liuHuQiang(node, ActionType_liuHuQiang.TI, off);
        }
        ...
        for (var i = 0; i < 4; i++) {
            ...
            if (msg.type == 2) {
                isTurn = false;
                var card = getNewCard_liuHuQiang(gangCard, 2, off, isTurn);
                ...
                MjClient.majiang.getCardShowType(tData.uids[tData.curPlayer], gangCard) == 2) 
            {
                if(MjClient.gameType === MjClient.GAME_TYPE.SHAO_YANG_FANG_PAO_FA){
                    ...
                    if (i == getShowCardIdx_liuHuQiang(node, "ti")) {
                        isTurn = false;
                    }
                    var card = getNewCard_liuHuQiang(gangCard, 2, off, isTurn);
                }
            } else if (MjClient.majiang.getCardShowType(tData.uids[tData.curPlayer], gangCard) == 1) {
                var card = getNewCard_liuHuQiang(gangCard, 2, off, false);
                var shade = new cc.Sprite("playing/paohuzi/huxiBG1.png");
                shade.opacity = 100;
                shade.x = shade.width / 2;
                shade.y = shade.height / 2;
                ...
                var card = getNewCard_liuHuQiang(gangCard, 2, off, true);
            }
            cardWidth = card.width //*eatNode.scaleX;
            card.zIndex = 4 - i;
            card.x = 0;
        ...
        RemoveHandCard_liuHuQiang(node, gangCard, off);
        RemoveHandCard_liuHuQiang(node, gangCard, off);
        RemoveHandCard_liuHuQiang(node, gangCard, off);
        if (msg.isGangHand) {
            RemoveHandCard_liuHuQiang(node, gangCard, off);
        ...
        MjClient.playui.ResetHandCard(node, off);
        // addTingSign_liuHuQiang(node); // 添加听牌角标 (提 跑可能手牌未变动)
        ...
        var parentPos = getChiCardParentPosition_liuHuQiang(node, cardWidth, off, 1, orignColPosX);
        var moveX0 = 0;
        var outNodeCount = eatNode.getChildrenCount();
        if (outNodeCount > 0) {
            if (node.getName() == "down" || node.getName() == "left") {
...
// 处理胡
function DealHu_liuHuQiang(node, msg, off){
    var sData = MjClient.data.sData;
    var tData = sData.tData;
    ...
    if(pl){
        pl.eatFlag = 0;
        MjClient.playui.EatVisibleCheck();
        ShowEatActionAnim_liuHuQiang(node,ActionType_liuHuQiang.HU,off);
    }
}

function DealAddCard_liuHuQiang(node,msg, off){
    if(MjClient.rePlayVideo == -1 && off != 0){
        return;
    }
    var cardArr = MjClient.HandCardArr;
...
//清除打出的牌
function RemovePutCardOut_liuHuQiang(noAction){
    ...
    for (var i = 0; i < uiList.length; i++) {
        ...
        // var pl = getUIPlayer_liuHuQiang(i);
        // if (MjClient.gameType == MjClient.GAME_TYPE.PAO_HU_ZI_SR || MjClient.gameType == MjClient.GAME_TYPE.PAO_HU_ZI_SR_King) {
        //     pl = getUIPlayer_liuHuQiang(getOffByXing_liuHuQiang(i));
        // }
...
//发手牌时再清理一遍
function clearMJHandCardUI_liuHuQiang() {
    // var handNode = MjClient.playui._downNode.getChildByName("handNode");
    // handNode.removeAllChildren();
...
//出牌表示状态
function ShowPutCardIcon_liuHuQiang() {
...
//清除手牌
function RemoveHandCard_liuHuQiang(node,card,off){
    if(MjClient.rePlayVideo == -1 && off != 0){
        return;
    }
    var cardArr = MjClient.HandCardArr;
...
// 初始化吃碰杠胡动作
function InitShowEatActionNode_liuHuQiang(plNode){
...
// 重置吃碰杠胡动作
function resetEatActionAnim_liuHuQiang()
{
    var jsBind = MjClient.playui.jsBind;
    var ui = [jsBind.down,jsBind.right,jsBind.left];
    var tData = MjClient.data.sData.tData;
    if(MjClient.gameType == MjClient.GAME_TYPE.HY_LIU_HU_QIANG && tData.maxPlayer == 4){
    ...
    {
        InitShowEatActionNode_liuHuQiang(ui[i]._node);
    }
}

//播放头像移动
function tableStartHeadMoveAction_liuHuQiang(node){
...
//重置3家头像位置
function reConectHeadLayout_liuHuQiang(node){
    var tData = MjClient.data.sData.tData;
    var down = node.getChildByName("down").getChildByName("head");
    var left = node.getChildByName("left").getChildByName("head");
    var right = node.getChildByName("right").getChildByName("head");
...
//根据偏移获得玩家node
function getNode_liuHuQiang(off){
    var _node = null;
    var tData = MjClient.data.sData.tData;
    if (tData.maxPlayer != 4){
 ...
 * @return {object} 玩家数据
 */
function getUIPlayer_liuHuQiang(off){
    var sData = MjClient.data.sData;
    var tData = sData.tData;
    var uids = tData.uids;
...
// 获取ui头像，通过偏移值
function getUIHeadByOff_liuHuQiang(off){
    var pl = getUIPlayer_liuHuQiang(off);
    if(!pl)
    {
        return {};
 ...
 * 下载录音, 调用 播放函数
 * */
function downAndPlayVoice_liuHuQiang(uid, filePath){
    var index = getUiOffByUid_liuHuQiang(uid);
...
function getUiOffByUid_liuHuQiang(uid){
    var sData = MjClient.data.sData;
    var tData = sData.tData;
    var uids = tData.uids;
    var selfIndex = uids.indexOf(SelfUid());
...
//设置微信头像
function setWxHead_liuHuQiang(node, d, off){
    if(d.uid == getUIHeadByOff_liuHuQiang(off).uid){
        ...
        COMMON_UI.addNobleHeadFrame(nobody,getUIPlayer_liuHuQiang(off))
    }
}

//显示玩家庄的ui
function showUserZhuangLogo_liuHuQiang(node, off){
    var tData = MjClient.data.sData.tData;
    var pl = getUIPlayer_liuHuQiang(off);
    node.zIndex = 100;
    if(tData && pl){
...
function showUserZhuangLogoZuoXing_liuHuQiang(node, off){
    var tData = MjClient.data.sData.tData;
    var pl = getUIPlayer_liuHuQiang(off);
    node.zIndex = 100;
    if(tData && pl){
...
//显示房主  add by sking
function showFangzhuTagIcon_liuHuQiang(node,off)
{
    var pl = getUIPlayer_liuHuQiang(off);
    if(!pl) //位置上没人则删掉房主标签
    {
...
// 清理ui
function clearCardUI_liuHuQiang(node,off){
    mylog("clearCardUI_liuHuQiang");
    ...
    for(var i = 0; i < children.length; i++)
    {
        ...
        {
            InitShowEatActionNode_liuHuQiang(ni.getParent());
        }
    }
}

function EatFlag_liuHuQiang(){
    var eat = MjClient.playui.jsBind.eat;
    var eatFlag = 0;
...
function huXiScore_liuHuQiang(type,card)
{
...
//刷新胡息
function UpdateHuXi_liuHuQiang(off){
    var sData = MjClient.data.sData;
    var tData = sData.tData;
    var score = 0;
    var pl = getUIPlayer_liuHuQiang(off);
    ...
    if(mjpao.length>0){
        for(var p=0;p<mjpao.length;p++){
            score += huXiScore_liuHuQiang("paoPai",mjpao[p]);
        }
    }
    ...
    if(mjpeng.length>0){
        for(var i=0;i<mjpeng.length;i++){
            score += huXiScore_liuHuQiang("peng",mjpeng[i]);
        }
    }
        ...
        var chiScore = function(cards){
            ...
            for(var k=0;k<chiXiArr.length;k++){
                ...
                if(cards.toString() == chiXiList.toString()){
                    score += huXiScore_liuHuQiang("chi",cards[0]);
                }
            }
        };
        ...
        if(mjti.length>0){
            for (var t = 0; t < mjti.length; t++) {
                if(MjClient.gameType == MjClient.GAME_TYPE.AN_HUA_PAO_HU_ZI || MjClient.majiang.getCardShowType(pl.info.uid, mjti[t]) != 0){
                    score += huXiScore_liuHuQiang("tiPai", mjti[t]);
                }
            }
        }
        //偎牌
        ...
        if(mjwei.length>0){
            for (var w = 0; w < mjwei.length; w++) {
                if(MjClient.majiang.getCardShowType(pl.info.uid, mjwei[w]) != 0) {
                    score += huXiScore_liuHuQiang("weiPai", mjwei[w]);
                }
            }
        }
    }else {
...
//设置转盘显示状态
function IsArrowVisible_liuHuQiang(){
    var pl = getUIPlayer_liuHuQiang(0);
    if (!pl)
    {
        return;
...
// 显示吃碰杠胡动作
function ShowEatActionAnim_liuHuQiang(node, actType, off){
    var delayTime = 1;
    switch(actType)
    {
        case ActionType_liuHuQiang.FANXING:
            delayTime = 1.6;
            break;
        default:
            break;
    ...
    switch(actType)
    {
        ...
        case ActionType_liuHuQiang.TI:
            ...
            break;
        case ActionType_liuHuQiang.WANGDIAO:
            ...
            break;
        case ActionType_liuHuQiang.WANGCHUANG:
            eatActionChild = eatActionNode.getChildByName("wangChuang");
            eatActionChild.visible = true;
            eatActionChild.runAction(cc.sequence(cc.delayTime(delayTime), cc.callFunc(callback)));
            break;
        case ActionType_liuHuQiang.XIAHUO:
...
//显示玩家信息
function showPlayerInfo_liuHuQiang(off, node){
    //var tData = MjClient.data.sData.tData;
    var pl = getUIPlayer_liuHuQiang(off);
    if(pl){
...
function resetChiParam_liuHuQiang(){

}

function setChiVisible_liuHuQiang(){
    var eat = MjClient.playui.jsBind.eat;
...
function showSelectEatCards_liuHuQiang(node,cardNum){
    var eat = MjClient.playui.jsBind.eat;
    eat.chi._node.visible = false;
    eat.guo._node.visible = false;
    eat.peng._node.visible = false;
    ...
    addEatCards_liuHuQiang(parent,cardArr,putCard);
}

//居中动画,数组里的节点居中排列 只关心x轴
doMoveCenterAction_liuHuQiang = function(arr,isScale){
    arr.reverse();
    var gap = 5;
...
function commitEatCards_liuHuQiang(eatCards, biArr){
    if ((MjClient.data.sData.players[SelfUid()].eatFlag & 32) > 0) {
        MjClient.showMsg("吃牌后视为过胡，确定吃吗？", function() {
            setChiVisible_liuHuQiang();
            HZChiToServer_liuHuQiang(eatCards, biArr); 
        }, function() {}, "1");
    } else {
        setChiVisible_liuHuQiang();
        HZChiToServer_liuHuQiang(eatCards, biArr); 
...
//添加可以吃的牌
function addEatCards_liuHuQiang(node,handCardArr,putCard){
    var cardPath = MjClient.cardPath_liuHuQiang;
    ...
    for(var i = 0;i < chiSet.length;i++){
        ...
        chiCloneBtn.addClickEventListener(function(chiBtn){
            ...
            if(tmpHandCardArr.indexOf(putCard) < 0){
                // setChiVisible_liuHuQiang();
                // HZChiToServer_liuHuQiang(chiArr,null);

                commitEatCards_liuHuQiang(chiArr, null);
            }else{
                var biSet = MjClient.majiang.getBiCards(tmpHandCardArr,putCard);
                if(biSet && biSet.length > 0){
                    addFirstBiCards_liuHuQiang(node,biSet,putCard,tmpHandCardArr,chiArr);
                }
            }
        });
    }
    if(ziPai.getUiLayoutType() == 0){//新布局
        doMoveCenterAction_liuHuQiang([chiBg], false);
    }
}

//添加第一个比牌
function addFirstBiCards_liuHuQiang(node,biSet,putCard,handCardArr,chiArr){
    var cardPath = MjClient.cardPath_liuHuQiang;
    if(MjClient.getAppType() == MjClient.APP_TYPE.HUNANWANGWANG || MjClient.getAppType() == MjClient.APP_TYPE.QXSYDTZ || MjClient.getAppType() == MjClient.APP_TYPE.BDHYZP){
        cardPath = ziPai.getCardFilePath();
    }
    ...
    for(var i = 0;i < biSet.length;i++){
        ...
        cloneBtn.addClickEventListener(function(btn){
            ...
            if(tmpHandCardArr.indexOf(putCard) >= 0){
                var biSet = MjClient.majiang.getBiCards(tmpHandCardArr,putCard);
                if(biSet && biSet.length > 0){
                    addSecBiCards_liuHuQiang(node,biSet,putCard,tmpHandCardArr,chiArr,biArr);
                }
            }else{
                // setChiVisible_liuHuQiang();
                // HZChiToServer_liuHuQiang(chiArr,[biArr]);
                commitEatCards_liuHuQiang(chiArr,[biArr]);
            }
        });
    }
    ...
    if(ziPai.getUiLayoutType() == 0){//新布局
        doMoveCenterAction_liuHuQiang([chiBg,biBg],isScale);
    }else{
        doMoveCenterAction2_liuHuQiang([chiBg,biBg],isScale);
    }
}

//添加第二个比牌
function addSecBiCards_liuHuQiang(node,biSet,putCard,handCardArr,chiArr,firstBiArr){
    var cardPath = MjClient.cardPath_liuHuQiang;
    if(MjClient.getAppType() == MjClient.APP_TYPE.HUNANWANGWANG || MjClient.getAppType() == MjClient.APP_TYPE.QXSYDTZ || MjClient.getAppType() == MjClient.APP_TYPE.BDHYZP){
        cardPath = ziPai.getCardFilePath();
    }
    ...
    for(var i = 0;i < biSet.length;i++){
        ...
        cloneBtn.addClickEventListener(function(btn){
            ...
            btn.setBright(false);
            // setChiVisible_liuHuQiang();
            // HZChiToServer_liuHuQiang(chiArr,[firstBiArr,biArr]);

            commitEatCards_liuHuQiang(chiArr,[firstBiArr,biArr]);
        });
    }
    ...
    if(ziPai.getUiLayoutType() == 0){//新布局
        doMoveCenterAction_liuHuQiang([chiBg,biBg0,biBg],isScale);
    }else{
        doMoveCenterAction2_liuHuQiang([chiBg,biBg0,biBg],isScale);
    }
}

function GetReadyVisible_liuHuQiang(node, off) {
    if (off < 0) {
        node.visible = false;
        return false;
    }
...
//设置玩家掉线头像
function setUserOffline_liuHuQiang(node, off){
...
//设置玩家掉线头像（邵阳新版大小结算专用）
function setRoundEndUserOffline_liuHuQiang(frame, pl){
    if(!pl){
        return;
    }
...
//显示玩家文字
function showUserChat_liuHuQiang(node, off, msg){
    var tData = MjClient.data.sData.tData;
 ...
 */
function curPlayerIsMe_liuHuQiang(off){
    var sData = MjClient.data.sData;
    var tData = sData.tData;
...
function getVisablePlayerCount_liuHuQiang(){
    var sData = MjClient.data.sData;
    var tData = sData.tData;
    var uids = tData.uids;
    var count = 0;
 ...
 */
function getXingPlayerIndex_liuHuQiang(){
    if(MjClient.data.sData.tData.maxPlayer != 4){
        return -1;
    }
    if(getVisablePlayerCount_liuHuQiang() < 3){
        return -1;
    }
    var sData = MjClient.data.sData;
    var tData = sData.tData;
 ...
 *                  提供 getUIPlayer_liuHuQiang(resoff) 获取 off位置玩家数据
 */
function getOffByXing_liuHuQiang(off){
    var resOff = off
    if(MjClient.data.sData.tData.maxPlayer != 4){
        return resOff;
    }
...
function resetHandAfterBegin_liuHuQiang(){
    var sData = MjClient.data.sData;
    var pl = sData.players[SelfUid() + ""];
...
/*
 设置弃胡状态
 */
function setQiHuState_liuHuQiang()
{
    var pl = getUIPlayer_liuHuQiang(0);
    if (pl.isQiHu) {
...
function changeMJBg_liuHuQiang(node, type)
{
    if (node.toString() == "[object ImageView]") {
        var oldFile = node.getRenderFile().file;
        var newFile = getNewMJBgFile_liuHuQiang(oldFile, type);
    ...
    for(var index in childArray)
    {
        ...
        changeMJBg_liuHuQiang(child, type);
    }
}

function getNewMJBgFile_liuHuQiang(oldFile, type)
{
    ...
    if (type == 0) {
        if (oldFile.indexOf("/type5") != -1){
            ...
            MjClient.cardPath_liuHuQiang = MjClient.cardPath_liuHuQiang.replace("/type5", "/type1");
        }else if (oldFile.indexOf("/type3") != -1){
            newFile = oldFile.replace("/type3", "/type1");
            MjClient.cardPath_liuHuQiang = MjClient.cardPath_liuHuQiang.replace("/type3", "/type1");
        }
    }
    else if (type == 1) {
        if (oldFile.indexOf("/type1") != -1){
            newFile = oldFile.replace("/type1", "/type5");
            MjClient.cardPath_liuHuQiang = MjClient.cardPath_liuHuQiang.replace("/type1", "/type5");
        }else if (oldFile.indexOf("/type3") != -1){
            newFile = oldFile.replace("/type3", "/type5");
            MjClient.cardPath_liuHuQiang = MjClient.cardPath_liuHuQiang.replace("/type3", "/type5");
        }
    }
    else if (type == 2 ) {
        if (oldFile.indexOf("/type1") != -1){
            newFile = oldFile.replace("/type1", "/type3");
            MjClient.cardPath_liuHuQiang = MjClient.cardPath_liuHuQiang.replace("/type1", "/type3");
        }else if (oldFile.indexOf("/type5") != -1){
            newFile = oldFile.replace("/type5", "/type3");
            MjClient.cardPath_liuHuQiang = MjClient.cardPath_liuHuQiang.replace("/type5", "/type3");
        }
    }
...
function getOtherWeiCards_liuHuQiang(card) {
    var sData = MjClient.data.sData;
...
function setHuCardMark_liuHuQiang(card){
...
*/
function createRadioBoxForCheckBoxs_liuHuQiang(nodes, callback, defaultIndex){
    var newobj = {};
...
*         var radioNode = createRadioBoxForCheckBoxs([node0, node1], function(index){}, 0);
*/
function createRadioBoxForCheckBoxs_liuHuQiang_new(nodes, callback, defaultIndex){
    var newobj = {};
...
//是否显示离开按钮（包括准备阶段）
function isShowBackBtn_liuHuQiang(){
    if(IsInviteVisible()){
        return true;
    }else{
        var sData = MjClient.data.sData;
...
function getAnimate_liuHuQiang(preName, len, delaytime, playCount, startIndex){
...
function delPutCard_liuHuQiang(){
    for(var i = 0; i < 4; i++){
        var node = getNode_liuHuQiang(i);
        if(node){
...
//居中动画,数组里的节点居中排列 只关心x轴
doMoveCenterAction2_liuHuQiang = function(arr,isScale){
    isScale = isScale === undefined ? true : isScale;
    var gap = 5;
    if(arr && arr.length > 0){
        var len = arr.length;
