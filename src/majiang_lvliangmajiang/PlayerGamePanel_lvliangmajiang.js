...
//向服务器发送 过消息
MjClient.MJPass2NetForlvliangmajiang = function()
{
   // console.log(">>>>>>>>>普通  过 <<<<<<<<");
...
function SetUserVisible_lvliangmajiang(node, off)
{
    //var sData = MjClient.data.sData;
    //return;
    cc.log("====================off======================" + off);
...
//处理出牌,放一张牌，打牌动作
function DealMJPut_lvliangmajiang(node, msg, off, outNum)
{
    DealMJPut_shanXiApp(node, msg, off, outNum);
}

function InitUserHandUI_lvliangmajiang(node, off)
{
    var sData = MjClient.data.sData;
    var tData = sData.tData;
    var pl = getUIPlayer(off);
    ...
    for(var i = 0; i < pl.mjput.length; i++)
    {
        ...
        DealMJPut_lvliangmajiang(node, msg, off, i);
    }
...
function initFlower_lvliangmajiang() {
    initFlower(false, false);
}


var PlayLayer_lvliangmajiang = cc.Layer.extend({
    jsBind: {
        _event: {
            mjhand: function() {
            ...
            roundEnd: function(msg) {
                ...
                function delayExe()
                {
                    ...
                    self.addChild(new EndOneView_lvliangmajiang(),500);
                }
        ...
        eat: {
            ...
            guo: {
                ...
                _touch: function(btn, eT) {
                    if (eT == 2)
                    {
                        MjClient.MJPass2NetForlvliangmajiang();
                    }
                },
                bgimg: {
                    _run: function() {
            ...
            changeui: {
                ...
                changeuibg: {
                    ...
                    guobg: {
                        guo: {
                            _touch: function(btn, eT) {
                                if (eT == 2) MjClient.MJPass2NetForlvliangmajiang();
                            }
                        },
                        fanhui: {
                            _touch: function(btn, et) {
    ...
    ctor: function() {
        this._super();
        var playui = ccs.load(res.Play_lvliangmajiang_json);
...
PlayLayer_lvliangmajiang.prototype.CardLayoutRestore = function(node, off)
{
    ...
    for(var i = 0; i < children.length; i++) //children 为 "down" 节点下的字节点
    {
        ...
        if(ci.name == "mjhand")
        {
            ...
            if(newC == null && newVal == ci.tag)
            {
               ...
               // if(MjClient.playui.isHaoDiao == true)//耗吊将时隐藏摸的牌
                //{
                  //  newC.visible = false;
                //}
                //else
    ...
    for(var i = 0; i < orders.length; i++)
    {
        ...
        if(off % 2 == 0)//自己或者对家
        {
            ...
            else
            {
                ...
                {
                    ci.x = start.x-50 + upSize.width * upS;
                }
...
// 判断吃碰杠胡的状态
PlayLayer_lvliangmajiang.prototype.EatVisibleCheck = function()
{
    var eat = MjClient.playui.jsBind.eat;
...
/*
    玩家ui，信息
 */
PlayLayer_lvliangmajiang.prototype.Node_player = function(off)
{
  ...
  //      {
    //        MjClient.playui.isHaoDiao=true;
      //  }
        ...
        //}
        SetUserVisible_lvliangmajiang(_node, off);
    ...
    UIEventBind(null, _node, "mjhand", function (eD) {
        InitUserHandUI_lvliangmajiang(_node, off);
    });
    ...
    UIEventBind(null, _node, "MJPut", function (eD) {
        ...
        // }
        DealMJPut_lvliangmajiang(_node,eD,off);
        hideTingBtn();
        if(_tingCardNumNode) _tingCardNumNode.visible = false;
