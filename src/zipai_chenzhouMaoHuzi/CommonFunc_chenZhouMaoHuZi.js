MjClient.MaxPlayerNum_chenZhouMaoHuZi = 3;
MjClient.cardPath_chenZhouMaoHuZi = "playing/paohuzi/";
var ActionType_chenZhouMaoHuZi =  //图片提示
    {
        CHI:1,
        PENG:2,
        WEI:3,
...
//初始化玩家金币、名字、胡息
function InitUserCoinAndName_chenZhouMaoHuZi(node, off){
    var pl = getUIPlayer_chenZhouMaoHuZi(off);
    if(!pl){
        return;
    }
...
//向服务器发送王闯王钓操作
function HZWangChuangToServer_chenZhouMaoHuZi(type){
    cc.log("====================HZWangChuangToServer_chenZhouMaoHuZi=================");
...
//向服务器发送打牌操作
function HZPutCardToServer_chenZhouMaoHuZi(card){
    cc.log("====================HZPutCardToServer_chenZhouMaoHuZi=================");
    MjClient.gamenet.request("pkroom.handler.tableMsg", {
        cmd: "MJPut",
        card: card
    });
...
// 向服务器发送吃牌
function HZChiToServer_chenZhouMaoHuZi(eatCards, biCards){
...
//像服务器发送碰牌
function HZPengToServer_chenZhouMaoHuZi()
{
    if (MjClient.rePlayVideo != -1) return; // 回放时候不能请求
    cc.log("====================HZPengToServer_chenZhouMaoHuZi=================");
    MjClient.gamenet.request("pkroom.handler.tableMsg", {
        cmd: "MJPeng"
    });
}

function MJPass2Net_chenZhouMaoHuZi(){
    cc.log("====================send======pass=====");
    ...
    if(IsTurnToMe() && tData.tState == TableState.waitPut){
        ...
        HZPassConfirmToServer_chenZhouMaoHuZi();
    }
};

// 向服务器发送过牌
function HZPassConfirmToServer_chenZhouMaoHuZi() {
    if (MjClient.rePlayVideo != -1)  {
        return; // 回放时候不能请求
    }
    ...
    MjClient.gamenet.request("pkroom.handler.tableMsg", {
        cmd: "MJPass",
        eatFlag: EatFlag_chenZhouMaoHuZi(),
        cardNext: tData.cardNext,
        card: tData.lastPutCard
    });
}
...
//发送跑牌命令
function HZGangToServer_chenZhouMaoHuZi(type){
    if (MjClient.rePlayVideo != -1) return; // 回放时候不能请求
    cc.log("====================HZGangToServer_chenZhouMaoHuZi=================");
    MjClient.gamenet.request("pkroom.handler.tableMsg", {
        cmd: "MJGang",
        type: type,
        eatFlag: EatFlag_chenZhouMaoHuZi()
...
//发送偎牌命令
function HZWeiToServer_chenZhouMaoHuZi(type){
    if (MjClient.rePlayVideo != -1) return; // 回放时候不能请求
    cc.log("====================HZWeiToServer_chenZhouMaoHuZi=================");
    MjClient.gamenet.request("pkroom.handler.tableMsg", {
        cmd: "HZWeiCard"
    });
}
...
//向服务器发送发牌命令
function HZNewCardToServer_chenZhouMaoHuZi(){
    if (MjClient.rePlayVideo != -1) return; // 回放时候不能请求
    cc.log("====================HZNewCardToServer_chenZhouMaoHuZi=================");
    MjClient.gamenet.request("pkroom.handler.tableMsg", {
        cmd: "HZNewCard"
    });
}
...
//向服务器发送胡牌命令
function MJHuToServer_chenZhouMaoHuZi(){
    if (MjClient.rePlayVideo != -1) return; // 回放时候不能请求
    cc.log("====================MJHuToServer=================");
    MjClient.gamenet.request("pkroom.handler.tableMsg", {
        cmd: "MJHu",
...
//添加吃的牌
function addEatCard_chenZhouMaoHuZi(node,name,mjNum,off){
    //根据牌的类型获得需要添加的节点
    ...
    var type = 2;
    //设置牌
    var newCard = getNewCard_chenZhouMaoHuZi(mjNum,type,off);
...
/**
 添加手牌(回放)
 */
function addHandCardReplay_chenZhouMaoHuZi(tagName,index,mjNum,off){
    var _node = getNode_chenZhouMaoHuZi(off);
    //根据牌的类型获得需要添加的节点
...
function cleanCardNode_chenZhouMaoHuZi(off) {
    //获取结点
    if(off != 0){
        return;
    }
...
function resetCardNode_chenZhouMaoHuZi(off) {
    if(off != 0){
        return;
    }
    var _node = getNode_chenZhouMaoHuZi(off);
...
function addOrAdjustHandCard_chenZhouMaoHuZi(tagName,index,mjNum,off){
    if(off != 0){
        return;
    }
    var _node = getNode_chenZhouMaoHuZi(off);
    ...
    if(!newCard){
        cc.log("chow", "newCard");
        newCard = getNewCard_chenZhouMaoHuZi(mjNum,1,off);
    }else{
        cc.log("chow", "getCard");
    }
    var scale_y = newCard.scaleY;
    ...
    if (MjClient.movingCard_paohuzi == newCard) {
        setCardSprite_chenZhouMaoHuZi(newCard, newCard.tag, 1, false);
    ...
    if(newCard.lastPosition){
        ...
        doMovetoAction_chenZhouMaoHuZi(newCard, cc.p(beginPoint.x, beginPoint.y + cardCount * off_y));
    }else{
        newCard.x = beginPoint.x;
        newCard.y = beginPoint.y + cardCount * off_y;
    }
    //newCard.opacity = 255;
    SetTouchCardHandler_chenZhouMaoHuZi(newCard,off);
...
function doMovetoAction_chenZhouMaoHuZi(node,endP){
    node.stopAllActions();
    var action = cc.moveTo(0.15,endP);
    node.runAction(action);
};

function resetHandCard_chenZhouMaoHuZi(posNode,off, needAction, isDelay){
    if(MjClient.rePlayVideo == -1){
        if(off == 0) {
            var handNode = posNode.getChildByName("handNode");
            handNode.visible = true;
            cleanCardNode_chenZhouMaoHuZi(off);
            ...
            for (var k = 0; k < cardArr.length; k++) {
                ...
                for (var j = 0; j < groupList.length; j++) {
                    addOrAdjustHandCard_chenZhouMaoHuZi(k, j, groupList[j], off);
                }
            }
            resetCardNode_chenZhouMaoHuZi(off);

            addTingSign_chenZhouMaoHuZi(posNode); // 添加听牌角标
            ...
            for (var i = 0; i < cardArr.length; i++) {
                ...
                if(addNode.lastPosition){
                    ...
                    doMovetoAction_chenZhouMaoHuZi(addNode, cc.p((winSize.width - totalWidth) / 2 + i * width * scale_x, 0));
                }else{
        ...
        for(var k = 0;k < cardArr.length;k++){
            ...
            for(var j = 0;j < groupList.length;j++){
                if(off == 0){
                    if(MjClient.data.sData.tData.maxPlayer == 3 || MjClient.data.sData.tData.maxPlayer == 2){
                        addHandCard_chenZhouMaoHuZi(k,j,groupList[j],off);
                    }else if(MjClient.data.sData.tData.maxPlayer == 4){
                        addHandCardReplay_chenZhouMaoHuZi(k,j,groupList[j],off);
                    }
                }else {
                    addHandCardReplay_chenZhouMaoHuZi(k,j,groupList[j],off);
                }
...
function checkCard_chenZhouMaoHuZi(node, off){
    ...
    if(cardHandArr.length > cardHand.length){
        ...
        for(var i = 0;i < cardHandArr.length; i++){
            if(cardHandArr[i] != cardHand[i]){
                RemoveHandCard_chenZhouMaoHuZi(node, cardHandArr[i], off);
...
function resetHandCard_chenZhouMaoHuZiEx(posNode,off, needAction, isDelay){
    if(off == 0) {
        ...
        checkCard_chenZhouMaoHuZi(posNode, off);
        var handNode = posNode.getChildByName("handNode");
        handNode.visible = true;
        cleanCardNode_chenZhouMaoHuZi(off);
        handNode.removeAllChildren();
 ...
 */
function addHandCard_chenZhouMaoHuZi(tagName,index,mjNum,off){
    if(off != 0){
        return;
    }
    var _node = getNode_chenZhouMaoHuZi(off);
    ...
    //设置牌
    var newCard = getNewCard_chenZhouMaoHuZi(mjNum,1,off);
    var scale_y = newCard.scaleY;
    //var parentCount = addNode.childrenCount;
    //首先根据name判断cpNode中是否已经添加
    var cardParent = addNode.getChildByTag(tagName);
    if(!cardParent){
        ...
        cardParent.width = newCard.width;
        addNode.addChild(cardParent);
        // SetTouchCardHandler_chenZhouMaoHuZi(cardParent);
    }
    ...
    cc.log("chow", "addHandCard_chenZhouMaoHuZi: name=" + newCard.getName() + " num=" + newCard.num + " tag =" + newCard.tag);
}
 ...
 **/
function getNewCard_chenZhouMaoHuZi(mjNum, type, off, isTurn){
    var _node = getNode_chenZhouMaoHuZi(off);
    var copyNode = null;
    if(type == 1){
        //手牌
    ...
    //cc.log("getNewCard_chenZhouMaoHuZi:"+mjNum+","+type+","+off+","+isTurn);
    if(mjNum > 0){
        //创建一个带有麻将信息的麻将 cp为创建后的麻将
        setCardSprite_chenZhouMaoHuZi(mjNode, mjNum, type, isTurn);
        if(type == 1){
            //只有手牌才会有点击事件
            SetTouchCardHandler_chenZhouMaoHuZi(mjNode,off);
        }
    }
...
/**
 ...
 type:类型，1:手牌 2：吃牌 3：打的牌
 */
function setCardSprite_chenZhouMaoHuZi(mjNode, mjNum, type, isTurn){
    MjClient.cardPath_chenZhouMaoHuZi = ziPai.getCardFilePath();

    if(type == 1){
        mjNode.loadTexture(MjClient.cardPath_chenZhouMaoHuZi+"hand" + mjNum + ".png");
    }
    if(type == 2){
        mjNode.loadTexture(MjClient.cardPath_chenZhouMaoHuZi+"out" + mjNum + ".png");
        if(isTurn){
            mjNode.loadTexture(MjClient.cardPath_chenZhouMaoHuZi+"huxiBG.png");
        }
    }
    if(type == 3 || type == 4){
        mjNode.loadTexture(MjClient.cardPath_chenZhouMaoHuZi+"put" + mjNum + ".png");
        if(isTurn){
            mjNode.loadTexture(MjClient.cardPath_chenZhouMaoHuZi+"normalBG.png");
        }
    }
    mjNode.tag = mjNum;
}

function checkTingCards_chenZhouMaoHuZi(putCard){
    //听牌检测
    var sData = MjClient.data.sData;

    if(MjClient.data.sData.tData.tState == TableState.waitPut && curPlayerIsMe_chenZhouMaoHuZi(0) && putCard === undefined){
        postEvent("showTingLayer", []);
        return;
    }

    if(putCard && (!curPlayerIsMe_chenZhouMaoHuZi(0) || sData.tData.tState != TableState.waitPut)){
        putCard = undefined;
    }
...
//刷新手牌移动时的坐标
var btn_oPos = null;
function updateBtnMovedPosition_chenZhouMaoHuZi(btn, eventType){
...
//MjClient.movingCard_chenZhouMaoHuZi = null;
//MjClient.cloneCard = null;
//MjClient.putCard = null;
MjClient.hasPut = false;
function SetTouchCardHandler_chenZhouMaoHuZi(card,off){
    ...
    card.addTouchEventListener(function(btn,eventType){
        ...
        if(eventType == ccui.Widget.TOUCH_BEGAN){
            ...
            setCardSprite_chenZhouMaoHuZi(cloneCard, cardTag, 1, false);
            ...
            btn.x = touchPos.x;
            btn.y = touchPos.y;
            // updateBtnMovedPosition_chenZhouMaoHuZi(btn, eventType);
            ShowPutCardIcon_chenZhouMaoHuZi();

            checkTingCards_chenZhouMaoHuZi(btn.tag);
            var alignWidth = btn.scale * btn.width;
            setCardSprite_chenZhouMaoHuZi(btn, btn.tag, 3, false);
        ...
        if(eventType == ccui.Widget.TOUCH_ENDED ||
            ...
            var pl = getUIPlayer_chenZhouMaoHuZi(0);
            var tData = MjClient.data.sData.tData;
            var isPut = false;
            var cutLine = MjClient.playui.jsBind.cutLine._node;

            setCardSprite_chenZhouMaoHuZi(btn, btn.tag, 1, false);
            ...
            var putCardCb = function () {
                ...
                if(((!pl.canNotPutCard||pl.canNotPutCard.indexOf(btnTag)==-1) &&(getOtherWeiCards_chenZhouMaoHuZi(btnTag)||(pl.limitHuPutCard && pl.limitHuPutCard.indexOf(btnTag) != -1))) &&
                    ...
                    !MjClient.isCommon&& !MjClient.hasPut)
                {
                    ...
                    var putCard = getNewCard_chenZhouMaoHuZi(btnTag, 3 ,off);
                    ...
                    if(getOtherWeiCards_chenZhouMaoHuZi(btnTag)){
                        MjClient.showMsg("放偎之后这局将不能再吃碰，是否确定？",
                                ...
                                if(pl.limitHuPutCard && pl.limitHuPutCard.indexOf(btnTag) != -1){
                                    MjClient.showMsg("吃边打边，这局将只能胡"+ MjClient.majiang.getLimitHuDesc(btnTag, tData)+"，是否确定？",
                                        function(){
                                            ...
                                            HZPutCardToServer_chenZhouMaoHuZi(btnTag);
                                            MjClient.playui.ResetHandCard(MjClient.playui._downNode,0);

                                            checkTingCards_chenZhouMaoHuZi(btnTag);
                                        },
                                        function(){
                                            MjClient.playui.ResetHandCard(MjClient.playui._downNode,0);
                                            ShowPutCardIcon_chenZhouMaoHuZi();
                            ...
                            function(){
                                ...
                                checkTingCards_chenZhouMaoHuZi();
                            }, "1");
            ...
            if(!isPut || isWangBa)
            {
                ...
                if(selectParentTag == btnParent.tag){
                    ...
                    if(selectArr){
                        ...
                        MjClient.HandCardArr[btnParent.tag].splice(parseInt(btnName),1);
                        _fixArrIndex_chenZhouMaoHuZi(selectArr, btnTag, btn);
                    }
                }
...
//根据坐标Y，添加到数组对应的位置
function _fixArrIndex_chenZhouMaoHuZi(arr, btnTag, btn){
    if(arr){
        if(_isKan_chenZhouMaoHuZi(arr)){
            arr.push(btnTag);
        ...
        cc.log('_fixArrIndex_chenZhouMaoHuZi====:',arr);
    }

}

//改变数组元素的位置
function _changeArrIndex_chenZhouMaoHuZi(arr, btnTag, btn){
    if(arr && !_isKan_chenZhouMaoHuZi(arr)){
        var len = arr.length;
        for(var i = 0; i < len; i++){
            var tag = arr[i];
            if(tag == btnTag){
        ...
        _fixArrIndex_chenZhouMaoHuZi(arr, btnTag, btn);
    }
}

//数组里的是否为三个相同的牌
function _isKan_chenZhouMaoHuZi(arr){
    if(arr){
        var len = arr.length;
        if(len < 3){
            return false;
...
//断线重连之后，显示之前玩家打的牌
function DealOffLineCard_chenZhouMaoHuZi(node, off) {
    cc.log("====================DealOffLineCard_chenZhouMaoHuZi=================", off);
    var sData = MjClient.data.sData;
    var tData = sData.tData;
    ...
    if(typeof(tData.currCard) != "undefined" && tData.currCard != -1){
        ...
        if(uids[selfIndex] == uids[tData.curPlayer]){
            var putCard = getNewCard_chenZhouMaoHuZi(tData.currCard, 3, off);
    ...
    cc.log("====================DealOffLineCard_chenZhouMaoHuZi================= end", off);
}

// 通过玩家idx获取UIBind（down left right xing）
function getUIBindByIndex_chenZhouMaoHuZi(idx) {
    var sData = MjClient.data.sData;
    var tData = sData.tData;
...
// 落牌动画
function fall_chenZhouMaoHuZi() {
    var sData = MjClient.data.sData;
    var tData = sData.tData;

    // tData.lastPlayer没同步!
    var lastPlayer = (tData.curPlayer - 1 + tData.maxPlayer) % tData.maxPlayer;
    cc.log("chow", "fall_chenZhouMaoHuZi"+ " lastPlayer = " + lastPlayer);
    ...
    if (out) {
        ...
        cc.log("chow", "fall_chenZhouMaoHuZi" + JSON.stringify(out.getPosition()));
...
//发牌
MjClient.isCommon = false;
function DealNewCard_chenZhouMaoHuZi(node, msg, off) {
    //没有发牌过来的时候直接忽略
    if (!msg.newCard) {
        RemovePutCardOut_chenZhouMaoHuZi();
        MjClient.playui.EatVisibleCheck();
        return;
    }

    AddPutCard_chenZhouMaoHuZi(node, msg, off); // 注意各个玩法验证！
    //存在发牌
    var sData = MjClient.data.sData;
    var tData = sData.tData;
    var uids = tData.uids;
    ...
    if (uids[selfIndex] == msg.uid) {
        cc.log("DealNewCard_chenZhouMaoHuZi  newCard@@ ", msg.newCard);
        //清理之前玩家打的牌
        ...
        var putCard = getNewCard_chenZhouMaoHuZi(msg.newCard, 3, off, showPkBack);
        var putNode = node.getChildByName("put");
        ...
        if (!msg.isCommon) {
            if (MjClient.rePlayVideo == -1) {
                ...
                for (var i = 0; i < cardArr.length; i++) {
                    ...
                    if (tmpArr.length <= 2) {
                        isAdd = true;
                        //addHandCard_chenZhouMaoHuZi(i,tmpArr.length.length + 1,msg.newCard,off);
...
function setPutCardPos_chenZhouMaoHuZi(outNode, outCard, idx, off) {
    var tData = MjClient.data.sData.tData;
    var maxPlayer = tData.maxPlayer;
    var type = ziPai.getUiLayoutType();
    var isZuoXing = false;
    cc.log("setPutCardPos_chenZhouMaoHuZi:" + isZuoXing);
...
function setJiachuiTextTips_chenZhouMaoHuZi(node, off) {
    node.visible = false;
    var tData = MjClient.data.sData.tData;
    var pl = getUIPlayer_chenZhouMaoHuZi(off);
...
function setJiachuiImgTips_chenZhouMaoHuZi(node, off) {
    node.visible = false;
    var tData = MjClient.data.sData.tData;
    var pl = getUIPlayer_chenZhouMaoHuZi(off);
...
function AddPutCard_chenZhouMaoHuZi(node, msg, off) {
    var sData = MjClient.data.sData;
    var tData = sData.tData;
    ...
    if (off_index == tData.curPlayer) { // off对应玩家为当前玩家
        ...
        var out = getNewCard_chenZhouMaoHuZi(card, 2, off);
        var count = pl.mjput.length - 1;
        var width = out.width;
        var height = out.height;
        setPutCardPos_chenZhouMaoHuZi(outNode, out, pl.mjput.length - 1, off);
...
//翻醒发牌
function DealFanXingNewCard_chenZhouMaoHuZi(card){
    var putCard = getNewCard_chenZhouMaoHuZi(card, 4, 0);
...
//王闯/王钓
function DealWangChuang_chenZhouMaoHuZi(node,msg,off){
    var sData = MjClient.data.sData;
    var tData = sData.tData;
    ...
    if(uids[selfIndex] == msg.uid){
        ...
        if(pl.wangType == 1){
            var putCard = getNewCard_chenZhouMaoHuZi(91, 3, off);
            var putNode = node.getChildByName("put");
            putCard.scaleX = putCard.width/putNode.width - 0.16;
            putCard.scaleY = putCard.width/putNode.width - 0.06;
            putCard.x = putCard.width/2;
            ...
            ShowEatActionAnim_chenZhouMaoHuZi(node, ActionType_chenZhouMaoHuZi.WANGDIAO, off);

            var delay = cc.delayTime(1.5);
            var callback = function(){
                RemovePutCardOut_chenZhouMaoHuZi();
                //HZNewCardToServer_chenZhouMaoHuZi();
            }
            //putNode.runAction(cc.sequence(delay,cc.callFunc(callback)));
        }else if(pl.wangType == 2){
            var putCard = getNewCard_chenZhouMaoHuZi(91, 3, off);
            var putCard1 = getNewCard_chenZhouMaoHuZi(91, 3, off);
            var putNode = node.getChildByName("put");
            putCard.scaleX = putCard.width/putNode.width - 0.16;
            putCard.scaleY = putCard.width/putNode.width - 0.06;
            ...
            ShowEatActionAnim_chenZhouMaoHuZi(node, ActionType_chenZhouMaoHuZi.WANGCHUANG, off);
            var delay = cc.delayTime(1.5);
            var callback = function(){
                RemovePutCardOut_chenZhouMaoHuZi();
                //HZNewCardToServer_chenZhouMaoHuZi();
            ...
            var putCard = getNewCard_chenZhouMaoHuZi(91, 3, off);
            var putCard1 = getNewCard_chenZhouMaoHuZi(91, 3, off);
            var putCard2 = getNewCard_chenZhouMaoHuZi(91, 3, off);
            var putNode = node.getChildByName("put");
            putCard.scaleX = putCard.width/putNode.width - 0.16;
            putCard.scaleY = putCard.width/putNode.width - 0.06;
            putCard1.scaleX = putCard.width/putNode.width - 0.16;
            ...
            ShowEatActionAnim_chenZhouMaoHuZi(node, ActionType_chenZhouMaoHuZi.WANGZHA, off);
            var delay = cc.delayTime(1.5);
            var callback = function(){
                RemovePutCardOut_chenZhouMaoHuZi();
                //HZNewCardToServer_chenZhouMaoHuZi();
...
//处理摸最后一张牌
function DealShowLastCard_chenZhouMaoHuZi(node, msg, off){
    cc.log("======DealShowLastCard_chenZhouMaoHuZi======= " + off);
    MjClient.hasPut = true;
    var sData = MjClient.data.sData;
    var tData = sData.tData;
    var uids = tData.uids;
    ...
    if(uids[selfIndex] == msg.uid)
    {
        ...
        setTimeout(function(){
            RemovePutCardOut_chenZhouMaoHuZi(true);
            var pl = getUIPlayer_chenZhouMaoHuZi(0);
...
//处理出牌
function DealPutCard_chenZhouMaoHuZi(node, msg, off) {
    AddPutCard_chenZhouMaoHuZi(node, msg, off); // 注意各个玩法验证！
    ...
    if (uids[selfIndex] == msg.uid) {
        ...
        if (SelfUid() != msg.uid || MjClient.rePlayVideo != -1) {
            ...
            putNode.stopAllActions();
            putNode.removeAllChildren();
            var putCard = getNewCard_chenZhouMaoHuZi(msg.card, 3, off);
            putCard.scaleX = putCard.width / putNode.width - 0.16;
            putCard.scaleY = putCard.width / putNode.width - 0.06;
            putCard.x = putCard.width / 2;
            putCard.y = putCard.height / 2;
        ...
        if (off == 0) {
            removeTingSign_chenZhouMaoHuZi(node);
        }

        if (MjClient.rePlayVideo != -1) {
            RemoveHandCard_chenZhouMaoHuZi(node, msg.card, off);
            MjClient.playui.ResetHandCard(node, off);
        }
    }
}

function calculateHintPutList_chenZhouMaoHuZi() {
...
function addTingSign_chenZhouMaoHuZi(node) {
...
function removeTingSign_chenZhouMaoHuZi(node) {
    var handNode = node.getChildByName("handNode");
    if (!handNode) {
        return;
    }
...
//吃牌
function DealChiCard_chenZhouMaoHuZi(node, msg, off) {
    cc.log("======DealChiCard_chenZhouMaoHuZi======= " + off);
    var sData = MjClient.data.sData;
    var tData = sData.tData;
    var uids = tData.uids;
    var selfIndex = (uids.indexOf(SelfUid()) + off) % MjClient.data.sData.tData.maxPlayer;
    if (tData.curPlayer == selfIndex) {
        ...
        for (var i = 0; i < tmpEatCards.length; i++) {
            RemoveHandCard_chenZhouMaoHuZi(node, tmpEatCards[i], off);
        }
            ...
            for (var k = 0; k < biCards.length; k++) {
                ...
                for (var m = 0; m < biArr.length; m++) {
                    if (tmpBiArr.indexOf(biArr[m]) >= 0) {
                        ...
                        RemoveHandCard_chenZhouMaoHuZi(node, biArr[m], off);
                    }
                }
            }
        }
        ...
        for (var i = 0; i < eatCards.length; i++) {
            var card = getNewCard_chenZhouMaoHuZi(eatCards[i], 2, off);
            if (i == 2) {
                card.setColor(cc.color(170, 170, 170));
            }
            ...
            card.zIndex = 4 - i;
            card.x = 0;
            card.y = off_count * getOffY_chenZhouMaoHuZi(node, card);
            setChiCardAnchorPoint_chenZhouMaoHuZi(node, card);
        ...
        if (biCards) {
            ...
            for (var k = 0; k < biCards.length; k++) {
                var biArr = biCards[k];
                for (var m = 0; m < biArr.length; m++) {
                    var card = getNewCard_chenZhouMaoHuZi(biArr[m], 2, off);
                    if (m == 2) {
                        card.setColor(cc.color(170, 170, 170));
                    }
                    setChiCardAnchorPoint_chenZhouMaoHuZi(node, card);
        ...
        var parentPos = getChiCardParentPosition_chenZhouMaoHuZi(node, cardWidth, off, selfColCount);
        var moveX0 = 0;
        ...
        parent.x = moveX0;
        setCardParentPosY_chenZhouMaoHuZi(node, parent);
        ...
        if (biCards) {
            ShowEatActionAnim_chenZhouMaoHuZi(node, ActionType_chenZhouMaoHuZi.XIAHUO, off);
        } else {
            ShowEatActionAnim_chenZhouMaoHuZi(node, ActionType_chenZhouMaoHuZi.CHI, off);
        }
...
function setChiCardAnchorPoint_chenZhouMaoHuZi(node,newCard) {
    if(node.getName() == "down"){
        newCard.anchorX = 0;
        newCard.anchorY = 0;
    }else if(node.getName() == "right"){
...
function getOffY_chenZhouMaoHuZi(node,newCard) {
    var off_y = 0;
    if(node.getName() == "down"){
        off_y = newCard.height;
    }else if(node.getName() == "right"){
...
function getChiCardParentPosition_chenZhouMaoHuZi(node, newCardWidth, off, selfColCount, orignColPosX) {
...
function setCardParentPosY_chenZhouMaoHuZi(node, cardParent) {
    var eatNode = node.getChildByName("eatNode");
    if(node.getName() == "down"){
        cardParent.y = 0;
    }else if (node.getName() == "right"){
...
function getShowCardIdx_chenZhouMaoHuZi(node, chiTip) {
    var idx;
    switch(node.getName()) {
        case "down":
        case "xing":
...
// 处理碰
function DealPengCard_chenZhouMaoHuZi(node, msg, off){
    cc.log("======DealChiCard_chenZhouMaoHuZi======= ");
    var sData = MjClient.data.sData;
    var tData = sData.tData;
    var uids = tData.uids;
    var selfIndex = (uids.indexOf(SelfUid()) + off) % MjClient.data.sData.tData.maxPlayer;
    if(tData.curPlayer == selfIndex){
        ...
        for(var i = 0;i < 3; i++) {
            var card = getNewCard_chenZhouMaoHuZi(pengCard,2,off);
            cardWidth = card.width//*eatNode.scaleX;
            var childCount = parent.childrenCount;
            card.zIndex = 4 - childCount;
            card.x = 0;
            card.y = childCount * getOffY_chenZhouMaoHuZi(node, card);
            setChiCardAnchorPoint_chenZhouMaoHuZi(node, card);
            parent.addChild(card);
        }
        eatNode.addChild(parent);

        RemoveHandCard_chenZhouMaoHuZi(node,pengCard,off);
        RemoveHandCard_chenZhouMaoHuZi(node,pengCard,off);
        MjClient.playui.ResetHandCard(node, off);
        ShowEatActionAnim_chenZhouMaoHuZi(node, ActionType_chenZhouMaoHuZi.PENG, off);
        ...
        var parentPos = getChiCardParentPosition_chenZhouMaoHuZi(node, cardWidth, off, 1);
        var moveX0 = 0;
        var outNodeCount = eatNode.getChildrenCount();
        if(outNodeCount>0){
            if(node.getName() == "down" || node.getName() == "left"){
...
//偎牌
function DealWeiCard_chenZhouMaoHuZi(node, msg, off) {
    cc.log("======DealWeiCard_chenZhouMaoHuZi======= ");
    var sData = MjClient.data.sData;
    var tData = sData.tData;
    var uids = tData.uids;
    var selfIndex = (uids.indexOf(SelfUid()) + off) % MjClient.data.sData.tData.maxPlayer;
    if (tData.curPlayer == selfIndex) {
        ...
        RemovePutCardOut_chenZhouMaoHuZi();
        ShowEatActionAnim_chenZhouMaoHuZi(node, ActionType_chenZhouMaoHuZi.WEI, off);
        ...
        for (var i = 0; i < 3; i++) {
            ...
            {
                if (i == getShowCardIdx_chenZhouMaoHuZi(node, "wei")) {
                    isTurn = false;
                }
                var card = getNewCard_chenZhouMaoHuZi(weiCard, 2, off, isTurn);
            } else if (MjClient.majiang.getCardShowType(tData.uids[tData.curPlayer], weiCard) == 1) {
                var card = getNewCard_chenZhouMaoHuZi(weiCard, 2, off);
                ...
                var card = getNewCard_chenZhouMaoHuZi(weiCard, 2, off, true);
            }
            cardWidth = card.width //*eatNode.scaleX;
            card.zIndex = 4 - i;
            card.x = 0;
            card.y = i * getOffY_chenZhouMaoHuZi(node, card);
            setChiCardAnchorPoint_chenZhouMaoHuZi(node, card);
            parent.addChild(card);
        }
        eatNode.addChild(parent);

        RemoveHandCard_chenZhouMaoHuZi(node, weiCard, off);
        RemoveHandCard_chenZhouMaoHuZi(node, weiCard, off);
        MjClient.playui.ResetHandCard(node, off);
...
// 跑牌或者提牌
function DealGangCard_chenZhouMaoHuZi(node, msg, off) {
    var sData = MjClient.data.sData;
    var tData = sData.tData;
    var uids = tData.uids;
    var selfIndex = (uids.indexOf(SelfUid()) + off) % MjClient.data.sData.tData.maxPlayer;
    ...
    if (uids.indexOf(gangUid) == selfIndex) {
        ...
        if (msg.type == 2) {
            RemovePutCardOut_chenZhouMaoHuZi();
            ShowEatActionAnim_chenZhouMaoHuZi(node, ActionType_chenZhouMaoHuZi.PAO, off);
        } else {
            if (!msg.isGangHand && msg.type == 1) {
                RemovePutCardOut_chenZhouMaoHuZi();
            }
            ShowEatActionAnim_chenZhouMaoHuZi(node, ActionType_chenZhouMaoHuZi.TI, off);
        }
        ...
        for (var i = 0; i < 4; i++) {
            ...
            if (msg.type == 2) {
                isTurn = false;
                var card = getNewCard_chenZhouMaoHuZi(gangCard, 2, off, isTurn);
                ...
                MjClient.majiang.getCardShowType(tData.uids[tData.curPlayer], gangCard) == 2)
            {
                if(MjClient.gameType === MjClient.GAME_TYPE.SHAO_YANG_FANG_PAO_FA){
                    ...
                    if (i == getShowCardIdx_chenZhouMaoHuZi(node, "ti")) {
                        isTurn = false;
                    }
                    var card = getNewCard_chenZhouMaoHuZi(gangCard, 2, off, isTurn);
                }
            } else if (MjClient.majiang.getCardShowType(tData.uids[tData.curPlayer], gangCard) == 1) {
                var card = getNewCard_chenZhouMaoHuZi(gangCard, 2, off, false);
                var shade = new cc.Sprite("playing/paohuzi/huxiBG1.png");
                shade.opacity = 100;
                shade.x = shade.width / 2;
                shade.y = shade.height / 2;
                ...
                var card = getNewCard_chenZhouMaoHuZi(gangCard, 2, off, true);
            }
            cardWidth = card.width //*eatNode.scaleX;
            card.zIndex = 4 - i;
            card.x = 0;
        ...
        RemoveHandCard_chenZhouMaoHuZi(node, gangCard, off);
        RemoveHandCard_chenZhouMaoHuZi(node, gangCard, off);
        RemoveHandCard_chenZhouMaoHuZi(node, gangCard, off);
        if (msg.isGangHand) {
            RemoveHandCard_chenZhouMaoHuZi(node, gangCard, off);
        ...
        MjClient.playui.ResetHandCard(node, off);
        // addTingSign_chenZhouMaoHuZi(node); // 添加听牌角标 (提 跑可能手牌未变动)
        ...
        var parentPos = getChiCardParentPosition_chenZhouMaoHuZi(node, cardWidth, off, 1, orignColPosX);
        var moveX0 = 0;
        var outNodeCount = eatNode.getChildrenCount();
        if (outNodeCount > 0) {
            if (node.getName() == "down" || node.getName() == "left") {
...
// 处理胡
function DealHu_chenZhouMaoHuZi(node, msg, off){
    var sData = MjClient.data.sData;
    var tData = sData.tData;
    ...
    if(pl){
        pl.eatFlag = 0;
        MjClient.playui.EatVisibleCheck();
        ShowEatActionAnim_chenZhouMaoHuZi(node,ActionType_chenZhouMaoHuZi.HU,off);
    }
}

function DealAddCard_chenZhouMaoHuZi(node,msg, off){
    if(MjClient.rePlayVideo == -1 && off != 0){
        return;
    }
    var cardArr = MjClient.HandCardArr;
...
//清除打出的牌
function RemovePutCardOut_chenZhouMaoHuZi(noAction){
...
//发手牌时再清理一遍
function clearMJHandCardUI_chenZhouMaoHuZi() {
    // var handNode = MjClient.playui._downNode.getChildByName("handNode");
    // handNode.removeAllChildren();
...
//出牌表示状态
function ShowPutCardIcon_chenZhouMaoHuZi() {
...
//清除手牌
function RemoveHandCard_chenZhouMaoHuZi(node,card,off){
    if(MjClient.rePlayVideo == -1 && off != 0){
        return;
    }
    var cardArr = MjClient.HandCardArr;
...
// 初始化吃碰杠胡动作
function InitShowEatActionNode_chenZhouMaoHuZi(plNode){
...
// 重置吃碰杠胡动作
function resetEatActionAnim_chenZhouMaoHuZi()
{
    var jsBind = MjClient.playui.jsBind;
    ...
    {
        InitShowEatActionNode_chenZhouMaoHuZi(ui[i]._node);
    }
}

//播放头像移动
function tableStartHeadMoveAction_chenZhouMaoHuZi(node){
...
//重置3家头像位置
function reConectHeadLayout_chenZhouMaoHuZi(node){
    var tData = MjClient.data.sData.tData;
    var down = node.getChildByName("down").getChildByName("head");
    var left = node.getChildByName("left").getChildByName("head");
    var right = node.getChildByName("right").getChildByName("head");
    ...
    resetEatActionAnim_chenZhouMaoHuZi();
...
//根据偏移获得玩家node
function getNode_chenZhouMaoHuZi(off){
    var _node = null;
    var tData = MjClient.data.sData.tData;
    if (tData.maxPlayer != 4){
        if (off == 1 || off == 2){
 ...
 * @return {object} 玩家数据
 */
function getUIPlayer_chenZhouMaoHuZi(off){
    var sData = MjClient.data.sData;
    var tData = sData.tData;
    var uids = tData.uids;
...
// 获取ui头像，通过偏移值
function getUIHeadByOff_chenZhouMaoHuZi(off){
    var pl = getUIPlayer_chenZhouMaoHuZi(off);
    if(!pl)
    {
        return {};
 ...
 * 下载录音, 调用 播放函数
 * */
function downAndPlayVoice_chenZhouMaoHuZi(uid, filePath){
    var index = getUiOffByUid_chenZhouMaoHuZi(uid);
...
function getUiOffByUid_chenZhouMaoHuZi(uid){
    var sData = MjClient.data.sData;
    var tData = sData.tData;
    var uids = tData.uids;
    var selfIndex = uids.indexOf(SelfUid());
...
//设置微信头像
function setWxHead_chenZhouMaoHuZi(node, d, off){
    if(d.uid == getUIHeadByOff_chenZhouMaoHuZi(off).uid){
        ...
        COMMON_UI.addNobleHeadFrame(nobody,getUIPlayer_chenZhouMaoHuZi(off))
    }
}

//显示玩家庄的ui
function showUserZhuangLogo_chenZhouMaoHuZi(node, off){
    var tData = MjClient.data.sData.tData;
    var pl = getUIPlayer_chenZhouMaoHuZi(off);
    node.zIndex = 100;
    if(tData && pl){
...
function showUserZhuangLogoZuoXing_chenZhouMaoHuZi(node, off){
    var tData = MjClient.data.sData.tData;
    var pl = getUIPlayer_chenZhouMaoHuZi(off);
    node.zIndex = 100;
    if(tData && pl){
...
//显示房主  add by sking
function showFangzhuTagIcon_chenZhouMaoHuZi(node,off)
{
    var pl = getUIPlayer_chenZhouMaoHuZi(off);
    if(!pl) //位置上没人则删掉房主标签
    {
...
// 清理ui
function clearCardUI_chenZhouMaoHuZi(node,off){
    mylog("clearCardUI_chenZhouMaoHuZi");
    ...
    for(var i = 0; i < children.length; i++)
    {
        ...
        {
            InitShowEatActionNode_chenZhouMaoHuZi(ni.getParent());
        }
    }
}

function EatFlag_chenZhouMaoHuZi(){
    var eat = MjClient.playui.jsBind.eat;
    var eatFlag = 0;
...
function huXiScore_chenZhouMaoHuZi(type,card)
{
...
//刷新胡息
function UpdateHuXi_chenZhouMaoHuZi(off){
    var sData = MjClient.data.sData;
    var tData = sData.tData;
    var score = 0;
    var pl = getUIPlayer_chenZhouMaoHuZi(off);
    ...
    if(mjpao.length>0){
        for(var p=0;p<mjpao.length;p++){
            score += huXiScore_chenZhouMaoHuZi("paoPai",mjpao[p]);
        }
    }
    ...
    if(mjpeng.length>0){
        for(var i=0;i<mjpeng.length;i++){
            score += huXiScore_chenZhouMaoHuZi("peng",mjpeng[i]);
        }
    }
        ...
        var chiScore = function(cards){
            ...
            for(var k=0;k<chiXiArr.length;k++){
                ...
                if(cards.toString() == chiXiList.toString()){
                    score += huXiScore_chenZhouMaoHuZi("chi",cards[0]);
                }
            }
        };
    ...
    if(mjti.length>0){
        for (var t = 0; t < mjti.length; t++) {
            score += huXiScore_chenZhouMaoHuZi("tiPai", mjti[t]);
        }
    }
    ...
    if(mjwei && mjwei.length>0){
        for (var w = 0; w < mjwei.length; w++) {
            if(MjClient.majiang.getCardShowType(pl.info.uid, mjwei[w]) != 0) {
                score += huXiScore_chenZhouMaoHuZi("weiPai", mjwei[w]);
            }
        }
    }
    ...
    if(tData.xingPlayer == selfIndex){
        pl = getUIPlayer_chenZhouMaoHuZi(2);
        //提牌
        var mjti = pl.mjgang1;
        if(mjti.length>0){
            for(var t=0;t<mjti.length;t++){
...
//设置转盘显示状态
function IsArrowVisible_chenZhouMaoHuZi(){
    var pl = getUIPlayer_chenZhouMaoHuZi(0);
    if (!pl)
    {
        return;
...
// 显示吃碰杠胡动作
function ShowEatActionAnim_chenZhouMaoHuZi(node, actType, off){
    var delayTime = 1;
    switch(actType)
    {
        case ActionType_chenZhouMaoHuZi.FANXING:
            delayTime = 1.6;
            break;
        default:
            break;
    ...
    switch(actType)
    {
        case ActionType_chenZhouMaoHuZi.CHI:
        ...
        case ActionType_chenZhouMaoHuZi.PENG:
        ...
        case ActionType_chenZhouMaoHuZi.WEI:
        ...
        case ActionType_chenZhouMaoHuZi.HU:

            eatActionChild = eatActionNode.getChildByName("hu");
            eatActionChild.visible = true;
            break;
        case ActionType_chenZhouMaoHuZi.PAO:
            eatActionChild = eatActionNode.getChildByName("pao");
            eatActionChild.visible = true;
            eatActionChild.runAction(cc.sequence(cc.delayTime(delayTime), cc.callFunc(callback)));
            break;
        case ActionType_chenZhouMaoHuZi.TI:
            ...
            eatActionChild.runAction(cc.sequence(cc.delayTime(delayTime), cc.callFunc(callback)));
            break;
        case ActionType_chenZhouMaoHuZi.WANGDIAO:
            eatActionChild = eatActionNode.getChildByName("wangDiao");
            eatActionChild.visible = true;
            eatActionChild.runAction(cc.sequence(cc.delayTime(delayTime), cc.callFunc(callback)));
            break;
        case ActionType_chenZhouMaoHuZi.WANGCHUANG:
            eatActionChild = eatActionNode.getChildByName("wangChuang");
            eatActionChild.visible = true;
            eatActionChild.runAction(cc.sequence(cc.delayTime(delayTime), cc.callFunc(callback)));
            break;
        case ActionType_chenZhouMaoHuZi.XIAHUO:
...
//显示玩家信息
function showPlayerInfo_chenZhouMaoHuZi(off, node){
    //var tData = MjClient.data.sData.tData;
    var pl = getUIPlayer_chenZhouMaoHuZi(off);
    if(pl){
...
function resetChiParam_chenZhouMaoHuZi(){

}

function setChiVisible_chenZhouMaoHuZi(){
    var eat = MjClient.playui.jsBind.eat;
...
function showSelectEatCards_chenZhouMaoHuZi(node,cardNum){
    var eat = MjClient.playui.jsBind.eat;
    eat.chi._node.visible = false;
    eat.guo._node.visible = false;
    eat.peng._node.visible = false;
    ...
    addEatCards_chenZhouMaoHuZi(parent,cardArr,putCard);
}

//居中动画,数组里的节点居中排列 只关心x轴
doMoveCenterAction_chenZhouMaoHuZi = function(arr,isScale){
    arr.reverse();
    var gap = 5;
...
function commitEatCards_chenZhouMaoHuZi(eatCards, biArr){
    if ((MjClient.data.sData.players[SelfUid()].eatFlag & 32) > 0) {
        MjClient.showMsg("吃牌后视为过胡，确定吃吗？", function() {
            setChiVisible_chenZhouMaoHuZi();
            HZChiToServer_chenZhouMaoHuZi(eatCards, biArr);
        }, function() {}, "1");
    } else {
        setChiVisible_chenZhouMaoHuZi();
        HZChiToServer_chenZhouMaoHuZi(eatCards, biArr);
...
//添加可以吃的牌
function addEatCards_chenZhouMaoHuZi(node,handCardArr,putCard){
    var cardPath = MjClient.cardPath_chenZhouMaoHuZi;
    ...
    for(var i = 0;i < chiSet.length;i++){
        ...
        chiCloneBtn.addClickEventListener(function(chiBtn){
            ...
            if(tmpHandCardArr.indexOf(putCard) < 0){
                // setChiVisible_chenZhouMaoHuZi();
                // HZChiToServer_chenZhouMaoHuZi(chiArr,null);

                commitEatCards_chenZhouMaoHuZi(chiArr, null);
            }else{
                var biSet = MjClient.majiang.getBiCards(tmpHandCardArr,putCard);
                if(biSet && biSet.length > 0){
                    addFirstBiCards_chenZhouMaoHuZi(node,biSet,putCard,tmpHandCardArr,chiArr);
                }
            }
        });
    }
    if(ziPai.getUiLayoutType() == 0){//新布局
        doMoveCenterAction_chenZhouMaoHuZi([chiBg], false);
    }
}

//添加第一个比牌
function addFirstBiCards_chenZhouMaoHuZi(node,biSet,putCard,handCardArr,chiArr){
    var cardPath = MjClient.cardPath_chenZhouMaoHuZi;
    cardPath = ziPai.getCardFilePath();
    ...
    for(var i = 0;i < biSet.length;i++){
        ...
        cloneBtn.addClickEventListener(function(btn){
            ...
            if(tmpHandCardArr.indexOf(putCard) >= 0){
                var biSet = MjClient.majiang.getBiCards(tmpHandCardArr,putCard);
                if(biSet && biSet.length > 0){
                    addSecBiCards_chenZhouMaoHuZi(node,biSet,putCard,tmpHandCardArr,chiArr,biArr);
                }
            }else{
                // setChiVisible_chenZhouMaoHuZi();
                // HZChiToServer_chenZhouMaoHuZi(chiArr,[biArr]);
                commitEatCards_chenZhouMaoHuZi(chiArr,[biArr]);
            }
        });
    }
    ...
    if(ziPai.getUiLayoutType() == 0){//新布局
        ...
        doMoveCenterAction_chenZhouMaoHuZi([chiBg,biBg],isScale);
    }else{
        biBg.scale = chiBg.scale;
        doMoveCenterAction_chenZhouMaoHuZi2([chiBg,biBg],isScale);
    }
}

//居中动画,数组里的节点居中排列 只关心x轴
doMoveCenterAction_chenZhouMaoHuZi2 = function(arr,isScale){
    isScale = isScale === undefined ? true : isScale;
    var gap = 5;
    if(arr && arr.length > 0){
        var len = arr.length;
...
//添加第二个比牌
function addSecBiCards_chenZhouMaoHuZi(node,biSet,putCard,handCardArr,chiArr,firstBiArr){
    var cardPath = MjClient.cardPath_chenZhouMaoHuZi;
    cardPath = ziPai.getCardFilePath();
    ...
    for(var i = 0;i < biSet.length;i++){
        ...
        cloneBtn.addClickEventListener(function(btn){
            ...
            btn.setBright(false);
            // setChiVisible_chenZhouMaoHuZi();
            // HZChiToServer_chenZhouMaoHuZi(chiArr,[firstBiArr,biArr]);

            commitEatCards_chenZhouMaoHuZi(chiArr,[firstBiArr,biArr]);
        });
    }
    ...
    if(ziPai.getUiLayoutType() == 0){//新布局
        ...
        doMoveCenterAction_chenZhouMaoHuZi([chiBg,biBg0,biBg],isScale);
    }else{
        biBg.scale = chiBg.scale;
        biBg0.scale = chiBg.scale;
        doMoveCenterAction_chenZhouMaoHuZi2([chiBg,biBg0,biBg],isScale);
    }
}

function GetReadyVisible_chenZhouMaoHuZi(node, off) {
    if (off < 0) {
        node.visible = false;
        return false;
    }
...
//设置玩家掉线头像
function setUserOffline_chenZhouMaoHuZi(node, off){
...
//设置玩家掉线头像（邵阳新版大小结算专用）
function setRoundEndUserOffline_chenZhouMaoHuZi(frame, pl){
    if(!pl){
        return;
    }
...
//显示玩家文字
function showUserChat_chenZhouMaoHuZi(node, off, msg){
    var tData = MjClient.data.sData.tData;
 ...
 */
function curPlayerIsMe_chenZhouMaoHuZi(off){
    var sData = MjClient.data.sData;
    var tData = sData.tData;
...
function getVisablePlayerCount_chenZhouMaoHuZi(){
    var sData = MjClient.data.sData;
    var tData = sData.tData;
    var uids = tData.uids;
    var count = 0;
 ...
 */
function getXingPlayerIndex_chenZhouMaoHuZi(){
    if(MjClient.data.sData.tData.maxPlayer != 4){
        return -1;
    }
    if(getVisablePlayerCount_chenZhouMaoHuZi() < 3){
        return -1;
    }
    var sData = MjClient.data.sData;
    var tData = sData.tData;
...
function resetHandAfterBegin_chenZhouMaoHuZi(){
    var sData = MjClient.data.sData;
    var pl = sData.players[SelfUid() + ""];
...
/*
 设置弃胡状态
 */
function setQiHuState_chenZhouMaoHuZi()
{
    var pl = getUIPlayer_chenZhouMaoHuZi(0);
    if (pl.isQiHu) {
...
function changeMJBg_chenZhouMaoHuZi(node, type)
{
    if (node.toString() == "[object ImageView]") {
        var oldFile = node.getRenderFile().file;
        var newFile = getNewMJBgFile_chenZhouMaoHuZi(oldFile, type);
    ...
    for(var index in childArray)
    {
        ...
        changeMJBg_chenZhouMaoHuZi(child, type);
    }
}

function getNewMJBgFile_chenZhouMaoHuZi(oldFile, type)
{
    ...
    if (type == 0) {
        ...
        if (oldFile.indexOf("/MJBg1") != -1)
            ...
            MjClient.cardPath_chenZhouMaoHuZi = "playing/paohuzi/MJBg1/";
        }
        ...
        if (oldFile.indexOf("/MJBg2") != -1)
            ...
            MjClient.cardPath_chenZhouMaoHuZi = "playing/paohuzi/MJBg2/";
        }
        else if (oldFile.indexOf("playing/paohuzi/") != -1) {
...
function getOtherWeiCards_chenZhouMaoHuZi(card) {
    var sData = MjClient.data.sData;
 ...
 */
function createRadioBoxForCheckBoxs_chenZhouMaoHuZi(nodes, callback, defaultIndex){
    var newobj = {};
 ...
 *  off:玩家的实际off
 **/
function changeUIOff_chenZhouMaoHuZi(off){
...
//是否显示离开按钮（包括准备阶段）
function isShowBackBtn_chenZhouMaoHuZi(){
    if(IsInviteVisible()){
        return true;
    }else{
        var sData = MjClient.data.sData;
 ...
 */
function getAnimate_chenZhouMaoHuZi(preName, len, delaytime, playCount, startIndex){
...
function paioFenToServer_chenZhouMaoHuZi(index) {
    var tData = MjClient.data.sData.tData;
