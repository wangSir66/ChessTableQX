...
PaodekuaiHaian.prototype.CARDTPYE = PDK_CARDTPYE;
var PDK_CARDCOUNT = {};
 ...
 * @return {number}
 */
PaodekuaiHaian.prototype.calPoint = function(num) {
    if (!num)
        return -1;
 ...
 * @return {boll} 
 */
PaodekuaiHaian.prototype.isSanShun = function(oCards){
 ...
 */
PaodekuaiHaian.prototype.getSanShunAndSanPai = function(oCards){
    var countSanSame = 0;
    // 记录所有3张的数值
    var sanSameList = [];
    // 记录散牌
...
PaodekuaiHaian.prototype.countDuiNum = function(oCards){
    var num = 0;
    var cardNumByValue = {};
    for(var i in oCards){
 ...
 */
PaodekuaiHaian.prototype.isFeiJi = function(oCards){
 ...
 * @return {bool}
 */
PaodekuaiHaian.prototype.isShun = function(oCards) {
 ...
 * @return {bool}
 */
PaodekuaiHaian.prototype.isLiandui = function(oCards) { //oCards有序
...
PaodekuaiHaian.prototype.calType = function(cards, areaSelectMode) {
    ...
    if (areaSelectMode.handCardType == 1 && cardCount == 3 && numToPoint[3] && this.calPoint(cards[0]) == PDK_APOINT)
    ...
    //三个3带一张
    if(cardCount == 4&& numToPoint[3] && numToPoint[3].indexOf("3") >= 0 && cards.indexOf(HONG_TAO_3) < 0 && areaSelectMode.canZhaDanDai1)
...
// 牌点比较函数
PaodekuaiHaian.prototype.cardValueCmp = function(a, b) {
...
PaodekuaiHaian.prototype.cardSortBigToSmall = function(a, b) {
    var pa = this.calPoint(a);
    var pb = this.calPoint(b);
    if (pa == pb)
        return b - a;
 ...
 * @return {number} 返回癞子数
 */
PaodekuaiHaian.prototype.transformAndGetLaizi = function(oCards, cards) {
    var laizi = 0;
    for (var i = 0; i < oCards.length; i++) {
 ...
 */
PaodekuaiHaian.prototype.sortHandCards = function(oCards, sortType) {
    var cards = oCards.slice();
 ...
 * @return {array}
 */
PaodekuaiHaian.prototype.findNSameCard = function(hands, point, n) {
 ...
 */
PaodekuaiHaian.prototype.lieju = function(parr, num){
    var tmps = [];
...
PaodekuaiHaian.prototype.formatCards = function(oCards) {
    var info = {};
...
PaodekuaiHaian.prototype.hadCards = function(oCards, fCards) {
...
PaodekuaiHaian.prototype.indexOfCards = function(cardList, cards) {
...
PaodekuaiHaian.prototype.isSameCards = function(cards1, cards2) {
...
PaodekuaiHaian.prototype.isZhaDan = function(oCards, areaSelectMode) {
    if(!oCards || oCards.length ==0) return false;

    var cardType = this.calType(oCards, areaSelectMode);
    return (cardType == PDK_CARDTPYE.sangeA ||
...
PaodekuaiHaian.prototype.findCardByRole = function(oCards, findNum, ignoreCardValued) {
...
PaodekuaiHaian.prototype.findCardByType = function(hands, laizi, type, lastCards) {
    ...
    else if (type == PDK_CARDTPYE.sangeAdaiyi) {
        for (var aLaizi = 0; aLaizi <= laizi; aLaizi++) 
        {
            var temp1 = this.findNSameCard(hands, PDK_APOINT, 3 - aLaizi);
            if (!temp1)
                continue;
            ...
            {
                var cardPoint = this.calPoint(hands[j]);
                if(cardPoint != PDK_APOINT){
                    rets.push(laizis.concat(temp1).concat(hands[j])); 
                }
            }
        }
    }
    else if (type == PDK_CARDTPYE.sange3daiyi) {
        for (var aLaizi = 0; aLaizi <= laizi; aLaizi++) 
        {
            var newhands = this.delPoint(hands, null, [this.cardCfg.hx[3]]);
            var temp1 = this.findNSameCard(newhands, PDK_MINPOINT, 3 - aLaizi);
            if (!temp1)
                continue;
    ...
    else if (type == PDK_CARDTPYE.sidaiyi) {
        for (var aLaizi = 0; aLaizi <= laizi; aLaizi++) 
        {
            for (var i = PDK_MINPOINT; i <= PDK_MAXPOINT; i++) 
            {
...
PaodekuaiHaian.prototype.getAllMaxCard = function(oCards){
...
PaodekuaiHaian.prototype.tipCards = function(oHands, oLastCards, areaSelectMode, isNextPlayerOneCard, isFirstRound) {
    ...
    if(areaSelectMode && areaSelectMode.canZhaDanDai1) {
        ret = this.findCardByType(hands, 0, PDK_CARDTPYE.sange3daiyi);
        if (ret && ret.length > 0 && this.canPut(ret[0], oLastCards, oHands.length, areaSelectMode)) {
            if (ret) rets = rets.concat(ret)
        }
        booms = this.findCardByType(hands, 0, PDK_CARDTPYE.sidaiyi);
        for (var i = 0; i < booms.length; i++) {
            if (this.canPut(booms[i], oLastCards, oHands.length, areaSelectMode)) {
                rets.push(booms[i]);
            }
    ...
    if(areaSelectMode && areaSelectMode.handCardType == 1) {
        ...
        ret = this.findCardByType(hands, 0, PDK_CARDTPYE.sangeAdaiyi);
        if (ret && ret.length > 0 && this.canPut(ret[0], oLastCards, oHands.length, areaSelectMode)) {
            if (ret) rets = rets.concat(ret)
        }
    }
...
// 求出牌型
PaodekuaiHaian.prototype.cardsType = function(oCards, areaSelectMode) {
    if (oCards) {
        var cards = [];
 ...
 */
PaodekuaiHaian.prototype.IsSametipCards = function(tipscardsArray, areaSelectMode)
{
...
//     
/**测试 * / 
var pdk = new PaodekuaiHaian()
// console.log(pdk.cardCfg);
var c = pdk.cardCfg;

var handCards = [c.ht[3], c.mh[3], c.fk[3]];
// var putCards = [c.ht[10], c.ht[10], c.hx[8], c.hx[8], c.hx[8], c.hx[7], c.hx[7], c.hx[7], c.hx[4], c.hx[4]];
var lastCards = [c.ht[2]];
// console.log('PaodekuaiHaian.prototype.checkPut', pdk.checkPut(handCards, putCards, lastCards) );
