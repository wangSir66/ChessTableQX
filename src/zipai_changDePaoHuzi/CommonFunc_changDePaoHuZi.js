MjClient.MaxPlayerNum_changDePaoHuZi = 3;
MjClient.cardPath_changDePaoHuZi = "playing/paohuzi/";
var ActionType_changDePaoHuZi =  //图片提示
    {
        CHI:1,
        PENG:2,
        WEI:3,
...
//初始化玩家金币、名字、胡息
function InitUserCoinAndName_changDePaoHuZi(node, off){
...
//向服务器发送王闯王钓操作
function HZWangChuangToServer_changDePaoHuZi(type){
    cc.log("====================HZWangChuangToServer_changDePaoHuZi=================");
...
//向服务器发送打牌操作
function HZPutCardToServer_changDePaoHuZi(card){
    cc.log("====================HZPutCardToServer_changDePaoHuZi=================");
    MjClient.gamenet.request("pkroom.handler.tableMsg", {
        cmd: "MJPut",
        card: card
    });
...
// 向服务器发送吃牌
function HZChiToServer_changDePaoHuZi(eatCards, biCards){
...
//像服务器发送碰牌
function HZPengToServer_changDePaoHuZi()
{
    if (MjClient.rePlayVideo != -1) return; // 回放时候不能请求
    cc.log("====================HZPengToServer_changDePaoHuZi=================");
    MjClient.gamenet.request("pkroom.handler.tableMsg", {
        cmd: "MJPeng"
    });
}

function MJPass2Net_changDePaoHuZi(){
    cc.log("====================send======pass=====");
    ...
    if(IsTurnToMe() && tData.tState == TableState.waitPut){
        ...
        HZPassConfirmToServer_changDePaoHuZi();
    }
};

// 向服务器发送过牌
function HZPassConfirmToServer_changDePaoHuZi() {
    if (MjClient.rePlayVideo != -1)  {
        return; // 回放时候不能请求
    }
    ...
    MjClient.gamenet.request("pkroom.handler.tableMsg", {
        cmd: "MJPass",
        eatFlag: EatFlag_changDePaoHuZi(),
        cardNext: tData.cardNext,
        card: tData.lastPutCard
    });
}
...
//发送跑牌命令
function HZGangToServer_changDePaoHuZi(type){
    if (MjClient.rePlayVideo != -1) return; // 回放时候不能请求
    cc.log("====================HZGangToServer_changDePaoHuZi=================");
    MjClient.gamenet.request("pkroom.handler.tableMsg", {
        cmd: "MJGang",
        type: type,
        eatFlag: EatFlag_changDePaoHuZi()
...
//发送偎牌命令
function HZWeiToServer_changDePaoHuZi(type){
    if (MjClient.rePlayVideo != -1) return; // 回放时候不能请求
    cc.log("====================HZWeiToServer_changDePaoHuZi=================");
    MjClient.gamenet.request("pkroom.handler.tableMsg", {
        cmd: "HZWeiCard"
    });
}
...
//向服务器发送发牌命令
function HZNewCardToServer_changDePaoHuZi(){
    if (MjClient.rePlayVideo != -1) return; // 回放时候不能请求
    cc.log("====================HZNewCardToServer_changDePaoHuZi=================");
    MjClient.gamenet.request("pkroom.handler.tableMsg", {
        cmd: "HZNewCard"
    });
}
...
//向服务器发送胡牌命令
function MJHuToServer_changDePaoHuZi(){
    if (MjClient.rePlayVideo != -1) return; // 回放时候不能请求
    cc.log("====================MJHuToServer=================");
    MjClient.gamenet.request("pkroom.handler.tableMsg", {
        cmd: "MJHu",
...
//添加吃的牌
function addEatCard_changDePaoHuZi(node,name,mjNum,off){
    //根据牌的类型获得需要添加的节点
    ...
    var type = 2;
    //设置牌
    var newCard = getNewCard_changDePaoHuZi(mjNum,type,off);
...
/**
 添加手牌(回放)
 */
function addHandCardReplay_changDePaoHuZi(tagName,index,mjNum,off){
    var _node = getNode_changDePaoHuZi(off);
    //根据牌的类型获得需要添加的节点
...
function cleanCardNode_changDePaoHuZi(off) {
    //获取结点
    if(off != 0){
        return;
    }
...
function resetCardNode_changDePaoHuZi(off) {
    if(off != 0){
        return;
    }
    var _node = getNode_changDePaoHuZi(off);
...
function addOrAdjustHandCard_changDePaoHuZi(tagName,index,mjNum,off){
    if(off != 0){
        return;
    }
    var _node = getNode_changDePaoHuZi(off);
    ...
    if(!newCard){
        cc.log("chow", "newCard");
        newCard = getNewCard_changDePaoHuZi(mjNum,1,off);
    }else{
        cc.log("chow", "getCard");
    }
    var scale_y = newCard.scaleY;
    ...
    if (MjClient.movingCard_paohuzi == newCard) {
        setCardSprite_changDePaoHuZi(newCard, newCard.tag, 1, false);
    ...
    if(newCard.lastPosition){
        ...
        doMovetoAction_changDePaoHuZi(newCard, cc.p(beginPoint.x, beginPoint.y + cardCount * off_y));
    }else{
        newCard.x = beginPoint.x;
        newCard.y = beginPoint.y + cardCount * off_y;
    }
    //newCard.opacity = 255;
    SetTouchCardHandler_changDePaoHuZi(newCard,off);
    ...
    //cc.log("chow", "addOrAdjustHandCard_changDePaoHuZi: name=" + newCard.getName() + " num=" + newCard.num + " tag =" + newCard.tag);
}

function doMovetoAction_changDePaoHuZi(node,endP){
    node.stopAllActions();
    var action = cc.moveTo(0.15,endP);
    node.runAction(action);
};

function resetHandCard_changDePaoHuZi(posNode,off, needAction, isDelay){
    if(MjClient.rePlayVideo == -1){
        if(off == 0) {
            var handNode = posNode.getChildByName("handNode");
            handNode.visible = true;
            cleanCardNode_changDePaoHuZi(off);
            ...
            for (var k = 0; k < cardArr.length; k++) {
                ...
                for (var j = 0; j < groupList.length; j++) {
                    addOrAdjustHandCard_changDePaoHuZi(k, j, groupList[j], off);
                }
            }
            resetCardNode_changDePaoHuZi(off);

            addTingSign_changDePaoHuZi(posNode); // 添加听牌角标
            ...
            for (var i = 0; i < cardArr.length; i++) {
                ...
                if(addNode.lastPosition){
                    ...
                    doMovetoAction_changDePaoHuZi(addNode, cc.p((winSize.width - totalWidth) / 2 + i * width * scale_x, 0));
                }else{
        ...
        var handCard = null;
        if(off == 0){
            if(MjClient.MaxPlayerNum_changDePaoHuZi == 3 || MjClient.MaxPlayerNum_changDePaoHuZi == 2){
                handNode = posNode.getChildByName("handNode");
                handCard = posNode.getChildByName("handCard");
            }else if(MjClient.MaxPlayerNum_changDePaoHuZi == 4){
        ...
        for(var k = 0;k < cardArr.length;k++){
            ...
            for(var j = 0;j < groupList.length;j++){
                if(off == 0){
                    if(MjClient.MaxPlayerNum_changDePaoHuZi == 3 || MjClient.MaxPlayerNum_changDePaoHuZi == 2){
                        addHandCard_changDePaoHuZi(k,j,groupList[j],off);
                    }else if(MjClient.MaxPlayerNum_changDePaoHuZi == 4){
                        addHandCardReplay_changDePaoHuZi(k,j,groupList[j],off);
                    }
                }else {
                    addHandCardReplay_changDePaoHuZi(k,j,groupList[j],off);
                }
        ...
        if(off == 0 && (MjClient.MaxPlayerNum_changDePaoHuZi == 3 || MjClient.MaxPlayerNum_changDePaoHuZi == 2)) {
            var scale_x = handCard.scaleX;
            var winSize = MjClient.size;
...
function checkCard_changDePaoHuZi(node, off){
    ...
    if(cardHandArr.length > cardHand.length){
        ...
        for(var i = 0;i < cardHandArr.length; i++){
            if(cardHandArr[i] != cardHand[i]){
                RemoveHandCard_changDePaoHuZi(node, cardHandArr[i], off);
...
function resetHandCard_changDePaoHuZiEx(posNode,off, needAction, isDelay){
    if(off == 0) {
        ...
        checkCard_changDePaoHuZi(posNode, off);
        var handNode = posNode.getChildByName("handNode");
        handNode.visible = true;
        cleanCardNode_changDePaoHuZi(off);
        handNode.removeAllChildren();
 ...
 */
function addHandCard_changDePaoHuZi(tagName,index,mjNum,off){
    if(off != 0){
        return;
    }
    var _node = getNode_changDePaoHuZi(off);
    ...
    //设置牌
    var newCard = getNewCard_changDePaoHuZi(mjNum,1,off);
    var scale_y = newCard.scaleY;
    //var parentCount = addNode.childrenCount;
    //首先根据name判断cpNode中是否已经添加
    var cardParent = addNode.getChildByTag(tagName);
    if(!cardParent){
        ...
        cardParent.width = newCard.width;
        addNode.addChild(cardParent);
        // SetTouchCardHandler_changDePaoHuZi(cardParent);
    }
    ...
    cc.log("chow", "addHandCard_changDePaoHuZi: name=" + newCard.getName() + " num=" + newCard.num + " tag =" + newCard.tag);
}
 ...
 **/
function getNewCard_changDePaoHuZi(mjNum, type, off, isTurn){
    var _node = getNode_changDePaoHuZi(off);
    var copyNode = null;
    if(type == 1){
        //手牌
    ...
    //cc.log("getNewCard_changDePaoHuZi:"+mjNum+","+type+","+off+","+isTurn);
    if(mjNum > 0){
        //创建一个带有麻将信息的麻将 cp为创建后的麻将
        setCardSprite_changDePaoHuZi(mjNode, mjNum, type, isTurn);
        if(type == 1){
            //只有手牌才会有点击事件
            SetTouchCardHandler_changDePaoHuZi(mjNode,off);
        }
    }
...
/**
 ...
 type:类型，1:手牌 2：吃牌 3：打的牌
 */
function setCardSprite_changDePaoHuZi(mjNode, mjNum, type, isTurn){
    MjClient.cardPath_changDePaoHuZi = ziPai.getCardFilePath();
    if(ziPai.getZiPaiType() == 3){
        MjClient.cardPath_changDePaoHuZi = "playing/paohuzi/MJBg3/";
    }

    if(type == 1){
        mjNode.loadTexture(MjClient.cardPath_changDePaoHuZi+"hand" + mjNum + ".png");
    }
    if(type == 2){
        mjNode.loadTexture(MjClient.cardPath_changDePaoHuZi+"out" + mjNum + ".png");
        if(isTurn){
            mjNode.loadTexture(MjClient.cardPath_changDePaoHuZi+"huxiBG.png");
        }
    }
    if(type == 3 || type == 4){
        mjNode.loadTexture(MjClient.cardPath_changDePaoHuZi+"put" + mjNum + ".png");
        if(isTurn){
            mjNode.loadTexture(MjClient.cardPath_changDePaoHuZi+"normalBG.png");
        }
    }
    mjNode.tag = mjNum;
}

function checkTingCards_changDePaoHuZi(putCard){
    //听牌检测
    var sData = MjClient.data.sData;

    if(MjClient.data.sData.tData.tState == TableState.waitPut && curPlayerIsMe_changDePaoHuZi(0) && putCard === undefined){
        postEvent("showTingLayer", []);
        return;
    }

    if(putCard && (!curPlayerIsMe_changDePaoHuZi(0) || sData.tData.tState != TableState.waitPut)){
        putCard = undefined;
    }
...
//刷新手牌移动时的坐标
var btn_oPos = null;
function updateBtnMovedPosition_changDePaoHuZi(btn, eventType){
...
//MjClient.movingCard_changDePaoHuZi = null;
//MjClient.cloneCard = null;
//MjClient.putCard = null;
MjClient.hasPut = false;
function SetTouchCardHandler_changDePaoHuZi(card,off){
    ...
    card.addTouchEventListener(function(btn,eventType){
        ...
        if(eventType == ccui.Widget.TOUCH_BEGAN){
            ...
            setCardSprite_changDePaoHuZi(cloneCard, cardTag, 1, false);
            ...
            btn.x = touchPos.x;
            btn.y = touchPos.y;
            // updateBtnMovedPosition_changDePaoHuZi(btn, eventType);
            ShowPutCardIcon_changDePaoHuZi();

            checkTingCards_changDePaoHuZi(btn.tag);
            var alignWidth = btn.scale * btn.width;
            setCardSprite_changDePaoHuZi(btn, btn.tag, 3, false);
        ...
        if(eventType == ccui.Widget.TOUCH_ENDED ||
            ...
            var pl = getUIPlayer_changDePaoHuZi(0);
            var tData = MjClient.data.sData.tData;
            var isPut = false;
            ...
            setCardSprite_changDePaoHuZi(btn, btn.tag, 1, false);
            ...
            var putCardCb = function () {
                ...
                if(((!pl.canNotPutCard||pl.canNotPutCard.indexOf(btnTag)==-1) &&(getOtherWeiCards_changDePaoHuZi(btnTag)||(pl.limitHuPutCard && pl.limitHuPutCard.indexOf(btnTag) != -1))) &&
                    ...
                    !MjClient.isCommon&& !MjClient.hasPut)
                {
                    ...
                    var putCard = getNewCard_changDePaoHuZi(btnTag, 3 ,off);
                    ...
                    var p = btnParent.convertToWorldSpace(cc.p(btn.x, btn.y));
                    ...
                    if(getOtherWeiCards_changDePaoHuZi(btnTag)){
                                ...
                                if(pl.limitHuPutCard && pl.limitHuPutCard.indexOf(btnTag) != -1){
                                    MjClient.showMsg("吃边打边，这局将只能胡"+ MjClient.majiang.getLimitHuDesc(btnTag, tData)+"，是否确定？",
                                        function(){
                                            ...
                                            HZPutCardToServer_changDePaoHuZi(btnTag);
                                            MjClient.playui.ResetHandCard(MjClient.playui._downNode,0);

                                            checkTingCards_changDePaoHuZi(btnTag);
                                        },
                                        function(){
                                            MjClient.playui.ResetHandCard(MjClient.playui._downNode,0);
                                            ShowPutCardIcon_changDePaoHuZi();
            ...
            if(!isPut || isWangBa)
            {
                ...
                if(selectParentTag == btnParent.tag){
                    ...
                    if(selectArr){
                        //_changeArrIndex_changDePaoHuZi(selectArr, btnTag, btn);
                        MjClient.HandCardArr[btnParent.tag].splice(parseInt(btnName),1);
                        _fixArrIndex_changDePaoHuZi(selectArr, btnTag, btn);
                    }
                }
...
//根据坐标Y，添加到数组对应的位置
function _fixArrIndex_changDePaoHuZi(arr, btnTag, btn){
    if(arr){
        if(_isKan_changDePaoHuZi(arr)){
            arr.push(btnTag);
        ...
        cc.log('_fixArrIndex_changDePaoHuZi====:',arr);
    }

}

//改变数组元素的位置
function _changeArrIndex_changDePaoHuZi(arr, btnTag, btn){
    if(arr && !_isKan_changDePaoHuZi(arr)){
        var len = arr.length;
        for(var i = 0; i < len; i++){
            var tag = arr[i];
            if(tag == btnTag){
        ...
        _fixArrIndex_changDePaoHuZi(arr, btnTag, btn);
    }
}

//数组里的是否为三个相同的牌
function _isKan_changDePaoHuZi(arr){
    if(arr){
        var len = arr.length;
        if(len < 3){
            return false;
...
//断线重连之后，显示之前玩家打的牌
function DealOffLineCard_changDePaoHuZi(node, off) {
    cc.log("====================DealOffLineCard_changDePaoHuZi=================", off);
    var sData = MjClient.data.sData;
    var tData = sData.tData;
    ...
    if(typeof(tData.currCard) != "undefined" && tData.currCard != -1){
        var uids = tData.uids;
        var selfIndex = (uids.indexOf(SelfUid()) + off) % MjClient.MaxPlayerNum_changDePaoHuZi;
        if(uids[selfIndex] == uids[tData.curPlayer]){
            var putCard = getNewCard_changDePaoHuZi(tData.currCard, 3, off);
    ...
    cc.log("====================DealOffLineCard_changDePaoHuZi================= end", off);
}

// 通过玩家idx获取UIBind（down left right xing）
function getUIBindByIndex_changDePaoHuZi(idx) {
    var sData = MjClient.data.sData;
    var tData = sData.tData;
...
// 落牌动画
function fall_changDePaoHuZi() {
    var sData = MjClient.data.sData;
    var tData = sData.tData;

    // tData.lastPlayer没同步!
    var lastPlayer = (tData.curPlayer - 1 + tData.maxPlayer) % tData.maxPlayer;
    cc.log("chow", "fall_changDePaoHuZi"+ " lastPlayer = " + lastPlayer);
    ...
    if (out) {
        ...
        cc.log("chow", "fall_changDePaoHuZi" + JSON.stringify(out.getPosition()));
...
//发牌
MjClient.isCommon = false;
function DealNewCard_changDePaoHuZi(node, msg, off) {
    //没有发牌过来的时候直接忽略
    if (!msg.newCard) {
        RemovePutCardOut_changDePaoHuZi();
        MjClient.playui.EatVisibleCheck();
        return;
    }

    AddPutCard_changDePaoHuZi(node, msg, off); // 注意各个玩法验证！
    //存在发牌
    var sData = MjClient.data.sData;
    var tData = sData.tData;
    var uids = tData.uids;
    ...
    if (uids[selfIndex] == msg.uid) {
        cc.log("DealNewCard_changDePaoHuZi  newCard@@ ", msg.newCard);
        //清理之前玩家打的牌
        var lastPlayer = tData.lastPlayer;
        if (lastPlayer != -1) {
            //var lastOff = (off + MjClient.MaxPlayerNum_changDePaoHuZi - selfIndex) % MjClient.MaxPlayerNum_changDePaoHuZi;
            // RemovePutCardOut_changDePaoHuZi();
        ...
        if (MjClient.rePlayVideo == -1){
            if (SelfUid() != msg.uid){
                if (msg.mjHide.indexOf(msg.newCard) >= 0){
                    showPkBack = true;
                }
            }
        }
        ...
        var putCard = getNewCard_changDePaoHuZi(msg.newCard, 3, off, showPkBack);
        var putNode = node.getChildByName("put");
        ...
        if (!msg.isCommon) {
            if (MjClient.rePlayVideo == -1) {
                ...
                for (var i = 0; i < cardArr.length; i++) {
                    ...
                    if (tmpArr.length <= 2) {
                        isAdd = true;
                        //addHandCard_changDePaoHuZi(i,tmpArr.length.length + 1,msg.newCard,off);
...
function setPutCardPos_changDePaoHuZi(outNode, outCard, idx, off) {
    var tData = MjClient.data.sData.tData;
    var maxPlayer = tData.maxPlayer;
    var type = ziPai.getUiLayoutType();
    var isZuoXing = false;
    cc.log("setPutCardPos_changDePaoHuZi:" + isZuoXing);
...
function setJiachuiTextTips_changDePaoHuZi(node, off) {
    node.visible = false;
    var tData = MjClient.data.sData.tData;
    var pl = getUIPlayer_changDePaoHuZi(off);
...
function setJiachuiImgTips_changDePaoHuZi(node, off) {
    node.visible = false;
    var tData = MjClient.data.sData.tData;
    var pl = getUIPlayer_changDePaoHuZi(off);
...
function AddPutCard_changDePaoHuZi(node, msg, off) {
    var sData = MjClient.data.sData;
    var tData = sData.tData;
    var off_index = (tData.uids.indexOf(SelfUid()) + off) % MjClient.MaxPlayerNum_changDePaoHuZi;
    if (off_index == tData.curPlayer) { // off对应玩家为当前玩家
        ...
        var out = getNewCard_changDePaoHuZi(card, 2, off);
        var count = pl.mjput.length - 1;
        var width = out.width;
        var height = out.height;
        setPutCardPos_changDePaoHuZi(outNode, out, pl.mjput.length - 1, off);
        ...
        if(maxPlayer == 2){
                ...
                cc.log("chow", "setPutCardPos_changDe off = " + off + " row = " + row + " line = " + line);
                if (off == 0) {
                    out.anchorX = 1;
                    out.anchorY = 0;
                ...
                cc.log("chow", "setPutCardPos_changDe" + JSON.stringify(out.getPosition()));
            }
        else {
            if (off == 0) { // todo
                    out.anchorX = 1;
                    ...
                    if (MjClient.MaxPlayerNum_changDePaoHuZi == 3) {
                        out.anchorX = 1;
                        out.anchorY = 1;
                        out.x = outNode.width - count * width;
...
//翻醒发牌
function DealFanXingNewCard_changDePaoHuZi(card){
    var putCard = getNewCard_changDePaoHuZi(card, 4, 0);
...
//王闯/王钓
function DealWangChuang_changDePaoHuZi(node,msg,off){
    var sData = MjClient.data.sData;
    var tData = sData.tData;
    ...
    if(uids[selfIndex] == msg.uid){
        ...
        if(pl.wangType == 1){
            var putCard = getNewCard_changDePaoHuZi(91, 3, off);
            var putNode = node.getChildByName("put");
            putCard.scaleX = putCard.width/putNode.width - 0.16;
            putCard.scaleY = putCard.width/putNode.width - 0.06;
            putCard.x = putCard.width/2;
            ...
            ShowEatActionAnim_changDePaoHuZi(node, ActionType_changDePaoHuZi.WANGDIAO, off);

            var delay = cc.delayTime(1.5);
            var callback = function(){
                RemovePutCardOut_changDePaoHuZi();
                //HZNewCardToServer_changDePaoHuZi();
            }
            //putNode.runAction(cc.sequence(delay,cc.callFunc(callback)));
        }else if(pl.wangType == 2){
            var putCard = getNewCard_changDePaoHuZi(91, 3, off);
            var putCard1 = getNewCard_changDePaoHuZi(91, 3, off);
            var putNode = node.getChildByName("put");
            putCard.scaleX = putCard.width/putNode.width - 0.16;
            putCard.scaleY = putCard.width/putNode.width - 0.06;
            ...
            ShowEatActionAnim_changDePaoHuZi(node, ActionType_changDePaoHuZi.WANGCHUANG, off);
            var delay = cc.delayTime(1.5);
            var callback = function(){
                RemovePutCardOut_changDePaoHuZi();
                //HZNewCardToServer_changDePaoHuZi();
            ...
            var putCard = getNewCard_changDePaoHuZi(91, 3, off);
            var putCard1 = getNewCard_changDePaoHuZi(91, 3, off);
            var putCard2 = getNewCard_changDePaoHuZi(91, 3, off);
            var putNode = node.getChildByName("put");
            putCard.scaleX = putCard.width/putNode.width - 0.16;
            putCard.scaleY = putCard.width/putNode.width - 0.06;
            putCard1.scaleX = putCard.width/putNode.width - 0.16;
            ...
            ShowEatActionAnim_changDePaoHuZi(node, ActionType_changDePaoHuZi.WANGZHA, off);
            var delay = cc.delayTime(1.5);
            var callback = function(){
                RemovePutCardOut_changDePaoHuZi();
                //HZNewCardToServer_changDePaoHuZi();
...
//处理摸最后一张牌
function DealShowLastCard_changDePaoHuZi(node, msg, off){
    cc.log("======DealShowLastCard_changDePaoHuZi======= " + off);
    MjClient.hasPut = true;
    var sData = MjClient.data.sData;
    var tData = sData.tData;
    var uids = tData.uids;
    ...
    if(uids[selfIndex] == msg.uid)
    {
        ...
        setTimeout(function(){
            RemovePutCardOut_changDePaoHuZi(true);
            var pl = getUIPlayer_changDePaoHuZi(0);
...
//处理出牌
function DealPutCard_changDePaoHuZi(node, msg, off) {
    AddPutCard_changDePaoHuZi(node, msg, off); // 注意各个玩法验证！
    ...
    if (uids[selfIndex] == msg.uid) {
        ...
        if (SelfUid() != msg.uid || MjClient.rePlayVideo != -1) {
            ...
            putNode.stopAllActions();
            putNode.removeAllChildren();
            var putCard = getNewCard_changDePaoHuZi(msg.card, 3, off);
            putCard.scaleX = putCard.width / putNode.width - 0.16;
            putCard.scaleY = putCard.width / putNode.width - 0.06;
            putCard.x = putCard.width / 2;
            putCard.y = putCard.height / 2;
        ...
        if (off == 0) {
            removeTingSign_changDePaoHuZi(node);
        }

        if (MjClient.rePlayVideo != -1) {
            RemoveHandCard_changDePaoHuZi(node, msg.card, off);
            MjClient.playui.ResetHandCard(node, off);
        }
    }
}

function calculateHintPutList_changDePaoHuZi() {
...
function addTingSign_changDePaoHuZi(node) {
...
function removeTingSign_changDePaoHuZi(node) {
    var handNode = node.getChildByName("handNode");
    if (!handNode) {
        return;
    }
...
//吃牌
function DealChiCard_changDePaoHuZi(node, msg, off) {
    cc.log("======DealChiCard_changDePaoHuZi======= " + off);
    var sData = MjClient.data.sData;
    var tData = sData.tData;
    var uids = tData.uids;
    var selfIndex = (uids.indexOf(SelfUid()) + off) % MjClient.MaxPlayerNum_changDePaoHuZi;
    if (tData.curPlayer == selfIndex) {
        ...
        for (var i = 0; i < tmpEatCards.length; i++) {
            RemoveHandCard_changDePaoHuZi(node, tmpEatCards[i], off);
        }
            ...
            for (var k = 0; k < biCards.length; k++) {
                ...
                for (var m = 0; m < biArr.length; m++) {
                    if (tmpBiArr.indexOf(biArr[m]) >= 0) {
                        ...
                        RemoveHandCard_changDePaoHuZi(node, biArr[m], off);
                    }
                }
            }
        }
        ...
        for (var i = 0; i < eatCards.length; i++) {
            var card = getNewCard_changDePaoHuZi(eatCards[i], 2, off);
            if (i == 2) {
                card.setColor(cc.color(170, 170, 170));
            }
            ...
            card.zIndex = 4 - i;
            card.x = 0;
            card.y = off_count * getOffY_changDePaoHuZi(node, card);
            setChiCardAnchorPoint_changDePaoHuZi(node, card);
        ...
        if (biCards) {
            ...
            for (var k = 0; k < biCards.length; k++) {
                var biArr = biCards[k];
                for (var m = 0; m < biArr.length; m++) {
                    var card = getNewCard_changDePaoHuZi(biArr[m], 2, off);
                    if (m == 2) {
                        card.setColor(cc.color(170, 170, 170));
                    }
                    setChiCardAnchorPoint_changDePaoHuZi(node, card);
        ...
        var parentPos = getChiCardParentPosition_changDePaoHuZi(node, cardWidth, off, selfColCount);
        var moveX0 = 0;
        ...
        parent.x = moveX0;
        setCardParentPosY_changDePaoHuZi(node, parent);


        var parentScale = parent.scale;
        var action5 = cc.scaleTo(0.3*0.24, parentScale);
        var delay = cc.delayTime(0.8*0.24);
        var action6 = cc.moveTo(0.8*0.24, parentPos.x, parentPos.y);
        ...
        if (biCards) {
            ShowEatActionAnim_changDePaoHuZi(node, ActionType_changDePaoHuZi.XIAHUO, off);
        } else {
            ShowEatActionAnim_changDePaoHuZi(node, ActionType_changDePaoHuZi.CHI, off);
        }
...
function setChiCardAnchorPoint_changDePaoHuZi(node,newCard) {
    if(node.getName() == "down"){
        newCard.anchorX = 0;
        newCard.anchorY = 0;
    }else if(node.getName() == "right"){
...
function getOffY_changDePaoHuZi(node,newCard) {
    var off_y = 0;
    if(node.getName() == "down"){
        off_y = newCard.height;
    }else if(node.getName() == "right"){
...
function getChiCardParentPosition_changDePaoHuZi(node, newCardWidth, off, selfColCount, msg) {
...
function setCardParentPosY_changDePaoHuZi(node, cardParent) {
    var eatNode = node.getChildByName("eatNode");
    if(node.getName() == "down"){
        cardParent.y = 0;
    }else if (node.getName() == "right"){
...
function getShowCardIdx_changDePaoHuZi(node, chiTip) {
    var idx;
    switch(node.getName()) {
        case "down":
        case "xing":
...
// 处理碰
function DealPengCard_changDePaoHuZi(node, msg, off){
    cc.log("======DealChiCard_changDePaoHuZi======= ");
    var sData = MjClient.data.sData;
    var tData = sData.tData;
    var uids = tData.uids;
    var selfIndex = (uids.indexOf(SelfUid()) + off) % MjClient.MaxPlayerNum_changDePaoHuZi;
    if(tData.curPlayer == selfIndex){
        ...
        for(var i = 0;i < 3; i++) {
            var card = getNewCard_changDePaoHuZi(pengCard,2,off);
            cardWidth = card.width//*eatNode.scaleX;
            var childCount = parent.childrenCount;
            card.zIndex = 4 - childCount;
            card.x = 0;
            card.y = childCount * getOffY_changDePaoHuZi(node, card);
            setChiCardAnchorPoint_changDePaoHuZi(node, card);
            parent.addChild(card);
        }
        eatNode.addChild(parent);

        RemoveHandCard_changDePaoHuZi(node,pengCard,off);
        RemoveHandCard_changDePaoHuZi(node,pengCard,off);
        MjClient.playui.ResetHandCard(node, off);
        ShowEatActionAnim_changDePaoHuZi(node, ActionType_changDePaoHuZi.PENG, off);
        ...
        var parentPos = getChiCardParentPosition_changDePaoHuZi(node, cardWidth, off, 1);
        var moveX0 = 0;
        var outNodeCount = eatNode.getChildrenCount();
        if(outNodeCount>0){
            if(node.getName() == "down" || node.getName() == "left"){
        ...
        var parentScale = parent.scale;
        var action5 = cc.scaleTo(0.8*0.3, parentScale);
        var delay = cc.delayTime(0.8*0.24);
        var action6 = cc.moveTo(0.8*0.24, parentPos.x, parentPos.y);
...
//偎牌
function DealWeiCard_changDePaoHuZi(node, msg, off) {
    cc.log("======DealWeiCard_changDePaoHuZi======= ");
    var sData = MjClient.data.sData;
    var tData = sData.tData;
    var uids = tData.uids;
    var selfIndex = (uids.indexOf(SelfUid()) + off) % MjClient.MaxPlayerNum_changDePaoHuZi;
    if (tData.curPlayer == selfIndex) {
        ...
        RemovePutCardOut_changDePaoHuZi();
        ShowEatActionAnim_changDePaoHuZi(node, ActionType_changDePaoHuZi.WEI, off);
        ...
        for (var i = 0; i < 3; i++) {
            ...
            {
                if (i == getShowCardIdx_changDePaoHuZi(node, "wei")) {
                    isTurn = false;
                }
                var card = getNewCard_changDePaoHuZi(weiCard, 2, off, isTurn);
            } else if (MjClient.majiang.getCardShowType(tData.uids[tData.curPlayer], weiCard) == 1) {
                var card = getNewCard_changDePaoHuZi(weiCard, 2, off);
                ...
                var card = getNewCard_changDePaoHuZi(weiCard, 2, off, true);
            }
            cardWidth = card.width //*eatNode.scaleX;
            card.zIndex = 4 - i;
            card.x = 0;
            card.y = i * getOffY_changDePaoHuZi(node, card);
            setChiCardAnchorPoint_changDePaoHuZi(node, card);
            parent.addChild(card);
        }
        eatNode.addChild(parent);

        RemoveHandCard_changDePaoHuZi(node, weiCard, off);
        RemoveHandCard_changDePaoHuZi(node, weiCard, off);
        MjClient.playui.ResetHandCard(node, off);
        ...
        var action3 = cc.callFunc(callback);
        var parentScale = parent.scale;
        var action5 = cc.scaleTo(0.3*0.8, parentScale);
        var delay = cc.delayTime(0.24*0.8);
        var action6 = cc.moveTo(0.24*0.8, parentPos.x, parentPos.y);
        parent.runAction(cc.sequence(action5,delay,action6,action3));
        // parent.runAction(cc.sequence(action2, action3));
...
// 跑牌或者提牌
function DealGangCard_changDePaoHuZi(node, msg, off) {
    var sData = MjClient.data.sData;
    var tData = sData.tData;
    var uids = tData.uids;
    var selfIndex = (uids.indexOf(SelfUid()) + off) % MjClient.MaxPlayerNum_changDePaoHuZi;
    ...
    if (uids.indexOf(gangUid) == selfIndex) {
        ...
        if (msg.type == 2) {
            RemovePutCardOut_changDePaoHuZi();
            ShowEatActionAnim_changDePaoHuZi(node, ActionType_changDePaoHuZi.PAO, off);
        } else {
            if (!msg.isGangHand && msg.type == 1) {
                RemovePutCardOut_changDePaoHuZi();
            }
            ShowEatActionAnim_changDePaoHuZi(node, ActionType_changDePaoHuZi.TI, off);
        }
        ...
        for (var i = 0; i < 4; i++) {
            ...
            if (msg.type == 2) {
                isTurn = false;
                var card = getNewCard_changDePaoHuZi(gangCard, 2, off, isTurn);
            }
            else {
                if (i == getShowCardIdx_changDePaoHuZi(node, "ti"))
                {
                    isTurn = false;
                }
                var card = getNewCard_changDePaoHuZi(gangCard, 2, off, isTurn);
        ...
        RemoveHandCard_changDePaoHuZi(node, gangCard, off);
        RemoveHandCard_changDePaoHuZi(node, gangCard, off);
        RemoveHandCard_changDePaoHuZi(node, gangCard, off);
        if (msg.isGangHand) {
            RemoveHandCard_changDePaoHuZi(node, gangCard, off);
        ...
        MjClient.playui.ResetHandCard(node, off);
        // addTingSign_changDePaoHuZi(node); // 添加听牌角标 (提 跑可能手牌未变动)
        ...
        var parentPos = getChiCardParentPosition_changDePaoHuZi(node, cardWidth, off, 1, msg);
        var moveX0 = 0;
        var outNodeCount = eatNode.getChildrenCount();
        if (outNodeCount > 0) {
            if (node.getName() == "down" || node.getName() == "left") {
...
// 处理胡
function DealHu_changDePaoHuZi(node, msg, off){
    var sData = MjClient.data.sData;
    var tData = sData.tData;
    var selfIndex = (tData.uids.indexOf(SelfUid()) + off) % MjClient.MaxPlayerNum_changDePaoHuZi;
    var pl = getUIPlayer_changDePaoHuZi(off);
    ...
    if(pl){
        pl.eatFlag = 0;
        MjClient.playui.EatVisibleCheck();
        ShowEatActionAnim_changDePaoHuZi(node,ActionType_changDePaoHuZi.HU,off);
    }
}

function DealAddCard_changDePaoHuZi(node,msg, off){
    if(MjClient.rePlayVideo == -1 && off != 0){
        return;
    }
    var cardArr = MjClient.HandCardArr;
...
//清除打出的牌
function RemovePutCardOut_changDePaoHuZi(noAction){

    var jsBind = MjClient.playui.jsBind;
    var uiList = [jsBind.down, jsBind.right, jsBind.left];
    if (MjClient.MaxPlayerNum_changDePaoHuZi == 4)
    {
        uiList = [jsBind.down, jsBind.xing, jsBind.right, jsBind.left];
    }
    if (MjClient.MaxPlayerNum_changDePaoHuZi == 2) {
...
//发手牌时再清理一遍
function clearMJHandCardUI_changDePaoHuZi() {
    // var handNode = MjClient.playui._downNode.getChildByName("handNode");
    // handNode.removeAllChildren();
...
//出牌表示状态
function ShowPutCardIcon_changDePaoHuZi() {
...
//清除手牌
function RemoveHandCard_changDePaoHuZi(node,card,off){
    if(MjClient.rePlayVideo == -1 && off != 0){
        return;
    }
    var cardArr = MjClient.HandCardArr;
...
// 初始化吃碰杠胡动作
function InitShowEatActionNode_changDePaoHuZi(plNode){
...
// 重置吃碰杠胡动作
function resetEatActionAnim_changDePaoHuZi()
{
    var jsBind = MjClient.playui.jsBind;
    ...
    var count = MjClient.MaxPlayerNum_changDePaoHuZi;
    for(var i = 0; i < count; i++)
    {
        InitShowEatActionNode_changDePaoHuZi(ui[i]._node);
    }
}

//播放头像移动
function tableStartHeadMoveAction_changDePaoHuZi(node){
...
//重置3家头像位置
function reConectHeadLayout_changDePaoHuZi(node){
    var tData = MjClient.data.sData.tData;
    var down = node.getChildByName("down").getChildByName("head");
    var left = node.getChildByName("left").getChildByName("head");
    var right = node.getChildByName("right").getChildByName("head");
    ...
    resetEatActionAnim_changDePaoHuZi();
...
//根据偏移获得玩家node
function getNode_changDePaoHuZi(off){
    var _node = null;
    var tData = MjClient.data.sData.tData;
    if (tData.maxPlayer != 4){
        if (off == 1 || off == 2){
 ...
 * @return {object} 玩家数据
 */
function getUIPlayer_changDePaoHuZi(off){
    var sData = MjClient.data.sData;
    var tData = sData.tData;
    var uids = tData.uids;
    var selfIndex = uids.indexOf(SelfUid());
    selfIndex = (selfIndex + off) % MjClient.MaxPlayerNum_changDePaoHuZi;
...
// 获取ui头像，通过偏移值
function getUIHeadByOff_changDePaoHuZi(off){
    var pl = getUIPlayer_changDePaoHuZi(off);
    if(!pl)
    {
        return {};
 ...
 * 下载录音, 调用 播放函数
 * */
function downAndPlayVoice_changDePaoHuZi(uid, filePath){
    var index = getUiOffByUid_changDePaoHuZi(uid);
...
function getUiOffByUid_changDePaoHuZi(uid){
    var sData = MjClient.data.sData;
    var tData = sData.tData;
    var uids = tData.uids;
    var selfIndex = uids.indexOf(SelfUid());
    var targetIndex = uids.indexOf(uid);
    return (targetIndex - selfIndex + MjClient.MaxPlayerNum_changDePaoHuZi) % MjClient.MaxPlayerNum_changDePaoHuZi;
}

//设置微信头像
function setWxHead_changDePaoHuZi(node, d, off){
    if(d.uid == getUIHeadByOff_changDePaoHuZi(off).uid){
        ...
        COMMON_UI.addNobleHeadFrame(nobody,getUIPlayer_changDePaoHuZi(off))
    }
}

//显示玩家庄的ui
function showUserZhuangLogo_changDePaoHuZi(node, off){
    var tData = MjClient.data.sData.tData;
    var pl = getUIPlayer_changDePaoHuZi(off);
    node.zIndex = 100;
    if(tData && pl){
...
function showUserZhuangLogoZuoXing_changDePaoHuZi(node, off){
    var tData = MjClient.data.sData.tData;
    var pl = getUIPlayer_changDePaoHuZi(off);
    node.zIndex = 100;
    if(tData && pl){
...
//显示房主  add by sking
function showFangzhuTagIcon_changDePaoHuZi(node,off)
{
    var pl = getUIPlayer_changDePaoHuZi(off);
    if(!pl) //位置上没人则删掉房主标签
    {
...
// 清理ui
function clearCardUI_changDePaoHuZi(node,off){
    mylog("clearCardUI_changDePaoHuZi");
    ...
    for(var i = 0; i < children.length; i++)
    {
        ...
        {
            InitShowEatActionNode_changDePaoHuZi(ni.getParent());
        }
    }
}

function EatFlag_changDePaoHuZi(){
    var eat = MjClient.playui.jsBind.eat;
    var eatFlag = 0;
...
function huXiScore_changDePaoHuZi(type,card)
{
...
//刷新胡息
function UpdateHuXi_changDePaoHuZi(off){
    var sData = MjClient.data.sData;
    var tData = sData.tData;
    var score = 0;
    var pl = getUIPlayer_changDePaoHuZi(off);
    ...
    if(mjpao.length>0){
        for(var p=0;p<mjpao.length;p++){
            score += huXiScore_changDePaoHuZi("paoPai",mjpao[p]);
        }
    }
    ...
    if(mjpeng.length>0){
        for(var i=0;i<mjpeng.length;i++){
            score += huXiScore_changDePaoHuZi("peng",mjpeng[i]);
        }
    }
        ...
        var chiScore = function(cards){
            ...
            for(var k=0;k<chiXiArr.length;k++){
                ...
                if(cards.toString() == chiXiList.toString()){
                    score += huXiScore_changDePaoHuZi("chi",cards[0]);
                }
            }
        };
    ...
    if(mjti.length>0){
        for (var t = 0; t < mjti.length; t++) {
            score += huXiScore_changDePaoHuZi("tiPai", mjti[t]);
        }
    }
    ...
    if(mjwei && mjwei.length>0){
        for (var w = 0; w < mjwei.length; w++) {
            if(MjClient.majiang.getCardShowType(pl.info.uid, mjwei[w]) != 0) {
                score += huXiScore_changDePaoHuZi("weiPai", mjwei[w]);
            }
        }
    }
    ...
    if(tData.xingPlayer == selfIndex){
        pl = getUIPlayer_changDePaoHuZi(2);
        //提牌
        var mjti = pl.mjgang1;
        if(mjti.length>0){
            for(var t=0;t<mjti.length;t++){
...
//设置转盘显示状态
function IsArrowVisible_changDePaoHuZi(){
    var pl = getUIPlayer_changDePaoHuZi(0);
    if (!pl)
    {
        return;
...
// 显示吃碰杠胡动作
function ShowEatActionAnim_changDePaoHuZi(node, actType, off){
    var delayTime = 1;
    switch(actType)
    {
        case ActionType_changDePaoHuZi.FANXING:
            delayTime = 1.6;
            break;
        default:
            break;
    ...
    switch(actType)
    {
        ...
        case ActionType_changDePaoHuZi.TI:
            ...
            break;
        case ActionType_changDePaoHuZi.WANGDIAO:
            ...
            break;
        case ActionType_changDePaoHuZi.WANGCHUANG:
            eatActionChild = eatActionNode.getChildByName("wangChuang");
            eatActionChild.visible = true;
            eatActionChild.runAction(cc.sequence(cc.delayTime(delayTime), cc.callFunc(callback)));
            break;
        case ActionType_changDePaoHuZi.XIAHUO:
...
//显示玩家信息
function showPlayerInfo_changDePaoHuZi(off, node){
    //var tData = MjClient.data.sData.tData;
    var pl = getUIPlayer_changDePaoHuZi(off);
    if(pl){
...
function resetChiParam_changDePaoHuZi(){

}

function setChiVisible_changDePaoHuZi(){
    var eat = MjClient.playui.jsBind.eat;
...
function showSelectEatCards_changDePaoHuZi(node,cardNum){
    var eat = MjClient.playui.jsBind.eat;
    eat.chi._node.visible = false;
    eat.guo._node.visible = false;
    eat.peng._node.visible = false;
    ...
    addEatCards_changDePaoHuZi(parent,cardArr,putCard);
}

//居中动画,数组里的节点居中排列 只关心x轴
doMoveCenterAction_changDePaoHuZi = function(arr,isScale){
    arr.reverse();
    var gap = 5;
...
function commitEatCards_changDePaoHuZi(eatCards, biArr){
    if ((MjClient.data.sData.players[SelfUid()].eatFlag & 32) > 0) {
        MjClient.showMsg("吃牌后视为过胡，确定吃吗？", function() {
            setChiVisible_changDePaoHuZi();
            HZChiToServer_changDePaoHuZi(eatCards, biArr);
        }, function() {}, "1");
    } else {
        setChiVisible_changDePaoHuZi();
        HZChiToServer_changDePaoHuZi(eatCards, biArr);
...
//添加可以吃的牌
function addEatCards_changDePaoHuZi(node,handCardArr,putCard){
    var cardPath = MjClient.cardPath_changDePaoHuZi;
    cardPath = ziPai.getCardFilePath();

    if(ziPai.getZiPaiType() == 3)
    {
        cardPath ="playing/paohuzi/MJBg3/";
    }
    ...
    for(var i = 0;i < chiSet.length;i++){
        ...
        chiCloneBtn.addClickEventListener(function(chiBtn){
            ...
            if(tmpHandCardArr.indexOf(putCard) < 0){
                // setChiVisible_changDePaoHuZi();
                // HZChiToServer_changDePaoHuZi(chiArr,null);

                commitEatCards_changDePaoHuZi(chiArr, null);
            }else{
                var biSet = MjClient.majiang.getBiCards(tmpHandCardArr,putCard);
                if(biSet && biSet.length > 0){
                    addFirstBiCards_changDePaoHuZi(node,biSet,putCard,tmpHandCardArr,chiArr);
                }
            }
        });
    }
    if(ziPai.getUiLayoutType() == 0){//新布局
        doMoveCenterAction_changDePaoHuZi([chiBg], false);
    }
}

//添加第一个比牌
function addFirstBiCards_changDePaoHuZi(node,biSet,putCard,handCardArr,chiArr){
    var cardPath = MjClient.cardPath_changDePaoHuZi;
    cardPath = ziPai.getCardFilePath();
    ...
    for(var i = 0;i < biSet.length;i++){
        ...
        cloneBtn.addClickEventListener(function(btn){
            ...
            if(tmpHandCardArr.indexOf(putCard) >= 0){
                var biSet = MjClient.majiang.getBiCards(tmpHandCardArr,putCard);
                if(biSet && biSet.length > 0){
                    addSecBiCards_changDePaoHuZi(node,biSet,putCard,tmpHandCardArr,chiArr,biArr);
                }
            }else{
                // setChiVisible_changDePaoHuZi();
                // HZChiToServer_changDePaoHuZi(chiArr,[biArr]);
                commitEatCards_changDePaoHuZi(chiArr,[biArr]);
            }
        });
    }
    ...
    if(ziPai.getUiLayoutType() == 0){//新布局
        ...
        doMoveCenterAction_changDePaoHuZi([chiBg,biBg],isScale);
    }else{
        biBg.scale = chiBg.scale;
        doMoveCenterAction_changDePaoHuZi2([chiBg,biBg],isScale);
    }
}

//居中动画,数组里的节点居中排列 只关心x轴
doMoveCenterAction_changDePaoHuZi2 = function(arr,isScale){
    isScale = isScale === undefined ? true : isScale;
    var gap = 5;
    if(arr && arr.length > 0){
        var len = arr.length;
...
//添加第二个比牌
function addSecBiCards_changDePaoHuZi(node,biSet,putCard,handCardArr,chiArr,firstBiArr){
    var cardPath = MjClient.cardPath_changDePaoHuZi;
    cardPath = ziPai.getCardFilePath();
    ...
    for(var i = 0;i < biSet.length;i++){
        ...
        cloneBtn.addClickEventListener(function(btn){
            ...
            btn.setBright(false);
            // setChiVisible_changDePaoHuZi();
            // HZChiToServer_changDePaoHuZi(chiArr,[firstBiArr,biArr]);

            commitEatCards_changDePaoHuZi(chiArr,[firstBiArr,biArr]);
        });
    }
    ...
    if(ziPai.getUiLayoutType() == 0){//新布局
        ...
        doMoveCenterAction_changDePaoHuZi([chiBg,biBg0,biBg],isScale);
    }else{
        biBg.scale = chiBg.scale;
        biBg0.scale = chiBg.scale;
        doMoveCenterAction_changDePaoHuZi2([chiBg,biBg0,biBg],isScale);
    }
}

function GetReadyVisible_changDePaoHuZi(node, off) {
    if (off < 0) {
        node.visible = false;
        return false;
    }
...
//设置玩家掉线头像
function setUserOffline_changDePaoHuZi(node, off){
...
//设置玩家掉线头像（邵阳新版大小结算专用）
function setRoundEndUserOffline_changDePaoHuZi(frame, pl){
    if(!pl){
        return;
    }
...
//显示玩家文字
function showUserChat_changDePaoHuZi(node, off, msg){
    var tData = MjClient.data.sData.tData;
 ...
 */
function curPlayerIsMe_changDePaoHuZi(off){
    var sData = MjClient.data.sData;
    var tData = sData.tData;
...
function getVisablePlayerCount_changDePaoHuZi(){
    var sData = MjClient.data.sData;
    var tData = sData.tData;
    var uids = tData.uids;
    var count = 0;
 ...
 */
function getXingPlayerIndex_changDePaoHuZi(){
    if(MjClient.MaxPlayerNum_changDePaoHuZi != 4){
        return -1;
    }
    if(getVisablePlayerCount_changDePaoHuZi() < 3){
        return -1;
    }
    var sData = MjClient.data.sData;
    var tData = sData.tData;
    ...
    var xingIndex = (zhuang + 2 + MjClient.MaxPlayerNum_changDePaoHuZi) % MjClient.MaxPlayerNum_changDePaoHuZi;
    var selfIndex = tData.uids.indexOf(SelfUid());
    xingIndex = (xingIndex - selfIndex + MjClient.MaxPlayerNum_changDePaoHuZi) % MjClient.MaxPlayerNum_changDePaoHuZi;
    return xingIndex;
}


//重新开始后，重置MjClient.HandCardArr
function resetHandAfterBegin_changDePaoHuZi(){
    var sData = MjClient.data.sData;
    var pl = sData.players[SelfUid() + ""];
...
/*
 设置弃胡状态
 */
function setQiHuState_changDePaoHuZi()
{
    var pl = getUIPlayer_changDePaoHuZi(0);
    if (pl.isQiHu) {
...
function changeMJBg_changDePaoHuZi(node, type)
{
    if (node.toString() == "[object ImageView]") {
        var oldFile = node.getRenderFile().file;
        var newFile = getNewMJBgFile_changDePaoHuZi(oldFile, type);
    ...
    for(var index in childArray)
    {
        ...
        changeMJBg_changDePaoHuZi(child, type);
    }
}

function getNewMJBgFile_changDePaoHuZi(oldFile, type)
{
    ...
    if (type == 0) {
        ...
        if (oldFile.indexOf("/MJBg1") != -1)
            ...
            MjClient.cardPath_changDePaoHuZi = "playing/paohuzi/MJBg1/";
        }
        ...
        if (oldFile.indexOf("/MJBg2") != -1)
            ...
            MjClient.cardPath_changDePaoHuZi = "playing/paohuzi/MJBg2/";
        }
        else if (oldFile.indexOf("playing/paohuzi/") != -1) {
...
function getOtherWeiCards_changDePaoHuZi(card) {
    var sData = MjClient.data.sData;
 ...
 */
function createRadioBoxForCheckBoxs_changDePaoHuZi(nodes, callback, defaultIndex){
    var newobj = {};
 ...
 *  off:玩家的实际off
 **/
function changeUIOff_changDePaoHuZi(off){
    if (MjClient.MaxPlayerNum_changDePaoHuZi != 4 && (off == 2 || off == 3)){
        return off - 1;
    }
    return off;
}
...
//是否显示离开按钮（包括准备阶段）
function isShowBackBtn_changDePaoHuZi(){
    if(IsInviteVisible()){
        return true;
    }else{
        var sData = MjClient.data.sData;
 ...
 */
function getAnimate_changDePaoHuZi(preName, len, delaytime, playCount, startIndex){
...
function paioFenToServer_changDePaoHuZi(index) {
    var tData = MjClient.data.sData.tData;
...
function setOfflineShow_changDePaoHuZi(node, pl) {
...
//裁剪字符串 超出以...表示
function sliceStrByLen_changDePaoHuZi(str,length) {
    var tempStr = str;
...
function getRoundInfo_changDePaoHuZi() {
    var tData = MjClient.data.sData.tData;
...
function setHuCardMark_changDePaoHuZi(card,clone){
