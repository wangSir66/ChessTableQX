...
//向服务器发送 过消息
MjClient.MJPass2NetForlinfenyingsanzui = function()
{
   // console.log(">>>>>>>>>普通  过 <<<<<<<<");
    cc.log("====================MJPass2NetForlinfenyingsanzui======pass=====");
...
function SetUserVisible_linfenyingsanzui(node, off)
{
    //var sData = MjClient.data.sData;
    //return;
...
//处理出牌,放一张牌，打牌动作
function DealMJPut_linfenyingsanzui(node, msg, off, outNum)
{
    DealMJPut_shanXiApp(node, msg, off, outNum);
}

// 处理杠
function DealMJGang_linfenyingsanzui(node, msg, off)
{
    ...
    for(var j = 0; j < 4; j++)
    {
        ...
        var angangkejian = tData.areaSelectMode.angangkejian;
        if(msg.gang == 3)//暗杠
        {
            if(j == 3)
            {
...
//初始化手牌的位置
function InitUserHandUI_linfenyingsanzui(node, off)
{
    var sData = MjClient.data.sData;
    var tData = sData.tData;
    var pl = getUIPlayer(off);
    ...
    for(var i = 0; i < pl.mjgang1.length; i++)
    {
        for(var j = 0; j < 4; j++)
        {
            ...
            else
            {
                if(!angangkejian && off != 0)
                {
                    getNewCard(node, "down", "gang1", -1, off);
                }else {
    ...
    for(var i = 0; i < pl.mjput.length; i++)
    {
        ...
        DealMJPut_linfenyingsanzui(node, msg, off, i);
    }
...
function initFlower_linfenyingsanzui() {
    initFlower(false, false);
}


var PlayLayer_linfenyingsanzui = cc.Layer.extend({
    jsBind: {
        _event: {
            mjhand: function() {
            ...
            roundEnd: function() {
                if (MjClient.getAppType() == MjClient.APP_TYPE.TXLINFENMJ)
                {
                    ...
                    function delayExe()
                    {
                        ...
                        self.addChild(new EndOneView_linfenyingsanzui(),500);
                    }
                    //hideCurrentTingNum();
            ...
            ting: {
                ...
                _touch: function(btn, eT) {
                    if (eT == 2)
                    {
                        ...
                        // var tingCards = MjClient.majiang.calTingSet_curr(pl.mjhand, null, pl.mjhand.concat(pl.mjpeng).concat(pl.mjgang1).concat(pl.mjgang0), pl);
            ...
            changeui: {
                ...
                changeuibg: {
                    ...
                    guobg: {
                        guo: {
                            _touch: function(btn, eT) {
                                if (eT == 2) MjClient.MJPass2NetForlinfenyingsanzui();
                            }
                        },
                        fanhui: {
                            _touch: function(btn, et) {
    ...
    ctor: function() {
        this._super();
        var playui = ccs.load("Play_linfenyingsanzui.json");
        ...
        if(pl && (pl.mjState == TableState.isReady || pl.mjState == TableState.roundFinish))
        {
            ...
            if(pl != null)
            {
                if (pl.isTing) {
                    ...
                    if (off == 0)
                    {
                        var tingSet = MjClient.majiang.calTingSet_curr(pl.mjhand, null, pl.mjhand.concat(pl.mjpeng).concat(pl.mjgang1).concat(pl.mjgang0), pl);
...
PlayLayer_linfenyingsanzui.prototype.CardLayoutRestore = function(node, off)
{
...
// 判断吃碰杠胡的状态
PlayLayer_linfenyingsanzui.prototype.EatVisibleCheck = function()
{
    var eat = MjClient.playui.jsBind.eat;
    ...
    if(tData.tState == TableState.waitPut && pl.mjState == TableState.waitPut)
    {
        if(IsTurnToMe())
        {
            ...
            if (pl.isNew && pl.eatFlag & 8) {
                cc.log("caofei PL", JSON.stringify(pl));
            ...
            if (!pl.isTing) {
                ...
                for (var i = 0; i < pl.mjhand.length; i++) {
                    ...
                    if (MjClient.majiang.canTing(cardsAfterPut, cardsAfterPut.concat(pl.mjpeng).concat(pl.mjgang1).concat(pl.mjgang0), pl)) {
            ...
            var allcards = pl.mjhand.concat(pl.mjpeng).concat(pl.mjgang1).concat(pl.mjgang0);
            var rtn = MjClient.majiang.canGang1(pl.mjpeng, pl.mjhand, pl.isTing, allcards, pl);
...
/*
    玩家ui，信息
 */
PlayLayer_linfenyingsanzui.prototype.Node_player = function(off)
{
    ...
    UIEventBind(null, _node, "initSceneData", function (eD) {
        SetUserVisible_linfenyingsanzui(_node, off);
    ...
    UIEventBind(null, _node, "mjhand", function (eD) {
        InitUserHandUI_linfenyingsanzui(_node, off);
    });
    ...
    UIEventBind(null, _node, "MJPut", function (eD) {
        ...
        // }
        DealMJPut_linfenyingsanzui(_node,eD,off);
        hideTingBtn();
        if(_tingCardNumNode) _tingCardNumNode.visible = false;
    ...
    UIEventBind(null, _node, "MJGang", function (eD) {
        DealMJGang_linfenyingsanzui(_node, eD, off);
        hideTingBtn();
        if(_skipPengIconTag) _skipPengIconTag.visible = false;
        setUserOffline(_node, off);
    });
