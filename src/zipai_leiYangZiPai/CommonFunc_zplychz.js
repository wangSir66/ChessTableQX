...
MjClient.cardPath_leiyang = "playing/ziPai/";
...
function setHandCardCommon(tagName,index,mjNum,off, area){
    ...
    if (MjClient.movingCard_paohuzi == newCard) {
        setCardSprite_leiyang(newCard, newCard.tag, 1, false);
...
function resetHandCard_leiyang(posNode,off){
    ...
    addTingSign_leiyang(posNode); // 添加听牌角标
...
function setCardSprite_leiyang(mjNode, mjNum, type, isTurn, isSelf){
    ...
    if(type == 1){
        ...
        console.log(MjClient.cardPath_leiyang+"hand" + mjNum + ".png");
    }
    if(type == 2){
...
function SetTouchCardHandler_leiyang(card,off){
    ...
    var acTime = 0.2;
    if(MjClient.getAppType() == MjClient.APP_TYPE.QXYYQP || MjClient.getAppType() == MjClient.APP_TYPE.BDHYZP || 
        MjClient.getAppType() == MjClient.APP_TYPE.HUNANWANGWANG || MjClient.getAppType() == MjClient.APP_TYPE.QXSYDTZ || 
        MjClient.getAppType() == MjClient.APP_TYPE.QXHHZP|| MjClient.getAppType() == MjClient.APP_TYPE.YLHUNANMJ){
        acTime = 0.25;
    }
    var cloneCard = null;
    var touchTime = 0;
    card.addTouchEventListener(function(btn,eventType){
        ...
        if(eventType == ccui.Widget.TOUCH_BEGAN){
            ...
            setCardSprite_leiyang(btn, btn.tag, 3, false);
        ...
        if(eventType == ccui.Widget.TOUCH_ENDED || 
            ...
            setCardSprite_leiyang(btn, btn.tag, 1, false);
...
function delPutCard_leiyang(){
    for(var i = 0; i < 4; i++){
        var node = getNode_leiyang(i);
        if(node){
...
function addEatCards_leiyang(node,handCardArr,putCard){
    ...
    for(var i = 0;i < chiSet.length;i++){
        ...
        for(var k = 0;k < cardList.length;k++){
            ...
            cardNode.loadTexture(MjClient.cardPath_leiyang+"hand" + card + ".png");
...
function addFirstBiCards_leiyang(node,biSet,putCard,handCardArr,chiArr){
    ...
    doMoveCenterAction2_leiyang([chiBg,biBg],isScale);
}
...
function addSecBiCards_leiyang(node,biSet,putCard,handCardArr,chiArr,firstBiArr){
    ...
    doMoveCenterAction2_leiyang([chiBg,biBg0,biBg],isScale);
}
...
function getNewMJBgFile_leiyang(oldFile, type)
{
    ...
    if (type == 0) {
        if (oldFile.indexOf("/type5") != -1){
            ...
            MjClient.cardPath_leiyang = MjClient.cardPath_leiyang.replace("/type5", "/type1");
        }else if (oldFile.indexOf("/type3") != -1){
            newFile = oldFile.replace("/type3", "/type1");
            MjClient.cardPath_leiyang = MjClient.cardPath_leiyang.replace("/type3", "/type1");
        }
    }
    else if (type == 1) {
        if (oldFile.indexOf("/type1") != -1){
            newFile = oldFile.replace("/type1", "/type5");
            MjClient.cardPath_leiyang = MjClient.cardPath_leiyang.replace("/type1", "/type5");
        }else if (oldFile.indexOf("/type3") != -1){
            newFile = oldFile.replace("/type3", "/type5");
            MjClient.cardPath_leiyang = MjClient.cardPath_leiyang.replace("/type3", "/type5");
        }
    }
    else if (type == 2 ) {
        if (oldFile.indexOf("/type1") != -1){
            newFile = oldFile.replace("/type1", "/type3");
            MjClient.cardPath_leiyang = MjClient.cardPath_leiyang.replace("/type1", "/type3");
        }else if (oldFile.indexOf("/type5") != -1){
            newFile = oldFile.replace("/type5", "/type3");
            MjClient.cardPath_leiyang = MjClient.cardPath_leiyang.replace("/type5", "/type3");
        }
    }
...
//是否显示离开按钮（包括准备阶段）
function isShowBackBtn_leiyang(){
    if(IsInviteVisible()){
        return true;
    }else{
        var sData = MjClient.data.sData;
...
function removeTingSign_leiyang(node) {
    var handNode = node.getChildByName("handNode");
    if (!handNode) {
        return;
    }
...
function setRoundEndUserOffline_leiyang(frame, pl){
    if(!pl){
        return;
    }
 ...
 */
function getAnimate_leiyang(preName, len, delaytime, playCount, startIndex){
...
doMoveCenterAction2_leiyang = function(arr,isScale){
