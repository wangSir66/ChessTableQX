...
(function() {
                ...
                if(biCards.length == 0 || (MjClient.gameType == MjClient.GAME_TYPE.ZP_LY_CHZ && this.remove34Left(baseArr) <= 3)) {
                    ...
                    for(var k = 0;k < biCards.length;k++){  
                        ...
                        if(array.indexOf(card) >= 0){
                            ...
                            if(secBiCards.length == 0 || (MjClient.gameType == MjClient.GAME_TYPE.ZP_LY_CHZ && this.remove34Left(array) <= 3)){
                                failCount ++;
                            }
                        }
                    }
    ...
    majiang.getBiCards = function(cardArr,card){
        ...
        for(var i = 0;i < biSet.length;i++){
            ...
            if(tmpCardArr.indexOf(card) >= 0){
                ...
                if(tmpSet.length == 0 || (MjClient.gameType == MjClient.GAME_TYPE.ZP_LY_CHZ && this.remove34Left(tmpCardArr) <= 3)){
                    indexArr.push(i);
                }else{
    ...
    majiang.getAllCardsTotal = function (){
        //衡阳放炮罚二人玩法有20张埋牌
        var tData = MjClient.data.sData.tData;
        if(MjClient.gameType === MjClient.GAME_TYPE.HENG_YANG_FANG_PAO_FA && MjClient.MaxPlayerNum_leiyang == 2){
            return 60;
        }
        //可以选择埋牌20张的玩法
        if((MjClient.gameType === MjClient.GAME_TYPE.HUAI_HUA_HONG_GUAI_WAN || MjClient.gameType === MjClient.GAME_TYPE.XIANG_TAN_PAO_HU_ZI ||
            MjClient.gameType === MjClient.GAME_TYPE.XIANG_XIANG_PAO_HU_ZI  || MjClient.gameType === MjClient.GAME_TYPE.XIANG_XIANG_GAO_HU_ZI ||
            MjClient.gameType === MjClient.GAME_TYPE.LOU_DI_FANG_PAO_FA) && MjClient.MaxPlayerNum_xiangxiang == 2 && tData.areaSelectMode.maipai){
            return 60;
        }else if(MjClient.gameType === MjClient.GAME_TYPE.SHAO_YANG_FANG_PAO_FA && MjClient.MaxPlayerNum_xiangxiang == 2 && tData.areaSelectMode.isMaiPai){
            return 60;
        }
        //岳阳耒阳字牌
        if (MjClient.gameType === MjClient.GAME_TYPE.ZP_LY_CHZ && tData.areaSelectMode.maiPaiNum > 0 && tData.maxPlayer == 2) {
            return 80-tData.areaSelectMode.maiPaiNum;
        }
        return 80;
    };
    ...
    majiang.sortByUser = function(arr){
        ...
        if(MjClient.gameType === MjClient.GAME_TYPE.XIANG_XIANG_GAO_HU_ZI || MjClient.gameType === MjClient.GAME_TYPE.LOU_DI_FANG_PAO_FA
            ...
            || MjClient.gameType === MjClient.GAME_TYPE.XIANG_TAN_PAO_HU_ZI || MjClient.gameType === MjClient.GAME_TYPE.HUAI_HUA_HONG_GUAI_WAN
    ...
    majiang.sortCard = function(hand) {
        ...
        function getLinkRow(card) {
            if (handTmp.length > 0) {
                ...
                } else if (tData && tData.areaSelectMode.isYiwushi && num[8] > 0) { // 1 - 5
                    switch (card) {
                        case 1:
                            delCard(handTmp, 1, 1);
                            delCard(handTmp, 5, 1);
                ...
                } else if (tData && tData.areaSelectMode.isYiwushi && num[9] > 0) { // 1-10  5-10
                    switch (card) {
                        case 1:
                            delCard(handTmp, 1, 1);
                            delCard(handTmp, 10, 1);
        ...
        if(MjClient.gameType === MjClient.GAME_TYPE.HUAI_HUA_HONG_GUAI_WAN){
            // 怀化  最大显示列数（10列） 处理
            var maxMatrixNum = cardMatrix.length + kanMatrix.length;
            if (maxMatrixNum > maxColNum) {
                for (var i = 1; i <= maxMatrixNum - maxColNum; i++) {
                    for (var j = cardMatrix.length-1; j >= 0 ; j--) {
        ...
        for (var i = 0; i < handMatrix.length; i++) {
            if (handMatrix[i].length == 4) {
                ...
                if (handMatrix[i][0] == handMatrix[i][1] && handMatrix[i][0] == handMatrix[i][2]) {
                    ...
                    else if (tData && tData.areaSelectMode.isYiwushi && row[0] == 1 && row[1] == 5 && row[2] == 10) {
                        huxi += 3;
                    } 
                    else if (tData && tData.areaSelectMode.isYiwushi && row[0] == 21 && row[1] == 25 && row[2] == 30) {
                        huxi += 6;
                    }
                }
            }
    ...
    majiang.getTingStats = function(sData, pl, putCard) {
    ...
    majiang.getCardShowType = function(uid, card) {
        ...
        if(MjClient.gameType == MjClient.GAME_TYPE.HUAI_HUA_HONG_GUAI_WAN){
            if(!skipPeng || skipPeng.indexOf(card) < 0){
                return 0;
            }
            return 1;
        }else{
    ...
    if (typeof(MjClient) != "undefined") {
        MjClient.majiang_zplychz_lyg = majiang;
        // DoTest();
    } else {
        module.exports = majiang;
    }

    // var mjhand = [1,1,1,2,2,2,3,3,3,4,4,4,27,27,7,7,8,9,5,6,];
    // var card = 7;
