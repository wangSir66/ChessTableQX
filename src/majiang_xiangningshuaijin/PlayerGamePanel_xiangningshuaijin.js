...
//向服务器发送 ----过消息
MjClient.MJPass2NetForxiangningshuaijin = function()
{
    // console.log(">>>>>>>>>普通  过 <<<<<<<<");
    cc.log("====================MJPass2NetForxiangningshuaijin======pass=====");
...
function SetUserVisible_xiangningshuaijin(node, off)
{
    //var sData = MjClient.data.sData;
    //return;
...
//处理出牌,放一张牌，打牌动作
function DealMJPut_xiangningshuaijin(node, msg, off, outNum)
{
    DealMJPut_shanXiApp(node, msg, off, outNum);
}

function InitUserHandUI_xiangningshuaijin(node, off)
{
    var sData = MjClient.data.sData;
    var tData = sData.tData;
    var pl = getUIPlayer(off);
...
function initFlower_xiangningshuaijin() {
    initFlower(false, false);
}


var PlayLayer_xiangningshuaijin = cc.Layer.extend({
    jsBind: {
        _event: {
            mjhand: function() {
            ...
            roundEnd: function() {
                ...
                if (MjClient.getAppType() == MjClient.APP_TYPE.TXLINFENMJ)
                {
                    ...
                    function delayExe()
                    {
                        ...
                        self.addChild(new EndOneView_xiangningshuaijin(),500);
                    }
                    //hideCurrentTingNum();
        ...
        eat: {
            ...
            ting: {
                ...
                _touch: function(btn, eT) {
                    if (eT == 2)
                    {
                        ...
                        var tingCards = {};
                        if(MjClient.majiang.isEqualHunCard(currentCard) || pl.rate > 0){//选中金牌或者已经摔过金
                            tingCards = MjClient.majiang.getCheckTingHuCards(currentCard, pl.mjhand, true);
                        }else{
                            tingCards = MjClient.majiang.getCheckTingHuCards(currentCard, pl.mjhand, false);
                        }
            ...
            changeui: {
                ...
                changeuibg: {
                    ...
                    guobg: {
                        guo: {
                            _touch: function(btn, eT) {
                                if (eT == 2) MjClient.MJPass2NetForxiangningshuaijin();
                            }
                        },
                        fanhui: {
                            _touch: function(btn, et) {
            ...
            _event: {
                ...
                // },
                mjhand: function(eD) {
                    console.log("HHH :，mjhand------1111111111111");
                    MjClient.playui.EatVisibleCheck();
                },
                waitPut: function(eD) {
                    console.log("HHH :，waitPut------11111111111");
                    var sData = MjClient.data.sData;
                    var tData = sData.tData;
    ...
    ctor: function() {
        this._super();
        var playui = ccs.load("Play_xiangningshuaijin.json");
        ...
        showBtn.addTouchEventListener(function(sender, Type) {
            switch (Type) {
                case ccui.Widget.TOUCH_ENDED:
                    ...
                    {
                        if(MjClient.majiang.isEqualHunCard(sender.cardTag) || pl.rate > 0){//选中金牌或者已经摔过金
                            tingCards = MjClient.majiang.getCheckTingHuCards(sender.cardTag, pl.mjhand, true);
                        }else{
                            tingCards = MjClient.majiang.getCheckTingHuCards(sender.cardTag, pl.mjhand, false);
                        }
                    }else{
                        if(pl.rate > 0 || MjClient.majiang.isEqualHunCard(sender.cardTag)){
                            tingCards = MjClient.majiang.calTingSet(pl.mjhand, MjClient.data.sData.tData.hunCard, true);
                        }else{
                            tingCards = MjClient.majiang.calTingSet(pl.mjhand, MjClient.data.sData.tData.hunCard, false);
                        }
                    }
        ...
        var tData = MjClient.data.sData.tData;
        if (!tData.areaSelectMode.baotingbaohu) {
            // 非报听模式的按钮状态
            COMMON_UI.tingBtnAndCards();
        } else {
...
PlayLayer_xiangningshuaijin.prototype.CardLayoutRestore = function(node, off)
{
 ...
 //   cc.log("playgamePanel_xiangningshuaijin--cardlayoutrestore--"+uiOrder);
    if(off == 1 || off == 2)
    {
 ...
 //   cc.log("playgamePanel_xiangningshuaijin--cardlayoutrestore--"+orders);
    //设置麻将大小
...
// 判断吃碰杠胡的状态
PlayLayer_xiangningshuaijin.prototype.EatVisibleCheck = function()
{
    if(MjClient.majiang != MjClient.majiang_xiangningshuaijin){
        return;
    }
    ...
    if(tData.tState == TableState.waitPut && pl.mjState == TableState.waitPut)
    {
        if(IsTurnToMe())
        {
            ...
            if (!pl.isTing) {
                ...
                if(tData.areaSelectMode.baotingbaohu && tingStat && vnode.indexOf(eat.ting._node) < 0) {
                    // 报听包干模式，已经听，vnode 没有听的按钮
            ...
            var rtn = MjClient.majiang.canGang1(pl.mjpeng, pl.mjhand, pl.isTing, pl.rate>0);
    ...
    cc.log(Object.keys(MjClient.canTingCards).length+"eatvisiblecheck");
    cc.log(IsTurnToMe());
    ...
    // if(pl.isTing){
    //     MjClient.playui._tingCardNumNode.visible=true;
    // }
    //显示，吃，碰，杠的那几张牌
...
/*
 玩家ui，信息
 */
PlayLayer_xiangningshuaijin.prototype.Node_player = function(off)
{
    ...
    UIEventBind(null, _node, "initSceneData", function (eD) {
        ...
        SetUserVisible_xiangningshuaijin(_node, off);
        ...
        if(off == 0 && pl.mjState != TableState.isReady && pl.mjState != TableState.roundFinish){
            var tingCards = {};
            if(pl.rate > 0)// && MjClient.majiang.quantingpai(pl.mjhand,MjClient.data.sData.tData.hunCard))
            {
                tingCards = MjClient.majiang.calTingSet(pl.mjhand, MjClient.data.sData.tData.hunCard, true);
            }
            else
    ...
    UIEventBind(null, _node, "mjhand", function (eD) {
        InitUserHandUI_xiangningshuaijin(_node, off);
    ...
    UIEventBind(null, _node, "MJPut", function (eD) {
        ...
        //     var tingSet = calTingSet(pl.mjhand);
        //     if(_tingCards) setTingCards(_tingCards,tingSet);
        // }
        var _tingCards = _node.getChildByName("tingCardsNode");
        if (_tingCards) {
        ...
        DealMJPut_xiangningshuaijin(_node,eD,off);
        hideTingBtn();
        //if(_tingCardNumNode) _tingCardNumNode.visible = false;
        ...
        if(off == 0){
            ...
            // if (tData.areaSelectMode.baotingbaohu) {
            //  //   MjClient.playui.setCurrentTingNum(tingCards, self._tingCard_showAll);
            //     cc.log("xiangningshuajin---------mjput");
            //   //  MjClient.playui.clearntingcardsign(tingCards);
            // }
        }
    });
...
PlayLayer_xiangningshuaijin.prototype.setCurrentTingNum = function(tingSet ,showall, cardTag)
{
...
PlayLayer_xiangningshuaijin.prototype.showQuanTingPai = function(quanting)
{
...
PlayLayer_xiangningshuaijin.prototype.addtingcardsign = function(children,tingCard)
{
    for (var i = 0; i < children.length; i++) {
        if (children[i].name == "mjhand")
        {
...
PlayLayer_xiangningshuaijin.prototype.clearntingcardsign = function(children)
{
    for (var i = 0; i < children.length; i++) {
        if (children[i].name == "mjhand")
        {
...
PlayLayer_xiangningshuaijin.prototype.calTingByShuaijin = function(off) {
    if (off != 0) { return; }
    var pl = getUIPlayer(off);
    var tingCards = MjClient.majiang.calTingSet(pl.mjhand,MjClient.data.sData.tData.hunCard, pl.rate > 0);
    MjClient.playui.setCurrentTingNum(tingCards);
    cc.log("xiangningshuajin---------caltingbyshuaijin");
}
//添加碰ui
PlayLayer_xiangningshuaijin.prototype.addPengUI=function (node,pl,tData,off) {
...
PlayLayer_xiangningshuaijin.prototype.addGangUI=function (node,pl,tData,off) {
...
PlayLayer_xiangningshuaijin.prototype.addAnGangUI=function (node,pl,tData,off) {
...
PlayLayer_xiangningshuaijin.prototype.addChiUI=function (node,pl,tData,off) {
    var chiIdx = 0;
...
PlayLayer_xiangningshuaijin.prototype.addOutCardUI=function (node,pl,tData,off) {
    for(var i = 0; i < pl.mjput.length; i++)
    {
        ...
        DealMJPut_xiangningshuaijin(node, msg, off, i);
    }

}
PlayLayer_xiangningshuaijin.prototype.addHandCardUI=function (node,pl,tData,off) {
...
PlayLayer_xiangningshuaijin.prototype.addFlowerCardUI=function (node,pl,tData,off) {
...
PlayLayer_xiangningshuaijin.prototype.deljinfunc=function (pl,tData) {
    var tingStat = false;
    var hasShuiJin = pl.rate > 0;
    var needErgodic = true; // 是否需要遍历数组
    var onlyDelJinCards = false; // 仅需要删除金牌
    if (tData.areaSelectMode.baotingbaohu && !hasShuiJin && MjClient.majiang.isHasJinCards(pl.mjhand,tData.hunCard)) {
        // 没摔金，手上有金牌的，看去掉金牌是否还能听
        ...
        onlyDelJinCards = true;
    }
    if (needErgodic) {
        for (var i = 0; i < pl.mjhand.length; i++) {
            var cardsAfterPut = pl.mjhand.slice(0);
            if (onlyDelJinCards && cardsAfterPut[i] != tData.hunCard) { continue; }
            cardsAfterPut.splice(i,1); //依次去掉某张牌看能不能听
            if (MjClient.majiang.canHu(cardsAfterPut,200, MjClient.data.sData.tData.hunCard, hasShuiJin)) {
...
// PlayLayer_xiangningshuaijin.prototype.HandleMJTing=function(node,msg,off)
// {
//     var sData=MjClient.data.sData;
//     var tData=sData.tData;
...
//     {
//         cc.log("播放听动画");
//         if(MjClient.majiang.isEqualHunCard(msg.card))//如果报听打出去的是金牌，则听动画延迟一秒
//         {
//             MjClient.playui.runAction( cc.sequence(cc.delayTime(1),
//                 cc.callFunc(function() {
