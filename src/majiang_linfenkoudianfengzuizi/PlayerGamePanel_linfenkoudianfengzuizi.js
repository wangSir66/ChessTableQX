...
//向服务器发送 过消息
MjClient.MJPass2NetForlinfenkoudianfengzuizi = function()
{
   // console.log(">>>>>>>>>普通  过 <<<<<<<<");
    cc.log("====================MJPass2NetForlinfenkoudianfengzuizi======pass=====");
...
function SetUserVisible_linfenkoudianfengzuizi(node, off)
{
    //var sData = MjClient.data.sData;
    //return;
    cc.log("====================off======================" + off);
...
//处理出牌,放一张牌，打牌动作
function DealMJPut_linfenkoudianfengzuizi(node, msg, off, outNum)
{
    DealMJPut_shanXiApp(node, msg, off, outNum)
}


function InitUserHandUI_linfenkoudianfengzuizi(node, off)
{
    var sData = MjClient.data.sData;
    var tData = sData.tData;
    var pl = getUIPlayer(off);
    ...
    for(var i = 0; i < pl.mjput.length; i++)
    {
        ...
        DealMJPut_linfenkoudianfengzuizi(node, msg, off, i);
    }
...
function initFlower_linfenkoudianfengzuizi() {
    initFlower(false, false);
}

var PlayLayer_linfenkoudianfengzuizi = cc.Layer.extend({
    jsBind: {
        _event: {
            mjhand: function() {
            ...
            roundEnd: function() {
                if (MjClient.getAppType() == MjClient.APP_TYPE.TXLINFENMJ)
                {
                    ...
                    function delayExe()
                    {
                        ...
                        self.addChild(new EndOneView_linfenkoudianfengzuizi(),500);
                    }
            ...
            moveHead: function() {
                ...
                initFlower_linfenkoudianfengzuizi();
            },
            ...
            changeui: {
                ...
                changeuibg: {
                    ...
                    guobg: {
                        guo: {
                            _touch: function(btn, eT) {
                                if (eT == 2) MjClient.MJPass2NetForlinfenkoudianfengzuizi();
                            }
                        },
                        fanhui: {
                            _touch: function(btn, et) {
    ...
    ctor: function() {
        this._super();
        var playui = ccs.load("Play_linfenkoudianfengzuizi.json");
...
PlayLayer_linfenkoudianfengzuizi.prototype.CardLayoutRestore = function(node, off)
{
...
// 判断吃碰杠胡的状态
PlayLayer_linfenkoudianfengzuizi.prototype.EatVisibleCheck = function()
{
    var eat = MjClient.playui.jsBind.eat;
...
/*
    玩家ui，信息
 */
PlayLayer_linfenkoudianfengzuizi.prototype.Node_player = function(off)
{
    ...
    UIEventBind(null, _node, "initSceneData", function (eD) {
        SetUserVisible_linfenkoudianfengzuizi(_node, off);
    ...
    UIEventBind(null, _node, "mjhand", function (eD) {
        InitUserHandUI_linfenkoudianfengzuizi(_node, off);
    });
    ...
    UIEventBind(null, _node, "MJPut", function (eD) {
        ...
        DealMJPut_linfenkoudianfengzuizi(_node,eD,off);
        hideTingBtn();
        if(_tingCardNumNode) _tingCardNumNode.visible = false;
