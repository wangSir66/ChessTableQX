...
//向服务器发送 过消息
MjClient.MJPass2NetForyiYang = function()
{
   // console.log(">>>>>>>>>普通  过 <<<<<<<<");
    cc.log("====================send======pass=====");
...
function SetUserVisible_yiYang(node, off)
{
    //var sData = MjClient.data.sData;
    //return;
    cc.log("====================off======================" + off);
...
function InitUserHandUI_yiYang(node, off)
{
    var sData = MjClient.data.sData;
    var tData = sData.tData;
    var pl = getUIPlayer(off);
...
function initFlower_yiYang() {
    initFlower(false, false);
}


var PlayLayer_yiYangMJ = cc.Layer.extend({
    jsBind: {
        _event: {
            mjhand: function() {
            ...
            roundEnd: function() {
                ...
                function showEndCards()
                {
                    ...
                    self.addChild(new EndOneView_yiYang(),500);
                }
                function delayExe()
                {
...
/*                MjClient.Scene.addChild(new HAHZ_showCardLayer(showCards,function(){
        ...
        hunPai:{
            ...
            //             if(HuncardMsg){
            //                 setCardSprite(this, parseInt(HuncardMsg), 2, true);
            //             }
            //         },
            //         initSceneData:function()
            //         {
            ...
            _event: {
                ...
                MJPut: function(eD) {
                    ...
                    if (eD.uid == SelfUid() && MjClient.data.sData.tData.areaSelectMode.isOpenTingTip)
                    {
                        ...
                        var tingSet = MjClient.majiang.calTingSet(pl.mjhand, MjClient.data.sData.tData.hunCard);
                        setTingCards(_tingCards,tingSet);
                        MjClient.playui.RemoveAllTingTips();
                    }
            ...
            ting: {
                ...
                _touch: function(btn, eT) {
                    if (eT == 2)
                    {
                        ...
                        var sData = MjClient.data.sData;
                        var tData = sData.tData;
                         if(pl.tingStatus < 0 && pl.putCount == 0 && pl.mjhand.length == 14 && tData.areaSelectMode.baoting)
                        {
                             // 设置当前听牌的张数
                            var eat = MjClient.playui.jsBind.eat;
                            eat.gang0._node.visible = false;
            ...
            changeui: {
                ...
                changeuibg: {
                    ...
                    guobg: {
                        guo: {
                            _touch: function(btn, eT) {
                                if (eT == 2) MjClient.MJPass2NetForyiYang();
                            }
                        },
                        fanhui: {
                            _touch: function(btn, et) {
        ...
        haidiCard: {
            ...
            _event:{
                PickHaiDiCard:function(msg){
                    cc.log("======PickHaiDiCard=========");
                    this.visible = true;
                    this.loadTexture(getNewMJBgFile("playing/MJ/MJBg"+ getCurrentMJBgType() + "/Mj_up_4.png"));
                    this.removeAllChildren();

                    this.setName("mjhand");
                    {
                        setCardSprite(this, msg.haidiCard, 4);
                        this.zIndex = 500;
                        // this.setScale(1);
                        // cardNode.setPosition(this.getPositionX(), this.getPositionY());
                    }
                    ...
                    SetArrowRotation(MjClient.arrowbkNode, tData.curPlayer);
                    MjClient.playui.EatVisibleCheck();
                },
                initSceneData:function(){
                    cc.log("hahahahahahah");
                    var sData = MjClient.data.sData;
                    var tData = sData.tData;
                    if(MjClient.majiang.getAllCardsTotal() - sData.tData.cardNext == 0)
                    {
                        this.visible = true;
                        this.loadTexture(getNewMJBgFile("playing/MJ/MJBg"+ getCurrentMJBgType() + "/Mj_up_4.png"));
                        ...
                        this.setName("mjhand");
                        {
                            // cc.log("=======" + this.children.length);
                            // var cardNode = getNewCard(MjClient.playui._downNode, "stand", "mjhand", tData.haidiCard, 0);
                            // cardNode.setPosition(this.getPositionX(), this.getPositionY());
                            setCardSprite(this, tData.haidiCard, 4);
                            this.zIndex = 500;
                            // this.setScale(1.2);
                        }
    ...
    ctor: function() {
        this._super();
        var playui = ccs.load("Play_yiyangMj.json");
        playMusic("bgFight");
...
//展示杠开的牌
// PlayLayer_yiYangMJ.prototype.showKaiGangBigCards = function(kaiGangKuang, addnewcard) {
//     var tData = MjClient.data.sData.tData;
//     var isKaiGang = true;
...
PlayLayer_yiYangMJ.prototype.CardLayoutRestore = function(node, off)
{
    ...
    var zimoHuType = (pl.huWord && pl.huWord == "zimo")
...
// 判断吃碰杠胡的状态
PlayLayer_yiYangMJ.prototype.EatVisibleCheck = function(isKaiGang)
{
    ...
    cc.log("=========EatVisibleCheck=======" , tData.uids[tData.curPlayer] == SelfUid())
    //安全检测，防止重连时弹错按钮
    ...
    if(tData.tState == TableState.waitPut && pl.mjState == TableState.waitPut) 
    {
        if(IsTurnToMe())
        {
            ...
            //闲家摸牌第一张检测报听，如果庄家起手闲家报听过了 后面就不用检测报听了
            cc.log("pl.tingStatus:" + pl.tingStatus + ":pl.putCount:" + pl.putCount + ":pl.mjhand.length:" + pl.mjhand.length
                + ":baoting:" + tData.areaSelectMode.baoting);
            if(pl.tingStatus < 0 && pl.putCount == 0 && pl.mjhand.length == 14 && tData.areaSelectMode.baoting)
            {
                 // cc.log("￥￥￥￥听牌监测");
    ...
    else if(tData.tState == TableState.waitEat)
    {
        ...
        if(!IsTurnToMe())
        {
            ...
            {
                var next = MjClient.majiang.getAllCardsTotal() - MjClient.data.sData.tData.cardNext;
                if(next != 0) vnode.push(eat.guo._node);
                eat.ting._node.visible = false;
                eat.noTing._node.visible = false;
                isCheckedTing = false;
            }
...
PlayLayer_yiYangMJ.prototype.addtingcardsign = function(children,tingCard)
{
    for (var i = 0; i < children.length; i++) {
        if (children[i].name == "mjhand")
        {
...
PlayLayer_yiYangMJ.prototype.clearntingcardsign = function(children)
{
    for (var i = 0; i < children.length; i++) {
        if (children[i].name == "mjhand") 
        {
...
//及时刷新杠分
PlayLayer_yiYangMJ.prototype.updateGangScore = function(node, data, off){
    node.visible = true;
...
PlayLayer_yiYangMJ.prototype.ShowAllTingTips = function ()
{
    if (!MjClient.data.sData.tData.areaSelectMode.isOpenTingTip)
        return;
...
PlayLayer_yiYangMJ.prototype.RemoveAllTingTips = function ()
{
    if (!MjClient.data.sData.tData.areaSelectMode.isOpenTingTip)
        return;
    if(!IsTurnToMe()) return;
    ...
    for (var i = 0; i < children.length; i++)
    {
        ...
        {
             var card = children[i];
             if(card.getChildByName("tingSign")) 
