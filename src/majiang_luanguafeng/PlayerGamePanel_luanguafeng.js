...
//向服务器发送 过消息
MjClient.MJPass2NetForluanguafeng = function()
{
   // console.log(">>>>>>>>>普通  过 <<<<<<<<");
    cc.log("====================MJPass2NetForluanguafeng ======pass=====");
...
function SetUserVisible_luanguafeng(node, off)
{
    //var sData = MjClient.data.sData;
    //return;
    cc.log("====================off======================" + off);
...
//处理出牌,放一张牌，打牌动作
function DealMJPut_luanguafeng(node, msg, off, outNum)
{
    DealMJPut_shanXiApp(node, msg, off, outNum);
}

function delFlowerCard(node, off){
    clearCardUI(node, off);
    InitUserHandUI_luanguafeng(node,off);
}

function InitUserHandUI_luanguafeng(node, off)
{
    var sData = MjClient.data.sData;
    var tData = sData.tData;
    var pl = getUIPlayer(off);
    ...
    for(var i = 0; i < pl.mjput.length; i++)
    {
        ...
        DealMJPut_luanguafeng(node, msg, off, i);
    }
    ...
    if(MjClient.rePlayVideo == -1)//表示正常游戏
    {
        ...
        else
        {
            ...
            cc.log("亮风 牌数" ,pl.liangfengCount);
            if (pl.liangfengCount) 
            {
                upCardCount -= pl.liangfengCount;
            }
...
function initFlower_luanguafeng() {
    initFlower(false, false);
}


var PlayLayer_luanguafeng = cc.Layer.extend({
    jsBind: {
        _event: {
            mjhand: function() {
            ...
            roundEnd: function() {
                ...
                function delayExe()
                {
                    ...
                    self.addChild(new EndOneView_luanguafeng(),500);
                }
        ...
        eat: {
            ...
            ting: {
                ...
                _touch: function(btn, eT) {
                    if (eT == 2)
                    {
                        ...
                        var tData = MjClient.data.sData.tData;
                        var tingCards = MjClient.majiang.calTingSet(copyhand, new Set([tData.hunCard, tData.hunCard2]),tData);
            ...
            guo: {
                ...
                _touch: function(btn, eT) {
                    if (eT == 2)
                    {
                        MjClient.MJPass2NetForluanguafeng();
                    }
                },
                bgimg: {
                    _run: function() {
            ...
            changeui: {
                ...
                changeuibg: {
                    ...
                    guobg: {
                        guo: {
                            _touch: function(btn, eT) {
                                if (eT == 2) MjClient.MJPass2NetForluanguafeng();
                            }
                        },
                        fanhui: {
                            _touch: function(btn, et) {
    ...
    ctor: function() {
        this._super();
        var playui = ccs.load("Play_luanguafeng.json");
        ...
        if(pl && (pl.mjState == TableState.isReady || pl.mjState == TableState.roundFinish))
        {
            ...
            if(pl != null)
            {
                if (pl.isTing) {
                    ...
                    if (off == 0)
                    {
                        var tingSet = MjClient.majiang.calTingSet(pl.mjhand, new Set([tData.hunCard, tData.hunCard2]),tData);
...
//     projNode.setPosition(800,200);
//     projNode.setScale(10);
//     if(COMMON_UI3D.is3DUI())
//     {
//         projNode.setScale(0.5);
//         projNode.setPosition(20,50);
//     }
...
PlayLayer_luanguafeng.prototype.CardLayoutRestore = function(node, off)
{
    ...
    var zimoHuType = (pl &&pl.huWord && (pl.huWord == "zimo" || pl.huWord == "tianhu"))
        || (pl && pl.mjState && tData && (tData.tState == TableState.waitPut && pl.mjState == TableState.waitPut ));
            ...
            if(i != 0)
            {
                if(ci.name == orders[i - 1].name)
                {
                    if(ci.isgang4)
                    {
                        ...
                        ci.setScale(orders[i-1].getScale());
                    }
            ...
            if(off == 0)
            {
                ...
                else if(i == orders.length - 1)
                {
                    ...
                    if(newC)
                    {
                        ...
                        if (MjClient.majiang.canchangecolor(ci.tag,pl.mjhand,pl)) 
                        {
                            ci.setColor(cc.color(190, 190, 190));
                        }
                        else
                    ...
                    else
                    {
                        ...
                        if (MjClient.majiang.canchangecolor(ci.tag,pl.mjhand,pl)) 
                        {   
                            ci.setColor(cc.color(190, 190, 190));//置灰
                        }
                        else
                        {
                            ci.setColor(cc.color(255, 255, 255));
...
// 判断吃碰杠胡的状态
PlayLayer_luanguafeng.prototype.EatVisibleCheck = function()
{
    var eat = MjClient.playui.jsBind.eat;
    ...
    if(tData.tState == TableState.waitPut && pl.mjState == TableState.waitPut)
    {
        if(IsTurnToMe())
        {
            ...
            if (pl.isNew && pl.eatFlag & 8) {
                ...
                // if (!pl.mustHu) {
                //     pl.mustHu = (tData.hunCard == pl.mjhand[pl.mjhand.length - 1] && MjClient.majiang.quantingpai(pl.mjhand.slice(0, -1), new Set([tData.hunCard, tData.hunCard2]),tData));
                // }
...
/*            //听
            if (!pl.isTing) {
                ...
                for (var i = 0; i < pl.mjhand.length; i++) {
                    ...
                    if (MjClient.majiang.canTing(cardsAfterPut,tData)) {
            ...
            var rtn = MjClient.majiang.canGang1(pl.mjpeng, pl.mjhand, pl.isTing, tData);
...
/*PlayLayer_luanguafeng.prototype.delethands = function(off)
{
    var pl = getUIPlayer(off);
    var children = getNode(off).children;
    if (pl.buCards && pl.mjflower.length > 0) //没有亮风或者亮风后补风完成
    {
        return;
    }
    if(pl && pl.buCards && pl.buCards.length >= 0 && pl.mjflower.length > 0){
        var realcardslength = 14 - pl.mjflower.length;//计算亮风，补风后的手牌数，具体数值自己改动
        
        for(var j = 0 ; j < realcardslength;j++)
        {
            for(var i = 0; i < children.length; i++)
            {
                ci = children[i];
...
/*
    玩家ui，信息
 */
PlayLayer_luanguafeng.prototype.Node_player = function(off)
{
    ...
    var _liangfeng_block = _node.getParent().getChildByName("liangfeng_block");
    setWgtLayout(_liangfeng_block, [0.25, 0.25], [0.5, 0.44], [0,0]);

    var _liangfeng_1 = _liangfeng_block.getChildByName("Liangfeng_1");
    var _liangfeng_2 = _liangfeng_block.getChildByName("Liangfeng_2");
    if(_liangfeng_block && off == 0){
        _liangfeng_1.visible = false;
        _liangfeng_2.visible = false;
        UIEventBind(null, _liangfeng_block, "waitPut", function (msg) {
            if(getUIPlayer(0) && msg.liangfengInfo &&
                getUIPlayer(0).info.uid == msg.liangfengInfo.uid 
                && msg.liangfengInfo.mjState == TableState.waitSelect){
                _liangfeng_1.visible = true;
                _liangfeng_2.visible = true;
                // setWgtLayout(_liangfeng_1, [0.15, 0.15], [-0.1, -0.06], [0, 0]);
                // setWgtLayout(_liangfeng_2, [0.15, 0.15], [0.165, -0.06], [0, 0]);
            }
/*            else{
                if(msg.liangfengInfo){
                    var cnode = getNode(off);
                    delFlowerCard(cnode, off);
                }
            }*/
        });
    }
    _liangfeng_1.addTouchEventListener(function(sender, type) {
        if (type == 2) {
            MjClient.gamenet.request("pkroom.handler.tableMsg", {
                cmd: "MJSelect",
                liangfeng:true,
            ...
            _liangfeng_2.visible = false;
            MjClient.playui.EatVisibleCheck();
            // _liangfeng_1.removeFromParent();
            // _liangfeng_2.removeFromParent();
        }
    });
    _liangfeng_2.addTouchEventListener(function(sender, type) {
        if (type == 2) {
            MjClient.gamenet.request("pkroom.handler.tableMsg", {
                cmd: "MJSelect",
                liangfeng:false,
    ...
    //亮风数
    var _fengTag = _head.getChildByName("fengTag");
    if (_fengTag)
    {
        UIEventBind(null, _fengTag, "waitPut", function (msg){
            _fengTag.visible = true;
            var pl= getUIPlayer(off);
            if(getUIPlayer(off) && msg.liangfengInfo && getUIPlayer(off).info.uid == msg.liangfengInfo.uid){
                _fengTag.visible = true;
                if (msg.liangfengInfo.mjflower){
                    _fengTag.getChildByName("num_text").setString("x" + msg.liangfengInfo.mjflower.length);
                }
                else if(msg.liangfengInfo.mjState){
                    _fengTag.getChildByName("num_text").setString("x0");
                }
                else{
                    _fengTag.getChildByName("num_text").setString("x0");
                }
            }else{
                if(pl && pl.mjflower.length <= 0){
                    _fengTag.getChildByName("num_text").setString("x0");
                }
            }
        });

        UIEventBind(null, _fengTag, "MJSelect", function (msg){
            if(getUIPlayer(off) && msg.liangfengInfo && getUIPlayer(off).info.uid == msg.liangfengInfo.uid){
                var pl = getUIPlayer(off);
                _fengTag.visible = true;
                if (msg.liangfengInfo.mjflower){
        ...
        UIEventBind(null, _fengTag, "MJFlower", function (msg){
            if(getUIPlayer(off)&&getUIPlayer(off).info.uid == msg.uid){
                var pl = getUIPlayer(off);
                _fengTag.visible = true;
                if (pl && pl.mjflower){
                    _fengTag.getChildByName("num_text").setString("x" + pl.mjflower.length);
                }else{
                    _fengTag.visible = false;
                }
            }
        });

        UIEventBind(null, _fengTag, "MJPeng", function (msg){
            var pl = getUIPlayer(off);
            if(pl){
                cc.log(" pl.mjflower.length:", pl.mjflower.length,off);
                _fengTag.visible = true;
                _fengTag.getChildByName("num_text").setString("x" + pl.mjflower.length);
            }
        ...
        UIEventBind(null, _fengTag, "initSceneData", function (msg){
            var pl = getUIPlayer(off);
            if (pl&& pl.mjState != TableState.isReady && pl.mjState != TableState.roundFinish) {
                _fengTag.visible = true;
                _fengTag.getChildByName("num_text").setString("x" + pl.mjflower.length);
        ...
        UIEventBind(null, _fengTag, "roundEnd", function (msg){
            _fengTag.visible = false;
        });
    }
    //托管
    ...
    var _down = _node.getParent().getChildByName("down");
    var _tingCardsNode = _down.getChildByName("tingCardsNode");
    // var _tingCardsNode = _node.getChildByName("tingCardsNode");
    if(_tingCardsNode)  _tingCardsNode.visible = false;

    var _down = _node.getParent().getChildByName("down");
    var _tingCardNumNode = _down.getChildByName("tingCardNumNode");
    ...
    UIEventBind(null, _node, "initSceneData", function (eD) {
        SetUserVisible_luanguafeng(_node, off);
        ...
        var tData = MjClient.data.sData.tData;
        if (pl && getUIPlayer(off).info.uid == SelfUid() && tData.tState != TableState.roundFinish)
        {
            var _down = _node.getParent().getChildByName("down");
            var _tingCards = _down.getChildByName("tingCardsNode");
            var tingSet = MjClient.majiang.calTingSet(pl.mjhand,tData.hunCard,tData);
            if(_tingCards) setTingCards_JINZHONGMJ(_tingCards,tingSet);
        }
        setWgtLayout(_liangfeng_block, [0.25, 0.25], [0.5, 0.44], [0,0]);
        if(pl && pl.mjState == TableState.waitSelect && pl.info.uid == SelfUid()){
            _liangfeng_1.visible = true;
            _liangfeng_2.visible = true;
            // setWgtLayout(_liangfeng_1, [0.15, 0.15], [-0.1, -0.06], [0, 0]);
            // setWgtLayout(_liangfeng_2, [0.15, 0.15], [0.165, -0.06], [0, 0]);
    ...
    UIEventBind(null, _node, "mjhand", function (eD) {
        InitUserHandUI_luanguafeng(_node, off);
    });
    ...
    UIEventBind(null, _node, "MJSelect", function (eD) {
        var pl = getUIPlayer(off);
        if (pl) 
        {   
            delFlowerCard(_node, off);
            MjClient.playui.EatVisibleCheck();
            MjClient.playui.CardLayoutRestore(_node, off);
            if(eD.liangfengInfo && pl.info.uid == eD.liangfengInfo.uid){
                ShowEatActionAnim(_node,ActionType.LIANG,off);//不能放在delFlowerCard前面，不然动画会删掉
            }
        }
    });
    ...
    UIEventBind(null, _node, "MJPut", function (eD) {
        ...
        var pl = getUIPlayer(off);
        var tData = MjClient.data.sData.tData;
        if (pl && eD.uid == getUIPlayer(off).info.uid && tData.tState != TableState.roundFinish)
        {
            var _tingCards = _node.getChildByName("tingCardsNode");
            var tingSet = MjClient.majiang.calTingSet(pl.mjhand,null,tData);
            if(_tingCards) setTingCards_JINZHONGMJ(_tingCards,tingSet);
        }
        DealMJPut_luanguafeng(_node,eD,off);
        hideTingBtn();
        if(_tingCardNumNode) _tingCardNumNode.visible = false;
    ...
    UIEventBind(null, _node, "waitPut", function (eD) {
        ...
        var pl = getUIPlayer(off);
        //自动亮风
        if(eD.liangfengInfo && eD.liangfengInfo != null && pl && pl.info.uid == eD.liangfengInfo.uid){
            ShowEatActionAnim(_node,ActionType.LIANG,off);
        }
        if(eD.liangfengInfo && eD.liangfengInfo!= null || eD.fengdui && eD.fengdui != -1)
        {     
            var cnode = getNode(off);
            // delFlowerCard(cnode, off);
            //此处会删除动画，增加延迟
            setTimeout(function() {
                delFlowerCard(cnode, off)}, 1000 * 0.2);
        }
    });
