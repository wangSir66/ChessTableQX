...
(function() {
    ...
    majiang.getNextCard = function(card) {
	    var ret = card;
	    if (ret % 10 == 0)
	        ret = card - 9;
	    else
	...
	majiang.isJingNextCard = function(card) {
		var cardN = this.getNextCard(MjClient.data.sData.tData.jiangCard);
		if(cardN % 20 == card % 20) {
			return true;
		}
		return false;
        ...
        for (var i = 0; i < handMatrix.length; i++) {
            if (handMatrix[i].length == 4) {
                if (handMatrix[i][0] == handMatrix[i][1] && handMatrix[i][0] == handMatrix[i][2] && handMatrix[i][0] == handMatrix[i][3]) {
                    if(this.isJingNextCard(handMatrix[i][0])) {
                    	huxi += handMatrix[i][0] > 20 ? 24 : 12;
                    }else {
    ...
    if (typeof(MjClient) != "undefined") {
        MjClient.majiang_daYeZiPai = majiang;
    } else {
