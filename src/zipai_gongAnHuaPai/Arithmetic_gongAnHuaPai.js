...
(function() {
    ...
    GongAnHuaPai.prototype.randomCards = function(areaSelectMode) {
        var cards = [
    ...
    //起手加精
    GongAnHuaPai.prototype.addJingCards = function(cards, areaSelectMode) {
        var result = [];
    ...
    GongAnHuaPai.prototype.getHandCardCount = function(pl, card) {
        var hand = pl.mjhand.slice();
        this.removeMjgang1(hand, pl.mjgang1);
        return this.getCardCount(hand, card);
    };

    GongAnHuaPai.prototype.getCardCount = function(hand, card) {
        var count = 0;
    ...
    GongAnHuaPai.prototype.delHandCard = function(hand, card) {
    ...
    GongAnHuaPai.prototype.delMjsort = function(mjsort, name, idx, isUpdata) {
        var pos = -1;
    ...
    GongAnHuaPai.prototype.getHandCards = function(hand, card, num, isSanJing) {
        ...
        if(cards.length < num) {
            //logger.debug("GongAnHuaPai.getHandCards error:", cards.length, num);
        }

        return cards;
    };

    GongAnHuaPai.prototype.isRed = function(card) {
        return [21, 22, 23, 31, 32, 33, 3, 5, 7, KING].indexOf(card) >= 0;
    };

    //起手可发给玩家的精牌
    GongAnHuaPai.prototype.getStartJingCards = function (isSanJing) {
        if(isSanJing) {
            return [3, 5, 7];
        }else {
            return [42, 3, 5, 7, 9];
    ...
    GongAnHuaPai.prototype.isSuJing = function(card, isSanJing) {
        if(isSanJing) {
    ...
    GongAnHuaPai.prototype.isHuaJing = function(card, isSanJing) {
        if(isSanJing) {
    ...
    GongAnHuaPai.prototype.isJingCard = function(card, isSanJing) {
        return this.isSuJing(card, isSanJing) || this.isHuaJing(card, isSanJing);
    };

    GongAnHuaPai.prototype.isDaiZiPai = function(card, isSanJing) {
        if(isSanJing) {
    ...
    GongAnHuaPai.prototype.getPengScore = function(cards, isZhujing, isSanJing) {
        var count = 0;
        var isJing = false;
    ...
    GongAnHuaPai.prototype.getKanScore = function(cards, isZhujing, isSanJing) {
        var count = 0;
        var isJing = false;
        for(var j = 0; j < cards.length; j++) {
            var c = cards[j];
    ...
    GongAnHuaPai.prototype.getZhaoScore = function(cards, isZhujing, isSanJing) {
        var count = 0;
        var isJing = false;
        for(var j = 0; j < cards.length; j++) {
            var c = cards[j];
    ...
    GongAnHuaPai.prototype.getFanScore = function(cards, isZhujing, isSanJing) {
        var count = 0;
        var isJing = false;
        for(var j = 0; j < cards.length; j++) {
            var c = cards[j];
    ...
    GongAnHuaPai.prototype.getZhaScore = function(cards, isZhujing, isSanJing) {
        if(cards.length == 4) {
    ...
    GongAnHuaPai.prototype.getMenZiScore = function(cards, zhuJing, isSanJing) {
        var count = 0;
        var num = 0;
        for(var j = 0; j < cards.length; j++) {
            var c = cards[j];
    ...
    GongAnHuaPai.prototype.getZhuJing = function(tb, pl, matrix) {
    ...
    //是否可以开贩
    GongAnHuaPai.prototype.getCanFanByCard = function(pl, card) {
    ...
    //card是否有扎
    GongAnHuaPai.prototype.hasInZhaByCard = function(pl, card) {
    ...
    GongAnHuaPai.prototype.getLiuCardsInHand = function (pl, tb) {
        var hand = pl.mjhand.slice();
        this.removeMjgang1(hand, pl.mjgang1);
    ...
    // 红字 王数量统计
    GongAnHuaPai.prototype.stats = function(pl, card) {
        var hand = pl.mjhand.slice();
        if(card){
            hand.push(card);
        }
    ...
    GongAnHuaPai.prototype.getAchv = function(tb, pl, matrix, be, stats, card) {
        var tData = tb.tData;
    ...
    GongAnHuaPai.prototype.canDepend = function (c1, c2) {
    ...
    GongAnHuaPai.prototype.removeMjgang1 = function(hand, mjgang1) {
        for(var i = 0; i < mjgang1.length; i++) {
            var gang = mjgang1[i];
            for(var j = 0; j < gang.length; j++) {
    ...
    GongAnHuaPai.prototype.canHu = function(tb, pl, card) {
        var tData = tb.tData;
    ...
    GongAnHuaPai.prototype.getHuInfo = function(tb, pl, card) {
        var hand = pl.mjhand.slice();
        //删掉扎的牌
        this.removeMjgang1(hand, pl.mjgang1);
    ...
    GongAnHuaPai.prototype.cardsCount = function(tb) {
        return 110 + tb.tData.areaSelectMode.bieGangNum;
    };

    GongAnHuaPai.prototype.getHandLink = function (tb, pl, putCard) {
        var hand = pl.mjhand.slice();
        this.removeMjgang1(hand, pl.mjgang1);
    ...
    GongAnHuaPai.prototype.getTingStats = function(tb, pl, putCard){
    ...
    //-----------------前端新增接口start--------------
    //乙，三，无，七，九等价处理
    GongAnHuaPai.prototype.replaceHuaJing = function(arr) {
    ...
    /*
        ...
        第9列
    */
    GongAnHuaPai.prototype.sortCard = function(hand, mjgang1, isLastDraw) {
    ...
    // 回放时.非自己手牌排序
    GongAnHuaPai.prototype.sortCardForOtherReplay = function(hand, mjgang1) {
        mjgang1 = mjgang1 || [];
    ...
    //删手牌重排序
    GongAnHuaPai.prototype.sortByUser = function(arr) {
        return [];
    }

    //排序.小结算用
    GongAnHuaPai.prototype.sortEndCard = function(arr, mjgang1) {
        if (!arr || !mjgang1)
            return [];
        arr = arr.slice();
        //非赢家手牌中的扎牌不能重复计算显示
    ...
    //一列手牌的排序
    GongAnHuaPai.prototype.sortRowCards = function(arr) {
        if (!arr || arr.length == 0) 
            return arr;
    ...
    //给牌定义一组有序的key
    GongAnHuaPai.prototype.orderCards = [

    ];

    GongAnHuaPai.prototype.getAllCardsTotal = function() {
        return 110 + MjClient.data.sData.tData.areaSelectMode.bieGangNum;
    }

    GongAnHuaPai.prototype.hintPutCardsToTing = function() {
    ...
    GongAnHuaPai.prototype.getTingCards = function(tb, pl, putCard) {
        var tingCards = [];
    ...
    GongAnHuaPai.prototype.getSortHuxi = function(tb, pl) {
        if (!pl)
            return 0;
    ...
    if (typeof(MjClient) != "undefined") {
        MjClient.majiang_gongAnHuaPai = new GongAnHuaPai();
    } 
