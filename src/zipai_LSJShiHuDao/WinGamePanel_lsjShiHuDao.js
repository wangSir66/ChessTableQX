...
//自己计算胡息
function huXiScore_lsjShiHuDao(type,card)
{ 
...
function addWxHeadToEndUI_lsjShiHuDao(node,off)
{
	...
	if(tData.areaSelectMode.isJiaChui){
		...
		var fileName = pl.jiachuiNum > 0 ? ("playing/other/chui_" + pl.jiachuiNum + ".png") : "playing/other/buchui.png";
...
function reInitarrCardVisible_lsjShiHuDao()
{
	...
	if(MjClient.cardNumImgNode && cc.sys.isObjectValid(MjClient.roundnumImgNode)){
        MjClient.cardNumImgNode.setVisible(false);
	}
}

var EndOneView_lsjShiHuDao = cc.Layer.extend({
    _block:null,
    _endoneuiNode:null, 
	jsBind:{
		block: {
		...
		btn_ready:{
			...
			_click: function(btn,eT){
				...
				reInitarrCardVisible_lsjShiHuDao();
			}
		},
		btn_logic: {
			_run: function(){
		...
		main:{
			...
			leftPanel:{
				_run:function(){
                    if (MjClient.gameType == MjClient.GAME_TYPE.LENG_SHUI_JIANG_SHI_HU_DAO &&  MjClient.data.sData.tData.maxPlayer == 2){
				...
				_run: function(){
					var sData = MjClient.data.sData;
		   			...
		   			if(MjClient.isDismiss) {
		   				this.getParent().getChildByName("huNameNode").setVisible(false);
		   				return; // 解散不显示底牌和埋牌
		   			}
			...
			uiBg:{
				headNode: {
					_run: function(){ 
						...
						addWxHeadToEndUI_lsjShiHuDao(this,winOff); 
						...
						var cloneNum = 2;
				   		...
				   		for(var index = 0;index<playerCount;index++){
				   			if(index != winOff){
				   				...
				   				addWxHeadToEndUI_lsjShiHuDao(cloneThis,index);
				   				cloneNum++;
				   			}
				   		}
					}
			...
			title_img: {
				_run:function(){
            		...
            		if (MjClient.isDismiss){
            			...
            			var selfPlayer = getUiOffByUid_hengYang(SelfUid());
            			if(tData.winner == -1){
				...
				score: {
					_run: function(){ 
						var sData = MjClient.data.sData;
		   				...
		   				var players = sData.players;
			   			var pl = players[tData.uids[tData.winner] + ""];
			   			this.setString(pl.hzdesc.score); //此处根据服务器数据填写
	                    this.ignoreContentAdaptWithSize(true);
					}
				}
			},
				...
				score: {
					_run: function(){ 
						var sData = MjClient.data.sData;
		   				...
		   				var players = sData.players;
			   			var pl = players[tData.uids[tData.winner] + ""];
			   			this.setString(pl.hzdesc.totalScore);	//此处根据服务器数据填写
	                    this.ignoreContentAdaptWithSize(true);
					}
				}
			}  
    ...
    ctor:function () {
        ...
        var endoneui = ccs.load("endOne_lsjShiHuDao.json");
    ...
    setWinCard: function(){ 
	    ...
	    for (var index in mjSortArr) {
	    	...
	    	if(name == "mjchi"){
				...
				var score = huXiScore_lsjShiHuDao(name, cardNum);
    ...
    setHuNameContnet: function(){
		...
		if(tData.winner == -1){
			//荒庄隐藏掉胡的类型
			this._endoneuiNode.getChildByName("uiNode").getChildByName("huNameNode").setVisible(false);
			this._endoneuiNode.getChildByName("uiNode").getChildByName("huXiNode").setVisible(false);
			return;
		}
		// 初始UI
    	var uiNode = this._endoneuiNode.getChildByName("uiNode");
    	var huNameNode = uiNode.getChildByName("huNameNode");
    	huNameNode.setVisible(false);		//先设置为不可见
    ...
    setOverScore: function(name,num,index){
        ...
        */
        ...
        /*
        node.getChildByName("huName").setString("胡息：");
        if(name == "totalFan")
        	node.getChildByName("huNum").setString(num);  
        */
    },
