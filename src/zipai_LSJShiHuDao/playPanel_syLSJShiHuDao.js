//新版冷水江
var playPanel_syLSJShiHuDao = playLayer_ziPai.extend({
	ctor: function() {
		this._super("Play_shaoYangLSJShiHuDao.json");
	...
	getJsBind: function() {
		var jsBind = {
			...
			img_banner: {
				btn_setting: {
					_click: function() {
						MjClient.Scene.addChild(new settingPanel_syLSJShiHuDao(), 6000);
...
playPanel_syLSJShiHuDao.prototype.showOrHideJiachuiText = function(node, off) {
    node.visible = false;
    var tData = MjClient.data.sData.tData;
    ...
    if (tData.tState == TableState.waitJiazhu) {
        if (pl.jiachuiNum == 0) {
            ...
            if (pl.jiachuiNum >= 1 && pl.jiachuiNum <= 3) {
...
//Override
playPanel_syLSJShiHuDao.prototype.checkChuiFlagVisible = function(node) {
...
//检测距离信息
playPanel_syLSJShiHuDao.prototype.checkCanShowDistance = function(layoutData) {
...
//字牌字体列表
playPanel_syLSJShiHuDao.prototype.getCardFontList = function() {
    return ["type1", "type2", "type3"];
};

//Override 
playPanel_syLSJShiHuDao.prototype.initSettingData = function() {
...
//Override
playPanel_syLSJShiHuDao.prototype.isShowLongCard = function() {
	return true;
};

playPanel_syLSJShiHuDao.prototype.isCheckTingStats = function() {
    return true;
}

//获取听的牌
playPanel_syLSJShiHuDao.prototype.getTingCards = function(sData, pl, putCard) {
 ...
 * 计算听牌提示
 */
playPanel_syLSJShiHuDao.prototype.calculateHintPutList = function() {
	MjClient.hintPutList_ziPai = MjClient.huzi.hintPutCardsToTing();
};

//Override
playPanel_syLSJShiHuDao.prototype.createGameOverLayer = function(type) {
	return new GameOverLayer_syZiPai();
};

//Override
playPanel_syLSJShiHuDao.prototype.createEndOneLayer = function(type) {
	return new EndOneView_lsjShiHuDao();
};

//字牌字体idx
playPanel_syLSJShiHuDao.prototype.getCardFontIdx = function() {
    return util.localStorageEncrypt.getNumberItem(MjClient.KEY_ZI_PAI_ZI_PAI_TYPE, 2);
};

//布局类型
playPanel_syLSJShiHuDao.prototype.getLayoutType = function() {
    return util.localStorageEncrypt.getNumberItem(MjClient.KEY_ZI_PAI_PLAY_UI_LAYOUT, 0);
};

//Override
playPanel_syLSJShiHuDao.prototype.getGameBgList = function() {
	return ["playing/gameTable/beijing_1.jpg", "playing/gameTable/beijing_2.jpg", "playing/gameTable/beijing_3.jpg"];
};

//Override
playPanel_syLSJShiHuDao.prototype.getGameBgIdx = function() {
	return util.localStorageEncrypt.getNumberItem(MjClient.KEY_ZI_PAI_GAME_BG_TYPE, 0);
};

//Override
playPanel_syLSJShiHuDao.prototype.getHuXiType = function() {
    var idx = util.localStorageEncrypt.getNumberItem(MjClient.KEY_ZI_PAI_HU_XI_TYPE, 0);
    return [1, 0][idx];
};

//Override
playPanel_syLSJShiHuDao.prototype.getXuXianType = function() {
    return util.localStorageEncrypt.getNumberItem(MjClient.KEY_ZI_PAI_XU_XIAN_TYPE, 1);
};

//Override
playPanel_syLSJShiHuDao.prototype.getSuDuType = function() {
    var old = util.localStorageEncrypt.getNumberItem(MjClient.KEY_ZI_PAI_SU_DU_TYPE, 2);
    return [2,0,1,3][old];
};

//Override
playPanel_syLSJShiHuDao.prototype.getTingPaiType = function() {
    var idx = util.localStorageEncrypt.getNumberItem(MjClient.KEY_ZI_PAI_PLAY_TING_PAI, 1);
    return [1, 0][idx];
};

playPanel_syLSJShiHuDao.prototype.isAniParallel = function() {
    return false;
};

playPanel_syLSJShiHuDao.prototype.changeEatBtnLayout = function(btnNode, len, idx) {
    var scale = 0.20;
    if(MjClient.getAppType() == MjClient.APP_TYPE.QXSYDTZ){
...
// 获取提、偎牌的显示类型
playPanel_syLSJShiHuDao.prototype.getCardShowType = function(card, off) {
