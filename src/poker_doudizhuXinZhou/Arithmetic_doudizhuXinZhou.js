...
DoudizhuXinzhou.prototype.CARDTPYE = CARDTPYE;
var CARDCOUNT = {};
...
DoudizhuXinzhou.prototype.randomCards = function() {
    var cards = [];
...
DoudizhuXinzhou.prototype.calPoint = calPoint;

DoudizhuXinzhou.prototype.isShun = function(oCards) { //oCards有序
    if (!oCards) {
        return false;
    }
...
DoudizhuXinzhou.prototype.isLiandui = function(oCards) { //oCards有序
    if (!oCards) {
        return false;
    }
    var cardCount = oCards.length;
...
DoudizhuXinzhou.prototype.isSanshun = function(cards) { //cards有序
    if (!cards) {
        return false;
    }
...
DoudizhuXinzhou.prototype.isFeiji2 = function(oCards) { //cards有序
    if (!oCards) {
        return false;
    }
...
DoudizhuXinzhou.prototype.isFeiji = function(oCards) { //cards有序
    if (!oCards) {
        return false;
    }
    var cards = oCards.slice();
...
// 计算牌型
DoudizhuXinzhou.prototype.calType = function(cards) {
    var cardCount = cards.length;
...
DoudizhuXinzhou.prototype.calCardsValue = function(cards, type) {
...
// 牌点比较函数
DoudizhuXinzhou.prototype.cardValueCmp = function(a, b) {
    var pa = calPoint(a);
    var pb = calPoint(b);
    if (pa == pb) {
        return a - b;
...
// 牌是否能压上
DoudizhuXinzhou.prototype.canPut = function(oCards, oLastCards) {
    var cards = oCards.slice();
...
// 检查是否能出牌
DoudizhuXinzhou.prototype.checkPut = function(oHands, oCards, lastCards) {
...
DoudizhuXinzhou.prototype.checkPutByOneTimes = function (oHands) {
...
// 对手牌排序，普通牌型和同花顺优先排序
DoudizhuXinzhou.prototype.sortHandCards = function(oCards, sortType) {
    var cards = oCards.slice();
...
// 找N张点数一样的牌
DoudizhuXinzhou.prototype.findNSameCard = function(hands, point, n) {
...
DoudizhuXinzhou.prototype.filterSort = function(hands, cardsArray, type) {
...
// 找出type牌型的牌
DoudizhuXinzhou.prototype.findCardByType = function(hands, type, cardCount) {
    var rets = [];
...
// 是否可以包含炸弹
DoudizhuXinzhou.prototype.isCanContainZhadan = function (type) {
...
// 不拆炸弹
DoudizhuXinzhou.prototype.dismantleZhadan = function (hands, cardsArray, type) {
...
// 求解提示出牌
DoudizhuXinzhou.prototype.tipCards = function(oHands, oLastCards) {
    var rets = [];
...
// 求出牌型
DoudizhuXinzhou.prototype.cardsType = function(oCards) {
    if (oCards) {
        var cards = oCards.slice();
        cards.sort(this.cardValueCmp);
...
// 计算真实的点
DoudizhuXinzhou.prototype.calRealPoint = function(num) {
   if (!num) {
    return -1;
   }
   var ceilNum = Math.ceil(num / 4);
...
DoudizhuXinzhou.prototype.findMinPointAndMinZhaDanByHands = function (hands) {
...
// 找最小牌的顺子/连对
DoudizhuXinzhou.prototype.findCardsByMinPointAndType = function (cardsMap, len, minPoint, cType) {
...
DoudizhuXinzhou.prototype.findSanZhangByMinPoint = function (cardsMap, len, minPoint) {
...
// 找最小牌的对子
DoudizhuXinzhou.prototype.findDuiZiByMinPoint = function (cardsMap, len, minPoint) {
...
DoudizhuXinzhou.prototype.changeCardsToMap = function (hands, isExcludeZhaDan) {
    var cardsMap = {}
    for (var i = 0; i < hands.length; i++) {
...
// 首出逻辑
DoudizhuXinzhou.prototype.getAutoPutCardWithFirstOut = function(hands) {
    // 1. 从小到大出牌
...
// 是否是三张系列
DoudizhuXinzhou.prototype.is3ZhangType = function(cType) {
...
DoudizhuXinzhou.prototype.findSelectCardForAutoUp = function(tipCards, selectValue) {
...
// 更换牌值
DoudizhuXinzhou.prototype.changeSelectCardForAuto = function(tipCards, idx, selectValue) {
    tipCards[idx] = selectValue;
    return tipCards;
}

// 有足够的三张牌组
DoudizhuXinzhou.prototype.isEnough3SameCards = function (upSelectCards, groupNum) { 
    var start = false;
    var tmpCards = [];
...
DoudizhuXinzhou.prototype.findEnough3SameCard = function(oHands, upSelectCards, groupNum, zhadanMap) {
    var tmpSelect = [];
    var lastPoint = -1;
    for (var i = 0; i < upSelectCards.length; i++) {
...
// 找三张配合的牌
DoudizhuXinzhou.prototype.findDaiCardsWithSanZhang = function(oHands, upSelectCards, cType, groupNum, zhadanMap) {
...
// 自动提牌-划动-三张系列
DoudizhuXinzhou.prototype.handleAutoByTouchMoveSanZhang = function(upSelectCards, cType, oLastCards, oHands) {
    var groupNum = 1; 
...
DoudizhuXinzhou.prototype.findTheRightTouchMoveCards = function(upSelectCards, cType, oLastCards) {
...
DoudizhuXinzhou.prototype.findZhadanTouchMoveCards = function(upSelectCards) {
    var tCards = [];
    var lastPoint = -1;
...
// 自动提牌-划牌
DoudizhuXinzhou.prototype.handleAutoByTouchMove = function (bTouchMove, upSelectCards, cType, tipCardsArray, oLastCards, oHands) {
...
// 自动提牌-点牌
DoudizhuXinzhou.prototype.handleAutoByFirstCard = function(tipCardsArray, upSelectCard, cType) {
...
// 计算斗地主划牌结果
DoudizhuXinzhou.prototype.calAutoPutCards = function (bTouchMove, tipCardsArray, upSelectCards, oLastCards, oHands) {
...
DoudizhuXinzhou.prototype.getFindCardForFirstPut = function(cards, cType, count) {
    var realCards = null;
    var rateType = {};
...
DoudizhuXinzhou.prototype.calAutoPutCardWithFirstOut = function(upSelectCards, hands) {
    var data = {hasCardToUp:false};
...
// 计算选择的牌,排除炸弹
DoudizhuXinzhou.prototype.calSelectWithoutZhadan = function(upSelectCards, zhadanMap) {
...
// 计算牌组的炸弹
DoudizhuXinzhou.prototype.getZhaDanMapByCards = function(oHands) {
    var hands = oHands.slice();
    hands.sort(this.cardValueCmp);
...
if(typeof(module)!="undefined" && module.exports)    
    module.exports = DoudizhuXinzhou;
if(typeof(MjClient)!="undefined")
    MjClient.doudizhu_xinzhou = new DoudizhuXinzhou();
})();