...
var HomeView_qxxzmj = cc.Layer.extend({
    ...
    ctor: function () {
        ...
        // this._BtnRuturn.visible = true;
        // // setWgtLayout(this._BtnRuturn, [0.1, 0.14], [1, 1.02], [-0.6, -2.2]);
        // // setWgtLayout(this._BtnRuturn, [0.27, 0.27], [0.172, 1.62], [0, -0.025]);
        // this._BtnRuturn.addTouchEventListener(function (sender, Type) {
        //     switch (Type) {
        //         case ccui.Widget.TOUCH_ENDED:
        //             MjClient.showMsg("真的要退出游戏吗？",
        //                 function() {
        //                     if (cc.sys.isNative) {
        //                         cc.director.end();
        //                     } else {
        ...
        //要新
        var _btn_newPlayerPrize = homeui.node.getChildByName("btn_newPlayerPrize");
        if(_btn_newPlayerPrize) _btn_newPlayerPrize.visible = false;
	...
	//公告按钮
        ...
        //setWgtLayout(this._youjian, [0.06, 0.13], [1,0.97], [-5.5, -0.8]);
        //setWgtLayout(_zhanji, [0.06, 0.13], [1, 0.97], [-4.2, -0.8]);
        //setWgtLayout(_BtnHTP, [0.06, 0.13], [1, 0.97], [-2.88, -0.8]);
        //setWgtLayout(_setting, [0.06, 0.13], [1, 0.97], [-1.5, -0.8]);


        //setWgtLayout(this._BtnRuturn, [0.1, 0.14], [0.13, 1.23], [-0.6, -2.2]);
        //setWgtLayout(_btnAdv, [0.1, 0.14], [1, 1.02], [-0.6, -2.2]);
        ...
        setWgtLayout(_gonggao, [0.21, 0.5], [0.035, 0.42], [0.0, 0.0]);
        ...
        if (MjClient.remoteCfg.guestLogin == true ||
            ...
            // setWgtLayout(_btnAdv, [0.12, 0.12], [0.35, 0.065], [0, 0.0]);
            ...
            // _btnAdv.removeAllChildren();
            // _btnAdv.ignoreContentAdaptWithSize(true);
        }
        ...
        btn_share.addTouchEventListener(function (sender, Type) {
            switch (Type) {
                case ccui.Widget.TOUCH_ENDED:
                    {
                        var _sprite_hongbao = _btnShareGet.getChildByName("hongBao");
                        var _sprite = _btnShareGet.getChildByName("hongDian");
                        MjClient.Scene.addChild(new shareTodayLayer(_sprite.visible || _sprite_hongbao.visible));
        ...
        bg_share.runAction(cc.sequence(cc.scaleTo(1,0.8),cc.scaleTo(1,1.2)).repeatForever());
        ...
        _btnShareGet.schedule(function () {
            ...
            var myMemberId = MjClient.data.pinfo.myMemberId;
            var currentStr = MjClient.dateFormat(new Date(), "yyyyMMdd");
            var _sprite = _btnShareGet.getChildByName("hongDian");
            var _sprite_hongbao = _btnShareGet.getChildByName("hongBao");
            _sprite.setVisible(false);
            _sprite_hongbao.setVisible(false);
            ...
            if(lastStr >= currentStr){
                ...
                if(myMemberId){//代理
                    ...
                    _sprite_hongbao.setVisible(true);
                }
            }

            if (_shareTip)
            {
                _shareTip.visible = _sprite_hongbao.visible || _sprite.visible;
            }
    ...
    setPanel: function (leftPanel, rightPanel) {
        ...
        jiaoBiao.setPosition(jiaoBiao.width*0.5, this._playGroundRoom.height - jiaoBiao.height*0.4);
    ...
    setGameType: function (type) {
        ...
        // this._createRoom.addChild(clipper);
        ...
        if (gameClassList.length > 1) {
            this._guangbo.loadTexture(GameClassTitleIcon[MjClient.gameClass]);
        }
        else {
            this._guangbo.loadTexture("game_picture/logo_home.png");
        }
