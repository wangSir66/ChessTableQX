MjClient.MaxPlayerNum_chenZhouZiPai = 3;
MjClient.cardPath_chenZhouZiPai = "playing/paohuzi/";
var ActionType_chenZhouZiPai =  //图片提示
    {
        CHI:1,
        PENG:2,
        WEI:3,
...
//初始化玩家金币、名字、胡息
function InitUserCoinAndName_chenZhouZiPai(node, off){
    var pl = getUIPlayer_chenZhouZiPai(off);
    if(!pl){
        return;
    }
...
//向服务器发送王闯王钓操作
function HZWangChuangToServer_chenZhouZiPai(type){
    cc.log("====================HZWangChuangToServer_chenZhouZiPai=================");
...
//向服务器发送打牌操作
function HZPutCardToServer_chenZhouZiPai(card){
    cc.log("====================HZPutCardToServer_chenZhouZiPai=================");
    MjClient.gamenet.request("pkroom.handler.tableMsg", {
        cmd: "MJPut",
        card: card
    });
...
// 向服务器发送吃牌
function HZChiToServer_chenZhouZiPai(eatCards, biCards){
...
//像服务器发送碰牌
function HZPengToServer_chenZhouZiPai()
{
    if (MjClient.rePlayVideo != -1) return; // 回放时候不能请求
    cc.log("====================HZPengToServer_chenZhouZiPai=================");
    MjClient.gamenet.request("pkroom.handler.tableMsg", {
        cmd: "MJPeng"
    });
}

function MJPass2Net_chenZhouZiPai(){
    cc.log("====================send======pass=====");
    ...
    if(IsTurnToMe() && tData.tState == TableState.waitPut){
        ...
        HZPassConfirmToServer_chenZhouZiPai();
    }
};

// 向服务器发送过牌
function HZPassConfirmToServer_chenZhouZiPai() {
    if (MjClient.rePlayVideo != -1)  {
        return; // 回放时候不能请求
    }
...
//发送跑牌命令
function HZGangToServer_chenZhouZiPai(type){
    if (MjClient.rePlayVideo != -1) return; // 回放时候不能请求
    cc.log("====================HZGangToServer_chenZhouZiPai=================");
    MjClient.gamenet.request("pkroom.handler.tableMsg", {
        cmd: "MJGang",
        type: type,
        eatFlag: EatFlag_chenZhouZiPai()
...
//发送偎牌命令
function HZWeiToServer_chenZhouZiPai(type){
    if (MjClient.rePlayVideo != -1) return; // 回放时候不能请求
    cc.log("====================HZWeiToServer_chenZhouZiPai=================");
    MjClient.gamenet.request("pkroom.handler.tableMsg", {
        cmd: "HZWeiCard"
    });
}
...
//向服务器发送发牌命令
function HZNewCardToServer_chenZhouZiPai(){
    if (MjClient.rePlayVideo != -1) return; // 回放时候不能请求
    cc.log("====================HZNewCardToServer_chenZhouZiPai=================");
    MjClient.gamenet.request("pkroom.handler.tableMsg", {
        cmd: "HZNewCard"
    });
}
...
//向服务器发送胡牌命令
function MJHuToServer_chenZhouZiPai(){
    if (MjClient.rePlayVideo != -1) return; // 回放时候不能请求
    cc.log("====================MJHuToServer=================");
    MjClient.gamenet.request("pkroom.handler.tableMsg", {
        cmd: "MJHu",
...
//添加吃的牌
function addEatCard_chenZhouZiPai(node,name,mjNum,off){
    //根据牌的类型获得需要添加的节点
    ...
    var type = 2;
    //设置牌
    var newCard = getNewCard_chenZhouZiPai(mjNum,type,off);
    ...
    if(MjClient.MaxPlayerNum_chenZhouZiPai == 2 &&
        (node.getName() == "right")) {
        newCard.y -= 10;
    }
...
/**
 添加手牌(回放)
 */
function addHandCardReplay_chenZhouZiPai(tagName,index,mjNum,off){
    var _node = getNode_chenZhouZiPai(off);
    //根据牌的类型获得需要添加的节点
...
function cleanCardNode_chenZhouZiPai(off) {
    //获取结点
    if(off != 0){
        return;
    }
...
function resetCardNode_chenZhouZiPai(off) {
    if(off != 0){
        return;
    }
    var _node = getNode_chenZhouZiPai(off);
...
function addOrAdjustHandCard_chenZhouZiPai(tagName,index,mjNum,off){
    if(off != 0){
        return;
    }
    var _node = getNode_chenZhouZiPai(off);
    ...
    if(!newCard){
        cc.log("chow", "newCard");
        newCard = getNewCard_chenZhouZiPai(mjNum,1,off);
    }else{
        cc.log("chow", "getCard");
    }
    var scale_y = newCard.scaleY;
    ...
    if (MjClient.movingCard_paohuzi == newCard) {
        setCardSprite_chenZhouZiPai(newCard, newCard.tag, 1, false);
    ...
    if(newCard.lastPosition){
        ...
        doMovetoAction_chenZhouZiPai(newCard, cc.p(beginPoint.x, beginPoint.y + cardCount * off_y));
    }else{
        newCard.x = beginPoint.x;
        newCard.y = beginPoint.y + cardCount * off_y;
    }
    //newCard.opacity = 255;
    SetTouchCardHandler_chenZhouZiPai(newCard,off);
    ...
    //cc.log("chow", "addOrAdjustHandCard_chenZhouZiPai: name=" + newCard.getName() + " num=" + newCard.num + " tag =" + newCard.tag);
}

function doMovetoAction_chenZhouZiPai(node,endP){
    node.stopAllActions();
    var action = cc.moveTo(0.15,endP);
    node.runAction(action);
};

function resetHandCard_chenZhouZiPai(posNode,off, needAction, isDelay){
    if(MjClient.rePlayVideo == -1){
        if(off == 0) {
            var handNode = posNode.getChildByName("handNode");
            handNode.visible = true;
            cleanCardNode_chenZhouZiPai(off);
            ...
            for (var k = 0; k < cardArr.length; k++) {
                ...
                for (var j = 0; j < groupList.length; j++) {
                    addOrAdjustHandCard_chenZhouZiPai(k, j, groupList[j], off);
                }
            }
            resetCardNode_chenZhouZiPai(off);

            addTingSign_chenZhouZiPai(posNode); // 添加听牌角标
            ...
            for (var i = 0; i < cardArr.length; i++) {
                ...
                if(addNode.lastPosition){
                    ...
                    doMovetoAction_chenZhouZiPai(addNode, cc.p((winSize.width - totalWidth) / 2 + i * width * scale_x, 0));
                }else{
        ...
        var handCard = null;
        if(off == 0){
            if(MjClient.MaxPlayerNum_chenZhouZiPai == 3 || MjClient.MaxPlayerNum_chenZhouZiPai == 2){
                handNode = posNode.getChildByName("handNode");
                handCard = posNode.getChildByName("handCard");
            }else if(MjClient.MaxPlayerNum_chenZhouZiPai == 4){
        ...
        for(var k = 0;k < cardArr.length;k++){
            ...
            for(var j = 0;j < groupList.length;j++){
                if(off == 0){
                    if(MjClient.MaxPlayerNum_chenZhouZiPai == 3 || MjClient.MaxPlayerNum_chenZhouZiPai == 2){
                        addHandCard_chenZhouZiPai(k,j,groupList[j],off);
                    }else if(MjClient.MaxPlayerNum_chenZhouZiPai == 4){
                        addHandCardReplay_chenZhouZiPai(k,j,groupList[j],off);
                    }
                }else {
                    addHandCardReplay_chenZhouZiPai(k,j,groupList[j],off);
                }
        ...
        if(off == 0 && (MjClient.MaxPlayerNum_chenZhouZiPai == 3 || MjClient.MaxPlayerNum_chenZhouZiPai == 2)) {
            var scale_x = handCard.scaleX;
            var winSize = MjClient.size;
...
function checkCard_chenZhouZiPai(node, off){
    ...
    if(cardHandArr.length > cardHand.length){
        ...
        for(var i = 0;i < cardHandArr.length; i++){
            if(cardHandArr[i] != cardHand[i]){
                RemoveHandCard_chenZhouZiPai(node, cardHandArr[i], off);
...
function resetHandCard_chenZhouZiPaiEx(posNode,off, needAction, isDelay){
    if(off == 0) {
        //一局结束后个人准备状态时规避手牌出现上一句残牌显示
        if(MjClient.data.sData.tData == TableState.roundFinish) {
            return;
        }
        ...
        checkCard_chenZhouZiPai(posNode, off);
        var handNode = posNode.getChildByName("handNode");
        handNode.visible = true;
        cleanCardNode_chenZhouZiPai(off);
        handNode.removeAllChildren();
 ...
 */
function addHandCard_chenZhouZiPai(tagName,index,mjNum,off){
    if(off != 0){
        return;
    }
    var _node = getNode_chenZhouZiPai(off);
    ...
    //设置牌
    var newCard = getNewCard_chenZhouZiPai(mjNum,1,off);
    var scale_y = newCard.scaleY;
    //var parentCount = addNode.childrenCount;
    //首先根据name判断cpNode中是否已经添加
    var cardParent = addNode.getChildByTag(tagName);
    if(!cardParent){
        ...
        cardParent.width = newCard.width;
        addNode.addChild(cardParent);
        // SetTouchCardHandler_chenZhouZiPai(cardParent);
    }
    ...
    cc.log("chow", "addHandCard_chenZhouZiPai: name=" + newCard.getName() + " num=" + newCard.num + " tag =" + newCard.tag);
}
 ...
 **/
function getNewCard_chenZhouZiPai(mjNum, type, off, isTurn){
    var _node = getNode_chenZhouZiPai(off);
    var copyNode = null;
    if(type == 1){
        //手牌
    ...
    //cc.log("getNewCard_chenZhouZiPai:"+mjNum+","+type+","+off+","+isTurn);
    if(mjNum > 0){
        //创建一个带有麻将信息的麻将 cp为创建后的麻将
        setCardSprite_chenZhouZiPai(mjNode, mjNum, type, isTurn);
        if(type == 1){
            //只有手牌才会有点击事件
            SetTouchCardHandler_chenZhouZiPai(mjNode,off);
        }
    }
...
/**
 ...
 type:类型，1:手牌 2：吃牌 3：打的牌
 */
function setCardSprite_chenZhouZiPai(mjNode, mjNum, type, isTurn){
    MjClient.cardPath_chenZhouZiPai = ziPai.getCardFilePath();

    if(type == 1){
        mjNode.loadTexture(MjClient.cardPath_chenZhouZiPai+"hand" + mjNum + ".png");
    }
    if(type == 2){
        mjNode.loadTexture(MjClient.cardPath_chenZhouZiPai+"out" + mjNum + ".png");
        if(isTurn){
            mjNode.loadTexture(MjClient.cardPath_chenZhouZiPai+"huxiBG.png");
        }
    }
    if(type == 3 || type == 4){
        mjNode.loadTexture(MjClient.cardPath_chenZhouZiPai+"put" + mjNum + ".png");
        if(isTurn){
            mjNode.loadTexture(MjClient.cardPath_chenZhouZiPai+"normalBG.png");
        }
    }
    mjNode.tag = mjNum;
}

function checkTingCards_chenZhouZiPai(putCard){
    //听牌检测
    var sData = MjClient.data.sData;

    if(MjClient.data.sData.tData.tState == TableState.waitPut && curPlayerIsMe_chenZhouZiPai(0) && putCard === undefined){
        postEvent("showTingLayer", []);
        return;
    }

    if(putCard && (!curPlayerIsMe_chenZhouZiPai(0) || sData.tData.tState != TableState.waitPut)){
        putCard = undefined;
    }
...
//刷新手牌移动时的坐标
var btn_oPos = null;
function updateBtnMovedPosition_chenZhouZiPai(btn, eventType){
...
//MjClient.movingCard_chenZhouZiPai = null;
//MjClient.cloneCard = null;
//MjClient.putCard = null;
MjClient.hasPut = false;
function SetTouchCardHandler_chenZhouZiPai(card,off){
    ...
    card.addTouchEventListener(function(btn,eventType){
        ...
        if(eventType == ccui.Widget.TOUCH_BEGAN){
            ...
            setCardSprite_chenZhouZiPai(cloneCard, cardTag, 1, false);
            ...
            btn.x = touchPos.x;
            btn.y = touchPos.y;
            // updateBtnMovedPosition_chenZhouZiPai(btn, eventType);
            ShowPutCardIcon_chenZhouZiPai();

            checkTingCards_chenZhouZiPai(btn.tag);
            var alignWidth = btn.scale * btn.width;
            setCardSprite_chenZhouZiPai(btn, btn.tag, 3, false);
        ...
        if(eventType == ccui.Widget.TOUCH_ENDED ||
            ...
            var pl = getUIPlayer_chenZhouZiPai(0);
            var tData = MjClient.data.sData.tData;
            var isPut = false;
            var cutLine = MjClient.playui.jsBind.cutLine._node;

            setCardSprite_chenZhouZiPai(btn, btn.tag, 1, false);
            ...
            var putCardCb = function () {
                ...
                if(((!pl.canNotPutCard||pl.canNotPutCard.indexOf(btnTag)==-1) &&(getOtherWeiCards_chenZhouZiPai(btnTag)||(pl.limitHuPutCard && pl.limitHuPutCard.indexOf(btnTag) != -1))) &&
                    ...
                    !MjClient.isCommon&& !MjClient.hasPut)
                {
                    ...
                    var putCard = getNewCard_chenZhouZiPai(btnTag, 3 ,off);
                    ...
                    if(getOtherWeiCards_chenZhouZiPai(btnTag)){
                                ...
                                if(pl.limitHuPutCard && pl.limitHuPutCard.indexOf(btnTag) != -1){
                                    MjClient.showMsg("吃边打边，这局将只能胡"+ MjClient.majiang.getLimitHuDesc(btnTag, tData)+"，是否确定？",
                                        function(){
                                            ...
                                            HZPutCardToServer_chenZhouZiPai(btnTag);
                                            MjClient.playui.ResetHandCard(MjClient.playui._downNode,0);

                                            checkTingCards_chenZhouZiPai(btnTag);
                                        },
                                        function(){
                                            MjClient.playui.ResetHandCard(MjClient.playui._downNode,0);
                                            ShowPutCardIcon_chenZhouZiPai();
            ...
            if(!isPut || isWangBa)
            {
                ...
                if(selectParentTag == btnParent.tag){
                    ...
                    if(selectArr){
                        //_changeArrIndex_chenZhouZiPai(selectArr, btnTag, btn);
                        MjClient.HandCardArr[btnParent.tag].splice(parseInt(btnName),1);
                        _fixArrIndex_chenZhouZiPai(selectArr, btnTag, btn);
                    }
                }
...
//根据坐标Y，添加到数组对应的位置
function _fixArrIndex_chenZhouZiPai(arr, btnTag, btn){
    if(arr){
        if(_isKan_chenZhouZiPai(arr)){
            arr.push(btnTag);
        ...
        cc.log('_fixArrIndex_chenZhouZiPai====:',arr);
    }

}

//改变数组元素的位置
function _changeArrIndex_chenZhouZiPai(arr, btnTag, btn){
    if(arr && !_isKan_chenZhouZiPai(arr)){
        var len = arr.length;
        for(var i = 0; i < len; i++){
            var tag = arr[i];
            if(tag == btnTag){
        ...
        _fixArrIndex_chenZhouZiPai(arr, btnTag, btn);
    }
}

//数组里的是否为三个相同的牌
function _isKan_chenZhouZiPai(arr){
    if(arr){
        var len = arr.length;
        if(len < 3){
            return false;
...
//断线重连之后，显示之前玩家打的牌
function DealOffLineCard_chenZhouZiPai(node, off) {
    cc.log("====================DealOffLineCard_chenZhouZiPai=================", off);
    var sData = MjClient.data.sData;
    var tData = sData.tData;
    ...
    if(typeof(tData.currCard) != "undefined" && tData.currCard != -1){
        var uids = tData.uids;
        var selfIndex = (uids.indexOf(SelfUid()) + off) % MjClient.MaxPlayerNum_chenZhouZiPai;
        if(uids[selfIndex] == uids[tData.curPlayer]){
            var putCard = getNewCard_chenZhouZiPai(tData.currCard, 3, off);
    ...
    cc.log("====================DealOffLineCard_chenZhouZiPai================= end", off);
}

// 通过玩家idx获取UIBind（down left right xing）
function getUIBindByIndex_chenZhouZiPai(idx) {
    var sData = MjClient.data.sData;
    var tData = sData.tData;
...
// 落牌动画
function fall_chenZhouZiPai() {
    var sData = MjClient.data.sData;
    var tData = sData.tData;

    // tData.lastPlayer没同步!
    var lastPlayer = (tData.curPlayer - 1 + tData.maxPlayer) % tData.maxPlayer;
    cc.log("chow", "fall_chenZhouZiPai"+ " lastPlayer = " + lastPlayer);
    ...
    if (out) {
        ...
        cc.log("chow", "fall_chenZhouZiPai" + JSON.stringify(out.getPosition()));
...
//发牌
MjClient.isCommon = false;
function DealNewCard_chenZhouZiPai(node, msg, off) {
    //没有发牌过来的时候直接忽略
    if (!msg.newCard) {
        RemovePutCardOut_chenZhouZiPai();
        MjClient.playui.EatVisibleCheck();
        return;
    }

    AddPutCard_chenZhouZiPai(node, msg, off); // 注意各个玩法验证！
    //存在发牌
    var sData = MjClient.data.sData;
    var tData = sData.tData;
    var uids = tData.uids;
    ...
    if (uids[selfIndex] == msg.uid) {
        cc.log("DealNewCard_chenZhouZiPai  newCard@@ ", msg.newCard);
        //清理之前玩家打的牌
        var lastPlayer = tData.lastPlayer;
        if (lastPlayer != -1) {
            //var lastOff = (off + MjClient.MaxPlayerNum_chenZhouZiPai - selfIndex) % MjClient.MaxPlayerNum_chenZhouZiPai;
            // RemovePutCardOut_chenZhouZiPai();
        ...
        var putCard = getNewCard_chenZhouZiPai(msg.newCard, 3, off, showPkBack);
        var putNode = node.getChildByName("put");
        ...
        if (!msg.isCommon) {
            if (MjClient.rePlayVideo == -1) {
                ...
                for (var i = 0; i < cardArr.length; i++) {
                    ...
                    if (tmpArr.length <= 2) {
                        isAdd = true;
                        //addHandCard_chenZhouZiPai(i,tmpArr.length.length + 1,msg.newCard,off);
...
function setPutCardPos_chenZhouZiPai(outNode, outCard, idx, off) {
    var tData = MjClient.data.sData.tData;
    var maxPlayer = tData.maxPlayer;
    var type = ziPai.getUiLayoutType();
    var isZuoXing = false;
    cc.log("setPutCardPos_chenZhouZiPai:" + isZuoXing);
    if (maxPlayer == 3 /*|| isZuoXing*/) { // 3人
        ...
        else if (off == 1)
        {
            ...
            //2人15张偏右 对家弃牌y坐标再向下一点
            if (ziPai.getUiLayoutType() == 0 && tData.areaSelectMode.isShiWuZhang)
            {
                outCard.y = (row + 2.3) * outCard.height;
            }
        }
...
function setJiachuiTextTips_chenZhouZiPai(node, off) {
    node.visible = false;
    var tData = MjClient.data.sData.tData;
    var pl = getUIPlayer_chenZhouZiPai(off);
...
function setJiachuiImgTips_chenZhouZiPai(node, off) {
    node.visible = false;
    var tData = MjClient.data.sData.tData;
    var pl = getUIPlayer_chenZhouZiPai(off);
...
function AddPutCard_chenZhouZiPai(node, msg, off) {
    var sData = MjClient.data.sData;
    var tData = sData.tData;
    var off_index = (tData.uids.indexOf(SelfUid()) + off) % MjClient.MaxPlayerNum_chenZhouZiPai;
    if (off_index == tData.curPlayer) { // off对应玩家为当前玩家
        ...
        var out = getNewCard_chenZhouZiPai(card, 2, off);
        var count = pl.mjput.length - 1;
        var width = out.width;
        var height = out.height;
        setPutCardPos_chenZhouZiPai(outNode, out, pl.mjput.length - 1, off);
...
//翻醒发牌
function DealFanXingNewCard_chenZhouZiPai(card){
    var putCard = getNewCard_chenZhouZiPai(card, 4, 0);
...
//王闯/王钓
function DealWangChuang_chenZhouZiPai(node,msg,off){
    var sData = MjClient.data.sData;
    var tData = sData.tData;
    ...
    if(uids[selfIndex] == msg.uid){
        ...
        if(pl.wangType == 1){
            var putCard = getNewCard_chenZhouZiPai(91, 3, off);
            var putNode = node.getChildByName("put");
            putCard.scaleX = putCard.width/putNode.width - 0.16;
            putCard.scaleY = putCard.width/putNode.width - 0.06;
            putCard.x = putCard.width/2;
            ...
            ShowEatActionAnim_chenZhouZiPai(node, ActionType_chenZhouZiPai.WANGDIAO, off);

            var delay = cc.delayTime(1.5);
            var callback = function(){
                RemovePutCardOut_chenZhouZiPai();
                //HZNewCardToServer_chenZhouZiPai();
            }
            //putNode.runAction(cc.sequence(delay,cc.callFunc(callback)));
        }else if(pl.wangType == 2){
            var putCard = getNewCard_chenZhouZiPai(91, 3, off);
            var putCard1 = getNewCard_chenZhouZiPai(91, 3, off);
            var putNode = node.getChildByName("put");
            putCard.scaleX = putCard.width/putNode.width - 0.16;
            putCard.scaleY = putCard.width/putNode.width - 0.06;
            ...
            ShowEatActionAnim_chenZhouZiPai(node, ActionType_chenZhouZiPai.WANGCHUANG, off);
            var delay = cc.delayTime(1.5);
            var callback = function(){
                RemovePutCardOut_chenZhouZiPai();
                //HZNewCardToServer_chenZhouZiPai();
            ...
            var putCard = getNewCard_chenZhouZiPai(91, 3, off);
            var putCard1 = getNewCard_chenZhouZiPai(91, 3, off);
            var putCard2 = getNewCard_chenZhouZiPai(91, 3, off);
            var putNode = node.getChildByName("put");
            putCard.scaleX = putCard.width/putNode.width - 0.16;
            putCard.scaleY = putCard.width/putNode.width - 0.06;
            putCard1.scaleX = putCard.width/putNode.width - 0.16;
            ...
            ShowEatActionAnim_chenZhouZiPai(node, ActionType_chenZhouZiPai.WANGZHA, off);
            var delay = cc.delayTime(1.5);
            var callback = function(){
                RemovePutCardOut_chenZhouZiPai();
                //HZNewCardToServer_chenZhouZiPai();
...
//处理摸最后一张牌
function DealShowLastCard_chenZhouZiPai(node, msg, off){
    cc.log("======DealShowLastCard_chenZhouZiPai======= " + off);
    MjClient.hasPut = true;
    var sData = MjClient.data.sData;
    var tData = sData.tData;
    var uids = tData.uids;
    ...
    if(uids[selfIndex] == msg.uid)
    {
        ...
        setTimeout(function(){
            RemovePutCardOut_chenZhouZiPai(true);
            var pl = getUIPlayer_chenZhouZiPai(0);
...
//处理出牌
function DealPutCard_chenZhouZiPai(node, msg, off) {
    AddPutCard_chenZhouZiPai(node, msg, off); // 注意各个玩法验证！
    ...
    if (uids[selfIndex] == msg.uid) {
        ...
        if (SelfUid() != msg.uid || MjClient.rePlayVideo != -1) {
            ...
            putNode.stopAllActions();
            putNode.removeAllChildren();
            var putCard = getNewCard_chenZhouZiPai(msg.card, 3, off);
            putCard.scaleX = putCard.width / putNode.width - 0.16;
            putCard.scaleY = putCard.width / putNode.width - 0.06;
            putCard.x = putCard.width / 2;
            putCard.y = putCard.height / 2;
        ...
        if (off == 0) {
            removeTingSign_chenZhouZiPai(node);
        }

        if (MjClient.rePlayVideo != -1) {
            RemoveHandCard_chenZhouZiPai(node, msg.card, off);
            MjClient.playui.ResetHandCard(node, off);
        }
    }
}

function calculateHintPutList_chenZhouZiPai() {
...
function addTingSign_chenZhouZiPai(node) {
...
function removeTingSign_chenZhouZiPai(node) {
    var handNode = node.getChildByName("handNode");
    if (!handNode) {
        return;
    }
...
//吃牌
function DealChiCard_chenZhouZiPai(node, msg, off) {
    cc.log("======DealChiCard_chenZhouZiPai======= " + off);
    var sData = MjClient.data.sData;
    var tData = sData.tData;
    var uids = tData.uids;
    var selfIndex = (uids.indexOf(SelfUid()) + off) % MjClient.MaxPlayerNum_chenZhouZiPai;
    if (tData.curPlayer == selfIndex) {
        ...
        for (var i = 0; i < tmpEatCards.length; i++) {
            RemoveHandCard_chenZhouZiPai(node, tmpEatCards[i], off);
        }
            ...
            for (var k = 0; k < biCards.length; k++) {
                ...
                for (var m = 0; m < biArr.length; m++) {
                    if (tmpBiArr.indexOf(biArr[m]) >= 0) {
                        ...
                        RemoveHandCard_chenZhouZiPai(node, biArr[m], off);
                    }
                }
            }
        }
        ...
        for (var i = 0; i < eatCards.length; i++) {
            var card = getNewCard_chenZhouZiPai(eatCards[i], 2, off);
            if (i == 2) {
                card.setColor(cc.color(170, 170, 170));
            }
            ...
            card.zIndex = 4 - i;
            card.x = 0;
            card.y = off_count * getOffY_chenZhouZiPai(node, card);
            setChiCardAnchorPoint_chenZhouZiPai(node, card);
        ...
        if (biCards) {
            ...
            for (var k = 0; k < biCards.length; k++) {
                var biArr = biCards[k];
                for (var m = 0; m < biArr.length; m++) {
                    var card = getNewCard_chenZhouZiPai(biArr[m], 2, off);
                    if (m == 2) {
                        card.setColor(cc.color(170, 170, 170));
                    }
                    setChiCardAnchorPoint_chenZhouZiPai(node, card);
        ...
        var parentPos = getChiCardParentPosition_chenZhouZiPai(node, cardWidth, off, selfColCount);
        var moveX0 = 0;
        ...
        parent.x = moveX0;
        setCardParentPosY_chenZhouZiPai(node, parent);
        ...
        if (biCards) {
            ShowEatActionAnim_chenZhouZiPai(node, ActionType_chenZhouZiPai.XIAHUO, off);
        } else {
            ShowEatActionAnim_chenZhouZiPai(node, ActionType_chenZhouZiPai.CHI, off);
        }
...
function setChiCardAnchorPoint_chenZhouZiPai(node,newCard) {
    if(node.getName() == "down"){
        newCard.anchorX = 0;
        newCard.anchorY = 0;
    }else if(node.getName() == "right"){
...
function getOffY_chenZhouZiPai(node,newCard) {
    var off_y = 0;
    if(node.getName() == "down"){
        off_y = newCard.height;
    }else if(node.getName() == "right"){
...
function getChiCardParentPosition_chenZhouZiPai(node, newCardWidth, off, selfColCount, orignColPosX) {
...
function setCardParentPosY_chenZhouZiPai(node, cardParent) {
    var eatNode = node.getChildByName("eatNode");
    if(node.getName() == "down"){
        cardParent.y = 0;
    }else if (node.getName() == "right"){
...
function getShowCardIdx_chenZhouZiPai(node, chiTip) {
    var idx;
    switch(node.getName()) {
        case "down":
        case "xing":
...
// 处理碰
function DealPengCard_chenZhouZiPai(node, msg, off){
    cc.log("======DealChiCard_chenZhouZiPai======= ");
    var sData = MjClient.data.sData;
    var tData = sData.tData;
    var uids = tData.uids;
    var selfIndex = (uids.indexOf(SelfUid()) + off) % MjClient.MaxPlayerNum_chenZhouZiPai;
    if(tData.curPlayer == selfIndex){
        ...
        for(var i = 0;i < 3; i++) {
            var card = getNewCard_chenZhouZiPai(pengCard,2,off);
            cardWidth = card.width//*eatNode.scaleX;
            var childCount = parent.childrenCount;
            card.zIndex = 4 - childCount;
            card.x = 0;
            card.y = childCount * getOffY_chenZhouZiPai(node, card);
            setChiCardAnchorPoint_chenZhouZiPai(node, card);
            parent.addChild(card);
        }
        eatNode.addChild(parent);

        RemoveHandCard_chenZhouZiPai(node,pengCard,off);
        RemoveHandCard_chenZhouZiPai(node,pengCard,off);
        MjClient.playui.ResetHandCard(node, off);
        ShowEatActionAnim_chenZhouZiPai(node, ActionType_chenZhouZiPai.PENG, off);
        ...
        var parentPos = getChiCardParentPosition_chenZhouZiPai(node, cardWidth, off, 1);
        var moveX0 = 0;
        var outNodeCount = eatNode.getChildrenCount();
        if(outNodeCount>0){
            if(node.getName() == "down" || node.getName() == "left"){
...
//偎牌
function DealWeiCard_chenZhouZiPai(node, msg, off) {
    cc.log("======DealWeiCard_chenZhouZiPai======= ");
    var sData = MjClient.data.sData;
    var tData = sData.tData;
    var uids = tData.uids;
    var selfIndex = (uids.indexOf(SelfUid()) + off) % MjClient.MaxPlayerNum_chenZhouZiPai;
    if (tData.curPlayer == selfIndex) {
        ...
        RemovePutCardOut_chenZhouZiPai();
        ShowEatActionAnim_chenZhouZiPai(node, ActionType_chenZhouZiPai.WEI, off);
        ...
        for (var i = 0; i < 3; i++) {
            ...
            {
                if (i == getShowCardIdx_chenZhouZiPai(node, "wei")) {
                    isTurn = false;
                }
                var card = getNewCard_chenZhouZiPai(weiCard, 2, off, isTurn);
            } else if (MjClient.majiang.getCardShowType(tData.uids[tData.curPlayer], weiCard) == 1) {
                var card = getNewCard_chenZhouZiPai(weiCard, 2, off);
                ...
                var card = getNewCard_chenZhouZiPai(weiCard, 2, off, true);
            }
            cardWidth = card.width //*eatNode.scaleX;
            card.zIndex = 4 - i;
            card.x = 0;
            card.y = i * getOffY_chenZhouZiPai(node, card);
            setChiCardAnchorPoint_chenZhouZiPai(node, card);
            parent.addChild(card);
        }
        eatNode.addChild(parent);

        RemoveHandCard_chenZhouZiPai(node, weiCard, off);
        RemoveHandCard_chenZhouZiPai(node, weiCard, off);
        MjClient.playui.ResetHandCard(node, off);
...
// 跑牌或者提牌
function DealGangCard_chenZhouZiPai(node, msg, off) {
    var sData = MjClient.data.sData;
    var tData = sData.tData;
    var uids = tData.uids;
    var selfIndex = (uids.indexOf(SelfUid()) + off) % MjClient.MaxPlayerNum_chenZhouZiPai;
    ...
    if (uids.indexOf(gangUid) == selfIndex) {
        ...
        if (msg.type == 2) {
            RemovePutCardOut_chenZhouZiPai();
            ShowEatActionAnim_chenZhouZiPai(node, ActionType_chenZhouZiPai.PAO, off);
        } else {
            if (!msg.isGangHand && msg.type == 1) {
                RemovePutCardOut_chenZhouZiPai();
            }
            ShowEatActionAnim_chenZhouZiPai(node, ActionType_chenZhouZiPai.TI, off);
        }
        ...
        for (var i = 0; i < 4; i++) {
            ...
            if (msg.type == 2) {
                isTurn = false;
                var card = getNewCard_chenZhouZiPai(gangCard, 2, off, isTurn);
                ...
                MjClient.majiang.getCardShowType(tData.uids[tData.curPlayer], gangCard) == 2)
            {
                if(MjClient.gameType === MjClient.GAME_TYPE.SHAO_YANG_FANG_PAO_FA){
                    ...
                    if (i == getShowCardIdx_chenZhouZiPai(node, "ti")) {
                        isTurn = false;
                    }
                    var card = getNewCard_chenZhouZiPai(gangCard, 2, off, isTurn);
                }
            } else if (MjClient.majiang.getCardShowType(tData.uids[tData.curPlayer], gangCard) == 1) {
                var card = getNewCard_chenZhouZiPai(gangCard, 2, off, false);
                var shade = new cc.Sprite("playing/paohuzi/huxiBG1.png");
                shade.opacity = 100;
                shade.x = shade.width / 2;
                shade.y = shade.height / 2;
                ...
                var card = getNewCard_chenZhouZiPai(gangCard, 2, off, true);
            }
            cardWidth = card.width //*eatNode.scaleX;
            card.zIndex = 4 - i;
            card.x = 0;
        ...
        RemoveHandCard_chenZhouZiPai(node, gangCard, off);
        RemoveHandCard_chenZhouZiPai(node, gangCard, off);
        RemoveHandCard_chenZhouZiPai(node, gangCard, off);
        if (msg.isGangHand) {
            RemoveHandCard_chenZhouZiPai(node, gangCard, off);
        ...
        MjClient.playui.ResetHandCard(node, off);
        // addTingSign_chenZhouZiPai(node); // 添加听牌角标 (提 跑可能手牌未变动)
        ...
        var parentPos = getChiCardParentPosition_chenZhouZiPai(node, cardWidth, off, 1, orignColPosX);
        var moveX0 = 0;
        var outNodeCount = eatNode.getChildrenCount();
        if (outNodeCount > 0) {
            if (node.getName() == "down" || node.getName() == "left") {
...
// 处理胡
function DealHu_chenZhouZiPai(node, msg, off){
    var sData = MjClient.data.sData;
    var tData = sData.tData;
    var selfIndex = (tData.uids.indexOf(SelfUid()) + off) % MjClient.MaxPlayerNum_chenZhouZiPai;
    var pl = getUIPlayer_chenZhouZiPai(off);
    ...
    if(pl){
        pl.eatFlag = 0;
        MjClient.playui.EatVisibleCheck();
        ShowEatActionAnim_chenZhouZiPai(node,ActionType_chenZhouZiPai.HU,off);
    }
}

function DealAddCard_chenZhouZiPai(node,msg, off){
    if(MjClient.rePlayVideo == -1 && off != 0){
        return;
    }
    var cardArr = MjClient.HandCardArr;
...
//清除打出的牌
function RemovePutCardOut_chenZhouZiPai(noAction){

    var jsBind = MjClient.playui.jsBind;
    var uiList = [jsBind.down, jsBind.right, jsBind.left];
    if (MjClient.MaxPlayerNum_chenZhouZiPai == 4)
    {
        uiList = [jsBind.down, jsBind.xing, jsBind.right, jsBind.left];
    }
...
//发手牌时再清理一遍
function clearMJHandCardUI_chenZhouZiPai() {
    // var handNode = MjClient.playui._downNode.getChildByName("handNode");
    // handNode.removeAllChildren();
...
//出牌表示状态
function ShowPutCardIcon_chenZhouZiPai() {
...
//清除手牌
function RemoveHandCard_chenZhouZiPai(node,card,off){
    if(MjClient.rePlayVideo == -1 && off != 0){
        return;
    }
    var cardArr = MjClient.HandCardArr;
...
// 初始化吃碰杠胡动作
function InitShowEatActionNode_chenZhouZiPai(plNode){
...
// 重置吃碰杠胡动作
function resetEatActionAnim_chenZhouZiPai()
{
    var jsBind = MjClient.playui.jsBind;
    ...
    var tData = MjClient.data.sData.tData;
    var count = MjClient.MaxPlayerNum_chenZhouZiPai;
    for(var i = 0; i < count; i++)
    {
        InitShowEatActionNode_chenZhouZiPai(ui[i]._node);
    }
}

//播放头像移动
function tableStartHeadMoveAction_chenZhouZiPai(node){
...
//重置3家头像位置
function reConectHeadLayout_chenZhouZiPai(node){
    var tData = MjClient.data.sData.tData;
    var down = node.getChildByName("down").getChildByName("head");
    var left = node.getChildByName("left").getChildByName("head");
    var right = node.getChildByName("right").getChildByName("head");
    ...
    resetEatActionAnim_chenZhouZiPai();
...
//根据偏移获得玩家node
function getNode_chenZhouZiPai(off){
    var _node = null;
    var tData = MjClient.data.sData.tData;
    if (tData.maxPlayer != 4){
        if (off == 1 || off == 2){
 ...
 * @return {object} 玩家数据
 */
function getUIPlayer_chenZhouZiPai(off){
    var sData = MjClient.data.sData;
    var tData = sData.tData;
    var uids = tData.uids;
...
// 获取ui头像，通过偏移值
function getUIHeadByOff_chenZhouZiPai(off){
    var pl = getUIPlayer_chenZhouZiPai(off);
    if(!pl)
    {
        return {};
 ...
 * 下载录音, 调用 播放函数
 * */
function downAndPlayVoice_chenZhouZiPai(uid, filePath){
    var index = getUiOffByUid_chenZhouZiPai(uid);
...
function getUiOffByUid_chenZhouZiPai(uid){
    var sData = MjClient.data.sData;
    var tData = sData.tData;
    var uids = tData.uids;
    var selfIndex = uids.indexOf(SelfUid());
...
//设置微信头像
function setWxHead_chenZhouZiPai(node, d, off){
    if(d.uid == getUIHeadByOff_chenZhouZiPai(off).uid){
        ...
        COMMON_UI.addNobleHeadFrame(nobody,getUIPlayer_chenZhouZiPai(off))
    }
}

//显示玩家庄的ui
function showUserZhuangLogo_chenZhouZiPai(node, off){
    var tData = MjClient.data.sData.tData;
    var pl = getUIPlayer_chenZhouZiPai(off);
    node.zIndex = 100;
    if(tData && pl){
...
function showUserZhuangLogoZuoXing_chenZhouZiPai(node, off){
    var tData = MjClient.data.sData.tData;
    var pl = getUIPlayer_chenZhouZiPai(off);
    node.zIndex = 100;
    if(tData && pl){
...
//显示房主  add by sking
function showFangzhuTagIcon_chenZhouZiPai(node,off)
{
    var pl = getUIPlayer_chenZhouZiPai(off);
    if(!pl) //位置上没人则删掉房主标签
    {
...
// 清理ui
function clearCardUI_chenZhouZiPai(node,off){
    mylog("clearCardUI_chenZhouZiPai");
    ...
    for(var i = 0; i < children.length; i++)
    {
        ...
        {
            InitShowEatActionNode_chenZhouZiPai(ni.getParent());
        }
    }
}

function EatFlag_chenZhouZiPai(){
    var eat = MjClient.playui.jsBind.eat;
    var eatFlag = 0;
...
function huXiScore_chenZhouZiPai(type,card)
{
...
//刷新胡息
function UpdateHuXi_chenZhouZiPai(off){
    var sData = MjClient.data.sData;
    var tData = sData.tData;
    var score = 0;
    var pl = getUIPlayer_chenZhouZiPai(off);
    ...
    if(mjpao.length>0){
        for(var p=0;p<mjpao.length;p++){
            score += huXiScore_chenZhouZiPai("paoPai",mjpao[p]);
        }
    }
    ...
    if(mjpeng.length>0){
        for(var i=0;i<mjpeng.length;i++){
            score += huXiScore_chenZhouZiPai("peng",mjpeng[i]);
        }
    }
        ...
        var chiScore = function(cards){
            ...
            for(var k=0;k<chiXiArr.length;k++){
                ...
                if(cards.toString() == chiXiList.toString()){
                    score += huXiScore_chenZhouZiPai("chi",cards[0]);
                }
            }
        };
    ...
    if(mjti.length>0){
        for (var t = 0; t < mjti.length; t++) {
            score += huXiScore_chenZhouZiPai("tiPai", mjti[t]);
        }
    }
    ...
    if(mjwei && mjwei.length>0){
        for (var w = 0; w < mjwei.length; w++) {
            if(MjClient.majiang.getCardShowType(pl.info.uid, mjwei[w]) != 0) {
                score += huXiScore_chenZhouZiPai("weiPai", mjwei[w]);
            }
        }
    }
    ...
    if(tData.xingPlayer == selfIndex){
        pl = getUIPlayer_chenZhouZiPai(2);
        //提牌
        var mjti = pl.mjgang1;
        if(mjti.length>0){
            for(var t=0;t<mjti.length;t++){
...
//设置转盘显示状态
function IsArrowVisible_chenZhouZiPai(){
    var pl = getUIPlayer_chenZhouZiPai(0);
    if (!pl)
    {
        return;
...
// 显示吃碰杠胡动作
function ShowEatActionAnim_chenZhouZiPai(node, actType, off){
    var delayTime = 1;
    switch(actType)
    {
        case ActionType_chenZhouZiPai.FANXING:
            delayTime = 1.6;
            break;
        default:
            break;
    ...
    switch(actType)
    {
        case ActionType_chenZhouZiPai.CHI:
        ...
        case ActionType_chenZhouZiPai.PENG:
        ...
        case ActionType_chenZhouZiPai.WEI:
        ...
        case ActionType_chenZhouZiPai.HU:

            eatActionChild = eatActionNode.getChildByName("hu");
            eatActionChild.visible = true;
            break;
        case ActionType_chenZhouZiPai.PAO:
            eatActionChild = eatActionNode.getChildByName("pao");
            eatActionChild.visible = true;
            eatActionChild.runAction(cc.sequence(cc.delayTime(delayTime), cc.callFunc(callback)));
            break;
        case ActionType_chenZhouZiPai.TI:
            ...
            eatActionChild.runAction(cc.sequence(cc.delayTime(delayTime), cc.callFunc(callback)));
            break;
        case ActionType_chenZhouZiPai.WANGDIAO:
            eatActionChild = eatActionNode.getChildByName("wangDiao");
            eatActionChild.visible = true;
            eatActionChild.runAction(cc.sequence(cc.delayTime(delayTime), cc.callFunc(callback)));
            break;
        case ActionType_chenZhouZiPai.WANGCHUANG:
            eatActionChild = eatActionNode.getChildByName("wangChuang");
            eatActionChild.visible = true;
            eatActionChild.runAction(cc.sequence(cc.delayTime(delayTime), cc.callFunc(callback)));
            break;
        case ActionType_chenZhouZiPai.XIAHUO:
...
//显示玩家信息
function showPlayerInfo_chenZhouZiPai(off, node){
    //var tData = MjClient.data.sData.tData;
    var pl = getUIPlayer_chenZhouZiPai(off);
    if(pl){
...
function resetChiParam_chenZhouZiPai(){

}

function setChiVisible_chenZhouZiPai(){
    var eat = MjClient.playui.jsBind.eat;
...
function showSelectEatCards_chenZhouZiPai(node,cardNum){
    var eat = MjClient.playui.jsBind.eat;
    eat.chi._node.visible = false;
    eat.guo._node.visible = false;
    eat.peng._node.visible = false;
    ...
    addEatCards_chenZhouZiPai(parent,cardArr,putCard);
}

//居中动画,数组里的节点居中排列 只关心x轴
doMoveCenterAction_chenZhouZiPai = function(arr,isScale){
    arr.reverse();
    var gap = 5;
...
function commitEatCards_chenZhouZiPai(eatCards, biArr){
    if ((MjClient.data.sData.players[SelfUid()].eatFlag & 32) > 0) {
        MjClient.showMsg("吃牌后视为过胡，确定吃吗？", function() {
            setChiVisible_chenZhouZiPai();
            HZChiToServer_chenZhouZiPai(eatCards, biArr);
        }, function() {}, "1");
    } else {
        setChiVisible_chenZhouZiPai();
        HZChiToServer_chenZhouZiPai(eatCards, biArr);
...
//添加可以吃的牌
function addEatCards_chenZhouZiPai(node,handCardArr,putCard){
    var cardPath = MjClient.cardPath_chenZhouZiPai;
    ...
    for(var i = 0;i < chiSet.length;i++){
        ...
        chiCloneBtn.addClickEventListener(function(chiBtn){
            ...
            if(tmpHandCardArr.indexOf(putCard) < 0){
                // setChiVisible_chenZhouZiPai();
                // HZChiToServer_chenZhouZiPai(chiArr,null);

                commitEatCards_chenZhouZiPai(chiArr, null);
            }else{
                var biSet = MjClient.majiang.getBiCards(tmpHandCardArr,putCard);
                if(biSet && biSet.length > 0){
                    addFirstBiCards_chenZhouZiPai(node,biSet,putCard,tmpHandCardArr,chiArr);
                }
            }
        });
    }
    //传统布局向右移 11-8 evan
    //if(ziPai.getUiLayoutType() == 0){//新布局
        doMoveCenterAction_chenZhouZiPai([chiBg], false);
    //}
}

//添加第一个比牌
function addFirstBiCards_chenZhouZiPai(node,biSet,putCard,handCardArr,chiArr){
    var cardPath = MjClient.cardPath_chenZhouZiPai;
    cardPath = ziPai.getCardFilePath();
    ...
    for(var i = 0;i < biSet.length;i++){
        ...
        cloneBtn.addClickEventListener(function(btn){
            ...
            if(tmpHandCardArr.indexOf(putCard) >= 0){
                var biSet = MjClient.majiang.getBiCards(tmpHandCardArr,putCard);
                if(biSet && biSet.length > 0){
                    addSecBiCards_chenZhouZiPai(node,biSet,putCard,tmpHandCardArr,chiArr,biArr);
                }
            }else{
                // setChiVisible_chenZhouZiPai();
                // HZChiToServer_chenZhouZiPai(chiArr,[biArr]);
                commitEatCards_chenZhouZiPai(chiArr,[biArr]);
            }
        });
    }
    ...
    if(ziPai.getUiLayoutType() == 0){//新布局
        ...
        doMoveCenterAction_chenZhouZiPai([chiBg,biBg],isScale);
    }else{
        //传统布局向右移 11-8 evan
        chiBg.scale = biBg.scale;
        doMoveCenterAction_chenZhouZiPai([chiBg,biBg],isScale);
        // biBg.scale = chiBg.scale;
        // doMoveCenterAction_chenZhouZiPai2([chiBg,biBg],isScale);
    }
}

//居中动画,数组里的节点居中排列 只关心x轴
doMoveCenterAction_chenZhouZiPai2 = function(arr,isScale){
    isScale = isScale === undefined ? true : isScale;
    var gap = 5;
    if(arr && arr.length > 0){
        var len = arr.length;
...
//添加第二个比牌
function addSecBiCards_chenZhouZiPai(node,biSet,putCard,handCardArr,chiArr,firstBiArr){
    var cardPath = MjClient.cardPath_chenZhouZiPai;
    cardPath = ziPai.getCardFilePath();
    ...
    for(var i = 0;i < biSet.length;i++){
        ...
        cloneBtn.addClickEventListener(function(btn){
            ...
            btn.setBright(false);
            // setChiVisible_chenZhouZiPai();
            // HZChiToServer_chenZhouZiPai(chiArr,[firstBiArr,biArr]);

            commitEatCards_chenZhouZiPai(chiArr,[firstBiArr,biArr]);
        });
    }
    ...
    if(ziPai.getUiLayoutType() == 0){//新布局
        ...
        doMoveCenterAction_chenZhouZiPai([chiBg,biBg0,biBg],isScale);
    }else{
        chiBg.scale = biBg.scale;
        biBg0.scale = biBg.scale;
        doMoveCenterAction_chenZhouZiPai([chiBg,biBg0,biBg],isScale);
        ...
        // doMoveCenterAction_chenZhouZiPai2([chiBg,biBg0,biBg],isScale);
    }
}

function GetReadyVisible_chenZhouZiPai(node, off) {
    if (off < 0) {
        node.visible = false;
        return false;
    }
...
//设置玩家掉线头像
function setUserOffline_chenZhouZiPai(node, off){
...
//设置玩家掉线头像（邵阳新版大小结算专用）
function setRoundEndUserOffline_chenZhouZiPai(frame, pl){
    if(!pl){
        return;
    }
...
//显示玩家文字
function showUserChat_chenZhouZiPai(node, off, msg){
    var tData = MjClient.data.sData.tData;
 ...
 */
function curPlayerIsMe_chenZhouZiPai(off){
    var sData = MjClient.data.sData;
    var tData = sData.tData;
    ...
    var selfIndex = tData.uids.indexOf(SelfUid());
    selfIndex = (selfIndex + off)%MjClient.MaxPlayerNum_chenZhouZiPai;
    return selfIndex == tData.curPlayer;
}

function getVisablePlayerCount_chenZhouZiPai(){
    var sData = MjClient.data.sData;
    var tData = sData.tData;
    var uids = tData.uids;
    var count = 0;
 ...
 */
function getXingPlayerIndex_chenZhouZiPai(){
    if(MjClient.MaxPlayerNum_chenZhouZiPai != 4){
        return -1;
    }
    if(getVisablePlayerCount_chenZhouZiPai() < 3){
        return -1;
    }
    var sData = MjClient.data.sData;
    var tData = sData.tData;
    ...
    var xingIndex = (zhuang + 2 + MjClient.MaxPlayerNum_chenZhouZiPai) % MjClient.MaxPlayerNum_chenZhouZiPai;
    var selfIndex = tData.uids.indexOf(SelfUid());
    xingIndex = (xingIndex - selfIndex + MjClient.MaxPlayerNum_chenZhouZiPai) % MjClient.MaxPlayerNum_chenZhouZiPai;
    return xingIndex;
}


//重新开始后，重置MjClient.HandCardArr
function resetHandAfterBegin_chenZhouZiPai(){
    var sData = MjClient.data.sData;
    var pl = sData.players[SelfUid() + ""];
...
/*
 设置弃胡状态
 */
function setQiHuState_chenZhouZiPai()
{
    var pl = getUIPlayer_chenZhouZiPai(0);
    if (pl.isQiHu) {
...
function changeMJBg_chenZhouZiPai(node, type)
{
    if (node.toString() == "[object ImageView]") {
        var oldFile = node.getRenderFile().file;
        var newFile = getNewMJBgFile_chenZhouZiPai(oldFile, type);
    ...
    for(var index in childArray)
    {
        ...
        changeMJBg_chenZhouZiPai(child, type);
    }
}

function getNewMJBgFile_chenZhouZiPai(oldFile, type)
{
    ...
    if (type == 0) {
        ...
        if (oldFile.indexOf("/MJBg1") != -1)
            ...
            MjClient.cardPath_chenZhouZiPai = "playing/paohuzi/MJBg1/";
        }
        ...
        if (oldFile.indexOf("/MJBg2") != -1)
            ...
            MjClient.cardPath_chenZhouZiPai = "playing/paohuzi/MJBg2/";
        }
        else if (oldFile.indexOf("playing/paohuzi/") != -1) {
...
function getOtherWeiCards_chenZhouZiPai(card) {
    var sData = MjClient.data.sData;
 ...
 */
function createRadioBoxForCheckBoxs_chenZhouZiPai(nodes, callback, defaultIndex){
    var newobj = {};
 ...
 *  off:玩家的实际off
 **/
function changeUIOff_chenZhouZiPai(off){
...
//是否显示离开按钮（包括准备阶段）
function isShowBackBtn_chenZhouZiPai(){
    if(IsInviteVisible()){
        return true;
    }else{
        var sData = MjClient.data.sData;
 ...
 */
function getAnimate_chenZhouZiPai(preName, len, delaytime, playCount, startIndex){
...
function paioFenToServer_chenZhouZiPai(index) {
    var tData = MjClient.data.sData.tData;
