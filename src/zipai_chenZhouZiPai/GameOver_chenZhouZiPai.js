function addWxHeadToEndUI_chenZhouZiPai_GO(node, off) {
...
function SetGameOverLayer_chenZhouZiPai(node,off,isNewUi){
    ...
    setDismissTypeImg(pl,node,0.5,0.21,"chang");
    var uidSelf = SelfUid();
    var MaxWinAll=0;
    var MaxDianPao=0;
    ...
    addWxHeadToEndUI_chenZhouZiPai_GO(wxNode,off);
    ...
    AddFangKaIcon(sData, node, tData, pl, MaxWinAll, MaxWinPlayer,  MaxloseAll, MaxlosePlayer, cc.p(40, 385));
    ...
    var showRoomStatistics = function(node,showList,data){
        ...
        for (var i = 0; i < showList.length; i++) {
            var name = showList[i];
            cc.log("showRoomStatistics", name);
    ...
    //
    showRoomStatistics(textNode,["huNum","ziNum","jiePaoNum","tiNum","paoNum"],pl.roomStatistics);
}

var GameOverLayer_chenZhouZiPai = cc.Layer.extend({
    sprite:null,
    jsBind:{
        back:{
            close :{
    ...
    ctor:function () {
        this._super();
        var endallui = ccs.load("endAll_ChenZhouZiPai.json");
        ...
        for(var i = 0 ; i < playerCount ; i++){
            ...
            SetGameOverLayer_chenZhouZiPai(clone,i, false);
