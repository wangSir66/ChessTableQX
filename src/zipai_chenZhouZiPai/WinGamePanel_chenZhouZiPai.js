function getOutCardRes_chenZhouZiPai(card){
    if (MjClient.playui && MjClient.playui.getCardSrc) {
        return MjClient.playui.getCardSrc("out", card);
    }else{
        return MjClient.cardPath_chenZhouZiPai+"out" + card + ".png";
    }
}

function addWxHeadToEndUI_chenZhouZiPai(node,off)
{
    ...
    setRoundEndUserOffline_chenZhouZiPai(frame, pl);
...
function reInitarrCardVisible_chenZhouZiPai()
{
...
//添加手牌
function addWinMjHand_chenZhouZiPai(_node,off) {
    var sData = MjClient.data.sData;
    var tData = sData.tData;
    ...
    for(var i = 0;i < sortArr.length;i++){
        ...
        for(var j = 0;j < childSort.length;j++){
            ...
            clone.visible = true;
            var src = getOutCardRes_chenZhouZiPai(card)
            //clone.loadTexture(src, MjClient.playui.getResType());
...
var EndOneView_chenZhouZiPai = cc.Layer.extend({
    _block:null,
    _endoneuiNode:null,
    jsBind:{
        block: {
        ...
        btn_ready:{
            ...
            _click: function(btn,eT){
                ...
                reInitarrCardVisible_chenZhouZiPai();
            }
        },
        btn_logic: {
            _run: function(){
        ...
        main:{
            ...
            leftPanel:{
                _run:function(){
                    ...
                    if(MjClient.data.sData.tData.maxPlayer == 2){
                        addWinMjHand_chenZhouZiPai(this, changeUIOff_chenZhouZiPai(2));
                    }else{
                        addWinMjHand_chenZhouZiPai(this, changeUIOff_chenZhouZiPai(3));
                    }
                }
            },
            rightPanel:{
            ...
            xingPanel:{
                _run:function(){
                    ...
                    addWinMjHand_chenZhouZiPai(this, changeUIOff_chenZhouZiPai(1));
                }
            }
        },
        uiNode: {
                ...
                _run: function(){
                    ...
                    var cloneFunc = function(cardNum){
                        if(cloneNum != 0){
                            ...
                            var src = getOutCardRes_chenZhouZiPai(cardNum);
                            //cloneCard.loadTexture(src, MjClient.playui.getResType());
                            MjClient.playui.loadCardTexture(cloneCard, src, MjClient.playui.getResType());
                            cloneNum++;
                            return cloneCard;
                        ...
                        var src = getOutCardRes_chenZhouZiPai(cardNum);
                        //cardImg.loadTexture(src, MjClient.playui.getResType());
                    ...
                    if(!oneLine && tData.areaSelectMode.maiPaiNum > 0) {
            ...
            uiBg:{
                headNode: {
                    _run: function(){
                        ...
                        if(tData.winner != -1)
                            winOff = getUiOffByUid_chenZhouZiPai(pl.info.uid);

                        addWxHeadToEndUI_chenZhouZiPai(this,winOff);
                        ...
                        for(var index = 0;index<playerCount;index++){
                            if(index != winOff){
                                ...
                                addWxHeadToEndUI_chenZhouZiPai(cloneThis,index);
                                cloneNum++;
                            }
                        }
                    }
    ...
    setWinCard: function(){
        ...
        var cloneCardsSetData = function(cardNode,i,num){
            ...
            var src = getOutCardRes_chenZhouZiPai(num);
            //cardImg.loadTexture(src, MjClient.playui.getResType());
            MjClient.playui.loadCardTexture(cardImg, src, MjClient.playui.getResType());
            return cardImg;
        }
                ...
                var getHuXi = function(cards){
                    ...
                    for(var t = 0;t < normal.length;t++){
                        if(normal[t].toString() == cards.toString()){
                            var huxi = huXiScore_chenZhouZiPai("chi",cards[0]);
                            return huxi;
                        }
                    }
                    return 0;
    ...
    setXingCardData: function(tData,num){
        ...
        var src = getOutCardRes_chenZhouZiPai(tData.hunCard);
        //card_img.loadTexture(src, MjClient.playui.getResType());
