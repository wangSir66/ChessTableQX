...
DoudizhuDaTong.prototype.CARDTPYE = CARDTPYE;
var CARDCOUNT = {};
...
DoudizhuDaTong.prototype.randomCards = function() {
    var cards = [];
...
DoudizhuDaTong.prototype.calPoint = calPoint;

DoudizhuDaTong.prototype.isShun = function(oCards) { //oCards有序
    if (!oCards) {
        return false;
    }
...
DoudizhuDaTong.prototype.isLiandui = function(oCards) { //oCards有序
    if (!oCards) {
        return false;
    }
    var cardCount = oCards.length;
...
DoudizhuDaTong.prototype.isSanshun = function(cards) { //cards有序
    if (!cards) {
        return false;
    }
...
DoudizhuDaTong.prototype.isFeiji2 = function(oCards) { //cards有序
    if (!oCards) {
        return false;
    }
...
DoudizhuDaTong.prototype.isFeiji = function(oCards) { //cards有序
    if (!oCards) {
        return false;
    }
    var cards = oCards.slice();
...
// 计算牌型
DoudizhuDaTong.prototype.calType = function(cards) {
    var cardCount = cards.length;
...
DoudizhuDaTong.prototype.calCardsValue = function(cards, type) {
...
// 牌点比较函数
DoudizhuDaTong.prototype.cardValueCmp = function(a, b) {
    var pa = calPoint(a);
    var pb = calPoint(b);
    if (pa == pb) {
        return a - b;
...
// 牌是否能压上
DoudizhuDaTong.prototype.canPut = function(oCards, oLastCards) {
    var cards = oCards.slice();
...
// 检查是否能出牌
DoudizhuDaTong.prototype.checkPut = function(oHands, oCards, lastCards) {
...
DoudizhuDaTong.prototype.checkPutByOneTimes = function (oHands) {
...
// 对手牌排序，普通牌型和同花顺优先排序
DoudizhuDaTong.prototype.sortHandCards = function(oCards, sortType) {
    var cards = oCards.slice();
...
// 找N张点数一样的牌
DoudizhuDaTong.prototype.findNSameCard = function(hands, point, n) { 
...
DoudizhuDaTong.prototype.filterSort = function(hands, cardsArray, type) { 
...
// 找出type牌型的牌
DoudizhuDaTong.prototype.findCardByType = function(hands, type, cardCount) { 
    var rets = [];
...
// 是否可以包含炸弹
DoudizhuDaTong.prototype.isCanContainZhadan = function (type) {
...
// 不拆炸弹
DoudizhuDaTong.prototype.dismantleZhadan = function (hands, cardsArray, type) {
...
// 求解提示出牌
DoudizhuDaTong.prototype.tipCards = function(oHands, oLastCards) {
    var rets = [];
...
// 求出牌型
DoudizhuDaTong.prototype.cardsType = function(oCards) {
    if (oCards) {
        var cards = oCards.slice();
        cards.sort(this.cardValueCmp);
...
// 计算真实的点
DoudizhuDaTong.prototype.calRealPoint = function(num) {
    if (!num) {
        return -1;
    }
    var ceilNum = Math.ceil(num / 4);
...
DoudizhuDaTong.prototype.findMinPointAndMinZhaDanByHands = function (hands) {
...
// 找最小牌的顺子/连对
DoudizhuDaTong.prototype.findCardsByMinPointAndType = function (cardsMap, len, minPoint, cType) {
...
DoudizhuDaTong.prototype.findSanZhangByMinPoint = function (cardsMap, len, minPoint) {
...
// 找最小牌的对子
DoudizhuDaTong.prototype.findDuiZiByMinPoint = function (cardsMap, len, minPoint) {
...
DoudizhuDaTong.prototype.changeCardsToMap = function (hands) {
    var cardsMap = {}
    for (var i = 0; i < hands.length; i++) {
...
// 首出逻辑
DoudizhuDaTong.prototype.getAutoPutCardWithFirstOut = function(hands) {
    // 1. 从小到大出牌
...
// 是否是三张系列
DoudizhuDaTong.prototype.is3ZhangType = function(cType) {
...
DoudizhuDaTong.prototype.findSelectCardForAutoUp = function(tipCards, selectValue) {
...
// 更换牌值
DoudizhuDaTong.prototype.changeSelectCardForAuto = function(tipCards, idx, selectValue) {
    tipCards[idx] = selectValue;
    return tipCards;
}

// 有足够的三张牌组
DoudizhuDaTong.prototype.isEnough3SameCards = function (upSelectCards, groupNum) {
    var start = false;
    var tmpCards = [];
...
DoudizhuDaTong.prototype.findEnough3SameCard = function(oHands, upSelectCards, groupNum, zhadanMap) {
    var tmpSelect = [];
    var lastPoint = -1;
    for (var i = 0; i < upSelectCards.length; i++) {
...
// 找三张配合的牌
DoudizhuDaTong.prototype.findDaiCardsWithSanZhang = function(oHands, upSelectCards, cType, groupNum, zhadanMap) {
...
// 自动提牌-划动-三张系列
DoudizhuDaTong.prototype.handleAutoByTouchMoveSanZhang = function(upSelectCards, cType, oLastCards, oHands) {
    var groupNum = 1;
...
DoudizhuDaTong.prototype.findTheRightTouchMoveCards = function(upSelectCards, cType, oLastCards) {
...
DoudizhuDaTong.prototype.findZhadanTouchMoveCards = function(upSelectCards) {
    var tCards = [];
    var lastPoint = -1;
...
// 自动提牌-划牌
DoudizhuDaTong.prototype.handleAutoByTouchMove = function (bTouchMove, upSelectCards, cType, tipCardsArray, oLastCards, oHands) {
...
// 自动提牌-点牌
DoudizhuDaTong.prototype.handleAutoByFirstCard = function(tipCardsArray, upSelectCard, cType) {
...
// 计算斗地主划牌结果
DoudizhuDaTong.prototype.calAutoPutCards = function (bTouchMove, tipCardsArray, upSelectCards, oLastCards, oHands) {
...
DoudizhuDaTong.prototype.getFindCardForFirstPut = function(cards, cType, count) {
    var realCards = null;
    var rateType = {};
...
DoudizhuDaTong.prototype.calAutoPutCardWithFirstOut = function(upSelectCards, hands) {
    var data = {hasCardToUp:false};
...
// 计算选择的牌,排除炸弹
DoudizhuDaTong.prototype.calSelectWithoutZhadan = function(upSelectCards, zhadanMap) {
...
// 计算牌组的炸弹
DoudizhuDaTong.prototype.getZhaDanMapByCards = function(oHands) {
    var hands = oHands.slice();
    hands.sort(this.cardValueCmp);
...
if(typeof(module)!="undefined" && module.exports)    
    module.exports = DoudizhuDaTong;
if(typeof(MjClient)!="undefined")
    MjClient.doudizhu_DaTong = new DoudizhuDaTong();
})();