...
function SetEndOneUserUI_doudizhuDaTong(node, off) {
    ...
    var uibind = {
        head: {
            ...
            stand: {
                ...
                _run: function () {
                    ...
                    CardLayoutRestoreForEndOne_doudizhuDaTong(node, pl);
                }
            },
            cardType: {
                _run: function () {
            ...
            jiaBeiFlag: {
                ...
                _run:function()
                {
                    var jiaZhuNum = MjClient.playui.endInfo.jiaZhuNums[pl.info.uid +""];
                    if (jiaZhuNum == 4) {
                        this.loadTexture("gameOver/chaojiJiaBeiFlag.png");
                    } else if (jiaZhuNum == 2) {
                        this.loadTexture("gameOver/jiaBeiFlag.png");
                    }
                    this.visible = (jiaZhuNum > 1);
                }
            },
        }
        , winNum: {
            _run: function () {
                this.ignoreContentAdaptWithSize(true);
                this.setColor(pl.winone > 0 ? cc.color("#FFFA7E") : cc.color("#9BEBFF"));
                this.enableOutline(pl.winone > 0 ? cc.color("#94261E") : cc.color("#3953A8"), 1);
            },
            _text: function () {
                var pre = "";
                if (pl.winone > 0) pre = "+";
        ...
        , allScore: {
            _run: function () {
                ...
                this.setColor(winScore > 0 ? cc.color(255,250,126) : cc.color(155,235,255));
                this.enableOutline(winScore > 0  ? cc.color(148,38,30) : cc.color(57,83,168), 1);
            },
            _text: function () {
                // 计算当前总分
                var winScore = 0;
...
function CardLayoutRestoreForEndOne_doudizhuDaTong(node, plNode) {
...
var EndOneView_doudizhuDaTong = cc.Layer.extend({
    jsBind: {
        block: {
        ...
        back: {
            ...
            head0: {
                ...
                _run: function () { SetEndOneUserUI_doudizhuDaTong(this, 0); },
            ...
            , head1: {
                ...
                _run: function () { SetEndOneUserUI_doudizhuDaTong(this, 1); }
            }
            ...
            , head2: {
                ...
                _run: function () { SetEndOneUserUI_doudizhuDaTong(this, 2); }
            },
            publicMutiple: {
                _run:function()
                {
                ...
                _text: function () {
                    var endInfo = MjClient.playui.endInfo;
                    var sData  = MjClient.data.sData;
                    var tData = sData.tData;
                    var text = "";
                    var index = tData.uids.indexOf(tData.firstDel);
                    if (index == -1) { // 没有解散
                        text += "底分X" + tData.areaSelectMode.difen;
                        text += "，叫分X" + endInfo.diFen;
                        text += tData.zhadan > 0 ? "，炸弹X" + Math.pow(2, tData.zhadan) : "";
                        text += tData.chuntianRate > 1 ? "，春天X" + tData.chuntianRate : "";
                        text += "=公共底分" + revise(tData.areaSelectMode.difen * endInfo.diFen * Math.pow(2, tData.zhadan) * tData.chuntianRate);
                    } else {
                        for (var i = 0; i < tData.maxPlayer; i++) {
                            var pl = MjClient.getPlayerByIndex((i + index) % tData.maxPlayer);
                            if (pl == null) { continue; }
                            text += (getPlayerName(unescape(pl.info.nickname)) + pl.mjdesc);
    ...
    ctor: function (zhuang) {
        ...
        var endoneui = ccs.load("endOne_doudizhuDaTong.json");
