...
PokerSanDaHa.prototype.CARDTPYE = PDK_CARDTPYE;
var PDK_CARDCOUNT = {};
...
PokerSanDaHa.prototype.randomCards = function(areaSelectMode, tData) {
    var cards = [];
    // 去掉3、4
    for (var i = 1; i <= 54; i++) {
...
PokerSanDaHa.prototype.cardlog = function(cards, zhuColorType) {
 ...
 */
PokerSanDaHa.prototype.countWinType = function(jiaofen, PfenPaiArr, isTouXiang, Pkoudifen, fengDing) {
    if( isTouXiang ) {
        return -4;
    }
    ...
    if(difffen < 0 && defen == 0 ) { // 大光 每个闲家-3
        winType = -3;
    } else if ( difffen < 0 && defen < 30) { // 小光 每个闲家-2
        winType = -2;
        ...
        if (fengDing == 0 || fengDing == 1 || difffen < 90) {  // 3楼封顶
            winType = 3;
        } else if (difffen < 100) { // 4楼
            winType = 4;
        } else {
            winType = Math.floor((difffen - 100) / 5) + 5;
            winType = fengDing == 2 && winType > 6 ? 6 : winType; // 6楼封顶  上楼不封顶
        }
    }
...
PokerSanDaHa.prototype.isWuZhu = function(zhuColorType) {
...
PokerSanDaHa.prototype.calPoint = function(num) {
    if (!num) return -1;
...
PokerSanDaHa.prototype.calPointBySdh = function(num, zhuColorType) {
    if (!num) return -1;
...
// 牌的花色
PokerSanDaHa.prototype.calColor = function(num) {
    if (!num) return -1;
    // 0:方块  1:梅花  2:梅花  3:黑桃
...
PokerSanDaHa.prototype.calColorSdh = function(num, zhuColorType) {
    if (!num) return -1;
...
// 判断此牌是否是主花
PokerSanDaHa.prototype.isZhuCard = function(num, zhuColorType) {
    var point = this.calPoint(num);
    var color = this.calColor(num);
...
PokerSanDaHa.prototype.getRoundFirstCards = function(putCardsRecord, uid, maxPlayNum) {
...
PokerSanDaHa.prototype.getRoundCardsByUid = function(putCardsRecord, uid, maxPlayNum) {
    if(!putCardsRecord || typeof putCardsRecord == 'undefined') {
        return null;
    }
...
// 获取本轮最大的牌， 同点数先出比后出的大
PokerSanDaHa.prototype.getMaxCardPlayerUid = function(roundPutCards, firstPutUid, uids, zhuColorType,bchou6,daWuZhu27NotTuoLaJi) {
    var frontUid = -1;
    if(!roundPutCards || typeof roundPutCards == 'undefined') {
        return frontUid;
    }
    ...
    for(var i=0; i<uids.length; i++) {
        ...
        if(frontUid > 0) {
            ...
            if(cards && tmpcards && this.isBigger(tmpcards, cards, zhuColorType, bchou6, daWuZhu27NotTuoLaJi) > 0) {
...
PokerSanDaHa.prototype.getAllCardFen = function( fenPaiArr ) {
    var fen = 0;
...
// 获得牌的分  非分牌返回0分 
PokerSanDaHa.prototype.getCardFen = function(num) {
...
// 获取所有对应牌数的拖拉机组合
PokerSanDaHa.prototype.getLianDuiCards = function(oCards, cardNum, zhuColorType, bchou6, daWuZhu27NotTuoLaJi) {
    if(oCards.length < cardNum) return false;
    var resCards = [];
    for(var i=0; i < oCards.length - cardNum + 1; i++) {
        var cards = oCards.slice(i, i + cardNum);
        if( this.isLiandui(cards, zhuColorType,bchou6,daWuZhu27NotTuoLaJi) ) 
            resCards.push(cards);
    }
...
// 获取所有对应牌数的拖拉机组合
PokerSanDaHa.prototype.getDuiZis = function(oCards, zhuColorType) {
...
PokerSanDaHa.prototype.countDuiNum = function(oCards, zhuColorType) {
    var num = 0;
    var cardNumByValue = {};
    for(var i in oCards){
 ...
 * @return {bool}
 */
PokerSanDaHa.prototype.isLiandui = function(oCards, zhuColorType,bchou6,daWuZhu27NotTuoLaJi) { //oCards有序
    var cardNum = PDK_CARDCOUNT[PDK_CARDTPYE.liandui];// 连对最小牌数

    if (!oCards || oCards.length < cardNum || oCards.length % 2 != 0)
        return false;

    if (daWuZhu27NotTuoLaJi && this.isWuZhu(zhuColorType)) {
        // 打无主27不算拖拉机
...
PokerSanDaHa.prototype.isDuizi = function(oCards) {
 ...
 */
PokerSanDaHa.prototype.calType = function(cards, zhuColorType,bchou6,daWuZhu27NotTuoLaJi) {
    var cardCount = cards.length;

    // 连对，2对起
    if (cardCount >= PDK_CARDCOUNT[PDK_CARDTPYE.liandui]  && this.isLiandui(cards, zhuColorType,bchou6,daWuZhu27NotTuoLaJi))
 ...
 * @return {number}
 */
PokerSanDaHa.prototype.calCardsValue = function(cards, zhuColorType) {
...
// // 牌点比较函数
PokerSanDaHa.prototype.cardValueCmp = function(a, b, zhuColorType) {
 ...
 */
PokerSanDaHa.prototype.isBigger = function(beiYaCards, oLastCards, zhuColorType, bchou6, daWuZhu27NotTuoLaJi) {

    var cardsType = this.calType(beiYaCards, zhuColorType, bchou6, daWuZhu27NotTuoLaJi);
    if (cardsType == -1){
        return -1;
    ...
    var lastCardsType = this.calType(oLastCards, zhuColorType,bchou6,daWuZhu27NotTuoLaJi);
...
PokerSanDaHa.prototype.checkPut = function(oHands, cards) {
 ...
 */
PokerSanDaHa.prototype.checkPutSdh = function(oHands, pOutCards, oBeiYaCard, zhuColorType, isCanShuaiPai) {
    ...
    var bchou6=MjClient.data.sData.tData.areaSelectMode.chou6;
    var daWuZhu27NotTuoLaJi = MjClient.data.sData.tData.areaSelectMode.daWuZhu27NotTuoLaJi;
    var allowTuoLaJiXiaoDui = MjClient.data.sData.tData.areaSelectMode.allowTuoLaJiXiaoDui;
    ...
    if(!oBeiYaCard || oBeiYaCard.length <= 0) {
        ...
        var outCardType = this.calType(outCards, zhuColorType,bchou6,daWuZhu27NotTuoLaJi);
    ...
    // 检测是否打出同花色的牌
        ...
        if(outCards.length <= handColorSameNum &&  outCardsSameColorNum == outCards.length) {
            var oBeiYaType = this.calType(oBeiYaCard, zhuColorType,bchou6,daWuZhu27NotTuoLaJi);
            var outCardType = this.calType(outCards, zhuColorType,bchou6,daWuZhu27NotTuoLaJi);
            // 被压的是拖拉机， 
            if(outCardType != oBeiYaType && oBeiYaType == PDK_CARDTPYE.liandui) {
                ...
                if (this.getLianDuiCards(handColorSameCards, oBeiYaCard.length, zhuColorType,bchou6,daWuZhu27NotTuoLaJi)) {
                    if (allowTuoLaJiXiaoDui) {
                        // 允许拖拉机消对，检查出的牌是否都是对子
                        for (var i = 0; i < outCards.length; i++) {
                            if (i % 2 == 0 && outCards[i] != outCards[i + 1]) {
                                needPutDuiZi = true;
                                break;
                            }
                        }
                    } else if (!this.isLiandui(outCards, zhuColorType,bchou6,daWuZhu27NotTuoLaJi)) {
...
PokerSanDaHa.prototype.findCardByType = function(oHands, oBeiYaCard, zhuColorType) {
 ...
 * @return {array} 提示的牌
 */
PokerSanDaHa.prototype.tipCards = function(oHands, oBeiYaCard, zhuColorType,bchou6,daWuZhu27NotTuoLaJi,allowTuoLaJiXiaoDui) {
    ...
    if(!oBeiYaCard) {
        ...
        var beiyaType = this.calType(oBeiYaCard, zhuColorType,bchou6,daWuZhu27NotTuoLaJi);
        var tiparr = [];
        ...
        if( handsColorCards.length >= cardNum ) {
            ...
            if(beiyaType == PDK_CARDTPYE.liandui) {
                ...
                var lianduis = this.getLianDuiCards(sourceCards, cardNum, zhuColorType,bchou6,daWuZhu27NotTuoLaJi);
                if( lianduis ) {
                    ...
                    if (allowTuoLaJiXiaoDui) {
                        ...
                        if(duizis && duizis.length * 2 >= cardNum) {
                            ...
                            for(var i = 0; i < (duizis.length - cardNum/2 + 1); i++) {
                                ...
                                if (!isHaveCards(tiparr,tmpduizi))
 ...
 */
PokerSanDaHa.prototype.sortHandCards = function(oCards, zhuColorType) {
 ...
 * @return {array}
 */
PokerSanDaHa.prototype.findNSameCard = function(hands, point, n, zhuColorType) {
...
// 删除相应牌值(不算花色)
PokerSanDaHa.prototype.delPoint = function(hands, points){
    var arr = hands.slice();
...
// 删除相同的牌(花色相同，牌值相同)
PokerSanDaHa.prototype.delCards = function(hands, cards) {
    var arr = hands.slice();
    for(var i=0; i < cards.length; i++){
        for (var j = 0; j < arr.length; j++) {
            if( arr[j] == cards[i] ){
...
// 从牌组中获取指定花色
PokerSanDaHa.prototype.getColorCards = function(cards, colors, zhuColorType) {
    var arr = [];
    for(var i=0; i < cards.length; i++) {
        var num = cards[i];
...
PokerSanDaHa.prototype.formatCards = function(oCards, zhuColorType) {
    var info = {};
...
PokerSanDaHa.prototype.isSameCards = function(cards1, cards2, zhuColorType) {
...
PokerSanDaHa.prototype.isAllZhuCard = function(cards, zhuColorType) {
    for (var i = 0; i < cards.length; i++) {
...
// 求出牌型
PokerSanDaHa.prototype.cardsType = function(oCards, zhuColorType,bchou6,daWuZhu27NotTuoLaJi) {
    if (oCards) {
        return this.calType(oCards, zhuColorType,bchou6,daWuZhu27NotTuoLaJi);
    }
    return -1;
};

// 检查是否是一次可以出完
PokerSanDaHa.prototype.checkPutByOneTimes = function (oHands, zhuColorType, bchou6, daWuZhu27NotTuoLaJi) {
    var cards = this.sortHandCards(oHands.slice(), zhuColorType);
    return this.cardsType(cards, zhuColorType, bchou6, daWuZhu27NotTuoLaJi) != -1;
};

if(typeof(module)!="undefined" && module.exports)    
    module.exports = PokerSanDaHa;

if(typeof(MjClient)!="undefined")
    MjClient.poker_SanDaHa = new PokerSanDaHa();
