...
//向服务器发送 过消息
MjClient.MJPass2NetForsuqian = function()
{
   // console.log(">>>>>>>>>普通  过 <<<<<<<<");
    cc.log("====================send======pass=====");
...
// 判断停牌操作
//function TingVisibleCheckForsuqian(eat) {
//
//    var sData = MjClient.data.sData;
//    var tData = sData.tData;
...
function SetUserVisible_suqian(node, off)
{
    //var sData = MjClient.data.sData;
    //return;
    cc.log("====================off======================" + off);
...
function InitUserHandUI_suqian(node, off)
{
    var sData = MjClient.data.sData;
    var tData = sData.tData;
    var pl = getUIPlayer(off);
...
var PlayLayer_suqian = cc.Layer.extend({
    _btnPutCard:null,
    jsBind: {
        _event: {
            mjhand: function() {
            ...
            roundEnd: function() {
                ...
                function delayExe()
                {
                    ...
                    self.addChild(new EndOneView_suqian(),500);
                }
            ...
            initSceneData: function() {
                ...
                CheckRoomUiDelete();
                //TingVisibleCheckForsuqian(MjClient.playui.eat._node);
            },
            ...
            changeui: {
                ...
                changeuibg: {
                    ...
                    guobg: {
                        guo: {
                            _touch: function(btn, eT) {
                                if (eT == 2) MjClient.MJPass2NetForsuqian();
                            }
                        },
                        fanhui: {
                            _touch: function(btn, et) {
    ...
    ctor: function() {
        ...
        var playui = ccs.load(res.Play_suqian_json);
        playMusic("bgFight");
...
PlayLayer_suqian.prototype.CardLayoutRestore = function(node, off)
{
    ...
    for(var i = 0; i < children.length; i++) //children 为 "down" 节点下的字节点
    {
        ...
        else if (ci.name == "gang1")
        {
            if (off != 0 && ci.isgang4 !== true)//暗杠不显示牌面
            {
                //if (pl.isTing)
                //{
                //    ci.loadTexture("playing/MJ/Mj_up_" + off + ".png");
...
// 判断吃碰杠胡的状态
PlayLayer_suqian.prototype.EatVisibleCheck = function()
{
    var eat = MjClient.playui.jsBind.eat;
    ...
    if(tData.tState == TableState.waitPut && pl.mjState == TableState.waitPut)
    {
        if(IsTurnToMe())
        {
            ...
            //碰后不能杠
            var handCards = MjClient.majiang.transformCards(pl.mjhand, tData.hunCard);  
            var rtn = MjClient.majiang.canGang1(pl.mjpeng, handCards, pl.isTing);
