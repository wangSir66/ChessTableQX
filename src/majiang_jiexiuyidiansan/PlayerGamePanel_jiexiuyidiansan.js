...
//向服务器发送 过消息
MjClient.MJPass2NetForjiexiuyidiansan = function()
{
   // console.log(">>>>>>>>>普通  过 <<<<<<<<");
    cc.log("====================MJPass2NetForjiexiuyidiansan======pass=====");
...
function SetUserVisible_jiexiuyidiansan(node, off)
{
    //var sData = MjClient.data.sData;
    //return;
    cc.log("====================off======================" + off);
...
//处理出牌,放一张牌，打牌动作
//处理出牌,放一张牌，打牌动作
function DealMJPut_jiexiuyidiansan(node, msg, off, outNum)
{
    DealMJPut_shanXiApp(node, msg, off, outNum);
}

function InitUserHandUI_jiexiuyidiansan(node, off)
{
    var sData = MjClient.data.sData;
    var tData = sData.tData;
    var pl = getUIPlayer(off);
    ...
    for(var i = 0; i < pl.mjput.length; i++)
    {
        ...
        DealMJPut_jiexiuyidiansan(node, msg, off, i);
    }
...
function initFlower_jiexiuyidiansan() {
    initFlower(false, false);
}


var PlayLayer_jiexiuyidiansan = cc.Layer.extend({
    jsBind: {
        _event: {
            mjhand: function() {
            ...
            roundEnd: function() {
                ...
                function delayExe()
                {
                    ...
                    self.addChild(new EndOneView_jiexiuyidiansan(),500);
                }
        ...
        banner: {
            ...
            Button_1: {
                _visible : true,
                _click: function() {
                    MjClient.openWeb({url:MjClient.GAME_TYPE.JIE_XIU_1_DIAN_3,help:true});
                }
            },
            hunPai:{
                baidaBg:{
        ...
        eat: {
            ...
            guo: {
                ...
                _touch: function(btn, eT) {
                    if (eT == 2)
                    {
                        MjClient.MJPass2NetForjiexiuyidiansan();
                    }
                },
                bgimg: {
                    _run: function() {
            ...
            changeui: {
                ...
                changeuibg: {
                    ...
                    guobg: {
                        guo: {
                            _touch: function(btn, eT) {
                                if (eT == 2) MjClient.MJPass2NetForjiexiuyidiansan();
                            }
                        },
                        fanhui: {
                            _touch: function(btn, et) {
    ...
    ctor: function() {
        this._super();
        var playui = ccs.load("Play_jiexiuyidiansan.json");
...
PlayLayer_jiexiuyidiansan.prototype.CardLayoutRestore = function(node, off)
{
...
// 判断吃碰杠胡的状态
PlayLayer_jiexiuyidiansan.prototype.EatVisibleCheck = function()
{
    var eat = MjClient.playui.jsBind.eat;
...
/*
    玩家ui，信息
 */
PlayLayer_jiexiuyidiansan.prototype.Node_player = function(off)
{
    ...
    UIEventBind(null, _node, "initSceneData", function (eD) {
        SetUserVisible_jiexiuyidiansan(_node, off);
    ...
    UIEventBind(null, _node, "mjhand", function (eD) {
        InitUserHandUI_jiexiuyidiansan(_node, off);
    });
    ...
    UIEventBind(null, _node, "MJPut", function (eD) {
        ...
        // }
        DealMJPut_jiexiuyidiansan(_node,eD,off);
        hideTingBtn();
        if(_tingCardNumNode) _tingCardNumNode.visible = false;
