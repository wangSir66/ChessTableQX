function addWxHeadToEndUI_xxghz(node,off)
{
...
function reInitarrCardVisible_xxghz()
{
...
var EndOneView_xxghz = cc.Layer.extend({
    _block:null,
	jsBind:{
		block:{
		    _run:function () {
				...
				_click:function(btn,eT){
	                ...
	                if (MjClient.rePlayVideo >= 0 && MjClient.replayui) {
	                    ...
	                    if(MjClient.getAppType() == MjClient.APP_TYPE.HUNANWANGWANG || MjClient.getAppType() == MjClient.APP_TYPE.QXSYDTZ){
	                    	...
	                    	MjClient.MJPass2NetForxiangxianggao();
	                    }
	                }
				   	...
				   	_run:function(){
				   		...
				   		for(var index = 0;index<playerCount;index++){
						    ...
						    if(MjClient.getAppType() == MjClient.APP_TYPE.HUNANWANGWANG || MjClient.getAppType() == MjClient.APP_TYPE.QXSYDTZ){
						    	...
						    	addWxHeadToEndUI_xxghz(cloneHead,showIndex);
						    }
				   		}
				   	},
				},
				showInfo:{
				   	...
				   	listView:{
				   		...
				   		_run:function(){
					   		...
					   		var item = null;
				   			if(pl.hzdesc.sanshiHuxi){
				   				item = getItem();
				   				item.getChildByName("imgName").setString("30胡息");
				   				item.getChildByName("count").setString("×2");
				   			}
				   			if(pl.hzdesc.sanshihuxishi){
				   				item = getItem();
				   				item.getChildByName("imgName").setString("30胡息(10红)");
				   			...
				   			if(pl.hzdesc.hongwuhu){
				   				item = getItem();
				   				item.getChildByName("imgName").setString("红乌");
				   				item.getChildByName("count").setString("+100");
				   			}
				   			if(pl.hzdesc.dihu1){
				   				item = getItem();
				   			...
				   			if(pl.hzdesc.dihu3){
				   				item = getItem();
				   				item.getChildByName("imgName").setString("地胡(10红)");
				   				item.getChildByName("count").setString("+100");
				   			}
				   			if(pl.hzdesc.heihu){
				   				item = getItem();
				   			...
				   			if(pl.hzdesc.dianpao){
				   				item = getItem();
				   				item.getChildByName("imgName").setString("放炮");
				   				item.getChildByName("count").setString("+10");
				   			}
				   			if(pl.hzdesc.tianhu){
				   				item = getItem();
			...
			main:{
				...
				leftPanel:{
					_run:function(){
						...
						addWinMjHand_xxghz(this,MjClient.MaxPlayerNum_xiangxiang - 1);
				...
				rightPanel:{
					_run:function(){
						if(MjClient.gameType == MjClient.GAME_TYPE.HY_LIU_HU_QIANG){
							addWinMjHand_xxghz(this,MjClient.MaxPlayerNum_xiangxiang - 2);
						}else{
							addWinMjHand_xxghz(this,1);
						}
						if(MjClient.rePlayVideo != -1){
	                    	this.visible = false;
	                    }
...
//添加手牌
function addWinMjHand_xxghz(_node,off) {
	var sData = MjClient.data.sData;
    var tData = sData.tData;
