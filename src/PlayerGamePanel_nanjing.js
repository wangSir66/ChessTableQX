...
//向服务器发送 过消息
MjClient.MJPass2NetFornanjing = function()
{
   // console.log(">>>>>>>>>普通  过 <<<<<<<<");
    cc.log("====================send======pass=====");
...
function SetUserVisible_nanjing(node, off)
{
    //var sData = MjClient.data.sData;
    //return;
    // cc.log("====================off======================" + off);
    ...
    if(pl)
    {
        cc.log("====================SetUserVisible_nanjing======================" + off);
...
//现场汇，飘分数，score 正数为加分，负数为减分
function showScores_nanjing(off,score)
{
    ...
    if(pl)
    {
        cc.log("====score update  = " + pl.winall);
        cc.log("====score update  = " + score);
        pl.winall += score;

        changeAtalsForLabel(_Node.getChildByName("coin"), pl.winall);
    }
...
//飞金币
function showCoins_nanjing(startOff,endOff)
{
    var getNode = function(off)
    {
        var _node = null;
...
function InitUserHandUI_nanjing(node, off)
{
    var sData = MjClient.data.sData;
    var tData = sData.tData;
    var pl = getUIPlayer(off);
...
function initFlower_nanjing() {
    initFlower(false, false);
}


//设置微信头像
function setZui_nanjing(node, off)
{
    var mypl = getUIPlayer(off);
    if(!mypl)
    {
        return;
    }
    ...
    cc.log(" ==== pl.zuiCount == " + mypl.zuiCount);
    cc.log(" ==== pl.zuiUid == " + mypl.zuiUid);
    if (cc.isUndefined(mypl.zuiCount) || cc.isUndefined(mypl.zuiUid) || mypl.zuiCount <= 0)
    {
    ...
    else
    {
        ...
        var sData = MjClient.data.sData;
        var pl = sData.players[mypl.zuiUid];
        ...
        cc.loader.loadImg(url, {isCrossOrigin : true}, function(err, texture)
        {
            if(!err&&texture)
            {
                var headSprite = node.getChildByName("Sprite_head");
                if (headSprite)
                {
                ...
                {
                    headSprite = new cc.Sprite(texture);
                    headSprite.setPosition(node.getContentSize().width/2, node.getContentSize().height/2);
                    headSprite.setScale((node.getContentSize().width-4)/headSprite.getContentSize().width);
                    node.addChild(headSprite);
                }
            }
        });


        var textZuiCount = node.getChildByName("zuiCount");
        textZuiCount.setString(mypl.zuiCount+"嘴");
        textZuiCount.ignoreContentAdaptWithSize(true);
    }
}


var PlayLayer_nanjing = cc.Layer.extend({
    jsBind: {
        _event: {
            mjhand: function() {
            ...
            roundEnd: function() {
                ...
                function delayExe()
                {
                    ...
                    self.addChild(new EndOneView_nanjing(),500);
                }
            ...
            payCash:function(data)//现场汇
            {
                cc.log(" pay Cash data = " + JSON.stringify(data));
                var eventName = data.eventName;
                var cashActionArray =  data.cashActions;

                var vecTotalScore = {};
                for(var i = 0; i <  cashActionArray.length ;i++)
                {
                    var fromUid = cashActionArray[i].from;
                    var toUid = cashActionArray[i].to;
                    var cash = cashActionArray[i].cash;

                    var fromOff = getUiOffByUid(fromUid);
                    var toOff   = getUiOffByUid(toUid);

                    if (cc.isNumber(vecTotalScore[fromUid+""]))
                    {
                        vecTotalScore[fromUid+""] += -cash;
                    }
                    else
                    {
                        vecTotalScore[fromUid+""] = -cash;
                    }
                    if (cc.isNumber(vecTotalScore[toUid+""]))
                    {
                        vecTotalScore[toUid+""] += cash;
                    }
                    else
                    {
                        vecTotalScore[toUid+""] = cash;
                    ...
                    //飞金币
                    showCoins_nanjing(fromOff,toOff);
                }

                //飘分
                for (var uid in vecTotalScore)
                {
                    var off = getUiOffByUid(parseInt(uid));
                    showScores_nanjing(off, vecTotalScore[uid]);
                }
        ...
        roundInfo:{
            ...
            _run:function()
            {
                ...
                //
                //
                // var str = (tData.areaSelectMode.playType ==0)? "进园子" + str31 + str32:"敞开头" + str33;
                //
                //
                // var strPayWay = "";
                // str += strPayWay;
                ...
                // if(tData.matchId){
                //     BSStr = ",10秒出牌";
                //     this.setString(GameCnName[MjClient.gameType]+","+str + BSStr);
                // }
                //
                // var tData = MjClient.data.sData.tData;
        ...
        banner: {
            ...
            Button_1: {
                _visible : true,
                _click: function() {
                    MjClient.openWeb({url:MjClient.GAME_TYPE.NAN_JING,help:true});
                }
            },
            hunPai:{
                _run: function() {
        ...
        //             var str33 = (tData.areaSelectMode.islimit300) ? "单把300上限," : "单把无限制";
        //
        //             var str3 = (tData.areaSelectMode.playType ==0)? "进园子," + str31 + str32:"敞开头," + str33;
        //             var strPayWay = "";
        ...
        //             }
        //             var str4 = strPayWay;
        //             var str5 = tData.roundNum + "局,三缺一,速度加入【"+AppCnName[MjClient.getAppType()]+"】\n" + "(复制此消息打开游戏可直接进入该房间)";
        //             GLog(str1+str2+str3+str4+str5);
        ...
        //             }
        //             var str4 = strPayWay;
        //             var str5 = tData.roundNum + "局," + "速度加入【"+AppCnName[MjClient.getAppType()]+"】";
        //             GLog(str1+str2+str3+str4+str5);
        //             MjClient.native.wxShareUrl(MjClient.remoteCfg.entreRoomUrl+"?vipTable="+tData.tableid, GameCnName[MjClient.gameType] + "  " + tData.tableid + "  三缺一  点击加入>>>",
        //                 str1+str2+str3+str4+str5);
        //         },
        //         _visible:function()
        //         {
            ...
            head: {
                ...
                tingIcon: {
                    ...
                    _event: {
                        ...
                        MJPut:function(eD)
                        {
                            if(MjClient.playui.isPlayerPutCard(eD,0))
                            {
                ...
                nobody_min:{
                 ...
                 _event:{
                        ...
                        MJGang: function(eD) {
                            ...
                            var tData = sData.tData;
                            var uids = tData.uids;
                            var selfIndex = (uids.indexOf(SelfUid()) + 0) % 4;
                            if(tData.curPlayer == selfIndex)
                            {
                                setZui_nanjing(this, 0);
                            }
                        },
                        MJPeng: function(eD) {
                            var sData = MjClient.data.sData;
                ...
                nobody_min:{
                 ...
                 _event:{
                        ...
                        MJGang: function(eD) {
                            ...
                            var tData = sData.tData;
                            var uids = tData.uids;
                            var selfIndex = (uids.indexOf(SelfUid()) + 1) % 4;
                            if(tData.curPlayer == selfIndex)
                            {
                                setZui_nanjing(this, 1);
                            }
                        },
                        MJPeng: function(eD) {
                            var sData = MjClient.data.sData;
                ...
                nobody_min:{
                 ...
                 _event:{
                        ...
                        MJGang: function(eD) {
                            ...
                            var tData = sData.tData;
                            var uids = tData.uids;
                            var selfIndex = (uids.indexOf(SelfUid()) + 2) % 4;
                            if(tData.curPlayer == selfIndex)
                            {
                                setZui_nanjing(this, 2);
                            }
                        },
                        MJPeng: function(eD) {
                            var sData = MjClient.data.sData;
                ...
                nobody_min:{
                 ...
                 _event:{
                        ...
                        MJGang: function(eD) {
                            ...
                            var tData = sData.tData;
                            var uids = tData.uids;
                            var selfIndex = (uids.indexOf(SelfUid()) + 3) % 4;
                            if(tData.curPlayer == selfIndex)
                            {
                                setZui_nanjing(this, 3);
                            }
                        },
                        MJPeng: function(eD) {
                            var sData = MjClient.data.sData;
            ...
            changeui: {
                ...
                changeuibg: {
                    ...
                    guobg: {
                        guo: {
                            _touch: function(btn, eT) {
                                if (eT == 2) MjClient.MJPass2NetFornanjing();
                            }
                        },
                        fanhui: {
                            _touch: function(btn, et) {
    ...
    ctor: function() {
        this._super();
        var playui = ccs.load(res.Play_nanjing_json);
        playMusic("bgFight");
    ...
    */
    setTingCardInfo:function(node,eD,off)
    {
        ...
        cc.log("%%%%%%%%%%%%%%%%setCard%%%%%%%%%%%%%%%%%%%  pl.jiaPai = " + pl.jiaPai);
...
PlayLayer_nanjing.prototype.CardLayoutRestore = function(node, off)
{
...
// 判断吃碰杠胡的状态
PlayLayer_nanjing.prototype.EatVisibleCheck = function()
{
    var eat = MjClient.playui.jsBind.eat;
    ...
    if(tData.tState == TableState.waitPut && pl.mjState == TableState.waitPut)
    {
        if(IsTurnToMe())
        {
            ...
            {
                if (pl.zuiCount == 3 && MjClient.majiang.canGang0(pl.mjhand.slice(0, -1), pl.mjhand[pl.mjhand.length - 1])) {
    ...
    else if(tData.tState == TableState.waitEat)
    {
        ...
        if(!IsTurnToMe())
        {
            ...
            // {
            //     if (MjClient.majiang.canPeng(pl.mjhand, tData.lastPutCard) && (pl.zuiCount == 3 || (pl.qingDui >=3 && tData.areaSelectMode["playType"] == 0))) {
            //         if (vnode.indexOf(eat.hu._node) < 0) {
            //             vnode.push(eat.hu._node);
            //         }
            //     }
            ...
            //     }
            // }
            cc.log("aaaaaas第三方    点点滴"+pl.eatFlag);
            if (pl.eatFlag & 8) {
                vnode.push(eat.hu._node);
            }
            if (pl.eatFlag & 4) {
            ...
            if (pl.eatFlag & 2) {
                cc.log("aaaaaas第三方    碰，碰"+pl.eatFlag);
                vnode.push(eat.peng._node);
            }
    ...
    ////如果弹出碰，杠，则隐藏，听的牌
    //if(eat.peng._node.visible || eat.gang0._node.visible || eat.gang1._node.visible || eat.gang2._node.visible)
    //{
    //    MjClient.playui._tingCardsNode.visible = false;
    //}
}