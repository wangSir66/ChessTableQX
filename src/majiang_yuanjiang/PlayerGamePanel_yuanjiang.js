...
var _isShowShaiziAni_yuanjiang = false; //为了控制mjhand之后播骰子动画时newCard的状态显示
//向服务器发送 过消息
MjClient.MJPass2NetForyuanjiang = function()
{
   // console.log(">>>>>>>>>普通  过 <<<<<<<<");
    cc.log("====================send======pass=====");
    ...
    if(IsTurnToMe() && tData.tState == TableState.waitPut)
    {
        ...
        if(eat.ting._node.visible && saveRoomMsgValueG != roomMsgValue)
        {
            msg += " 听 ";
        }
...
var showShaiziAni_yuanjiang = false;
function showShaiziAni_yuanjiang_old(poff)
{
    if(poff == 0) {
        ...
        if(MjClient.rePlayVideo != -1)//回放
        {
            for (var off = 0; off < 4; off++) {
                ...
                InitUserHandUI_yuanjiang(_node, off);
            }
            playEffect("shuffle");
        ...
        _isShowShaiziAni_yuanjiang = true;
        ...
        firstFrame.runAction(cc.sequence(playSoundFunc, animate,showPonitFunc,cc.delayTime(0.5),cc.callFunc(function(){
            //发牌
            _isShowShaiziAni_yuanjiang = false;
...
function updateHeadUI_yuanjiang(node, off)
{      
    cc.log('updateHeadUI_yuanjiang', off)
    var pl = getUIPlayer(off);
    if(!pl) return;
    showHandCard_yuanjiang(node, off);
    MjClient.playui.CardLayoutRestore(node, off);
    cc.log('updateHeadUI_yuanjiang update success', off)
...
function SetUserVisible_yuanjiang(node, off)
{
    if (_isShowShaiziAni_yuanjiang)  // 解决摇骰子时断线重连调用本函数，会手牌重复的bug by cyc
        return;
...
function showHandCard_yuanjiang(node, off)
{
    var sData = MjClient.data.sData;
    var tData = sData.tData;
    var pl = getUIPlayer(off);
...
function InitUserHandUI_yuanjiang(node, off)
{
    var sData = MjClient.data.sData;
    var tData = sData.tData;
    var pl = getUIPlayer(off);
    ...
    if (SelfUid() == pl.info.uid) {
        if (tData.tState == TableState.waitJiazhu) {
            if (pl.mjState == TableState.waitJiazhu) {
                cc.log("piaofenType",tData.areaSelectMode["piaofen"]);
                var layer = new JiaZhu_PaodekuaiTY(tData.areaSelectMode["piaofen"]);
...
function initFlower_yuanjiang() {
    initFlower(false, false);
}


var PlayLayer_yuanjiang = cc.Layer.extend({
                ...
                _event: {
                    ...
                    roundEnd: function() {
                        ...
                        function showEndCards()
                        {
                            ...
                            self.addChild(new EndOneView_yuanjiang(),500);
                        }
                        function delayExe()
                        {
              ...
              //           }
                ...
                BtnPutCard:{ //add by  sking for put card button
                    ...
                    _event:{
                        ...
                        MJGang: function(eD) {
                            ...
                            var pl = getUIPlayer(0);
                            var rtn = pl.gangList||[];
                            if(IsTurnToMe() && rtn && rtn.length > 0)
                            {
                                this.visible = true;
                            }else{
                                this.visible = false;
                        ...
                        waitPut: function() {
                            if (_isShowShaiziAni_yuanjiang) return;
                ...
                down: {
                    ...
                    _event: {
                        ...
                        initSceneData: function(eD) {
                            SetUserVisible_yuanjiang(this, 0);
                        },
                        addPlayer: function(eD) {
                            SetUserVisible_yuanjiang(this, 0);
                        },
                        ...
                        mjhand: function(eD) {
                            if (showShaiziAni_yuanjiang){
                                ...
                                InitUserHandUI_yuanjiang(this, 0);
                                if (MjClient.data.sData.tData.areaSelectMode.isOpenTingTip) {
                                    var pl = getUIPlayer(0);
                                    if (pl && getOffByIndex(MjClient.data.sData.tData.curPlayer) != 0) {
                                        var tingSet = {};
                        ...
                        waitJiazhu: function(msg) {
                            postEvent("returnPlayerLayer");
                            var tData = MjClient.data.sData.tData;
                            var piaofenType = tData.areaSelectMode["piaofen"];
                            cc.log("piaofenType",tData.areaSelectMode["piaofen"]);
                            //弹窗选择是否飘分
                            var layer = new JiaZhu_PaodekuaiTY(piaofenType,function(){
                                //弹窗等待
                                MjClient.playui._jiazhuWait.visible = true;
                            });
                            if (layer.isVisible())
                ...
                right: {
                    ...
                    _event: {
                        ...
                        initSceneData: function(eD) {
                            SetUserVisible_yuanjiang(this, 1);
                        },
                        addPlayer: function(eD) {
                            SetUserVisible_yuanjiang(this, 1);
                        },
                        ...
                        mjhand: function(eD) {
                            if (!showShaiziAni_yuanjiang)
                                InitUserHandUI_yuanjiang(this, 1);
                        },
                        roundEnd: function() {
                ...
                top: {
                    ...
                    _event: {
                        ...
                        initSceneData: function(eD) {
                            SetUserVisible_yuanjiang(this, 2);
                        },
                        addPlayer: function(eD) {
                            SetUserVisible_yuanjiang(this, 2);
                        },
                        ...
                        mjhand: function(eD) {
                            if (!showShaiziAni_yuanjiang)
                                InitUserHandUI_yuanjiang(this, 2);
                        },
                        roundEnd: function() {
                ...
                left: {
                    ...
                    _event: {
                        ...
                        initSceneData: function(eD) {
                            SetUserVisible_yuanjiang(this, 3);
                        },
                        addPlayer: function(eD) {
                            SetUserVisible_yuanjiang(this, 3);
                        },
                        ...
                        mjhand: function(eD) {
                            if (!showShaiziAni_yuanjiang)
                                InitUserHandUI_yuanjiang(this, 3);
                        },
                        roundEnd: function() {
                ...
                eat: {
                        ...
                        _touch: function(btn, eT) {
                            if (eT == 2)
                            {
                                ...
                                function clickting(){
                                     ...
                                     */
                                    var pl = getUIPlayer(0);
                                    cc.log("MjClient.canTingCards  ",JSON.stringify(MjClient.canTingCards),JSON.stringify(pl.tingLists));
                                    if (IsTurnToMe()) 
                                    {
                                    ...
                                    if (pl.mjhand.length == 13) 
                                    {
                                        MjClient.playui.resetcardcolorforqsting(MjClient.playui._downNode);
                                    }
                    ...
                    noTing : {
                        ...
                        _touch: function(btn, eT) {
                            if (eT == 2)
                            {
                                ...
                                if (MjClient.playui.jsBind.eat.ting._node.visible) {
                    ...
                    guo: {
                        ...
                        _touch: function(btn, eT) {
                            if (eT == 2)
                            {
                                MjClient.MJPass2NetForyuanjiang();
                            }
                        },
                        bgimg: {
                            _run: function() {
                    ...
                    changeui: {
                        ...
                        changeuibg: {
                            ...
                            guobg: {
                                guo: {
                                    _touch: function(btn, eT) {
                                        if (eT == 2) MjClient.MJPass2NetForyuanjiang();
                                    }
                                },
                                fanhui: {
                                    _touch: function(btn, et) {
    ...
    ctor: function() {
        this._super();
        var playui = ccs.load("Play_yuanjiang.json");
...
PlayLayer_yuanjiang.prototype.willHuShowArrow = function()
{
    var tData = MjClient.data.sData.tData;
...
PlayLayer_yuanjiang.prototype.CardLayoutRestore = function(node, off, isNewCard)
{
    ...
    stand.stopActionByTag(20180531);
    MjClient.playui.willHuShowArrow();
};
PlayLayer_yuanjiang.prototype.hideAllEat = function(){
    var eat = MjClient.playui.jsBind.eat;
    eat.chi0._node.visible = false;
    eat.chi1._node.visible = false;
    eat.chi2._node.visible = false;
...
PlayLayer_yuanjiang.prototype.EatBaoTingVisibleCheck = function(tData, vnode, eat, pl) 
{
    var eat = MjClient.playui.jsBind.eat;
    var sData = MjClient.data.sData;
    var tData = sData.tData;
...
// 判断吃碰杠胡的状态
PlayLayer_yuanjiang.prototype.EatVisibleCheck = function()
{
    var eat = MjClient.playui.jsBind.eat;
    ...
    if(tData.tState == TableState.waitPut && pl.mjState == TableState.waitPut)
    {
        if(IsTurnToMe())
        {
            ...
            MjClient.canTingCards = {};
            if (pl.tingLists && !pl.isPass) 
            {
                for (var i in pl.tingLists) {
                    MjClient.canTingCards[i] = 1;
                    if (pl.needTing && vnode.indexOf(eat.ting._node) < 0) 
                    {
                        vnode.push(eat.ting._node);
                    }
                }
    ...
    cc.log("pl.tingLists    ---------- ",JSON.stringify(MjClient.canTingCards));
}

PlayLayer_yuanjiang.prototype.jiaozhuToPiaoniao = function(jiazhuLayer, msg) {
    var tData = MjClient.data.sData.tData;
...
PlayLayer_yuanjiang.prototype.resetcardcolorforqsting = function (node) {
    var _node = node;
    var children = node.children;
    for (var i = 0; i < children.length; i++) {
        if (children[i].name == "mjhand") 
...
//剥离托管层
PlayLayer_yuanjiang.prototype.detachTuoGuanLayer = function(node, jsBind){
...
PlayLayer_yuanjiang.prototype.getTuoGuanLayerZIndex = function(node){
    return this.getChildByName("tuoGuanLayer").getLocalZOrder();
};

//构造码牌动画层
PlayLayer_yuanjiang.prototype.getShuffleEffectNode = function(){
    var self = this;
    var sData = MjClient.data.sData;
    var tData = sData.tData;
...
//获取麻将背景类型
PlayLayer_yuanjiang.prototype.getMaJiangBgType = function(){
    return getCurrentMJBgType();
};

PlayLayer_yuanjiang.prototype.getCardBgList = function(){
    if(COMMON_UI3D.is3DUI()) {
