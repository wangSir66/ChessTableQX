...
(function() {
    ...
    majiang.canTing = function (cards) {
        ...
        if (tData.maxPlayer !== 4 && tData.areaSelectMode.dingque) {  // 2，3人的缺2门才可以胡牌

            for (var i = 0; i < cards.length; i++) {
                if (Math.floor(cards[i] / 10) === pl.que[0]) {
                    return false;
                }
            }
        }
    ...
    majiang.calTingSet = function(handCards, hunCard) {
        ...
        {
            var _cd = cds[k];
            if(MjClient.playui.isQueCard(_cd)) //手里有缺的麻将
            {
                return {};
            }
        }
    ...
    {
        MjClient.majiang_guizhouGuiYangZhuoJi = majiang;
    }
    else
    {
