 ...
 */
var actionZindex_xiangyintuidaohu = 1000;
var _isShowShaiziAni_xiangyintuidaohu = false; //为了控制mjhand之后播骰子动画时newCard的状态显示
//向服务器发送 过消息
MjClient.MJPass2NetForxiangyintuidaohu = function()
{
   // console.log(">>>>>>>>>普通  过 <<<<<<<<");
    cc.log("====================send======pass=====");
...
MjClient.MJPass3NetForxiangyintuidaohu = function(eatFlag, eatFlag2)
{
    cc.log("====================send======pass=====");
...
var showShaiziAni_xiangyintuidaohu = false;
function showShaiziAni_xiangyintuidaohu_old(poff)
{
    if(poff == 0) {
        ...
        if(MjClient.rePlayVideo != -1)//回放
        {
            for (var off = 0; off < 4; off++) {
                ...
                InitUserHandUI_xiangyintuidaohu(_node, off);
            }
            playEffect("shuffle");
        ...
        _isShowShaiziAni_xiangyintuidaohu = true;
        ...
        firstFrame.runAction(cc.sequence(playSoundFunc, animate,showPonitFunc,cc.delayTime(0.8),cc.callFunc(function(){
            //发牌
            _isShowShaiziAni_xiangyintuidaohu = false;
...
function updateHeadUI_xiangyintuidaohu(node, off , actionType)
{      
    cc.log('updateHeadUI_xiangyintuidaohu', off)
    var pl = getUIPlayer(off);
    if(!pl) return;
    // showHandCard_MLHZ(node, off);
    showHandCard_xiangyintuidaohu(node,off);
    MjClient.playui.CardLayoutRestore(node, off);
    cc.log('updateHeadUI_xiangyintuidaohu update success', off)
...
function SetUserVisible_xiangyintuidaohu(node, off)
{
    if (_isShowShaiziAni_xiangyintuidaohu)  // 解决摇骰子时断线重连调用本函数，会手牌重复的bug by cyc
        return;
...
function showHandCard_xiangyintuidaohu(node, off)
{
    var sData = MjClient.data.sData;
    var tData = sData.tData;
    var pl = getUIPlayer(off);
...
function InitUserHandUI_xiangyintuidaohu(node, off)
{
    var sData = MjClient.data.sData;
    var tData = sData.tData;
    var pl = getUIPlayer(off);
    ...
    // } 
     ...
     if (tData.tState == TableState.waitJiazhu && SelfUid() == pl.info.uid && pl.jiazhuNum < 0) {
        if (pl.mjState == TableState.waitJiazhu && MjClient.playui.getChildByName("JiaZhu") == null) {
...
var PlayLayer_xiangyintuidaohu = cc.Layer.extend({
    jsBind: {
        _event: {
            mjhand: function() {
            ...
            roundEnd: function() {
                ...
                //胡牌语音播放
                MjClient.playui.playeffectinroundend(self);
                ...
                function showEndCards()
                {
                    ...
                    self.addChild(new EndOneView_xiangyintuidaohu(),500);
                }
                function delayExe()
                {
                ...
                {
                    MjClient.playui.showzhuaniao(showCards);
                    cc.log("addzhuaniao when roundEnd");
                }

                var showCards = tData.mopai;
                // for(var off = 0;off <MjClient.MaxPlayerNum;off++)
                // {
                //     var pl = getUIPlayer(getOffForPlayerNum(off));
                ...
                {   
                    cc.log("海底牌弹窗");
                    MjClient.playui.showhaidipai(tData.haidiCard);
                    //海底捞
                    if(tData.winner != -1 && tData.mHaidiPl >= 0)
                    {
                        var huangzhuangsign = haidipaikuang.getChildByName("huangzhuangsign");
                        if(huangzhuangsign) huangzhuangsign.removeFromParent();
                ...
                //小结算弹窗
                cc.log("isUndefined "+ cc.isUndefined(showCards) + " !  " + !showCards + " length " +showCards.length );
                ...
                cc.log("tData.mHaidiPl"+ tData.mHaidiPl);
                //抓鸟弹窗
                if (tData.mHaidiPl < 0) 
                {
                    cc.log("抓鸟弹窗");
                    //this.runAction(cc.sequence(cc.DelayTime(1.2),cc.callFunc(addshouwcardlayer)));
                    this.runAction(cc.sequence(cc.callFunc(addshouwcardlayer),cc.callFunc(delayExe)));
                }
                /*this.runAction(cc.sequence(cc.DelayTime(1.2),cc.callFunc(addshouwcardlayer)));*/
...
/*                if(MjClient.getAppType() == MjClient.APP_TYPE.QXYYQP)
            ...
            initSceneData: function() {
                ...
                reConectHeadLayout(this);
                cc.log("initSceneData -----------------xiangyintuidaohu = " + JSON.stringify(this));
                CheckRoomUiDelete();
        ...
        BtnPutCard:{ //add by  sking for put card button
            ...
            _event:{
                ...
                waitPut: function() {
                    if (_isShowShaiziAni_xiangyintuidaohu) return;
                    var pl = getUIPlayer(0);
                    var eat = MjClient.playui.jsBind.eat;
                    ...
                    if (IsTurnToMe() && pl.isTing && !eat.hu._node.visible && !eat.gang0._node.visible && !eat.gang1._node.visible && !eat.gang2._node.visible) {
                        cc.log("*********自动出牌*********");
                        this.runAction(cc.sequence(cc.delayTime(0.7), 
        ...
        BtnReady: {
            ...
            _click: function(_this) {
                MjClient.MJPass2NetForxiangyintuidaohu();
...
/*                        waitHaiDiLao: function()
            ...
            play_tips: {
                ...
                _run: function () {
                    this.zIndex = actionZindex_xiangyintuidaohu;
                },
                _visible:false,
            },
            tai_layout:{
            ...
            tingCardsNode: {
                ...
                _event: {
                    ...
                    initSceneData:function(eD)
                    {
                        ...
                        var pl = getUIPlayer(0);
                        var tData = MjClient.data.sData.tData;
                        if(pl && tData.tState == TableState.waitPut || tData.tState == TableState.waitEat || tData.tState == TableState.waitCard)
                        {
            ...
            _event: {
                ...
                initSceneData: function(eD) {
                    SetUserVisible_xiangyintuidaohu(this, 0);
                    ...
                    if(pl.isTing) return; // 开杠后不显示
                    var tingSet = MjClient.majiang.calTingSet(pl.mjhand, tData.hunCard);
                ...
                mjhand: function(eD) {
                    if (showShaiziAni_xiangyintuidaohu)
                        showShaiziAni_xiangyintuidaohu(0);
                    else
                        InitUserHandUI_xiangyintuidaohu(this, 0);
                },
                roundEnd: function() {
                    var self = this;
                ...
                MJChi: function(eD) {
                    // DealMJChi(this, eD, 0);
                    updateHeadUI_xiangyintuidaohu(this, 0 , ActionType.CHI);
                    setUserOffline(this, 0);
                ...
                MJPeng: function(eD) {
                    ...
                    if (MjClient.rePlayVideo != -1) {
                        ...
                        updateHeadUI_xiangyintuidaohu(this, 0, ActionType.PENG);
                    }
                    setUserOffline(this, 0);
                    MjClient.playui.EatVisibleCheck();
                    if (MjClient.data.sData.tData.curPlayer == getPlayerIndex(0))
...
/*                        waitHaiDiLao: function()
            ...
            _event: {
                ...
                initSceneData: function(eD) {
                    SetUserVisible_xiangyintuidaohu(this, 1);
                },
                addPlayer: function(eD) {
                    SetUserVisible_xiangyintuidaohu(this, 1);
                },
                ...
                mjhand: function(eD) {
                    if (!showShaiziAni_xiangyintuidaohu)
                        InitUserHandUI_xiangyintuidaohu(this, 1);
                },
                roundEnd: function() {
                    var parent_node = this.getParent();
                    var self = this;
                ...
                MJChi: function(eD) {
                    if (MjClient.rePlayVideo != -1)
                        ...
                        updateHeadUI_xiangyintuidaohu(this, 1, ActionType.CHI);
                    setUserOffline(this, 1);
                },
                MJGang: function(eD) {
                ...
                MJPeng: function(eD) {
                    if (MjClient.rePlayVideo != -1)
                        ...
                        updateHeadUI_xiangyintuidaohu(this, 1, ActionType.PENG);
                    setUserOffline(this, 1);
                },
                MJHu: function(eD) {
...
/*                        waitHaiDiLao: function()
            ...
            _event: {
                ...
                initSceneData: function(eD) {
                    SetUserVisible_xiangyintuidaohu(this, 2);
                },
                addPlayer: function(eD) {
                    SetUserVisible_xiangyintuidaohu(this, 2);
                },
                ...
                mjhand: function(eD) {
                    if (!showShaiziAni_xiangyintuidaohu)
                        InitUserHandUI_xiangyintuidaohu(this, 2);
                },
                roundEnd: function() {
                    var parent_node = this.getParent();
                    var self = this;
                ...
                MJChi: function(eD) {
                    if (MjClient.rePlayVideo != -1)
                        ...
                        updateHeadUI_xiangyintuidaohu(this, 2, ActionType.CHI);
                    setUserOffline(this, 2);
                },
                MJGang: function(eD) {
                ...
                MJPeng: function(eD) {
                    if (MjClient.rePlayVideo != -1)
                        ...
                        updateHeadUI_xiangyintuidaohu(this, 2, ActionType.PENG);
                    setUserOffline(this, 2);
                },
                MJHu: function(eD) {
...
/*                        waitHaiDiLao: function()
            ...
            _event: {
                ...
                initSceneData: function(eD) {
                    SetUserVisible_xiangyintuidaohu(this, 3);
                },
                addPlayer: function(eD) {
                    SetUserVisible_xiangyintuidaohu(this, 3);
                },
                ...
                mjhand: function(eD) {
                    if (!showShaiziAni_xiangyintuidaohu)
                        InitUserHandUI_xiangyintuidaohu(this, 3);
                },
                roundEnd: function() {
                    var parent_node = this.getParent();
                    var self = this;
                ...
                MJChi: function(eD) {
                    if (MjClient.rePlayVideo != -1)
                        ...
                        updateHeadUI_xiangyintuidaohu(this, 3, ActionType.CHI);
                    setUserOffline(this, 3);
                },
                MJGang: function(eD) {
                ...
                MJPeng: function(eD) {
                    if (MjClient.rePlayVideo != -1)
                        ...
                        updateHeadUI_xiangyintuidaohu(this, 3, ActionType.PENG);
                    setUserOffline(this, 3);
                },
                MJHu: function(eD) {
            ...
            changeui: {
                ...
                changeuibg: {
                    ...
                    guobg: {
                        guo: {
                            _touch: function(btn, eT) {
                                if (eT == 2) MjClient.MJPass2NetForxiangyintuidaohu();
                            }
                        },
                        fanhui: {
                            _touch: function(btn, et) {
    ...
    ctor: function() {
        ...
        var playui = ccs.load("Play_xiangyintuidaohu.json");
...
PlayLayer_xiangyintuidaohu.prototype.CardLayoutRestore = function(node, off, isNewCard)
{
...
// 提示等待起手胡
PlayLayer_xiangyintuidaohu.prototype.ShowQiShouHuTip = function()
{
    var eat = MjClient.playui.jsBind.eat;
    var sData = MjClient.data.sData;
...
PlayLayer_xiangyintuidaohu.prototype.ShowQiShouHuBtn = function()
{
    var eat = MjClient.playui.jsBind.eat;
    var sData = MjClient.data.sData;
    var pl = sData.players[SelfUid()];
...
PlayLayer_xiangyintuidaohu.prototype.EatKaiGangVisibleCheck = function(tData, vnode, eat, pl) 
{
    ...
    for (var i = 0; i < 2; i++) {
        ...
        if ((eatFlag & 4) && MjClient.majiang.canKaiGang(pl.mjhand, lastPutCard, false, pl.mjpeng, isKaiGangAfterSelfAnGang, tData)) {
...
PlayLayer_xiangyintuidaohu.prototype.showKaiGangBigCards = function() {
    var tData = MjClient.data.sData.tData;
...
PlayLayer_xiangyintuidaohu.prototype.showhaidipai = function(haidiCard)
{
    var tData = MjClient.data.sData.tData;
...
PlayLayer_xiangyintuidaohu.prototype.showzhuaniao = function(showCards)
{
    cc.log("addzhuaniao");
...
PlayLayer_xiangyintuidaohu.prototype.hideAllEat = function(){
    var eat = MjClient.playui.jsBind.eat;
...
// 判断吃碰杠胡的状态
PlayLayer_xiangyintuidaohu.prototype.EatVisibleCheck = function()
{
    ...
    else if(tData.tState == TableState.waitPut && pl.mjState == TableState.waitPut)
    {
        if(IsTurnToMe())
        {
            ...
            if(rtn.length > 0 && !pl.isPass)
            {
                ...
                {
                    if (pl.justPeng != rtn[i] && MjClient.majiang.canKaiGang(pl.mjhand, rtn[i], true, pl.mjpeng, false, tData))
                    {
                    ...
                    else if (pl.isNew && !pl.isTing) { // 补杠
                        if (pl.mjhand[pl.mjhand.length - 1] == rtn[i] || !tData.areaSelectMode["minggangguogangbunengbu"]) {
                            MjClient.gangCards.push(rtn[i]);
                            if (vnode.indexOf(eat.gang0._node) == -1) {
                                vnode.push(eat.gang0._node); 
                            }
...
PlayLayer_xiangyintuidaohu.prototype.RemoveNewCardOut = function(node, card)
{
...
PlayLayer_xiangyintuidaohu.prototype.DealKaiGang = function(node, msg, off)
{
...
PlayLayer_xiangyintuidaohu.prototype.MJChiCardchange_kaiGang = function(){
...
PlayLayer_xiangyintuidaohu.prototype.MJPengCardchange_kaiGang = function() {
...
PlayLayer_xiangyintuidaohu.prototype.addtingcardsign = function(children,tingCard)
{
    for (var i = 0; i < children.length; i++) {
        if (children[i].name == "mjhand")
        {
...
PlayLayer_xiangyintuidaohu.prototype.clearntingcardsign = function(children)
{
    for (var i = 0; i < children.length; i++) {
        if (children[i].name == "mjhand") 
        {
...
PlayLayer_xiangyintuidaohu.prototype.gettingcard = function(children,card)
{
/*    var canTingCards = {};
...
/*    var canTingCards = {};
    for (var i = 0; i < children.length; i++) {
        if (children[i].name == "mjhand") {
            var tingCards = MjClient.majiang.getCheckTingHuCards(children[i].tag,card);
            
            if (Object.keys(tingCards).length > 0) 
            {
                canTingCards[children[i].tag] = 1;
            }
        }
    }
    return canTingCards;*/
}
PlayLayer_xiangyintuidaohu.prototype.changesound = function(soundstr,pl)
{
    //男女声切换
    if (pl && pl.info.sex == 1)
    {
        var newPath = soundstr.replace("/nv/", "/nan/");
    ...
    var str = "sound/"+soundstr+".mp3";
    return str;
}

PlayLayer_xiangyintuidaohu.prototype.geteffectinroundend = function(off)
{ 
    //var pl = null;
    ...
    for (var i in SoundMap) {
        if ((""+pl.mjdesc).indexOf(i) >= 0) 
        {
            var soundfile = "tuidaohu/nv/" + SoundMap[i];
            var soundname = MjClient.playui.changesound(soundfile,pl);
...
PlayLayer_xiangyintuidaohu.prototype.playeffectinroundend = function(node)
{

    var soundlist = [];
    for (var i = 0; i < 4 ; i++) {
        var soundres = MjClient.playui.geteffectinroundend(i);
        if (soundres) 
        {
...
/*    for (var i = 0; i < soundlist.length; i++) { 
        (function(i) { 
            node.runAction(
                cc.sequence(cc.delayTime(1.0*i),
                cc.callFunc(
                    function(){
                        var index = i;
                        cc.log("soundlist"+ soundlist[index]);
                        reallyPlayEffect(soundlist[index],false); 
                    })
                ));
        })(i);       
    }
