...
PaodekuaiXuzhou.prototype.CARDTPYE = PDK_CARDTPYE;
var PDK_CARDCOUNT = {};
...
PaodekuaiXuzhou.prototype.allTipsNoOrder = [
    [PDK_CARDTPYE.danpai, []],
    [PDK_CARDTPYE.duizi, []],
 ...
 * @return {number}
 */
PaodekuaiXuzhou.prototype.calPoint = function(num) {
    if (!num)
        return -1;
 ...
 * @return {boll} 
 */
PaodekuaiXuzhou.prototype.isSanShun = function(oCards){
...
// console.log( PaodekuaiXuzhou.prototype.isSanShun([3,3,3]) );
 ...
 */
PaodekuaiXuzhou.prototype.getSanShunAndSanPai = function(oCards){
    var countSanSame = 0;
    // 记录所有3张的数值
    var sanSameList = [];
    // 记录散牌
...
// console.log( PaodekuaiXuzhou.prototype.getSanShunAndSanPai(handCards) );


PaodekuaiXuzhou.prototype.countDuiNum = function(oCards){
    var num = 0;
    var cardNumByValue = {};
    for(var i in oCards){
 ...
 */
PaodekuaiXuzhou.prototype.isFeiJi = function(oCards, areaSelectMode){
 ...
 * @return {bool}
 */
PaodekuaiXuzhou.prototype.isShun = function(oCards) {
...
PaodekuaiXuzhou.prototype.isZhaDan = function(oCards, areaSelectMode) {
...
PaodekuaiXuzhou.prototype.isHadZhaDan = function(cards, areaSelectMode) {
    ...
    if( areaSelectMode.can3ge3ZhaDan ) {
        ...
        var isSan3 = this.findNSameCard(newHands, PDK_MINPOINT, 3);
...
PaodekuaiXuzhou.prototype.calType = function(pCards, areaSelectMode) {
    ...
    // 三带二
    if (cardCount == PDK_CARDCOUNT[PDK_CARDTPYE.sandaier] && maxCount >= 3 && areaSelectMode.can3daiNum == 2)
...
PaodekuaiXuzhou.prototype.cardsType = function(cards, areaSelectMode) {
 ...
 * @return {number}
 */
PaodekuaiXuzhou.prototype.calCardsValue = function(cards, type, areaSelectMode) {
...
// 牌点比较函数
PaodekuaiXuzhou.prototype.cardValueCmp = function(a, b) {
...
PaodekuaiXuzhou.prototype.cardSortBigToSmall = function(a, b) {
    var pa = this.calPoint(a);
    var pb = this.calPoint(b);
    if (pa == pb)
        return b - a;
 ...
 * @return {number} 返回癞子数
 */
PaodekuaiXuzhou.prototype.transformAndGetLaizi = function(oCards, cards) {
    var laizi = 0;
    for (var i = 0; i < oCards.length; i++) {
...
PaodekuaiXuzhou.prototype.canPut = function(oCards, oLastCards, handsNum, areaSelectMode) {
    ...
    if( cardsType == PDK_CARDTPYE.sanzhang || (cardsType == PDK_CARDTPYE.sandaiyi && areaSelectMode.can3daiNum == 2) ){
...
PaodekuaiXuzhou.prototype.getMaxDanPai = function(oCards) {
 ...
 */
PaodekuaiXuzhou.prototype.sortHandCards = function(oCards, sortType) {
 ...
 * @return {array}
 */
PaodekuaiXuzhou.prototype.findNSameCard = function(hands, point, n) {
...
// 删除相应牌值
PaodekuaiXuzhou.prototype.delPoint = function(hands, points){
    var arr = hands.slice();
 ...
 */
PaodekuaiXuzhou.prototype.lieju = function(parr, num){
    var tmps = [];
...
PaodekuaiXuzhou.prototype.formatCards = function(oCards) {
    var info = {};
...
PaodekuaiXuzhou.prototype.indexOfCards = function(cardList, cards) {
...
PaodekuaiXuzhou.prototype.isSameCards = function(cards1, cards2) {
...
PaodekuaiXuzhou.prototype.findCardByRole = function(oCards, findNum, ignoreCardValued) {
...
PaodekuaiXuzhou.prototype.getAllMaxCard = function(oCards){
...
PaodekuaiXuzhou.prototype.sortByCardType = function(cardsList, areaSelectMode) {
...
PaodekuaiXuzhou.prototype.findDaiPai = function(oCards, type, findNum, same, buChaiTypes) {
    var ret = [];
...
PaodekuaiXuzhou.prototype.initCanPutTypes = function(hands, includeCard, areaSelectMode) {
 ...
 * @return {array} 提示的牌
 */
PaodekuaiXuzhou.prototype.findPutTipCards = function(hands, lastCards, areaSelectMode, isNextPlayerOneCard, isFirstRound, putOrder, allBuChaiTypes) {
...
PaodekuaiXuzhou.prototype.tipCards = function(oHands, oLastCards, areaSelectMode, isNextPlayerOneCard, isFirstRound, isSmartTip) {
    ...
    if (areaSelectMode.can3ge3ZhaDan) {
        var cardtypes = this.findCardByType(hands, 0, PDK_CARDTPYE.sange3, null, areaSelectMode);
    ...
    var booms = this.findCardByType(hands, 0, PDK_CARDTPYE.sizha, null, areaSelectMode);
    // 不是压三带二的时候， 最后提示炸弹
    ...
    if(areaSelectMode.can3aZhaDan){
        rets = rets.concat(this.findCardByType(hands, 0, PDK_CARDTPYE.sangeA, null, areaSelectMode) );
    }
    return rets;
};
 ...
 */
PaodekuaiXuzhou.prototype.IsSametipCards = function(tipscardsArray, areaSelectMode)
{
 ...
 * @param  {Boolean} areaSelectMode 创建房间的选项
 */
PaodekuaiXuzhou.prototype.delDaipai = function(Cards, areaSelectMode)
{
    var cardtype = this.calType(Cards, areaSelectMode);
...
// console.log('PaodekuaiXuzhou.prototype.checkPut', pdk.checkPut(handCards, putCards, lastCards) );
...
var areaSelectMode = {"can3daiNum":2, "firstHeiTao3":true,"cardNumIndex":0,"showCardNumber":false,"zhaDanBuChai":false,"can4dai2":true,"can4dai3":true,"can3aZhaDan":false,"hongTao10Niao":false,"fangZuoBi":true,"payWay":0}
var change = function(cards) {
    ...
    var feijiInfo = pdk.formatFeiJiType(cards, areaSelectMode);
    // console.log(cards);
    console.log(str);
}
...
handCards = [c.mh[7], c.hx[7], c.fk[7],c.mh[8], c.hx[8], c.fk[8], c.mh[9], c.mh[10], c.mh[11], c.mh[12], c.mh[6], c.hx[6]];
lastCards = [c.ht[3], c.mh[3], c.fk[3], c.hx[4], c.mh[4],c.ht[4], c.mh[5], c.fk[5], c.hx[5], c.mh[6] ];
