...
var shareLayer_picture = cc.Layer.extend({
    ...
    ctor: function (filePath, isShare,callback) {
		...
		if (isUseUIv3) {
			var UI = ccs.load("ShareLayer_3.0.json");
		} else {
...
var YaoQingHaoBao_share = function(data, sp) {
	...
	var layer = new shareLayer_picture();
...
var YaoQingHaoBao_layer = cc.Layer.extend({
	onExit:function()
    {
        this._super();
	...
	ctor: function() {
		...
		_btn_mybao.addTouchEventListener(function(sender, type) {
			if (type == 2) {
				MjClient.Scene.addChild(new YaoQingHaoBao_main());
			}
		}, this);
		...
		Text_1.setString("1.每邀请一名新玩家下载绑定并完成对局双方即可领取拼手气红包");
		var Text_2= _back.getChildByName("Text_2");
		Text_2.ignoreContentAdaptWithSize(true);
		Text_2.setString("2.新玩家下载绑定并完成8场后可在邮件中领取红包，注意查收~");
	...
	fresh_table: function() {
			...
			if (type == 2) {
				...
				cc.loader.loadImg(imageUrl,   {
					...
					if (err) {
						...
						YaoQingHaoBao_share(self._data, sprite_bg);
					}
				});
		...
		this._btn_xiangqing.addTouchEventListener(function(sender, type) {
			if (type == 2) {
				MjClient.Scene.addChild(new YaoQingHaoBao_rule(this._data));
			}
		}, this);
...
var YaoQingHaoBao_main = cc.Layer.extend({
	ctor: function(data) {
		this._super();
	...
	choose_sort: function(number) {
		...
		if (number == 0) { // 未完成
			...
			for (var i = 0; i < list.length; i++) {
				if (list[i].game < this._data.round || !list[i].mobileNum) {
	...
	createItem: function(oneData) {
		...
		var str = (oneData.game >= this._data.round && oneData.mobileNum) ? "完成" : "未完成";
...
var YaoQingHaoBao_rule = cc.Layer.extend({
	ctor: function(data) {
		this._super();
		...
		var str = "活动详情：\n" +
		"一、本次活动为周期邀请红包活动，15天为一个周期，每更换一次周期邀请人数清零,邀请人数奖励也会重置\n" +
		"二、只针对于邀请人与新玩家，每邀请一名新玩家下载游戏、绑定手机并完成对战"+ data.round+ "场后，双方即可获得一个手气红包奖励\n" +
		"三、每天可随时提现,24小时后到账公众号,注意查收，现金红包请前往公众号“" + MjClient.systemConfig.gongzhonghao + "”领取.\n" +
		"四、额外奖励：累计邀请并获得奖励人数达标后可获得额外现金奖励~"+
			"\n"+
			"1、每个周期内累积邀请并获得奖励人数达到" + data.awardList[0].condition + "人,额外奖励" + data.awardList[0].award + "元；\n" +
			"2、每个周期内累积邀请并获得奖励人数达到" + data.awardList[1].condition + "人,额外奖励" + data.awardList[1].award + "元；\n" +
			"3、每个周期内累积邀请并获得奖励人数达到" + data.awardList[2].condition + "人,额外奖励" + data.awardList[2].award + "元；\n" +
			"4、每个周期内累积邀请并获得奖励人数达到" + data.awardList[3].condition + "人,额外奖励" + data.awardList[3].award + "元；\n" +
			"5、每个周期内累积邀请并获得奖励人数达到" + data.awardList[4].condition + "人,额外奖励" + data.awardList[4].award + "元；\n" +
