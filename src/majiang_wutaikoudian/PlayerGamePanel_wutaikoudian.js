...
//向服务器发送 过消息
MjClient.MJPass2NetForwutaikoudian = function()
{
   // console.log(">>>>>>>>>普通  过 <<<<<<<<");
...
function SetUserVisible_wutaikoudian(node, off)
{
    //var sData = MjClient.data.sData;
    //return;
    cc.log("====================off======================" + off);
...
//处理出牌,放一张牌，打牌动作
function DealMJPut_wutaikoudian(node, msg, off, outNum)
{
...
function InitUserHandUI_wutaikoudian(node, off)
{
    var sData = MjClient.data.sData;
    var tData = sData.tData;
    var pl = getUIPlayer(off);
    ...
    for(var i = 0; i < pl.mjput.length; i++)
    {
        ...
        DealMJPut_wutaikoudian(node, msg, off, i);
    }
...
function initFlower_koudian() {
    initFlower(false, false);
}


var PlayLayer_wutaikoudian = cc.Layer.extend({
    jsBind: {
        _event: {
            mjhand: function() {
            ...
            roundEnd: function() {
                ...
                function delayExe()
                {
                    ...
                    self.addChild(new EndOneView_wutaikoudian(),500);
                }
        ...
        eat: {
            ...
            guo: {
                ...
                _touch: function(btn, eT) {
                    if (eT == 2)
                    {
                        MjClient.MJPass2NetForwutaikoudian();
                    }
                },
                bgimg: {
                    _run: function() {
            ...
            changeui: {
                ...
                changeuibg: {
                    ...
                    guobg: {
                        guo: {
                            _touch: function(btn, eT) {
                                if (eT == 2) MjClient.MJPass2NetForwutaikoudian();
                            }
                        },
                        fanhui: {
                            _touch: function(btn, et) {
    ...
    ctor: function() {
        this._super();
        var playui = ccs.load("Play_wutaikoudian.json");
...
PlayLayer_wutaikoudian.prototype.CardLayoutRestore = function(node, off)
{
...
// 判断吃碰杠胡的状态
PlayLayer_wutaikoudian.prototype.EatVisibleCheck = function()
{
    var eat = MjClient.playui.jsBind.eat;
...
/*
    玩家ui，信息
 */
PlayLayer_wutaikoudian.prototype.Node_player = function(off)
{
    ...
    UIEventBind(null, _node, "initSceneData", function (eD) {
        SetUserVisible_wutaikoudian(_node, off);
    ...
    UIEventBind(null, _node, "mjhand", function (eD) {
        InitUserHandUI_wutaikoudian(_node, off);
    });
    ...
    UIEventBind(null, _node, "MJPut", function (eD) {
        ...
        // }
        DealMJPut_wutaikoudian(_node,eD,off);
        hideTingBtn();
        if(_tingCardNumNode) _tingCardNumNode.visible = false;
