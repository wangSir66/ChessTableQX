...
daZhaDan.CardThrowListLayer = cc.Node.extend({
    /**
     * @type {ui.LayoutNode}
     */
    _layerNode : null,
    ctor : function () {
        ...
        this._layerNode = new daZhaDan.LayoutNode();
    ...
    showHandCards : function(list) {
        this.unscheduleAllCallbacks();
        this._layerNode.removeAllChildren();
        MjClient.majiang.formatSort(list);
        var len = list.length;
        for(var i = 0; i < len; i++)
        {
            ...
            card.setStatus(daZhaDan.CardStatus.UNHOOD);
    ...
    showCards : function (list, sex, isLocal) {
        ...
        var isSZ = MjClient.majiang.isSanZhang(list, tData.areaSelectMode, -1);
        if(flg || isSZ) {
            var feiJiObj; 
            if(isSZ){
                feiJiObj = MjClient.majiang.splitSanZhang(list); 
            }else{
                feiJiObj = MjClient.majiang.splitFeiJi(list, tData.shunType, tData.hasWings, tData.bodyLen); 
            }
            ...
            var list = MjClient.majiang.formatPutCard(list, tData.lastPutCardType, tData.areaSelectMode, tData.bodyLen);
            for(var i = 0; i < len; i++)
            {
                var info = new daZhaDan.CardInfo();
                info.type = list[i];
...
daZhaDan.CardListLayer = cc.Node.extend({

    /**
     * 牌信息列表
     * @type Array.<daZhaDan.CardInfo>
     */
    _list : null,
    /**
     * 卡牌列表
    ...
    unTouchCards : function () {
        ...
        for(var i = 0; i < this._cardsList.length; i++){
            ...
            if(flg){
                c.setStatus(daZhaDan.CardStatus.GRAY);
    ...
    _initUI : function () {
        var node = new daZhaDan.LayoutNode();
    ...
    showTeamHand : function(isShow, handCards){
        ...
        if(isShow){
            ...
            handCards = MjClient.majiang.sortCard(handCards, MjClient.data.sData.tData.areaSelectMode, MjClient.majiang.sortType);
            var arr = [];
    ...
    upBtnNode : function () {
        ...
        //是否要显示多牌型按钮
        this.checkAndShowOtherBtn();
    },

    checkAndShowOtherBtn : function(){
        var sData = MjClient.data.sData;
    ...
    grayNotFindCards: function() {
        var sData = MjClient.data.sData;
        var pl = sData.players[SelfUid()];
        if(pl.bao != -1 || !this._btnNode.visible){
            return;
        }

        var mjhand = MjClient.data.sData.players[MjClient.data.pinfo.uid].mjhand;
        var cards = MjClient.majiang.getNotFindCards(mjhand);
        for(var i = 0; i < this._cardsList.length; i++){
            var c = this._cardsList[i];
            var flg = this._hasTypeInArr(c.getInfo().type, cards);
            if(flg){
                c.setStatus(daZhaDan.CardStatus.GRAY);
            }
        }
        ...
        if(flg){
            ...
            if(!tData || tData.lastPutPlayer < 0){
                ...
                if(uid == MjClient.data.pinfo.uid || !MjClient.data.sData.tData.lastPutCard){
                    ...
                    //     var cards = tData.lastPutCard;
                    //     var type = tData.lastPutCardType;
                    //     var flg = MjClient.majiang.canBeat(pl.mjhand, cards, type, tData.areaSelectMode, tData.bodyLen);
                    //     if(!flg){
    ...
    /**
     * 获取可以大过当前牌型的牌一种组合
     * @return {Array.<daZhaDan.CardInfo>}
     */
    getTipList : function () {
        this._unSelectAllCards();
        ...
        if(!this._tipList){
            ...
            this._tipList = MjClient.majiang.hintPutCard(mjhand, tData.lastPutCard, tData.lastPutCardType, 
                tData.areaSelectMode, tData.bodyLen);
    ...
    _handBtnClick : function (sender) {
            ...
            sender == this._twoBtn)
        {
            ...
            if(cards.length <= 0){
                ...
                var tData = MjClient.data.sData.tData;
                var desc = MjClient.majiang.getBreakUpDesc(hand, arr, tData.areaSelectMode);
                if(desc.length > 0){
                    ...
                    MjClient.showMsg(msg,
                        function() {
                            ...
                            var type = MjClient.majiang.getCardType(arr, tData.areaSelectMode, hand.length);
                            if(type == daZhaDan.CARD_TYPE.noType){
            ...
            var arr = this._getSelectCards();
            var card = arr[0];
            if(arr.length == 1 && card){
                MjClient.gamenet.request("pkroom.handler.tableMsg", {
                    cmd: "PKFindFriend",
                    card: card._info.type
                }); 
                this._unSelectAllCards();
            }else{
                if(arr.length > 1){
                    MjClient.showToast("只能选择一张找队友的牌");
                }else{
                    MjClient.showToast("请先选择找队友的牌");
                }
            }
            
            playEffect("daZhaDan/effect/btnClick", false);
        }else if(sender == this._bpBtn){
            MjClient.showMsg("您确定要包牌吗?", function() {
                MjClient.gamenet.request("pkroom.handler.tableMsg", {
                    cmd: "PKBao",
                    bao: 1
                });
    ...
    handSelfPutCards : function(cards){
        ...
        postEvent("PKPut_daZhaDan",{cards:cards, sex: MjClient.data.pinfo.sex});
        playEffect("daZhaDan/effect/cardOut", false);
        playSoundEffect_daZhaDan(cards, SelfUid(), true);
    },
    ...
    refreshHandCards : function(){
        ...
        arr = MjClient.majiang.sortCard(arr, MjClient.data.sData.tData.areaSelectMode, MjClient.majiang.sortType);
        cc.log("refreshHandCards arr:", arr);
    ...
    _unGrayAllCards : function () {
        ...
        {
            var c = this._cardsList[i];
            c.setStatus(daZhaDan.CardStatus.UNHOOD);
        }
    },
    ...
    doMarkCard : function (card) {
        if(card && card.status != daZhaDan.CardStatus.GRAY)
        {
            var ty = card.y;
            if(card.isMark)
            {
                ...
                card.isMark = true;
                card.setStatus(daZhaDan.CardStatus.MARK);
            }
        }else
        {
            var len = this._cardsList.length;
            ...
            {
                var c = this._cardsList[i];
                if(card.status == daZhaDan.CardStatus.MARK){
                    c.isMark = false;
                    c.setStatus(daZhaDan.CardStatus.UNHOOD);
                }
    ...
    doSelectAction : function (card) {
        ...
        }else if(card && card.status == daZhaDan.CardStatus.GRAY){
            card.stopAllActions();
            var ty = card.y;
            if(card.isSelected)
            {
    ...
    _checkBtnStatus : function(){
        if(this._btnNode.visible){
            ...
            var handCount = 0;
            if(pl){
                handCount = pl.mjhand.length;
            }
            if(tData.lastPutCard && tData.lastPutCard.length > 0 && tData.lastPutCardType >= 0){
                var flg = MjClient.majiang.isBigger(typeList, tData.lastPutCard, tData.lastPutCardType, 
                    tData.areaSelectMode, handCount, tData.bodyLen);
                ...
                var tData = MjClient.data.sData.tData;
                var type = MjClient.majiang.getCardType(typeList, tData.areaSelectMode, handCount);
                if(type == daZhaDan.CARD_TYPE.noType){
                    this._chuBtn.setIsTouchEnable(false);
                    this._chuBtn.setBright(false);
                }
    ...
    _unSelectAllCards : function()
    {
        ...
        for(var i = 0; i < len; i++)
        {
            ...
            if(c && cc.sys.isObjectValid(c))
            {
                // c.y = c.row * c.rowGap;
                c.isSelected = false;
                if(c.status != daZhaDan.CardStatus.GRAY)
                {
                    c.isMark = false;
                    c.setStatus(daZhaDan.CardStatus.UNHOOD);
    ...
    _addCard : function () {
        ...
        card.y = 0;
        card.row = 0;
        card.setStatus(daZhaDan.CardStatus.HOOD);
        this.addChild(card);
        this._cardsList.push(card);
    ...
    _unhoodCard : function () {
        ...
        // card && card.setStatus(daZhaDan.CardStatus.UNHOOD);
        card && card.rotateAction(true);

        this._count += 1;
    },

    doLiPai : function(sortType){
        ...
        arr = MjClient.majiang.sortCard(arr, MjClient.data.sData.tData.areaSelectMode, sortType);
        cc.log("doLiPai arr:", arr);
