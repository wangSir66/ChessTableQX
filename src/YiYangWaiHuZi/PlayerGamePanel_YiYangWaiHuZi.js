...
//     }else{
//         HZPassConfirmToServer_YiYangWaiHuZi();
//     }
// };

// 这个没看懂干嘛的
// off 是四个位置，根据off 显示四个位置的信息 by sking
function SetUserVisible_YiYangWaiHuZi(node, off){
    ...
    if(pl){
        ...
        setUserOffline_YiYangWaiHuZi(node, off);
        // InitUserHandUI_YiYangWaiHuZi(node, off);
...
function InitUserHandUI_YiYangWaiHuZi(node, off){
    ...
    if (pl.jiazhuNum == 2 && MjClient.rePlayVideo == -1)
    {
        ...
        MjClient.majiang.setJiaZhuNum(node, getUIPlayer_YiYangWaiHuZi(off));
    }
...
var PlayLayer_YiYangWaiHuZi = cc.Layer.extend({
    jsBind: {
        _event: {
            mjhand: function() {
            ...
            endRoom: function(msg) {
                ...
                if (msg.showEnd){
                    this.addChild(new GameOverLayer_YiYangWaiHuZi(),500);
                }else{
            ...
            roundEnd: function() {
                ...
                if(!MjClient.isDismiss && tData.hunCard && tData.hunCard != -1 && tData.areaSelectMode.xingType==2){
                    ...
                    ShowEatActionAnim_YiYangWaiHuZi(MjClient.playui._downNode, ActionType_paohuzi.FANXING, 0);
                    DealFanXingNewCard_YiYangWaiHuZi(hunCard);
                }
                ...
                function delayExe(){
                    ...
                    if (sData.tData.roundNum <= 0){
                        var layer = new GameOverLayer_YiYangWaiHuZi();
                        layer.setVisible(false);
                        self.addChild(layer,500);
                    }
                    if(!MjClient.endoneui){
                        self.addChild(new EndOneView_YiYangWaiHuZi(),500);
                    }
                ...
                var showShen = function () {
                    var showShen = MjClient.playui.jsBind.showShen;
                    showShen._node.zOrder = 400;
                    showShen._node.visible = true;

                    var card = showShen._node.getChildByName("card");
                    var huangZhuang = showShen._node.getChildByName("huangZhuang");
                    if(tData.shenJudge == -1){
                        ...
                        var cardNum = getNewCard_YiYangWaiHuZi(tData.shenCard, 3, 0);
                        cardNum.removeAllChildren();//断线重连 当前打牌玩家为自己 put节点下面有字牌贴图
                        cardNum.scaleX = card.width/cardNum.width;
                        cardNum.scaleY = card.width/cardNum.width;
                        cardNum.setPosition(card.width / 2, card.height / 2);
                        card.addChild(cardNum);

                        var shenIcon = card.getChildByName("shenIcon");
                        var shenSound = ["meishen", "youshen"];
                        var fileName = ["wushen" , "youshen"];
                        shenIcon.loadTexture("playing/waihuzi/" + fileName[tData.shenJudge] + ".png");
            ...
            moveHead: function() {
                ...
                //tableStartHeadMoveAction_YiYangWaiHuZi(this);   //不涉及到头像移动动作
            },
            initSceneData: function() {
                ...
                reConectHeadLayout_YiYangWaiHuZi(this);        //不涉及到头像移动动作
                CheckRoomUiDelete();
                tableStartHeadMoveAction_YiYangWaiHuZi(this);   //不涉及到头像移动动作
            ...
            changeMJBgEvent: function() {
                changeMJBg_YiYangWaiHuZi(this, ziPai.getZiPaiType());
            },
            MJPeng: function() {
                MjClient.playui.showAndHideHeadEffect();
            },
            ...
            EZP_layout : function(){
                if(MjClient.data.sData.tData.maxPlayer < 4){
                    ...
                    if(IsArrowVisible_YiYangWaiHuZi()){
                        ...
                        MjClient.playui.ResetPutCard(MjClient.playui._rightNode, MjClient.MaxPlayerNum_YiYangWaiHuZi - 2);
                        MjClient.playui.ResetPutCard(MjClient.playui._topNode, MjClient.MaxPlayerNum_YiYangWaiHuZi - 1);
                    }
                }
        ...
        cardNumImg: {
            ...
            _event: {
                initSceneData: function(eD) {
                    this.visible = IsArrowVisible_YiYangWaiHuZi();
                },
                mjhand: function(eD) {
                    this.visible = IsArrowVisible_YiYangWaiHuZi();
                },
            ...
            cardNumImg:{
                _event:{
                    initSceneData:function(){
                        setCardNumImage_YiYangWaiHuZi(this);
                    },
                    mjhand:function(){
                        setCardNumImage_YiYangWaiHuZi(this);
                    },
            ...
            shenIcon:{
                ...
                _event:{
                    initSceneData:function(){
                        setShenIcon_YiYangWaiHuZi(this);
                    },
        ...
        operateWait: {
            ...
            _event: {
                initSceneData: function (eD) {
                    if (MjClient.playui.jsBind.left.head.timeImg.time._node.getString() == "0" && curPlayerIsMe_YiYangWaiHuZi(MjClient.MaxPlayerNum_YiYangWaiHuZi - 1) ||
                        MjClient.playui.jsBind.right.head.timeImg.time._node.getString() == "0" && curPlayerIsMe_YiYangWaiHuZi(MjClient.MaxPlayerNum_YiYangWaiHuZi - 2) && MjClient.MaxPlayerNum_YiYangWaiHuZi != 2) {
                        //this.visible = true;
                    }
                },
                mjhand: function (eD) {
        ...
        banner: {
            ...
            getRoomNum: {
                ...
                _click: function() {
                    ...
                    str += tData.areaSelectMode.isMaiPai ? "埋牌" + tData.areaSelectMode.maiPaiNum + "张," : "";
                    ...
                    var sData = MjClient.data.sData;
                    var str7 = "  二缺一";
                    if(MjClient.MaxPlayerNum_YiYangWaiHuZi == 3){
                        if((MjClient.MaxPlayerNum_YiYangWaiHuZi - Object.keys(sData.players).length) == 2){
                            str7 = "  一缺二";
                        }
                    }else if(MjClient.MaxPlayerNum_YiYangWaiHuZi == 4){
                        if((MjClient.MaxPlayerNum_YiYangWaiHuZi - Object.keys(sData.players).length) == 3){
                            str7 = "  一缺三";
                        }else if((MjClient.MaxPlayerNum_YiYangWaiHuZi - Object.keys(sData.players).length) == 2){
                            str7 = "二缺二";
                        }else  if((MjClient.MaxPlayerNum_YiYangWaiHuZi - Object.keys(sData.players).length) == 1){
                            str7 = "三缺一";
                        }
                    }
            ...
            gps_btn: {
                ...
                _run:function () {
                    //this.visible = MjClient.MaxPlayerNum_YiYangWaiHuZi >= 3 ? true : false;
                    this.setVisible(MjClient.MaxPlayerNum_YiYangWaiHuZi >= 3);
                },
                _click: function() {
                    if(MjClient.MaxPlayerNum_YiYangWaiHuZi == 3){
        ...
        wait: {
            ...
            delroom: {
                ...
                _event: {
                    initSceneData: function(eD) {
                        //this.visible = isShowBackBtn_YiYangWaiHuZi();
                    },
                    removePlayer: function(eD) {
                        //this.visible = isShowBackBtn_YiYangWaiHuZi();
                    },
        ...
        down: {
            head: {
                siShou:{
                    ...
                    _event:{
                        initSceneData:function () {
                            setSiShou_YiYangWaiHuZi(this, 0);
                        },
                        MJPeng: function(eD) {
                            setSiShou_YiYangWaiHuZi(this, 0);
                        },
                ...
                zhuang: {
                    ...
                    _event: {
                        waitPut: function(){
                            showUserZhuangLogo_YiYangWaiHuZi(this, 0);
                        },
                        initSceneData: function(){
                            if (IsArrowVisible_YiYangWaiHuZi()) showUserZhuangLogo_YiYangWaiHuZi(this, 0);
                        },
                        mjhand: function(){
                            if (IsArrowVisible_YiYangWaiHuZi()) showUserZhuangLogo_YiYangWaiHuZi(this, 0);
                        }
                    ...
                    chattext: {
                        _event: {
                            ...
                            MJChat: function(msg) {
                                ...
                                showUserChat_YiYangWaiHuZi(this, 0, msg);
                            },
                            playVoice: function(voicePath) {
                                MjClient.data._tempMessage.msg = voicePath;
                                showUserChat_YiYangWaiHuZi(this, 0, MjClient.data._tempMessage);
                            }
                        }
                    }
                },
                _click: function(btn) {
                    showPlayerInfo_YiYangWaiHuZi(0, btn);
                },
                _event: {
                    loadWxHead: function(d) {
                        setWxHead_YiYangWaiHuZi(this, d, 0);
                    },
                    addPlayer: function(eD) {
                        showFangzhuTagIcon_YiYangWaiHuZi(this,0);
                    },
                    removePlayer: function(eD) {
                        showFangzhuTagIcon_YiYangWaiHuZi(this,0);
                    }
                ...
                huxi:{
                    ...
                    _event:{
                        ...
                        initSceneData:function(){
                            this.visible = true;
                            var huXi = UpdateHuXi_YiYangWaiHuZi(0);
                            if(!IsArrowVisible_YiYangWaiHuZi()){
                                huXi = 0;
                                this.visible = false;
                            }
                ...
                timeImg:{
                    ...
                    _event:{
                        initSceneData: function(eD) {
                            //this.visible = curPlayerIsMe_YiYangWaiHuZi(0);
                        },
                        onlinePlayer: function(eD) {
                            //this.visible = curPlayerIsMe_YiYangWaiHuZi(0) && IsArrowVisible_YiYangWaiHuZi();
                        },
                        waitPut: function(eD) {
                            //this.visible = curPlayerIsMe_YiYangWaiHuZi(0);
                        },
                    ...
                    time:{
                        ...
                        _event: {
                            ...
                            onlinePlayer: function(){
                                ...
                                if(pl && pl.onLine && curPlayerIsMe_YiYangWaiHuZi(0)){
                                    arrowbkNumberUpdate(this);
                                }
                            },
                            roundEnd: function() {
            ...
            ready: {
                ...
                _run: function() {
                    GetReadyVisible_YiYangWaiHuZi(this, 0);
                },
                _event: {
                    startShuffleCards:function () {
                        GetReadyVisible_YiYangWaiHuZi(this, 0);
                    ...
                    moveHead: function() {
                        GetReadyVisible_YiYangWaiHuZi(this, -1);
                    },
                    addPlayer: function() {
                        GetReadyVisible_YiYangWaiHuZi(this, 0);//根据状态设置ready 是否可见 add by sking
                    },
                    removePlayer: function() {
                        GetReadyVisible_YiYangWaiHuZi(this, 0);
                    },
            ...
            handNode: {
                ...
                _event:  {
                    initSceneData: function() {
                        calculateHintPutList_YiYangWaiHuZi();
                    },
                    mjhand:function () {
                        calculateHintPutList_YiYangWaiHuZi();
                    },
                    ...
                    MJPass: function() { // 天胡 提 偎 跑后过胡
                        calculateHintPutList_YiYangWaiHuZi();
                        addTingSign_YiYangWaiHuZi(MjClient.playui._downNode);
                    },
                    HZCheckRaise: function() {
                        calculateHintPutList_YiYangWaiHuZi();
                        addTingSign_YiYangWaiHuZi(MjClient.playui._downNode);
            ...
            handCard: {
                _run:function () {
                    ...
                    var newFile = getNewMJBgFile_YiYangWaiHuZi(oldFile, ziPai.getZiPaiType());
            ...
            _event: {
                clearCardUI: function() {
                    clearCardUI_YiYangWaiHuZi(this, 0);
                },
                ...
                initSceneData: function(eD) {
                    SetUserVisible_YiYangWaiHuZi(this, 0);
                    InitUserHandUI_YiYangWaiHuZi(this, 0);
                    InitUserCoinAndName_YiYangWaiHuZi(this, 0);
                    MjClient.hasPut = false; // 重连回来重置打掉牌标志（打牌时候断线)
                    ShowPutCardIcon_YiYangWaiHuZi();
                    DealOffLineCard_YiYangWaiHuZi(this,0);
                    resetHandAfterBegin_YiYangWaiHuZi();
                },
                addPlayer: function(eD) {
                    SetUserVisible_YiYangWaiHuZi(this, 0);
                    InitUserCoinAndName_YiYangWaiHuZi(this, 0);
                ...
                mjhand: function(eD) {
                    ...
                    var uid = getUIPlayer_YiYangWaiHuZi(0).info.uid;
                    if (eD.jiazhuNums)
                    {
                ...
                HZNewCard: function(eD) {
                    ...
                    DealNewCard_YiYangWaiHuZi(this,eD,0);
                },
                MJPut: function(eD) {
                    DealPutCard_YiYangWaiHuZi(this,eD,0);
                },
                HZChiCard: function(eD) {
                    DealChiCard_YiYangWaiHuZi(this, eD, 0);
                    ShowPutCardIcon_YiYangWaiHuZi();
                },
                HZWeiCard: function(eD){
                    DealWeiCard_YiYangWaiHuZi(this,eD,0);
                },
                HZLiuCard: function(eD) {
                    DealGangCard_YiYangWaiHuZi(this, eD, 0);
                },
                MJPeng: function(eD) {
                    DealPengCard_YiYangWaiHuZi(this, eD, 0);
                    ShowPutCardIcon_YiYangWaiHuZi();
                },
                MJHu: function(eD) {
                    DealHu_YiYangWaiHuZi(this, eD, 0);
                },
                onlinePlayer: function(eD) {
                    resetHandAfterBegin_YiYangWaiHuZi();
                    setUserOffline_YiYangWaiHuZi(this, 0);
                },
                ...
                MJJiazhu: function(msg) {
                    ...
                    MjClient.majiang.setJiaZhuNum(MjClient.playui._downNode, getUIPlayer_YiYangWaiHuZi(0));
                    MjClient.majiang.setJiaZhuNum(MjClient.playui._rightNode, getUIPlayer_YiYangWaiHuZi(1));
                    MjClient.majiang.setJiaZhuNum(MjClient.playui._topNode, getUIPlayer_YiYangWaiHuZi(2));
                },
                ...
                HZAddCards:function (eD) {
                    DealAddCard_YiYangWaiHuZi(this,eD, 0);
                },
        ...
        xing: {
            _run:function () {
                if(MjClient.MaxPlayerNum_YiYangWaiHuZi == 3 || MjClient.MaxPlayerNum_YiYangWaiHuZi == 2){
                    this.setVisible(false);
                }
            },
            head: {
                siShou:{
                    ...
                    _event:{
                        initSceneData:function () {
                            if(MjClient.MaxPlayerNum_YiYangWaiHuZi == 4) {
                                setSiShou_YiYangWaiHuZi(this, 1);
                            }
                        },
                        MJPeng: function(eD) {
                            if(MjClient.MaxPlayerNum_YiYangWaiHuZi == 4) {
                ...
                zhuang: {
                    ...
                    _event: {
                        waitPut: function() {
                            if(MjClient.MaxPlayerNum_YiYangWaiHuZi == 4) {
                                showUserZhuangLogo_YiYangWaiHuZi(this, 1);
                            }
                        },
                        initSceneData: function() {
                            if(MjClient.MaxPlayerNum_YiYangWaiHuZi == 4) {
                                if (IsArrowVisible_YiYangWaiHuZi()) showUserZhuangLogo_YiYangWaiHuZi(this, 1);
                            }
                        },
                        mjhand: function(){
                            if(MjClient.MaxPlayerNum_YiYangWaiHuZi == 4) {
                ...
                chatbg: {
                    ...
                    chattext: {
                        _event: {
                            MJChat: function(msg) {
                                if(MjClient.MaxPlayerNum_YiYangWaiHuZi == 4) {
                                    showUserChat_YiYangWaiHuZi(this, 1, msg);
                                }
                            },
                            playVoice: function(voicePath) {
                                if(MjClient.MaxPlayerNum_YiYangWaiHuZi == 4) {
                                    MjClient.data._tempMessage.msg = voicePath;
                                    showUserChat_YiYangWaiHuZi(this, 1, MjClient.data._tempMessage);
                                }
                            }
                        }
                    }
                ...
                _click: function(btn) {
                    if(MjClient.MaxPlayerNum_YiYangWaiHuZi == 4) {
                        showPlayerInfo_YiYangWaiHuZi(1, btn);
                    }
                },
                _event: {
                    loadWxHead: function(d) {
                        if(MjClient.MaxPlayerNum_YiYangWaiHuZi == 4) {
                            setWxHead_YiYangWaiHuZi(this, d, 1);
                        }
                    },
                    addPlayer: function(eD) {
                        if(MjClient.MaxPlayerNum_YiYangWaiHuZi == 4) {
                            showFangzhuTagIcon_YiYangWaiHuZi(this, 1);
                        }
                    },
                    removePlayer: function(eD) {
                        if(MjClient.MaxPlayerNum_YiYangWaiHuZi == 4) {
                ...
                huxi:{
                    ...
                    _event:{
                        ...
                        initSceneData:function(){
                            if(MjClient.MaxPlayerNum_YiYangWaiHuZi == 4) {
                                // this.visible = true;
                                var huXi = UpdateHuXi_YiYangWaiHuZi(1);
                                if (!IsArrowVisible_YiYangWaiHuZi()) {
                                    huXi = 0;
                                }
                ...
                timeImg:{
                    ...
                    _event:{
                        initSceneData: function(eD) {
                            if(MjClient.MaxPlayerNum_YiYangWaiHuZi == 4) {
                                //this.visible = curPlayerIsMe_YiYangWaiHuZi(1);
                            }
                            // MjClient.playui.refreshHeadLight(true);
                        },
                        onlinePlayer: function(eD) {
                            if(MjClient.MaxPlayerNum_YiYangWaiHuZi == 4) {
                                //this.visible = curPlayerIsMe_YiYangWaiHuZi(1) && IsArrowVisible_YiYangWaiHuZi();
                            }
                        },
                        waitPut: function(eD) {
                            if(MjClient.MaxPlayerNum_YiYangWaiHuZi == 4) {
                        ...
                        _event: {
                            initSceneData: function(eD) {
                                if(MjClient.MaxPlayerNum_YiYangWaiHuZi == 4) {
                                    ...
                                    stopEffect(playTimeUpEff);
                                    playTimeUpEff = null;
                                    if (curPlayerIsMe_YiYangWaiHuZi(1)) {
                                        arrowbkNumberUpdate(this);
                                        this.setString("0");
                                    }
                                }
                            ...
                            onlinePlayer: function(){
                                if(MjClient.MaxPlayerNum_YiYangWaiHuZi == 4) {
                                    ...
                                    stopEffect(playTimeUpEff);
                                    playTimeUpEff = null;
                                    var pl = getUIPlayer_YiYangWaiHuZi(1);
                                    if (pl && pl.onLine && curPlayerIsMe_YiYangWaiHuZi(1)) {
                                        arrowbkNumberUpdate(this);
                                    }
                                }
                            },
            ...
            _event: {
                clearCardUI: function() {
                    if(MjClient.MaxPlayerNum_YiYangWaiHuZi == 4) {
                        clearCardUI_YiYangWaiHuZi(this, 1);
                    }
                },
                clearCardArr: function(){
                    if(MjClient.MaxPlayerNum_YiYangWaiHuZi == 4) {
                ...
                initSceneData: function(eD) {
                    if(MjClient.MaxPlayerNum_YiYangWaiHuZi == 4) {
                        SetUserVisible_YiYangWaiHuZi(this, 1);
                        InitUserHandUI_YiYangWaiHuZi(this, 1);
                        InitUserCoinAndName_YiYangWaiHuZi(this, 1);
                        DealOffLineCard_YiYangWaiHuZi(this, 1);
                    }
                },
                addPlayer: function(eD) {
                    if(MjClient.MaxPlayerNum_YiYangWaiHuZi == 4) {
                ...
                HZNewCard: function(eD) {
                    if(MjClient.MaxPlayerNum_YiYangWaiHuZi == 4) {
                        ...
                        DealNewCard_YiYangWaiHuZi(this, eD, 1);
                    }
                },
                roundEnd: function() {
                    if(MjClient.MaxPlayerNum_YiYangWaiHuZi == 4) {
                ...
                MJPut: function(eD) {
                    if(MjClient.MaxPlayerNum_YiYangWaiHuZi == 4) {
                        DealPutCard_YiYangWaiHuZi(this, eD, 1);
                    }
                },
                HZChiCard: function(eD) {
                    if(MjClient.MaxPlayerNum_YiYangWaiHuZi == 4) {
                        DealChiCard_YiYangWaiHuZi(this, eD, 1);
                    }
                },
                HZLiuCard: function(eD) {
                    if(MjClient.MaxPlayerNum_YiYangWaiHuZi == 4) {
                        DealGangCard_YiYangWaiHuZi(this, eD, 1);
                    }
                },
                MJPeng: function(eD) {
                    if(MjClient.MaxPlayerNum_YiYangWaiHuZi == 4) {
                        DealPengCard_YiYangWaiHuZi(this, eD, 1);
                    }
                },
                HZWeiCard: function(eD){
                    if(MjClient.MaxPlayerNum_YiYangWaiHuZi == 4) {
                        DealWeiCard_YiYangWaiHuZi(this, eD, 1);
                    }
                },
                MJHu: function(eD) {
                    if(MjClient.MaxPlayerNum_YiYangWaiHuZi == 4) {
                        DealHu_YiYangWaiHuZi(this, eD, 1);
                    }
                },
                onlinePlayer: function(eD) {
                    if(MjClient.MaxPlayerNum_YiYangWaiHuZi == 4) {
                        setUserOffline_YiYangWaiHuZi(this, 1);
                    }
                },
                playerStatusChange: function(eD) {
                    if(MjClient.MaxPlayerNum_YiYangWaiHuZi == 4) {
                ...
                HZAddCards:function (eD) {
                    if(MjClient.MaxPlayerNum_YiYangWaiHuZi == 4) {
                        DealAddCard_YiYangWaiHuZi(this, eD, 1);
                    }
                },
                HZCheckRaise:function (msg) {
                    if(MjClient.MaxPlayerNum_YiYangWaiHuZi == 4) {
        ...
        right: {
            _run:function () {
                if(MjClient.MaxPlayerNum_YiYangWaiHuZi == 2){
                    this.visible = false;
                }
            },
            head: {
                siShou:{
                    ...
                    _event:{
                        initSceneData:function () {
                            setSiShou_YiYangWaiHuZi(this, MjClient.MaxPlayerNum_YiYangWaiHuZi - 2);
                        },
                        MJPeng: function(eD) {
                            setSiShou_YiYangWaiHuZi(this, MjClient.MaxPlayerNum_YiYangWaiHuZi - 2);
                        },
                ...
                zhuang: {
                    ...
                    _event: {
                        waitPut: function() {
                            showUserZhuangLogo_YiYangWaiHuZi(this, MjClient.MaxPlayerNum_YiYangWaiHuZi - 2);
                        },
                        initSceneData: function() {
                            if (IsArrowVisible_YiYangWaiHuZi()) showUserZhuangLogo_YiYangWaiHuZi(this, MjClient.MaxPlayerNum_YiYangWaiHuZi - 2);
                        },
                        mjhand: function(){
                            if (IsArrowVisible_YiYangWaiHuZi()) showUserZhuangLogo_YiYangWaiHuZi(this, MjClient.MaxPlayerNum_YiYangWaiHuZi - 2);
                        }
                ...
                chatbg: {
                    ...
                    chattext: {
                        _event: {
                            MJChat: function(msg) {
                                showUserChat_YiYangWaiHuZi(this, MjClient.MaxPlayerNum_YiYangWaiHuZi - 2, msg);
                            },
                            playVoice: function(voicePath) {
                                MjClient.data._tempMessage.msg = voicePath;
                                showUserChat_YiYangWaiHuZi(this, MjClient.MaxPlayerNum_YiYangWaiHuZi - 2, MjClient.data._tempMessage);
                            }
                        }
                    }
                },
                _click: function(btn) {
                    showPlayerInfo_YiYangWaiHuZi(MjClient.MaxPlayerNum_YiYangWaiHuZi - 2, btn);
                },
                _event: {
                    loadWxHead: function(d) {
                        setWxHead_YiYangWaiHuZi(this, d, MjClient.MaxPlayerNum_YiYangWaiHuZi - 2);
                    },
                    addPlayer: function(eD) {
                        showFangzhuTagIcon_YiYangWaiHuZi(this,MjClient.MaxPlayerNum_YiYangWaiHuZi - 2);
                    },
                    removePlayer: function(eD) {
                        showFangzhuTagIcon_YiYangWaiHuZi(this,MjClient.MaxPlayerNum_YiYangWaiHuZi - 2);
                    }
                ...
                huxi:{
                    ...
                    _event:{
                        ...
                        initSceneData:function(){
                            this.visible = true;
                            var huXi = UpdateHuXi_YiYangWaiHuZi(MjClient.MaxPlayerNum_YiYangWaiHuZi - 2);
                            if(!IsArrowVisible_YiYangWaiHuZi()){
                                huXi = 0;
                                this.visible = false;
                            }
                ...
                timeImg:{
                    ...
                    _event:{
                        initSceneData: function(eD) {
                            //this.visible = curPlayerIsMe_YiYangWaiHuZi(MjClient.MaxPlayerNum_YiYangWaiHuZi - 2);
                        },
                        onlinePlayer: function(eD) {
                            //this.visible = curPlayerIsMe_YiYangWaiHuZi(MjClient.MaxPlayerNum_YiYangWaiHuZi - 2) && IsArrowVisible_YiYangWaiHuZi();
                        },
                        waitPut: function(eD) {
                            //this.visible = curPlayerIsMe_YiYangWaiHuZi(MjClient.MaxPlayerNum_YiYangWaiHuZi - 2);
                        },
                    ...
                    time:{
                        ...
                        _event: {
                            initSceneData: function(eD) {
                                ...
                                stopEffect(playTimeUpEff);
                                playTimeUpEff = null;
                                if(curPlayerIsMe_YiYangWaiHuZi(MjClient.MaxPlayerNum_YiYangWaiHuZi - 2)){
                                    arrowbkNumberUpdate(this);
                                    this.setString("0");
                                }
                            },
                            ...
                            onlinePlayer: function(){
                                ...
                                stopEffect(playTimeUpEff);
                                playTimeUpEff = null;
                                var pl = getUIPlayer_YiYangWaiHuZi(MjClient.MaxPlayerNum_YiYangWaiHuZi - 2);
                                if(pl && pl.onLine && curPlayerIsMe_YiYangWaiHuZi(MjClient.MaxPlayerNum_YiYangWaiHuZi - 2)){
                                    arrowbkNumberUpdate(this);
                                }
                            },
                            roundEnd: function() {
            ...
            ready: {
                _layout: [[0.07, 0.07],[0.5, 0.5],[2.5, 2.5]],
                _run: function() {
                    GetReadyVisible_YiYangWaiHuZi(this, MjClient.MaxPlayerNum_YiYangWaiHuZi - 2);
                },
                _event: {
                    startShuffleCards:function () {
                        GetReadyVisible_YiYangWaiHuZi(this, MjClient.MaxPlayerNum_YiYangWaiHuZi - 2);
            ...
            _event: {
                clearCardUI: function() {
                    clearCardUI_YiYangWaiHuZi(this, MjClient.MaxPlayerNum_YiYangWaiHuZi - 2);
                },
                clearCardArr: function(){
                    var eatNode = this.getChildByName("eatNode");
                    eatNode.removeAllChildren();
                ...
                initSceneData: function(eD) {
                    SetUserVisible_YiYangWaiHuZi(this, MjClient.MaxPlayerNum_YiYangWaiHuZi - 2);
                    InitUserHandUI_YiYangWaiHuZi(this, MjClient.MaxPlayerNum_YiYangWaiHuZi - 2);
                    InitUserCoinAndName_YiYangWaiHuZi(this, MjClient.MaxPlayerNum_YiYangWaiHuZi - 2);
                    DealOffLineCard_YiYangWaiHuZi(this,MjClient.MaxPlayerNum_YiYangWaiHuZi - 2);
                },
                addPlayer: function(eD) {
                    SetUserVisible_YiYangWaiHuZi(this, MjClient.MaxPlayerNum_YiYangWaiHuZi - 2);
                    InitUserCoinAndName_YiYangWaiHuZi(this, MjClient.MaxPlayerNum_YiYangWaiHuZi - 2);
                ...
                HZNewCard: function(eD) {
                    ...
                    DealNewCard_YiYangWaiHuZi(this,eD,MjClient.MaxPlayerNum_YiYangWaiHuZi - 2);
                },
                roundEnd: function() {
                    InitUserCoinAndName_YiYangWaiHuZi(this, MjClient.MaxPlayerNum_YiYangWaiHuZi - 2);
                    MjClient.playui.ResetOtherCard(this, MjClient.MaxPlayerNum_YiYangWaiHuZi - 2);
                },
                waitPut: function(eD) {
                    //DealWaitPut(this, eD, MjClient.MaxPlayerNum_YiYangWaiHuZi - 2);
                },
                MJPut: function(eD) {
                    DealPutCard_YiYangWaiHuZi(this, eD, MjClient.MaxPlayerNum_YiYangWaiHuZi - 2);
                },
                HZChiCard: function(eD) {
                    DealChiCard_YiYangWaiHuZi(this, eD, MjClient.MaxPlayerNum_YiYangWaiHuZi - 2);
                },
                HZLiuCard: function(eD) {
                    DealGangCard_YiYangWaiHuZi(this, eD, MjClient.MaxPlayerNum_YiYangWaiHuZi - 2);
                },
                MJPeng: function(eD) {
                    DealPengCard_YiYangWaiHuZi(this, eD, MjClient.MaxPlayerNum_YiYangWaiHuZi - 2);
                },
                HZWeiCard: function(eD){
                    DealWeiCard_YiYangWaiHuZi(this, eD, MjClient.MaxPlayerNum_YiYangWaiHuZi - 2);
                },
                MJHu: function(eD) {
                    DealHu_YiYangWaiHuZi(this, eD, MjClient.MaxPlayerNum_YiYangWaiHuZi - 2);
                },
                onlinePlayer: function(eD) {
                    setUserOffline_YiYangWaiHuZi(this, MjClient.MaxPlayerNum_YiYangWaiHuZi - 2);
                },
                playerStatusChange: function(eD) {
                    setUserOffline_YiYangWaiHuZi(this, MjClient.MaxPlayerNum_YiYangWaiHuZi - 2);
                },
                MJFlower: function(eD) {
                    HandleMJFlower(this, eD, MjClient.MaxPlayerNum_YiYangWaiHuZi - 2);
                },
                MJTing: function (eD) {
                    HandleMJTing(this, eD, MjClient.MaxPlayerNum_YiYangWaiHuZi - 2);
                },
                waitJiazhu:function (msg) {
                    postEvent("returnPlayerLayer");
                    /*
                ...
                HZPickCard:function (eD) {
                    if(MjClient.rePlayVideo != -1){
                        var pl = getUIPlayer_YiYangWaiHuZi(MjClient.MaxPlayerNum_YiYangWaiHuZi - 2);
                        if(eD.uid == pl.info.uid) {
                            MjClient.OtherHandArr[MjClient.MaxPlayerNum_YiYangWaiHuZi - 2] = MjClient.majiang.sortCard3(pl.mjhand, 2)
                        }
                    }

                    RemovePutCardOut_YiYangWaiHuZi(true);
                    MjClient.playui.ResetHandCard(this,MjClient.MaxPlayerNum_YiYangWaiHuZi - 2);
                },
                HZAddCards:function (eD) {
                    DealAddCard_YiYangWaiHuZi(this,eD, MjClient.MaxPlayerNum_YiYangWaiHuZi - 2);
                },
                HZCheckRaise:function (msg) {
                    ShowPutCardIcon_YiYangWaiHuZi();
                    //MjClient.playui.EatVisibleCheck();
        ...
        left: {
            head: {
                siShou:{
                    ...
                    _event:{
                        initSceneData:function () {
                            setSiShou_YiYangWaiHuZi(this, MjClient.MaxPlayerNum_YiYangWaiHuZi - 1);
                        },
                        MJPeng: function(eD) {
                            setSiShou_YiYangWaiHuZi(this, MjClient.MaxPlayerNum_YiYangWaiHuZi - 1);
                        },
                ...
                zhuang: {
                    ...
                    _event: {
                        waitPut: function() {
                            showUserZhuangLogo_YiYangWaiHuZi(this, MjClient.MaxPlayerNum_YiYangWaiHuZi - 1);
                        },
                        initSceneData: function() {
                            if (IsArrowVisible_YiYangWaiHuZi()) showUserZhuangLogo_YiYangWaiHuZi(this, MjClient.MaxPlayerNum_YiYangWaiHuZi - 1);
                        },
                        mjhand: function(){
                            if (IsArrowVisible_YiYangWaiHuZi()) showUserZhuangLogo_YiYangWaiHuZi(this, MjClient.MaxPlayerNum_YiYangWaiHuZi - 1);
                        }
                ...
                chatbg: {
                    ...
                    chattext: {
                        _event: {
                            MJChat: function(msg) {
                                showUserChat_YiYangWaiHuZi(this, MjClient.MaxPlayerNum_YiYangWaiHuZi - 1, msg);
                            },
                            playVoice: function(voicePath) {
                                MjClient.data._tempMessage.msg = voicePath;
                                showUserChat_YiYangWaiHuZi(this, MjClient.MaxPlayerNum_YiYangWaiHuZi - 1, MjClient.data._tempMessage);
                            }
                        }
                    }
                },
                _click: function(btn) {
                    showPlayerInfo_YiYangWaiHuZi(MjClient.MaxPlayerNum_YiYangWaiHuZi - 1, btn);
                },
                _event: {
                    loadWxHead: function(d) {
                        setWxHead_YiYangWaiHuZi(this, d, MjClient.MaxPlayerNum_YiYangWaiHuZi - 1);
                    },
                    addPlayer: function(eD) {
                        showFangzhuTagIcon_YiYangWaiHuZi(this,MjClient.MaxPlayerNum_YiYangWaiHuZi - 1);
                    },
                    removePlayer: function(eD) {
                        showFangzhuTagIcon_YiYangWaiHuZi(this,MjClient.MaxPlayerNum_YiYangWaiHuZi - 1);
                    }
                ...
                huxi:{
                    ...
                    _event:{
                        ...
                        initSceneData:function(){
                            this.visible = true;
                            var huXi = UpdateHuXi_YiYangWaiHuZi(MjClient.MaxPlayerNum_YiYangWaiHuZi - 1);
                            if(!IsArrowVisible_YiYangWaiHuZi()){
                                huXi = 0;
                                this.visible = false;
                            }
                ...
                timeImg:{
                    ...
                    _event:{
                        initSceneData: function(eD) {
                            //this.visible = curPlayerIsMe_YiYangWaiHuZi(MjClient.MaxPlayerNum_YiYangWaiHuZi - 1);
                        },
                        onlinePlayer: function(eD) {
                            //this.visible = curPlayerIsMe_YiYangWaiHuZi(MjClient.MaxPlayerNum_YiYangWaiHuZi - 1) && IsArrowVisible_YiYangWaiHuZi();
                        },
                        waitPut: function(eD) {
                            //this.visible = curPlayerIsMe_YiYangWaiHuZi(MjClient.MaxPlayerNum_YiYangWaiHuZi - 1);
                        },
                    ...
                    time:{
                        ...
                        _event: {
                            initSceneData: function(eD) {
                                ...
                                stopEffect(playTimeUpEff);
                                playTimeUpEff = null;
                                if(curPlayerIsMe_YiYangWaiHuZi(MjClient.MaxPlayerNum_YiYangWaiHuZi - 1)){
                                    arrowbkNumberUpdate(this);
                                    this.setString("0");
                                }
                            },
                            ...
                            onlinePlayer: function(){
                                ...
                                stopEffect(playTimeUpEff);
                                playTimeUpEff = null;
                                var pl = getUIPlayer_YiYangWaiHuZi(MjClient.MaxPlayerNum_YiYangWaiHuZi - 1);
                                if(pl && pl.onLine && curPlayerIsMe_YiYangWaiHuZi(MjClient.MaxPlayerNum_YiYangWaiHuZi - 1)){
                                    arrowbkNumberUpdate(this);
                                }
                            },
                            roundEnd: function() {
            ...
            ready: {
                _layout: [[0.07, 0.07],[0.5, 0.5],[-2.5, 2.5]],
                _run: function() {
                    GetReadyVisible_YiYangWaiHuZi(this, MjClient.MaxPlayerNum_YiYangWaiHuZi - 1);
                },
                _event: {
                    startShuffleCards:function () {
                        GetReadyVisible_YiYangWaiHuZi(this, MjClient.MaxPlayerNum_YiYangWaiHuZi - 1);
            ...
            _event: {
                clearCardUI: function() {
                    clearCardUI_YiYangWaiHuZi(this, MjClient.MaxPlayerNum_YiYangWaiHuZi - 1);
                },
                clearCardArr: function(){
                    var eatNode = this.getChildByName("eatNode");
                    eatNode.removeAllChildren();
                ...
                initSceneData: function(eD) {
                    SetUserVisible_YiYangWaiHuZi(this, MjClient.MaxPlayerNum_YiYangWaiHuZi - 1);
                    InitUserHandUI_YiYangWaiHuZi(this, MjClient.MaxPlayerNum_YiYangWaiHuZi - 1);
                    InitUserCoinAndName_YiYangWaiHuZi(this, MjClient.MaxPlayerNum_YiYangWaiHuZi - 1);
                    DealOffLineCard_YiYangWaiHuZi(this,MjClient.MaxPlayerNum_YiYangWaiHuZi - 1);
                },
                addPlayer: function(eD) {
                    SetUserVisible_YiYangWaiHuZi(this, MjClient.MaxPlayerNum_YiYangWaiHuZi - 1);
                    InitUserCoinAndName_YiYangWaiHuZi(this, MjClient.MaxPlayerNum_YiYangWaiHuZi - 1);
                ...
                HZNewCard: function(eD) {
                    ...
                    DealNewCard_YiYangWaiHuZi(this,eD,MjClient.MaxPlayerNum_YiYangWaiHuZi - 1);
                },
                roundEnd: function() {
                    InitUserCoinAndName_YiYangWaiHuZi(this, MjClient.MaxPlayerNum_YiYangWaiHuZi - 1);
                    MjClient.playui.ResetOtherCard(this, MjClient.MaxPlayerNum_YiYangWaiHuZi - 1);
                },
                waitPut: function(eD) {
                    //DealWaitPut(this, eD, MjClient.MaxPlayerNum_YiYangWaiHuZi - 1);
                },
                MJPut: function(eD) {
                    DealPutCard_YiYangWaiHuZi(this, eD, MjClient.MaxPlayerNum_YiYangWaiHuZi - 1);
                },
                HZChiCard: function(eD) {
                    DealChiCard_YiYangWaiHuZi(this, eD, MjClient.MaxPlayerNum_YiYangWaiHuZi - 1);
                },
                HZPaoCard: function(eD) {
                    DealPaoCard_YiYangWaiHuZi(this, eD, MjClient.MaxPlayerNum_YiYangWaiHuZi - 1);
                },
                HZLiuCard: function(eD) {
                    DealGangCard_YiYangWaiHuZi(this, eD, MjClient.MaxPlayerNum_YiYangWaiHuZi - 1);
                },
                MJPeng: function(eD) {
                    DealPengCard_YiYangWaiHuZi(this, eD, MjClient.MaxPlayerNum_YiYangWaiHuZi - 1);
                },
                HZWeiCard: function(eD){
                    DealWeiCard_YiYangWaiHuZi(this, eD, MjClient.MaxPlayerNum_YiYangWaiHuZi - 1);
                },
                MJHu: function(eD) {
                    DealHu_YiYangWaiHuZi(this, eD, MjClient.MaxPlayerNum_YiYangWaiHuZi - 1);
                },
                onlinePlayer: function(eD) {
                    setUserOffline_YiYangWaiHuZi(this, MjClient.MaxPlayerNum_YiYangWaiHuZi - 1);
                },
                playerStatusChange: function(eD) {
                    setUserOffline_YiYangWaiHuZi(this, MjClient.MaxPlayerNum_YiYangWaiHuZi - 1);
                },
                MJFlower: function(eD) {
                    HandleMJFlower(this, eD, MjClient.MaxPlayerNum_YiYangWaiHuZi - 1);
                },
                MJTing: function (eD) {
                    HandleMJTing(this, eD, MjClient.MaxPlayerNum_YiYangWaiHuZi - 1);
                },
                HZPickCard:function (eD) {
                    if(MjClient.rePlayVideo != -1) {
                        var pl = getUIPlayer_YiYangWaiHuZi(MjClient.MaxPlayerNum_YiYangWaiHuZi - 1);
                        if(eD.uid == pl.info.uid){
                            MjClient.OtherHandArr[MjClient.MaxPlayerNum_YiYangWaiHuZi - 1] = MjClient.majiang.sortCard3(pl.mjhand, 2)
                        }
                    }
                    RemovePutCardOut_YiYangWaiHuZi(true);
                    MjClient.playui.ResetHandCard(this,MjClient.MaxPlayerNum_YiYangWaiHuZi - 1);
                },
                HZAddCards:function (eD) {
                    DealAddCard_YiYangWaiHuZi(this,eD, MjClient.MaxPlayerNum_YiYangWaiHuZi - 1);
                },
                HZCheckRaise:function (msg) {
                    ShowPutCardIcon_YiYangWaiHuZi();
                    //MjClient.playui.EatVisibleCheck();
        ...
        eat: {
                ...
                _touch: function(btn, eT) {
                    if (eT == 2){
                        ...
                        if(ziPai.getHasFastEat()){
                            ...
                            if (chiList.length == 1) {
                                commitEatCards_YiYangWaiHuZi(chiList[0], null);
                                return;
                            }
                        }
                        
                        showSelectEatCards_YiYangWaiHuZi(this,btn.tag);
            ...
            liu:{
                ...
                _touch: function(btn, eT) {
                    ...
                    if (eT == 2){
                        showSelectLiuCards_YiYangWaiHuZi(this, 0);
                    }
                }
            },
            piao:{
                ...
                _touch: function(btn, eT) {
                    ...
                    if (eT == 2){
                        showSelectLiuCards_YiYangWaiHuZi(this,1);
                    }
                }
            },
            guo: {
                    ...
                    if (eT == 2){
                        if ((MjClient.data.sData.players[SelfUid()].eatFlag & 32) > 0) {
                            MjClient.showMsg("确定过胡吗？", function() {
                                HZPassConfirmToServer_YiYangWaiHuZi(); // todo 
                                setChiVisible_YiYangWaiHuZi();//add by maoyu
                            }, function() {}, "1");
                        } else {
                            HZPassConfirmToServer_YiYangWaiHuZi(); // todo 
                            setChiVisible_YiYangWaiHuZi();//add by maoyu
            ...
            wangDiao:{
                ...
                _touch: function(btn, eT) {
                    console.log(">>>> lf，点击碰按钮");
                    if (eT == 2) {
                        HZWangChuangToServer_YiYangWaiHuZi(1);
                        setChiVisible_YiYangWaiHuZi();//add by maoyu
                    }
                }
            },
            wangChuang:{
                ...
                _touch: function(btn, eT) {
                    console.log(">>>> lf，点击碰按钮");
                    if (eT == 2) {
                        HZWangChuangToServer_YiYangWaiHuZi(2);
                        setChiVisible_YiYangWaiHuZi();//add by maoyu
                    }
                }
            },
            wangZha:{
                ...
                _touch: function(btn, eT) {
                    console.log(">>>> lf，点击碰按钮");
                    if (eT == 2) {
                        HZWangChuangToServer_YiYangWaiHuZi(4);
                        setChiVisible_YiYangWaiHuZi();//add by maoyu
                    }
                }
            },
                    ...
                    this.adaptChiLayout = function() {
                        ...
                        if (cards_option.length == 0) { // 吃牌选好了
                            if ((MjClient.data.sData.players[SelfUid()].eatFlag & 32) > 0) {
                                MjClient.showMsg("选择吃后视为过胡，确定吃吗？", function() {
                                    ...
                                    HZChiToServer_YiYangWaiHuZi(cards_select[0], biCards);
                                    this.visible = false;
                                }.bind(this), function() {
                                    cards_select.pop();
                                    this.adaptChiLayout();
                        ...
                        for (var i = 0; i < cards_select.length; i++) {
                            ...
                            for (var j = 0; j < row.length; j++) {
                                ...
                                chiCardImg.loadTexture(MjClient.cardPath_YiYangWaiHuZi + "out" + row[j] + ".png");
        ...
        chat_btn: {
            ...
            _click: function() {
                ...
                // MjClient.Scene.addChild(new EndOneView_YiYangWaiHuZi(),500);
                // MjClient.Scene.addChild(new GameOverLayer_YiYangWaiHuZi(),500);
            }
        },
        voice_btn: {
            ...
            _event: {
                ...
                downAndPlayVoice: function(msg) {
                    ...
                    // downAndPlayVoice_YiYangWaiHuZi(msg.uid, msg.msg);
    ...
    ctor: function() {
        this._super();
        ziPai.initZiPaiData();
        var playui = ccs.load("Play_YiYangWaiHuZi.json");
        playMusic("bgHongZi");
        var tData = MjClient.data.sData.tData;
        MjClient.MaxPlayerNum_YiYangWaiHuZi = tData.maxPlayer;
        cc.log(MjClient.MaxPlayerNum_YiYangWaiHuZi);
        ...
        this.newTingLayer = new YiYangWaiHuZiNewTingLayer();
        playui.node.addChild(this.newTingLayer);

        this.tingLayer = new YiYangWaiHuZiPhzTingLayer();
    ...
    resetJiaZhuTip: function()
    {
        ...
        for (var i = 0; i < MjClient.MaxPlayerNum_YiYangWaiHuZi; i ++)
        {
...
//重置手牌的顺序
PlayLayer_YiYangWaiHuZi.prototype.ResetHandCard = function(posNode,off, needAction, isDelay){
    ...
    if(MjClient.rePlayVideo == -1){
        if(off == 0) {
            if(!needAction && !isDelay && MjClient.addGroupIndex != 0){
                refreshNode_YiYangWaiHuZi(posNode,off,needAction);
            ...
            checkCard_YiYangWaiHuZi(posNode, off);
            ...
            for (var k = 0; k < cardArr.length; k++) {
                ...
                for (var j = 0; j < groupList.length; j++) {
                    addHandCard_YiYangWaiHuZi(k, j, groupList[j], off);
                }
            }

            addTingSign_YiYangWaiHuZi(posNode); // 添加听牌角标
        ...
        //if(MjClient.movingCard_YiYangWaiHuZi && MjClient.movingCard_YiYangWaiHuZi.getParent()){
        //    MjClient.movingCard_YiYangWaiHuZi.removeFromParent(true);
            ...
            for(var j = 0;j < groupList.length;j++){
                if(off == 0){
                    if(MjClient.MaxPlayerNum_YiYangWaiHuZi == 3 || MjClient.MaxPlayerNum_YiYangWaiHuZi == 2){
                        ...
                        addHandCardReplay_YiYangWaiHuZi(k,j,groupList[j],off);
                    }
                }else {
                    addHandCardReplay_YiYangWaiHuZi(k,j,groupList[j],off);
                }
        ...
        if(off == 0 && (MjClient.MaxPlayerNum_YiYangWaiHuZi == 3 || MjClient.MaxPlayerNum_YiYangWaiHuZi == 2)) {
            var type = ziPai.getZiPaiType();
            var sizeArr = [98,95,100,103];
            var width =sizeArr[type] * changeCardSize; // handCard.getVirtualRendererSize().width;
...
PlayLayer_YiYangWaiHuZi.prototype._doMovetoAction = function(node,endP){
    node.stopAllActions();
    var ac = cc.moveTo(0.3,endP);
    node.runAction(ac);
};

PlayLayer_YiYangWaiHuZi.prototype.ResetOtherCard = function(posNode, off) {
    // 添加吃、碰、跑、偎、提
    var sData = MjClient.data.sData;
    var tData = null;
    if (sData) {
        ...
        // 提/偎牌的特殊处理
         var callback = function(childList) {
             ...
             for (var i = 0; i < childList.length; i++) {
                 ...
                 // } else {
                 //     child.loadTexture(MjClient.cardPath_YiYangWaiHuZi + "huxiBG.png");
                 // }

                 var chiTip = childList.length == 3 ? "wei" : "ti";
                 if (i != getShowCardIdx_YiYangWaiHuZi(posNode, chiTip)) {
                     child.loadTexture(MjClient.cardPath_YiYangWaiHuZi + "huxiBG.png");
                 }
        ...
        for (var k = 0; k < mjSortArr.length; k++) {
            ...
            //提
             if (name == "mjgang1") {
                 ...
                 for (var i = 0; i < 4; i++) {
                     addEatCard_YiYangWaiHuZi(posNode, "mjgang1" + k, cardNum, off);
                 }
            ...
            if (name == "mjgang2") {
                ...
                for (var i = 0; i < 4; i++) {
                    addEatCard_YiYangWaiHuZi(posNode, "mjgang2" + k, cardNum, off);
                }
            ...
            //偎
             if (name == "mjwei") {
                 var cardNum = pl.mjwei[pos];
                 for (var i = 0; i < 3; i++) {
                     addEatCard_YiYangWaiHuZi(posNode, "mjwei" + k, cardNum, off);
                 }
            ...
            if (name == "mjgang0") {
                ...
                for (var i = 0; i < 4; i++) {
                    addEatCard_YiYangWaiHuZi(posNode, "mjgang0" + k, cardNum, off);
                }
            ...
            if (name == "mjpeng") {
                ...
                for (var i = 0; i < 3; i++) {
                    addEatCard_YiYangWaiHuZi(posNode, "mjpeng" + k, cardNum, off);
                }
            }
            //吃
            if (name == "mjchi") {
                ...
                for (var i = 0; i < eatCards.length; i++) {
                    addEatCard_YiYangWaiHuZi(posNode, "mjchi" + k, eatCards[i], off);
                }
                ...
                if (biCards && biCards.length > 0) {
                    for (var i = 0; i < biCards.length; i++) {
                        ...
                        for (var m = 0; m < biArr.length; m++) {
                            addEatCard_YiYangWaiHuZi(posNode, "mjbi" + k + i, biArr[m], off);
                        }
                    }
                }
    ...
    //    var outCard = getNewCard_YiYangWaiHuZi(pl.mjput[i], 2, off);
    //    if(off == 0){
    //        outCard.anchorX = 1;
    //        outCard.anchorY = 0;
...
PlayLayer_YiYangWaiHuZi.prototype.ResetPutCard = function(posNode,off){
    ...
    if(type == 0 && MjClient.data.sData.tData.maxPlayer < 4){
        ...
        for(var i = 0; i < pl.mjput.length; i++){
            ...
            outNode.addChild(outCard);
            addPutFrame_YiYangWaiHuZi(outCard, pl.mjputType[i]);
        }
    }else{
        for(var i = 0; i < pl.mjput.length; i++){
            if(curUId == pl.info.uid && i == pl.mjput.length - 1 && tData.currCard == pl.mjput[i]){
...
//重置牌的顺序
PlayLayer_YiYangWaiHuZi.prototype.CardLayoutRestore = function(posNode,off){
    if(MjClient.rePlayVideo == -1){
...
PlayLayer_YiYangWaiHuZi.prototype.ShowHuTypeText = function(){
    var pl = getUIPlayer_YiYangWaiHuZi(0);
    cc.log("test . log ----------------------");   
...
// 判断吃、碰、跑、偎、提、胡的状态
PlayLayer_YiYangWaiHuZi.prototype.EatVisibleCheck = function(off){
    var eat = MjClient.playui.jsBind.eat;
...
PlayLayer_YiYangWaiHuZi.prototype.showAndHideHeadEffect = function(){
    ...
    else if (curPlayerIsMe_YiYangWaiHuZi(1)){
        curPlayerNode = arr[1].getChildByName("head");
    }else if (curPlayerIsMe_YiYangWaiHuZi(2)){
        curPlayerNode = arr[2].getChildByName("head");
    }else if (curPlayerIsMe_YiYangWaiHuZi(3)){
