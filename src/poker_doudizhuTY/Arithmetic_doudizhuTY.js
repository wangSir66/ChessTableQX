...
(function () {
    ...
    DoudizhuTY.prototype.CARDTPYE = CARDTPYE;
    var CARDCOUNT = {};
    ...
    DoudizhuTY.prototype.randomCards = function () {
        var cards = [];
    ...
    DoudizhuTY.prototype.calPoint = calPoint;

    DoudizhuTY.prototype.isShun = function (oCards) { //oCards有序
        if (!oCards) {
            return false;
        }
    ...
    DoudizhuTY.prototype.isLiandui = function (oCards) { //oCards有序
        if (!oCards) {
            return false;
        }
        var cardCount = oCards.length;
    ...
    DoudizhuTY.prototype.isSanshun = function (cards) { //cards有序
        if (!cards) {
            return false;
        }
    ...
    DoudizhuTY.prototype.isFeiji2 = function (oCards) { //cards有序
        if (!oCards) {
            return false;
        }
    ...
    DoudizhuTY.prototype.isFeiji = function (oCards) { //cards有序
        if (!oCards) {
            return false;
        }
        var cards = oCards.slice();
    ...
    // 计算牌型
    DoudizhuTY.prototype.calType = function (cards) {
        var cardCount = cards.length;
    ...
    DoudizhuTY.prototype.calCardsValue = function (cards, type) {
    ...
    // 牌点比较函数
    DoudizhuTY.prototype.cardValueCmp = function (a, b) {
        var pa = calPoint(a);
        var pb = calPoint(b);
        if (pa == pb) {
            return a - b;
    ...
    // 牌是否能压上
    DoudizhuTY.prototype.canPut = function (oCards, oLastCards) {
        var cards = oCards.slice();
    ...
    // 检查是否能出牌
    DoudizhuTY.prototype.checkPut = function (oHands, oCards, lastCards) {
    ...
    DoudizhuTY.prototype.checkPutByOneTimes = function (oHands) {
    ...
    // 对手牌排序，普通牌型和同花顺优先排序
    DoudizhuTY.prototype.sortHandCards = function (oCards, sortType) {
        var cards = oCards.slice();
    ...
    // 找N张点数一样的牌
    DoudizhuTY.prototype.findNSameCard = function (hands, point, n) {
    ...
    DoudizhuTY.prototype.filterSort = function (hands, cardsArray, type) {
    ...
    // 找出type牌型的牌
    DoudizhuTY.prototype.findCardByType = function (hands, type, cardCount) {
        var rets = [];
...
// 是否可以包含炸弹
    DoudizhuTY.prototype.isCanContainZhadan = function (type) {
...
// 不拆炸弹
    DoudizhuTY.prototype.dismantleZhadan = function (hands, cardsArray, type) {
...
// 求解提示出牌
    DoudizhuTY.prototype.tipCards = function (oHands, oLastCards) {
        var hands = oHands.slice();
    ...
    // 求出牌型
    DoudizhuTY.prototype.cardsType = function (oCards) {
        if (oCards) {
            var cards = oCards.slice();
            cards.sort(this.cardValueCmp);
...
// 是否是三张系列
    DoudizhuTY.prototype.is3ZhangType = function (cType) {
...
// 例如 红桃8梅花8方块8 出 88, tipCardsArray 仅有 方块8和梅花8，出红桃8梅花8
    DoudizhuTY.prototype.findSelectCardForAutoUp = function (tipCards, selectValue) {
...
// 更换牌值
    DoudizhuTY.prototype.changeSelectCardForAuto = function (tipCards, idx, selectValue) {
        tipCards[idx] = selectValue;
        return tipCards;
    }

// 有足够的三张牌组
    DoudizhuTY.prototype.isEnough3SameCards = function (upSelectCards, groupNum) {
        var start = false;
        var tmpCards = [];
...
// 不足3张的牌组，是否能在手牌找到合法的3张
    DoudizhuTY.prototype.findEnough3SameCard = function (oHands, upSelectCards, groupNum, zhadanMap) {
        var tmpSelect = [];
        var lastPoint = -1;
        for (var i = 0; i < upSelectCards.length; i++) {
...
// 找三张配合的牌
    DoudizhuTY.prototype.findDaiCardsWithSanZhang = function (oHands, upSelectCards, cType, groupNum, zhadanMap) {
...
// 自动提牌-划动-三张系列
    DoudizhuTY.prototype.handleAutoByTouchMoveSanZhang = function (upSelectCards, cType, oLastCards, oHands) {
        var groupNum = 1;
    ...
    DoudizhuTY.prototype.findTheRightTouchMoveCards = function (upSelectCards, cType, oLastCards) {
    ...
    DoudizhuTY.prototype.findZhadanTouchMoveCards = function (upSelectCards) {
        var tCards = [];
        var lastPoint = -1;
...
// 自动提牌-划牌
    DoudizhuTY.prototype.handleAutoByTouchMove = function (bTouchMove, upSelectCards, cType, tipCardsArray, oLastCards, oHands) {
...
// 自动提牌-点牌
    DoudizhuTY.prototype.handleAutoByFirstCard = function (tipCardsArray, upSelectCard, cType) {
...
// 计算斗地主划牌结果
    DoudizhuTY.prototype.calAutoPutCards = function (bTouchMove, tipCardsArray, upSelectCards, oLastCards, oHands) {
    ...
    DoudizhuTY.prototype.getFindCardForFirstPut = function (cards, cType, count) {
        var realCards = null;
        var rateType = {};
...
// 计算斗地主，首出牌划牌的结果(首出提示：顺子 > 飞机2 > 飞机 > 连对)
    DoudizhuTY.prototype.calAutoPutCardWithFirstOut = function (upSelectCards, hands) {
        var data = {hasCardToUp: false};
...
// 计算选择的牌,排除炸弹
    DoudizhuTY.prototype.calSelectWithoutZhadan = function (upSelectCards, zhadanMap) {
...
// 计算牌组的炸弹
    DoudizhuTY.prototype.getZhaDanMapByCards = function (oHands) {
        var hands = oHands.slice();
        hands.sort(this.cardValueCmp);
...
// 计算真实的点
    DoudizhuTY.prototype.calRealPoint = function (num) {
        if (!num) {
            return -1;
        }
        var ceilNum = Math.ceil(num / 4);
    ...
    DoudizhuTY.prototype.findMinPointAndMinZhaDanByHands = function (hands) {
    ...
    // 找最小牌的顺子/连对
    DoudizhuTY.prototype.findCardsByMinPointAndType = function (cardsMap, len, minPoint, cType) {
    ...
    DoudizhuTY.prototype.findSanZhangByMinPoint = function (cardsMap, len, minPoint) {
    ...
    // 找最小牌的对子
    DoudizhuTY.prototype.findDuiZiByMinPoint = function (cardsMap, len, minPoint) {
    ...
    DoudizhuTY.prototype.changeCardsToMap = function (hands, isExcludeZhaDan) {
        var cardsMap = {}
        for (var i = 0; i < hands.length; i++) {
    ...
    // 首出逻辑
    DoudizhuTY.prototype.getAutoPutCardWithFirstOut = function (hands) {
        // 1. 从小到大出牌
    ...
    if (typeof (module) != "undefined" && module.exports)
        module.exports = DoudizhuTY;
    if (typeof (MjClient) != "undefined")
        MjClient.doudizhu_tongyong = new DoudizhuTY();
})();