...
function playScoreEffect_BaZhaDan(txtNode){
    if(txtNode){
...
function playSoundEffect_BaZhaDan(cards, uid, isLocal){
    ...
    if(uid == SelfUid()){
        type = MjClient.majiang.getCardType(cards, tData.shunType, tData.hasWings);
    }
    var cardVal = cards[0] % 100; 

    var url = "datongzi/nv/";
    switch (type){
        case baZhaDan.CARD_TYPE.danWang:
        case baZhaDan.CARD_TYPE.danZhang:
            url += cardVal;
            playEffect(url, false, sex);
            break;
        case baZhaDan.CARD_TYPE.duiZi:
            url += cardVal + "d";
            playEffect(url, false, sex);
            break;
        case baZhaDan.CARD_TYPE.lianDui:
            url += "double_line";
            playEffect(url, false, sex);
            break;
        case baZhaDan.CARD_TYPE.sanZhang:
            var obj = MjClient.majiang.analyzeSanZhang(cards);
            cardVal = obj.body;
            url += cardVal + "t";
            playEffect(url, false, sex);
            break;
        case baZhaDan.CARD_TYPE.feiJi:
            url += "wing";
            playEffect(url, false, sex);
            break;
        case baZhaDan.CARD_TYPE.zhaDan:
            
            if(cards.length > 10 || cards.length < 4){
                url += "zhaDan_ty";
            ...
            playEffect(url, false, sex);
            break;
        case baZhaDan.CARD_TYPE.zaHuaWuShiK:
            url += "f510k";
            playEffect(url, false, sex);
            break;
        case baZhaDan.CARD_TYPE.tongHuaWuShiK:
            url += "z510k";
            playEffect(url, false, sex);
            break;
        case baZhaDan.CARD_TYPE.shunZi:
            url += "shunZi";
            playEffect(url, false, sex);
            break;
    }
...
//显示玩家信息
function showPlayerInfo_BaZhaDan(off, node)
{
    //var tData = MjClient.data.sData.tData;
...
//剩余牌的张数
function currentLeftCardCount_BaZhaDan(UIoff)
{
    var tData = MjClient.data.sData.tData;
    var isShowLeft = tData.areaSelectMode.isShowLeft;
    cc.log("currentLeftCardCount_BaZhaDan");
    ...
    if (pl.handCount == 1)
    {
        ...
        _Text_count.setString(pl.handCount);
        _sprite_single.visible = false;
        // var ac = getAnimate_BaZhaDan("baodan_", 5);
...
function getAnimate_BaZhaDan(preName, len, delaytime, playCount){
...
//重置四个玩家的位置
function resetPlayerHead_BaZhaDan()
{
    cc.log("重置头像信息位置");
    ...
    reConectHeadLayout_BaZhaDan(node.parent);
}

//设置玩家掉线头像
function setUserOffline_BaZhaDan(node, off)
{
    var pl = getUIPlayer_BaZhaDan(off);
    if (!pl)
    {
...
function tableStartHeadMoveAction_BaZhaDan(node)
{
...
//重置4家头像位置
function reConectHeadLayout_BaZhaDan(node)
{
    var tData = MjClient.data.sData.tData;
    var down = node.getChildByName("down").getChildByName("head");
    var top = node.getChildByName("top").getChildByName("head");
...
//出牌回调
function DealWaitPut_BaZhaDan(node, msg, off)
{
    var pl = getUIPlayer_BaZhaDan(off);
    var tData = MjClient.data.sData.tData;
...
// 清理ui
function clearCardUI_BaZhaDan(node)
{
    mylog("clearCardUI");
    if(node.cardList){
...
function initCardList_BaZhaDan(){
    ...
    if(!MjClient.playui._downNode.throwList && !cc.sys.isObjectValid(MjClient.playui._downNode.throwList)){
        var throwList = new baZhaDan.CardThrowListLayer();
    ...
    if(!MjClient.playui._downNode.cardList && !cc.sys.isObjectValid(MjClient.playui._downNode.cardList)){
        var cardList = new baZhaDan.CardListLayer();
        MjClient.playui._downNode.addChild(cardList);
        setWgtLayout(cardList, [1568/1280 * ratio * 0.75, 0], [0.10, 0.1], [0, 0]);
        if(isIPhoneX()){
            setWgtLayout(cardList, [1568/1280 * ratio * 0.75, 0], [0.1, 0.1], [0, 0]);
        }
...
//过的回调
function DealPKPass_BaZhaDan(off)
{
...
/*
    每次更新选牌显示
 */
function UpdataCurrentPutCard_BaZhaDan()
 {
...
function reConnectShowDeskCard_BaZhaDan()//为 down  top, left ,right 节点
{
    var sData = MjClient.data.sData;
    var tData = sData.tData;
    var uids = tData.uids;
    ...
    {
        //cc.log("============UIOFF = " + UIOff);
        showCurrentPutCards_BaZhaDan(UIOff, uids[off]);
    }
...
function getNewCard_BaZhaDan(node, copy, name, tag)
{
...
//处理出牌,放一张牌，打牌动作
function DealMJPut_BaZhaDan(node, msg, UIOff, outNum)
{
    ...
    if(uids[selfIndex] == msg.uid)
    {
        ...
        showCurrentPutCards_BaZhaDan(UIOff, msg.uid);

        var pl = getUIPlayer_BaZhaDan(UIOff);
...
function showCurrentPutCards_BaZhaDan(UIoff, uid)
{
...
//设置牌的渲染
function setCardSprite_BaZhaDan(node, cd, isOnDesk)
{
...
//设置牌的渲染
function setCardSprite_BaZhaDan2(node, cd, isOnDesk)
{
...
//初始化玩家金币和名字
function InitUserCoinAndName_BaZhaDan(node, off) {
    var pl = getUIPlayer_BaZhaDan(off);
    if (!pl) {
        return;
    }
...
function loaderHeadImage_BaZhaDan(node, pl){
...
function showFangzhuTagIcon_BaZhaDan(node,off){
    var tData = MjClient.data.sData.tData;
