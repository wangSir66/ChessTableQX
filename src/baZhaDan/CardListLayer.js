...
baZhaDan.CardThrowListLayer = cc.Node.extend({
    /**
     * @type {ui.LayoutNode}
     */
    _layerNode : null,
...
baZhaDan.CardListLayer = cc.Node.extend({
                ...
                if(desc.length > 0){
                    ...
                    MjClient.showMsg(msg,
                        function() {
                            ...
                            if(type == baZhaDan.CARD_TYPE.noType){
    ...
    handSelfPutCards : function(cards){
        ...
        postEvent("PKPut_BaZhaDan",{cards:cards, sex: MjClient.data.pinfo.sex});
        playEffect("datongzi/effect/cardOut", false);
        playSoundEffect_BaZhaDan(cards, SelfUid(), true);
    },
            ...
            onTouchBegan: function (touch, event) {
                ...
                if(MjClient.playui._handMode == 1){
                    ...
                    if(index >= 0 && self._movingCard == null){
                        ...
                        cClone = new baZhaDan.Card(c.getInfo());
                        cClone.sourceCard = c;
                        c.parent.addChild(cClone);
                        cClone.y = c.y;
                        cClone.x = c.x;
                        c.setOpacity(125);
                        self._movingCard = cClone;
                    }else{
            ...
            onTouchEnded: function (touch, event) {
                ...
                if(MjClient.playui._handMode == 1){
                    if(cClone){
                        ...
                        if(arr){
                            if(oldIndex != index && arr.length < 8){
                                //删除 原来的
    ...
    _checkBtnStatus : function(){
        if(this._btnNode.visible){
            ...
            if(tData.lastPutCard && tData.lastPutCard.length > 0 && tData.lastPutCardType >= 0){
                var flg = MjClient.majiang.isBigger(typeList, tData.lastPutCard, tData.lastPutCardType, tData.shunType, tData.hasWings, tData.bodyLen);
    ...
    addCards2 : function(list, isShow){
        ...
        for(var key in obj){
            ...
            for(var i = 0; i < arr.length; i++){
                var card = new baZhaDan.Card({type:arr[i]});
                // card.setAnchorPoint(0,0);
                ...
                {
                    card.setOpacity(0);
                    dt = this._cardsList.length * 0.05;
                    card.runAction(cc.sequence(cc.delayTime(dt),cc.fadeIn(0.3))); 
                }
    ...
    refreshAllCards : function(dt){
        ...
        for(var i = 0; i < this.cardsMatrix.length; i++){
            ...
            for(var j = 0; j < list.length; j++){
                var card = list[j];
                if(card && cc.sys.isObjectValid(card)){
                    card.y = j * card.rowGap;
                    card.row = j;
                    card.x = i * this._gap + card.width * 0.5;
