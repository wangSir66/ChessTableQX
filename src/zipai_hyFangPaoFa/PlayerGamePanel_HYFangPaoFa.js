...
function SetUserVisible_HYFangPaoFa(node, off){
    ...
    if(pl){
        ...
        // InitUserHandUI_HYFangPaoFa(node, off);
...
function InitUserHandUI_HYFangPaoFa(node, off){
...
var PlayLayer_HYFangPaoFa = cc.Layer.extend({
    jsBind: {
        _event: {
            ...
            roundEnd: function() {
                ...
                function delayExe(){
                    ...
                    if(!MjClient.endoneui){
                        self.addChild(new EndOneView_hyZiPai(),500);//EndOneView_xxghz()
                    }
                }
        ...
        banner: {
            ...
            gps_btn: {
                // _layout: [[80/1280, 0],[0.97, 0.4025],[0, 0]],
                _run:function () {
                    if(MjClient.MaxPlayerNum_xiangxiang == 2){
                        this.visible = false;
                    }
        ...
        down: {
            ...
            eatNode: {
                ...
                //     if(layoutType == 0)
                //     {
                //         setWgtLayout(this, [0.14, 0.14], [0.396, 0.452], [0 , 0]);
                //     }
                //     else if(layoutType == 1)
                //     {
                //         setWgtLayout(this, [0.14, 0.14], [0.02, 0.23], [0 , 0]);
                //     }
                // },
                _visible: true,
            ...
            _event: {
                ...
                initSceneData: function(eD) {
                    SetUserVisible_HYFangPaoFa(this, 0);
                    InitUserHandUI_HYFangPaoFa(this, 0);
        ...
        right: {
            ...
            _event: {
                ...
                initSceneData: function(eD) {
                    SetUserVisible_HYFangPaoFa(this, 1);
                    InitUserHandUI_HYFangPaoFa(this, 1);
        ...
        left: {
            ...
            head: {
                zhuang: {
                    ...
                    _event: {
                        waitPut: function() {
                            if(MjClient.MaxPlayerNum_xiangxiang != 2){
                                showUserZhuangLogo_xiangxiang(this, 2);
                            }
                        },
                        initSceneData: function() {
                            if (IsArrowVisible_xiangxiang() && MjClient.MaxPlayerNum_xiangxiang != 2) showUserZhuangLogo_xiangxiang(this, 2);
                        },
                        mjhand: function(){
                            if (IsArrowVisible_xiangxiang() && MjClient.MaxPlayerNum_xiangxiang != 2) showUserZhuangLogo_xiangxiang(this, 2);
                        }
                ...
                _event: {
                    ...
                    addPlayer: function(eD) {
                        if(MjClient.MaxPlayerNum_xiangxiang != 2){
                            showFangzhuTagIcon_xiangxiang(this,2);
                        }
                    },
                    removePlayer: function(eD) {
                        if(MjClient.MaxPlayerNum_xiangxiang != 2){
            ...
            _event: {
                ...
                initSceneData: function(eD) {
                    ...
                    SetUserVisible_HYFangPaoFa(this, 2);
                    InitUserHandUI_HYFangPaoFa(this, 2);
                ...
                MJPut: function(eD) {
                    ...
                    MjClient.playui.UpdateEatCardsAfterFirst(this,2);
                },
                HZChiCard: function(eD) {
                ...
                HZAddCards:function (eD) {
                    ...
                    DealAddCard_xiangxiang(this,eD, 2);
                },
                HZCheckRaise:function (msg) {
                    if(MjClient.MaxPlayerNum_xiangxiang == 2){
                        return;
    ...
    ctor: function() {
        this._super();
        var playui = ccs.load("Play_HYFangPaoFa.json");
        playMusic("bgFight");
        var tData = MjClient.data.sData.tData;
...
//庄家打出第一张牌之后，刷新提垅的牌
PlayLayer_HYFangPaoFa.prototype.UpdateEatCardsAfterFirst = function(posNode,off){
    var sData = MjClient.data.sData;
    var tData = MjClient.data.sData.tData;
...
//重置手牌的顺序
PlayLayer_HYFangPaoFa.prototype.ResetHandCard = function(posNode,off, needAction, isDelay){
...
PlayLayer_HYFangPaoFa.prototype._doMovetoAction = function(node,endP){
    node.stopAllActions();
    var ac = cc.moveTo(0.3,endP);
    node.runAction(ac);
};

PlayLayer_HYFangPaoFa.prototype.ResetOtherCard = function(posNode,off){
    //添加吃、碰、跑、偎、提
    var sData = MjClient.data.sData;
    var tData = null;
    if(sData){
...
PlayLayer_HYFangPaoFa.prototype.ResetPutCard = function(posNode,off){
    //添加打出的牌
    var pl = getUIPlayer_xiangxiang(off);
...
//重置牌的顺序
PlayLayer_HYFangPaoFa.prototype.CardLayoutRestore = function(posNode,off){
    if(MjClient.rePlayVideo == -1){
...
// 判断吃、碰、跑、偎、提、胡的状态
PlayLayer_HYFangPaoFa.prototype.EatVisibleCheck = function(off){
    var eat = MjClient.playui.jsBind.eat;
    ...
    if(vnode.length > 0){
        ...
        for(var i = 0; i < vnode.length; i++){
            ...
            //     setWgtLayout(vnode[i], [0, 0.23], [0.5, 0.18], [ (i - (vnode.length - 1) / 2) * 1.3, 1.56], false, false); 
            // }

            if(ziPai.getUiLayoutType() == 0){
                setWgtLayout(vnode[i], [0, 0.20], [0.88 - (vnode.length - 1 - i) * 0.12, 0.11], [ 0, 1.8], false, false);
                // if(vnode[i].getName() == "hu"){ 
                //     setWgtLayout(vnode[i], [0, 0.23], [0.88 - (vnode.length - 1 - i) * 0.12, 0.11], [ 0, 1.56], false, false);  
                // }
            }
        }
    }
...
PlayLayer_HYFangPaoFa.prototype.showAndHideHeadEffect = function(){
