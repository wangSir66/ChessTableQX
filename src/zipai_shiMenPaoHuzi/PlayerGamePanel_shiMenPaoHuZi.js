...
function SetUserVisible_shiMenPaoHuZi(node, off){
    ...
    if(pl){
        ...
        setUserOffline_shiMenPaoHuZi(node, off);
...
function InitUserHandUI_shiMenPaoHuZi(node, off){
    ...
    if (pl.jiazhuNum == 2 && MjClient.rePlayVideo == -1)
    {
        ...
        MjClient.majiang.setJiaZhuNum(node, getUIPlayer_shiMenPaoHuZi(off));
    }
...
var PlayLayer_shiMenPaoHuZi = cc.Layer.extend({
    jsBind: {
        _event: {
            ...
            endRoom: function(msg) {
                ...
                if (msg.showEnd){
                    this.addChild(new GameOverLayer_shiMenPaoHuZi(),500);
                }else{
            ...
            roundEnd: function() {
                ...
                if(!MjClient.isDismiss && tData.hunCard && tData.hunCard != -1 && tData.areaSelectMode.xingType==2){
                    ...
                    ShowEatActionAnim_shiMenPaoHuZi(MjClient.playui._downNode, ActionType_paohuzi.FANXING, 0);
                    DealFanXingNewCard_shiMenPaoHuZi(hunCard);
                }
                ...
                function delayExe(){
                    ...
                    if (sData.tData.roundNum <= 0){
                        var layer = new GameOverLayer_shiMenPaoHuZi();
                        layer.setVisible(false);
                        self.addChild(layer,500);
                    }
                    if(!MjClient.endoneui){
                        self.addChild(new EndOneView_shiMenPaoHuZi(),500);
                    }
            ...
            moveHead: function() {
                ...
                //tableStartHeadMoveAction_shiMenPaoHuZi(this);   //不涉及到头像移动动作
            },
            initSceneData: function() {
                ...
                reConectHeadLayout_shiMenPaoHuZi(this);        //不涉及到头像移动动作
                CheckRoomUiDelete();
                tableStartHeadMoveAction_shiMenPaoHuZi(this);   //不涉及到头像移动动作
            ...
            changeMJBgEvent: function() {
                changeMJBg_shiMenPaoHuZi(this, ziPai.getZiPaiType());
            },
            MJPeng: function() {
                MjClient.playui.showAndHideHeadEffect();
            },
            ...
            EZP_layout : function(){
                if(MjClient.data.sData.tData.maxPlayer < 4){
                    ...
                    if(IsArrowVisible_shiMenPaoHuZi()){
                        MjClient.playui.ResetPutCard(MjClient.playui._downNode, 0);
                        MjClient.playui.ResetPutCard(MjClient.playui._topNode, MjClient.MaxPlayerNum_shiMenPaoHuZi - 1);
                        MjClient.playui.ResetPutCard(MjClient.playui._rightNode, MjClient.MaxPlayerNum_shiMenPaoHuZi - 2);
                    }
                }
            }
        },
        cardNumImg: {
            ...
            _event: {
                initSceneData: function(eD) {
                    var sData = MjClient.data.sData;
                    var tData = sData.tData;
                    this.visible = IsArrowVisible_shiMenPaoHuZi() && TableState.waitJiazhu != tData.tState;
                },
                mjhand: function(eD) {
                    var sData = MjClient.data.sData;
                    var tData = sData.tData;
        ...
        roundInfo:{
            ...
            _run:function(){
                if(MjClient.getAppType() == MjClient.APP_TYPE.YLHUNANMJ || MjClient.getAppType() == MjClient.APP_TYPE.QXYYQP) {
                    ...
                    if (item = tData.areaSelectMode.diFen + "分"){
                        infos.push(item);
                    }
                    ...
                    //全名堂
                    var allPlayTypeList = {0:"天胡", 1:"地胡", 2:"海胡", 3:"听胡", 4:"红胡", 5:"黑胡", 6:"点胡", 7:"大胡", 8:"小胡",
                        9:"对子胡", 10:"耍候", 11:"黄番", 12:"团胡", 13:"行行息", 14:"假行行", 15:"三提五坎"};
                    var allPlayList = tData.areaSelectMode.allPlayList;
                    for(var i = 0; i < allPlayList.length; i++)
                    {
                        item = allPlayTypeList[allPlayList[i]];
                        if(tData.areaSelectMode.playType==0){
                            infos.push(item);
                        }
                    }
                    //大六八番
                    var daLiuBaFantype = {0:"大六八番", 1:"小六八番"};
                    var daLiuBaFantypeValue = tData.areaSelectMode.daLiuBaFantype;
                    item = daLiuBaFantype[daLiuBaFantypeValue];
                    if(tData.areaSelectMode.playType==0)
                    {
                        //全名堂
                        infos.push(item);
                    ...
                    //土炮胡
                    var tuPlayTypeList = {0:"红胡", 1:"黑胡", 2:"点胡", 3:"红乌", 4:"对子胡"};
                    var tuPlayList = tData.areaSelectMode.tuPlayList;
                    for(var i = 0; i < tuPlayList.length; i++)
                    {
                        item = tuPlayTypeList[tuPlayList[i]];
        ...
        banner: {
            ...
            getRoomNum: {
                ...
                _click: function() {
                    ...
                    var sData = MjClient.data.sData;
                    var str7 = " ";
                    if (Object.keys(sData.players).length > 0 && MjClient.MaxPlayerNum_shiMenPaoHuZi > Object.keys(sData.players).length){
                        str7 += ["一","二","三"][Object.keys(sData.players).length - 1] + "缺" + ["一","二","三"][MjClient.MaxPlayerNum_shiMenPaoHuZi - Object.keys(sData.players).length - 1];
                    }
            ...
            roundnumImg: {
                ...
                _event: {
                    initSceneData: function(eD) {
                        this.visible = IsArrowVisible_shiMenPaoHuZi();
                    },
                    mjhand: function(eD) {
                        this.visible = IsArrowVisible_shiMenPaoHuZi();
                    },
        ...
        wait: {
            ...
            backHomebtn: {
                ...
                _event: {
                    ...
                    initSceneData: function(eD) {
                        this.visible = isShowBackBtn_shiMenPaoHuZi();
                    },
                    removePlayer: function(eD) {
                        this.visible = isShowBackBtn_shiMenPaoHuZi();
                    },
        ...
        btns_piao:{
            ...
            btn_piao0: {
                _click:function() {
                    var pl = getUIPlayer_shiMenPaoHuZi(0);
                    pl.piaoFen = -1;
                    paioFenToServer_shiMenPaoHuZi(0);
                },
            },
            btn_piao1: {
                _click:function() {
                    var pl = getUIPlayer_shiMenPaoHuZi(0);
                    pl.piaoFen = -1;
                    paioFenToServer_shiMenPaoHuZi(1);
                },
            },
            btn_piao2: {
                _click:function() {
                    var pl = getUIPlayer_shiMenPaoHuZi(0);
                    pl.piaoFen = -1;
                    paioFenToServer_shiMenPaoHuZi(2);
                },
            },
            btn_piao3: {
                _click:function() {
                    var pl = getUIPlayer_shiMenPaoHuZi(0);
                    pl.piaoFen = -1;
                    paioFenToServer_shiMenPaoHuZi(3);
                },
            },
    },
        BtnReady:{
        ...
        down: {
            head: {
                chuizi_tips : {
                    _run: function(){
                        setJiachuiImgTips_shiMenPaoHuZi(this, 0);
                    },
                    _event: {
                        initSceneData: function() {
                            setJiachuiImgTips_shiMenPaoHuZi(this, 0);
                ...
                jiachuiText_tips : {
                    _run: function() {
                        setJiachuiTextTips_shiMenPaoHuZi(this, 0);
                    },
                    _event:{
                        initSceneData: function() {
                            setJiachuiTextTips_shiMenPaoHuZi(this, 0);
                ...
                zhuang: {
                    ...
                    _event: {
                        waitPut: function(){
                            showUserZhuangLogo_shiMenPaoHuZi(this, 0);
                        },
                        initSceneData: function(){
                            if (IsArrowVisible_shiMenPaoHuZi()) showUserZhuangLogo_shiMenPaoHuZi(this, 0);
                        },
                        mjhand: function(){
                            if (IsArrowVisible_shiMenPaoHuZi()) showUserZhuangLogo_shiMenPaoHuZi(this, 0);
                        }
                    ...
                    chattext: {
                        _event: {
                            ...
                            MJChat: function(msg) {
                                ...
                                showUserChat_shiMenPaoHuZi(this, 0, msg);
                            },
                            playVoice: function(voicePath) {
                                MjClient.data._tempMessage.msg = voicePath;
                                showUserChat_shiMenPaoHuZi(this, 0, MjClient.data._tempMessage);
                            }
                        }
                    }
                },
                _click: function(btn) {
                    showPlayerInfo_shiMenPaoHuZi(0, btn);
                },
                _event: {
                    loadWxHead: function(d) {
                        setWxHead_shiMenPaoHuZi(this, d, 0);
                    },
                    addPlayer: function(eD) {
                        showFangzhuTagIcon_shiMenPaoHuZi(this,0);
                    },
                    removePlayer: function(eD) {
                        showFangzhuTagIcon_shiMenPaoHuZi(this,0);
                    }
                ...
                huxi:{
                    ...
                    _event:{
                        ...
                        initSceneData:function(){
                            this.visible = true;
                            var huXi = UpdateHuXi_shiMenPaoHuZi(0);
                            if(!IsArrowVisible_shiMenPaoHuZi()){
                                huXi = 0;
                                this.visible = false;
                            }
                ...
                timeImg:{
                    ...
                    // _event:{
                    //     initSceneData: function(eD) {
                    //         this.visible = curPlayerIsMe_shiMenPaoHuZi(0);
                    //     },
                    //     mjhand: function(eD) {
                    //         this.visible = curPlayerIsMe_shiMenPaoHuZi(0);
                    //     },
                    //     onlinePlayer: function(eD) {
                    //         this.visible = curPlayerIsMe_shiMenPaoHuZi(0) && IsArrowVisible_shiMenPaoHuZi();
                    //     },
                    //     waitPut: function(eD) {
                    //         this.visible = curPlayerIsMe_shiMenPaoHuZi(0);
                    //     },
                    ...
                    //             stopEffect(playTimeUpEff);
                    //             playTimeUpEff = null;
                    //             if(curPlayerIsMe_shiMenPaoHuZi(0)){
                    ...
                    //             playTimeUpEff = null;
                    //             var pl = getUIPlayer_shiMenPaoHuZi(0);
                    //             if(pl && pl.onLine && curPlayerIsMe_shiMenPaoHuZi(0)){
                    //                 arrowbkNumberUpdate(this);
                    //             }
                    //         },
                    //         roundEnd: function() {
            ...
            ready: {
                ...
                _run: function() {
                    GetReadyVisible_shiMenPaoHuZi(this, 0);
                },
                _event: {
                    startShuffleCards:function (d) {
                        GetReadyVisible_shiMenPaoHuZi(this, -1);
                    },
                    moveHead: function() {
                        GetReadyVisible_shiMenPaoHuZi(this, -1);
                    },
                    ...
                    addPlayer: function() {
                        GetReadyVisible_shiMenPaoHuZi(this, 0);//根据状态设置ready 是否可见 add by sking
                    },
                    removePlayer: function() {
                        GetReadyVisible_shiMenPaoHuZi(this, 0);
                    },
            ...
            handNode: {
                ...
                _event:  {
                    initSceneData: function() {
                        calculateHintPutList_shiMenPaoHuZi();
                    },
                    HZPickCard: function() {
                        calculateHintPutList_shiMenPaoHuZi();
                    },
                    ...
                    MJPass: function() { // 天胡 提 偎 跑后过胡
                        calculateHintPutList_shiMenPaoHuZi();
                        addTingSign_shiMenPaoHuZi(MjClient.playui._downNode);
                    },
            ...
            _event: {
                clearCardUI: function() {
                    clearCardUI_shiMenPaoHuZi(this, 0);
                },
                ...
                initSceneData: function(eD) {
                    SetUserVisible_shiMenPaoHuZi(this, 0);
                    InitUserHandUI_shiMenPaoHuZi(this, 0);
                    InitUserCoinAndName_shiMenPaoHuZi(this, 0);
                    MjClient.hasPut = false; // 重连回来重置打掉牌标志（打牌时候断线)
                    ShowPutCardIcon_shiMenPaoHuZi();
                    DealOffLineCard_shiMenPaoHuZi(this,0);
                },
                addPlayer: function(eD) {
                    SetUserVisible_shiMenPaoHuZi(this, 0);
                    InitUserCoinAndName_shiMenPaoHuZi(this, 0);
                ...
                mjhand: function(eD) {
                    ...
                    var uid = getUIPlayer_shiMenPaoHuZi(0).info.uid;
                    // if (eD.jiazhuNums)
                    // {
                ...
                HZNewCard: function(eD) {
                    ...
                    DealNewCard_shiMenPaoHuZi(this,eD,0);
                },
                MJPut: function(eD) {
                    DealPutCard_shiMenPaoHuZi(this,eD,0);
                },
                HZChiCard: function(eD) {
                    DealChiCard_shiMenPaoHuZi(this, eD, 0);
                },
                HZWeiCard: function(eD){
                    DealWeiCard_shiMenPaoHuZi(this,eD,0);
                },
                HZGangCard: function(eD) {
                    DealGangCard_shiMenPaoHuZi(this, eD, 0);
                },
                MJPeng: function(eD) {
                    DealPengCard_shiMenPaoHuZi(this, eD, 0);
                },
                MJHu: function(eD) {
                    DealHu_shiMenPaoHuZi(this, eD, 0);
                },
                onlinePlayer: function(eD) {
                    resetHandAfterBegin_shiMenPaoHuZi();
                    setUserOffline_shiMenPaoHuZi(this, 0);
                },
                ...
                MJJiazhu: function(msg) {
                    ...
                    MjClient.majiang.setJiaZhuNum(MjClient.playui._downNode, getUIPlayer_shiMenPaoHuZi(0));
                    MjClient.majiang.setJiaZhuNum(MjClient.playui._rightNode, getUIPlayer_shiMenPaoHuZi(1));
                    MjClient.majiang.setJiaZhuNum(MjClient.playui._topNode, getUIPlayer_shiMenPaoHuZi(2));
                },
                HZPickCard:function (eD) {
                    ...
                    DealShowLastCard_shiMenPaoHuZi(this, eD, 0);
                },
                HZAddCards:function (eD) {
                    DealAddCard_shiMenPaoHuZi(this,eD, 0);
                },
        ...
        right: {
            head: {
                chuizi_tips : {
                    _run: function(){
                        setJiachuiImgTips_shiMenPaoHuZi(this, changeUIOff_shiMenPaoHuZi(2));
                    },
                    _event: {
                        initSceneData: function() {
                            setJiachuiImgTips_shiMenPaoHuZi(this, changeUIOff_shiMenPaoHuZi(2));
                ...
                jiachuiText_tips : {
                    _run:function() {
                        setJiachuiTextTips_shiMenPaoHuZi(this, changeUIOff_shiMenPaoHuZi(2));
                    },
                    _event:{
                        initSceneData: function() {
                            setJiachuiTextTips_shiMenPaoHuZi(this, changeUIOff_shiMenPaoHuZi(2));
                        ...
                        MJJiazhu: function(msg) {
                            var pl = getUIPlayer_shiMenPaoHuZi(changeUIOff_shiMenPaoHuZi(2))
                            if (msg.uid == pl.info.uid) {
                                setJiachuiTextTips_shiMenPaoHuZi(this, changeUIOff_shiMenPaoHuZi(2));
                            }
                        },
                ...
                zhuang: {
                    ...
                    _event: {
                        waitPut: function() {
                            showUserZhuangLogo_shiMenPaoHuZi(this, changeUIOff_shiMenPaoHuZi(2));
                        },
                        initSceneData: function() {
                            if (IsArrowVisible_shiMenPaoHuZi()) showUserZhuangLogo_shiMenPaoHuZi(this, changeUIOff_shiMenPaoHuZi(2));
                        },
                        mjhand: function(){
                            if (IsArrowVisible_shiMenPaoHuZi()) showUserZhuangLogo_shiMenPaoHuZi(this, changeUIOff_shiMenPaoHuZi(2));
                        }
                ...
                chatbg: {
                    ...
                    chattext: {
                        _event: {
                            MJChat: function(msg) {
                                showUserChat_shiMenPaoHuZi(this, changeUIOff_shiMenPaoHuZi(2), msg);
                            },
                            playVoice: function(voicePath) {
                                MjClient.data._tempMessage.msg = voicePath;
                                showUserChat_shiMenPaoHuZi(this, changeUIOff_shiMenPaoHuZi(2), MjClient.data._tempMessage);
                            }
                        }
                    }
                },
                _click: function(btn) {
                    showPlayerInfo_shiMenPaoHuZi(changeUIOff_shiMenPaoHuZi(2), btn);
                },
                _event: {
                    loadWxHead: function(d) {
                        setWxHead_shiMenPaoHuZi(this, d, changeUIOff_shiMenPaoHuZi(2));
                    },
                    addPlayer: function(eD) {
                        showFangzhuTagIcon_shiMenPaoHuZi(this,changeUIOff_shiMenPaoHuZi(2));
                    },
                    removePlayer: function(eD) {
                        showFangzhuTagIcon_shiMenPaoHuZi(this,changeUIOff_shiMenPaoHuZi(2));
                    }
                ...
                huxi:{
                    ...
                    _event:{
                        ...
                        initSceneData:function(){
                            this.visible = true;
                            var huXi = UpdateHuXi_shiMenPaoHuZi(changeUIOff_shiMenPaoHuZi(2));
                            if(!IsArrowVisible_shiMenPaoHuZi()){
                                huXi = 0;
                                this.visible = false;
                            }
                ...
                timeImg:{
                    ...
                    // _event:{
                    //     initSceneData: function(eD) {
                    //         this.visible = curPlayerIsMe_shiMenPaoHuZi(1);
                    //     },
                    //     mjhand: function(eD) {
                    //         this.visible = curPlayerIsMe_shiMenPaoHuZi(1);
                    //     },
                    //     onlinePlayer: function(eD) {
                    //         this.visible = curPlayerIsMe_shiMenPaoHuZi(1) && IsArrowVisible_shiMenPaoHuZi();
                    //     },
                    //     waitPut: function(eD) {
                    //         this.visible = curPlayerIsMe_shiMenPaoHuZi(1);
                    //     },
                    ...
                    //             stopEffect(playTimeUpEff);
                    //             playTimeUpEff = null;
                    //             if(curPlayerIsMe_shiMenPaoHuZi(1)){
                    //                 arrowbkNumberUpdate(this);
                    //                 this.setString("00");
                    //             }
                    //         },
                    ...
                    //             stopEffect(playTimeUpEff);
                    //             playTimeUpEff = null;
                    //             var pl = getUIPlayer_shiMenPaoHuZi(1);
                    //             if(pl && pl.onLine && curPlayerIsMe_shiMenPaoHuZi(1)){
                    //                 arrowbkNumberUpdate(this);
                    //             }
                    //         },
                    //         roundEnd: function() {
            ...
            ready: {
                _layout: [[0.07, 0.07],[0.5, 0.5],[2.5, 2.5]],
                _run: function() {
                    GetReadyVisible_shiMenPaoHuZi(this, changeUIOff_shiMenPaoHuZi(2));
                },
                _event: {
                    startShuffleCards:function (d) {
                        GetReadyVisible_shiMenPaoHuZi(this, -1);
            ...
            _event: {
                clearCardUI: function() {
                    clearCardUI_shiMenPaoHuZi(this, changeUIOff_shiMenPaoHuZi(2));
                },
                clearCardArr: function(){
                    var eatNode = this.getChildByName("eatNode");
                    eatNode.removeAllChildren();
                ...
                initSceneData: function(eD) {
                    SetUserVisible_shiMenPaoHuZi(this, changeUIOff_shiMenPaoHuZi(2));
                    InitUserHandUI_shiMenPaoHuZi(this, changeUIOff_shiMenPaoHuZi(2));
                    InitUserCoinAndName_shiMenPaoHuZi(this, changeUIOff_shiMenPaoHuZi(2));
                    DealOffLineCard_shiMenPaoHuZi(this,changeUIOff_shiMenPaoHuZi(2));
                },
                addPlayer: function(eD) {
                    SetUserVisible_shiMenPaoHuZi(this, changeUIOff_shiMenPaoHuZi(2));
                    InitUserCoinAndName_shiMenPaoHuZi(this, changeUIOff_shiMenPaoHuZi(2));
                ...
                HZNewCard: function(eD) {
                    ...
                    DealNewCard_shiMenPaoHuZi(this,eD,changeUIOff_shiMenPaoHuZi(2));
                },
                roundEnd: function() {
                    InitUserCoinAndName_shiMenPaoHuZi(this, changeUIOff_shiMenPaoHuZi(2));
                    MjClient.playui.ResetOtherCard(this, changeUIOff_shiMenPaoHuZi(2));
                },
                waitPut: function(eD) {
                    DealWaitPut(this, eD, 1);
                },
                MJPut: function(eD) {
                    DealPutCard_shiMenPaoHuZi(this, eD, changeUIOff_shiMenPaoHuZi(2));
                },
                HZChiCard: function(eD) {
                    DealChiCard_shiMenPaoHuZi(this, eD, changeUIOff_shiMenPaoHuZi(2));
                },
                HZGangCard: function(eD) {
                    DealGangCard_shiMenPaoHuZi(this, eD, changeUIOff_shiMenPaoHuZi(2));
                },
                MJPeng: function(eD) {
                    DealPengCard_shiMenPaoHuZi(this, eD, changeUIOff_shiMenPaoHuZi(2));
                },
                HZWeiCard: function(eD){
                    DealWeiCard_shiMenPaoHuZi(this, eD, changeUIOff_shiMenPaoHuZi(2));
                },
                MJHu: function(eD) {
                    DealHu_shiMenPaoHuZi(this, eD, changeUIOff_shiMenPaoHuZi(2));
                },
                onlinePlayer: function(eD) {
                    setUserOffline_shiMenPaoHuZi(this, changeUIOff_shiMenPaoHuZi(2));
                },
                playerStatusChange: function(eD) {
                    setUserOffline_shiMenPaoHuZi(this, changeUIOff_shiMenPaoHuZi(2));
                },
                MJFlower: function(eD) {
                    HandleMJFlower(this, eD, changeUIOff_shiMenPaoHuZi(2));
                },
                MJTing: function (eD) {
                    HandleMJTing(this, eD, changeUIOff_shiMenPaoHuZi(2));
                },
                HZPickCard:function (eD) {
                    if(MjClient.rePlayVideo != -1){
                        MjClient.playui._doPickResetHandCardForPlayBack(eD, changeUIOff_shiMenPaoHuZi(2));
                    }

                    // RemovePutCardOut_shiMenPaoHuZi(true);
                    // MjClient.playui.ResetHandCard(this,1);

                    DealShowLastCard_shiMenPaoHuZi(this, eD, changeUIOff_shiMenPaoHuZi(2));
                },
                HZAddCards:function (eD) {
                    DealAddCard_shiMenPaoHuZi(this,eD, changeUIOff_shiMenPaoHuZi(2));
                },
                HZCheckRaise:function (msg) {
                    ShowPutCardIcon_shiMenPaoHuZi();
                    //MjClient.playui.EatVisibleCheck();
        ...
        left: {
            head: {
                chuizi_tips : {
                    _run: function(){
                        setJiachuiImgTips_shiMenPaoHuZi(this, MjClient.MaxPlayerNum_shiMenPaoHuZi - 1);
                    },
                    _event: {
                        initSceneData: function() {
                            setJiachuiImgTips_shiMenPaoHuZi(this, MjClient.MaxPlayerNum_shiMenPaoHuZi - 1);
                ...
                jiachuiText_tips : {
                    _visible:false,
                    _run:function() {
                        setJiachuiTextTips_shiMenPaoHuZi(this, MjClient.MaxPlayerNum_shiMenPaoHuZi - 1);
                    },
                    _event:{
                        initSceneData: function() {
                            setJiachuiTextTips_shiMenPaoHuZi(this, MjClient.MaxPlayerNum_shiMenPaoHuZi - 1);
                        ...
                        MJJiazhu: function(msg) {
                            var pl = getUIPlayer_shiMenPaoHuZi(MjClient.MaxPlayerNum_shiMenPaoHuZi - 1)
                            if (msg.uid == pl.info.uid) {
                                setJiachuiTextTips_shiMenPaoHuZi(this, MjClient.MaxPlayerNum_shiMenPaoHuZi - 1);
                            }
                        },
                ...
                zhuang: {
                    ...
                    _event: {
                        waitPut: function() {
                            showUserZhuangLogo_shiMenPaoHuZi(this, MjClient.MaxPlayerNum_shiMenPaoHuZi - 1);
                        },
                        initSceneData: function() {
                            if (IsArrowVisible_shiMenPaoHuZi()) showUserZhuangLogo_shiMenPaoHuZi(this, MjClient.MaxPlayerNum_shiMenPaoHuZi - 1);
                        },
                        mjhand: function(){
                            if (IsArrowVisible_shiMenPaoHuZi()) showUserZhuangLogo_shiMenPaoHuZi(this, MjClient.MaxPlayerNum_shiMenPaoHuZi - 1);
                        }
                ...
                chatbg: {
                    ...
                    chattext: {
                        _event: {
                            MJChat: function(msg) {
                                showUserChat_shiMenPaoHuZi(this, MjClient.MaxPlayerNum_shiMenPaoHuZi - 1, msg);
                            },
                            playVoice: function(voicePath) {
                                MjClient.data._tempMessage.msg = voicePath;
                                showUserChat_shiMenPaoHuZi(this, MjClient.MaxPlayerNum_shiMenPaoHuZi - 1, MjClient.data._tempMessage);
                            }
                        }
                    }
                },
                _click: function(btn) {
                    showPlayerInfo_shiMenPaoHuZi(MjClient.MaxPlayerNum_shiMenPaoHuZi - 1, btn);
                },
                _event: {
                    loadWxHead: function(d) {
                        setWxHead_shiMenPaoHuZi(this, d, MjClient.MaxPlayerNum_shiMenPaoHuZi - 1);
                    },
                    addPlayer: function(eD) {
                        showFangzhuTagIcon_shiMenPaoHuZi(this,MjClient.MaxPlayerNum_shiMenPaoHuZi - 1);
                    },
                    removePlayer: function(eD) {
                        showFangzhuTagIcon_shiMenPaoHuZi(this,MjClient.MaxPlayerNum_shiMenPaoHuZi - 1);
                    }
                ...
                huxi:{
                    ...
                    _event:{
                        ...
                        initSceneData:function(){
                            this.visible = true;
                            var huXi = UpdateHuXi_shiMenPaoHuZi(MjClient.MaxPlayerNum_shiMenPaoHuZi - 1);
                            if(!IsArrowVisible_shiMenPaoHuZi()){
                                huXi = 0;
                                this.visible = false;
                            }
                ...
                timeImg:{
                    ...
                    // _event:{
                    //     initSceneData: function(eD) {
                    //         this.visible = curPlayerIsMe_shiMenPaoHuZi(2);
                    //     },
                    //     mjhand: function(eD) {
                    //         this.visible = curPlayerIsMe_shiMenPaoHuZi(2);
                    //     },
                    //     onlinePlayer: function(eD) {
                    //         this.visible = curPlayerIsMe_shiMenPaoHuZi(2) && IsArrowVisible_shiMenPaoHuZi();
                    //     },
                    //     waitPut: function(eD) {
                    //         this.visible = curPlayerIsMe_shiMenPaoHuZi(2);
                    //     },
                    ...
                    //             stopEffect(playTimeUpEff);
                    //             playTimeUpEff = null;
                    //             if(curPlayerIsMe_shiMenPaoHuZi(2)){
                    //                 arrowbkNumberUpdate(this);
                    //                 this.setString("00");
                    //             }
                    //         },
                    ...
                    //             stopEffect(playTimeUpEff);
                    //             playTimeUpEff = null;
                    //             var pl = getUIPlayer_shiMenPaoHuZi(2);
                    //             if(pl && pl.onLine && curPlayerIsMe_shiMenPaoHuZi(2)){
                    //                 arrowbkNumberUpdate(this);
                    //             }
                    //         },
                    //         roundEnd: function() {
            ...
            ready: {
                _layout: [[0.07, 0.07],[0.5, 0.5],[-2.5, 2.5]],
                _run: function() {
                    GetReadyVisible_shiMenPaoHuZi(this, MjClient.MaxPlayerNum_shiMenPaoHuZi - 1);
                },
                _event: {
                    startShuffleCards:function (d) {
                        GetReadyVisible_shiMenPaoHuZi(this, -1);
            ...
            _event: {
                clearCardUI: function() {
                    clearCardUI_shiMenPaoHuZi(this, MjClient.MaxPlayerNum_shiMenPaoHuZi - 1);
                },
                clearCardArr: function(){
                    var eatNode = this.getChildByName("eatNode");
                    eatNode.removeAllChildren();
                ...
                initSceneData: function(eD) {
                    SetUserVisible_shiMenPaoHuZi(this, MjClient.MaxPlayerNum_shiMenPaoHuZi - 1);
                    InitUserHandUI_shiMenPaoHuZi(this, MjClient.MaxPlayerNum_shiMenPaoHuZi - 1);
                    InitUserCoinAndName_shiMenPaoHuZi(this, MjClient.MaxPlayerNum_shiMenPaoHuZi - 1);
                    DealOffLineCard_shiMenPaoHuZi(this,MjClient.MaxPlayerNum_shiMenPaoHuZi - 1);
                },
                addPlayer: function(eD) {
                    SetUserVisible_shiMenPaoHuZi(this, MjClient.MaxPlayerNum_shiMenPaoHuZi - 1);
                    InitUserCoinAndName_shiMenPaoHuZi(this, MjClient.MaxPlayerNum_shiMenPaoHuZi - 1);
                ...
                HZNewCard: function(eD) {
                    ...
                    DealNewCard_shiMenPaoHuZi(this,eD,MjClient.MaxPlayerNum_shiMenPaoHuZi - 1);
                },
                roundEnd: function() {
                    InitUserCoinAndName_shiMenPaoHuZi(this,MjClient.MaxPlayerNum_shiMenPaoHuZi - 1);
                    MjClient.playui.ResetOtherCard(this, MjClient.MaxPlayerNum_shiMenPaoHuZi - 1);
                },
                waitPut: function(eD) {
                    DealWaitPut(this, eD, MjClient.MaxPlayerNum_shiMenPaoHuZi - 1);
                },
                MJPut: function(eD) {
                    DealPutCard_shiMenPaoHuZi(this, eD, MjClient.MaxPlayerNum_shiMenPaoHuZi - 1);
                },
                HZChiCard: function(eD) {
                    DealChiCard_shiMenPaoHuZi(this, eD, MjClient.MaxPlayerNum_shiMenPaoHuZi - 1);
                },
                HZGangCard: function(eD) {
                    DealGangCard_shiMenPaoHuZi(this, eD, MjClient.MaxPlayerNum_shiMenPaoHuZi - 1);
                },
                MJPeng: function(eD) {
                    DealPengCard_shiMenPaoHuZi(this, eD, MjClient.MaxPlayerNum_shiMenPaoHuZi - 1);
                },
                HZWeiCard: function(eD){
                    DealWeiCard_shiMenPaoHuZi(this, eD, MjClient.MaxPlayerNum_shiMenPaoHuZi - 1);
                },
                MJHu: function(eD) {
                    DealHu_shiMenPaoHuZi(this, eD, MjClient.MaxPlayerNum_shiMenPaoHuZi - 1);
                },
                onlinePlayer: function(eD) {
                    setUserOffline_shiMenPaoHuZi(this, MjClient.MaxPlayerNum_shiMenPaoHuZi - 1);
                },
                playerStatusChange: function(eD) {
                    setUserOffline_shiMenPaoHuZi(this, MjClient.MaxPlayerNum_shiMenPaoHuZi - 1);
                },
                MJFlower: function(eD) {
                    HandleMJFlower(this, eD, MjClient.MaxPlayerNum_shiMenPaoHuZi - 1);
                },
                MJTing: function (eD) {
                    HandleMJTing(this, eD, MjClient.MaxPlayerNum_shiMenPaoHuZi - 1);
                },
                HZPickCard:function (eD) {
                    if(MjClient.rePlayVideo != -1) {
                        MjClient.playui._doPickResetHandCardForPlayBack(eD, MjClient.MaxPlayerNum_shiMenPaoHuZi - 1);
                    }
                    // RemovePutCardOut_shiMenPaoHuZi(true);
                    // MjClient.playui.ResetHandCard(this,2);

                    DealShowLastCard_shiMenPaoHuZi(this, eD, MjClient.MaxPlayerNum_shiMenPaoHuZi - 1);
                },
                HZAddCards:function (eD) {
                    DealAddCard_shiMenPaoHuZi(this,eD, MjClient.MaxPlayerNum_shiMenPaoHuZi - 1);
                },
                HZCheckRaise:function (msg) {
                    ShowPutCardIcon_shiMenPaoHuZi();
                    //MjClient.playui.EatVisibleCheck();
        ...
        xing: {
            ...
            head: {
                chuizi_tips : {
                    _run: function(){
                        setJiachuiImgTips_shiMenPaoHuZi(this, changeUIOff_shiMenPaoHuZi(1));
                    },
                    _event: {
                        initSceneData: function() {
                            setJiachuiImgTips_shiMenPaoHuZi(this, changeUIOff_shiMenPaoHuZi(1));
                ...
                jiachuiText_tips : {
                    _run:function() {
                        setJiachuiTextTips_shiMenPaoHuZi(this, changeUIOff_shiMenPaoHuZi(1));
                    },
                    _event:{
                        initSceneData: function() {
                            setJiachuiTextTips_shiMenPaoHuZi(this, changeUIOff_shiMenPaoHuZi(1));
                        ...
                        MJJiazhu: function(msg) {
                            var pl = getUIPlayer_shiMenPaoHuZi(changeUIOff_shiMenPaoHuZi(1))
                            if (msg.uid == pl.info.uid) {
                                setJiachuiTextTips_shiMenPaoHuZi(this, changeUIOff_shiMenPaoHuZi(1));
                            }
                        },
                ...
                zhuang: {
                    ...
                    _event: {
                        waitPut: function() {
                            if(MjClient.MaxPlayerNum_shiMenPaoHuZi == 4 ) {
                                showUserZhuangLogo_shiMenPaoHuZi(this, changeUIOff_shiMenPaoHuZi(1));
                            }
                        },
                        initSceneData: function() {
                            if(MjClient.MaxPlayerNum_shiMenPaoHuZi == 4 ) {
                                if (IsArrowVisible_shiMenPaoHuZi()) showUserZhuangLogo_shiMenPaoHuZi(this, changeUIOff_shiMenPaoHuZi(1));
                            }
                        },
                        mjhand: function(){
                            if(MjClient.MaxPlayerNum_shiMenPaoHuZi == 4 ) {
                ...
                chatbg: {
                    ...
                    chattext: {
                        _event: {
                            MJChat: function(msg) {
                                if(MjClient.MaxPlayerNum_shiMenPaoHuZi == 4) {
                                    showUserChat_shiMenPaoHuZi(this, changeUIOff_shiMenPaoHuZi(1), msg);
                                }
                            },
                            playVoice: function(voicePath) {
                                if(MjClient.MaxPlayerNum_shiMenPaoHuZi == 4) {
                                    MjClient.data._tempMessage.msg = voicePath;
                                    showUserChat_shiMenPaoHuZi(this, changeUIOff_shiMenPaoHuZi(1), MjClient.data._tempMessage);
                                }
                            }
                        }
                    }
                ...
                _click: function(btn) {
                    if(MjClient.MaxPlayerNum_shiMenPaoHuZi == 4) {
                        showPlayerInfo_shiMenPaoHuZi(changeUIOff_shiMenPaoHuZi(1), btn);
                    }
                },
                _event: {
                    loadWxHead: function(d) {
                        if(MjClient.MaxPlayerNum_shiMenPaoHuZi == 4) {
                            setWxHead_shiMenPaoHuZi(this, d, changeUIOff_shiMenPaoHuZi(1));
                        }
                    },
                    addPlayer: function(eD) {
                        showFangzhuTagIcon_shiMenPaoHuZi(this,changeUIOff_shiMenPaoHuZi(1));
                    },
                    removePlayer: function(eD) {
                        showFangzhuTagIcon_shiMenPaoHuZi(this,changeUIOff_shiMenPaoHuZi(1));
                    }
                ...
                huxi:{
                    ...
                    _event:{
                        ...
                        initSceneData:function(){
                            if(MjClient.MaxPlayerNum_shiMenPaoHuZi == 4) {
                                this.visible = true;
                                var huXi = UpdateHuXi_shiMenPaoHuZi(changeUIOff_shiMenPaoHuZi(1));
                                if (!IsArrowVisible_shiMenPaoHuZi()) {
                                    huXi = 0;
                                    this.visible = false;
                                }
            ...
            ready: {
                ...
                _run: function() {
                    if(MjClient.MaxPlayerNum_shiMenPaoHuZi == 4) {
                        GetReadyVisible_shiMenPaoHuZi(this, changeUIOff_shiMenPaoHuZi(1));
                    }
                },
                _event: {
                    startShuffleCards:function (d) {
            ...
            _event: {
                clearCardUI: function() {
                    if(MjClient.MaxPlayerNum_shiMenPaoHuZi == 4) {
                        clearCardUI_shiMenPaoHuZi(this, changeUIOff_shiMenPaoHuZi(1));
                    }
                },
                clearCardArr: function(){
                    if(MjClient.MaxPlayerNum_shiMenPaoHuZi == 4) {
                ...
                initSceneData: function(eD) {
                    if(MjClient.MaxPlayerNum_shiMenPaoHuZi == 4) {
                        SetUserVisible_shiMenPaoHuZi(this, changeUIOff_shiMenPaoHuZi(1));
                        InitUserHandUI_shiMenPaoHuZi(this, changeUIOff_shiMenPaoHuZi(1));
                        InitUserCoinAndName_shiMenPaoHuZi(this, changeUIOff_shiMenPaoHuZi(1));
                        DealOffLineCard_shiMenPaoHuZi(this, changeUIOff_shiMenPaoHuZi(1));
                    }
                },
                addPlayer: function(eD) {
                    if(MjClient.MaxPlayerNum_shiMenPaoHuZi == 4) {
                ...
                HZNewCard: function(eD) {
                    if(MjClient.MaxPlayerNum_shiMenPaoHuZi == 4) {
                        ...
                        DealNewCard_shiMenPaoHuZi(this, eD, changeUIOff_shiMenPaoHuZi(1));
                    }
                },
                roundEnd: function() {
                    if(MjClient.MaxPlayerNum_shiMenPaoHuZi == 4) {
                        InitUserCoinAndName_shiMenPaoHuZi(this, changeUIOff_shiMenPaoHuZi(1));
                        MjClient.playui.ResetOtherCard(this, changeUIOff_shiMenPaoHuZi(1));
                    }
                },
                waitPut: function(eD) {
                    if(MjClient.MaxPlayerNum_shiMenPaoHuZi == 4) {
                        DealWaitPut(this, eD, changeUIOff_shiMenPaoHuZi(1));
                    }
                },
                MJPut: function(eD) {
                    if(MjClient.MaxPlayerNum_shiMenPaoHuZi == 4) {
                        DealPutCard_shiMenPaoHuZi(this, eD, changeUIOff_shiMenPaoHuZi(1));
                    }
                },
                HZChiCard: function(eD) {
                    if(MjClient.MaxPlayerNum_shiMenPaoHuZi == 4) {
                        DealChiCard_shiMenPaoHuZi(this, eD, changeUIOff_shiMenPaoHuZi(1));
                    }
                },
                HZGangCard: function(eD) {
                    if(MjClient.MaxPlayerNum_shiMenPaoHuZi == 4) {
                        DealGangCard_shiMenPaoHuZi(this, eD, changeUIOff_shiMenPaoHuZi(1));
                    }
                },
                MJPeng: function(eD) {
                    if(MjClient.MaxPlayerNum_shiMenPaoHuZi == 4) {
                        DealPengCard_shiMenPaoHuZi(this, eD, changeUIOff_shiMenPaoHuZi(1));
                    }
                },
                HZWeiCard: function(eD){
                    if(MjClient.MaxPlayerNum_shiMenPaoHuZi == 4) {
                        DealWeiCard_shiMenPaoHuZi(this, eD, changeUIOff_shiMenPaoHuZi(1));
                    }
                },
                MJHu: function(eD) {
                    if(MjClient.MaxPlayerNum_shiMenPaoHuZi == 4) {
                        DealHu_shiMenPaoHuZi(this, eD, changeUIOff_shiMenPaoHuZi(1));
                    }
                },
                onlinePlayer: function(eD) {
                    if(MjClient.MaxPlayerNum_shiMenPaoHuZi == 4) {
                        setUserOffline_shiMenPaoHuZi(this, changeUIOff_shiMenPaoHuZi(1));
                    }
                },
                playerStatusChange: function(eD) {
                    if(MjClient.MaxPlayerNum_shiMenPaoHuZi == 4) {
                ...
                MJFlower: function(eD) {
                    if(MjClient.MaxPlayerNum_shiMenPaoHuZi == 4) {
                        HandleMJFlower(this, eD, changeUIOff_shiMenPaoHuZi(1));
                    }
                },
                MJTing: function (eD) {
                    if(MjClient.MaxPlayerNum_shiMenPaoHuZi == 4) {
                        HandleMJTing(this, eD, changeUIOff_shiMenPaoHuZi(1));
                    }
                },
                HZPickCard:function (eD) {
                    if(MjClient.MaxPlayerNum_shiMenPaoHuZi == 4) {
                        if (MjClient.rePlayVideo != -1 ) {
                            var pl = getUIPlayer_shiMenPaoHuZi(changeUIOff_shiMenPaoHuZi(1));
                            if (eD.uid == pl.info.uid) {
                                MjClient.OtherHandArr[changeUIOff_shiMenPaoHuZi(1)] = MjClient.majiang.sortHandCardSpecial(pl.mjhand)
                            }
                        }
                        DealShowLastCard_shiMenPaoHuZi(this, eD, changeUIOff_shiMenPaoHuZi(1));
                    }
                },
                HZAddCards:function (eD) {
                    if(MjClient.MaxPlayerNum_shiMenPaoHuZi == 4) {
                        DealAddCard_shiMenPaoHuZi(this, eD, changeUIOff_shiMenPaoHuZi(1));
                    }
                },
                HZCheckRaise:function (msg) {
                    if(MjClient.MaxPlayerNum_shiMenPaoHuZi == 4) {
        ...
        eat: {
            chi: {
                ...
                _touch: function(btn, eT) {
                    if (eT == 2){
                        if(ziPai.getHasFastEat()){
                            ...
                            if (chiSet.length == 1) {
                                ...
                                if (pl.mjhand.indexOf(putCard) < 0 || copy.indexOf(putCard) >= 0) {
                                    ...
                                    commitEatCards_shiMenPaoHuZi(eatCards, null);
                        ...
                        showSelectEatCards_shiMenPaoHuZi(this,btn.tag);
            ...
            hu: {
                ...
                _touch: function(btn, eT) {
                    if (eT == 2) {
                        MJHuToServer_shiMenPaoHuZi();
                        setChiVisible_shiMenPaoHuZi();//add by maoyu
                    }
                }
            },
            cancel: {
            ...
            wangDiao:{
                ...
                _touch: function(btn, eT) {
                    console.log(">>>> lf，点击碰按钮");
                    if (eT == 2) {
                        HZWangChuangToServer_shiMenPaoHuZi(1);
                        setChiVisible_shiMenPaoHuZi();//add by maoyu
                    }
                }
            },
            wangChuang:{
                ...
                _touch: function(btn, eT) {
                    console.log(">>>> lf，点击碰按钮");
                    if (eT == 2) {
                        HZWangChuangToServer_shiMenPaoHuZi(2);
                        setChiVisible_shiMenPaoHuZi();//add by maoyu
                    }
                }
            },
            wangZha:{
                ...
                _touch: function(btn, eT) {
                    console.log(">>>> lf，点击碰按钮");
                    if (eT == 2) {
                        HZWangChuangToServer_shiMenPaoHuZi(4);
                        setChiVisible_shiMenPaoHuZi();//add by maoyu
                    }
                }
            },
                    ...
                    this.adaptChiLayout = function() {
                        ...
                        if (cards_option.length == 0) { // 吃牌选好了
                            if ((MjClient.data.sData.players[SelfUid()].eatFlag & 32) > 0) {
                                MjClient.showMsg("选择吃后视为过胡，确定吃吗？", function() {
                                    ...
                                    HZChiToServer_shiMenPaoHuZi(cards_select[0], biCards);
                                    this.visible = false;
                                }.bind(this), function() {
                                    cards_select.pop();
                                    this.adaptChiLayout();
                        ...
                        for (var i = 0; i < cards_select.length; i++) {
                            ...
                            for (var j = 0; j < row.length; j++) {
                                ...
                                chiCardImg.loadTexture(MjClient.cardPath_shiMenPaoHuZi + "out" + row[j] + ".png");
        ...
        voice_btn: {
            ...
            _event: {
                ...
                downAndPlayVoice: function(msg) {
                    ...
                    downAndPlayVoice_shiMenPaoHuZi(msg.uid, msg.msg);
                }
            }
        },
        cutLine:{
    ...
    ctor: function() {
        this._super();
        ziPai.initZiPaiData();
        var playui = ccs.load("Play_shiMenPaoHuZi.json");
        playMusic("bgFight");
        var tData = MjClient.data.sData.tData;
        MjClient.MaxPlayerNum_shiMenPaoHuZi = tData.maxPlayer;
        ...
        //三人玩时恢复
            ...
            this.jsBind.right = PlayLayer_shiMenPaoHuZi.jsBindrightCopy;
        }
        ...
        this.tingLayer = new shiMenPaoHuZiTingLayer();
    ...
    resetJiaZhuTip: function()
    {
        ...
        for (var i = 0; i < MjClient.MaxPlayerNum_shiMenPaoHuZi; i ++)
        {
            var pl = getUIPlayer_shiMenPaoHuZi(i);
            if (!pl) 
                continue;
...
PlayLayer_shiMenPaoHuZi.jsBindrightCopy = PlayLayer_shiMenPaoHuZi.prototype.jsBind.right;

//重置手牌的顺序


PlayLayer_shiMenPaoHuZi.prototype.ResetHandCard = function(posNode,off, needAction, isDelay){

    needAction = needAction === undefined ? false : needAction;
    // needAction = false;

    if(off == 0){
        resetHandCard_shiMenPaoHuZiEx(posNode,off, needAction, isDelay);
...
//回放时其他家补牌重置手牌的处理
PlayLayer_shiMenPaoHuZi.prototype._doPickResetHandCardForPlayBack = function(msg, off){
    var pl = getUIPlayer_shiMenPaoHuZi(off);
...
PlayLayer_shiMenPaoHuZi.prototype._doMovetoAction = function(node,endP){
    node.stopAllActions();
    var ac = cc.moveTo(0.3,endP);
    node.runAction(ac);
};

PlayLayer_shiMenPaoHuZi.prototype.ResetOtherCard = function(posNode, off) {
    //添加吃、碰、跑、偎、提
    var sData = MjClient.data.sData;
    var tData = null;
    if (sData) {
    ...
    if (mjSortArr) {
            ...
            for (var i = 0; i < childList.length; i++) {
                ...
                if(tData.maxPlayer == 4){
                    if((off <= 1 && i != childList.length - 1) ||
                        (off >= 2 && i != 0)){
                        child.loadTexture(MjClient.cardPath_shiMenPaoHuZi + "huxiBG.png");
                    }
                }else{
        ...
        for (var k = 0; k < mjSortArr.length; k++) {
            ...
            if (name == "mjgang1") {
                ...
                for (var i = 0; i < 4; i++) {
                    addEatCard_shiMenPaoHuZi(posNode, "mjgang1" + k, cardNum, off);
                }
            ...
            if (name == "mjwei") {
                var cardNum = pl.mjwei[pos];
                for (var i = 0; i < 3; i++) {
                    addEatCard_shiMenPaoHuZi(posNode, "mjwei" + k, cardNum, off);
                }
            ...
            if (name == "mjgang0") {
                ...
                for (var i = 0; i < 4; i++) {
                    addEatCard_shiMenPaoHuZi(posNode, "mjgang0" + k, cardNum, off);
                }
            }
            //碰
            if (name == "mjpeng") {
                ...
                for (var i = 0; i < 3; i++) {
                    addEatCard_shiMenPaoHuZi(posNode, "mjpeng" + k, cardNum, off);
                }
            }
            //吃
            if (name == "mjchi") {
                ...
                for (var i = 0; i < eatCards.length; i++) {
                    addEatCard_shiMenPaoHuZi(posNode, "mjchi" + k, eatCards[i], off);
                }
                ...
                if (biCards && biCards.length > 0) {
                    for (var i = 0; i < biCards.length; i++) {
                        ...
                        for (var m = 0; m < biArr.length; m++) {
                            addEatCard_shiMenPaoHuZi(posNode, "mjbi" + k + i, biArr[m], off);
                        }
                    }
                }
    ...
    //    var outCard = getNewCard_shiMenPaoHuZi(pl.mjput[i], 2, off);
    //    if(off == 0){
    //        outCard.anchorX = 1;
    //        outCard.anchorY = 0;
...
PlayLayer_shiMenPaoHuZi.prototype.ResetPutCard = function(posNode,off){
    //添加打出的牌
    var pl = getUIPlayer_shiMenPaoHuZi(off);
    if (!pl) return;
    var outNode = posNode.getChildByName("outNode");
    ...
    for (var i = 0; i < pl.mjput.length; i++) {
        ...
        setPutCardPos_shiMenPaoHuZi(outNode, outCard, i, off);
...
//重置牌的顺序
PlayLayer_shiMenPaoHuZi.prototype.CardLayoutRestore = function(posNode,off){
    if(MjClient.rePlayVideo == -1){
...
// 判断吃、碰、跑、偎、提、胡的状态
PlayLayer_shiMenPaoHuZi.prototype.EatVisibleCheck = function(off){
    var eat = MjClient.playui.jsBind.eat;
    ...
    if(tData.tState != TableState.roundFinish && pl.mjState != TableState.roundFinish){
        if(IsTurnToMe()){
            ...
            if(pl.eatFlag & 4){
                ...
                if(!canHu){
                    // HZGangToServer_shiMenPaoHuZi(2);
                    return;
                }
            }
            //吃
...
PlayLayer_shiMenPaoHuZi.prototype.showAndHideHeadEffect = function(){
    ...
    else if (curPlayerIsMe_shiMenPaoHuZi(1)){
        curPlayerNode = arr[1].getChildByName("head");
    }else if (curPlayerIsMe_shiMenPaoHuZi(2)){
        curPlayerNode = arr[2].getChildByName("head");
    }else if (curPlayerIsMe_shiMenPaoHuZi(3)){
