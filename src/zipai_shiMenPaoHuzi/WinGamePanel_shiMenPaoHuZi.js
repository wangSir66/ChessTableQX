...
function addWxHeadToEndUI_shiMenPaoHuZi(node,off)
{
    ...
    setRoundEndUserOffline_shiMenPaoHuZi(frame, pl);
...
function reInitarrCardVisible_shiMenPaoHuZi()
{
...
//添加手牌
function addWinMjHand_shiMenPaoHuZi(_node,off) {
    var sData = MjClient.data.sData;
    var tData = sData.tData;
    if(MjClient.GAME_TYPE.PAO_HU_ZI_SR == MjClient.gameType){
        off = getOffByXing_shiMenPaoHuZi(off);
    }
    ...
    for(var i = 0;i < sortArr.length;i++){
        ...
        for(var j = 0;j < childSort.length;j++){
            ...
            clone.visible = true;
            clone.loadTexture(MjClient.cardPath_shiMenPaoHuZi+"out" + card +".png");
            var off_y = 0;
            if(_node.getName() == "rightPanel"){
                off_y = clone.height;
                clone.anchorX = 1;
...
var EndOneView_shiMenPaoHuZi = cc.Layer.extend({
    _block:null,
    _endoneuiNode:null,
    jsBind:{
        block: {
        ...
        btn_ready:{
            ...
            _click: function(btn,eT){
                ...
                reInitarrCardVisible_shiMenPaoHuZi();
            }
        },
        btn_logic: {
            _run: function(){
        ...
        main:{
                ...
                _run:function(){
                    ...
                    if(tData.cardNext < cards.length){
                        ...
                        for(var i = tData.cardNext;i < cards.length;i++){
                            ...
                            cloneCard.loadTexture(MjClient.cardPath_shiMenPaoHuZi+"out" + card +".png");
            ...
            leftPanel:{
                _run:function(){
                    addWinMjHand_shiMenPaoHuZi(this, 2);
                    if(MjClient.rePlayVideo != -1  || MjClient.MaxPlayerNum_shiMenPaoHuZi < 3){
                        this.visible = false;
                    }
                }
            },
            rightPanel:{
                _run:function(){
                    addWinMjHand_shiMenPaoHuZi(this,1);
                ...
                _run: function(){
                    ...
                    var cloneFunc = function(cardNum){
                        if(cloneNum != 0){
                            ...
                            cloneCard.loadTexture(MjClient.cardPath_shiMenPaoHuZi+"out" + cardNum + ".png");
                            cloneNum++;
                            return cloneCard;
                        }
                        cloneNum++;
                        var cardImg = self.getChildByName("card");
                        cardImg.loadTexture(MjClient.cardPath_shiMenPaoHuZi+"out" + cardNum + ".png");
                        return cardImg;
                    }
                    ...
                    var scrollView;
                    var diPaiRowCount = Math.ceil(sumCardNum/lineMaxNum);
                    //牌池还有牌的情况下，埋牌或者底牌一行显示不完时，都需要两行
                    if(sumCardNum > 0 && ((tData.maxPlayer == 2 && tData.areaSelectMode.maiPai20) || diPaiRowCount >  1)){
                        oneLine = false;
                        //埋牌情况下，超过21张需要滚动显示底牌
                        if(tData.maxPlayer == 2 && tData.areaSelectMode.maiPai20 && diPaiRowCount >  1){
                            var cardModel = this.getChildByName("card");
                            scrollView = ccui.ScrollView.create();
                            scrollView.setContentSize(cc.size(45 *20, cardModel.height));
                            scrollView.setDirection(ccui.ScrollView.DIR_HORIZONTAL);
                            scrollView.setBounceEnabled(true);
                            scrollView.setInnerContainerSize(cc.size(45 * sumCardNum, cardModel.height));
                            scrollView.setPosition(cc.pSub(cc.p(startPosX,35),cc.p(cardModel.width/2, cardModel.height/2)));
                    ...
                    for(var i = tData.cardNext; i < cards.length; i++){
                        ...
                        if(scrollView && cc.sys.isObjectValid(scrollView)){
                            ...
                            if(cc.sys.isObjectValid(cardImg.getParent())) cardImg.removeFromParent();
                            scrollView.addChild(cardImg);
                        }else{
                            var posX = (i - tData.cardNext) % lineMaxNum * 45 + startPosX;
                            var posY;
                            if(Math.floor((i - tData.cardNext) / lineMaxNum) < 1){
                                if((tData.maxPlayer == 2 && tData.areaSelectMode.maiPai20) || diPaiRowCount == 1)
                                    posY = 35;
                                else
                                    posY = 76
                            ...
                            cardImg.setPosition(cc.p(posX,  posY));
                        }
                    }
            ...
            uiBg:{
                headNode: {
                    _run: function(){
                        ...
                        if(tData.winner != -1)
                            winOff = getUiOffByUid_shiMenPaoHuZi(pl.info.uid);

                        addWxHeadToEndUI_shiMenPaoHuZi(this,winOff);
                        ...
                        for(var index = 0;index<playerCount;index++){
                            if(index != winOff){
                                ...
                                addWxHeadToEndUI_shiMenPaoHuZi(cloneThis,index);
                                cloneNum++;
                            }
                        }
                    }
    ...
    ctor:function () {
        ...
        var endoneui = ccs.load("endOne_shiMenPaoHuZi.json");
    ...
    setWinCard: function(){
        ...
        var cloneCardsSetData = function(cardNode,i,num){
            ...
            cardImg.loadTexture(MjClient.cardPath_shiMenPaoHuZi+"out" + num + ".png");
            return cardImg;
        }
                ...
                var getHuXi = function(cards){
                    ...
                    for(var t = 0;t < normal.length;t++){
                        if(normal[t].toString() == cards.toString()){
                            var huxi = huXiScore_shiMenPaoHuZi("chi",cards[0]);
                            return huxi;
                        }
                    }
                    return 0;
    ...
    setHuNameContnet: function(){
        ...
        // 初始UI
        var uiNode = this._endoneuiNode.getChildByName("uiNode");
        var mingTangList =uiNode.getChildByName("mingTangList");
        var huNameNode = mingTangList.getChildByName("huNameNode");
      //  var startPosY = huNameNode.getPositionY();
        ...
        var daLiuBaFantype = tData.areaSelectMode.daLiuBaFantype == 0 ? 8 : 6; 
        for (var index in pl.hzdesc) {
            ...
            if(index == "huXi" || index == "totalFan" || index == "tunNum" ){
                ...
                var self = this;
                var setHuItem = function(){
                    var infoData = self.getMingTangInfo(index,tData);
                    ...
                    if(mingTangIndex != 0){
                        ...
                        mingTangList.addChild(cloneNode);
                    ...
                    if(index == "ziMoJiaYiTun")
                    {
                        ...
                        if(index == "daTuanYuan"){ 
                            num.setString(infoData.icon + daLiuBaFantype + value)
                        }else{
                            num.setString(infoData.icon + pl.hzdesc[index] + value);
                        }
                ...
                if(index == "daTuanYuan" && (pl.hzdesc[index] / daLiuBaFantype) > 1){
                    for (var i = 0; i < pl.hzdesc[index] / daLiuBaFantype; i++) {
                        setHuItem(); 
                    }
                }else{
                  setHuItem();  
    ...
    setXingCardData: function(tData,num){
        ...
        card_img.loadTexture(MjClient.cardPath_shiMenPaoHuZi+"out" + tData.hunCard +".png");
    ...
    getMingTangInfo: function(name,tData){
        ...
        var table = {ziMoJiaYiTun:"自摸",ziMo:"自摸",tianHu:"天胡", diHu:"地胡",haiDiLao:"海底捞",hongHu:"红胡",heiHu:"黑胡",
            dianHu:"点胡",daHu:"大字胡",xiaoHu:"小字胡",duiZiHu:"对子胡",shuaHou:"耍候",
            huangFan:"黄番",jiaHangHang:"假行行",siQiHong:"四七红",daTuanYuan:"大团圆",sanTiWuKan:"三提五坎",
