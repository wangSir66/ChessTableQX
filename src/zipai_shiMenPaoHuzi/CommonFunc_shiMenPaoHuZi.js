MjClient.MaxPlayerNum_shiMenPaoHuZi = 3;
MjClient.cardPath_shiMenPaoHuZi = "playing/paohuzi/";
var ActionType_shiMenPaoHuZi =  //图片提示
    {
        CHI:1,
        PENG:2,
        WEI:3,
...
//初始化玩家金币、名字、胡息
function InitUserCoinAndName_shiMenPaoHuZi(node, off){
    var pl = getUIPlayer_shiMenPaoHuZi(off);
    if(!pl){
        return;
    }
...
//向服务器发送王闯王钓操作
function HZWangChuangToServer_shiMenPaoHuZi(type){
    cc.log("====================HZWangChuangToServer_shiMenPaoHuZi=================");
...
//向服务器发送打牌操作
function HZPutCardToServer_shiMenPaoHuZi(card){
    cc.log("====================HZPutCardToServer_shiMenPaoHuZi=================");
    MjClient.gamenet.request("pkroom.handler.tableMsg", {
        cmd: "MJPut",
        card: card
    });
...
// 向服务器发送吃牌
function HZChiToServer_shiMenPaoHuZi(eatCards, biCards){
...
//像服务器发送碰牌
function HZPengToServer_shiMenPaoHuZi()
{
    if (MjClient.rePlayVideo != -1) return; // 回放时候不能请求
    cc.log("====================HZPengToServer_shiMenPaoHuZi=================");
    MjClient.gamenet.request("pkroom.handler.tableMsg", {
        cmd: "MJPeng"
    });
}

function MJPass2Net_shiMenPaoHuZi(){
    cc.log("====================send======pass=====");
    ...
    if(IsTurnToMe() && tData.tState == TableState.waitPut){
        ...
        HZPassConfirmToServer_shiMenPaoHuZi();
    }
};

// 向服务器发送过牌
function HZPassConfirmToServer_shiMenPaoHuZi() {
    if (MjClient.rePlayVideo != -1)  {
        return; // 回放时候不能请求
    }
...
//发送跑牌命令
function HZGangToServer_shiMenPaoHuZi(type){
    if (MjClient.rePlayVideo != -1) return; // 回放时候不能请求
    cc.log("====================HZGangToServer_shiMenPaoHuZi=================");
    MjClient.gamenet.request("pkroom.handler.tableMsg", {
        cmd: "MJGang",
        type: type,
        eatFlag: EatFlag_shiMenPaoHuZi()
...
//发送偎牌命令
function HZWeiToServer_shiMenPaoHuZi(type){
    if (MjClient.rePlayVideo != -1) return; // 回放时候不能请求
    cc.log("====================HZWeiToServer_shiMenPaoHuZi=================");
    MjClient.gamenet.request("pkroom.handler.tableMsg", {
        cmd: "HZWeiCard"
    });
}
...
//向服务器发送发牌命令
function HZNewCardToServer_shiMenPaoHuZi(){
    if (MjClient.rePlayVideo != -1) return; // 回放时候不能请求
    cc.log("====================HZNewCardToServer_shiMenPaoHuZi=================");
    MjClient.gamenet.request("pkroom.handler.tableMsg", {
        cmd: "HZNewCard"
    });
}
...
//向服务器发送胡牌命令
function MJHuToServer_shiMenPaoHuZi(){
    if (MjClient.rePlayVideo != -1) return; // 回放时候不能请求
    cc.log("====================MJHuToServer=================");
    MjClient.gamenet.request("pkroom.handler.tableMsg", {
        cmd: "MJHu",
...
//添加吃的牌
function addEatCard_shiMenPaoHuZi(node,name,mjNum,off){
    //根据牌的类型获得需要添加的节点
    ...
    var type = 2;
    //设置牌
    var newCard = getNewCard_shiMenPaoHuZi(mjNum,type,off);
    ...
    if(MjClient.MaxPlayerNum_shiMenPaoHuZi == 2 &&
        (node.getName() == "right")) {
        newCard.y -= 10;
    }
...
/**
 添加手牌(回放)
 */
function addHandCardReplay_shiMenPaoHuZi(tagName,index,mjNum,off){
    var _node = getNode_shiMenPaoHuZi(off);
    //根据牌的类型获得需要添加的节点
...
function cleanCardNode_shiMenPaoHuZi(off) {
    //获取结点
    if(off != 0){
        return;
    }
...
function resetCardNode_shiMenPaoHuZi(off) {
    if(off != 0){
        return;
    }
    var _node = getNode_shiMenPaoHuZi(off);
...
function addOrAdjustHandCard_shiMenPaoHuZi(tagName,index,mjNum,off){
    if(off != 0){
        return;
    }
    var _node = getNode_shiMenPaoHuZi(off);
    ...
    if(!newCard){
        cc.log("chow", "newCard");
        newCard = getNewCard_shiMenPaoHuZi(mjNum,1,off);
    }else{
        cc.log("chow", "getCard");
    }
    var scale_y = newCard.scaleY;
    ...
    if (MjClient.movingCard_paohuzi == newCard) {
        setCardSprite_shiMenPaoHuZi(newCard, newCard.tag, 1, false);
    ...
    if(newCard.lastPosition){
        ...
        doMovetoAction_shiMenPaoHuZi(newCard, cc.p(beginPoint.x, beginPoint.y + cardCount * off_y));
    }else{
        newCard.x = beginPoint.x;
        newCard.y = beginPoint.y + cardCount * off_y;
    }
    //newCard.opacity = 255;
    SetTouchCardHandler_shiMenPaoHuZi(newCard,off);
    ...
    //cc.log("chow", "addOrAdjustHandCard_shiMenPaoHuZi: name=" + newCard.getName() + " num=" + newCard.num + " tag =" + newCard.tag);
}

function doMovetoAction_shiMenPaoHuZi(node,endP){
    node.stopAllActions();
    var action = cc.moveTo(0.15,endP);
    node.runAction(action);
};

function resetHandCard_shiMenPaoHuZi(posNode,off, needAction, isDelay){
    if(MjClient.rePlayVideo == -1){
        if(off == 0) {
            var handNode = posNode.getChildByName("handNode");
            handNode.visible = true;
            cleanCardNode_shiMenPaoHuZi(off);
            ...
            for (var k = 0; k < cardArr.length; k++) {
                ...
                for (var j = 0; j < groupList.length; j++) {
                    addOrAdjustHandCard_shiMenPaoHuZi(k, j, groupList[j], off);
                }
            }
            resetCardNode_shiMenPaoHuZi(off);

            addTingSign_shiMenPaoHuZi(posNode); // 添加听牌角标
            ...
            for (var i = 0; i < cardArr.length; i++) {
                ...
                if(addNode.lastPosition){
                    ...
                    doMovetoAction_shiMenPaoHuZi(addNode, cc.p((winSize.width - totalWidth) / 2 + i * width * scale_x, 0));
                }else{
        ...
        var handCard = null;
        if(off == 0){
            if(MjClient.MaxPlayerNum_shiMenPaoHuZi == 3 || MjClient.MaxPlayerNum_shiMenPaoHuZi == 2){
                handNode = posNode.getChildByName("handNode");
                handCard = posNode.getChildByName("handCard");
            }else if(MjClient.MaxPlayerNum_shiMenPaoHuZi == 4){
        ...
        for(var k = 0;k < cardArr.length;k++){
            ...
            for(var j = 0;j < groupList.length;j++){
                if(off == 0){
                    if(MjClient.MaxPlayerNum_shiMenPaoHuZi == 3 || MjClient.MaxPlayerNum_shiMenPaoHuZi == 2){
                        addHandCard_shiMenPaoHuZi(k,j,groupList[j],off);
                    }else if(MjClient.MaxPlayerNum_shiMenPaoHuZi == 4){
                        addHandCardReplay_shiMenPaoHuZi(k,j,groupList[j],off);
                    }
                }else {
                    addHandCardReplay_shiMenPaoHuZi(k,j,groupList[j],off);
                }
        ...
        if(off == 0 && (MjClient.MaxPlayerNum_shiMenPaoHuZi == 3 || MjClient.MaxPlayerNum_shiMenPaoHuZi == 2)) {
            var scale_x = handCard.scaleX;
            var winSize = MjClient.size;
...
function checkCard_shiMenPaoHuZi(node, off){
    ...
    if(cardHandArr.length > cardHand.length){
        ...
        for(var i = 0;i < cardHandArr.length; i++){
            if(cardHandArr[i] != cardHand[i]){
                RemoveHandCard_shiMenPaoHuZi(node, cardHandArr[i], off);
...
function resetHandCard_shiMenPaoHuZiEx(posNode,off, needAction, isDelay){
    if(off == 0) {
        ...
        checkCard_shiMenPaoHuZi(posNode, off);
        var handNode = posNode.getChildByName("handNode");
        handNode.visible = true;
        cleanCardNode_shiMenPaoHuZi(off);
        handNode.removeAllChildren();
 ...
 */
function addHandCard_shiMenPaoHuZi(tagName,index,mjNum,off){
    if(off != 0){
        return;
    }
    var _node = getNode_shiMenPaoHuZi(off);
    ...
    //设置牌
    var newCard = getNewCard_shiMenPaoHuZi(mjNum,1,off);
    var scale_y = newCard.scaleY;
    //var parentCount = addNode.childrenCount;
    //首先根据name判断cpNode中是否已经添加
    var cardParent = addNode.getChildByTag(tagName);
    if(!cardParent){
        ...
        cardParent.width = newCard.width;
        addNode.addChild(cardParent);
        // SetTouchCardHandler_shiMenPaoHuZi(cardParent);
    }
    ...
    cc.log("chow", "addHandCard_shiMenPaoHuZi: name=" + newCard.getName() + " num=" + newCard.num + " tag =" + newCard.tag);
}
 ...
 **/
function getNewCard_shiMenPaoHuZi(mjNum, type, off, isTurn){
    var _node = getNode_shiMenPaoHuZi(off);
    var copyNode = null;
    if(type == 1){
        //手牌
    ...
    //cc.log("getNewCard_shiMenPaoHuZi:"+mjNum+","+type+","+off+","+isTurn);
    if(mjNum > 0){
        //创建一个带有麻将信息的麻将 cp为创建后的麻将
        setCardSprite_shiMenPaoHuZi(mjNode, mjNum, type, isTurn);
        if(type == 1){
            //只有手牌才会有点击事件
            SetTouchCardHandler_shiMenPaoHuZi(mjNode,off);
        }
    }
...
/**
 ...
 type:类型，1:手牌 2：吃牌 3：打的牌
 */
function setCardSprite_shiMenPaoHuZi(mjNode, mjNum, type, isTurn){
    MjClient.cardPath_shiMenPaoHuZi = ziPai.getCardFilePath();

    if(type == 1){
        mjNode.loadTexture(MjClient.cardPath_shiMenPaoHuZi+"hand" + mjNum + ".png");
    }
    if(type == 2){
        mjNode.loadTexture(MjClient.cardPath_shiMenPaoHuZi+"out" + mjNum + ".png");
        if(isTurn){
            mjNode.loadTexture(MjClient.cardPath_shiMenPaoHuZi+"huxiBG.png");
        }
    }
    if(type == 3 || type == 4){
        mjNode.loadTexture(MjClient.cardPath_shiMenPaoHuZi+"put" + mjNum + ".png");
        if(isTurn){
            mjNode.loadTexture(MjClient.cardPath_shiMenPaoHuZi+"normalBG.png");
        }
    }
    mjNode.tag = mjNum;
}

function checkTingCards_shiMenPaoHuZi(putCard){
    //听牌检测
    var sData = MjClient.data.sData;

    if(MjClient.data.sData.tData.tState == TableState.waitPut && curPlayerIsMe_shiMenPaoHuZi(0) && putCard === undefined){
        postEvent("showTingLayer", []);
        return;
    }

    if(putCard && (!curPlayerIsMe_shiMenPaoHuZi(0) || sData.tData.tState != TableState.waitPut)){
        putCard = undefined;
    }
...
//刷新手牌移动时的坐标
var btn_oPos = null;
function updateBtnMovedPosition_shiMenPaoHuZi(btn, eventType){
...
//MjClient.movingCard_shiMenPaoHuZi = null;
//MjClient.cloneCard = null;
//MjClient.putCard = null;
MjClient.hasPut = false;
function SetTouchCardHandler_shiMenPaoHuZi(card,off){
    ...
    card.addTouchEventListener(function(btn,eventType){
        ...
        if(eventType == ccui.Widget.TOUCH_BEGAN){
            ...
            setCardSprite_shiMenPaoHuZi(cloneCard, cardTag, 1, false);
            ...
            btn.x = touchPos.x;
            btn.y = touchPos.y;
            // updateBtnMovedPosition_shiMenPaoHuZi(btn, eventType);
            ShowPutCardIcon_shiMenPaoHuZi();

            checkTingCards_shiMenPaoHuZi(btn.tag);
            var alignWidth = btn.scale * btn.width;
            setCardSprite_shiMenPaoHuZi(btn, btn.tag, 3, false);
        ...
        if(eventType == ccui.Widget.TOUCH_ENDED ||
            ...
            var pl = getUIPlayer_shiMenPaoHuZi(0);
            var tData = MjClient.data.sData.tData;
            var isPut = false;
            ...
            setCardSprite_shiMenPaoHuZi(btn, btn.tag, 1, false);
            ...
            var putCardCb = function () {
                ...
                if(((!pl.canNotPutCard||pl.canNotPutCard.indexOf(btnTag)==-1) &&(getOtherWeiCards_shiMenPaoHuZi(btnTag)||(pl.limitHuPutCard && pl.limitHuPutCard.indexOf(btnTag) != -1))) &&
                    ...
                    !MjClient.isCommon&& !MjClient.hasPut)
                {
                    ...
                    var putCard = getNewCard_shiMenPaoHuZi(btnTag, 3 ,off);
                    ...
                    if(getOtherWeiCards_shiMenPaoHuZi(btnTag)){
                                ...
                                if(pl.limitHuPutCard && pl.limitHuPutCard.indexOf(btnTag) != -1){
                                    MjClient.showMsg("吃边打边，这局将只能胡"+ MjClient.majiang.getLimitHuDesc(btnTag, tData)+"，是否确定？",
                                        function(){
                                            ...
                                            HZPutCardToServer_shiMenPaoHuZi(btnTag);
                                            MjClient.playui.ResetHandCard(MjClient.playui._downNode,0);

                                            checkTingCards_shiMenPaoHuZi(btnTag);
                                        },
                                        function(){
                                            MjClient.playui.ResetHandCard(MjClient.playui._downNode,0);
                                            ShowPutCardIcon_shiMenPaoHuZi();
            ...
            if(!isPut || isWangBa)
            {
                ...
                if(selectParentTag == btnParent.tag){
                    ...
                    if(selectArr){
                        //_changeArrIndex_shiMenPaoHuZi(selectArr, btnTag, btn);
                        MjClient.HandCardArr[btnParent.tag].splice(parseInt(btnName),1);
                        _fixArrIndex_shiMenPaoHuZi(selectArr, btnTag, btn);
                    }
                }
...
//根据坐标Y，添加到数组对应的位置
function _fixArrIndex_shiMenPaoHuZi(arr, btnTag, btn){
    if(arr){
        if(_isKan_shiMenPaoHuZi(arr)){
            arr.push(btnTag);
        ...
        cc.log('_fixArrIndex_shiMenPaoHuZi====:',arr);
    }

}

//改变数组元素的位置
function _changeArrIndex_shiMenPaoHuZi(arr, btnTag, btn){
    if(arr && !_isKan_shiMenPaoHuZi(arr)){
        var len = arr.length;
        for(var i = 0; i < len; i++){
            var tag = arr[i];
            if(tag == btnTag){
        ...
        _fixArrIndex_shiMenPaoHuZi(arr, btnTag, btn);
    }
}

//数组里的是否为三个相同的牌
function _isKan_shiMenPaoHuZi(arr){
    if(arr){
        var len = arr.length;
        if(len < 3){
            return false;
...
//断线重连之后，显示之前玩家打的牌
function DealOffLineCard_shiMenPaoHuZi(node, off) {
    cc.log("====================DealOffLineCard_shiMenPaoHuZi=================", off);
    var sData = MjClient.data.sData;
    var tData = sData.tData;
    ...
    if(typeof(tData.currCard) != "undefined" && tData.currCard != -1){
        var uids = tData.uids;
        var selfIndex = (uids.indexOf(SelfUid()) + off) % MjClient.MaxPlayerNum_shiMenPaoHuZi;
        if(uids[selfIndex] == uids[tData.curPlayer]){
            var putCard = getNewCard_shiMenPaoHuZi(tData.currCard, 3, off);
    ...
    cc.log("====================DealOffLineCard_shiMenPaoHuZi================= end", off);
}

// 通过玩家idx获取UIBind（down left right xing）
function getUIBindByIndex_shiMenPaoHuZi(idx) {
    var sData = MjClient.data.sData;
    var tData = sData.tData;
...
// 落牌动画
function fall_shiMenPaoHuZi() {
    var sData = MjClient.data.sData;
    var tData = sData.tData;

    // tData.lastPlayer没同步!
    var lastPlayer = (tData.curPlayer - 1 + tData.maxPlayer) % tData.maxPlayer;
    cc.log("chow", "fall_shiMenPaoHuZi"+ " lastPlayer = " + lastPlayer);
    ...
    if (out) {
        ...
        cc.log("chow", "fall_shiMenPaoHuZi" + JSON.stringify(out.getPosition()));
...
//发牌
MjClient.isCommon = false;
function DealNewCard_shiMenPaoHuZi(node, msg, off) {
    //没有发牌过来的时候直接忽略
    if (!msg.newCard) {
        RemovePutCardOut_shiMenPaoHuZi();
        MjClient.playui.EatVisibleCheck();
        return;
    }

    AddPutCard_shiMenPaoHuZi(node, msg, off); // 注意各个玩法验证！
    //存在发牌
    var sData = MjClient.data.sData;
    var tData = sData.tData;
    var uids = tData.uids;
    ...
    if (uids[selfIndex] == msg.uid) {
        cc.log("DealNewCard_shiMenPaoHuZi  newCard@@ ", msg.newCard);
        //清理之前玩家打的牌
        var lastPlayer = tData.lastPlayer;
        if (lastPlayer != -1) {
            //var lastOff = (off + MjClient.MaxPlayerNum_shiMenPaoHuZi - selfIndex) % MjClient.MaxPlayerNum_shiMenPaoHuZi;
            // RemovePutCardOut_shiMenPaoHuZi();
        ...
        var putCard = getNewCard_shiMenPaoHuZi(msg.newCard, 3, off, showPkBack);
        var putNode = node.getChildByName("put");
        ...
        if (!msg.isCommon) {
            if (MjClient.rePlayVideo == -1) {
                ...
                for (var i = 0; i < cardArr.length; i++) {
                    ...
                    if (tmpArr.length <= 2) {
                        isAdd = true;
                        //addHandCard_shiMenPaoHuZi(i,tmpArr.length.length + 1,msg.newCard,off);
...
function setPutCardPos_shiMenPaoHuZi(outNode, outCard, idx, off) {
    var tData = MjClient.data.sData.tData;
    var maxPlayer = tData.maxPlayer;
    var type = ziPai.getUiLayoutType();
    var isZuoXing = false;
    cc.log("setPutCardPos_shiMenPaoHuZi:" + isZuoXing);
...
function setJiachuiTextTips_shiMenPaoHuZi(node, off) {
    node.visible = false;
    var tData = MjClient.data.sData.tData;
    var pl = getUIPlayer_shiMenPaoHuZi(off);
...
function setJiachuiImgTips_shiMenPaoHuZi(node, off) {
    node.visible = false;
    var tData = MjClient.data.sData.tData;
    var pl = getUIPlayer_shiMenPaoHuZi(off);
...
function AddPutCard_shiMenPaoHuZi(node, msg, off) {
    var sData = MjClient.data.sData;
    var tData = sData.tData;
    var off_index = (tData.uids.indexOf(SelfUid()) + off) % MjClient.MaxPlayerNum_shiMenPaoHuZi;
    if (off_index == tData.curPlayer) { // off对应玩家为当前玩家
        ...
        var out = getNewCard_shiMenPaoHuZi(card, 2, off);
        var count = pl.mjput.length - 1;
        var width = out.width;
        var height = out.height;
        setPutCardPos_shiMenPaoHuZi(outNode, out, pl.mjput.length - 1, off);
        ...
        if(maxPlayer == 2){
                ...
                cc.log("chow", "setPutCardPos_shiMen off = " + off + " row = " + row + " line = " + line);
                if (off == 0) {
                    out.anchorX = 1;
                    out.anchorY = 0;
                ...
                cc.log("chow", "setPutCardPos_shiMen" + JSON.stringify(out.getPosition()));
            }
        else {
            if (off == 0) { // todo
                    out.anchorX = 1;
                    ...
                    if (MjClient.MaxPlayerNum_shiMenPaoHuZi == 3) {
                        out.anchorX = 1;
                        out.anchorY = 1;
                        out.x = outNode.width - count * width;
...
//翻醒发牌
function DealFanXingNewCard_shiMenPaoHuZi(card){
    var putCard = getNewCard_shiMenPaoHuZi(card, 4, 0);
...
//王闯/王钓
function DealWangChuang_shiMenPaoHuZi(node,msg,off){
    var sData = MjClient.data.sData;
    var tData = sData.tData;
    ...
    if(uids[selfIndex] == msg.uid){
        ...
        if(pl.wangType == 1){
            var putCard = getNewCard_shiMenPaoHuZi(91, 3, off);
            var putNode = node.getChildByName("put");
            putCard.scaleX = putCard.width/putNode.width - 0.16;
            putCard.scaleY = putCard.width/putNode.width - 0.06;
            putCard.x = putCard.width/2;
            ...
            ShowEatActionAnim_shiMenPaoHuZi(node, ActionType_shiMenPaoHuZi.WANGDIAO, off);

            var delay = cc.delayTime(1.5);
            var callback = function(){
                RemovePutCardOut_shiMenPaoHuZi();
                //HZNewCardToServer_shiMenPaoHuZi();
            }
            //putNode.runAction(cc.sequence(delay,cc.callFunc(callback)));
        }else if(pl.wangType == 2){
            var putCard = getNewCard_shiMenPaoHuZi(91, 3, off);
            var putCard1 = getNewCard_shiMenPaoHuZi(91, 3, off);
            var putNode = node.getChildByName("put");
            putCard.scaleX = putCard.width/putNode.width - 0.16;
            putCard.scaleY = putCard.width/putNode.width - 0.06;
            ...
            ShowEatActionAnim_shiMenPaoHuZi(node, ActionType_shiMenPaoHuZi.WANGCHUANG, off);
            var delay = cc.delayTime(1.5);
            var callback = function(){
                RemovePutCardOut_shiMenPaoHuZi();
                //HZNewCardToServer_shiMenPaoHuZi();
            ...
            var putCard = getNewCard_shiMenPaoHuZi(91, 3, off);
            var putCard1 = getNewCard_shiMenPaoHuZi(91, 3, off);
            var putCard2 = getNewCard_shiMenPaoHuZi(91, 3, off);
            var putNode = node.getChildByName("put");
            putCard.scaleX = putCard.width/putNode.width - 0.16;
            putCard.scaleY = putCard.width/putNode.width - 0.06;
            putCard1.scaleX = putCard.width/putNode.width - 0.16;
            ...
            ShowEatActionAnim_shiMenPaoHuZi(node, ActionType_shiMenPaoHuZi.WANGZHA, off);
            var delay = cc.delayTime(1.5);
            var callback = function(){
                RemovePutCardOut_shiMenPaoHuZi();
                //HZNewCardToServer_shiMenPaoHuZi();
...
//处理摸最后一张牌
function DealShowLastCard_shiMenPaoHuZi(node, msg, off){
    cc.log("======DealShowLastCard_shiMenPaoHuZi======= " + off);
    MjClient.hasPut = true;
    var sData = MjClient.data.sData;
    var tData = sData.tData;
    var uids = tData.uids;
    ...
    if(uids[selfIndex] == msg.uid)
    {
        ...
        setTimeout(function(){
            RemovePutCardOut_shiMenPaoHuZi(true);
            var pl = getUIPlayer_shiMenPaoHuZi(0);
...
//处理出牌
function DealPutCard_shiMenPaoHuZi(node, msg, off) {
    AddPutCard_shiMenPaoHuZi(node, msg, off); // 注意各个玩法验证！
    ...
    if (uids[selfIndex] == msg.uid) {
        ...
        if (SelfUid() != msg.uid || MjClient.rePlayVideo != -1) {
            ...
            putNode.stopAllActions();
            putNode.removeAllChildren();
            var putCard = getNewCard_shiMenPaoHuZi(msg.card, 3, off);
            putCard.scaleX = putCard.width / putNode.width - 0.16;
            putCard.scaleY = putCard.width / putNode.width - 0.06;
            putCard.x = putCard.width / 2;
            putCard.y = putCard.height / 2;
        ...
        if (off == 0) {
            removeTingSign_shiMenPaoHuZi(node);
        }

        if (MjClient.rePlayVideo != -1) {
            RemoveHandCard_shiMenPaoHuZi(node, msg.card, off);
            MjClient.playui.ResetHandCard(node, off);
        }
    }
}

function calculateHintPutList_shiMenPaoHuZi() {
...
function addTingSign_shiMenPaoHuZi(node) {
...
function removeTingSign_shiMenPaoHuZi(node) {
    var handNode = node.getChildByName("handNode");
    if (!handNode) {
        return;
    }
...
//吃牌
function DealChiCard_shiMenPaoHuZi(node, msg, off) {
    cc.log("======DealChiCard_shiMenPaoHuZi======= " + off);
    var sData = MjClient.data.sData;
    var tData = sData.tData;
    var uids = tData.uids;
    var selfIndex = (uids.indexOf(SelfUid()) + off) % MjClient.MaxPlayerNum_shiMenPaoHuZi;
    if (tData.curPlayer == selfIndex) {
        ...
        for (var i = 0; i < tmpEatCards.length; i++) {
            RemoveHandCard_shiMenPaoHuZi(node, tmpEatCards[i], off);
        }
            ...
            for (var k = 0; k < biCards.length; k++) {
                ...
                for (var m = 0; m < biArr.length; m++) {
                    if (tmpBiArr.indexOf(biArr[m]) >= 0) {
                        ...
                        RemoveHandCard_shiMenPaoHuZi(node, biArr[m], off);
                    }
                }
            }
        }
        ...
        for (var i = 0; i < eatCards.length; i++) {
            var card = getNewCard_shiMenPaoHuZi(eatCards[i], 2, off);
            if (i == 2) {
                card.setColor(cc.color(170, 170, 170));
            }
            ...
            card.zIndex = 4 - i;
            card.x = 0;
            card.y = off_count * getOffY_shiMenPaoHuZi(node, card);
            setChiCardAnchorPoint_shiMenPaoHuZi(node, card);
        ...
        if (biCards) {
            ...
            for (var k = 0; k < biCards.length; k++) {
                var biArr = biCards[k];
                for (var m = 0; m < biArr.length; m++) {
                    var card = getNewCard_shiMenPaoHuZi(biArr[m], 2, off);
                    if (m == 2) {
                        card.setColor(cc.color(170, 170, 170));
                    }
                    setChiCardAnchorPoint_shiMenPaoHuZi(node, card);
        ...
        var parentPos = getChiCardParentPosition_shiMenPaoHuZi(node, cardWidth, off, selfColCount);
        var moveX0 = 0;
        ...
        parent.x = moveX0;
        setCardParentPosY_shiMenPaoHuZi(node, parent);
        ...
        if (biCards) {
            ShowEatActionAnim_shiMenPaoHuZi(node, ActionType_shiMenPaoHuZi.XIAHUO, off);
        } else {
            ShowEatActionAnim_shiMenPaoHuZi(node, ActionType_shiMenPaoHuZi.CHI, off);
        }
...
function setChiCardAnchorPoint_shiMenPaoHuZi(node,newCard) {
    if(node.getName() == "down"){
        newCard.anchorX = 0;
        newCard.anchorY = 0;
    }else if(node.getName() == "right"){
...
function getOffY_shiMenPaoHuZi(node,newCard) {
    var off_y = 0;
    if(node.getName() == "down"){
        off_y = newCard.height;
    }else if(node.getName() == "right"){
...
function getChiCardParentPosition_shiMenPaoHuZi(node, newCardWidth, off, selfColCount, msg) {
...
function setCardParentPosY_shiMenPaoHuZi(node, cardParent) {
    var eatNode = node.getChildByName("eatNode");
    if(node.getName() == "down"){
        cardParent.y = 0;
    }else if (node.getName() == "right"){
...
function getShowCardIdx_shiMenPaoHuZi(node, chiTip) {
    var idx;
    switch(node.getName()) {
        case "down":
        case "xing":
...
// 处理碰
function DealPengCard_shiMenPaoHuZi(node, msg, off){
    cc.log("======DealChiCard_shiMenPaoHuZi======= ");
    var sData = MjClient.data.sData;
    var tData = sData.tData;
    var uids = tData.uids;
    var selfIndex = (uids.indexOf(SelfUid()) + off) % MjClient.MaxPlayerNum_shiMenPaoHuZi;
    if(tData.curPlayer == selfIndex){
        ...
        for(var i = 0;i < 3; i++) {
            var card = getNewCard_shiMenPaoHuZi(pengCard,2,off);
            cardWidth = card.width//*eatNode.scaleX;
            var childCount = parent.childrenCount;
            card.zIndex = 4 - childCount;
            card.x = 0;
            card.y = childCount * getOffY_shiMenPaoHuZi(node, card);
            setChiCardAnchorPoint_shiMenPaoHuZi(node, card);
            parent.addChild(card);
        }
        eatNode.addChild(parent);

        RemoveHandCard_shiMenPaoHuZi(node,pengCard,off);
        RemoveHandCard_shiMenPaoHuZi(node,pengCard,off);
        MjClient.playui.ResetHandCard(node, off);
        ShowEatActionAnim_shiMenPaoHuZi(node, ActionType_shiMenPaoHuZi.PENG, off);
        ...
        var parentPos = getChiCardParentPosition_shiMenPaoHuZi(node, cardWidth, off, 1);
        var moveX0 = 0;
        var outNodeCount = eatNode.getChildrenCount();
        if(outNodeCount>0){
            if(node.getName() == "down" || node.getName() == "left"){
...
//偎牌
function DealWeiCard_shiMenPaoHuZi(node, msg, off) {
    cc.log("======DealWeiCard_shiMenPaoHuZi======= ");
    var sData = MjClient.data.sData;
    var tData = sData.tData;
    var uids = tData.uids;
    var selfIndex = (uids.indexOf(SelfUid()) + off) % MjClient.MaxPlayerNum_shiMenPaoHuZi;
    if (tData.curPlayer == selfIndex) {
        ...
        RemovePutCardOut_shiMenPaoHuZi();
        ShowEatActionAnim_shiMenPaoHuZi(node, ActionType_shiMenPaoHuZi.WEI, off);
        ...
        for (var i = 0; i < 3; i++) {
            ...
            {
                if (i == getShowCardIdx_shiMenPaoHuZi(node, "wei")) {
                    isTurn = false;
                }
                var card = getNewCard_shiMenPaoHuZi(weiCard, 2, off, isTurn);
            } else if (MjClient.majiang.getCardShowType(tData.uids[tData.curPlayer], weiCard) == 1) {
                var card = getNewCard_shiMenPaoHuZi(weiCard, 2, off);
                ...
                var card = getNewCard_shiMenPaoHuZi(weiCard, 2, off, true);
            }
            cardWidth = card.width //*eatNode.scaleX;
            card.zIndex = 4 - i;
            card.x = 0;
            card.y = i * getOffY_shiMenPaoHuZi(node, card);
            setChiCardAnchorPoint_shiMenPaoHuZi(node, card);
            parent.addChild(card);
        }
        eatNode.addChild(parent);

        RemoveHandCard_shiMenPaoHuZi(node, weiCard, off);
        RemoveHandCard_shiMenPaoHuZi(node, weiCard, off);
        MjClient.playui.ResetHandCard(node, off);
...
// 跑牌或者提牌
function DealGangCard_shiMenPaoHuZi(node, msg, off) {
    var sData = MjClient.data.sData;
    var tData = sData.tData;
    var uids = tData.uids;
    var selfIndex = (uids.indexOf(SelfUid()) + off) % MjClient.MaxPlayerNum_shiMenPaoHuZi;
    ...
    if (uids.indexOf(gangUid) == selfIndex) {
        ...
        if (msg.type == 2) {
            RemovePutCardOut_shiMenPaoHuZi();
            ShowEatActionAnim_shiMenPaoHuZi(node, ActionType_shiMenPaoHuZi.PAO, off);
        } else {
            if (!msg.isGangHand && msg.type == 1) {
                RemovePutCardOut_shiMenPaoHuZi();
            }
            ShowEatActionAnim_shiMenPaoHuZi(node, ActionType_shiMenPaoHuZi.TI, off);
        }
        ...
        for (var i = 0; i < 4; i++) {
            ...
            if (msg.type == 2) {
                isTurn = false;
                var card = getNewCard_shiMenPaoHuZi(gangCard, 2, off, isTurn);
            }
            else {
                if (i == getShowCardIdx_shiMenPaoHuZi(node, "ti"))
                {
                    isTurn = false;
                }
                var card = getNewCard_shiMenPaoHuZi(gangCard, 2, off, isTurn);
        ...
        RemoveHandCard_shiMenPaoHuZi(node, gangCard, off);
        RemoveHandCard_shiMenPaoHuZi(node, gangCard, off);
        RemoveHandCard_shiMenPaoHuZi(node, gangCard, off);
        if (msg.isGangHand) {
            RemoveHandCard_shiMenPaoHuZi(node, gangCard, off);
        ...
        MjClient.playui.ResetHandCard(node, off);
        // addTingSign_shiMenPaoHuZi(node); // 添加听牌角标 (提 跑可能手牌未变动)
        ...
        var parentPos = getChiCardParentPosition_shiMenPaoHuZi(node, cardWidth, off, 1, msg);
        var moveX0 = 0;
        var outNodeCount = eatNode.getChildrenCount();
        if (outNodeCount > 0) {
            if (node.getName() == "down" || node.getName() == "left") {
...
// 处理胡
function DealHu_shiMenPaoHuZi(node, msg, off){
    var sData = MjClient.data.sData;
    var tData = sData.tData;
    var selfIndex = (tData.uids.indexOf(SelfUid()) + off) % MjClient.MaxPlayerNum_shiMenPaoHuZi;
    var pl = getUIPlayer_shiMenPaoHuZi(off);
    ...
    if(pl){
        pl.eatFlag = 0;
        MjClient.playui.EatVisibleCheck();
        ShowEatActionAnim_shiMenPaoHuZi(node,ActionType_shiMenPaoHuZi.HU,off);
    }
}

function DealAddCard_shiMenPaoHuZi(node,msg, off){
    if(MjClient.rePlayVideo == -1 && off != 0){
        return;
    }
    var cardArr = MjClient.HandCardArr;
...
//清除打出的牌
function RemovePutCardOut_shiMenPaoHuZi(noAction){

    var jsBind = MjClient.playui.jsBind;
    var uiList = [jsBind.down, jsBind.right, jsBind.left];
    if (MjClient.MaxPlayerNum_shiMenPaoHuZi == 4)
    {
        uiList = [jsBind.down, jsBind.xing, jsBind.right, jsBind.left];
    }
    if (MjClient.MaxPlayerNum_shiMenPaoHuZi == 2) {
...
//发手牌时再清理一遍
function clearMJHandCardUI_shiMenPaoHuZi() {
    // var handNode = MjClient.playui._downNode.getChildByName("handNode");
    // handNode.removeAllChildren();
...
//出牌表示状态
function ShowPutCardIcon_shiMenPaoHuZi() {
...
//清除手牌
function RemoveHandCard_shiMenPaoHuZi(node,card,off){
    if(MjClient.rePlayVideo == -1 && off != 0){
        return;
    }
    var cardArr = MjClient.HandCardArr;
...
// 初始化吃碰杠胡动作
function InitShowEatActionNode_shiMenPaoHuZi(plNode){
...
// 重置吃碰杠胡动作
function resetEatActionAnim_shiMenPaoHuZi()
{
    var jsBind = MjClient.playui.jsBind;
    ...
    var count = MjClient.MaxPlayerNum_shiMenPaoHuZi;
    for(var i = 0; i < count; i++)
    {
        InitShowEatActionNode_shiMenPaoHuZi(ui[i]._node);
    }
}

//播放头像移动
function tableStartHeadMoveAction_shiMenPaoHuZi(node){
...
//重置3家头像位置
function reConectHeadLayout_shiMenPaoHuZi(node){
    var tData = MjClient.data.sData.tData;
    var down = node.getChildByName("down").getChildByName("head");
    var left = node.getChildByName("left").getChildByName("head");
    var right = node.getChildByName("right").getChildByName("head");
    ...
    resetEatActionAnim_shiMenPaoHuZi();
...
//根据偏移获得玩家node
function getNode_shiMenPaoHuZi(off){
    var _node = null;
    var tData = MjClient.data.sData.tData;
    if (tData.maxPlayer != 4){
        if (off == 1 || off == 2){
 ...
 * @return {object} 玩家数据
 */
function getUIPlayer_shiMenPaoHuZi(off){
    var sData = MjClient.data.sData;
    var tData = sData.tData;
    var uids = tData.uids;
    var selfIndex = uids.indexOf(SelfUid());
    selfIndex = (selfIndex + off) % MjClient.MaxPlayerNum_shiMenPaoHuZi;
...
// 获取ui头像，通过偏移值
function getUIHeadByOff_shiMenPaoHuZi(off){
    var pl = getUIPlayer_shiMenPaoHuZi(off);
    if(!pl)
    {
        return {};
 ...
 * 下载录音, 调用 播放函数
 * */
function downAndPlayVoice_shiMenPaoHuZi(uid, filePath){
    var index = getUiOffByUid_shiMenPaoHuZi(uid);
...
function getUiOffByUid_shiMenPaoHuZi(uid){
    var sData = MjClient.data.sData;
    var tData = sData.tData;
    var uids = tData.uids;
    var selfIndex = uids.indexOf(SelfUid());
    var targetIndex = uids.indexOf(uid);
    return (targetIndex - selfIndex + MjClient.MaxPlayerNum_shiMenPaoHuZi) % MjClient.MaxPlayerNum_shiMenPaoHuZi;
}

//设置微信头像
function setWxHead_shiMenPaoHuZi(node, d, off){
    if(d.uid == getUIHeadByOff_shiMenPaoHuZi(off).uid){
        ...
        COMMON_UI.addNobleHeadFrame(nobody,getUIPlayer_shiMenPaoHuZi(off))
    }
}

//显示玩家庄的ui
function showUserZhuangLogo_shiMenPaoHuZi(node, off){
    var tData = MjClient.data.sData.tData;
    var pl = getUIPlayer_shiMenPaoHuZi(off);
    node.zIndex = 100;
    if(tData && pl){
...
function showUserZhuangLogoZuoXing_shiMenPaoHuZi(node, off){
    var tData = MjClient.data.sData.tData;
    var pl = getUIPlayer_shiMenPaoHuZi(off);
    node.zIndex = 100;
    if(tData && pl){
...
//显示房主  add by sking
function showFangzhuTagIcon_shiMenPaoHuZi(node,off)
{
    var pl = getUIPlayer_shiMenPaoHuZi(off);
    if(!pl) //位置上没人则删掉房主标签
    {
...
// 清理ui
function clearCardUI_shiMenPaoHuZi(node,off){
    mylog("clearCardUI_shiMenPaoHuZi");
    ...
    for(var i = 0; i < children.length; i++)
    {
        ...
        {
            InitShowEatActionNode_shiMenPaoHuZi(ni.getParent());
        }
    }
}

function EatFlag_shiMenPaoHuZi(){
    var eat = MjClient.playui.jsBind.eat;
    var eatFlag = 0;
...
function huXiScore_shiMenPaoHuZi(type,card)
{
...
//刷新胡息
function UpdateHuXi_shiMenPaoHuZi(off){
    var sData = MjClient.data.sData;
    var tData = sData.tData;
    var score = 0;
    var pl = getUIPlayer_shiMenPaoHuZi(off);
    ...
    if(mjpao.length>0){
        for(var p=0;p<mjpao.length;p++){
            score += huXiScore_shiMenPaoHuZi("paoPai",mjpao[p]);
        }
    }
    ...
    if(mjpeng.length>0){
        for(var i=0;i<mjpeng.length;i++){
            score += huXiScore_shiMenPaoHuZi("peng",mjpeng[i]);
        }
    }
        ...
        var chiScore = function(cards){
            ...
            for(var k=0;k<chiXiArr.length;k++){
                ...
                if(cards.toString() == chiXiList.toString()){
                    score += huXiScore_shiMenPaoHuZi("chi",cards[0]);
                }
            }
        };
    ...
    if(mjti.length>0){
        for (var t = 0; t < mjti.length; t++) {
            score += huXiScore_shiMenPaoHuZi("tiPai", mjti[t]);
        }
    }
    ...
    if(mjwei && mjwei.length>0){
        for (var w = 0; w < mjwei.length; w++) {
            if(MjClient.majiang.getCardShowType(pl.info.uid, mjwei[w]) != 0) {
                score += huXiScore_shiMenPaoHuZi("weiPai", mjwei[w]);
            }
        }
    }
    ...
    if(tData.xingPlayer == selfIndex){
        pl = getUIPlayer_shiMenPaoHuZi(2);
        //提牌
        var mjti = pl.mjgang1;
        if(mjti.length>0){
            for(var t=0;t<mjti.length;t++){
...
//设置转盘显示状态
function IsArrowVisible_shiMenPaoHuZi(){
    var pl = getUIPlayer_shiMenPaoHuZi(0);
    if (!pl)
    {
        return;
...
// 显示吃碰杠胡动作
function ShowEatActionAnim_shiMenPaoHuZi(node, actType, off){
    var delayTime = 1;
    switch(actType)
    {
        case ActionType_shiMenPaoHuZi.FANXING:
            delayTime = 1.6;
            break;
        default:
            break;
    ...
    switch(actType)
    {
        ...
        case ActionType_shiMenPaoHuZi.TI:
            ...
            break;
        case ActionType_shiMenPaoHuZi.WANGDIAO:
            ...
            break;
        case ActionType_shiMenPaoHuZi.WANGCHUANG:
            eatActionChild = eatActionNode.getChildByName("wangChuang");
            eatActionChild.visible = true;
            eatActionChild.runAction(cc.sequence(cc.delayTime(delayTime), cc.callFunc(callback)));
            break;
        case ActionType_shiMenPaoHuZi.XIAHUO:
...
//显示玩家信息
function showPlayerInfo_shiMenPaoHuZi(off, node){
    //var tData = MjClient.data.sData.tData;
    var pl = getUIPlayer_shiMenPaoHuZi(off);
    if(pl){
...
function resetChiParam_shiMenPaoHuZi(){

}

function setChiVisible_shiMenPaoHuZi(){
    var eat = MjClient.playui.jsBind.eat;
...
function showSelectEatCards_shiMenPaoHuZi(node,cardNum){
    var eat = MjClient.playui.jsBind.eat;
    eat.chi._node.visible = false;
    eat.guo._node.visible = false;
    eat.peng._node.visible = false;
    ...
    addEatCards_shiMenPaoHuZi(parent,cardArr,putCard);
}

//居中动画,数组里的节点居中排列 只关心x轴
doMoveCenterAction_shiMenPaoHuZi = function(arr,isScale){
    arr.reverse();
    var gap = 5;
...
function commitEatCards_shiMenPaoHuZi(eatCards, biArr){
    if ((MjClient.data.sData.players[SelfUid()].eatFlag & 32) > 0) {
        MjClient.showMsg("吃牌后视为过胡，确定吃吗？", function() {
            setChiVisible_shiMenPaoHuZi();
            HZChiToServer_shiMenPaoHuZi(eatCards, biArr);
        }, function() {}, "1");
    } else {
        setChiVisible_shiMenPaoHuZi();
        HZChiToServer_shiMenPaoHuZi(eatCards, biArr);
...
//添加可以吃的牌
function addEatCards_shiMenPaoHuZi(node,handCardArr,putCard){
    var cardPath = MjClient.cardPath_shiMenPaoHuZi;
    ...
    for(var i = 0;i < chiSet.length;i++){
        ...
        chiCloneBtn.addClickEventListener(function(chiBtn){
            ...
            if(tmpHandCardArr.indexOf(putCard) < 0){
                // setChiVisible_shiMenPaoHuZi();
                // HZChiToServer_shiMenPaoHuZi(chiArr,null);

                commitEatCards_shiMenPaoHuZi(chiArr, null);
            }else{
                var biSet = MjClient.majiang.getBiCards(tmpHandCardArr,putCard);
                if(biSet && biSet.length > 0){
                    addFirstBiCards_shiMenPaoHuZi(node,biSet,putCard,tmpHandCardArr,chiArr);
                }
            }
        });
    }
    if(ziPai.getUiLayoutType() == 0){//新布局
        doMoveCenterAction_shiMenPaoHuZi([chiBg], false);
    }
}

//添加第一个比牌
function addFirstBiCards_shiMenPaoHuZi(node,biSet,putCard,handCardArr,chiArr){
    var cardPath = MjClient.cardPath_shiMenPaoHuZi;
    cardPath = ziPai.getCardFilePath();
    ...
    for(var i = 0;i < biSet.length;i++){
        ...
        cloneBtn.addClickEventListener(function(btn){
            ...
            if(tmpHandCardArr.indexOf(putCard) >= 0){
                var biSet = MjClient.majiang.getBiCards(tmpHandCardArr,putCard);
                if(biSet && biSet.length > 0){
                    addSecBiCards_shiMenPaoHuZi(node,biSet,putCard,tmpHandCardArr,chiArr,biArr);
                }
            }else{
                // setChiVisible_shiMenPaoHuZi();
                // HZChiToServer_shiMenPaoHuZi(chiArr,[biArr]);
                commitEatCards_shiMenPaoHuZi(chiArr,[biArr]);
            }
        });
    }
    ...
    if(ziPai.getUiLayoutType() == 0){//新布局
        ...
        doMoveCenterAction_shiMenPaoHuZi([chiBg,biBg],isScale);
    }else{
        biBg.scale = chiBg.scale;
        doMoveCenterAction_shiMenPaoHuZi2([chiBg,biBg],isScale);
    }
}

//居中动画,数组里的节点居中排列 只关心x轴
doMoveCenterAction_shiMenPaoHuZi2 = function(arr,isScale){
    isScale = isScale === undefined ? true : isScale;
    var gap = 5;
    if(arr && arr.length > 0){
        var len = arr.length;
...
//添加第二个比牌
function addSecBiCards_shiMenPaoHuZi(node,biSet,putCard,handCardArr,chiArr,firstBiArr){
    var cardPath = MjClient.cardPath_shiMenPaoHuZi;
    cardPath = ziPai.getCardFilePath();
    ...
    for(var i = 0;i < biSet.length;i++){
        ...
        cloneBtn.addClickEventListener(function(btn){
            ...
            btn.setBright(false);
            // setChiVisible_shiMenPaoHuZi();
            // HZChiToServer_shiMenPaoHuZi(chiArr,[firstBiArr,biArr]);

            commitEatCards_shiMenPaoHuZi(chiArr,[firstBiArr,biArr]);
        });
    }
    ...
    if(ziPai.getUiLayoutType() == 0){//新布局
        ...
        doMoveCenterAction_shiMenPaoHuZi([chiBg,biBg0,biBg],isScale);
    }else{
        biBg.scale = chiBg.scale;
        biBg0.scale = chiBg.scale;
        doMoveCenterAction_shiMenPaoHuZi2([chiBg,biBg0,biBg],isScale);
    }
}

function GetReadyVisible_shiMenPaoHuZi(node, off) {
    if (off < 0) {
        node.visible = false;
        return false;
    }
...
//设置玩家掉线头像
function setUserOffline_shiMenPaoHuZi(node, off){
...
//设置玩家掉线头像（邵阳新版大小结算专用）
function setRoundEndUserOffline_shiMenPaoHuZi(frame, pl){
    if(!pl){
        return;
    }
...
//显示玩家文字
function showUserChat_shiMenPaoHuZi(node, off, msg){
    var tData = MjClient.data.sData.tData;
 ...
 */
function curPlayerIsMe_shiMenPaoHuZi(off){
    var sData = MjClient.data.sData;
    var tData = sData.tData;
...
function getVisablePlayerCount_shiMenPaoHuZi(){
    var sData = MjClient.data.sData;
    var tData = sData.tData;
    var uids = tData.uids;
    var count = 0;
 ...
 */
function getXingPlayerIndex_shiMenPaoHuZi(){
    if(MjClient.MaxPlayerNum_shiMenPaoHuZi != 4){
        return -1;
    }
    if(getVisablePlayerCount_shiMenPaoHuZi() < 3){
        return -1;
    }
    var sData = MjClient.data.sData;
    var tData = sData.tData;
    ...
    var xingIndex = (zhuang + 2 + MjClient.MaxPlayerNum_shiMenPaoHuZi) % MjClient.MaxPlayerNum_shiMenPaoHuZi;
    var selfIndex = tData.uids.indexOf(SelfUid());
    xingIndex = (xingIndex - selfIndex + MjClient.MaxPlayerNum_shiMenPaoHuZi) % MjClient.MaxPlayerNum_shiMenPaoHuZi;
    return xingIndex;
}


//重新开始后，重置MjClient.HandCardArr
function resetHandAfterBegin_shiMenPaoHuZi(){
    var sData = MjClient.data.sData;
    var pl = sData.players[SelfUid() + ""];
...
/*
 设置弃胡状态
 */
function setQiHuState_shiMenPaoHuZi()
{
    var pl = getUIPlayer_shiMenPaoHuZi(0);
    if (pl.isQiHu) {
...
function changeMJBg_shiMenPaoHuZi(node, type)
{
    if (node.toString() == "[object ImageView]") {
        var oldFile = node.getRenderFile().file;
        var newFile = getNewMJBgFile_shiMenPaoHuZi(oldFile, type);
    ...
    for(var index in childArray)
    {
        ...
        changeMJBg_shiMenPaoHuZi(child, type);
    }
}

function getNewMJBgFile_shiMenPaoHuZi(oldFile, type)
{
    ...
    if (type == 0) {
        ...
        if (oldFile.indexOf("/MJBg1") != -1)
            ...
            MjClient.cardPath_shiMenPaoHuZi = "playing/paohuzi/MJBg1/";
        }
        ...
        if (oldFile.indexOf("/MJBg2") != -1)
            ...
            MjClient.cardPath_shiMenPaoHuZi = "playing/paohuzi/MJBg2/";
        }
        else if (oldFile.indexOf("playing/paohuzi/") != -1) {
...
function getOtherWeiCards_shiMenPaoHuZi(card) {
    var sData = MjClient.data.sData;
 ...
 */
function createRadioBoxForCheckBoxs_shiMenPaoHuZi(nodes, callback, defaultIndex){
    var newobj = {};
 ...
 *  off:玩家的实际off
 **/
function changeUIOff_shiMenPaoHuZi(off){
    if (MjClient.MaxPlayerNum_shiMenPaoHuZi != 4 && (off == 2 || off == 3)){
        return off - 1;
    }
    return off;
}
...
//是否显示离开按钮（包括准备阶段）
function isShowBackBtn_shiMenPaoHuZi(){
    if(IsInviteVisible()){
        return true;
    }else{
        var sData = MjClient.data.sData;
 ...
 */
function getAnimate_shiMenPaoHuZi(preName, len, delaytime, playCount, startIndex){
...
function paioFenToServer_shiMenPaoHuZi(index) {
    var tData = MjClient.data.sData.tData;
