function SetGameOverLayer_dianTuo(node,off,isNewUi){
    ...
    var uibind={
        ...
        listView:{
            _run:function(){
                ...
                var nameText = [" 炸弹次数："," 510K个数:"];
                var numNameArr = ["zhaDanNum","wuShiKNum"];
                ...
                for (var i = 0; i < 2; i++) {
                    ...
                    var num = 0;
                    if (pl.stats_type) {
                        num = pl.stats_type[numNameArr[i]];
                    }
                    scoreText.setString(num || 0);
    ...
    // setRoundEndUserOffline_dianTuo(wxNode,pl);
    ...
    var xifen_view =  node.getChildByName("xifen_view"); 
    xifen_view.setVisible( (tData.areaSelectMode.isBombScore || tData.areaSelectMode.isRedBlackScore) );
    var xi_winNum1 = xifen_view.getChildByName("winNum1");
    var xi_winNum2 = xifen_view.getChildByName("winNum2");
    // xi_winNum1.setString(pl.score_xi_all || 0);

    var zongFen_view = node.getChildByName("zongFen_view");
    var winNum1 = zongFen_view.getChildByName("winNum1");
    var winNum2 = zongFen_view.getChildByName("winNum2");

    winNum1.ignoreContentAdaptWithSize(true);
    winNum2.ignoreContentAdaptWithSize(true);
    xi_winNum1.ignoreContentAdaptWithSize(true);
    xi_winNum2.ignoreContentAdaptWithSize(true);

    var xiScore = pl.score_xi_all || 0;
    if(xiScore >= 0){
        xi_winNum2.visible = false;
        xi_winNum1.setString(xiScore);
    }else{
        xi_winNum1.visible = false;
        xi_winNum2.setString(xiScore);
        xifen_view.loadTexture("playing/diantuo/loser_xiFen.png");
    }

    if(pl.winall >= 0){ 
        winNum2.setVisible(false);
        winNum1.setString(pl.winall );
    }else{
        winNum1.setVisible(false);
        winNum2.setString(pl.winall );
        zongFen_view.loadTexture("playing/diantuo/loser_zongFen.png");
    }
    ...
    if(pl.winall == MaxWinAll && MaxWinAll != 0){
        bigWinner.setVisible(true);
    }
...
/*
    红字大结算
 */
var GameOverLayer_dianTuo = cc.Layer.extend({
    sprite:null,
    jsBind:{
        back:{
            share:{
    ...
    ctor:function () {
        this._super();
        var endallui = ccs.load("endAll_DianTuo.json");
        ...
        if(MjClient.getAppType() == MjClient.APP_TYPE.QXJSMJ && !tData.matchId){
            ...
            // 局数 
            var strRoundNum = (tData.roundAll - tData.roundNumPre + 1) + "/" + tData.roundAll + "局";
        ...
        for(var i = 0 ; i < playerCount ; i++){
            ...
            SetGameOverLayer_dianTuo(clone,i, isNewUi);
